...
| `canvas.width = canvas.clientWidth` — CSS px, **DPR 1** | `src/render/renderer.js:1413‑1064` |
...
| `ensureAudio(fetch)` is exported from `src/scenes/shared.js:490`, is safe un‑awaited, and `audio.ensure` attaches its own gesture‑resume (`src/systems/audio.js:177`, `ensure`) | — |
...
export async function runLauncher(canvas, renderer, status) {
  ...
  // (audio.js:95-168), which is all the volume preview needs.

...
| `canvas.width = canvas.clientWidth` — CSS px, **DPR 1** | `src/render/renderer.js:1433‑1064` |
...
| `ensureAudio(fetch)` is exported from `src/scenes/shared.js:504`, is safe un‑awaited, and `audio.ensure` attaches its own gesture‑resume (`src/systems/audio.js:177`, `ensure`) | — |

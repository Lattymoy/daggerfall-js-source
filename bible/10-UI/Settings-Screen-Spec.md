...
| `ensureAudio(fetch)` is exported from `src/scenes/shared.js:477`, is safe un‑awaited, and `audio.ensure` attaches its own gesture‑resume (`src/systems/audio.js:152`, `ensure`) | — |
...
export async function runLauncher(canvas, renderer, status) {
  ...
  // AUDIT: boot audio HERE. main.js:85 runs this scene before the splash's
...
| The launcher gate (wizard shown when unvalidated OR `ShowOptionsAtStart` OR a held key; skip straight to Options when the path is good) | `SceneControl.cs:46`, wizard `:154` → `main.js:85` |

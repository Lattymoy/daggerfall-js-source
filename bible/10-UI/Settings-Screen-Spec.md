...
* **`Video/FieldOfView` as a live setting.** `Math.PI/3` is hardcoded at `worldModes.js:937`, `world.js:1552` + `:1615`, `interior.js:131`, `exterior.js:1010` + `:1078`, `dungeon.js:356`. Wiring it is worth doing and is a separate commit with its own pin; until then the row is `stored` and operable (its range is DFU law).

...
Node 22). Suite: 287 tests across 72 files.
...
| effects.test.js | 8 | HealHealth (10,8) instant + the verbatim duration arithmetic (base + mod x floor(level/per), per-0 guard), continuous (1,0) joining actives with the ONCE-rolled save percent scaling EVERY per-round magnitude roll, tick + expiry at 0, immune saves never joining, empty lists safe, the S8 buff kinds (slowfall/waterWalking/chameleonNormal keys, incumbent RENEW not stack, duration arithmetic, expiry clearing the query); S13 the SpellPoints/magicka family - HealSpellPoints (10,9) instant IncreaseMagicka, DamageSpellPoints (4,2) instant saving-throw scaled DecreaseMagicka, ContinuousDamageSpellPoints (1,2) joining actives and draining per round; S14 FortifyAttribute (type 9, subType=stat) - a live additive stat mod via liveStat, incumbent renewal not stacking, expiry restoring the base, and a fortified stat raising combat statsToHit through the formulas |

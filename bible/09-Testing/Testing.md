...
| effects.test.js | 8 | HealHealth (10,8) instant + the verbatim duration arithmetic (base + mod x floor(level/per) with the multiplier CLAMPED AT 1, per-0 guard), continuous (1,0) firing its INITIAL round at cast with saves rolled FRESH every round, the DFU expiry shape (a 0-round entry survives its final pass and Ends the next), immune targets joining at zero damage, empty lists safe, the S8 buff kinds (slowfall/waterWalking/chameleonNormal keys, incumbent rounds STACKING on re-cast, expiry clearing the query); S13 the SpellPoints/magicka family - HealSpellPoints (10,9) instant IncreaseMagicka, DamageSpellPoints (4,2) instant saving-throw scaled DecreaseMagicka, ContinuousDamageSpellPoints (1,2) draining from the cast round with per-round saves; S14 FortifyAttribute (type 9, subType=stat) - a live additive stat mod via liveStat, like-kind (stat + settings) stacking rounds, different-settings instances coexisting with mods summing, expiry restoring the base, and a fortified stat raising combat statsToHit through the formulas |

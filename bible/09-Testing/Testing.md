...
Node 22). Suite: 333 tests across 79 files.
...
| motorStairs.test.js | 7 | Unity CharacterController parity (prefab stepOffset 0.5 / slopeLimit 70) by numeric trace: the 0.3-riser staircase summit, riser 0.45 climbs + the 60/78-deg slope-limit pins, the wall-ladder regression stays dead, a stairwell ceiling CAPS the lift (the blind +0.5 needed 2.3 headroom - the live block), a can't-fit tread never gets a grounded stand, the AcrobatMotor jump multiplier (skill 30 -> apex 0.67 not the one-frame 0.069), the 0.1 s grounded bunny-hop gate |

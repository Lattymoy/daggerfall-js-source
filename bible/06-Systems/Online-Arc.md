...
this player's own" (`dungeonContext.js:1147`, AUDIT WORLD B2): a quest
...
`src/scenes/worldModes.js:6180` read, on one physical line:
...
(`dungeonContext.js:4270`). With the property missing that call is a
...
- **A summon or ally cannot kill a peer's foe.** `fromPlayer === false`
  ...
  in it.
- **A foe's blast on a puppet is credited to ME.** `world.js:4183` and
...
**Now (`src/net/wire.js:889`, `src/net/online.js:1567`):**

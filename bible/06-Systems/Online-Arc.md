...
`src/scenes/worldModes.js:6155` read, on one physical line:
...
(`dungeonContext.js:4274`). With the property missing that call is a
...
- **A summon or ally cannot kill a peer's foe.** `fromPlayer === false`
  ...
  in it.
- **A foe's blast on a puppet is credited to ME.** `world.js:4234` and
...
**Now (`src/net/wire.js:896`, `src/net/online.js:1655`):**

...
this player's own" (`dungeonContext.js:1065`, AUDIT WORLD B2): a quest
...
`src/scenes/worldModes.js:5168` read, on one physical line:
...
(`dungeonContext.js:3958`). With the property missing that call is a
...
- **A summon or ally cannot kill a peer's foe.** `fromPlayer === false`
  ...
  in it.
- **A foe's blast on a puppet is credited to ME.** `world.js:3215` and

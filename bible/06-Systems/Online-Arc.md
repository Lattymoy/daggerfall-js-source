...
this player's own" (`dungeonContext.js:1110`, AUDIT WORLD B2): a quest
...
`src/scenes/worldModes.js:5815` read, on one physical line:
...
(`dungeonContext.js:4131`). With the property missing that call is a
...
- **A summon or ally cannot kill a peer's foe.** `fromPlayer === false`
  ...
  in it.
- **A foe's blast on a puppet is credited to ME.** `world.js:3443` and
...
**Now (`src/net/wire.js:723`, `src/net/online.js:1162`):**

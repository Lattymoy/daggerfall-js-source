...
`src/scenes/worldModes.js:5222` read, on one physical line:
...
(`dungeonContext.js:4121`). With the property missing that call is a
...
- **A summon or ally cannot kill a peer's foe.** `fromPlayer === false`
  ...
  in it.
- **A foe's blast on a puppet is credited to ME.** `world.js:3318` and

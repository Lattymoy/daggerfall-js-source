...
`src/scenes/worldModes.js:6735` read, on one physical line:
...
(`dungeonContext.js:4449`). With the property missing that call is a
...
- **A summon or ally cannot kill a peer's foe.** `fromPlayer === false`
  ...
  in it.
- **A foe's blast on a puppet is credited to ME.** `world.js:4874` and
  `:2925` pass `foeSinks: (f) => enchantFoeSinks(f)`, dropping the
  provenance argument `applySpellToFoe` hands them (`hostMagic.js:276`)

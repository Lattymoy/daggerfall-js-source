...
`playerTicker.advance(60)` (`exterior.js:1066`, `world.js:1471`), fired
...
it (`renderer.js:2951`). `colors` was `undefined`, `asBytes` reads
...
leaves `UNPACK_FLIP_Y_WEBGL` off (`renderer.js:2941`), and `BB_VS`
...
`world.js:2681` said the dungeon-mode enchant ctx was "FLAGGED there
...
nobody could sit on. `motor.js:640` passed `riding: false` into the
...
LITERAL FOLLOWS THE ROW IT PINS: `world\.js:5915` in citedrift.test.js

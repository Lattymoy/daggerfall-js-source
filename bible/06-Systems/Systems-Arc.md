...
`playerTicker.advance(60)` (`exterior.js:1065`, `world.js:1445`), fired
...
it (`renderer.js:2851`). `colors` was `undefined`, `asBytes` reads
...
leaves `UNPACK_FLIP_Y_WEBGL` off (`renderer.js:2841`), and `BB_VS`
...
`world.js:2647` said the dungeon-mode enchant ctx was "FLAGGED there
...
nobody could sit on. `motor.js:612` passed `riding: false` into the
...
LITERAL FOLLOWS THE ROW IT PINS: `world\.js:5825` in citedrift.test.js

...
`:417` runs BEFORE the gate. A false sentence in a comment is what
...
`:499-497`) - the DEATH arm included, since DFU's death path sets
...
`:379-379` is two calls inside one sub-tick - `RaiseTime` and then
...
`:449`. The port's tick had already reordered the normalize loop
...
`playerTicker.advance(60)` (`exterior.js:1071`, `world.js:2181`), fired
...
it (`renderer.js:3061`). `colors` was `undefined`, `asBytes` reads
...
leaves `UNPACK_FLIP_Y_WEBGL` off (`renderer.js:3051`), and `BB_VS`
samples the quad's top at v=1 (`renderer.js:369-393`). A browser decode
...
`world.js:3512` said the dungeon-mode enchant ctx was "FLAGGED there
...
LITERAL FOLLOWS THE ROW IT PINS: `world\.js:6825` in citedrift.test.js
...
port has parsed the byte since SAV2 (`saveVarsFile.js:173`, offset

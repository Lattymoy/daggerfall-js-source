...
`playerTicker.advance(60)` (`exterior.js:928`, `world.js:849`), fired
...
it (`renderer.js:1905`). `colors` was `undefined`, `asBytes` reads
...
leaves `UNPACK_FLIP_Y_WEBGL` off (`renderer.js:1895`), and `BB_VS`
samples the quad's top at v=1 (`renderer.js:306-331`). A browser decode
...
`world.js:1701` said the dungeon-mode enchant ctx was "FLAGGED there
...
nobody could sit on. `motor.js:611` passed `riding: false` into the
...
LITERAL FOLLOWS THE ROW IT PINS: `world\.js:3985` in citedrift.test.js

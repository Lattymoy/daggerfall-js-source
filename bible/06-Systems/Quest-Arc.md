...
- WHENNPCISAVAILABLE: the click PULSE - available when the player
  ...
  wired at src/scenes/worldModes.js:530). activeFactionPersons walks NON-COMPLETE quests only -
...
and both hosts mint a fresh one at every click - worldModes.js:1152's
...
- the greeting says the NPC's name once reaction is above zero, and
  "stranger" below it (`townTalk.js:512`). Every static NPC in the
...
already ships, at `worldModes.js:530`. A pending marker over shipped
...
`dungeon.js:260`, the standalone host, has always had the right shape:
...
spawns and the NPC-guard conversion with it (world.js:2861-2950) — and
...
other two engine-owning hosts wire (`world.js:3165-3166`,
`dungeonContext.js:2209-2210`) is wired here now, and with it
`CastSpellDo`'s two world reads — `getClassicSpellEffects` and the
byte-folded `spellHasMatchForClassicEffect` (`world.js:7176-7179`),

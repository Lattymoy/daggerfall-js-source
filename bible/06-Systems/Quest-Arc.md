...
- WHENNPCISAVAILABLE: the click PULSE - available when the player
  ...
  wired at src/scenes/worldModes.js:556). activeFactionPersons walks NON-COMPLETE quests only -
...
and both hosts mint a fresh one at every click - worldModes.js:1171's
...
- the greeting says the NPC's name once reaction is above zero, and
  "stranger" below it (`townTalk.js:539`). Every static NPC in the
...
already ships, at `worldModes.js:556`. A pending marker over shipped
...
`dungeon.js:261`, the standalone host, has always had the right shape:
...
spawns and the NPC-guard conversion with it (world.js:2969-3058) — and
...
other two engine-owning hosts wire (`world.js:3274-3275`,
`dungeonContext.js:2151-2152`) is wired here now, and with it
`CastSpellDo`'s two world reads — `getClassicSpellEffects` and the
byte-folded `spellHasMatchForClassicEffect` (`world.js:7198-7201`),

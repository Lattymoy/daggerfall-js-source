...
- WHENNPCISAVAILABLE: the click PULSE - available when the player
  ...
  wired at src/scenes/worldModes.js:453). activeFactionPersons walks NON-COMPLETE quests only -
...
and both hosts mint a fresh one at every click - worldModes.js:917's
...
already ships, at `worldModes.js:453`. A pending marker over shipped
...
`dungeon.js:220`, the standalone host, has always had the right shape:
...
spawns and the NPC-guard conversion with it (world.js:1923-2007) — and
...
other two engine-owning hosts wire (`world.js:2173-2174`,
`dungeonContext.js:1791-1792`) is wired here now, and with it
`CastSpellDo`'s two world reads — `getClassicSpellEffects` and the
byte-folded `spellHasMatchForClassicEffect` (`world.js:4921-4924`),

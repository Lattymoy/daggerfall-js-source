...
- WHENNPCISAVAILABLE: the click PULSE - available when the player
  ...
  wired at src/scenes/worldModes.js:478). activeFactionPersons walks NON-COMPLETE quests only -
...
and both hosts mint a fresh one at every click - worldModes.js:1058's
...
already ships, at `worldModes.js:478`. A pending marker over shipped
...
spawns and the NPC-guard conversion with it (world.js:2220-2302) — and
...
other two engine-owning hosts wire (`world.js:2468-2469`,
`dungeonContext.js:1982-1983`) is wired here now, and with it
`CastSpellDo`'s two world reads — `getClassicSpellEffects` and the
byte-folded `spellHasMatchForClassicEffect` (`world.js:5537-5540`),
absent which the action self-completes at *parse*
(`actions.js:2751`/`:2758`) and the task can never arm at all.

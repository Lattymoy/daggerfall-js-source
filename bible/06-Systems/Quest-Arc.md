...
- WHENNPCISAVAILABLE: the click PULSE - available when the player
  ...
  wired at src/scenes/worldModes.js:469). activeFactionPersons walks NON-COMPLETE quests only -
...
and both hosts mint a fresh one at every click - worldModes.js:986's
...
already ships, at `worldModes.js:469`. A pending marker over shipped
...
`dungeon.js:239`, the standalone host, has always had the right shape:
...
spawns and the NPC-guard conversion with it (world.js:1998-2082) — and
...
other two engine-owning hosts wire (`world.js:2248-2249`,
`dungeonContext.js:1835-1836`) is wired here now, and with it
`CastSpellDo`'s two world reads — `getClassicSpellEffects` and the
byte-folded `spellHasMatchForClassicEffect` (`world.js:5150-5153`),

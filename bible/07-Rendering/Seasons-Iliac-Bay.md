...
uploads as-is with `UNPACK_FLIP_Y_WEBGL` off (`renderer.js:2389`), and
`BB_VS` samples the quad's TOP at v=1, i.e. the LAST row
(`renderer.js:409-411`). The seasonal record arrived in PNG raster
...
`getImageData` - and `world.js:1277` / `exterior.js:832` handed that

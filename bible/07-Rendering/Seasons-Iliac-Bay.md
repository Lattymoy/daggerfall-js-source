...
uploads as-is with `UNPACK_FLIP_Y_WEBGL` off (`renderer.js:3051`), and
`BB_VS` samples the quad's TOP at v=1, i.e. the LAST row
(`renderer.js:391-393`). The seasonal record arrived in PNG raster
...
`getImageData` - and `world.js:2747` / `exterior.js:958` handed that

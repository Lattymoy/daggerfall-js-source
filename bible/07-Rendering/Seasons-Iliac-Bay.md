...
uploads as-is with `UNPACK_FLIP_Y_WEBGL` off (`renderer.js:1886`), and
`BB_VS` samples the quad's TOP at v=1, i.e. the LAST row
(`renderer.js:324-327`). The seasonal record arrived in PNG raster
...
`getImageData` - and `world.js:1233` / `exterior.js:812` handed that

...
uploads as-is with `UNPACK_FLIP_Y_WEBGL` off (`renderer.js:1857`), and
`BB_VS` samples the quad's TOP at v=1, i.e. the LAST row
(`renderer.js:323-326`). The seasonal record arrived in PNG raster
...
`getImageData` - and `world.js:1223` / `exterior.js:803` handed that

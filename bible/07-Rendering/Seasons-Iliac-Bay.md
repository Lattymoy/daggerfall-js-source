...
uploads as-is with `UNPACK_FLIP_Y_WEBGL` off (`renderer.js:2172`), and
`BB_VS` samples the quad's TOP at v=1, i.e. the LAST row
(`renderer.js:355-357`). The seasonal record arrived in PNG raster
...
`getImageData` - and `world.js:1257` / `exterior.js:821` handed that

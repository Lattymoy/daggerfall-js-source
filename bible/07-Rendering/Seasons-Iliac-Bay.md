...
uploads as-is with `UNPACK_FLIP_Y_WEBGL` off (`renderer.js:3027`), and
`BB_VS` samples the quad's TOP at v=1, i.e. the LAST row
(`renderer.js:390-392`). The seasonal record arrived in PNG raster
...
`getImageData` - and `world.js:2084` / `exterior.js:954` handed that

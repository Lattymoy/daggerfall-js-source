...
### TR6 - the integration, rebuilt (2026-09-03, Mac: "add the trees back from the revert and properly integrate them with no rushing")

The trees came back from the revert and were read against the host's
own draw loop before a line moved. Four faults in TR1's integration,
each of which alone hid every tree in the game, and none of which TR4's
probe could see because its harness was not the host:

1. **The flat's size is an object.** `scaledBillboardSize` answers
   `{w, h}`; the host read `size[1]` - undefined - so every base was
   NaN and the scale was NaN. No tree ever had a position. And the
   base was lowered by half a height the position did not carry:
   `flatBatchAabb` spans `minY..minY + h`, the billboard STANDS on its
   stored position, so the mesh stands on the same point as it is.
2. **The instances were pixel-local, drawn as world.** The streaming
   world keeps a pixel's content in the pixel's frame and hands the flat
   batches the pixel's translation each frame (`b.origin = t`, under
   the floating origin's compensation). The tree shader took the
   instance as world, so every pixel's trees stood on the ORIGIN pixel
   and moved with each recentre. The batches are collected in the pixel
   walk exactly as the flats are - culled by the flat's EV3 box, given
   `origin` - and the shader adds it. The gust's wave rides the pixel's
   GRID translation without the compensation, world-fixed, so a
   recentre never twitches a crown and a gust crosses a pixel border as
   one wave.
3. **One batch per record, for the whole world.** `build()` keyed on
   ...
   pixel's trees with its own, and a pixel leaving disposed the key and
   took every other pixel's trees of that record with it. The GEOMETRY
   is per record now and shared (one side buffer, one crown-top raster
   and upload, one height match); the INSTANCES are per pixel, keyed
   `${pixelKey}|${archive}_${record}`, and `disposePixel` frees a
   pixel's own and nothing else's. A record's texture binds once per
   frame however many pixels stand it.
4. **The pixel buffers wore the wrong names.** `getColor32` answers
   `{colors, width, height}`; the crown-top read `.data` (every alpha
   0, an empty raster) and returned `.data`, which `uploadTexture`
   (reading `.colors`) could not upload, so the try/catch dropped every
   top in silence. Both ends speak `colors`, the pipeline's shape; the
   old shape is refused rather than rendered empty. The probe's own
   stand-in had worn `.data` too, which is why it saw tops.

Also: a tree is drawn in the world's depth (test and write on, blend
off) whatever the pass before left; the flat's box joins the pixel's
bounds when the flat becomes a tree, so a pixel whose edge holds only
trees is not culled away (EV3); the wind is the one hoisted object the
grass and the trees both read, as TR1 had it, with WX2's dim riding the
front beside it. The classic path and the enhanced-environments switch
are as TR1 left them; `?trees=off` stands.

Pinned, 24 tests: the shared geometry and the per-pixel instances through
a recording GL stand-in (two pixels, one buffer, one top upload; a pixel
leaving frees its instances alone; a batch without an origin does not
draw; one texture bind for two pixels), the shader's `uOrigin` and
world-fixed gust, the host's collection beside the flats with the flat's
box and the grid translation, `size.h` and the centres as bases with the
old reads pinned out, and the pixel shape both ways with the upload law
and getColor32's return held beside them.

**Still not seen: the trees in the game.** No ARENA2 here and the
partner's pack is not in the tree, so neither the walk nor the probe ran.
The four faults above are the ones the code could show; the walk is
Mac's. ROADS 25a landed beside this because the boot itself failed
first: the roadless first pixel returned nothing to the camera.

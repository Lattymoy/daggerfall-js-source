...
800 lines from the host's `onPlayerArrowHitFoe` (`world.js:12674`); all
...
`worldModes.js:8683`, `dungeonContext.js:6513`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:7205` and
`chargenSession.js`'s `worldModes.js:8796` are bumped and pinned. Four

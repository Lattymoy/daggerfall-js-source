...
dungeonContext.js:1680 by tools/fistProbe.mjs), the melee tally on
...
and its `world.js:7504` left naming a `WorldTime`/`PauseWhileOpen` note
800 lines from the host's `onPlayerArrowHitFoe` (`world.js:8475`); all
...
`worldModes.js:7183`, `dungeonContext.js:5181`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:5924` and
`chargenSession.js`'s `worldModes.js:7296` are bumped and pinned. Four

...
`worldModes.js:9476`, `dungeonContext.js:6833`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:7991` and
`chargenSession.js`'s `worldModes.js:9589` are bumped and pinned. Four

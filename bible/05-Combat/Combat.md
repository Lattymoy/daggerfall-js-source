...
dungeonContext.js:1655 by tools/fistProbe.mjs), the melee tally on
...
and its `world.js:7239` left naming a `WorldTime`/`PauseWhileOpen` note
800 lines from the host's `onPlayerArrowHitFoe` (`world.js:8189`); all
...
`worldModes.js:7146`, `dungeonContext.js:4907`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:5888` and
`chargenSession.js`'s `worldModes.js:7259` are bumped and pinned. Four

...
and its `world.js:13461` left naming a `WorldTime`/`PauseWhileOpen` note
800 lines from the host's `onPlayerArrowHitFoe` (`world.js:15025`); all
...
`worldModes.js:8954`, `dungeonContext.js:6645`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:7470` and
`chargenSession.js`'s `worldModes.js:9067` are bumped and pinned. Four

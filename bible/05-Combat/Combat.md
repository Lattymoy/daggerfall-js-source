...
dungeonContext.js:2014 by tools/fistProbe.mjs), the melee tally on
...
and its `world.js:13536` left naming a `WorldTime`/`PauseWhileOpen` note
800 lines from the host's `onPlayerArrowHitFoe` (`world.js:15100`); all
...
`worldModes.js:8981`, `dungeonContext.js:6728`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:7497` and
`chargenSession.js`'s `worldModes.js:9094` are bumped and pinned. Four

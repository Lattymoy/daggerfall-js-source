...
`worldModes.js:8620`, `dungeonContext.js:6454`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:7164` and
`chargenSession.js`'s `worldModes.js:8733` are bumped and pinned. Four

...
dungeonContext.js:1930 by tools/fistProbe.mjs), the melee tally on
...
and its `world.js:10192` left naming a `WorldTime`/`PauseWhileOpen` note
800 lines from the host's `onPlayerArrowHitFoe` (`world.js:11619`); all
...
wrong. All three are resolved by content (`townTalk.js:1203`,
`worldModes.js:8433`, `dungeonContext.js:6398`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:6987` and
`chargenSession.js`'s `worldModes.js:8550` are bumped and pinned. Four

...
and its `world.js:9323` left naming a `WorldTime`/`PauseWhileOpen` note
800 lines from the host's `onPlayerArrowHitFoe` (`world.js:10534`); all
...
`worldModes.js:7556`, `dungeonContext.js:5886`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:6211` and
`chargenSession.js`'s `worldModes.js:7669` are bumped and pinned. Four

...
and its `world.js:9506` left naming a `WorldTime`/`PauseWhileOpen` note
800 lines from the host's `onPlayerArrowHitFoe` (`world.js:10745`); all
...
`worldModes.js:7590`, `dungeonContext.js:5909`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:6220` and
`chargenSession.js`'s `worldModes.js:7703` are bumped and pinned. Four

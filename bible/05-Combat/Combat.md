...
dungeonContext.js:1865 by tools/fistProbe.mjs), the melee tally on
...
and its `world.js:9798` left naming a `WorldTime`/`PauseWhileOpen` note
800 lines from the host's `onPlayerArrowHitFoe` (`world.js:11188`); all
...
`worldModes.js:7708`, `dungeonContext.js:6115`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:6326` and
`chargenSession.js`'s `worldModes.js:7821` are bumped and pinned. Four

...
and its `world.js:10163` left naming a `WorldTime`/`PauseWhileOpen` note
800 lines from the host's `onPlayerArrowHitFoe` (`world.js:11592`); all
...
`worldModes.js:8578`, `dungeonContext.js:6432`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:7122` and
`chargenSession.js`'s `worldModes.js:8691` are bumped and pinned. Four

...
dungeonContext.js:1915 by tools/fistProbe.mjs), the melee tally on
...
and its `world.js:9850` left naming a `WorldTime`/`PauseWhileOpen` note
800 lines from the host's `onPlayerArrowHitFoe` (`world.js:11218`); all
...
`worldModes.js:7716`, `dungeonContext.js:6175`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:6334` and

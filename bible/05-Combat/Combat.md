...
dungeonContext.js:1956 by tools/fistProbe.mjs), the melee tally on
...
and its `world.js:10165` left naming a `WorldTime`/`PauseWhileOpen` note
800 lines from the host's `onPlayerArrowHitFoe` (`world.js:11594`); all
...
`worldModes.js:8576`, `dungeonContext.js:6429`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:7122` and
`chargenSession.js`'s `worldModes.js:8689` are bumped and pinned. Four

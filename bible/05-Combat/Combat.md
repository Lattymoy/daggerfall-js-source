...
and its `world.js:11689` left naming a `WorldTime`/`PauseWhileOpen` note
800 lines from the host's `onPlayerArrowHitFoe` (`world.js:13176`); all
...
`worldModes.js:8760`, `dungeonContext.js:6571`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:7258` and
`chargenSession.js`'s `worldModes.js:8873` are bumped and pinned. Four

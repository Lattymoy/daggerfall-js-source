...
and its `world.js:12361` left naming a `WorldTime`/`PauseWhileOpen` note
800 lines from the host's `onPlayerArrowHitFoe` (`world.js:13876`); all
...
`worldModes.js:8767`, `dungeonContext.js:6574`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:7265` and
`chargenSession.js`'s `worldModes.js:8880` are bumped and pinned. Four

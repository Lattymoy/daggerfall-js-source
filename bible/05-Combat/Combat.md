...
and its `world.js:12434` left naming a `WorldTime`/`PauseWhileOpen` note
800 lines from the host's `onPlayerArrowHitFoe` (`world.js:13957`); all
...
`worldModes.js:8855`, `dungeonContext.js:6588`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:7346` and
`chargenSession.js`'s `worldModes.js:8968` are bumped and pinned. Four

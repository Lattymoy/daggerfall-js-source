...
dungeonContext.js:1772 by tools/fistProbe.mjs), the melee tally on
...
and its `world.js:8717` left naming a `WorldTime`/`PauseWhileOpen` note
800 lines from the host's `onPlayerArrowHitFoe` (`world.js:9848`); all
...
`worldModes.js:7458`, `dungeonContext.js:5804`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:6125` and
`chargenSession.js`'s `worldModes.js:7571` are bumped and pinned. Four

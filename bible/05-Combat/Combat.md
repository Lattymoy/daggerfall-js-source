...
and its `world.js:11241` left naming a `WorldTime`/`PauseWhileOpen` note
800 lines from the host's `onPlayerArrowHitFoe` (`world.js:12676`); all
...
`worldModes.js:8689`, `dungeonContext.js:6538`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:7188` and
`chargenSession.js`'s `worldModes.js:8802` are bumped and pinned. Four

...
and its `world.js:13433` left naming a `WorldTime`/`PauseWhileOpen` note
800 lines from the host's `onPlayerArrowHitFoe` (`world.js:14994`); all
...
`worldModes.js:9178`, `dungeonContext.js:6685`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:7694` and
`chargenSession.js`'s `worldModes.js:9291` are bumped and pinned. Four

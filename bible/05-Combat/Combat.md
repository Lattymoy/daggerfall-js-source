...
and its `world.js:7884` left naming a `WorldTime`/`PauseWhileOpen` note
800 lines from the host's `onPlayerArrowHitFoe` (`world.js:8900`); all
...
`worldModes.js:7363`, `dungeonContext.js:5726`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:6066` and
`chargenSession.js`'s `worldModes.js:7476` are bumped and pinned. Four

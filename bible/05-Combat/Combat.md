...
and its `world.js:12843` left naming a `WorldTime`/`PauseWhileOpen` note
800 lines from the host's `onPlayerArrowHitFoe` (`world.js:14388`); all
...
`worldModes.js:8877`, `dungeonContext.js:6608`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:7367` and
`chargenSession.js`'s `worldModes.js:8990` are bumped and pinned. Four

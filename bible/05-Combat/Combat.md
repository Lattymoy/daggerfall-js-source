...
and its `world.js:11478` left naming a `WorldTime`/`PauseWhileOpen` note
800 lines from the host's `onPlayerArrowHitFoe` (`world.js:12925`); all
...
`worldModes.js:8731`, `dungeonContext.js:6545`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:7230` and
`chargenSession.js`'s `worldModes.js:8844` are bumped and pinned. Four

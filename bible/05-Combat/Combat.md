...
and its `world.js:11509` left naming a `WorldTime`/`PauseWhileOpen` note
800 lines from the host's `onPlayerArrowHitFoe` (`world.js:12954`); all
...
`worldModes.js:8733`, `dungeonContext.js:6549`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:7232` and
`chargenSession.js`'s `worldModes.js:8846` are bumped and pinned. Four

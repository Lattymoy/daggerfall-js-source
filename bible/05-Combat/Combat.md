...
dungeonContext.js:1929 by tools/fistProbe.mjs), the melee tally on
...
and its `world.js:10090` left naming a `WorldTime`/`PauseWhileOpen` note
800 lines from the host's `onPlayerArrowHitFoe` (`world.js:11494`); all
...
wrong. All three are resolved by content (`townTalk.js:1196`,
`worldModes.js:8413`, `dungeonContext.js:6397`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:6977` and
`chargenSession.js`'s `worldModes.js:8526` are bumped and pinned. Four

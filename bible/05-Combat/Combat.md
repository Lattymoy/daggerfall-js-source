...
dungeonContext.js:2016 by tools/fistProbe.mjs), the melee tally on
...
(`arrowFlight.js:295` is a `backstabChance:` field; the unconditional
...
and its `world.js:12951` left naming a `WorldTime`/`PauseWhileOpen` note
800 lines from the host's `onPlayerArrowHitFoe` (`world.js:14508`); all
...
`worldModes.js:8884`, `dungeonContext.js:6611`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:7374` and
`chargenSession.js`'s `worldModes.js:8997` are bumped and pinned. Four

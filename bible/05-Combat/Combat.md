...
800 lines from the host's `onPlayerArrowHitFoe` (`world.js:8770`); all
...
`worldModes.js:7314`, `dungeonContext.js:5654`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:6049` and
`chargenSession.js`'s `worldModes.js:7427` are bumped and pinned. Four

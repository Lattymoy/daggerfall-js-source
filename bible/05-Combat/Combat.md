...
`worldModes.js:7095`, `dungeonContext.js:4884`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:5837` and
`chargenSession.js`'s `worldModes.js:7193` are bumped and pinned. Four

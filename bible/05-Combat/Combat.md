...
`worldModes.js:7442`, `dungeonContext.js:5782`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:6116` and
`chargenSession.js`'s `worldModes.js:7555` are bumped and pinned. Four

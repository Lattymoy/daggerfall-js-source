...
dungeonContext.js:1926 by tools/fistProbe.mjs), the melee tally on
...
and its `world.js:9941` left naming a `WorldTime`/`PauseWhileOpen` note
800 lines from the host's `onPlayerArrowHitFoe` (`world.js:11347`); all
...
`worldModes.js:8244`, `dungeonContext.js:6314`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:6824` and
`chargenSession.js`'s `worldModes.js:8357` are bumped and pinned. Four

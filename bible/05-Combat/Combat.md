...
dungeonContext.js:1720 by tools/fistProbe.mjs), the melee tally on
...
and its `world.js:7727` left naming a `WorldTime`/`PauseWhileOpen` note
800 lines from the host's `onPlayerArrowHitFoe` (`world.js:8743`); all
...
`worldModes.js:7345`, `dungeonContext.js:5702`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:6066` and
`chargenSession.js`'s `worldModes.js:7458` are bumped and pinned. Four

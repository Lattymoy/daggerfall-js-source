...
and its `world.js:11389` left naming a `WorldTime`/`PauseWhileOpen` note
800 lines from the host's `onPlayerArrowHitFoe` (`world.js:12858`); all
...
`worldModes.js:8717`, `dungeonContext.js:6544`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:7216` and
`chargenSession.js`'s `worldModes.js:8830` are bumped and pinned. Four

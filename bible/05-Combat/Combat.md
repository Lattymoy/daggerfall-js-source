...
and its `world.js:11665` left naming a `WorldTime`/`PauseWhileOpen` note
800 lines from the host's `onPlayerArrowHitFoe` (`world.js:13144`); all
...
`worldModes.js:8736`, `dungeonContext.js:6563`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:7234` and
`chargenSession.js`'s `worldModes.js:8849` are bumped and pinned. Four

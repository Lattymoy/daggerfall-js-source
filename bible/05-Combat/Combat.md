...
and its `world.js:10628` left naming a `WorldTime`/`PauseWhileOpen` note
800 lines from the host's `onPlayerArrowHitFoe` (`world.js:12063`); all
...
`worldModes.js:8606`, `dungeonContext.js:6459`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:7149` and
`chargenSession.js`'s `worldModes.js:8719` are bumped and pinned. Four

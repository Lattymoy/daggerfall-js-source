...
and its `world.js:9116` left naming a `WorldTime`/`PauseWhileOpen` note
800 lines from the host's `onPlayerArrowHitFoe` (`world.js:10284`); all
...
`worldModes.js:7492`, `dungeonContext.js:5872`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:6153` and
`chargenSession.js`'s `worldModes.js:7605` are bumped and pinned. Four

...
dungeonContext.js:1992 by tools/fistProbe.mjs), the melee tally on
...
`totalWeight` (inventory.js:332) IS `ItemCollection.GetWeight`, so the
...
(`arrowFlight.js:294` is a `backstabChance:` field; the unconditional
...
and its `world.js:12753` left naming a `WorldTime`/`PauseWhileOpen` note
800 lines from the host's `onPlayerArrowHitFoe` (`world.js:14299`); all
...
wrong. All three are resolved by content (`townTalk.js:1236`,
`worldModes.js:8873`, `dungeonContext.js:6597`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:7364` and
`chargenSession.js`'s `worldModes.js:8986` are bumped and pinned. Four

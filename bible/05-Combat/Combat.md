...
dungeonContext.js:1718 by tools/fistProbe.mjs), the melee tally on
...
and its `world.js:7634` left naming a `WorldTime`/`PauseWhileOpen` note
800 lines from the host's `onPlayerArrowHitFoe` (`world.js:8605`); all
...
`worldModes.js:7274`, `dungeonContext.js:5541`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:6015` and
`chargenSession.js`'s `worldModes.js:7387` are bumped and pinned. Four

...
dungeonContext.js:1997 by tools/fistProbe.mjs), the melee tally on
...
and its `world.js:11743` left naming a `WorldTime`/`PauseWhileOpen` note
800 lines from the host's `onPlayerArrowHitFoe` (`world.js:13231`); all
...
`worldModes.js:8780`, `dungeonContext.js:6577`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:7271` and
`chargenSession.js`'s `worldModes.js:8893` are bumped and pinned. Four

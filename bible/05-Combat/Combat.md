...
`worldModes.js:8868`, `dungeonContext.js:6592`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:7359` and
`chargenSession.js`'s `worldModes.js:8981` are bumped and pinned. Four

...
`worldModes.js:8861`, `dungeonContext.js:6604`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:7381` and
`chargenSession.js`'s `worldModes.js:8974` are bumped and pinned. Four

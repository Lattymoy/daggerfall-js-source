...
and its `world.js:7836` left naming a `WorldTime`/`PauseWhileOpen` note
800 lines from the host's `onPlayerArrowHitFoe` (`world.js:8853`); all
...
`worldModes.js:7367`, `dungeonContext.js:5726`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:6070` and
`chargenSession.js`'s `worldModes.js:7480` are bumped and pinned. Four

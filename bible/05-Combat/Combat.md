...
and its `world.js:7967` left naming a `WorldTime`/`PauseWhileOpen` note
800 lines from the host's `onPlayerArrowHitFoe` (`world.js:8992`); all
...
`worldModes.js:7369`, `dungeonContext.js:5726`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:6072` and
`chargenSession.js`'s `worldModes.js:7482` are bumped and pinned. Four

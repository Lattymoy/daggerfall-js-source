...
dungeonContext.js:1965 by tools/fistProbe.mjs), the melee tally on
...
and its `world.js:10650` left naming a `WorldTime`/`PauseWhileOpen` note
800 lines from the host's `onPlayerArrowHitFoe` (`world.js:12085`); all
...
`worldModes.js:8615`, `dungeonContext.js:6468`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:7154` and
`chargenSession.js`'s `worldModes.js:8728` are bumped and pinned. Four

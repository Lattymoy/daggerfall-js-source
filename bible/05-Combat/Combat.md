...
and its `world.js:11724` left naming a `WorldTime`/`PauseWhileOpen` note
800 lines from the host's `onPlayerArrowHitFoe` (`world.js:13210`); all
...
`worldModes.js:8744`, `dungeonContext.js:6564`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:7241` and
`chargenSession.js`'s `worldModes.js:8857` are bumped and pinned. Four

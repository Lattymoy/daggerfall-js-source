...
dungeonContext.js:1783 by tools/fistProbe.mjs), the melee tally on
...
and its `world.js:8664` left naming a `WorldTime`/`PauseWhileOpen` note
800 lines from the host's `onPlayerArrowHitFoe` (`world.js:9736`); all
...
`worldModes.js:7476`, `dungeonContext.js:5864`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:6137` and
`chargenSession.js`'s `worldModes.js:7589` are bumped and pinned. Four

...
dungeonContext.js:1989 by tools/fistProbe.mjs), the melee tally on
...
and its `world.js:11561` left naming a `WorldTime`/`PauseWhileOpen` note
800 lines from the host's `onPlayerArrowHitFoe` (`world.js:13030`); all
...
wrong. All three are resolved by content (`townTalk.js:1234`,
`worldModes.js:8720`, `dungeonContext.js:6558`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:7218` and
`chargenSession.js`'s `worldModes.js:8833` are bumped and pinned. Four

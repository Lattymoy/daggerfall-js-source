...
and its `world.js:8368` left naming a `WorldTime`/`PauseWhileOpen` note
800 lines from the host's `onPlayerArrowHitFoe` (`world.js:9407`); all
...
`worldModes.js:7453`, `dungeonContext.js:5804`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:6120` and
`chargenSession.js`'s `worldModes.js:7566` are bumped and pinned. Four

...
and its `world.js:9990` left naming a `WorldTime`/`PauseWhileOpen` note
800 lines from the host's `onPlayerArrowHitFoe` (`world.js:11396`); all
...
`worldModes.js:8335`, `dungeonContext.js:6363`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:6915` and
`chargenSession.js`'s `worldModes.js:8448` are bumped and pinned. Four

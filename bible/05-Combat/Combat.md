...
dungeonContext.js:1722 by tools/fistProbe.mjs), the melee tally on
...
and its `world.js:8255` left naming a `WorldTime`/`PauseWhileOpen` note
800 lines from the host's `onPlayerArrowHitFoe` (`world.js:9298`); all
...
`worldModes.js:7415`, `dungeonContext.js:5741`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:6113` and
`chargenSession.js`'s `worldModes.js:7528` are bumped and pinned. Four

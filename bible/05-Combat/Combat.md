...
dungeonContext.js:1868 by tools/fistProbe.mjs), the melee tally on
...
`worldModes.js:7714`, `dungeonContext.js:6128`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:6327` and
`chargenSession.js`'s `worldModes.js:7827` are bumped and pinned. Four

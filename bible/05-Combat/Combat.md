...
and its `world.js:8750` left naming a `WorldTime`/`PauseWhileOpen` note
800 lines from the host's `onPlayerArrowHitFoe` (`world.js:9822`); all
...
`worldModes.js:7501`, `dungeonContext.js:5872`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:6162` and
`chargenSession.js`'s `worldModes.js:7614` are bumped and pinned. Four

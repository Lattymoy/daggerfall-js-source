...
dungeonContext.js:1773 by tools/fistProbe.mjs), the melee tally on
...
and its `world.js:8579` left naming a `WorldTime`/`PauseWhileOpen` note
800 lines from the host's `onPlayerArrowHitFoe` (`world.js:9641`); all
...
wrong. All three are resolved by content (`townTalk.js:1154`,
`worldModes.js:7478`, `dungeonContext.js:5805`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:6142` and
`chargenSession.js`'s `worldModes.js:7591` are bumped and pinned. Four

...
and its `world.js:14691` left naming a `WorldTime`/`PauseWhileOpen` note
800 lines from the host's `onPlayerArrowHitFoe` (`world.js:16389`); all
...
`worldModes.js:9482`, `dungeonContext.js:6842`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:7997` and
`chargenSession.js`'s `worldModes.js:9595` are bumped and pinned. Four

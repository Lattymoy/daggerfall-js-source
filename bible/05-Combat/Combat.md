...
and its `world.js:11238` left naming a `WorldTime`/`PauseWhileOpen` note
800 lines from the host's `onPlayerArrowHitFoe` (`world.js:12673`); all
...
`worldModes.js:8686`, `dungeonContext.js:6513`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:7208` and
`chargenSession.js`'s `worldModes.js:8799` are bumped and pinned. Four

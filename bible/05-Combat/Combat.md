...
dungeonContext.js:1701 by tools/fistProbe.mjs), the melee tally on
...
and its `world.js:7668` left naming a `WorldTime`/`PauseWhileOpen` note
800 lines from the host's `onPlayerArrowHitFoe` (`world.js:8650`); all
...
`worldModes.js:7299`, `dungeonContext.js:5630`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:6034` and
`chargenSession.js`'s `worldModes.js:7412` are bumped and pinned. Four

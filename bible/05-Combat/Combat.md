...
and its `world.js:12675` left naming a `WorldTime`/`PauseWhileOpen` note
800 lines from the host's `onPlayerArrowHitFoe` (`world.js:14190`); all
...
`worldModes.js:8768`, `dungeonContext.js:6565`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:7266` and
`chargenSession.js`'s `worldModes.js:8881` are bumped and pinned. Four

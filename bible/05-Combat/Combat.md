...
and its `world.js:13390` left naming a `WorldTime`/`PauseWhileOpen` note
800 lines from the host's `onPlayerArrowHitFoe` (`world.js:14948`); all
...
`worldModes.js:8901`, `dungeonContext.js:6683`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:7417` and
`chargenSession.js`'s `worldModes.js:9014` are bumped and pinned. Four

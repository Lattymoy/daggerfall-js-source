...
and its `world.js:8211` left naming a `WorldTime`/`PauseWhileOpen` note
800 lines from the host's `onPlayerArrowHitFoe` (`world.js:9254`); all
...
`worldModes.js:7402`, `dungeonContext.js:5728`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:6100` and
`chargenSession.js`'s `worldModes.js:7515` are bumped and pinned. Four

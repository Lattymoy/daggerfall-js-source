...
and its `world.js:8262` left naming a `WorldTime`/`PauseWhileOpen` note
800 lines from the host's `onPlayerArrowHitFoe` (`world.js:9305`); all
...
`worldModes.js:7417`, `dungeonContext.js:5741`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:6115` and
`chargenSession.js`'s `worldModes.js:7530` are bumped and pinned. Four

...
dungeonContext.js:1707 by tools/fistProbe.mjs), the melee tally on
...
and its `world.js:7572` left naming a `WorldTime`/`PauseWhileOpen` note
800 lines from the host's `onPlayerArrowHitFoe` (`world.js:8543`); all
...
`worldModes.js:7185`, `dungeonContext.js:5529`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:5926` and
`chargenSession.js`'s `worldModes.js:7298` are bumped and pinned. Four

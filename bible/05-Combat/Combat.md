...
dungeonContext.js:2007 by tools/fistProbe.mjs), the melee tally on
...
and its `world.js:12922` left naming a `WorldTime`/`PauseWhileOpen` note
800 lines from the host's `onPlayerArrowHitFoe` (`world.js:14478`); all
...
`worldModes.js:8877`, `dungeonContext.js:6624`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:7396` and

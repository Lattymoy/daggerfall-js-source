...
and its `world.js:10048` left naming a `WorldTime`/`PauseWhileOpen` note
800 lines from the host's `onPlayerArrowHitFoe` (`world.js:11454`); all
...
`worldModes.js:8400`, `dungeonContext.js:6383`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:6975` and
`chargenSession.js`'s `worldModes.js:8513` are bumped and pinned. Four

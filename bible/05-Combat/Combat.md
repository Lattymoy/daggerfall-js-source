...
and its `world.js:13280` left naming a `WorldTime`/`PauseWhileOpen` note
800 lines from the host's `onPlayerArrowHitFoe` (`world.js:14837`); all
...
`worldModes.js:8903`, `dungeonContext.js:6613`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:7392` and
`chargenSession.js`'s `worldModes.js:9016` are bumped and pinned. Four

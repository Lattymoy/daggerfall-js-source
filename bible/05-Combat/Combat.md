...
`worldModes.js:8684`, `dungeonContext.js:6513`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:7206` and
`chargenSession.js`'s `worldModes.js:8797` are bumped and pinned. Four

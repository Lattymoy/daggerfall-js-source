...
and its `world.js:14887` left naming a `WorldTime`/`PauseWhileOpen` note
800 lines from the host's `onPlayerArrowHitFoe` (`world.js:16585`); all
...
`worldModes.js:9526`, `dungeonContext.js:6829`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:8041` and
`chargenSession.js`'s `worldModes.js:9639` are bumped and pinned. Four

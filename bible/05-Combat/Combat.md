...
dungeonContext.js:1668 by tools/fistProbe.mjs), the melee tally on
...
and its `world.js:7406` left naming a `WorldTime`/`PauseWhileOpen` note
800 lines from the host's `onPlayerArrowHitFoe` (`world.js:8377`); all
...
`worldModes.js:7162`, `dungeonContext.js:4924`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:5903` and
`chargenSession.js`'s `worldModes.js:7275` are bumped and pinned. Four

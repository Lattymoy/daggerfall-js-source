...
and its `world.js:13435` left naming a `WorldTime`/`PauseWhileOpen` note
800 lines from the host's `onPlayerArrowHitFoe` (`world.js:14996`); all
...
`worldModes.js:9347`, `dungeonContext.js:6685`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:7862` and
`chargenSession.js`'s `worldModes.js:9460` are bumped and pinned. Four

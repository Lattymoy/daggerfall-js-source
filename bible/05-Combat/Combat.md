...
and its `world.js:12791` left naming a `WorldTime`/`PauseWhileOpen` note
800 lines from the host's `onPlayerArrowHitFoe` (`world.js:14335`); all
...
`worldModes.js:8867`, `dungeonContext.js:6591`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:7358` and
`chargenSession.js`'s `worldModes.js:8980` are bumped and pinned. Four

...
and its `world.js:12721` left naming a `WorldTime`/`PauseWhileOpen` note
800 lines from the host's `onPlayerArrowHitFoe` (`world.js:14242`); all
...
`worldModes.js:8819`, `dungeonContext.js:6565`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:7317` and
`chargenSession.js`'s `worldModes.js:8932` are bumped and pinned. Four

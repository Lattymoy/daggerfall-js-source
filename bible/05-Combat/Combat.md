...
dungeonContext.js:1998 by tools/fistProbe.mjs), the melee tally on
...
and its `world.js:12448` left naming a `WorldTime`/`PauseWhileOpen` note
800 lines from the host's `onPlayerArrowHitFoe` (`world.js:13984`); all
...
`worldModes.js:8859`, `dungeonContext.js:6588`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:7350` and
`chargenSession.js`'s `worldModes.js:8972` are bumped and pinned. Four

...
and its `world.js:8212` left naming a `WorldTime`/`PauseWhileOpen` note
...
`worldModes.js:7403`, `dungeonContext.js:5728`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:6101` and
`chargenSession.js`'s `worldModes.js:7516` are bumped and pinned. Four

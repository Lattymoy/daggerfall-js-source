...
and its `world.js:10233` left naming a `WorldTime`/`PauseWhileOpen` note
800 lines from the host's `onPlayerArrowHitFoe` (`world.js:11662`); all
...
`worldModes.js:8619`, `dungeonContext.js:6455`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:7141` and
`chargenSession.js`'s `worldModes.js:8732` are bumped and pinned. Four

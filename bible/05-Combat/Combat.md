...
and its `world.js:7122` left naming a `WorldTime`/`PauseWhileOpen` note
800 lines from the host's `onPlayerArrowHitFoe` (`world.js:8013`); all
...
`worldModes.js:7079`, `dungeonContext.js:4867`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:5824` and
`chargenSession.js`'s `worldModes.js:7177` are bumped and pinned. Four

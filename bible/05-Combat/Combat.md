...
and its `world.js:9957` left naming a `WorldTime`/`PauseWhileOpen` note
800 lines from the host's `onPlayerArrowHitFoe` (`world.js:11363`); all
...
`worldModes.js:8264`, `dungeonContext.js:6334`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:6844` and
`chargenSession.js`'s `worldModes.js:8377` are bumped and pinned. Four

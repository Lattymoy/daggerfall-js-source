...
and its `world.js:9657` left naming a `WorldTime`/`PauseWhileOpen` note
800 lines from the host's `onPlayerArrowHitFoe` (`world.js:11002`); all
...
`worldModes.js:7639`, `dungeonContext.js:5935`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:6267` and
`chargenSession.js`'s `worldModes.js:7752` are bumped and pinned. Four

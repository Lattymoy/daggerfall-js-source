...
dungeonContext.js:1923 by tools/fistProbe.mjs), the melee tally on
...
and its `world.js:9799` left naming a `WorldTime`/`PauseWhileOpen` note
800 lines from the host's `onPlayerArrowHitFoe` (`world.js:11202`); all
...
`worldModes.js:7718`, `dungeonContext.js:6185`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:6336` and
`chargenSession.js`'s `worldModes.js:7831` are bumped and pinned. Four

...
dungeonContext.js:1643 by tools/fistProbe.mjs), the melee tally on
...
and its `world.js:7215` left naming a `WorldTime`/`PauseWhileOpen` note
800 lines from the host's `onPlayerArrowHitFoe` (`world.js:8128`); all
...
`worldModes.js:7089`, `dungeonContext.js:4867`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:5832` and
`chargenSession.js`'s `worldModes.js:7187` are bumped and pinned. Four

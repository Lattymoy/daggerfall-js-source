...
and its `world.js:13434` left naming a `WorldTime`/`PauseWhileOpen` note
800 lines from the host's `onPlayerArrowHitFoe` (`world.js:14995`); all
...
`worldModes.js:9246`, `dungeonContext.js:6685`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:7761` and
`chargenSession.js`'s `worldModes.js:9359` are bumped and pinned. Four

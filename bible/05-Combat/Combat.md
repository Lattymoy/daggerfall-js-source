...
and its `world.js:13587` left naming a `WorldTime`/`PauseWhileOpen` note
800 lines from the host's `onPlayerArrowHitFoe` (`world.js:15151`); all
...
`worldModes.js:9001`, `dungeonContext.js:6728`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:7517` and
`chargenSession.js`'s `worldModes.js:9114` are bumped and pinned. Four

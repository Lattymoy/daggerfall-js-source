...
dungeonContext.js:1777 by tools/fistProbe.mjs), the melee tally on
...
and its `world.js:8602` left naming a `WorldTime`/`PauseWhileOpen` note
800 lines from the host's `onPlayerArrowHitFoe` (`world.js:9673`); all
...
`worldModes.js:7463`, `dungeonContext.js:5841`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:6126` and
`chargenSession.js`'s `worldModes.js:7576` are bumped and pinned. Four

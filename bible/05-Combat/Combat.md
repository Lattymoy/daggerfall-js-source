...
and its `world.js:9892` left naming a `WorldTime`/`PauseWhileOpen` note
800 lines from the host's `onPlayerArrowHitFoe` (`world.js:11298`); all
...
`worldModes.js:8177`, `dungeonContext.js:6278`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:6780` and
`chargenSession.js`'s `worldModes.js:8290` are bumped and pinned. Four

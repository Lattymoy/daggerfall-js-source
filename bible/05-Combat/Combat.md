...
dungeonContext.js:1752 by tools/fistProbe.mjs), the melee tally on
...
and its `world.js:8326` left naming a `WorldTime`/`PauseWhileOpen` note
800 lines from the host's `onPlayerArrowHitFoe` (`world.js:9369`); all
...
`worldModes.js:7464`, `dungeonContext.js:5782`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:6128` and
`chargenSession.js`'s `worldModes.js:7577` are bumped and pinned. Four

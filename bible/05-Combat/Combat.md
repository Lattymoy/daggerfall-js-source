...
dungeonContext.js:1957 by tools/fistProbe.mjs), the melee tally on
...
and its `world.js:10186` left naming a `WorldTime`/`PauseWhileOpen` note
800 lines from the host's `onPlayerArrowHitFoe` (`world.js:11615`); all
...
`worldModes.js:8593`, `dungeonContext.js:6432`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:7137` and
`chargenSession.js`'s `worldModes.js:8706` are bumped and pinned. Four

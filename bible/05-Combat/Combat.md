...
and its `world.js:9928` left naming a `WorldTime`/`PauseWhileOpen` note
800 lines from the host's `onPlayerArrowHitFoe` (`world.js:11334`); all
...
`worldModes.js:8221`, `dungeonContext.js:6304`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:6807` and
`chargenSession.js`'s `worldModes.js:8334` are bumped and pinned. Four

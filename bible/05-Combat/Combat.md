...
dungeonContext.js:1979 by tools/fistProbe.mjs), the melee tally on
...
and its `world.js:10240` left naming a `WorldTime`/`PauseWhileOpen` note
800 lines from the host's `onPlayerArrowHitFoe` (`world.js:11669`); all
...
wrong. All three are resolved by content (`townTalk.js:1218`,
`worldModes.js:8625`, `dungeonContext.js:6479`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:7146` and
`chargenSession.js`'s `worldModes.js:8738` are bumped and pinned. Four

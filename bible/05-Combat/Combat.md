...
dungeonContext.js:1780 by tools/fistProbe.mjs), the melee tally on
...
and its `world.js:8634` left naming a `WorldTime`/`PauseWhileOpen` note
800 lines from the host's `onPlayerArrowHitFoe` (`world.js:9706`); all
...
`worldModes.js:7473`, `dungeonContext.js:5844`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:6134` and
`chargenSession.js`'s `worldModes.js:7586` are bumped and pinned. Four

...
dungeonContext.js:1714 by tools/fistProbe.mjs), the melee tally on
...
and its `world.js:7596` left naming a `WorldTime`/`PauseWhileOpen` note
800 lines from the host's `onPlayerArrowHitFoe` (`world.js:8567`); all
...
`worldModes.js:7194`, `dungeonContext.js:5536`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:5935` and
`chargenSession.js`'s `worldModes.js:7307` are bumped and pinned. Four

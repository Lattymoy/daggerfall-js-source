...
800 lines from the host's `onPlayerArrowHitFoe` (`world.js:8168`); all
...
`worldModes.js:7138`, `dungeonContext.js:4872`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:5880` and
`chargenSession.js`'s `worldModes.js:7236` are bumped and pinned. Four

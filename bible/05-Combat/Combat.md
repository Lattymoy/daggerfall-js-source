...
`worldModes.js:8858`, `dungeonContext.js:6588`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:7349` and
`chargenSession.js`'s `worldModes.js:8971` are bumped and pinned. Four

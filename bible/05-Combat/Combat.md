...
dungeonContext.js:2013 by tools/fistProbe.mjs), the melee tally on
...
and its `world.js:13380` left naming a `WorldTime`/`PauseWhileOpen` note
800 lines from the host's `onPlayerArrowHitFoe` (`world.js:14938`); all
...
`worldModes.js:8900`, `dungeonContext.js:6659`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:7416` and
`chargenSession.js`'s `worldModes.js:9013` are bumped and pinned. Four

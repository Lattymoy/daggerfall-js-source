...
and its `world.js:8205` left naming a `WorldTime`/`PauseWhileOpen` note
800 lines from the host's `onPlayerArrowHitFoe` (`world.js:9248`); all
...
`worldModes.js:7401`, `dungeonContext.js:5728`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:6099` and
`chargenSession.js`'s `worldModes.js:7514` are bumped and pinned. Four

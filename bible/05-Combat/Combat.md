...
dungeonContext.js:1892 by tools/fistProbe.mjs), the melee tally on
...
and its `world.js:9847` left naming a `WorldTime`/`PauseWhileOpen` note
800 lines from the host's `onPlayerArrowHitFoe` (`world.js:11215`); all
...
`worldModes.js:7715`, `dungeonContext.js:6148`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:6333` and
`chargenSession.js`'s `worldModes.js:7828` are bumped and pinned. Four

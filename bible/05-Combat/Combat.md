...
and its `world.js:12884` left naming a `WorldTime`/`PauseWhileOpen` note
800 lines from the host's `onPlayerArrowHitFoe` (`world.js:14429`); all
...
`worldModes.js:8875`, `dungeonContext.js:6594`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:7366` and
`chargenSession.js`'s `worldModes.js:8988` are bumped and pinned. Four

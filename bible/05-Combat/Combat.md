...
and its `world.js:11253` left naming a `WorldTime`/`PauseWhileOpen` note
800 lines from the host's `onPlayerArrowHitFoe` (`world.js:12688`); all
...
`worldModes.js:8688`, `dungeonContext.js:6538`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:7187` and
`chargenSession.js`'s `worldModes.js:8801` are bumped and pinned. Four

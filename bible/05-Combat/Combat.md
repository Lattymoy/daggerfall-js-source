...
`worldModes.js:8529`, `dungeonContext.js:6398`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:7104` and
`chargenSession.js`'s `worldModes.js:8642` are bumped and pinned. Four

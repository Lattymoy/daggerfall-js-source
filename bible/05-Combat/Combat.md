...
and its `world.js:8230` left naming a `WorldTime`/`PauseWhileOpen` note
800 lines from the host's `onPlayerArrowHitFoe` (`world.js:9255`); all
...
wrong. All three are resolved by content (`townTalk.js:1150`,
`worldModes.js:7374`, `dungeonContext.js:5726`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:6072` and
`chargenSession.js`'s `worldModes.js:7487` are bumped and pinned. Four

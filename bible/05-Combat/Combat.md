...
and its `world.js:8538` left naming a `WorldTime`/`PauseWhileOpen` note
800 lines from the host's `onPlayerArrowHitFoe` (`world.js:9600`); all
...
`worldModes.js:7459`, `dungeonContext.js:5804`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:6126` and
`chargenSession.js`'s `worldModes.js:7572` are bumped and pinned. Four

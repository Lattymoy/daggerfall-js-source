...
and its `world.js:11728` left naming a `WorldTime`/`PauseWhileOpen` note
800 lines from the host's `onPlayerArrowHitFoe` (`world.js:13222`); all
...
`worldModes.js:8746`, `dungeonContext.js:6563`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:7244` and
`chargenSession.js`'s `worldModes.js:8859` are bumped and pinned. Four

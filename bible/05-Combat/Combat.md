...
dungeonContext.js:1991 by tools/fistProbe.mjs), the melee tally on
...
(`arrowFlight.js:293` is a `backstabChance:` field; the unconditional
...
and its `world.js:12408` left naming a `WorldTime`/`PauseWhileOpen` note
800 lines from the host's `onPlayerArrowHitFoe` (`world.js:13929`); all
...
`worldModes.js:8818`, `dungeonContext.js:6574`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:7316` and
`chargenSession.js`'s `worldModes.js:8931` are bumped and pinned. Four

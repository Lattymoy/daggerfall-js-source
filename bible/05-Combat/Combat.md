...
and its `world.js:8311` left naming a `WorldTime`/`PauseWhileOpen` note
800 lines from the host's `onPlayerArrowHitFoe` (`world.js:9354`); all
...
`worldModes.js:7428`, `dungeonContext.js:5770`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:6113` and
`chargenSession.js`'s `worldModes.js:7541` are bumped and pinned. Four

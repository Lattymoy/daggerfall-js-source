...
dungeonContext.js:1807 by tools/fistProbe.mjs), the melee tally on
...
`totalWeight` (inventory.js:329) IS `ItemCollection.GetWeight`, so the
...
(`arrowFlight.js:209` is a `backstabChance:` field; the unconditional
...
and its `world.js:9611` left naming a `WorldTime`/`PauseWhileOpen` note
800 lines from the host's `onPlayerArrowHitFoe` (`world.js:10956`); all
...
`worldModes.js:7637`, `dungeonContext.js:5932`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:6267` and
`chargenSession.js`'s `worldModes.js:7750` are bumped and pinned. Four

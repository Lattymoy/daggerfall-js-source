...
and its `world.js:8551` left naming a `WorldTime`/`PauseWhileOpen` note
800 lines from the host's `onPlayerArrowHitFoe` (`world.js:9613`); all
...
`worldModes.js:7468`, `dungeonContext.js:5805`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:6132` and
`chargenSession.js`'s `worldModes.js:7581` are bumped and pinned. Four

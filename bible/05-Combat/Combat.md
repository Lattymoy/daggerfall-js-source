...
and its `world.js:13430` left naming a `WorldTime`/`PauseWhileOpen` note
800 lines from the host's `onPlayerArrowHitFoe` (`world.js:14991`); all
...
`worldModes.js:9095`, `dungeonContext.js:6685`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:7611` and
`chargenSession.js`'s `worldModes.js:9208` are bumped and pinned. Four

...
dungeonContext.js:1830 by tools/fistProbe.mjs), the melee tally on
...
and its `world.js:9746` left naming a `WorldTime`/`PauseWhileOpen` note
800 lines from the host's `onPlayerArrowHitFoe` (`world.js:11104`); all
...
`worldModes.js:7701`, `dungeonContext.js:6025`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:6320` and
`chargenSession.js`'s `worldModes.js:7814` are bumped and pinned. Four

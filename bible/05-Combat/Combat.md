...
dungeonContext.js:1679 by tools/fistProbe.mjs), the melee tally on
...
and its `world.js:7493` left naming a `WorldTime`/`PauseWhileOpen` note
800 lines from the host's `onPlayerArrowHitFoe` (`world.js:8464`); all
...
`worldModes.js:7165`, `dungeonContext.js:5179`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:5906` and
`chargenSession.js`'s `worldModes.js:7278` are bumped and pinned. Four

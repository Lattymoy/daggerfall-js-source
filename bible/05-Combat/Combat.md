...
and its `world.js:11490` left naming a `WorldTime`/`PauseWhileOpen` note
800 lines from the host's `onPlayerArrowHitFoe` (`world.js:12972`); all
...
`worldModes.js:8723`, `dungeonContext.js:6558`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:7221` and
`chargenSession.js`'s `worldModes.js:8836` are bumped and pinned. Four

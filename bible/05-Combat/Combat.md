...
dungeonContext.js:1988 by tools/fistProbe.mjs), the melee tally on
...
and its `world.js:11275` left naming a `WorldTime`/`PauseWhileOpen` note
800 lines from the host's `onPlayerArrowHitFoe` (`world.js:12723`); all
...
`worldModes.js:8718`, `dungeonContext.js:6538`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:7217` and
`chargenSession.js`'s `worldModes.js:8831` are bumped and pinned. Four

...
800 lines from the host's `onPlayerArrowHitFoe` (`world.js:13942`); all
...
`worldModes.js:8821`, `dungeonContext.js:6563`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:7319` and
`chargenSession.js`'s `worldModes.js:8934` are bumped and pinned. Four

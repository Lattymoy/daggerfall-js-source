...
and its `world.js:7636` left naming a `WorldTime`/`PauseWhileOpen` note
800 lines from the host's `onPlayerArrowHitFoe` (`world.js:8607`); all
...
`worldModes.js:7276`, `dungeonContext.js:5544`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:6017` and
`chargenSession.js`'s `worldModes.js:7389` are bumped and pinned. Four

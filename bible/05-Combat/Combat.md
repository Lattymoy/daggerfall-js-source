...
800 lines from the host's `onPlayerArrowHitFoe` (`world.js:14338`); all
...
`worldModes.js:8869`, `dungeonContext.js:6590`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:7360` and
`chargenSession.js`'s `worldModes.js:8982` are bumped and pinned. Four

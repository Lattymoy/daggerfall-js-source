...
dungeonContext.js:2011 by tools/fistProbe.mjs), the melee tally on
...
and its `world.js:13440` left naming a `WorldTime`/`PauseWhileOpen` note
800 lines from the host's `onPlayerArrowHitFoe` (`world.js:15004`); all
...
`worldModes.js:8953`, `dungeonContext.js:6635`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:7469` and
`chargenSession.js`'s `worldModes.js:9066` are bumped and pinned. Four

...
800 lines from the host's `onPlayerArrowHitFoe` (`world.js:11133`); all
...
`worldModes.js:7696`, `dungeonContext.js:6024`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:6315` and
`chargenSession.js`'s `worldModes.js:7809` are bumped and pinned. Four

...
and its `world.js:9917` left naming a `WorldTime`/`PauseWhileOpen` note
800 lines from the host's `onPlayerArrowHitFoe` (`world.js:11320`); all
...
`worldModes.js:7792`, `dungeonContext.js:6199`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:6405` and
`chargenSession.js`'s `worldModes.js:7905` are bumped and pinned. Four

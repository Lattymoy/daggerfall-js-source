...
`worldModes.js:7880`, `dungeonContext.js:6278`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:6483` and
`chargenSession.js`'s `worldModes.js:7993` are bumped and pinned. Four

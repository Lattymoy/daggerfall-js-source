...
dungeonContext.js:1746 by tools/fistProbe.mjs), the melee tally on
...
and its `world.js:8318` left naming a `WorldTime`/`PauseWhileOpen` note
800 lines from the host's `onPlayerArrowHitFoe` (`world.js:9361`); all
...
`worldModes.js:7430`, `dungeonContext.js:5770`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:6115` and
`chargenSession.js`'s `worldModes.js:7543` are bumped and pinned. Four

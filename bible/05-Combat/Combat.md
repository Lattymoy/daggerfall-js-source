...
and its `world.js:13255` left naming a `WorldTime`/`PauseWhileOpen` note
800 lines from the host's `onPlayerArrowHitFoe` (`world.js:14812`); all
...
`worldModes.js:8898`, `dungeonContext.js:6612`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:7388` and
`chargenSession.js`'s `worldModes.js:9011` are bumped and pinned. Four

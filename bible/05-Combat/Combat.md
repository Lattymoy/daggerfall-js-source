...
dungeonContext.js:2001 by tools/fistProbe.mjs), the melee tally on
...
and its `world.js:12772` left naming a `WorldTime`/`PauseWhileOpen` note
800 lines from the host's `onPlayerArrowHitFoe` (`world.js:14289`); all
...
`worldModes.js:8864`, `dungeonContext.js:6591`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:7355` and
`chargenSession.js`'s `worldModes.js:8977` are bumped and pinned. Four

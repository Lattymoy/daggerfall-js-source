...
dungeonContext.js:1702 by tools/fistProbe.mjs), the melee tally on
...
and its `world.js:7681` left naming a `WorldTime`/`PauseWhileOpen` note
800 lines from the host's `onPlayerArrowHitFoe` (`world.js:8666`); all
...
`worldModes.js:7297`, `dungeonContext.js:5657`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:6032` and
`chargenSession.js`'s `worldModes.js:7410` are bumped and pinned. Four

...
dungeonContext.js:1829 by tools/fistProbe.mjs), the melee tally on
...
and its `world.js:9769` left naming a `WorldTime`/`PauseWhileOpen` note
800 lines from the host's `onPlayerArrowHitFoe` (`world.js:11121`); all
...
wrong. All three are resolved by content (`townTalk.js:1158`,
`worldModes.js:7716`, `dungeonContext.js:6024`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:6335` and
`chargenSession.js`'s `worldModes.js:7829` are bumped and pinned. Four

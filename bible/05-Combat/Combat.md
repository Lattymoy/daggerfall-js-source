...
800 lines from the host's `onPlayerArrowHitFoe` (`world.js:14308`); all
...
`worldModes.js:8866`, `dungeonContext.js:6601`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:7386` and
`chargenSession.js`'s `worldModes.js:8979` are bumped and pinned. Four

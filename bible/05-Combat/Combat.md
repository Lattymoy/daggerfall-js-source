...
and its `world.js:8547` left naming a `WorldTime`/`PauseWhileOpen` note
800 lines from the host's `onPlayerArrowHitFoe` (`world.js:9609`); all
...
`worldModes.js:7462`, `dungeonContext.js:5805`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:6126` and
`chargenSession.js`'s `worldModes.js:7575` are bumped and pinned. Four

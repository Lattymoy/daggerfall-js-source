...
and its `world.js:11714` left naming a `WorldTime`/`PauseWhileOpen` note
800 lines from the host's `onPlayerArrowHitFoe` (`world.js:13195`); all
...
`worldModes.js:8743`, `dungeonContext.js:6563`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:7241` and
`chargenSession.js`'s `worldModes.js:8856` are bumped and pinned. Four

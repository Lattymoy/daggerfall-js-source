...
dungeonContext.js:1851 by tools/fistProbe.mjs), the melee tally on
...
800 lines from the host's `onPlayerArrowHitFoe` (`world.js:11165`); all
...
`worldModes.js:7717`, `dungeonContext.js:6065`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:6335` and
`chargenSession.js`'s `worldModes.js:7830` are bumped and pinned. Four

...
and its `world.js:9727` left naming a `WorldTime`/`PauseWhileOpen` note
800 lines from the host's `onPlayerArrowHitFoe` (`world.js:11073`); all
...
`worldModes.js:7666`, `dungeonContext.js:6024`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:6285` and
`chargenSession.js`'s `worldModes.js:7779` are bumped and pinned. Four

...
`worldModes.js:7650`, `dungeonContext.js:5969`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:6278` and
`chargenSession.js`'s `worldModes.js:7763` are bumped and pinned. Four

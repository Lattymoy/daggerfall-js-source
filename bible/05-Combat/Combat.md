...
dungeonContext.js:2044 by tools/fistProbe.mjs), the melee tally on
...
and its `world.js:13828` left naming a `WorldTime`/`PauseWhileOpen` note
800 lines from the host's `onPlayerArrowHitFoe` (`world.js:15421`); all
...
`worldModes.js:9356`, `dungeonContext.js:6716`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:7871` and
`chargenSession.js`'s `worldModes.js:9469` are bumped and pinned. Four

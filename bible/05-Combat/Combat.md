...
`worldModes.js:8508`, `dungeonContext.js:6397`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:7083` and
`chargenSession.js`'s `worldModes.js:8621` are bumped and pinned. Four

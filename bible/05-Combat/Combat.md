...
800 lines from the host's `onPlayerArrowHitFoe` (`world.js:11145`); all
...
`worldModes.js:7709`, `dungeonContext.js:6063`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:6327` and
`chargenSession.js`'s `worldModes.js:7822` are bumped and pinned. Four

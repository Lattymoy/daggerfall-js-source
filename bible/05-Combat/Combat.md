...
`worldModes.js:9409`, `dungeonContext.js:6685`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:7924` and
`chargenSession.js`'s `worldModes.js:9522` are bumped and pinned. Four

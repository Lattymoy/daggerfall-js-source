...
dungeonContext.js:1721 by tools/fistProbe.mjs), the melee tally on
...
and its `world.js:8245` left naming a `WorldTime`/`PauseWhileOpen` note
800 lines from the host's `onPlayerArrowHitFoe` (`world.js:9270`); all
...
`worldModes.js:7388`, `dungeonContext.js:5739`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:6086` and
`chargenSession.js`'s `worldModes.js:7501` are bumped and pinned. Four

...
`worldModes.js:7781`, `dungeonContext.js:6199`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:6394` and
`chargenSession.js`'s `worldModes.js:7894` are bumped and pinned. Four

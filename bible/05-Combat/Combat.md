...
and its `world.js:12530` left naming a `WorldTime`/`PauseWhileOpen` note
800 lines from the host's `onPlayerArrowHitFoe` (`world.js:14054`); all
...
`worldModes.js:8862`, `dungeonContext.js:6591`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:7353` and
`chargenSession.js`'s `worldModes.js:8975` are bumped and pinned. Four

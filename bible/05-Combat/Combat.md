...
800 lines from the host's `onPlayerArrowHitFoe` (`world.js:8852`); all
...
`worldModes.js:7364`, `dungeonContext.js:5726`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:6067` and
`chargenSession.js`'s `worldModes.js:7477` are bumped and pinned. Four

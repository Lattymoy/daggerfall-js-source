...
and its `world.js:11233` left naming a `WorldTime`/`PauseWhileOpen` note
800 lines from the host's `onPlayerArrowHitFoe` (`world.js:12668`); all
...
`worldModes.js:8657`, `dungeonContext.js:6513`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:7179` and
`chargenSession.js`'s `worldModes.js:8770` are bumped and pinned. Four

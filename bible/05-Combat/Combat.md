...
and its `world.js:13464` left naming a `WorldTime`/`PauseWhileOpen` note
800 lines from the host's `onPlayerArrowHitFoe` (`world.js:15028`); all
...
`worldModes.js:8956`, `dungeonContext.js:6728`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:7472` and
`chargenSession.js`'s `worldModes.js:9069` are bumped and pinned. Four

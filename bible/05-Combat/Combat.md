...
and its `world.js:13516` left naming a `WorldTime`/`PauseWhileOpen` note
800 lines from the host's `onPlayerArrowHitFoe` (`world.js:15080`); all
...
`worldModes.js:8959`, `dungeonContext.js:6728`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:7475` and
`chargenSession.js`'s `worldModes.js:9072` are bumped and pinned. Four

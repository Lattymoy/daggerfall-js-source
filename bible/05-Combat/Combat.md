...
dungeonContext.js:2000 by tools/fistProbe.mjs), the melee tally on
...
and its `world.js:12762` left naming a `WorldTime`/`PauseWhileOpen` note
800 lines from the host's `onPlayerArrowHitFoe` (`world.js:14285`); all
...
`worldModes.js:8856`, `dungeonContext.js:6590`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:7347` and
`chargenSession.js`'s `worldModes.js:8969` are bumped and pinned. Four

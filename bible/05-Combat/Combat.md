...
and its `world.js:10061` left naming a `WorldTime`/`PauseWhileOpen` note
800 lines from the host's `onPlayerArrowHitFoe` (`world.js:11467`); all
...
`worldModes.js:8519`, `dungeonContext.js:6397`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:7094` and
`chargenSession.js`'s `worldModes.js:8632` are bumped and pinned. Four

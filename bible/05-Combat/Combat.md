...
dungeonContext.js:2008 by tools/fistProbe.mjs), the melee tally on
...
and its `world.js:12928` left naming a `WorldTime`/`PauseWhileOpen` note
800 lines from the host's `onPlayerArrowHitFoe` (`world.js:14484`); all
...
`worldModes.js:8879`, `dungeonContext.js:6625`) and pinned as a set.

The `worldModes.js` fix inserts one line, so cites into that host past
it move by one: the dungeon's `worldModes.js:7397` and
`chargenSession.js`'s `worldModes.js:8992` are bumped and pinned. Four

...
- **systems/breath.js**: PlayerEntity.FixedUpdate's breath clause
  ...
  (worldModes.js:606). exterior.js and world.js have no submersion
...
`host` (`worldModes.js:344`), and its only two callers are `world.js`
(`:6147`) and `exterior.js` (`:2769`), both of which pass their own Set
and both of whose WINDOW-level handlers (`world.js:4792-4793`,
`exterior.js:2369-2370`) call `mouseCode(e.button)` and add/delete
unconditionally - outside every mode and overlay gate. `MOUSE_CODES`
maps button 2 to `Mouse2` (`input.js:278`), which is the shipped
...
motor's own mirror of that getter (`world.js:7819` already did at its

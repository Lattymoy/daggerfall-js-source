...
| **CLK3** | DAY AND NIGHT ON THE CLOCK - the remaining steps. The moon's phase is a day-granularity step (DFU's `GetLunarPhase`); the enhanced dome interpolates it within the day so the lit fraction and the moon's place move continuously (the classic lane's flats keep DFU's step). The dome's sun elevation and the rig's `daylightScale` are checked against each other at twilight and pinned to one curve where they disagree. FOUND: they were one curve already (both ride worldClock's `dayFraction`; pinned - the same minute puts the sun on the horizon and the rig's light at zero, and the two climb together). The moon: `gameDate.lunarPhaseFraction` is DFU's own 32-day ratio with the minute of the day added, mapped onto the dome's 0..8 ring (New 0, Full 4), never a ring step from the ladder; the dome and the lab take it, every SYSTEM (the lycanthrope's full moon, the enchantment ctx) keeps DFU's step. A moon that used to jump 45 degrees along its arc at midnight - and the world's moonlight with it - walks there through the day now. | SHIPPED 2026-09-08 |

...
## REST-VEIL2 - THE SELECTION PAGE TOO (2026-09-19)

Mac: *"opening up the rest menu has a black background behind it"*.

CLK4 relaxed the RESTING page only, so the window answered the
question "does this hide the world?" two different ways three
keystrokes apart: press the key and the world is gone behind DFU's
opaque black, pick a rest and the world comes back under the veil.
The page a player actually opens was the blacked-out one.

`_drawNative` tested `this.state === 'resting' && isEnhanced()`; it
tests `isEnhanced()` now. The veil is the enhanced skin's answer for
every page of this window, and the CLASSIC skin is untouched, because
the opaque black IS DFU there (DaggerfallRestWindow Setup :137-138,
`ParentPanel.BackgroundColor = Color.black`).

Nothing else in the window moved: the panel art, the counter, the
hour count and the vitals row all draw exactly where they did, on a
veil instead of a black.

Pins: `clockArc.test.js` (the expression, and that no page is veiled
while another is blacked out) and `restlodging.test.js` (the D3 native
pages). Mutants: `tools/mutants/restveil.json`, 3, 3 dead - the
per-state test put back, the classic skin taking the veil, and the
veil turned opaque.

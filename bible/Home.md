...
## Open flags (regenerated 2026-08-18, U16 summary)

Regenerated at the U16 close: ONE flag ADDED and none retired. The new
row is chargenArt's skills block - DFU's SkillsRollout carries THREE
LeftRightSpinners, one per group, each showing its own remaining pool,
where the port collapses the nine rows onto one cursor and draws a
single spinner. Every point can still be spent; classic just shows all
three at once, which the summary screen makes obvious. Line numbers
refreshed.
...
- `src/ui/chargenArt.js:470` - AUDIT 17g FLAGGED: the scrollbar THUMB does not draw. Its geometry
- `src/ui/chargenArt.js:544` - *  U16 FLAGGED: DFU's rollout carries THREE LeftRightSpinners, one per

...
- `03-World/Roads-Arc.md` - ACTIVE, opened 2026-08-29. THE ROAD SYSTEM'S OWN PAGE, and the first thing it records is that it did not have one: R1-R7, a twelve-slice fix run, a whole-arc geometry revert and the RA1 audit had their entire record spread across one Ledger row, two Testing.md rows and the module headers - the F303 defect exactly. Roads are an ENHANCED-SKIN-ONLY departure with no DFU original (classic Daggerfall has no roads at all), GENERATED on the player's own machine from their own ARENA2 and never vendored, with the per-pixel cost term and the travel price IMPORTED from TravelTimeCalculator rather than restated - so the drawn route and the charged time are one arithmetic, and a road route can never lose to classic's own walk. Carries RF1 (2026-08-29), the re-audit: the switch module's header described the OPPOSITE of the shipping default (R7 flipped PREF_DEFAULTS.roads and left the prose reading "the preference defaults false" under a heading explaining why roads are off), and six sites across five modules still argued from the "twenty-six second" bake RA1 had already taken to 2.3s - two of them building a design argument on it. Both fixed, both pinned two-way, re-measured at 3.24s on the reference fixture's shape. Open: every skin constant is still tuned against SYNTHETIC terrain and awaits real WOODS relief.
...
- `src/scenes/dungeonContext.js:254` - the chain lives, the motion is INTERIM (loud) until flats can tween.
- `src/scenes/dungeonContext.js:785` - index into the 18 careers) or the INTERIM default Warrior (16,
- `src/scenes/dungeonContext.js:1359` - FS1 - FLAGGED (THE FOUR HOSTS RULE): THE ENCHANT CTX IS NOT
- `src/scenes/dungeonContext.js:1394` - onTeleport: () => hudText.add('(Recall pends in the standalone dungeon - the anchor machinery lives in the streaming ?world host)'),   // TP-slice INTERIM
- `src/scenes/dungeonContext.js:1430` - onTeleport INTERIM shape). Absent, the engine's dispatch
- `src/scenes/dungeonContext.js:3266` - PX3 FLAGGED: questMessages - the dungeon quest mount is
- `src/scenes/dungeonContext.js:3374` - rest-for-a-while. DFU's toggle-close binding is FLAGGED in
- `src/scenes/exterior.js:614` - S3d: the INTERIM dagger seed is the FALLBACK only - a character
- `src/scenes/exterior.js:620` - pre-chargen INTERIM entity (flat skills 30, maxHealth 50) for the
- `src/scenes/exterior.js:858` - onTeleport: () => townTalk.say('(Recall pends here - the anchor machinery lives in the streaming ?world host)'),   // TP-slice INTERIM
- `src/scenes/exterior.js:1096` - PX3 FLAGGED: questMessages - this test host mounts no quest
- `src/scenes/exterior.js:1439` - (FLAGGED: the climate People table pends; the test city is
...
- `src/scenes/world.js:3008` - slot. Dungeon-mode popups pend the dungeon overlay seam (FLAGGED:
- `src/scenes/world.js:3788` - castle interior (FLAGGED with the palace blocks).
- `src/scenes/world.js:4716` - building doors are the E-enter seam, not bashables - FLAGGED
- `src/scenes/worldModes.js:1178` - at the concluded deal (:1036-1051). FLAGGED: the equipped test
- `src/scenes/worldModes.js:1193` - G4: THE GUILD STORE ARM. This had been a FLAGGED null since
- `src/scenes/worldModes.js:1402` - is the quest machine's, FLAGGED with it.
- `src/scenes/worldModes.js:1407` - FLAGGED, above ground only, each with the DFU line it owes:
- `src/scenes/worldModes.js:1598` - CW1 retired the FLAGGED list that lived here - every arm it
- `src/scenes/worldModes.js:1838` - fixed ship scenes and stays FLAGGED, so those buttons keep
- `src/scenes/worldModes.js:1870` - H3: the sell price, which was FLAGGED at zero because it needs
- `src/scenes/worldModes.js:2091` - FactionData; the port's pre-chargen INTERIM entity does not, and
- `src/scenes/worldModes.js:2213` - skip, the refusal line. Only the destination was a FLAGGED null,
- `src/scenes/worldModes.js:2488` - been a FLAGGED null since G3.
- `src/scenes/worldModes.js:2533` - destination has been a FLAGGED null since G3.
- `src/scenes/worldModes.js:2552` - been FLAGGED nulls since G3.
- `src/scenes/worldModes.js:2657` - with the trade window's own mode flow, the same INTERIM the
- `src/scenes/worldModes.js:2882` - first (owned houses and quest buildings FLAGGED/seamed per
- `src/scenes/worldModes.js:2894` - FLAGGED, and narrowed to what is actually missing: the two BASH
- `src/scenes/worldModes.js:2956` - the FLAGGED note above), so it starts false.
- `src/scenes/worldModes.js:4106` - string is the seam that was a FLAGGED null until this slice,
- `src/scenes/worldModes.js:4205` - *  FLAGGED null this slice closed. */
- `src/scenes/worldModes.js:4543` - *  FLAGGED: pause-and-resume is the DFU behaviour and a
...
- `src/ui/enhancedMenu.js:1574` - FLAGGED: the rest of the keyboard. The wizard walks to `done` with
- `src/ui/enhancedMenu.js:1745` - if (action === 'delete') return;   // FLAGGED: no save manager yet

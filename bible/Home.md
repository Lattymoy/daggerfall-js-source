...
## Open flags (regenerated 2026-08-18, U15 wizard order + random name)

Regenerated at the U15 close: ONE flag RETIRED and its sentence
DELETED, per the rule - chargenArt's "the random-name button is drawn
by the art but inert" is gone, because the button works. The wizard
ORDER leaves the chargen queue with it. What remains of the chargen
arc is the custom-class screen and the class picker's scrollbar thumb.
...
- `src/ui/chargen.js:51` - SelectBiographyMethod, BiographyQuestions, SelectName...). FLAGGED:

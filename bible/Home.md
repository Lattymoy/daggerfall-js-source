...
- `src/scenes/dungeonContext.js:768` - index into the 18 careers) or the INTERIM default Warrior (16,
- `src/scenes/dungeonContext.js:1299` - FS1 - FLAGGED (THE FOUR HOSTS RULE): THE ENCHANT CTX IS NOT
- `src/scenes/dungeonContext.js:1334` - onTeleport: () => hudText.add('(Recall pends in the standalone dungeon - the anchor machinery lives in the streaming ?world host)'),   // TP-slice INTERIM
- `src/scenes/dungeonContext.js:1370` - onTeleport INTERIM shape). Absent, the engine's dispatch
- `src/scenes/dungeonContext.js:3178` - PX3 FLAGGED: questMessages - the dungeon quest mount is
- `src/scenes/dungeonContext.js:3249` - rest-for-a-while. DFU's toggle-close binding is FLAGGED in
...
- `src/scenes/world.js:2959` - slot. Dungeon-mode popups pend the dungeon overlay seam (FLAGGED:
- `src/scenes/world.js:3739` - castle interior (FLAGGED with the palace blocks).
- `src/scenes/world.js:4654` - building doors are the E-enter seam, not bashables - FLAGGED

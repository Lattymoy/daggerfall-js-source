...
- `src/characters/playerEntity.js:27` - fatigue: 3200,    // (Str 50 + End 0) x 64 pre-chargen (INTERIM stats above); ...
...
- `src/scenes/worldModes.js:84` - say -> console FLAGGED: the interior HUD-text layer pends its arc.
- `src/scenes/worldModes.js:124` - if (!isShop(b.buildingType)) return;   // Library/Guild/Temple bookshelves + o...
- `src/scenes/worldModes.js:217` - Person placed in a dungeon is the quest machine's, FLAGGED
- `src/scenes/worldModes.js:256` - FLAGGED, each with the slice it waits on:
- `src/scenes/worldModes.js:275` - FactionData; the port's pre-chargen INTERIM entity does not, and
- `src/scenes/worldModes.js:311` - onTalk: () => townTalk?.say?.('You get no response.'),   // FLAGGED: TalkToSta...
- `src/scenes/worldModes.js:317` - FLAGGED: every destination window is U24 or later
- `src/scenes/worldModes.js:971` - a castle reads as a plain dungeon interior. FLAGGED.

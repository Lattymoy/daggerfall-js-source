...
- `src/scenes/dungeonContext.js:525` - index into the 18 careers) or the INTERIM default Warrior (16,
- `src/scenes/dungeonContext.js:531` - effects FLAGGED to the effect-library slice.
- `src/scenes/dungeonContext.js:558` - "database FLAGGED" narrows to the skill/loot message ids).
...
- `src/scenes/exterior.js:652` - (FLAGGED); swallowing the browser reload is not optional.
- `src/scenes/exterior.js:779` - (FLAGGED: the climate People table pends; the test city is
- `src/scenes/exterior.js:912` - FLAGGED here exactly as in world.js - no tile lookup yet).
...
- `src/scenes/world.js:799` - (FLAGGED); swallowing the browser reload is not optional.
- `src/scenes/world.js:1104` - exemption (PlayerTileMapIndex == 0) is FLAGGED: this host
- `src/scenes/world.js:1319` - doors are the E-enter seam, not bashables - FLAGGED with the

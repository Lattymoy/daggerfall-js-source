...
- `06-Systems/Online-Arc.md` - ONLINE (ONLINE1, 2026-09-12, Mac: "the basic bones of multiplayer ... being able to see and traverse with other players"). The port's own (DFU has none): a Cloudflare relay in `server/` (one Durable Object per room, nothing stored), the streaming world sharded into 16-pixel cells with a 3-pixel range, towns, dungeons and interiors rooms by location; `net/online.js` the session (a pose at 10 Hz when moved, the peers eased between poses), `net/remotePlayers.js` every peer as their paperdoll at their feet with the name over the head; the front door's ONLINE brings the most recent save in. The Morrowind body for a peer, chat and anything shared beyond presence are the next iterations.
...
- `src/ui/enhancedMenu.js:2176` - FLAGGED: the rest of the keyboard. The wizard walks to `done` with

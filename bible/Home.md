...
- `src/scenes/exterior.js:662` - (FLAGGED); swallowing the browser reload is not optional.
- `src/scenes/exterior.js:794` - (FLAGGED: the climate People table pends; the test city is
- `src/scenes/exterior.js:929` - FLAGGED here exactly as in world.js - no tile lookup yet).
...
- `src/scenes/world.js:567` - FLAGGED loud: the People faction rides the START location's
- `src/scenes/world.js:588` - S3d: the INTERIM dagger seed is the FALLBACK only - a character
- `src/scenes/world.js:594` - pre-chargen INTERIM entity (flat skills 30, maxHealth 50) for the
- `src/scenes/world.js:1013` - (FLAGGED); swallowing the browser reload is not optional.
- `src/scenes/world.js:1390` - exemption (PlayerTileMapIndex == 0) is FLAGGED: this host
- `src/scenes/world.js:1650` - doors are the E-enter seam, not bashables - FLAGGED with the

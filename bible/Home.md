...
## Open flags (regenerated 2026-08-18, U11 message box)

Regenerated at the U11 close: the "no message-box FRAME art yet" and
"the race description rides the same missing frame" sentences are
DELETED - DaggerfallMessageBox ships - and U6's "backgrounds FLAGGED
pending art-name verification" note with them. The remaining chargen
row is the honest one: biography/reflexes/custom-class have no flow
states. Line numbers refreshed. Every row below is a live
...
- `src/scenes/dungeonContext.js:643` - drained strength lowers the ceiling). INTERIM (loud): the
- `src/scenes/dungeonContext.js:762` - FLAGGED: DFU recomputes per-effect via the cost tables (that
- `src/scenes/dungeonContext.js:764` - FLAGGED to the effect library (caster-only buffs, touch, areas).
- `src/scenes/dungeonContext.js:976` - 129; the inventory/equip UI pends - the INTERIM dagger note
- `src/scenes/dungeonContext.js:1714` - actions) is FLAGGED - the player snapshot only.
...
- `src/scenes/exterior.js:571` - (FLAGGED: the climate People table pends; the test city is
- `src/scenes/exterior.js:640` - FLAGGED here exactly as in world.js - no tile lookup yet).
...
- `src/scenes/world.js:866` - exemption (PlayerTileMapIndex == 0) is FLAGGED: this host
- `src/scenes/world.js:1065` - doors are the E-enter seam, not bashables - FLAGGED with the
...
- `src/ui/messageBox.js:136` - so the strip never rides higher than that. FLAGGED as a

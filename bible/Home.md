...
## Open flags (regenerated 2026-08-17, G3)
...
- `src/scenes/cityGuards.js:99` - attack.rangedAttack = false;   // FLAGGED: guard archers pend exterior foe arrows
...
- `src/scenes/exterior.js:458` - (FLAGGED: the climate People table pends; the test city is
- `src/scenes/exterior.js:527` - FLAGGED here exactly as in world.js - no tile lookup yet).
- `src/scenes/townTalk.js:16` - FLAGGED loud: Info mode opens the same talk window (DFU routes
- `src/scenes/townTalk.js:259` - GetAnswerWhereIs (knows-always FLAGGED) + the %hnt hint chain:
- `src/scenes/townTalk.js:267` - .replaceAll('%hnr', 'Sir').replaceAll('%ra', 'Breton');   // honorific/race macros FLAG...
...
- `src/scenes/world.js:746` - exemption (PlayerTileMapIndex == 0) is FLAGGED: this host
- `src/scenes/world.js:925` - doors are the E-enter seam, not bashables - FLAGGED with the

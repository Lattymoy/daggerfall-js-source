...
## Open flags (regenerated 2026-08-17, the C11 slice)
...
- `src/characters/mobileUnit.js:11` - DEFERRED (FLAGGED): RangedAttack1/2 + Spell anim states (monster
...
- `src/scenes/dungeonContext.js:432` - INTERIM (loud): flying/aquatic monsters ride the grounded
- `src/scenes/dungeonContext.js:464` - index into the 18 careers) or the INTERIM default Warrior (16,
- `src/scenes/dungeonContext.js:470` - effects FLAGGED to the effect-library slice.
- `src/scenes/dungeonContext.js:484` - "database FLAGGED" narrows to the skill/loot message ids).
- `src/scenes/dungeonContext.js:618` - drained strength lowers the ceiling). INTERIM (loud): the
- `src/scenes/dungeonContext.js:733` - FLAGGED: DFU recomputes per-effect via the cost tables (that
- `src/scenes/dungeonContext.js:735` - FLAGGED to the effect library (caster-only buffs, touch, areas).
- `src/scenes/dungeonContext.js:941` - 129; the inventory/equip UI pends - the INTERIM dagger note
- `src/scenes/dungeonContext.js:1623` - actions) is FLAGGED - the player snapshot only.

...
## Open flags (regenerated 2026-08-18, S3e biography)

Regenerated at the S3e close: the chargen arc's "biography questions
are not in the flow yet" sentence is DELETED - they ship - and the new
rows are the biography's own honest gaps (AE/AF/AO unimplemented as
DFU leaves them, rf faction rep queued for the faction slice, the
backstory text and the wizard ORDER). Line numbers refreshed. Every row below is a live
...
- `src/scenes/dungeonContext.js:769` - FLAGGED: DFU recomputes per-effect via the cost tables (that
- `src/scenes/dungeonContext.js:771` - FLAGGED to the effect library (caster-only buffs, touch, areas).
- `src/scenes/dungeonContext.js:983` - 129; the inventory/equip UI pends - the INTERIM dagger note
- `src/scenes/dungeonContext.js:1721` - actions) is FLAGGED - the player snapshot only.
...
- `src/scenes/exterior.js:405` - day-skip is a no-op FLAGGED until the shared calendar lands).
- `src/scenes/exterior.js:471` - (FLAGGED); swallowing the browser reload is not optional.
- `src/scenes/exterior.js:573` - (FLAGGED: the climate People table pends; the test city is
- `src/scenes/exterior.js:642` - FLAGGED here exactly as in world.js - no tile lookup yet).
...
- `src/scenes/world.js:575` - day-skip is a no-op FLAGGED until the shared calendar lands).
- `src/scenes/world.js:630` - (FLAGGED); swallowing the browser reload is not optional.
- `src/scenes/world.js:868` - exemption (PlayerTileMapIndex == 0) is FLAGGED: this host
- `src/scenes/world.js:1067` - doors are the E-enter seam, not bashables - FLAGGED with the
...
- `src/systems/chargenSession.js:103` - S3d: the real starting kit replaces the INTERIM dagger seed -
...
- `src/ui/chargen.js:49` - SelectBiographyMethod, BiographyQuestions, SelectName...). FLAGGED:
- `src/ui/chargenArt.js:26` - FLAGGED loud: biography questions, reflexes and the custom-class
- `src/ui/chargenArt.js:319` - AUDIT 17g FLAGGED: the scrollbar THUMB does not draw. Its geometry

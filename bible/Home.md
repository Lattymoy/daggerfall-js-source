...
- `04-Characters/Characters-Arc.md` - ACTIVE. C2 shipped: exterior NPCs (faction rule verbatim, 76-NPC corpus) + townsfolk name banks (NameHelper over DFRandom, NameGen data committed). C3 shipped: dungeon enemies - classic tables + selection verbatim, standing billboards (Privateer's 42-enemy pin; city crawl 128). PIVOTED to the player paperdoll: townsfolk stay classic billboards; C4's render path + vendored rig serve the paperdoll (C5 data -> C6 authored pieces -> C7 live rotating viewport). C5 COMPLETE (slots + 288 templates + layer order, tint tables, equip rules). Next: C6 first authored paperdoll piece.

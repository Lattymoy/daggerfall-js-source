...
- `01-Overview/Audit-51.md` - CLOSED 2026-09-02: 1:1 PARITY WITH BASIC ROADS, BY ORACLE - the mod's PaintPath transliterated from the C# into tools/roadsOracle.py and run over 651 cases against our painter: 639 differed (his tables pave water; rotate before the zero check), then 11 (the mod paints THROUGH the location rect and roads pave its padding - which is how his towns ring), then 0. The smoother ported as SmoothRoadsJob with one recorded divergence, the mod's transposed SAMPLE base - corrected onto that index by AUDIT 58 (f2/hosts), which found the note had recorded the correction on the tile read, where `y*tDim + x` IS the mod's own `Idx(x, y, tDim)` and there was nothing to correct. Data byte-identical to the dfmod.
...
- `01-Overview/Audit-60.md` - CLOSED 2026-09-04: PH1, THE PEGAS HORSE, BEFORE IT MERGES. Eight sweeps on the other account's branch merged with main: the wiring rung by rung, save/load symmetry, the classic skin, the cites, the pins on the script's numbers - all hold. Three findings: the 951-line script the port cites 78 times is not in the tree (a vendoring call for Mac), LOAD_FIX_DISTANCE is transcribed and applied by nothing, and the clock's USE is unpinned - per-render-frame stepping fails no pin. Not seen.
...
- `01-Overview/Port-Status-2026-09-02.md` - the RE-MEASURED 1:1 status at the campaign's end, superseding `Port-Status-2026-09.md`: every figure re-derived by command against the tree and the DFU reference (three of the superseded page's figures corrected), the DFU class-citation sweep per directory as a lower bound, the sixty windows, the effect library, the quest actions, the formats, the systems - and the three lists that are the point: the nineteen open flags it was measured over - SEVEN STAND, the count `node tools/regenOpenFlags.mjs --check` answers and this file's own "Open flags" list holds - each with its blocker, the section C rows still routed, and the departures not on the road.
...
- `13-Pegas/Pegas-Arc.md` - ACTIVE, opened 2026-09-04 (Mac: "this mod wasn't implemented 1:1 with what I gave you" - "Everything needs to be 1:1"): THE PLAN for Pegas Horse Ranch (MADMAX and Team, 2004) in the enhanced skin, read out of the mod's own ESM - 80 scripts, 761 dialogue infos, 53 creatures, 108 globals - and transcribed as LAWS with their constants and script lines. The decisions: enhanced only, the classic horse byte-identical; a horse is a CREATURE in the world; the port's motor moves the rider with the script's speeds; frames are 30 Hz (`PEGAS_SCRIPT_HZ`, a recorded assumption - a trot of 10 units/frame is a real trot at 30, a car at 60); 69.99 units a metre. The laws: the riding script whole (mount gates, the saddle, facing, trot/gallop, the tired law, the special move, the jump, free view, the slope stop, the four dismount doors), the record and its foods, the twenty-one breeds and their caps, the daily pen and the price polynomial, Dirdayvin's buying and selling, the trainers' three hidden groups and the grandmaster, Liokys's breeding table and the foal's three days, companion riding, the ranch's cells. Slices PH1 the horse in the world and the riding script; PH2 the record; PH3 the economy; PH4 the ranch in the world (WHERE in the Iliac Bay is Mac's decision); PH5 the extras. What shipped before this page (MW-D41/42/50) was the mesh under Daggerfall's mount, which this arc supersedes in the enhanced skin.
...
- `src/ui/enhancedMenu.js:1826` - FLAGGED: the rest of the keyboard. The wizard walks to `done` with
...
correction: NIGHTSKY.COL really is fetched - scenes/shared.js:47 loads

...
- `01-Overview/Hardening.md` - THE HARDENING PROGRAM (HARD, 2026-09-14, Mac: "I really want to focus on making everything clean, hardened and not spaghetti ... I think i want to take a break of additions and do this right"). The plan and the running record for a pass that adds no features. Its principle, learned from AUDIT 66 and from `audit24_lifetimes.test.js` before it: A RULE ENFORCED BY AN ENUMERATION IS A RULE ENFORCED BY MEMORY - a gate that pins the defects an audit found protects the past, and the next instance of the same class ships green. Every slice converts a remembered rule into a DERIVED one that reads the source, finds every site the rule applies to, and fails closed on the undecided ones. HARD1 (the generative lifetime gate) is shipped; HARD2 (one contract per cross-cutting system, so a law is not wired four times by hand), HARD3 (types at the three seams where a wrong shape crashes), HARD4 (the mod-port registry) and HARD5 (Home.md back to an index) are written up there with their risks.
...
- `src/ui/enhancedMenu.js:2338` - FLAGGED: the rest of the keyboard. The wizard walks to `done` with
...
`dungeonContext.js:1702` before the fix): bare hands are a NULL weapon

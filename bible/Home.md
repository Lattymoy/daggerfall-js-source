...
## Open flags (regenerated 2026-08-17, U8e)
...
- `src/scenes/exterior.js:286` - it). say -> console FLAGGED: this host has no HUD-text layer yet.
- `src/scenes/exterior.js:351` - day-skip is a no-op FLAGGED until the shared calendar lands).
- `src/scenes/exterior.js:509` - (FLAGGED: the climate People table pends; the test city is
- `src/scenes/exterior.js:578` - FLAGGED here exactly as in world.js - no tile lookup yet).
...
- `src/scenes/world.js:424` - it). say -> console FLAGGED: this host has no HUD-text layer yet.
- `src/scenes/world.js:428` - FLAGGED loud: the People faction rides the START location's
- `src/scenes/world.js:521` - day-skip is a no-op FLAGGED until the shared calendar lands).
- `src/scenes/world.js:805` - exemption (PlayerTileMapIndex == 0) is FLAGGED: this host
- `src/scenes/world.js:997` - doors are the E-enter seam, not bashables - FLAGGED with the
...
- `src/ui/nativeInventory.js:129` - INTERIM info panel: name/weight/value (DFU's 1016 info text
- `src/ui/nativeInventory.js:151` - equip/use: FLAGGED - the paperdoll/use arcs pend

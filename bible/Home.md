...
## Open flags (regenerated 2026-08-17, the C14 slice)
...
- `src/characters/mobileUnit.js:16` - DEFERRED (FLAGGED): the Seducer transform pair, the -1 frame as
...
- `src/scenes/dungeonContext.js:471` - index into the 18 careers) or the INTERIM default Warrior (16,
- `src/scenes/dungeonContext.js:477` - effects FLAGGED to the effect-library slice.
- `src/scenes/dungeonContext.js:491` - "database FLAGGED" narrows to the skill/loot message ids).
- `src/scenes/dungeonContext.js:625` - drained strength lowers the ceiling). INTERIM (loud): the
- `src/scenes/dungeonContext.js:740` - FLAGGED: DFU recomputes per-effect via the cost tables (that
- `src/scenes/dungeonContext.js:742` - FLAGGED to the effect library (caster-only buffs, touch, areas).
- `src/scenes/dungeonContext.js:951` - 129; the inventory/equip UI pends - the INTERIM dagger note
- `src/scenes/dungeonContext.js:1656` - actions) is FLAGGED - the player snapshot only.
- `src/scenes/exterior.js:258` - it). say -> console FLAGGED: this host has no HUD-text layer yet.
- `src/scenes/exterior.js:398` - FLAGGED here exactly as in world.js - no tile lookup yet).
- `src/scenes/world.js:327` - it). say -> console FLAGGED: this host has no HUD-text layer yet.
- `src/scenes/world.js:492` - exemption (PlayerTileMapIndex == 0) is FLAGGED: this host
- `src/scenes/world.js:615` - doors are the E-enter seam, not bashables - FLAGGED with the
...
- `src/systems/effects.js:509` - out.skipped++;   // FLAGGED: the library grows one family at a time

...
- `src/characters/enemyMotor.js:374` - FLAGGED, until target prediction ships). At zero the foe stops.
...
- `src/combat/formulas.js:78` - *  INTERIM_WEAPON ever baked. S3d's assignStartingGear mints its
- `src/combat/formulas.js:155` - *  FLAGGED: DFU's player arm takes the UNDEAD modifier while the
- `src/combat/formulas.js:399` - FLAGGED: CalculateProficiencyModifiers pends the career
- `src/combat/formulas.js:519` - *  MobileEnemy.Weight, class = female 240 / male 350. FLAGGED: the
...
- `src/scenes/cityGuards.js:365` - motor disables, TallyCrimeGuildRequirements(false, 5) FLAGGED,
- `src/scenes/droppedLoot.js:16` - FLAGGED loud: EXTERIOR pile persistence (the save arc snapshots
- `src/scenes/dungeonContext.js:198` - the chain lives, the motion is INTERIM (loud) until flats can tween.
- `src/scenes/dungeonContext.js:528` - index into the 18 careers) or the INTERIM default Warrior (16,
- `src/scenes/dungeonContext.js:534` - effects FLAGGED to the effect-library slice.
- `src/scenes/dungeonContext.js:561` - "database FLAGGED" narrows to the skill/loot message ids).
- `src/scenes/exterior.js:452` - S3d: the INTERIM dagger seed is the FALLBACK only - a character
- `src/scenes/exterior.js:458` - pre-chargen INTERIM entity (flat skills 30, maxHealth 50) for the
- `src/scenes/exterior.js:653` - (FLAGGED); swallowing the browser reload is not optional.
- `src/scenes/exterior.js:780` - (FLAGGED: the climate People table pends; the test city is
- `src/scenes/exterior.js:913` - FLAGGED here exactly as in world.js - no tile lookup yet).
...
- `src/scenes/world.js:553` - FLAGGED loud: the People faction rides the START location's
- `src/scenes/world.js:574` - S3d: the INTERIM dagger seed is the FALLBACK only - a character
- `src/scenes/world.js:580` - pre-chargen INTERIM entity (flat skills 30, maxHealth 50) for the
- `src/scenes/world.js:800` - (FLAGGED); swallowing the browser reload is not optional.
- `src/scenes/world.js:1105` - exemption (PlayerTileMapIndex == 0) is FLAGGED: this host
- `src/scenes/world.js:1320` - doors are the E-enter seam, not bashables - FLAGGED with the
- `src/scenes/worldModes.js:176` - say -> console FLAGGED: the interior HUD-text layer pends its arc.
- `src/scenes/worldModes.js:218` - if (!isShop(b.buildingType)) return;   // Library/Guild/Temple bookshelves + owned-house storage pend (FLAGGED)
- `src/scenes/worldModes.js:311` - Person placed in a dungeon is the quest machine's, FLAGGED
- `src/scenes/worldModes.js:351` - FLAGGED, each with the slice it waits on:
- `src/scenes/worldModes.js:370` - FactionData; the port's pre-chargen INTERIM entity does not, and
- `src/scenes/worldModes.js:413` - onTalk: () => townTalk?.say?.('You get no response.'),   // FLAGGED: TalkToStaticNPC pends the static-NPC conversation
- `src/scenes/worldModes.js:420` - FLAGGED by name in guildServiceFlow.SERVICE_DESTINATION.

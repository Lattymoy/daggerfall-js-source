...
- `src/scenes/dungeonContext.js:745` - index into the 18 careers) or the INTERIM default Warrior (16,
- `src/scenes/dungeonContext.js:751` - effects FLAGGED to the effect-library slice.
- `src/scenes/dungeonContext.js:761` - "database FLAGGED" narrows to the skill/loot message ids).
- `src/scenes/dungeonContext.js:1280` - onTeleport: () => hudText.add('(Recall pends in the standalone dungeon - the anchor machinery lives in the streaming ?world host)'),   // TP-slice INTERIM
- `src/scenes/dungeonContext.js:1316` - onTeleport INTERIM shape). Absent, the engine's dispatch
- `src/scenes/dungeonContext.js:3103` - PX3 FLAGGED: questMessages - the dungeon quest mount is
- `src/scenes/dungeonContext.js:3173` - rest-for-a-while. DFU's toggle-close binding is FLAGGED in
...
- `src/scenes/world.js:2369` - FLAGGED in systems/healthStatus.js (macro producers pend).
- `src/scenes/world.js:2462` - (FLAGGED); swallowing the browser reload is not optional.
- `src/scenes/world.js:2751` - slot. Dungeon-mode popups pend the dungeon overlay seam (FLAGGED:
- `src/scenes/world.js:3105` - PlayerGPS.GetCourtOfCurrentRegion is FLAGGED: the port has no
- `src/scenes/world.js:3468` - FLAGGED: the QuestComplete loot window pends the UI arc - the
- `src/scenes/world.js:3495` - castle interior (FLAGGED with the palace blocks).
- `src/scenes/world.js:4003` - exemption (PlayerTileMapIndex == 0) is FLAGGED: this host
- `src/scenes/world.js:4377` - doors are the E-enter seam, not bashables - FLAGGED with the
...
- `src/scenes/worldModes.js:3644` - too. FLAGGED: interior loot containers are the loot arc's -
- `src/scenes/worldModes.js:3786` - string is the seam that was a FLAGGED null until this slice,
- `src/scenes/worldModes.js:3885` - *  FLAGGED null this slice closed. */
- `src/scenes/worldModes.js:4223` - *  FLAGGED: pause-and-resume is the DFU behaviour and a
...
- `src/ui/enhancedMenu.js:1484` - FLAGGED: the rest of the keyboard. The wizard walks to `done` with
- `src/ui/enhancedMenu.js:1646` - if (action === 'delete') return;   // FLAGGED: no save manager yet

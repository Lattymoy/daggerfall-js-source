...
- `06-Systems/Weapon-Sheathing.md` - WEAPON SHEATHING (WS1, 2026-09-17, Mac: "Can we implement this for the morrowind model") - Greatness7's Weapon Sheathing 1.6: the seventy-one scabbards and three skeleton addons vendored (credit and no fee, the mod's own permission), the OpenMW mechanism ported for the port's Morrowind third-person body - the addon's fourteen bones injected into the skeleton by parent name, the sheathed weapon at its type's sheathing bone in its scabbard (shown while the hand is empty, the scabbard always), a quiver of the pack's arrows on a bow; `?` none - the Features row `mod-weapon-sheathing`, on by default. Not seen on a GPU here.
...
- `src/ui/enhancedMenu.js:2696` - FLAGGED: the rest of the keyboard. The wizard walks to `done` with

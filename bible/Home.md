...
- `01-Overview/Audit-51.md` - CLOSED 2026-09-02: 1:1 PARITY WITH BASIC ROADS, BY ORACLE - the mod's PaintPath transliterated from the C# into tools/roadsOracle.py and run over 651 cases against our painter: 639 differed (his tables pave water; rotate before the zero check), then 11 (the mod paints THROUGH the location rect and roads pave its padding - which is how his towns ring), then 0. The smoother ported as SmoothRoadsJob with one recorded divergence, the mod's transposed tile read. Data byte-identical to the dfmod.
...
- `01-Overview/Audit-49.md` - CLOSED 2026-09-02: ROADS 14-18, THE CALIBRATED NETWORK - half the real-map build was 44 island pairs paying three A* boxes each, fixed by a land flood-fill (7.25s -> 4.44s, network byte-identical); the through-road merge found invisible to every fixture and measured on the real map as small (junctions 7.0 -> 6.5%) with the hairpin win re-credited to the wide join; six mutants, four dead, two recorded.
- `01-Overview/Audit-49.md` - CLOSED 2026-09-02: THE LAB'S GRASS AND WEATHER IN THE GAME. Four findings around the verbatim text: F1 the blades were CULLED - the lab never enables CULL_FACE, the renderer does every frame, so every blade whose winding faced away vanished and turning changed which half of the sward was there; F2 the scatter did not follow the floating origin and jumped a pixel at every boundary; F3 an archive the exterior host had uploaded came back cached and grew no grass; F4 uWind was passed without the lab's gust. Verified: both lab programs declare what they use (the sweep composes HEAD + FIELD + body), both draws restore the state they touched, both mark the EV6 seam. Costs stated: 169ms per re-place on the main thread; a million blades unseen by the harness.

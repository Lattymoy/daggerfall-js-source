...
- `src/scenes/worldModes.js:1360` - fixed ship scenes and stays FLAGGED, so those buttons keep
- `src/scenes/worldModes.js:1389` - H3: the sell price, which was FLAGGED at zero because it needs
- `src/scenes/worldModes.js:1567` - FactionData; the port's pre-chargen INTERIM entity does not, and
- `src/scenes/worldModes.js:1623` - FLAGGED by name in guildServiceFlow.SERVICE_DESTINATION.
- `src/scenes/worldModes.js:1681` - skip, the refusal line. Only the destination was a FLAGGED null,
- `src/scenes/worldModes.js:1852` - FLAGGED: a coven's failure spawns daedric foes ON YOU
- `src/scenes/worldModes.js:1856` - if (r.spawnFoes) console.warn('[summon] a coven failure owes you daedra; the interior has no foe pool (FLAGGED)');
- `src/scenes/worldModes.js:1923` - been a FLAGGED null since G3.
- `src/scenes/worldModes.js:1965` - destination has been a FLAGGED null since G3.
- `src/scenes/worldModes.js:1984` - been FLAGGED nulls since G3.
- `src/scenes/worldModes.js:2089` - with the trade window's own mode flow, the same INTERIM the
- `src/scenes/worldModes.js:2314` - first (owned houses and quest buildings FLAGGED/seamed per
- `src/scenes/worldModes.js:2322` - (:515). X3 wired the Open-spell bypass (:519-520). FLAGGED: the bash arms with
- `src/scenes/worldModes.js:3123` - too. FLAGGED: interior loot containers are the loot arc's -
- `src/scenes/worldModes.js:3265` - string is the seam that was a FLAGGED null until this slice,
- `src/scenes/worldModes.js:3364` - *  FLAGGED null this slice closed. */
- `src/scenes/worldModes.js:3702` - *  FLAGGED: pause-and-resume is the DFU behaviour and a
- `src/scenes/worldModes.js:4131` - *  FLAGGED: this host has no interior enemy pool (the Q4-v flag on

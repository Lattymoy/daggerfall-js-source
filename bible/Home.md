...
Regenerated by the 2026-08-13 DFU parity audit (F1-F17). Every row
below is a live INTERIM/FLAGGED/PENDING site in src; the code comment
at each site is the authority. The prior 07-06f list's stale rows
(chargen, spellbook, S10 costs, the Warrior-16 default, input map,
GetMonsterCareerTemplate, E3 entity wiring) are gone from the code
and so from this list. `src/render/characterSprite.js:73-76` (FP
viewmodel framing constants) remains open to nudging from live play.
...
- `src/scenes/dungeonContext.js:305` - index into the 18 careers) or the INTERIM default Warrior (16,
- `src/scenes/dungeonContext.js:311` - effects FLAGGED to the effect-library slice.
- `src/scenes/dungeonContext.js:414` - FLAGGED: DFU recomputes per-effect via the cost tables (that
- `src/scenes/dungeonContext.js:416` - FLAGGED to the effect library (caster-only buffs, touch, areas).
- `src/scenes/dungeonContext.js:569` - 129; the inventory/equip UI pends - the INTERIM dagger note
- `src/scenes/dungeonContext.js:994` - actions) is FLAGGED - the player snapshot only.
...
- `src/ui/hud.js:83` - [art.fatigue, 1, 1],                                    // FLAGGED: fatigue stat pends

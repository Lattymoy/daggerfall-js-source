...
## Open flags (regenerated 2026-08-17, E2)
...
- `src/scenes/exterior.js:283` - it). say -> console FLAGGED: this host has no HUD-text layer yet.
- `src/scenes/exterior.js:337` - day-skip is a no-op FLAGGED until the shared calendar lands).
- `src/scenes/exterior.js:471` - (FLAGGED: the climate People table pends; the test city is
- `src/scenes/exterior.js:540` - FLAGGED here exactly as in world.js - no tile lookup yet).
...
- `src/scenes/world.js:421` - it). say -> console FLAGGED: this host has no HUD-text layer yet.
- `src/scenes/world.js:425` - FLAGGED loud: the People faction rides the START location's
- `src/scenes/world.js:507` - day-skip is a no-op FLAGGED until the shared calendar lands).
- `src/scenes/world.js:767` - exemption (PlayerTileMapIndex == 0) is FLAGGED: this host
- `src/scenes/world.js:946` - doors are the E-enter seam, not bashables - FLAGGED with the
...
- `src/systems/shopStock.js:94` - if (group === 'MagicItems') continue;   // INTERIM loud (the loot MI interim)

...
- `src/scenes/dungeonContext.js:540` - index into the 18 careers) or the INTERIM default Warrior (16,
- `src/scenes/dungeonContext.js:546` - effects FLAGGED to the effect-library slice.
- `src/scenes/dungeonContext.js:573` - "database FLAGGED" narrows to the skill/loot message ids).
...
- `src/ui/nativeInventory.js:111` - *  list to the cart's own 750kg collection. FLAGGED: Transportation
- `src/ui/nativeInventory.js:125` - *  is 0 until those effect channels exist (FLAGGED). Exported so the
- `src/ui/nativeInventory.js:203` - *  ClickAnywhereToClose message box. FLAGGED loud, exactly as the
- `src/ui/nativeInventory.js:369` - field is INTERIM-pending with the local Remove split.
- `src/ui/nativeInventory.js:510` - FLAGGED: the hover fill is the mouse-move seam's, not this

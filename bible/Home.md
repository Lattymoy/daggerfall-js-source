...
- `src/scenes/dungeonContext.js:2739` - rest-for-a-while. DFU's toggle-close binding is FLAGGED in
...
- `src/scenes/exterior.js:1010` - (FLAGGED); swallowing the browser reload is not optional.
- `src/scenes/exterior.js:1305` - (FLAGGED: the climate People table pends; the test city is
- `src/scenes/exterior.js:1462` - FLAGGED here exactly as in world.js - no tile lookup yet).
- `src/scenes/exterior.js:1803` - ground (FLAGGED with world.js's same gap), so Detect Treasure
...
- `src/scenes/world.js:1637` - FLAGGED: the BUILDING arm (Respawner :559-567, StartBuilding-
- `src/scenes/world.js:1963` - (FLAGGED); swallowing the browser reload is not optional.
- `src/scenes/world.js:2242` - slot. Dungeon-mode popups pend the dungeon overlay seam (FLAGGED:
- `src/scenes/world.js:2415` - if (sd.siteType === SITE_TYPES.Building) return false;   // FLAGGED above - the building arm pends
- `src/scenes/world.js:2507` - PlayerGPS.GetCourtOfCurrentRegion is FLAGGED: the port has no
- `src/scenes/world.js:2772` - FLAGGED: the QuestComplete loot window pends the UI arc - the
- `src/scenes/world.js:2799` - castle interior (FLAGGED with the palace blocks).
...
- `src/scenes/worldModes.js:3005` - too. FLAGGED: interior loot containers are the loot arc's -
- `src/scenes/worldModes.js:3147` - string is the seam that was a FLAGGED null until this slice,
- `src/scenes/worldModes.js:3246` - *  FLAGGED null this slice closed. */
- `src/scenes/worldModes.js:3584` - *  FLAGGED: pause-and-resume is the DFU behaviour and a
- `src/scenes/worldModes.js:4007` - *  FLAGGED: this host has no interior enemy pool (the Q4-v flag on

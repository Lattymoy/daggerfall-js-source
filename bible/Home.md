...
- `11-Multiplayer/` - co-op: the three locked decisions, the architecture, the arc (the design the ONLINE arc grows into - presence, then chat, then the room's memory (WORLD1), then the room's simulation (WORLD2), then the room's events (WORLD3) - `06-Systems/Online-Arc.md`)
...
`dungeonContext.js:1690` before the fix): bare hands are a NULL weapon

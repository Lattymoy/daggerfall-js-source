...
## Open flags (regenerated 2026-08-17, U10 chargen art)

Regenerated at the U10 close: the chargen text-panel INTERIM and the
"art FLAGGED pending name verification" sentence are DELETED - the
real CHAR0*/PICK00/TMAP00 screens ship - and the U8f/U8g "Breton male
0 INTERIM until chargen" notes go with them. New rows: the
DaggerfallMessageBox frame the gender screen waits on, and the
biography/reflexes/custom-class states the flow has no place for yet.
Line numbers refreshed. Every row below is a live
...
- `src/scenes/dungeonContext.js:163` - the chain lives, the motion is INTERIM (loud) until flats can tween.
- `src/scenes/dungeonContext.js:488` - index into the 18 careers) or the INTERIM default Warrior (16,
- `src/scenes/dungeonContext.js:494` - effects FLAGGED to the effect-library slice.
- `src/scenes/dungeonContext.js:508` - "database FLAGGED" narrows to the skill/loot message ids).
- `src/scenes/dungeonContext.js:642` - drained strength lowers the ceiling). INTERIM (loud): the
- `src/scenes/dungeonContext.js:761` - FLAGGED: DFU recomputes per-effect via the cost tables (that
- `src/scenes/dungeonContext.js:763` - FLAGGED to the effect library (caster-only buffs, touch, areas).
- `src/scenes/dungeonContext.js:975` - 129; the inventory/equip UI pends - the INTERIM dagger note
- `src/scenes/dungeonContext.js:1713` - actions) is FLAGGED - the player snapshot only.
- `src/scenes/exterior.js:321` - S3d: the INTERIM dagger seed is the FALLBACK only - a character
- `src/scenes/exterior.js:327` - pre-chargen INTERIM entity (flat skills 30, maxHealth 50) for the
- `src/scenes/exterior.js:402` - day-skip is a no-op FLAGGED until the shared calendar lands).
- `src/scenes/exterior.js:468` - (FLAGGED); swallowing the browser reload is not optional.
- `src/scenes/exterior.js:568` - (FLAGGED: the climate People table pends; the test city is
- `src/scenes/exterior.js:637` - FLAGGED here exactly as in world.js - no tile lookup yet).
...
- `src/scenes/world.js:437` - FLAGGED loud: the People faction rides the START location's
- `src/scenes/world.js:450` - S3d: the INTERIM dagger seed is the FALLBACK only - a character
- `src/scenes/world.js:456` - pre-chargen INTERIM entity (flat skills 30, maxHealth 50) for the
- `src/scenes/world.js:572` - day-skip is a no-op FLAGGED until the shared calendar lands).
- `src/scenes/world.js:627` - (FLAGGED); swallowing the browser reload is not optional.
- `src/scenes/world.js:863` - exemption (PlayerTileMapIndex == 0) is FLAGGED: this host
- `src/scenes/world.js:1062` - doors are the E-enter seam, not bashables - FLAGGED with the
...
- `src/ui/chargenArt.js:12` - DaggerfallMessageBox - its frame art is FLAGGED below)
- `src/ui/chargenArt.js:20` - FLAGGED loud:
- `src/ui/chargenArt.js:204` - FLAGGED: DaggerfallMessageBox frame art pends its own port.

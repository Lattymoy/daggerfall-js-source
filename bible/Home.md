...
- `01-Overview/Audit-28.md` - IN PROGRESS 2026-08-30: THE POST-26 SWEEP. One reader against the DFU checkout over the 240 modules changed since AUDIT 26. W1 - the settings sweep: every `stored` key has a DFU consumer, so the stored list IS a list of unported laws; four were ported WITH THE SETTING NAMED IN THEIR OWN COMMENT and never read (QuestRumorWeight, DisableEnemyDeathAlert, DungeonAmbientLightScale, NightAmbientLightScale) - closed, 6 pins, 5 mutants killed; 35 gameplay keys classified, the default-ON absentees queued for W2: W2a THE ARROW COUNTER shipped (DaggerfallHUD :270-292, never drawn; found on the way that the interior frame handed the classic HUD no font), W2b MELEE FRIENDLY PROTECTION shipped (WeaponManager :930-944 - a pacified foe beside a hostile one took the swing); W2c THE EXIT-DOOR WAGON PROMPT shipped (PlayerActivate :649-664 - the producer inventorySession's NT3 note had been waiting for); W2d THE NEAR-DEATH WARNING shipped (HUDFlickerController + the three curves, whole). The default-ON queue is CLOSED. W3a HelmAndShieldMaterialDisplay (four values, read live) and W3b AlternateRandomEnemySelection (AddRandomRDBEnemy, the alternate arm the port never had) shipped; W3c RandomDungeonTextures shipped whole (the five modes, the main-story gate, the alternate table). W4 SmallerDungeons shipped whole (the five-block plus, the quest's frozen state, the load-time warp). W5 ToggleSneak shipped (the press-flip capture in the motor). W6 ShopQualityHUDDelay shipped (the HUD line's duration). W7 MouseLookSmoothingFactor shipped (the look filter, all four hosts - DFU's default look is smoothed and the port's now is too). W8 MovementAcceleration shipped (InputManager's axes, all four producers). W9 CameraRecoilStrength shipped (CameraRecoiler whole - the default reel on a hit). W10 HeadBobbing shipped (HeadBobber whole - the default walk bob and nod). W11 WeaponAttackThreshold (THE GATE WAS TEN TIMES TOO STRICT: the setting ships 0.005, the port used the 0.05 field default) + WeaponSwingMode shipped; WeaponSensitivity refuted (DFU reads it nowhere). W12 BowDrawback shipped (the draw-and-hold path the machine had waited for). W13 Handedness shipped (the left-hand flip) - the camera-feel and weapon-input set is CLOSED. THE SETTINGS SWEEP IS COMPLETE (the closing sweep ran W1's shape over the whole tree: clean, two stored keys recorded - MeleeAttackDetection's Quality arm queued, SoundFont has no file to load). Remaining: the six UI-only keys (the UI arc's), MeleeAttackDetection Quality, then lens 3, the post-26 module reads. Refutations recorded.
...
- `src/scenes/dungeonContext.js:781` - index into the 18 careers) or the INTERIM default Warrior (16,
- `src/scenes/dungeonContext.js:1356` - FS1 - FLAGGED (THE FOUR HOSTS RULE): THE ENCHANT CTX IS NOT
- `src/scenes/dungeonContext.js:1391` - onTeleport: () => hudText.add('(Recall pends in the standalone dungeon - the anchor machinery lives in the streaming ?world host)'),   // TP-slice INTERIM
- `src/scenes/dungeonContext.js:1427` - onTeleport INTERIM shape). Absent, the engine's dispatch
- `src/scenes/dungeonContext.js:3278` - PX3 FLAGGED: questMessages - the dungeon quest mount is
- `src/scenes/dungeonContext.js:3386` - rest-for-a-while. DFU's toggle-close binding is FLAGGED in
...
- `src/scenes/exterior.js:873` - onTeleport: () => townTalk.say('(Recall pends here - the anchor machinery lives in the streaming ?world host)'),   // TP-slice INTERIM
- `src/scenes/exterior.js:1116` - PX3 FLAGGED: questMessages - this test host mounts no quest
- `src/scenes/exterior.js:1461` - (FLAGGED: the climate People table pends; the test city is
...
- `src/scenes/world.js:3022` - slot. Dungeon-mode popups pend the dungeon overlay seam (FLAGGED:
- `src/scenes/world.js:3817` - castle interior (FLAGGED with the palace blocks).
- `src/scenes/world.js:4800` - building doors are the E-enter seam, not bashables - FLAGGED
- `src/scenes/worldModes.js:1188` - at the concluded deal (:1036-1051). FLAGGED: the equipped test
- `src/scenes/worldModes.js:1203` - G4: THE GUILD STORE ARM. This had been a FLAGGED null since
- `src/scenes/worldModes.js:1412` - is the quest machine's, FLAGGED with it.
- `src/scenes/worldModes.js:1417` - FLAGGED, above ground only, each with the DFU line it owes:
- `src/scenes/worldModes.js:1608` - CW1 retired the FLAGGED list that lived here - every arm it
- `src/scenes/worldModes.js:1848` - fixed ship scenes and stays FLAGGED, so those buttons keep
- `src/scenes/worldModes.js:1880` - H3: the sell price, which was FLAGGED at zero because it needs
- `src/scenes/worldModes.js:2101` - FactionData; the port's pre-chargen INTERIM entity does not, and
- `src/scenes/worldModes.js:2223` - skip, the refusal line. Only the destination was a FLAGGED null,
- `src/scenes/worldModes.js:2498` - been a FLAGGED null since G3.
- `src/scenes/worldModes.js:2543` - destination has been a FLAGGED null since G3.
- `src/scenes/worldModes.js:2562` - been FLAGGED nulls since G3.
- `src/scenes/worldModes.js:2667` - with the trade window's own mode flow, the same INTERIM the
- `src/scenes/worldModes.js:2892` - first (owned houses and quest buildings FLAGGED/seamed per
- `src/scenes/worldModes.js:2904` - FLAGGED, and narrowed to what is actually missing: the two BASH
- `src/scenes/worldModes.js:2966` - the FLAGGED note above), so it starts false.
- `src/scenes/worldModes.js:4183` - string is the seam that was a FLAGGED null until this slice,
- `src/scenes/worldModes.js:4282` - *  FLAGGED null this slice closed. */
- `src/scenes/worldModes.js:4620` - *  FLAGGED: pause-and-resume is the DFU behaviour and a
...
- `src/ui/enhancedMenu.js:1674` - FLAGGED: the rest of the keyboard. The wizard walks to `done` with
- `src/ui/enhancedMenu.js:1845` - if (action === 'delete') return;   // FLAGGED: no save manager yet

...
## Open flags (regenerated 2026-08-18, AUDIT 17h)

Regenerated at the AUDIT 17h close: no flag retired or added - the audit shipped fixes, not slices. Previously, at the U13 close: chargenArt's "reflexes ... not in the
flow yet" sentence is DELETED - the screen ships - and the biography's
backstory/reputation-box rows go with it. What remains of the chargen
arc is the custom-class screen and the wizard ORDER. Line numbers refreshed. Every row below is a live
...
- `src/scenes/dungeonContext.js:775` - FLAGGED: DFU recomputes per-effect via the cost tables (that
- `src/scenes/dungeonContext.js:777` - FLAGGED to the effect library (caster-only buffs, touch, areas).
- `src/scenes/dungeonContext.js:989` - 129; the inventory/equip UI pends - the INTERIM dagger note
- `src/scenes/dungeonContext.js:1727` - actions) is FLAGGED - the player snapshot only.
...
- `src/ui/chargen.js:50` - SelectBiographyMethod, BiographyQuestions, SelectName...). FLAGGED:
- `src/ui/chargenArt.js:26` - FLAGGED loud: the CUSTOM-CLASS path has no screen yet
- `src/ui/chargenArt.js:388` - AUDIT 17g FLAGGED: the scrollbar THUMB does not draw. Its geometry

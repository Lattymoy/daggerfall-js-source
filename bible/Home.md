...
- `src/scenes/worldModes.js:286` - FLAGGED: factionId is not threaded here yet, so a GuildHall reads
- `src/systems/songManager.js:290` - temple falls to Interior rather than inventing an alignment. FLAGGED.
- `src/scenes/worldModes.js:75` - say -> console FLAGGED: the interior HUD-text layer pends its arc.
- `src/scenes/worldModes.js:107` - if (!isShop(b.buildingType)) return;   // Library/Guild/Temple bookshelves + owned-house storage...

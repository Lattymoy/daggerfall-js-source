...
- `02-Formats/Morrowind-Rules.md` - REFERENCE ONLY, nothing implemented. What OpenMW actually does for the first-person body, cited file:line, after the first Morrowind import arc was reverted whole for shipping three fixes built on guesses. Six corrections are recorded there, including the one that matters: a first-person body part is a RECORD whose id ends in `1st`, not a mesh filename.

...
- `src/scenes/dungeonContext.js:971` - exterior spell paths are FLAGGED with their own hosts.
- `src/scenes/dungeonContext.js:1133` - 129; the inventory/equip UI pends - the INTERIM dagger note
...
- `src/scenes/exterior.js:656` - (FLAGGED: the climate People table pends; the test city is
- `src/scenes/exterior.js:775` - FLAGGED here exactly as in world.js - no tile lookup yet).
...
- `src/scenes/world.js:986` - exemption (PlayerTileMapIndex == 0) is FLAGGED: this host
- `src/scenes/world.js:1197` - doors are the E-enter seam, not bashables - FLAGGED with the
- `src/scenes/worldModes.js:126` - say -> console FLAGGED: the interior HUD-text layer pends its arc.
- `src/scenes/worldModes.js:167` - if (!isShop(b.buildingType)) return;   // Library/Guild/Temple bookshelves + owned-house storage pend (FLAGGED)
- `src/scenes/worldModes.js:260` - Person placed in a dungeon is the quest machine's, FLAGGED
- `src/scenes/worldModes.js:300` - FLAGGED, each with the slice it waits on:
- `src/scenes/worldModes.js:319` - FactionData; the port's pre-chargen INTERIM entity does not, and
- `src/scenes/worldModes.js:362` - onTalk: () => townTalk?.say?.('You get no response.'),   // FLAGGED: TalkToStaticNPC pends the static-NPC conversation
- `src/scenes/worldModes.js:369` - FLAGGED by name in guildServiceFlow.SERVICE_DESTINATION.
...
AND ONE TOOL WHERE THERE WERE TWO. Both lanes noticed the open-flags
list rots on every host edit and both wrote a regenerator - AUDIT 21's
`tools/regenOpenFlags.mjs` (with a `--check` mode, and the guard test
that pins the list to it) and a second one on this branch, since
DELETED.
They disagreed about quoting: one truncates a long line to 78
characters with an ellipsis, the other does not, so running the wrong
one turned the guard red. The AUDIT 21 tool is the one the guard
pins, so the other is DELETED rather than kept as a second answer to
one question.

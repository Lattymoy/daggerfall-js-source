...
- `04-Characters/Characters-Arc.md` - ACTIVE. C2 shipped: exterior NPCs (faction rule verbatim, 76-NPC corpus) + townsfolk name banks (NameHelper over DFRandom, NameGen data committed). C3 shipped: dungeon enemies - classic tables + selection verbatim, standing billboards (Privateer's 42-enemy pin; city crawl 128). PIVOTED to the player paperdoll: townsfolk stay classic billboards; C4's render path + vendored rig serve the paperdoll (C5 data -> C6 authored pieces -> C7 live rotating viewport). C6a-h shipped: art addressing + extractor, then THE REDESIGN - the body IS the classic body (sprite reliefs in shared canvas space, exact-x identity witness, distance-field depth), backs derived IN-HOUSE (C6i: ramp relight from our own geometry + procedural detail off the front's landmarks; no AI generation - project policy). ARCHITECTURE RESET (C6j, Mac): the VOXLIGHT RIG is the geometry again (real 3D, paperdoll stance on DAGGER_SPEC), the classic sprite is the 1:1 shading (front projection per face); relief construction retires to the texture toolkit. Next: our own textures from the classic data for the unseen faces, pieces re-seat, then C7 viewport. No AI generation - project policy.

...
- `src/scenes/world.js:1333` - S3d: the INTERIM dagger seed is the FALLBACK only - a character
- `src/scenes/world.js:1339` - pre-chargen INTERIM entity (flat skills 30, maxHealth 50) for the
- `src/scenes/world.js:1744` - *  through the `if`. This host's FLAGGED note (below, at the mode
- `src/scenes/world.js:2786` - landing stands in for it - FLAGGED for the first session with
- `src/scenes/world.js:4152` - slot. Dungeon-mode popups pend the dungeon overlay seam (FLAGGED:
- `src/scenes/world.js:5212` - ROAD-B CLOSED THE FLAG THAT STOOD HERE. It read: "FLAGGED: DFU's
- `src/scenes/world.js:6399` - building doors are the E-enter seam, not bashables - FLAGGED
...
- `src/ui/enhancedMenu.js:1734` - FLAGGED: the rest of the keyboard. The wizard walks to `done` with
- `src/ui/enhancedMenu.js:1905` - if (action === 'delete') return;   // FLAGGED: no save manager yet

...
- `src/scenes/exterior.js:820` - (FLAGGED: the climate People table pends; the test city is
- `src/scenes/exterior.js:962` - FLAGGED here exactly as in world.js - no tile lookup yet).
...
- `src/scenes/world.js:603` - FLAGGED loud: the People faction rides the START location's
- `src/scenes/world.js:628` - S3d: the INTERIM dagger seed is the FALLBACK only - a character
- `src/scenes/world.js:634` - pre-chargen INTERIM entity (flat skills 30, maxHealth 50) for the
- `src/scenes/world.js:1141` - (FLAGGED); swallowing the browser reload is not optional.
- `src/scenes/world.js:1354` - slot. Dungeon-mode popups pend the dungeon overlay seam (FLAGGED:
- `src/scenes/world.js:1446` - FLAGGED: the QuestComplete loot window pends the UI arc - the
- `src/scenes/world.js:1465` - castle interior (FLAGGED with the palace blocks).
- `src/scenes/world.js:1705` - exemption (PlayerTileMapIndex == 0) is FLAGGED: this host
- `src/scenes/world.js:1971` - doors are the E-enter seam, not bashables - FLAGGED with the
...
- `src/scenes/worldModes.js:467` - FLAGGED, each with the slice it waits on:
- `src/scenes/worldModes.js:486` - FactionData; the port's pre-chargen INTERIM entity does not, and
- `src/scenes/worldModes.js:530` - onTalk: () => townTalk?.say?.('You get no response.'),   // FLAGGED: TalkToStaticNPC pends the static-NPC conversation
- `src/scenes/worldModes.js:537` - FLAGGED by name in guildServiceFlow.SERVICE_DESTINATION.

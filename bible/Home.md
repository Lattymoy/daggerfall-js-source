...
- `src/combat/formulas.js:409` - FLAGGED: CalculateProficiencyModifiers pends the career
- `src/combat/formulas.js:529` - *  MobileEnemy.Weight, class = female 240 / male 350. FLAGGED: the
...
- `src/scenes/dungeonContext.js:199` - the chain lives, the motion is INTERIM (loud) until flats can tween.
- `src/scenes/dungeonContext.js:539` - index into the 18 careers) or the INTERIM default Warrior (16,
- `src/scenes/dungeonContext.js:545` - effects FLAGGED to the effect-library slice.
- `src/scenes/dungeonContext.js:572` - "database FLAGGED" narrows to the skill/loot message ids).
...
- `src/scenes/exterior.js:789` - (FLAGGED: the climate People table pends; the test city is
- `src/scenes/exterior.js:924` - FLAGGED here exactly as in world.js - no tile lookup yet).
...
- `src/scenes/world.js:560` - FLAGGED loud: the People faction rides the START location's
- `src/scenes/world.js:581` - S3d: the INTERIM dagger seed is the FALLBACK only - a character
- `src/scenes/world.js:587` - pre-chargen INTERIM entity (flat skills 30, maxHealth 50) for the
- `src/scenes/world.js:971` - (FLAGGED); swallowing the browser reload is not optional.
- `src/scenes/world.js:1319` - exemption (PlayerTileMapIndex == 0) is FLAGGED: this host
- `src/scenes/world.js:1555` - doors are the E-enter seam, not bashables - FLAGGED with the
...
- `src/systems/effects.js:676` - out.skipped++;   // FLAGGED: the library grows one family at a time
...
- `src/ui/nativeInventory.js:109` - *  list to the cart's own 750kg collection. FLAGGED: Transportation
- `src/ui/nativeInventory.js:123` - *  is 0 until those effect channels exist (FLAGGED). Exported so the
- `src/ui/nativeInventory.js:200` - *  ClickAnywhereToClose message box. FLAGGED loud, exactly as the
- `src/ui/nativeInventory.js:360` - field is INTERIM-pending with the local Remove split.
- `src/ui/nativeInventory.js:500` - FLAGGED: the hover fill is the mouse-move seam's, not this

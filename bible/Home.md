...
- `src/scenes/dungeonContext.js:240` - the chain lives, the motion is INTERIM (loud) until flats can tween.
- `src/scenes/dungeonContext.js:682` - index into the 18 careers) or the INTERIM default Warrior (16,
- `src/scenes/dungeonContext.js:688` - effects FLAGGED to the effect-library slice.
- `src/scenes/dungeonContext.js:698` - "database FLAGGED" narrows to the skill/loot message ids).
- `src/scenes/dungeonContext.js:1183` - onTeleport: () => hudText.add('(Recall pends in the standalone dungeon - the anchor machinery lives in the streaming ?world host)'),   // TP-slice INTERIM
- `src/scenes/dungeonContext.js:2789` - rest-for-a-while. DFU's toggle-close binding is FLAGGED in
- `src/scenes/exterior.js:546` - S3d: the INTERIM dagger seed is the FALLBACK only - a character
- `src/scenes/exterior.js:552` - pre-chargen INTERIM entity (flat skills 30, maxHealth 50) for the
- `src/scenes/exterior.js:780` - onTeleport: () => townTalk.say('(Recall pends here - the anchor machinery lives in the streaming ?world host)'),   // TP-slice INTERIM
- `src/scenes/exterior.js:1024` - (FLAGGED); swallowing the browser reload is not optional.
- `src/scenes/exterior.js:1321` - (FLAGGED: the climate People table pends; the test city is
- `src/scenes/exterior.js:1480` - FLAGGED here exactly as in world.js - no tile lookup yet).
- `src/scenes/exterior.js:1821` - ground (FLAGGED with world.js's same gap), so Detect Treasure
...
- `src/scenes/world.js:766` - FLAGGED loud: the People faction rides the START location's
- `src/scenes/world.js:801` - S3d: the INTERIM dagger seed is the FALLBACK only - a character
- `src/scenes/world.js:807` - pre-chargen INTERIM entity (flat skills 30, maxHealth 50) for the
- `src/scenes/world.js:1243` - none); the dungeon-mode ctx is dungeonContext's to mount - FLAGGED
- `src/scenes/world.js:1701` - FLAGGED: the BUILDING arm (Respawner :559-567, StartBuilding-
- `src/scenes/world.js:2104` - (FLAGGED); swallowing the browser reload is not optional.
- `src/scenes/world.js:2383` - slot. Dungeon-mode popups pend the dungeon overlay seam (FLAGGED:
- `src/scenes/world.js:2556` - if (sd.siteType === SITE_TYPES.Building) return false;   // FLAGGED above - the building arm pends
- `src/scenes/world.js:2648` - PlayerGPS.GetCourtOfCurrentRegion is FLAGGED: the port has no
- `src/scenes/world.js:2926` - FLAGGED: the QuestComplete loot window pends the UI arc - the
- `src/scenes/world.js:2953` - castle interior (FLAGGED with the palace blocks).
- `src/scenes/world.js:3409` - exemption (PlayerTileMapIndex == 0) is FLAGGED: this host
- `src/scenes/world.js:3746` - doors are the E-enter seam, not bashables - FLAGGED with the
- `src/scenes/world.js:3819` - (FLAGGED: exterior corpse containers are the loot arc's), so
...
- `src/scenes/worldModes.js:669` - if (!isShop(b.buildingType)) return;   // Library/Guild/Temple bookshelves + owned-house storage pend (FLAGGED)
- `src/scenes/worldModes.js:678` - FLAGGED to the crime arc, as the Ledger records.
- `src/scenes/worldModes.js:747` - at the concluded deal (:1036-1051). FLAGGED: the equipped test
- `src/scenes/worldModes.js:762` - G4: THE GUILD STORE ARM. This had been a FLAGGED null since
- `src/scenes/worldModes.js:800` - credit instead. FLAGGED: there is nowhere to cash one yet, so
- `src/scenes/worldModes.js:943` - is the quest machine's, FLAGGED with it.
- `src/scenes/worldModes.js:948` - FLAGGED, above ground only, each with the DFU line it owes:
- `src/scenes/worldModes.js:1042` - trade window in Sell mode. The BANKING arm stays FLAGGED below;
- `src/scenes/worldModes.js:1131` - FLAGGED, each with the slice it waits on:
- `src/scenes/worldModes.js:1319` - fixed ship scenes and stays FLAGGED, so those buttons keep
- `src/scenes/worldModes.js:1346` - H3: the sell price, which was FLAGGED at zero because it needs
- `src/scenes/worldModes.js:1524` - FactionData; the port's pre-chargen INTERIM entity does not, and
- `src/scenes/worldModes.js:1578` - FLAGGED by name in guildServiceFlow.SERVICE_DESTINATION.
- `src/scenes/worldModes.js:1636` - skip, the refusal line. Only the destination was a FLAGGED null,
- `src/scenes/worldModes.js:1807` - FLAGGED: a coven's failure spawns daedric foes ON YOU
- `src/scenes/worldModes.js:1811` - if (r.spawnFoes) console.warn('[summon] a coven failure owes you daedra; the interior has no foe pool (FLAGGED)');
- `src/scenes/worldModes.js:1878` - been a FLAGGED null since G3.
- `src/scenes/worldModes.js:1920` - destination has been a FLAGGED null since G3.
- `src/scenes/worldModes.js:1939` - been FLAGGED nulls since G3.
- `src/scenes/worldModes.js:2042` - with the trade window's own mode flow, the same INTERIM the
- `src/scenes/worldModes.js:2267` - first (owned houses and quest buildings FLAGGED/seamed per
- `src/scenes/worldModes.js:2275` - (:515). X3 wired the Open-spell bypass (:519-520). FLAGGED: the bash arms with
- `src/scenes/worldModes.js:3067` - too. FLAGGED: interior loot containers are the loot arc's -
- `src/scenes/worldModes.js:3209` - string is the seam that was a FLAGGED null until this slice,
- `src/scenes/worldModes.js:3308` - *  FLAGGED null this slice closed. */
- `src/scenes/worldModes.js:3646` - *  FLAGGED: pause-and-resume is the DFU behaviour and a
- `src/scenes/worldModes.js:4075` - *  FLAGGED: this host has no interior enemy pool (the Q4-v flag on
...
- `src/systems/banking.js:642` - FLAGGED, with the slices they wait on:

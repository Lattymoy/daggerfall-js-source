...
- `src/scenes/worldModes.js:1042` - a castle reads as a plain dungeon interior. FLAGGED.
...
- `src/systems/chargen.js:149` - spendPoolLowest(stats, STAT_KEYS, bonusPool);                        // INTERI...
...
- `src/ui/inventory.js:13` - Enter readies one (retires ?spell). INTERIM loud: with no
- `src/ui/inventory.js:92` - /** The known list: entity.spells when it exists; the INTERIM fallback
...
- `src/ui/nativeInventory.js:101` - *  list to the cart's own 750kg collection. FLAGGED: Transportation
- `src/ui/nativeInventory.js:115` - *  is 0 until those effect channels exist (FLAGGED). Exported so the
- `src/ui/nativeInventory.js:192` - *  ClickAnywhereToClose message box. FLAGGED loud, exactly as the
- `src/ui/nativeInventory.js:450` - FLAGGED: the hover fill is the mouse-move seam's, not this

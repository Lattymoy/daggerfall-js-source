...
- `08-Audio/Audio.md` - ACTIVE. A1 + A2 (2026-08-16: action PlaySound on every Play, torch Burning loops at 5m linear/0.7 via the new loop3d engine seam, animal random barks on the classic rand()<=100 cadence at 19.2m - dungeon-scoped) + A3 SCENE AMBIENCE (2026-08-16: AmbientEffectsPlayer verbatim - the 14 dungeon one-shots somewhere-around on the scene's serialized 5/28 waits + classic-cadence water/bubbles, the weather/time presets in BOTH exterior scenes on 5/25 with rain/crickets loops and horizon storms, one shared ambient channel; building interiors silent, verbatim) SHIPPED + A4 EXTERIOR ANIMALS (2026-08-17: the shared PlayRandomlyIfPlayerNear pass - systems/animalAmbience.js, one implementation for dungeon + both exteriors incl. the streaming world's floating-origin sources; exterior torches SILENT verbatim - Burning is RDBLayout-only in DFU) SHIPPED; transition stingers CLOSED verbatim-N/A (2026-08-17: DFU plays nothing on enter/exit or ladders - no source law to port). Next: music strategy (Mac's call) - the queue's last row. + A6 PLAYER FOOTSTEPS (2026-08-20, the FS-slice: PlayerFootsteps.cs whole - the sound-set decision with the snow gate finally consuming isSnowFreeClimate, wood in buildings, stone in dungeons with the water arms, and the classic 2.5-unit alternating stride with the landing step and the boot swallow - driven by all four hosts off their live motors; the path/water tile arms and the mount gate are the struck row's residue) SHIPPED. + A5c FROM PLAY 2026-08-27: THE HMI CLOCK - the reader ran every song 8x fast (0x0D2's 480 taken for the time base; the tick is 1/BPM, 60 per quarter, as WildMIDI, foo_midi and DFU's own shipped .mid conversions all have it); SUNNYDAY's 2.76-hour loop closed the same day - foo_midi's delta shunt, read against the real bytes
- `08-Audio/Enhanced-Music-Arc.md` - ACTIVE (2026-08-27, Mac's call: the port's OWN score behind the enhanced toggle - a procedural system plus his own full tracks crossfaded over it). EM1 SHIPPED: the dungeon palette (a record), the composer (seeded, deterministic, the scheduler's own event shape on the HMI clock), and the door (the director's play sink asks the enhanced side first; null under the classic skin or for a place without a palette, so the classic path is untouched). Rendered to WAV through the real voice code; 7 pins, 4 mutants dead. + EM2a THE TITLE THEME (same day: Mac's main theme, MP3 in the repo as OURS, streamed through a media source with a ramped gain, played by the enhanced front door before any folder pick on a clock without the archives, pended for the gesture rule, faded under by the game's own music; 22/22 in the door probe with signal measured on the bus; eleven probes moved off networkidle, which a streaming element never reaches) + EM2b THE LEVEL AND THE SETTING (Mac: too low, and the slider must work - the track took the FM synth's 0.22 trim on top of the setting, a ninth of itself, so tracks take the setting alone; and MusicVolume was LIVE only in tier, read once per player, so settings.setValue now PUBLISHES every write and the service re-levels all three players on it; door probe 24/24 reading the gain node follow the slider) + EM2c THE DUNGEON TRACK (same day: Mac's dungeon track as the first place score, its key measured off the file - B aeolian, his to confirm - the composed piece under it IN THAT KEY at UNDERSCORE_TRIM on the scheduler's new master trim, tracks crossfading through a layer per play under one master; and 'fix it also' - replacement music packs take the setting alone; 4 mutants dead) + EM4 DANGER (same day: Mac's danger track; the signal is AreEnemiesNearby's own `detected && inSight`, the meter a slew with a six-second hold and hysteresis so it never flutters, the place's track crossfading into danger and back with the underscore quiet under it, reset on a new cue, reported from drawFoes and the exterior's two pools; 3 mutants dead) + THE PIVOT AND EM4b (same day, Mac: the imported tracks for the places are one long song, 'forget this entirely and completely focus on the procedural audio system' - dungeon.mp3 and danger.mp3 out, the tables empty by decision, the theme stays; and the stem idea built where the stems exist by construction: the dungeon palette grew TENSION and DRIVE layers composed for the whole loop and silent at rest, a mix law per layer, a mix gain per channel in the scheduler that the danger meter's slewed level drives every frame, change-guarded; heard on an offline calm-danger-calm render; 21 pins). Board: EM2 Mac's tracks (streamed, crossfaded, the piece underneath), EM3 the other places, EM4 danger/health dynamics, EM5 the lab.
...
- `src/scenes/dungeonContext.js:685` - index into the 18 careers) or the INTERIM default Warrior (16,
- `src/scenes/dungeonContext.js:691` - effects FLAGGED to the effect-library slice.
- `src/scenes/dungeonContext.js:701` - "database FLAGGED" narrows to the skill/loot message ids).
- `src/scenes/dungeonContext.js:1197` - onTeleport: () => hudText.add('(Recall pends in the standalone dungeon - the anchor machinery lives in the streaming ?world host)'),   // TP-slice INTERIM
- `src/scenes/dungeonContext.js:2812` - rest-for-a-while. DFU's toggle-close binding is FLAGGED in
...
- `src/scenes/shared.js:245` - *  The pre-chargen guard is load-bearing: playerEntity's INTERIM
- `src/scenes/shared.js:262` - *  mirrors motorStats (the INTERIM entity carries no stats). */
- `src/scenes/shared.js:367` - FLAGGED: the Skeleton's Key artifact (IsArtifact + world texture
- `src/scenes/townTalk.js:16` - FLAGGED loud: Info mode opens the same talk window (DFU routes
- `src/scenes/world.js:774` - FLAGGED loud: the People faction rides the START location's
- `src/scenes/world.js:809` - S3d: the INTERIM dagger seed is the FALLBACK only - a character
- `src/scenes/world.js:815` - pre-chargen INTERIM entity (flat skills 30, maxHealth 50) for the
- `src/scenes/world.js:1251` - none); the dungeon-mode ctx is dungeonContext's to mount - FLAGGED
- `src/scenes/world.js:1709` - FLAGGED: the BUILDING arm (Respawner :559-567, StartBuilding-
- `src/scenes/world.js:2112` - (FLAGGED); swallowing the browser reload is not optional.
- `src/scenes/world.js:2391` - slot. Dungeon-mode popups pend the dungeon overlay seam (FLAGGED:
- `src/scenes/world.js:2564` - if (sd.siteType === SITE_TYPES.Building) return false;   // FLAGGED above - the building arm pends
- `src/scenes/world.js:2656` - PlayerGPS.GetCourtOfCurrentRegion is FLAGGED: the port has no
- `src/scenes/world.js:2934` - FLAGGED: the QuestComplete loot window pends the UI arc - the
- `src/scenes/world.js:2961` - castle interior (FLAGGED with the palace blocks).
- `src/scenes/world.js:3417` - exemption (PlayerTileMapIndex == 0) is FLAGGED: this host
- `src/scenes/world.js:3756` - doors are the E-enter seam, not bashables - FLAGGED with the
- `src/scenes/world.js:3829` - (FLAGGED: exterior corpse containers are the loot arc's), so
...
- `src/scenes/worldModes.js:679` - FLAGGED to the crime arc, as the Ledger records.
- `src/scenes/worldModes.js:748` - at the concluded deal (:1036-1051). FLAGGED: the equipped test
- `src/scenes/worldModes.js:763` - G4: THE GUILD STORE ARM. This had been a FLAGGED null since
- `src/scenes/worldModes.js:801` - credit instead. FLAGGED: there is nowhere to cash one yet, so
- `src/scenes/worldModes.js:944` - is the quest machine's, FLAGGED with it.
- `src/scenes/worldModes.js:949` - FLAGGED, above ground only, each with the DFU line it owes:
- `src/scenes/worldModes.js:1043` - trade window in Sell mode. The BANKING arm stays FLAGGED below;
- `src/scenes/worldModes.js:1132` - FLAGGED, each with the slice it waits on:
- `src/scenes/worldModes.js:1361` - fixed ship scenes and stays FLAGGED, so those buttons keep
- `src/scenes/worldModes.js:1390` - H3: the sell price, which was FLAGGED at zero because it needs
- `src/scenes/worldModes.js:1568` - FactionData; the port's pre-chargen INTERIM entity does not, and
- `src/scenes/worldModes.js:1624` - FLAGGED by name in guildServiceFlow.SERVICE_DESTINATION.
- `src/scenes/worldModes.js:1682` - skip, the refusal line. Only the destination was a FLAGGED null,
- `src/scenes/worldModes.js:1853` - FLAGGED: a coven's failure spawns daedric foes ON YOU
- `src/scenes/worldModes.js:1857` - if (r.spawnFoes) console.warn('[summon] a coven failure owes you daedra; the interior has no foe pool (FLAGGED)');
- `src/scenes/worldModes.js:1887` - FLAGGED: a refusal owes 3-5 daedra at 8..64 units
- `src/scenes/worldModes.js:1890` - spawnRefusalFoes: () => console.warn('[summon] a refusal owes you daedra; the interior has no foe pool (FLAGGED)'),
- `src/scenes/worldModes.js:1949` - been a FLAGGED null since G3.
- `src/scenes/worldModes.js:1991` - destination has been a FLAGGED null since G3.
- `src/scenes/worldModes.js:2010` - been FLAGGED nulls since G3.
- `src/scenes/worldModes.js:2115` - with the trade window's own mode flow, the same INTERIM the
- `src/scenes/worldModes.js:2340` - first (owned houses and quest buildings FLAGGED/seamed per
- `src/scenes/worldModes.js:2348` - (:515). X3 wired the Open-spell bypass (:519-520). FLAGGED: the bash arms with
- `src/scenes/worldModes.js:3149` - too. FLAGGED: interior loot containers are the loot arc's -
- `src/scenes/worldModes.js:3291` - string is the seam that was a FLAGGED null until this slice,
- `src/scenes/worldModes.js:3390` - *  FLAGGED null this slice closed. */
- `src/scenes/worldModes.js:3728` - *  FLAGGED: pause-and-resume is the DFU behaviour and a
- `src/scenes/worldModes.js:4157` - *  FLAGGED: this host has no interior enemy pool (the Q4-v flag on
...
- `src/systems/save.js:132` - (playerEntity's INTERIM skills: 30) - spreading it threw.
...
- `src/systems/shopStock.js:184` - (DaggerfallLootDataTables.cs:61). The INTERIM skip predated
...
- `src/ui/daedraSummonedWindow.js:6` - refuse looses 3-5 daedra on the summoner's floor (:83-88 - FLAGGED
- `src/ui/daedraSummonedWindow.js:59` - *   spawnRefusalFoes() - optional; the daedric punishment (FLAGGED:
- `src/ui/enhancedMenu.js:739` - FLAGGED: the rest of the keyboard. The wizard walks to `done` with
- `src/ui/enhancedMenu.js:880` - if (action === 'delete') return;   // FLAGGED: no save manager yet
...
- `src/ui/spellbookWindow.js:873` - *  drawn as a flat bar in the panel's own brass - FLAGGED. */

...
- `04-Characters/Characters-Arc.md` - ACTIVE. C2 shipped: exterior NPCs (faction rule verbatim, 76-NPC corpus) + townsfolk name banks (NameHelper over DFRandom, NameGen data committed). C3 shipped: dungeon enemies - classic tables + selection verbatim, standing billboards (Privateer's 42-enemy pin; city crawl 128). PIVOTED to the player paperdoll: townsfolk stay classic billboards; C4's render path + vendored rig serve the paperdoll (C5 data -> C6 authored pieces -> C7 live rotating viewport). C6a-c shipped: paperdoll art addressing + reference extractor, the 1:1 sprite-shell piece method (geometric round-trip proof), and the plate cuirass ON the rig in-engine at measured torso fit. DECIDE-C1 review shot delivered. Next: C7 live rotating viewport.

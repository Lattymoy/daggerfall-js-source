...
## Open flags (regenerated 2026-08-17, U8f)
...
- `src/scenes/exterior.js:287` - it). say -> console FLAGGED: this host has no HUD-text layer yet.
- `src/scenes/exterior.js:311` - preloadPaperDollArt({ renderer, fetchBytes, palette });   // U8f: SCBG/BODY/FACE warm a...
- `src/scenes/exterior.js:353` - day-skip is a no-op FLAGGED until the shared calendar lands).
- `src/scenes/exterior.js:512` - (FLAGGED: the climate People table pends; the test city is
- `src/scenes/exterior.js:581` - FLAGGED here exactly as in world.js - no tile lookup yet).
...
- `src/scenes/world.js:425` - it). say -> console FLAGGED: this host has no HUD-text layer yet.
- `src/scenes/world.js:429` - FLAGGED loud: the People faction rides the START location's
- `src/scenes/world.js:440` - preloadPaperDollArt({ renderer, fetchBytes, palette });   // U8f: SCBG/BODY/FACE warm a...
- `src/scenes/world.js:523` - day-skip is a no-op FLAGGED until the shared calendar lands).
- `src/scenes/world.js:808` - exemption (PlayerTileMapIndex == 0) is FLAGGED: this host
- `src/scenes/world.js:1001` - doors are the E-enter seam, not bashables - FLAGGED with the
...
- `src/systems/equip.js:20` - FLAGGED: equip sounds (the audio seam pends), enchantment
...
- `src/ui/nativeInventory.js:132` - INTERIM info panel: name/weight/value (DFU's 1016 info text
- `src/ui/nativeInventory.js:154` - equip/use: FLAGGED - the paperdoll/use arcs pend
...
- `src/ui/paperDoll.js:19` - INTERIM loud: race Breton / male / face 0 until chargen fronts

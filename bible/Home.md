...
- `src/scenes/dungeonContext.js:222` - the chain lives, the motion is INTERIM (loud) until flats can tween.
...
- `src/scenes/exterior.js:504` - S3d: the INTERIM dagger seed is the FALLBACK only - a character
- `src/scenes/exterior.js:510` - pre-chargen INTERIM entity (flat skills 30, maxHealth 50) for the
- `src/scenes/exterior.js:659` - onTeleport: () => townTalk.say('(Recall pends here - the anchor machinery lives in the streaming ?world host)'),   // TP-slice INTERIM
- `src/scenes/exterior.js:924` - (FLAGGED); swallowing the browser reload is not optional.
- `src/scenes/exterior.js:1187` - (FLAGGED: the climate People table pends; the test city is
- `src/scenes/exterior.js:1339` - FLAGGED here exactly as in world.js - no tile lookup yet).
- `src/scenes/exterior.js:1674` - ground (FLAGGED with world.js's same gap), so Detect Treasure
...
- `src/scenes/world.js:698` - FLAGGED loud: the People faction rides the START location's
- `src/scenes/world.js:733` - S3d: the INTERIM dagger seed is the FALLBACK only - a character
- `src/scenes/world.js:739` - pre-chargen INTERIM entity (flat skills 30, maxHealth 50) for the
- `src/scenes/world.js:1106` - none); the dungeon-mode ctx is dungeonContext's to mount - FLAGGED
- `src/scenes/world.js:1109` - stay the E1 FLAGGED seams no host computes.
- `src/scenes/world.js:1355` - FLAGGED: the BUILDING arm (Respawner :559-567, StartBuilding-
- `src/scenes/world.js:1710` - (FLAGGED); swallowing the browser reload is not optional.
- `src/scenes/world.js:1979` - slot. Dungeon-mode popups pend the dungeon overlay seam (FLAGGED:
- `src/scenes/world.js:2149` - if (sd.siteType === SITE_TYPES.Building) return false;   // FLAGGED above - the building arm pends
- `src/scenes/world.js:2226` - PlayerGPS.GetCourtOfCurrentRegion is FLAGGED: the port has no
- `src/scenes/world.js:2459` - FLAGGED: the QuestComplete loot window pends the UI arc - the
- `src/scenes/world.js:2478` - castle interior (FLAGGED with the palace blocks).
- `src/scenes/world.js:2837` - exemption (PlayerTileMapIndex == 0) is FLAGGED: this host
- `src/scenes/world.js:3144` - doors are the E-enter seam, not bashables - FLAGGED with the
- `src/scenes/world.js:3217` - (FLAGGED: exterior corpse containers are the loot arc's), so
- `src/scenes/worldModes.js:338` - FLAGGED (Port-Ledger Q4-v, NARROWED by B1/B2): quest FOES still
- `src/scenes/worldModes.js:486` - FLAGGED: clicks on dungeon quest NPC/item flats pend the dungeon
- `src/scenes/worldModes.js:563` - if (!isShop(b.buildingType)) return;   // Library/Guild/Temple bookshelves + owned-house storage pend (FLAGGED)
- `src/scenes/worldModes.js:626` - G4: THE GUILD STORE ARM. This had been a FLAGGED null since
- `src/scenes/worldModes.js:664` - credit instead. FLAGGED: there is nowhere to cash one yet, so
- `src/scenes/worldModes.js:779` - Person placed in a dungeon is the quest machine's, FLAGGED
- `src/scenes/worldModes.js:842` - trade window in Sell mode. The BANKING arm stays FLAGGED below;
- `src/scenes/worldModes.js:920` - FLAGGED, each with the slice it waits on:
- `src/scenes/worldModes.js:1037` - FLAGGED: house and ship OWNERSHIP need the building directory
- `src/scenes/worldModes.js:1152` - FactionData; the port's pre-chargen INTERIM entity does not, and
- `src/scenes/worldModes.js:1206` - FLAGGED by name in guildServiceFlow.SERVICE_DESTINATION.
- `src/scenes/worldModes.js:1264` - skip, the refusal line. Only the destination was a FLAGGED null,
- `src/scenes/worldModes.js:1410` - been a FLAGGED null since G3.
- `src/scenes/worldModes.js:1447` - destination has been a FLAGGED null since G3.
- `src/scenes/worldModes.js:1466` - been FLAGGED nulls since G3.
- `src/scenes/worldModes.js:1569` - with the trade window's own mode flow, the same INTERIM the
- `src/scenes/worldModes.js:1771` - first (owned houses and quest buildings FLAGGED/seamed per
- `src/scenes/worldModes.js:1779` - (:515). X3 wired the Open-spell bypass (:519-520). FLAGGED: the bash arms with
...
- `src/scenes/worldModes.js:2970` - U48 FLAGGED: no toggleRest here, and the omission is the SHAPE
- `src/scenes/worldModes.js:3163` - *  FLAGGED: this host has no interior enemy pool (the Q4-v flag on
...
- `src/systems/banking.js:412` - FLAGGED, with the slices they wait on:
...
- `src/systems/effects.js:565` - enchantment bundles are FLAGGED to their own arc.
- `src/systems/effects.js:883` - manager, and the port has no such re-entry yet. FLAGGED.
- `src/systems/effects.js:1164` - out.skipped++;   // FLAGGED: the library grows one family at a time
- `src/systems/effects.js:1168` - not the effect itself. FLAGGED (recorded divergence): DFU re-runs
...
- `src/systems/enchantments.js:335` - *  condition holds. Params 0-3 seasons, 4-6 moons (FLAGGED: lunar
- `src/systems/enchantments.js:601` - a MagicRound-FLAGGED row's is the payload callback :1767
...
- `src/systems/guildServiceFlow.js:262` - ReceiveHouse: null,      // FLAGGED: banking / house ownership
...
- `src/systems/itemInfo.js:166` - *  screen. FLAGGED as a group - they land with their own arcs.

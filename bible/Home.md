...
- `01-Overview/Audit-46.md` - CLOSED 2026-09-01: THE BLACK WORLD, AND THE GATE THAT COULD NOT SEE IT. F1: the tile array was allocated with an UNSIZED gl.RGBA, which generateMipmap is not guaranteed to accept - a failed chain leaves the sampler mipmap-incomplete, and an incomplete sampler returns BLACK for every tile; RGBA8 now, with a soft fallback to NEAREST. F2, and it matters more: every gate passed on the black world, because eslint, the node suite and a vite build have no GL context between them and the lab's render gate loads a page that needs no game data. tools/glGate.mjs now runs the renderer's real upload path against a real WebGL2 context in both modes - AND reintroducing the bug did not fail it, because SwiftShader accepts what a real driver may not. So the gate is kept for the next unconditional format fault and is NOT claimed to confirm this one. Since I have guessed at the cause three times, the ground's three states are a URL door instead: ?ground=classic|tiles|drawn, cache-keyed by mode, so whichever is black names the slice in a reload. Verified: the builder against the REAL archive gives a grass tile mean of 85.2, not 0 - the surfaces are not coming out empty.
- `01-Overview/Audit-45.md` - CLOSED 2026-09-01: ENHANCED ENVIRONMENTS, THE PARITY LEDGER. The honest answer to "is it 1:1" is no, and the useful answer is a ledger of exactly what crossed and what did not. IN: the sky's shader (the lab lifted it from here), the horizon projection cap and the star cell fix (EE2), mipmaps and anisotropy (EE3), cloud shadows from the deck's own field (EE4), and this audit's two - F1 the sunset band was three rungs wide for the eight degrees a whole sunrise happens in, now four keys with an ember-to-peach horizon and a violet zenith; F2 the deck was thinned to a QUARTER at the horizon by multiplying cover by cover, where the sky is largest. NOT IN, named: the drawn surfaces (built, unwired, seamed), the ground's normal map and detail tiling, the 3D grass (no instanced draw exists), the weather particles, and the surface state field. The lab's front is deliberately not ported - the game has its own eased weather and two easings would fight. Two mutants dead; one pin re-taught (elev 2 became a real key) and the CPU occlusion updated to thin exactly as the shader does.
...
- `01-Overview/Audit-44.md` - CLOSED 2026-09-01: ENHANCED ENVIRONMENTS, SLICES 1-3. F1: EE1's rename swept src/ and not tools/, so enhancedMenuProbe still clicked "Procedural sky" and read the retired key - it would have failed on a row it could not find rather than on a fault. F2: uploadTileArray returned its cached array BEFORE consulting enhancedGround, and the cache lives on the renderer which survives a world load, so the switch was really a page-reload-only setting while the row promised "when the world next loads"; the mode is part of the cache key now (mutant dead). Verified: nothing live reads proceduralSky, the migration is one-way and once, classic cannot inherit the enhanced sampler, ?sky=classic still forces the panorama, and the lab's RAY convention fix was deliberately NOT ported because it was a lab bug rather than the game's.
...
- `src/scenes/dungeonContext.js:256` - the chain lives, the motion is INTERIM (loud) until flats can tween.
- `src/scenes/dungeonContext.js:809` - index into the 18 careers) or the INTERIM default Warrior (16,
- `src/scenes/dungeonContext.js:1421` - FS1 - FLAGGED (THE FOUR HOSTS RULE): THE ENCHANT CTX IS NOT
- `src/scenes/dungeonContext.js:1458` - A10: THE RECALL ARRIVAL, ROUTED. This used to be an INTERIM line
- `src/scenes/dungeonContext.js:1513` - onTeleport INTERIM shape). Absent, the engine's dispatch
- `src/scenes/dungeonContext.js:3438` - PX3 FLAGGED: questMessages - the dungeon quest mount is
- `src/scenes/dungeonContext.js:3549` - rest-for-a-while. DFU's toggle-close binding is FLAGGED in
- `src/scenes/exterior.js:719` - S3d: the INTERIM dagger seed is the FALLBACK only - a character
- `src/scenes/exterior.js:725` - pre-chargen INTERIM entity (flat skills 30, maxHealth 50) for the
- `src/scenes/exterior.js:984` - onTeleport: () => townTalk.say('(Recall pends here - the anchor machinery lives in the streaming ?world host)'),   // TP-slice INTERIM
- `src/scenes/exterior.js:1236` - PX3 FLAGGED: questMessages - this test host mounts no quest
- `src/scenes/exterior.js:1612` - (FLAGGED: the climate People table pends; the test city is
...
- `src/scenes/world.js:1247` - S3d: the INTERIM dagger seed is the FALLBACK only - a character
- `src/scenes/world.js:1253` - pre-chargen INTERIM entity (flat skills 30, maxHealth 50) for the
- `src/scenes/world.js:2618` - landing stands in for it - FLAGGED for the first session with
- `src/scenes/world.js:3934` - slot. Dungeon-mode popups pend the dungeon overlay seam (FLAGGED:
- `src/scenes/world.js:4853` - castle interior (FLAGGED with the palace blocks).
- `src/scenes/world.js:4959` - FLAGGED: DFU's INDOOR arm spawns 2-5 guards at the interior's
- `src/scenes/world.js:6178` - building doors are the E-enter seam, not bashables - FLAGGED
- `src/scenes/worldModes.js:1259` - at the concluded deal (:1036-1051). FLAGGED: the equipped test
- `src/scenes/worldModes.js:1277` - G4: THE GUILD STORE ARM. This had been a FLAGGED null since
- `src/scenes/worldModes.js:1489` - is the quest machine's, FLAGGED with it.
- `src/scenes/worldModes.js:1494` - FLAGGED, above ground only, each with the DFU line it owes:
- `src/scenes/worldModes.js:1762` - CW1 retired the FLAGGED list that lived here - every arm it
- `src/scenes/worldModes.js:2052` - fixed ship scenes and stays FLAGGED, so those buttons keep
- `src/scenes/worldModes.js:2084` - H3: the sell price, which was FLAGGED at zero because it needs
- `src/scenes/worldModes.js:2306` - FactionData; the port's pre-chargen INTERIM entity does not, and
- `src/scenes/worldModes.js:2428` - skip, the refusal line. Only the destination was a FLAGGED null,
- `src/scenes/worldModes.js:2703` - been a FLAGGED null since G3.
- `src/scenes/worldModes.js:2748` - destination has been a FLAGGED null since G3.
- `src/scenes/worldModes.js:2767` - been FLAGGED nulls since G3.
- `src/scenes/worldModes.js:2872` - with the trade window's own mode flow, the same INTERIM the
- `src/scenes/worldModes.js:3112` - first (owned houses and quest buildings FLAGGED/seamed per
- `src/scenes/worldModes.js:3124` - FLAGGED, and narrowed to what is actually missing: the two BASH
- `src/scenes/worldModes.js:3186` - the FLAGGED note above), so it starts false.
- `src/scenes/worldModes.js:4688` - string is the seam that was a FLAGGED null until this slice,
- `src/scenes/worldModes.js:4787` - *  FLAGGED null this slice closed. */
- `src/scenes/worldModes.js:5125` - *  FLAGGED: pause-and-resume is the DFU behaviour and a
...
- `src/systems/loot.js:244` - G4: THE VALUE IS OVERWRITTEN (:632). This had been FLAGGED here
...
- `src/systems/shopStock.js:23` - A2 (ROAD TO 1:1) closed the two INTERIM clauses that stood here.
...
- `src/ui/enhancedMenu.js:1760` - FLAGGED: the rest of the keyboard. The wizard walks to `done` with
- `src/ui/enhancedMenu.js:1931` - if (action === 'delete') return;   // FLAGGED: no save manager yet
...
- `src/ui/prisonScreen.js:29` - FLAGGED, and honestly: the trial's own message boxes still stand on
...
- `src/ui/restWindow.js:141` - FLAGGED, all three from OnPop/Update and all three belonging to
- `src/ui/restWindow.js:422` - where classic counts DOWN. The backgrounds are still FLAGGED

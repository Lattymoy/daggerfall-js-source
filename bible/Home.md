...
## Open flags (audit-generated 2026-08-16, from the code)

Regenerated by the 2026-08-16 dungeon + FP audit. Every row below is a
live INTERIM/FLAGGED/PENDING site in src; the code comment at each site
is the authority. Gone from the code and so from this list since 08-13:
the dungeonEnemies stale playerLevel default (wired live), the
characterSprite FP framing rows (probe-locked; the constants stay open
to Mac's eye in live play), the actionSystem DrainMagicka INTERIM tail
(the header note remains where the comment does). New: the move-flag
FLAT relay (flats cannot tween yet - the chain lives, the motion is
loud INTERIM in dungeonContext).
...
- `src/scenes/dungeonContext.js:1129` - actions) is FLAGGED - the player snapshot only.
- `src/scenes/dungeonContext.js:140` - the chain lives, the motion is INTERIM (loud) until flats can tween.
- `src/scenes/dungeonContext.js:377` - index into the 18 careers) or the INTERIM default Warrior (16,
- `src/scenes/dungeonContext.js:383` - effects FLAGGED to the effect-library slice.
- `src/scenes/dungeonContext.js:486` - FLAGGED: DFU recomputes per-effect via the cost tables (that
- `src/scenes/dungeonContext.js:488` - FLAGGED to the effect library (caster-only buffs, touch, areas).
- `src/scenes/dungeonContext.js:641` - 129; the inventory/equip UI pends - the INTERIM dagger note
...
- `src/systems/effects.js:200` - out.skipped++;   // FLAGGED: the library grows one family at a time
- `src/systems/effects.js:21` - these keys stay FLAGGED skipped (the library grows here).
- `src/systems/effects.js:33` - (concealment); waterWalking tracks but its consumer FLAGGED
...
- `src/ui/hud.js:89` - [art.fatigue, 1, 1],                                    // FLAGGED: ...
- `src/ui/hud.js:8` - VerticalProgress shape). Fatigue draws FULL, FLAGGED - the
...
- `src/world/actionSystem.js:39` - DrainMagicka (0x1c): INTERIM no-op - the magicka stat pends the

...
- `01-Overview/Audit-25.md` - CLOSED 2026-08-23: THE COMPLETENESS AUDIT. The first audit whose denominator is the DFU tree rather than `src/`: 27 subsystem groups over a real 849-file checkout, a surveyor and an adversarial refuter each, four reconciliation passes. 767 surviving gaps, ~63,400 JS lines. Six systems at or near zero and seven P0 host seams, all of them wire for laws already ported. ALL SEVEN SEAMS CLOSED 2026-08-23 (B1-B7), and FIVE OF THE SIX SYSTEMS CLOSED 2026-08-23..25: enchanting (E1/E2), both automaps (A1/A2), the magic crafting windows (S1/M2/M4/U42), banking (B1/B2/H1-H3/S41) and the pause menu + keybinding registry (I1-I4). The classic `.SAV` reader is the one still at zero; the page is struck entry by entry. The gap register and the slice order live in the page; the ledger rows it found are folded into `01-Overview/Port-Ledger.md` section C.
...
- `08-Audio/Audio.md` - ACTIVE. A1 + A2 (2026-08-16: action PlaySound on every Play, torch Burning loops at 5m linear/0.7 via the new loop3d engine seam, animal random barks on the classic rand()<=100 cadence at 19.2m - dungeon-scoped) + A3 SCENE AMBIENCE (2026-08-16: AmbientEffectsPlayer verbatim - the 14 dungeon one-shots somewhere-around on the scene's serialized 5/28 waits + classic-cadence water/bubbles, the weather/time presets in BOTH exterior scenes on 5/25 with rain/crickets loops and horizon storms, one shared ambient channel; building interiors silent, verbatim) SHIPPED + A4 EXTERIOR ANIMALS (2026-08-17: the shared PlayRandomlyIfPlayerNear pass - systems/animalAmbience.js, one implementation for dungeon + both exteriors incl. the streaming world's floating-origin sources; exterior torches SILENT verbatim - Burning is RDBLayout-only in DFU) SHIPPED; transition stingers CLOSED verbatim-N/A (2026-08-17: DFU plays nothing on enter/exit or ladders - no source law to port). + A5/A5b MUSIC (2026-08-24..25: the MIDI.BSA song reader, SongManager's playlists and SelectCurrentSong live in all four hosts, and the instrument source DECIDED - `systems/gmSynth.js` synthesises a TWO-OPERATOR FM bank, the synthesis Daggerfall was actually scored for, behind an interface an SF2 can replace later; no sample data ships and none is asked for. Ledger A carries the decision; the 2026-08-25 FM-bank audit fixed the drum map, the articulation family and the sustain pedal) SHIPPED - the queue's last row is struck. + A6 PLAYER FOOTSTEPS (2026-08-20, the FS-slice: PlayerFootsteps.cs whole - the sound-set decision with the snow gate finally consuming isSnowFreeClimate, wood in buildings, stone in dungeons with the water arms, and the classic 2.5-unit alternating stride with the landing step and the boot swallow - driven by all four hosts off their live motors; the path/water tile arms and the mount gate are the struck row's residue) SHIPPED.
...
- `src/characters/enemyCasting.js:74` - * the magicka and a ranged spell at all. FLAGGED, and narrow: in DFU
- `src/characters/enemyMotor.js:962` - FLAGGED, until target prediction ships). At zero the foe stops.
...
- `src/combat/formulas.js:630` - *  MobileEnemy.Weight, class = female 240 / male 350. FLAGGED: the
...
- `src/scenes/arrestFlow.js:177` - SeverePunishmentFlags |= 1 consequences pend (FLAGGED)
- `src/scenes/arrestFlow.js:245` - FLAGGED, still owed to their own slices: PreventEnemySpawns across the
- `src/scenes/cityGuards.js:27` - FLAGGED loud: enemy-vs-enemy stays out (C15 residual). (The
- `src/scenes/cityGuards.js:284` - IS Murder; TallyCrimeGuildRequirements(false, 1) FLAGGED to
- `src/scenes/cityGuards.js:540` - motor disables, TallyCrimeGuildRequirements(false, 5) FLAGGED,
- `src/scenes/dungeonContext.js:235` - the chain lives, the motion is INTERIM (loud) until flats can tween.
- `src/scenes/dungeonContext.js:677` - index into the 18 careers) or the INTERIM default Warrior (16,
...
- `src/scenes/dungeonContext.js:2799` - rest-for-a-while. DFU's toggle-close binding is FLAGGED in
- `src/scenes/exterior.js:543` - S3d: the INTERIM dagger seed is the FALLBACK only - a character
- `src/scenes/exterior.js:549` - pre-chargen INTERIM entity (flat skills 30, maxHealth 50) for the
- `src/scenes/exterior.js:786` - onTeleport: () => townTalk.say('(Recall pends here - the anchor machinery lives in the streaming ?world host)'),   // TP-slice INTERIM
- `src/scenes/exterior.js:1023` - (FLAGGED); swallowing the browser reload is not optional.
- `src/scenes/exterior.js:1315` - (FLAGGED: the climate People table pends; the test city is
- `src/scenes/exterior.js:1492` - FLAGGED here exactly as in world.js - no tile lookup yet).
...
- `src/scenes/world.js:757` - FLAGGED loud: the People faction rides the START location's
- `src/scenes/world.js:792` - S3d: the INTERIM dagger seed is the FALLBACK only - a character
- `src/scenes/world.js:798` - pre-chargen INTERIM entity (flat skills 30, maxHealth 50) for the
- `src/scenes/world.js:1262` - none); the dungeon-mode ctx is dungeonContext's to mount - FLAGGED
- `src/scenes/world.js:1266` - while inSunlight/inHolyPlace stay the E1 FLAGGED seams no host
- `src/scenes/world.js:1676` - FLAGGED: the BUILDING arm (Respawner :559-567, StartBuilding-
- `src/scenes/world.js:1792` - from data. FLAGGED.
- `src/scenes/world.js:2048` - (FLAGGED); swallowing the browser reload is not optional.
- `src/scenes/world.js:2320` - slot. Dungeon-mode popups pend the dungeon overlay seam (FLAGGED:
- `src/scenes/world.js:2493` - if (sd.siteType === SITE_TYPES.Building) return false;   // FLAGGED above - the building arm pends
- `src/scenes/world.js:2592` - PlayerGPS.GetCourtOfCurrentRegion is FLAGGED: the port has no
- `src/scenes/world.js:2857` - FLAGGED: the QuestComplete loot window pends the UI arc - the
- `src/scenes/world.js:2884` - castle interior (FLAGGED with the palace blocks).
- `src/scenes/world.js:3319` - exemption (PlayerTileMapIndex == 0) is FLAGGED: this host
- `src/scenes/world.js:3663` - doors are the E-enter seam, not bashables - FLAGGED with the
- `src/scenes/worldModes.js:332` - rather than pretending to scan. FLAGGED.
- `src/scenes/worldModes.js:466` - FLAGGED (Port-Ledger Q4-v, NARROWED by B1/B2): quest FOES still
- `src/scenes/worldModes.js:619` - FLAGGED: clicks on dungeon quest NPC/item flats pend the dungeon
- `src/scenes/worldModes.js:723` - *  FLAGGED, and narrowed to what it always was: the SHOPLIFTING
- `src/scenes/worldModes.js:735` - if (!isShop(b.buildingType)) return;   // Library/Guild/Temple bookshelves + owned-house storage pend (FLAGGED)
- `src/scenes/worldModes.js:808` - at the concluded deal (:1036-1051). FLAGGED: the equipped test
- `src/scenes/worldModes.js:823` - G4: THE GUILD STORE ARM. This had been a FLAGGED null since
- `src/scenes/worldModes.js:861` - credit instead. FLAGGED: there is nowhere to cash one yet, so
- `src/scenes/worldModes.js:1026` - is the quest machine's, FLAGGED with it.
- `src/scenes/worldModes.js:1031` - FLAGGED, above ground only, each with the DFU line it owes:
- `src/scenes/worldModes.js:1125` - trade window in Sell mode. The BANKING arm stays FLAGGED below;
- `src/scenes/worldModes.js:1214` - FLAGGED, each with the slice it waits on:
- `src/scenes/worldModes.js:1436` - H3: the sell price, which was FLAGGED at zero because it needs
- `src/scenes/worldModes.js:1611` - FactionData; the port's pre-chargen INTERIM entity does not, and
- `src/scenes/worldModes.js:1665` - FLAGGED by name in guildServiceFlow.SERVICE_DESTINATION.
- `src/scenes/worldModes.js:1723` - skip, the refusal line. Only the destination was a FLAGGED null,
- `src/scenes/worldModes.js:1894` - FLAGGED: a coven's failure spawns daedric foes ON YOU
- `src/scenes/worldModes.js:1898` - if (r.spawnFoes) console.warn('[summon] a coven failure owes you daedra; the interior has no foe pool (FLAGGED)');
- `src/scenes/worldModes.js:1965` - been a FLAGGED null since G3.
- `src/scenes/worldModes.js:2007` - destination has been a FLAGGED null since G3.
- `src/scenes/worldModes.js:2026` - been FLAGGED nulls since G3.
- `src/scenes/worldModes.js:2129` - with the trade window's own mode flow, the same INTERIM the
- `src/scenes/worldModes.js:2354` - first (owned houses and quest buildings FLAGGED/seamed per
- `src/scenes/worldModes.js:2362` - (:515). X3 wired the Open-spell bypass (:519-520). FLAGGED: the bash arms with
- `src/scenes/worldModes.js:2677` - FLAGGED, unchanged by this: DFU then raises the private-
- `src/scenes/worldModes.js:3182` - too. FLAGGED: interior loot containers are the loot arc's -
- `src/scenes/worldModes.js:3324` - string is the seam that was a FLAGGED null until this slice,
- `src/scenes/worldModes.js:3423` - *  FLAGGED null this slice closed. */
- `src/scenes/worldModes.js:3761` - *  FLAGGED: pause-and-resume is the DFU behaviour and a
- `src/scenes/worldModes.js:4182` - *  FLAGGED: this host has no interior enemy pool (the Q4-v flag on
...
- `src/systems/infection.js:62` - FLAGGED, with the slice it waits on:
...
- `src/systems/loot.js:130` - *  FLAGGED, as at every other book seam: DFU then prices the book from
- `src/systems/loot.js:250` - G4: THE VALUE IS OVERWRITTEN (:632). This had been FLAGGED here
...
- `src/systems/save.js:121` - (playerEntity's INTERIM skills: 30) - spreading it threw.
...
- `src/systems/shopStock.js:30` - INTERIM (loud): book items carry the template price (classic prices
- `src/systems/shopStock.js:231` - FLAGGED: the mint needs MAGIC.DEF - `magicItemTemplates`,
- `src/systems/shopStock.js:703` - *  FLAGGED: the MagicItems arm (0x04, on three of the five tables)
...
- `src/systems/talk.js:265` - *  FLAGGED to the crime slice - the state lands now, verbatim).
- `src/systems/talk.js:278` - TallyCrimeGuildRequirements(true, 1) FLAGGED: the TG
...
- `src/ui/enhancedMenu.js:634` - if (action === 'delete') return;   // FLAGGED: no save manager yet
...
- `src/ui/itemMakerWindow.js:199` - *  FLAGGED: the thumb is three Resources texture slices (:212-222)
- `src/ui/itemMakerWindow.js:516` - row scrolled off either edge is not drawn. INTERIM, this
...
- `src/ui/nativeTalk.js:295` - lands with the Tell-me-about slice (FLAGGED).
- `src/ui/nativeTalk.js:319` - B5-6: the four pages that were INTERIM no-ops. Each falls back
...
- `src/ui/restWindow.js:385` - where classic counts DOWN. The backgrounds are still FLAGGED
- `src/ui/spellMakerWindow.js:60` - *  FLAGGED - AUDIT 26 F181, the TARGET half of this same law is NOT
...
- `src/ui/tavernWindow.js:49` - FLAGGED, with the slices they wait on:
...
- `src/world/actionSystem.js:829` - *  NOT carried: failedSkillLevel (see addDoor - a live gap, FLAGGED,
...
`src/main.js` is the scene router (244 lines - it grew the data gate, the
launcher gate and the enhanced front door's pre-boot arm). It boots four
entry scenes from `src/scenes/`: exterior, interior, dungeon and world.
There is no terrain scene. THE FOUR HOSTS rule above names a different
four - `exterior.js`, `world.js`, `worldModes.js` and `dungeonContext.js` -
and that is the set a slice must enumerate. Shared helpers in
`src/scenes/shared.js`; each file carries its milestone header. Data
readers in `src/formats/`, world assembly in `src/world/`, GL in
`src/render/`. Extraction was verified pixel-identical when it was made.
...
- Bible lives in ten numbered directories under `bible/` (`01-Overview`
  through `10-UI`). This file is the index. No Dashboard.md.

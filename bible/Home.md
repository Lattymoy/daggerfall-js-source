...
- `04-Characters/Characters-Arc.md` - ACTIVE. Combat layer COMPLETE + audited (2026-07-06): all 19 weapon templates on the grip-station system (the 2H CONTRACT transfers the solved pose + coupled strikes to every two-hander), 4 poses incl. the view-space FP viewmodel, three attack tables (2H armR pre-solved offline; bow draw-hold-loose), and weaponStates.js owning ALL timing DFU-verbatim (tick/cooldown formulas, hit frames, gesture radials, interrupt rule, drag-to-swing input). One open gate: dedicated FP swing clips. See the Current State section atop the arc.

...
- `src/scenes/arrestFlow.js:133` - SeverePunishmentFlags |= 1 consequences pend (FLAGGED)
...
- `src/scenes/dungeonContext.js:888` - exterior spell paths are FLAGGED with their own hosts.
- `src/scenes/dungeonContext.js:1040` - 129; the inventory/equip UI pends - the INTERIM dagger note
...
- `src/scenes/exterior.js:686` - FLAGGED here exactly as in world.js - no tile lookup yet).
...
- `src/scenes/world.js:898` - exemption (PlayerTileMapIndex == 0) is FLAGGED: this host
- `src/scenes/world.js:1107` - doors are the E-enter seam, not bashables - FLAGGED with the
...
- `src/ui/chargenArt.js:653` - *  AUDIT 17g FLAGGED: the scrollbar THUMB does not draw. Its geometry
- `src/ui/chargenArt.js:785` - *  from the difficulty law). FLAGGED: the dagger's one-second fading
- `src/ui/chargenArt.js:875` - *  constellation chart. FLAGGED loud: the three FLC constellation

...
- `07-Rendering/Water-Arc.md` - ENHANCED WATER (WATER1, 2026-09-08, Mac: "develop proper water shader for the oceans/rivers/ponds of daggerfall"). Water was record 0 of the tile array drawn as dirt is drawn - flat, opaque, unmoving. `render/waterSurface.js` + `drawWaterSurface`: the pixel's own terrain grid drawn again, lifted, every non-water texel discarded, the rest a surface - waves on the eased wind, the dome reflected by Fresnel, the sun's and the moon's glints, the cloud deck's shadow, rain, the shore feathered off a water-corner table that inverts the marching squares. Switch `enhancedWater` (on by default), `?water=off` the kill switch; the classic lane untouched. On the way: the ocean band's compare was the double's, not float32's, so no corner anywhere was ever water and every sea tile this port drew was beach - fixed at the source, the jitter and the constant with it. Audited the same day (WATER-AUDIT, two adversarial lanes: the wave trains by rotation, the point lights on the water, the water's own index set, the table pinned through the producer). Seen in the water lab (`water.html`, 10/10 probe checks), not on a real GPU. WATER4 (2026-09-11, Mac: "not taking into account all the water textures that are on land... not traced well, just square... way too see through"): the water is the ART'S - `world/waterArt.js` reads each archive's own bitmaps once (record 0's palette is the water), the shader traces the record's outline at the terrain's own transformed uv, a puddle no corner reaches draws and swims, the shallows 0.80 opaque; the corner table is the fallback. WATER5 (the same day, Mac: "It's still not a perfect trace"): PER TEXEL - a 64x64 mask and signed distance field per record on a texture array of the tile array's own shape, the edge the field's zero crossing over one screen pixel, the foam on the outline, the palette widened by colour, `?water=mask` for the eye that judges it.
...
- `src/ui/enhancedMenu.js:2065` - FLAGGED: the rest of the keyboard. The wizard walks to `done` with
...
`dungeonContext.js:1668` before the fix): bare hands are a NULL weapon

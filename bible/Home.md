...
- `01-Overview/Audit-24.md` - ACTIVE, started 2026-08-21: THE FULL-CODEBASE PARITY SWEEP. A 145-agent workflow read the 161 port modules that cite a C# original against the DFU tree in 21 groups; every claim went to two independent refuters (one C#-side, one JS-side, both refute-by-default) and 54 of 62 survived both. Wave 1 SHIPPED: the hovering flyer's fall anchor, the DFRandom byte that stopped drawing mid-swing (with the melee timer that re-arms anyway and the bow roll that does not), the bow band's missing DetectedTarget/GiveUpTimer gates, the Horse's MobileTeams struct default, the swimmer measuring from the standing capsule at all five water sites, the legal-rep clamp running before the restore, the shield that never reached the forbidden-material arm, the Place expansion stealing the pronoun context, the five missing capitalized pronoun macros, the quest popup that revealed no dialog links, the CurrentLogMessageId latch that made every %qdt print the accept date, and the six macro reveals that deferred their rebuild. THREE EXISTING PINS HAD RECORDED THE BUG AS THE LAW and were corrected, not extended
...
- `src/characters/enemyMotor.js:409` - FLAGGED, until target prediction ships). At zero the foe stops.
...
- `src/scenes/exterior.js:691` - (FLAGGED); swallowing the browser reload is not optional.
- `src/scenes/exterior.js:827` - (FLAGGED: the climate People table pends; the test city is
- `src/scenes/exterior.js:969` - FLAGGED here exactly as in world.js - no tile lookup yet).
...
- `src/scenes/world.js:620` - FLAGGED loud: the People faction rides the START location's
- `src/scenes/world.js:650` - S3d: the INTERIM dagger seed is the FALLBACK only - a character
- `src/scenes/world.js:656` - pre-chargen INTERIM entity (flat skills 30, maxHealth 50) for the
- `src/scenes/world.js:1182` - (FLAGGED); swallowing the browser reload is not optional.
- `src/scenes/world.js:1395` - slot. Dungeon-mode popups pend the dungeon overlay seam (FLAGGED:
- `src/scenes/world.js:1481` - PlayerGPS.GetCourtOfCurrentRegion is FLAGGED: the port has no
- `src/scenes/world.js:1658` - FLAGGED: the QuestComplete loot window pends the UI arc - the
- `src/scenes/world.js:1677` - castle interior (FLAGGED with the palace blocks).
- `src/scenes/world.js:1925` - exemption (PlayerTileMapIndex == 0) is FLAGGED: this host
- `src/scenes/world.js:2191` - doors are the E-enter seam, not bashables - FLAGGED with the
...
- `src/scenes/worldModes.js:418` - Person placed in a dungeon is the quest machine's, FLAGGED
- `src/scenes/worldModes.js:493` - FLAGGED, each with the slice it waits on:
- `src/scenes/worldModes.js:512` - FactionData; the port's pre-chargen INTERIM entity does not, and
- `src/scenes/worldModes.js:556` - onTalk: () => townTalk?.say?.('You get no response.'),   // FLAGGED: TalkToStaticNPC pends the static-NPC conversation
- `src/scenes/worldModes.js:563` - FLAGGED by name in guildServiceFlow.SERVICE_DESTINATION.
...
- `src/systems/guildServiceFlow.js:244` - Repair: null,            // FLAGGED: DaggerfallTradeWindow Repair mode

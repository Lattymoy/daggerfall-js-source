...
## Open flags (regenerated 2026-08-17, U8d)
...
- `src/scenes/dungeonContext.js:490` - effects FLAGGED to the effect-library slice.
- `src/scenes/dungeonContext.js:504` - "database FLAGGED" narrows to the skill/loot message ids).
- `src/scenes/dungeonContext.js:638` - drained strength lowers the ceiling). INTERIM (loud): the
- `src/scenes/dungeonContext.js:753` - FLAGGED: DFU recomputes per-effect via the cost tables (that
- `src/scenes/dungeonContext.js:755` - FLAGGED to the effect library (caster-only buffs, touch, areas).
...
- `src/scenes/exterior.js:285` - it). say -> console FLAGGED: this host has no HUD-text layer yet.
- `src/scenes/exterior.js:341` - day-skip is a no-op FLAGGED until the shared calendar lands).
- `src/scenes/exterior.js:497` - (FLAGGED: the climate People table pends; the test city is
- `src/scenes/exterior.js:566` - FLAGGED here exactly as in world.js - no tile lookup yet).
...
- `src/scenes/world.js:423` - it). say -> console FLAGGED: this host has no HUD-text layer yet.
- `src/scenes/world.js:427` - FLAGGED loud: the People faction rides the START location's
- `src/scenes/world.js:511` - day-skip is a no-op FLAGGED until the shared calendar lands).
- `src/scenes/world.js:793` - exemption (PlayerTileMapIndex == 0) is FLAGGED: this host
- `src/scenes/world.js:972` - doors are the E-enter seam, not bashables - FLAGGED with the
...
- `src/ui/nativeInventory.js:29` - whole remote (dropped-pile) side are FLAGGED loud - equipping
- `src/ui/nativeInventory.js:113` - INTERIM info panel: name/weight/value (DFU's 1016 info text
- `src/ui/nativeInventory.js:122` - equip/use/remove: FLAGGED - the paperdoll/use/drop arcs pend
- `src/ui/nativeInventory.js:152` - the remote (dropped-pile) scroller: consumed, FLAGGED to the
- `src/ui/nativeTalk.js:22` - N/P page, Esc/E goodbye. People/Things/Work are INTERIM no-ops
- `src/ui/nativeTalk.js:131` - Tell me about / People / Things / Work: INTERIM no-ops (pend)

...
## Open flags (regenerated 2026-08-17, 17d)
...
- `src/scenes/exterior.js:284` - it). say -> console FLAGGED: this host has no HUD-text layer yet.
- `src/scenes/exterior.js:339` - day-skip is a no-op FLAGGED until the shared calendar lands).
- `src/scenes/exterior.js:485` - (FLAGGED: the climate People table pends; the test city is
- `src/scenes/exterior.js:554` - FLAGGED here exactly as in world.js - no tile lookup yet).
...
- `src/scenes/world.js:422` - it). say -> console FLAGGED: this host has no HUD-text layer yet.
- `src/scenes/world.js:426` - FLAGGED loud: the People faction rides the START location's
- `src/scenes/world.js:509` - day-skip is a no-op FLAGGED until the shared calendar lands).
- `src/scenes/world.js:781` - exemption (PlayerTileMapIndex == 0) is FLAGGED: this host
- `src/scenes/world.js:960` - doors are the E-enter seam, not bashables - FLAGGED with the

...
- `03-World/World-Of-Daggerfall.md` - WORLD OF DAGGERFALL, Kamer's second mod, 1:1 with permission (WOD1, 2026-09-23): his wilderness loader and the 227,938 places it names (one site to a pixel), the ground levelled under each, the spawns they hold, and his camp outside Privateer's Hold - the source his bundle ships, cited line by line
- `03-World/Aquatic-Sprites.md` - AQUATIC SPRITES, Cliffworms' mod, 1:1 under his readme's own terms (AS1, 2026-09-25): three flooded dungeon blocks gain the underwater flats Daggerfall ships and never places, carried as the author's EDIT and rebuilt from the player's own `BLOCKS.BSA` (`02-Formats/World-Data-Patches.md`, WD1).
- `03-World/Detailed-Ships.md` - DETAILED SHIPS, Cliffworms' second, 1:1 (DS1, 2026-09-25): both ships you can own revamped inside and out; its thirteen pictures on the texture door (four files, nine BUILT from the player's own classic records - WD2), and the ten models and twenty-seven flats it borrows from Daggerfall Expanded Textures stood in by the port (Mac: "Build your own"), with the table of what each is.
...
- `07-Rendering/Retro-Mode.md` - DFU'S RETRO MODE, PORTED (RETRO1, 2026-09-24, Mac: "Can we get retro mode from DFU ported over?") - the five `[Video]` keys live on the settings screen: the world at 320x200 or 640x400 (the docked-HUD twins 320x154 / 640x308), presented point-sampled through DFU's 640x400 target, posterized or palettized (art_pal through InitLut's LUT and FastColorPalette's tree, -sky variants), pillarboxed to 4:3 or 16:10, mip chains dropped unless asked for, Shift-F11 the effect's toggle; under both lanes. Recorded departures: the 2D layer keeps the whole canvas (DFU's CustomScreenRect), no -0.75 mip bias, two clamps, the LUT rebuilt per shift, the mip switch landing at once and both ways, the cursor pick through the presented rect - and, since AUDIT RETRO1 (Mac: "Audit this"), the twin chosen by the drawn bar, DFU's camera-clear quirks, the -sky test's billboards and far ring, the first-person arms - and, since its second pass (Mac: "One more audit"), Shift-F11 only toggling where DFU's press also quick-loads, with the LUT streamed a slab at a time under a block cap. Not seen on a GPU.
...
- `06-Systems/Climates-Calories.md` - CLIMATES & CALORIES, OVERHAULED (SURV, 2026-09-18, Mac: "we have been given permission to completely overhaul this mod ... tackle everything here properly") - Ralzar's mod read off its DLL's IL and rebuilt as the port's own survival arc: a felt temperature from climate, season, hour, weather, clothes and armour; hunger, thirst, sleep and wetness as needs on the world minute; food that spoils; the mod's items as custom templates above DFU's 288 with its icons through a vendor-only archive; camps and campfires as shared world objects, a rest priced by where you sleep, hunting (the later slices); all of it played at one of three tiers (SURV-TIERS, 2026-09-23): Off, Casual - the default, where a neglected need only borrows stamina, never past half the bar, and repays it when met - and Hard, the whole arc.
...
- `10-UI/Controls.md` - THE KEYBINDING STANDARD (KB1, 2026-09-23, Mac: "We have a lot of mods, a lot of keybinds. I really want to formalize a solid solution"): one registry owns every world key - DFU's, the port's and every vendored mod's (which each mod read off its own TextKey) - drawn in Controls by group, a mod's while it is on; one key, one action; a held key asks before it moves; every key read through the registry, a mod off answering nothing; a saved file carried forward once. Mac's four calls: E is Interact (AbortSpell to `), online Enter is the chat and Y frees the mouse, the digit row is the quickbar/hotbar (HCC to , and .), CenterView and PrintScreen built, ToggleConsole and Slide hidden. Pinned in `test/kb1_keybinds.test.js`.
...
- `src/ui/enhancedMenu.js:3493` - FLAGGED: the rest of the keyboard. The wizard walks to `done` with

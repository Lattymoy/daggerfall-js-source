...
- `10-UI/Features-Arc.md` - ACTIVE, opened 2026-09-13 (Mac): ONE HOME for every enhanceable feature, one list, three colour-coded labels with a filter - Enhanced (in house), Mod Authored (the mods ported 1:1), DFU Classic (DFU's own optional features). The stores stay; the presentation merges. Nothing moves in bulk: the inventory is the work list, one row per slice, audited and fixed before it moves. FT0 the home, FT1 Smaller Dungeons.
...
- `src/ui/enhancedMenu.js:2199` - FLAGGED: the rest of the keyboard. The wizard walks to `done` with
...
`dungeonContext.js:1707` before the fix): bare hands are a NULL weapon

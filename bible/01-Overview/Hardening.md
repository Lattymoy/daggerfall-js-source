...
`worldModes.js` that pointed between them cited `world.js:6603` and
...
2. *"`npcSession.onWorldChanged()` is on both door exits and not on the
   ...
   function owns the call (`world.js:5467`, DFU's `OnMapPixelChanged` /
...
3. *"`worldModes.js:9570` disposes the dungeon overlay that

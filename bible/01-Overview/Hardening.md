...
`worldModes.js` that pointed between them cited `world.js:4456` and
...
2. *"`npcSession.onWorldChanged()` is on both door exits and not on the
   ...
   function owns the call (`world.js:3572`, DFU's `OnMapPixelChanged` /
...
3. *"`worldModes.js:8001` disposes the dungeon overlay that

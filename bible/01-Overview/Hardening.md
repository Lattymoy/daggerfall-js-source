...
`worldModes.js` that pointed between them cited `world.js:5809` and
`dungeonContext.js:6230` - lines that had moved to `:4884` and `:6205`.
...
2. *"`npcSession.onWorldChanged()` is on both door exits and not on the
   ...
   function owns the call (`world.js:4735`, DFU's `OnMapPixelChanged` /
...
3. *"`worldModes.js:9428` disposes the dungeon overlay that
   ...
   known, already written down, at `dungeonContext.js:7024-7025`:

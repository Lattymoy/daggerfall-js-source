...
`worldModes.js` that pointed between them cited `world.js:6776` and
...
name at `world.js:510` - and two math helpers in the shot path. **No
drift.**
...
2. *"`npcSession.onWorldChanged()` is on both door exits and not on the
   ...
   function owns the call (`world.js:5578`, DFU's `OnMapPixelChanged` /
...
3. *"`worldModes.js:9716` disposes the dungeon overlay that

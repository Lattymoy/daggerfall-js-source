...
`worldModes.js` that pointed between them cited `world.js:4512` and
...
name at `world.js:363` - and two math helpers in the shot path. **No
drift.**
...
2. *"`npcSession.onWorldChanged()` is on both door exits and not on the
   ...
   function owns the call (`world.js:3608`, DFU's `OnMapPixelChanged` /

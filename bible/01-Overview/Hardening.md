...
`worldModes.js` that pointed between them cited `world.js:6838` and
`dungeonContext.js:6343` - lines that had moved to `:4992` and `:6318`.
...
name at `world.js:500` - and two math helpers in the shot path. **No
drift.**
...
2. *"`npcSession.onWorldChanged()` is on both door exits and not on the
   ...
   function owns the call (`world.js:5666`, DFU's `OnMapPixelChanged` /
...
3. *"`worldModes.js:9638` disposes the dungeon overlay that
   ...
   known, already written down, at `dungeonContext.js:7145-7146`:

...
`worldModes.js` that pointed between them cited `world.js:5338` and
...
2. *"`npcSession.onWorldChanged()` is on both door exits and not on the
   ...
   function owns the call (`world.js:4334`, DFU's `OnMapPixelChanged` /
...
3. *"`worldModes.js:8304` disposes the dungeon overlay that

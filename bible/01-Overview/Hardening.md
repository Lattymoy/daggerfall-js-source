...
`worldModes.js` that pointed between them cited `world.js:7479` and
`dungeonContext.js:6515` - lines that had moved to `:5159` and `:6488`.
...
name at `world.js:563` - and two math helpers in the shot path. **No
drift.**
...
2. *"`npcSession.onWorldChanged()` is on both door exits and not on the
   ...
   function owns the call (`world.js:6238`, DFU's `OnMapPixelChanged` /
...
3. *"`worldModes.js:10224` disposes the dungeon overlay that
   `dungeonCtx.destroy()` disposes again - HARD1's double free."* Already
   known, already written down, at `dungeonContext.js:7330-7331`:

...
`worldModes.js` that pointed between them cited `world.js:4547` and
...
2. *"`npcSession.onWorldChanged()` is on both door exits and not on the
   ...
   function owns the call (`world.js:3639`, DFU's `OnMapPixelChanged` /
...
3. *"`worldModes.js:8054` disposes the dungeon overlay that

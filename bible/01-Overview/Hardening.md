...
`worldModes.js` that pointed between them cited `world.js:4824` and
`dungeonContext.js:5637` - lines that had moved to `:4574` and `:5643`.
...
name at `world.js:382` - and two math helpers in the shot path. **No
drift.**
...
2. *"`npcSession.onWorldChanged()` is on both door exits and not on the
   ...
   function owns the call (`world.js:3857`, DFU's `OnMapPixelChanged` /
...
3. *"`worldModes.js:8126` disposes the dungeon overlay that
   `dungeonCtx.destroy()` disposes again - HARD1's double free."* Already
   known, already written down, at `dungeonContext.js:6270-6271`:

...
`worldModes.js` that pointed between them cited `world.js:4879` and
`dungeonContext.js:5657` - lines that had moved to `:4594` and `:5663`.
...
2. *"`npcSession.onWorldChanged()` is on both door exits and not on the
   ...
   function owns the call (`world.js:3912`, DFU's `OnMapPixelChanged` /
...
3. *"`worldModes.js:8129` disposes the dungeon overlay that
   ...
   known, already written down, at `dungeonContext.js:6290-6291`:

...
`worldModes.js` that pointed between them cited `world.js:4588` and
`dungeonContext.js:5567` - lines that had moved to `:4513` and `:5573`.
...
name at `world.js:373` - and two math helpers in the shot path. **No
drift.**
...
2. *"`npcSession.onWorldChanged()` is on both door exits and not on the
   ...
   function owns the call (`world.js:3680`, DFU's `OnMapPixelChanged` /
...
3. *"`worldModes.js:8081` disposes the dungeon overlay that
   `dungeonCtx.destroy()` disposes again - HARD1's double free."* Already
   known, already written down, at `dungeonContext.js:6192-6193`:

...
`worldModes.js` that pointed between them cited `world.js:5337` and
`dungeonContext.js:5728` - lines that had moved to `:4623` and `:5711`.
...
name at `world.js:414` - and two math helpers in the shot path. **No
drift.**
...
2. *"`npcSession.onWorldChanged()` is on both door exits and not on the
   ...
   function owns the call (`world.js:4333`, DFU's `OnMapPixelChanged` /
...
3. *"`worldModes.js:8313` disposes the dungeon overlay that
   `dungeonCtx.destroy()` disposes again - HARD1's double free."* Already
   known, already written down, at `dungeonContext.js:6387-6388`:

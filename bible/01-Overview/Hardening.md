...
`worldModes.js` that pointed between them cited `world.js:6584` and
`dungeonContext.js:6252` - lines that had moved to `:4902` and `:6227`.
...
2. *"`npcSession.onWorldChanged()` is on both door exits and not on the
   ...
   function owns the call (`world.js:5464`, DFU's `OnMapPixelChanged` /
...
3. *"`worldModes.js:9533` disposes the dungeon overlay that

...
`worldModes.js` that pointed between them cited `world.js:5801` and
`dungeonContext.js:6229` - lines that had moved to `:4884` and `:6204`.
...
name at `world.js:443` - and two math helpers in the shot path. **No
drift.**
...
2. *"`npcSession.onWorldChanged()` is on both door exits and not on the
   ...
   function owns the call (`world.js:4727`, DFU's `OnMapPixelChanged` /
...
3. *"`worldModes.js:9396` disposes the dungeon overlay that
   `dungeonCtx.destroy()` disposes again - HARD1's double free."* Already
   known, already written down, at `dungeonContext.js:6998-6999`:

...
`worldModes.js` that pointed between them cited `world.js:5157` and
...
2. *"`npcSession.onWorldChanged()` is on both door exits and not on the
   ...
   function owns the call (`world.js:4135`, DFU's `OnMapPixelChanged` /
...
3. *"`worldModes.js:8215` disposes the dungeon overlay that
   `dungeonCtx.destroy()` disposes again - HARD1's double free."* Already
   known, already written down, at `dungeonContext.js:6312-6313`:

...
`worldModes.js` that pointed between them cited `world.js:4472` and
`dungeonContext.js:5519` - lines that had moved to `:4484` and `:5525`.
...
name at `world.js:358` - and two math helpers in the shot path. **No
drift.**
...
2. *"`npcSession.onWorldChanged()` is on both door exits and not on the
   ...
   function owns the call (`world.js:3581`, DFU's `OnMapPixelChanged` /
...
3. *"`worldModes.js:8016` disposes the dungeon overlay that
   `dungeonCtx.destroy()` disposes again - HARD1's double free."* Already
   known, already written down, at `dungeonContext.js:6144-6145`:

...
`worldModes.js` that pointed between them cited `world.js:4439` and
...
name at `world.js:356` - and two math helpers in the shot path. **No
drift.**
...
2. *"`npcSession.onWorldChanged()` is on both door exits and not on the
   ...
   function owns the call (`world.js:3555`, DFU's `OnMapPixelChanged` /

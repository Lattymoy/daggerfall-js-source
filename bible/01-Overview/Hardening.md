...
`dungeonContext.js:5714` - lines that had moved to `:4614` and `:5697`.
...
name at `world.js:408` - and two math helpers in the shot path. **No
drift.**
...
2. *"`npcSession.onWorldChanged()` is on both door exits and not on the
   ...
   function owns the call (`world.js:4236`, DFU's `OnMapPixelChanged` /

...
`worldModes.js` that pointed between them cited `world.js:4540` and
`dungeonContext.js:5536` - lines that had moved to `:4488` and `:5542`.
...
name at `world.js:370` - and two math helpers in the shot path. **No
drift.**
...
2. *"`npcSession.onWorldChanged()` is on both door exits and not on the
   ...
   function owns the call (`world.js:3636`, DFU's `OnMapPixelChanged` /
...
3. *"`worldModes.js:8041` disposes the dungeon overlay that
   `dungeonCtx.destroy()` disposes again - HARD1's double free."* Already
   known, already written down, at `dungeonContext.js:6161-6162`:

...
`worldModes.js` that pointed between them cited `world.js:5899` and
...
name at `world.js:452` - and two math helpers in the shot path. **No
drift.**
...
2. *"`npcSession.onWorldChanged()` is on both door exits and not on the
   ...
   function owns the call (`world.js:4821`, DFU's `OnMapPixelChanged` /
...
3. *"`worldModes.js:9445` disposes the dungeon overlay that
   `dungeonCtx.destroy()` disposes again - HARD1's double free."* Already
   known, already written down, at `dungeonContext.js:7030-7031`:

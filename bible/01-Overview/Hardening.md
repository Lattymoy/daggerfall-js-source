...
`worldModes.js` that pointed between them cited `world.js:4685` and
...
name at `world.js:388` - and two math helpers in the shot path. **No
drift.**
...
2. *"`npcSession.onWorldChanged()` is on both door exits and not on the
   ...
   function owns the call (`world.js:3769`, DFU's `OnMapPixelChanged` /
...
3. *"`worldModes.js:8111` disposes the dungeon overlay that

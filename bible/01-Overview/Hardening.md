...
`worldModes.js` that pointed between them cited `world.js:6772` and
...
name at `world.js:506` - and two math helpers in the shot path. **No
drift.**
...
2. *"`npcSession.onWorldChanged()` is on both door exits and not on the
   ...
   function owns the call (`world.js:5574`, DFU's `OnMapPixelChanged` /
...
3. *"`worldModes.js:9691` disposes the dungeon overlay that

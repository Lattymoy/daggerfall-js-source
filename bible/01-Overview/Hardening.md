...
`worldModes.js` that pointed between them cited `world.js:5630` and
...
name at `world.js:435` - and two math helpers in the shot path. **No
drift.**
...
2. *"`npcSession.onWorldChanged()` is on both door exits and not on the
   ...
   function owns the call (`world.js:4591`, DFU's `OnMapPixelChanged` /

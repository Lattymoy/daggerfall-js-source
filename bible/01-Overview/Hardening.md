...
`worldModes.js` that pointed between them cited `world.js:4554` and
...
name at `world.js:371` - and two math helpers in the shot path. **No
drift.**
...
2. *"`npcSession.onWorldChanged()` is on both door exits and not on the
   ...
   function owns the call (`world.js:3646`, DFU's `OnMapPixelChanged` /
...
3. *"`worldModes.js:8056` disposes the dungeon overlay that
...
it omitted `tick`, which `interior.js:364` calls unguarded **every
frame**.

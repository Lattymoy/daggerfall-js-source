...
`worldModes.js` that pointed between them cited `world.js:4950` and
`dungeonContext.js:5665` - lines that had moved to `:4602` and `:5671`.
...
name at `world.js:389` - and two math helpers in the shot path. **No
drift.**
...
2. *"`npcSession.onWorldChanged()` is on both door exits and not on the
   ...
   function owns the call (`world.js:3958`, DFU's `OnMapPixelChanged` /
...
3. *"`worldModes.js:8156` disposes the dungeon overlay that
   `dungeonCtx.destroy()` disposes again - HARD1's double free."* Already
   known, already written down, at `dungeonContext.js:6298-6299`:

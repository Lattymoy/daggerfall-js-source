...
`worldModes.js` that pointed between them cited `world.js:5457` and
`dungeonContext.js:5969` - lines that had moved to `:4784` and `:5944`.
...
2. *"`npcSession.onWorldChanged()` is on both door exits and not on the
   ...
   function owns the call (`world.js:4453`, DFU's `OnMapPixelChanged` /
...
3. *"`worldModes.js:8434` disposes the dungeon overlay that
   `dungeonCtx.destroy()` disposes again - HARD1's double free."* Already
   known, already written down, at `dungeonContext.js:6632-6633`:

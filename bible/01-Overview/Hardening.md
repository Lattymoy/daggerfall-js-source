...
`worldModes.js` that pointed between them cited `world.js:7431` and
`dungeonContext.js:6409` - lines that had moved to `:5058` and `:6384`.
...
name at `world.js:547` - and two math helpers in the shot path. **No
drift.**
...
2. *"`npcSession.onWorldChanged()` is on both door exits and not on the
   ...
   function owns the call (`world.js:6221`, DFU's `OnMapPixelChanged` /
...
3. *"`worldModes.js:10106` disposes the dungeon overlay that
   `dungeonCtx.destroy()` disposes again - HARD1's double free."* Already
   known, already written down, at `dungeonContext.js:7214-7215`:

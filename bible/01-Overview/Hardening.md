...
`worldModes.js` that pointed between them cited `world.js:4538` and
`dungeonContext.js:5524` - lines that had moved to `:4489` and `:5530`.
...
name at `world.js:368` - and two math helpers in the shot path. **No
drift.**
...
2. *"`npcSession.onWorldChanged()` is on both door exits and not on the
   ...
   function owns the call (`world.js:3634`, DFU's `OnMapPixelChanged` /
...
3. *"`worldModes.js:8041` disposes the dungeon overlay that
   ...
   known, already written down, at `dungeonContext.js:6149-6150`:

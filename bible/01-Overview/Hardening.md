...
`worldModes.js` that pointed between them cited `world.js:5158` and
`dungeonContext.js:5685` - lines that had moved to `:4602` and `:5685`.
...
name at `world.js:406` - and two math helpers in the shot path. **No
drift.**
...
2. *"`npcSession.onWorldChanged()` is on both door exits and not on the
   ...
   function owns the call (`world.js:4154`, DFU's `OnMapPixelChanged` /
...
3. *"`worldModes.js:8240` disposes the dungeon overlay that
   `dungeonCtx.destroy()` disposes again - HARD1's double free."* Already
   known, already written down, at `dungeonContext.js:6332-6333`:

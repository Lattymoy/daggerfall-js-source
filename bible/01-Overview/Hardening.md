...
`worldModes.js` that pointed between them cited `world.js:6616` and
`dungeonContext.js:6277` - lines that had moved to `:4926` and `:6252`.
...
name at `world.js:485` - and two math helpers in the shot path. **No
drift.**
...
2. *"`npcSession.onWorldChanged()` is on both door exits and not on the
   ...
   function owns the call (`world.js:5480`, DFU's `OnMapPixelChanged` /
...
3. *"`worldModes.js:9571` disposes the dungeon overlay that
   `dungeonCtx.destroy()` disposes again - HARD1's double free."* Already
   known, already written down, at `dungeonContext.js:7078-7079`:

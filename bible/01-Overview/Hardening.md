...
`worldModes.js` that pointed between them cited `world.js:5640` and
`dungeonContext.js:6100` - lines that had moved to `:4795` and `:6075`.
...
name at `world.js:430` - and two math helpers in the shot path. **No
drift.**
...
2. *"`npcSession.onWorldChanged()` is on both door exits and not on the
   ...
   function owns the call (`world.js:4606`, DFU's `OnMapPixelChanged` /
...
3. *"`worldModes.js:9112` disposes the dungeon overlay that
   `dungeonCtx.destroy()` disposes again - HARD1's double free."* Already
   known, already written down, at `dungeonContext.js:6853-6854`:
...
mis-read `townTalk.js:1249` as unguarded; its guard sits eight lines up -

...
`worldModes.js` that pointed between them cited `world.js:4444` and
`dungeonContext.js:5499` - lines that had moved to `:4484` and `:5505`.
...
name at `world.js:357` - and two math helpers in the shot path. **No
drift.**
...
2. *"`npcSession.onWorldChanged()` is on both door exits and not on the
   ...
   function owns the call (`world.js:3560`, DFU's `OnMapPixelChanged` /
...
3. *"`worldModes.js:7998` disposes the dungeon overlay that
   `dungeonCtx.destroy()` disposes again - HARD1's double free."* Already
   known, already written down, at `dungeonContext.js:6124-6125`:

...
`worldModes.js` that pointed between them cited `world.js:5629` and
`dungeonContext.js:6175` - lines that had moved to `:4856` and `:6150`.
...
name at `world.js:434` - and two math helpers in the shot path. **No
drift.**
...
2. *"`npcSession.onWorldChanged()` is on both door exits and not on the
   ...
   function owns the call (`world.js:4590`, DFU's `OnMapPixelChanged` /
...
3. *"`worldModes.js:9149` disposes the dungeon overlay that
   `dungeonCtx.destroy()` disposes again - HARD1's double free."* Already
   known, already written down, at `dungeonContext.js:6868-6869`:
...
mis-read `townTalk.js:1256` as unguarded; its guard sits eight lines up -

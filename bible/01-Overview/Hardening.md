...
`worldModes.js` that pointed between them cited `world.js:4550` and
`dungeonContext.js:5538` - lines that had moved to `:4489` and `:5544`.
...
name at `world.js:372` - and two math helpers in the shot path. **No
drift.**
...
2. *"`npcSession.onWorldChanged()` is on both door exits and not on the
   ...
   function owns the call (`world.js:3642`, DFU's `OnMapPixelChanged` /
...
3. *"`worldModes.js:8068` disposes the dungeon overlay that
   `dungeonCtx.destroy()` disposes again - HARD1's double free."* Already
   known, already written down, at `dungeonContext.js:6163-6164`:
...
mis-read `townTalk.js:1207` as unguarded; its guard sits eight lines up -

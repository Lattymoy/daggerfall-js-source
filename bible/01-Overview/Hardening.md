...
`worldModes.js` that pointed between them cited `world.js:4881` and
...
2. *"`npcSession.onWorldChanged()` is on both door exits and not on the
   ...
   function owns the call (`world.js:3914`, DFU's `OnMapPixelChanged` /
...
3. *"`worldModes.js:8129` disposes the dungeon overlay that

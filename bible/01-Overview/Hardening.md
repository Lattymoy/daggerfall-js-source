...
`worldModes.js` that pointed between them cited `world.js:6497` and
`dungeonContext.js:6261` - lines that had moved to `:4910` and `:6236`.
...
name at `world.js:463` - and two math helpers in the shot path. **No
drift.**
...
2. *"`npcSession.onWorldChanged()` is on both door exits and not on the
   ...
   function owns the call (`world.js:5376`, DFU's `OnMapPixelChanged` /
...
3. *"`worldModes.js:9481` disposes the dungeon overlay that
   `dungeonCtx.destroy()` disposes again - HARD1's double free."* Already
   known, already written down, at `dungeonContext.js:7059-7060`:

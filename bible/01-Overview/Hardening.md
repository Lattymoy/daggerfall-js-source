...
`worldModes.js` that pointed between them cited `world.js:5043` and
...
name at `world.js:402` - and two math helpers in the shot path. **No
drift.**
...
2. *"`npcSession.onWorldChanged()` is on both door exits and not on the
   ...
   function owns the call (`world.js:4043`, DFU's `OnMapPixelChanged` /
...
3. *"`worldModes.js:8147` disposes the dungeon overlay that

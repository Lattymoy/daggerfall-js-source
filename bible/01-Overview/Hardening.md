...
`worldModes.js` that pointed between them cited `world.js:6734` and
`dungeonContext.js:6299` - lines that had moved to `:4946` and `:6274`.
...
name at `world.js:498` - and two math helpers in the shot path. **No
drift.**
...
2. *"`npcSession.onWorldChanged()` is on both door exits and not on the
   ...
   function owns the call (`world.js:5582`, DFU's `OnMapPixelChanged` /
...
3. *"`worldModes.js:9619` disposes the dungeon overlay that
   `dungeonCtx.destroy()` disposes again - HARD1's double free."* Already
   known, already written down, at `dungeonContext.js:7101-7102`:

...
`worldModes.js` that pointed between them cited `world.js:6971` and
`dungeonContext.js:6402` - lines that had moved to `:5051` and `:6377`.
...
name at `world.js:523` - and two math helpers in the shot path. **No
drift.**
...
2. *"`npcSession.onWorldChanged()` is on both door exits and not on the
   ...
   function owns the call (`world.js:5781`, DFU's `OnMapPixelChanged` /
...
3. *"`worldModes.js:10144` disposes the dungeon overlay that
   `dungeonCtx.destroy()` disposes again - HARD1's double free."* Already
   known, already written down, at `dungeonContext.js:7204-7205`:

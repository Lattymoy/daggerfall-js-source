...
`worldModes.js` that pointed between them cited `world.js:5923` and
`dungeonContext.js:6236` - lines that had moved to `:4889` and `:6211`.
...
name at `world.js:453` - and two math helpers in the shot path. **No
drift.**
...
2. *"`npcSession.onWorldChanged()` is on both door exits and not on the
   ...
   function owns the call (`world.js:4837`, DFU's `OnMapPixelChanged` /
...
3. *"`worldModes.js:9447` disposes the dungeon overlay that
   `dungeonCtx.destroy()` disposes again - HARD1's double free."* Already
   known, already written down, at `dungeonContext.js:7034-7035`:

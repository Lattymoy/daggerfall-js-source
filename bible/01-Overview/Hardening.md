...
`worldModes.js` that pointed between them cited `world.js:5166` and
`dungeonContext.js:5685` - lines that had moved to `:4608` and `:5691`.
...
name at `world.js:404` - and two math helpers in the shot path. **No
drift.**
...
2. *"`npcSession.onWorldChanged()` is on both door exits and not on the
   ...
   function owns the call (`world.js:4141`, DFU's `OnMapPixelChanged` /

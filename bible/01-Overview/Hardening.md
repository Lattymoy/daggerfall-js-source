...
`worldModes.js` that pointed between them cited `world.js:4766` and
`dungeonContext.js:5601` - lines that had moved to `:4539` and `:5607`.
...
name at `world.js:377` - and two math helpers in the shot path. **No
drift.**
...
2. *"`npcSession.onWorldChanged()` is on both door exits and not on the
   ...
   function owns the call (`world.js:3803`, DFU's `OnMapPixelChanged` /
...
3. *"`worldModes.js:8112` disposes the dungeon overlay that
   `dungeonCtx.destroy()` disposes again - HARD1's double free."* Already
   known, already written down, at `dungeonContext.js:6226-6227`:

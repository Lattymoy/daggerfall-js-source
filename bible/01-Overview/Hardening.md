...
`worldModes.js` that pointed between them cited `world.js:6575` and
...
2. *"`npcSession.onWorldChanged()` is on both door exits and not on the
   ...
   function owns the call (`world.js:5455`, DFU's `OnMapPixelChanged` /
...
3. *"`worldModes.js:9535` disposes the dungeon overlay that

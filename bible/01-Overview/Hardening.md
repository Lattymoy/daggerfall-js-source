...
`worldModes.js` that pointed between them cited `world.js:6011` and
...
name at `world.js:450` - and two math helpers in the shot path. **No
drift.**
...
2. *"`npcSession.onWorldChanged()` is on both door exits and not on the
   ...
   function owns the call (`world.js:4937`, DFU's `OnMapPixelChanged` /

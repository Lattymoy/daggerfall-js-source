...
`worldModes.js` that pointed between them cited `world.js:4822` and
`dungeonContext.js:5634` - lines that had moved to `:4571` and `:5640`.
...
2. *"`npcSession.onWorldChanged()` is on both door exits and not on the
   ...
   function owns the call (`world.js:3855`, DFU's `OnMapPixelChanged` /
...
3. *"`worldModes.js:8116` disposes the dungeon overlay that
   `dungeonCtx.destroy()` disposes again - HARD1's double free."* Already
   known, already written down, at `dungeonContext.js:6265-6266`:

...
`worldModes.js` that pointed between them cited `world.js:5517` and
...
2. *"`npcSession.onWorldChanged()` is on both door exits and not on the
   ...
   function owns the call (`world.js:4512`, DFU's `OnMapPixelChanged` /
...
3. *"`worldModes.js:8889` disposes the dungeon overlay that

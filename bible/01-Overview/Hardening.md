...
`worldModes.js` that pointed between them cited `world.js:5622` and
`dungeonContext.js:6174` - lines that had moved to `:4855` and `:6149`.
...
name at `world.js:431` - and two math helpers in the shot path. **No
drift.**
...
2. *"`npcSession.onWorldChanged()` is on both door exits and not on the
   ...
   function owns the call (`world.js:4589`, DFU's `OnMapPixelChanged` /
...
3. *"`worldModes.js:9125` disposes the dungeon overlay that
   `dungeonCtx.destroy()` disposes again - HARD1's double free."* Already
   known, already written down, at `dungeonContext.js:6867-6868`:

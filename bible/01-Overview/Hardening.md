...
`worldModes.js` that pointed between them cited `world.js:4525` and
...
2. *"`npcSession.onWorldChanged()` is on both door exits and not on the
   ...
   function owns the call (`world.js:3621`, DFU's `OnMapPixelChanged` /
...
3. *"`worldModes.js:8022` disposes the dungeon overlay that

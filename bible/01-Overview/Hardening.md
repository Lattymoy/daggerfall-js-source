...
`worldModes.js` that pointed between them cited `world.js:5952` and
`dungeonContext.js:6251` - lines that had moved to `:4900` and `:6226`.
...
name at `world.js:457` - and two math helpers in the shot path. **No
drift.**
...
2. *"`npcSession.onWorldChanged()` is on both door exits and not on the
   ...
   function owns the call (`world.js:4864`, DFU's `OnMapPixelChanged` /
...
3. *"`worldModes.js:9457` disposes the dungeon overlay that
   `dungeonCtx.destroy()` disposes again - HARD1's double free."* Already
   known, already written down, at `dungeonContext.js:7049-7050`:

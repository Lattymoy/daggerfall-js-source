...
`worldModes.js` that pointed between them cited `world.js:7478` and
`dungeonContext.js:6506` - lines that had moved to `:5142` and `:6479`.
...
2. *"`npcSession.onWorldChanged()` is on both door exits and not on the
   ...
   function owns the call (`world.js:6237`, DFU's `OnMapPixelChanged` /

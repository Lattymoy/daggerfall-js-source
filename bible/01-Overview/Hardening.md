...
`worldModes.js` that pointed between them cited `world.js:4601` and
`dungeonContext.js:5579` - lines that had moved to `:4519` and `:5585`.
...
2. *"`npcSession.onWorldChanged()` is on both door exits and not on the
   ...
   function owns the call (`world.js:3693`, DFU's `OnMapPixelChanged` /
...
3. *"`worldModes.js:8117` disposes the dungeon overlay that
   `dungeonCtx.destroy()` disposes again - HARD1's double free."* Already
   known, already written down, at `dungeonContext.js:6214-6215`:

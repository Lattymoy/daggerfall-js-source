...
`worldModes.js` that pointed between them cited `world.js:5798` and
`dungeonContext.js:6207` - lines that had moved to `:4862` and `:6182`.
...
2. *"`npcSession.onWorldChanged()` is on both door exits and not on the
   ...
   function owns the call (`world.js:4724`, DFU's `OnMapPixelChanged` /
...
3. *"`worldModes.js:9370` disposes the dungeon overlay that
   `dungeonCtx.destroy()` disposes again - HARD1's double free."* Already
   known, already written down, at `dungeonContext.js:6976-6977`:

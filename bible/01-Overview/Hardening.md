...
`worldModes.js` that pointed between them cited `world.js:7712` and
...
name at `world.js:565` - and two math helpers in the shot path. **No
drift.**
...
2. *"`npcSession.onWorldChanged()` is on both door exits and not on the
   ...
   function owns the call (`world.js:6470`, DFU's `OnMapPixelChanged` /
...
3. *"`worldModes.js:10214` disposes the dungeon overlay that
   ...
   known, already written down, at `dungeonContext.js:7318-7319`:

...
`worldModes.js` that pointed between them cited `world.js:5670` and
`dungeonContext.js:6170` - lines that had moved to `:4849` and `:6145`.
...
name at `world.js:437` - and two math helpers in the shot path. **No
drift.**
...
2. *"`npcSession.onWorldChanged()` is on both door exits and not on the
   ...
   function owns the call (`world.js:4596`, DFU's `OnMapPixelChanged` /
...
3. *"`worldModes.js:9332` disposes the dungeon overlay that
   `dungeonCtx.destroy()` disposes again - HARD1's double free."* Already
   known, already written down, at `dungeonContext.js:6939-6940`:

...
`worldModes.js` that pointed between them cited `world.js:6636` and
`dungeonContext.js:6284` - lines that had moved to `:4936` and `:6259`.
...
name at `world.js:487` - and two math helpers in the shot path. **No
drift.**
...
2. *"`npcSession.onWorldChanged()` is on both door exits and not on the
   ...
   function owns the call (`world.js:5486`, DFU's `OnMapPixelChanged` /
...
3. *"`worldModes.js:9588` disposes the dungeon overlay that
   `dungeonCtx.destroy()` disposes again - HARD1's double free."* Already
   known, already written down, at `dungeonContext.js:7085-7086`:

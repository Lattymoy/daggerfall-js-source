...
`worldModes.js` that pointed between them cited `world.js:4851` and
...
name at `world.js:386` - and two math helpers in the shot path. **No
drift.**
...
2. *"`npcSession.onWorldChanged()` is on both door exits and not on the
   ...
   function owns the call (`world.js:3884`, DFU's `OnMapPixelChanged` /

...
`worldModes.js` that pointed between them cited `world.js:4820` and
`dungeonContext.js:5632` - lines that had moved to `:4569` and `:5638`.
...
name at `world.js:380` - and two math helpers in the shot path. **No
drift.**
...
2. *"`npcSession.onWorldChanged()` is on both door exits and not on the
   ...
   function owns the call (`world.js:3853`, DFU's `OnMapPixelChanged` /
...
3. *"`worldModes.js:8115` disposes the dungeon overlay that
   ...
   known, already written down, at `dungeonContext.js:6263-6264`:

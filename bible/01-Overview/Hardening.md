...
`worldModes.js` that pointed between them cited `world.js:7198` and
`dungeonContext.js:6322` - lines that had moved to `:4971` and `:6297`.
...
2. *"`npcSession.onWorldChanged()` is on both door exits and not on the
   ...
   function owns the call (`world.js:6006`, DFU's `OnMapPixelChanged` /
...
3. *"`worldModes.js:9644` disposes the dungeon overlay that
   `dungeonCtx.destroy()` disposes again - HARD1's double free."* Already
   known, already written down, at `dungeonContext.js:7127-7128`:

...
`worldModes.js` that pointed between them cited `world.js:6737` and
...
name at `world.js:497` - and two math helpers in the shot path. **No
drift.**
...
2. *"`npcSession.onWorldChanged()` is on both door exits and not on the
   ...
   function owns the call (`world.js:5565`, DFU's `OnMapPixelChanged` /

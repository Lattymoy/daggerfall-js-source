...
`worldModes.js` that pointed between them cited `world.js:6771` and
`dungeonContext.js:6321` - lines that had moved to `:4965` and `:6294`.
...
name at `world.js:505` - and two math helpers in the shot path. **No
drift.**
...
2. *"`npcSession.onWorldChanged()` is on both door exits and not on the
   ...
   function owns the call (`world.js:5573`, DFU's `OnMapPixelChanged` /
...
3. *"`worldModes.js:9688` disposes the dungeon overlay that
   `dungeonCtx.destroy()` disposes again - HARD1's double free."* Already
   known, already written down, at `dungeonContext.js:7123-7124`:

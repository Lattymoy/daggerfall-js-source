...
`worldModes.js` that pointed between them cited `world.js:5932` and
...
name at `world.js:447` - and two math helpers in the shot path. **No
drift.**
...
2. *"`npcSession.onWorldChanged()` is on both door exits and not on the
   ...
   function owns the call (`world.js:4858`, DFU's `OnMapPixelChanged` /
...
3. *"`worldModes.js:9430` disposes the dungeon overlay that

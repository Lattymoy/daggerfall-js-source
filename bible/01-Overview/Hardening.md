...
`worldModes.js` that pointed between them cited `world.js:6740` and
`dungeonContext.js:6332` - lines that had moved to `:4981` and `:6307`.
...
2. *"`npcSession.onWorldChanged()` is on both door exits and not on the
   ...
   function owns the call (`world.js:5568`, DFU's `OnMapPixelChanged` /
...
3. *"`worldModes.js:9629` disposes the dungeon overlay that
   ...
   known, already written down, at `dungeonContext.js:7134-7135`:

...
`worldModes.js` that pointed between them cited `world.js:5153` and
...
name at `world.js:411` - and two math helpers in the shot path. **No
drift.**
...
2. *"`npcSession.onWorldChanged()` is on both door exits and not on the
   ...
   function owns the call (`world.js:4131`, DFU's `OnMapPixelChanged` /

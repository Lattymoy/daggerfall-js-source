...
`worldModes.js` that pointed between them cited `world.js:4441` and
`dungeonContext.js:5505` - lines that had moved to `:4472` and `:5511`.
...
2. *"`npcSession.onWorldChanged()` is on both door exits and not on the
   ...
   function owns the call (`world.js:3557`, DFU's `OnMapPixelChanged` /
...
3. *"`worldModes.js:7984` disposes the dungeon overlay that
   `dungeonCtx.destroy()` disposes again - HARD1's double free."* Already
   known, already written down, at `dungeonContext.js:6130-6131`:

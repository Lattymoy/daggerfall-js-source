...
`worldModes.js` that pointed between them cited `world.js:5942` and
...
2. *"`npcSession.onWorldChanged()` is on both door exits and not on the
   ...
   function owns the call (`world.js:4854`, DFU's `OnMapPixelChanged` /
...
3. *"`worldModes.js:9450` disposes the dungeon overlay that

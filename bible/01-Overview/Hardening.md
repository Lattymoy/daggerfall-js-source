...
`worldModes.js` that pointed between them cited `world.js:6779` and
`dungeonContext.js:6404` - lines that had moved to `:5048` and `:6377`.
...
`?region` and `?loc` to `bootExterior`; the front door (`main.js:226`)
...
name at `world.js:512` - and two math helpers in the shot path. **No
drift.**
...
2. *"`npcSession.onWorldChanged()` is on both door exits and not on the
   ...
   function owns the call (`world.js:5580`, DFU's `OnMapPixelChanged` /
...
3. *"`worldModes.js:9737` disposes the dungeon overlay that
   `dungeonCtx.destroy()` disposes again - HARD1's double free."* Already
   known, already written down, at `dungeonContext.js:7216-7217`:

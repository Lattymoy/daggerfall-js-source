...
`worldModes.js` that pointed between them cited `world.js:6775` and
...
name at `world.js:509` - and two math helpers in the shot path. **No
drift.**
...
2. *"`npcSession.onWorldChanged()` is on both door exits and not on the
   ...
   function owns the call (`world.js:5577`, DFU's `OnMapPixelChanged` /
...
3. *"`worldModes.js:9694` disposes the dungeon overlay that

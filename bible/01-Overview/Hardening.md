...
`worldModes.js` that pointed between them cited `world.js:6922` and
`dungeonContext.js:6346` - lines that had moved to `:4995` and `:6321`.
...
2. *"`npcSession.onWorldChanged()` is on both door exits and not on the
   ...
   function owns the call (`world.js:5732`, DFU's `OnMapPixelChanged` /
...
3. *"`worldModes.js:9638` disposes the dungeon overlay that
   ...
   known, already written down, at `dungeonContext.js:7148-7149`:

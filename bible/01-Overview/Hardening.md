...
`worldModes.js` that pointed between them cited `world.js:6756` and
`dungeonContext.js:6369` - lines that had moved to `:5018` and `:6344`.
...
name at `world.js:503` - and two math helpers in the shot path. **No
drift.**
...
2. *"`npcSession.onWorldChanged()` is on both door exits and not on the
   ...
   function owns the call (`world.js:5584`, DFU's `OnMapPixelChanged` /
...
3. *"`worldModes.js:9631` disposes the dungeon overlay that
   `dungeonCtx.destroy()` disposes again - HARD1's double free."* Already
   known, already written down, at `dungeonContext.js:7171-7172`:

...
`worldModes.js` that pointed between them cited `world.js:4553` and
`dungeonContext.js:5525` - lines that had moved to `:4489` and `:5531`.
...
2. *"`npcSession.onWorldChanged()` is on both door exits and not on the
   ...
   function owns the call (`world.js:3645`, DFU's `OnMapPixelChanged` /
...
3. *"`worldModes.js:8055` disposes the dungeon overlay that
   `dungeonCtx.destroy()` disposes again - HARD1's double free."* Already
   known, already written down, at `dungeonContext.js:6150-6151`:

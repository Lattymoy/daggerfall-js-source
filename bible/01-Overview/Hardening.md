...
`worldModes.js` that pointed between them cited `world.js:5950` and
`dungeonContext.js:6251` - lines that had moved to `:4901` and `:6226`.
...
2. *"`npcSession.onWorldChanged()` is on both door exits and not on the
   ...
   function owns the call (`world.js:4862`, DFU's `OnMapPixelChanged` /
...
3. *"`worldModes.js:9458` disposes the dungeon overlay that
   `dungeonCtx.destroy()` disposes again - HARD1's double free."* Already
   known, already written down, at `dungeonContext.js:7050-7051`:

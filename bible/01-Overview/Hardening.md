...
`worldModes.js` that pointed between them cited `world.js:6739` and
`dungeonContext.js:6318` - lines that had moved to `:4967` and `:6293`.
...
2. *"`npcSession.onWorldChanged()` is on both door exits and not on the
   ...
   function owns the call (`world.js:5567`, DFU's `OnMapPixelChanged` /
...
3. *"`worldModes.js:9629` disposes the dungeon overlay that
   ...
   known, already written down, at `dungeonContext.js:7120-7121`:

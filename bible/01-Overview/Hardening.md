...
`worldModes.js` that pointed between them cited `world.js:6643` and
`dungeonContext.js:6288` - lines that had moved to `:4940` and `:6263`.
...
2. *"`npcSession.onWorldChanged()` is on both door exits and not on the
   ...
   function owns the call (`world.js:5487`, DFU's `OnMapPixelChanged` /
...
3. *"`worldModes.js:9595` disposes the dungeon overlay that
   `dungeonCtx.destroy()` disposes again - HARD1's double free."* Already
   known, already written down, at `dungeonContext.js:7089-7090`:

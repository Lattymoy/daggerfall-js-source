...
`dungeonContext.js:5763` - lines that had moved to `:4658` and `:5746`.
...
name at `world.js:415` - and two math helpers in the shot path. **No
drift.**
...
2. *"`npcSession.onWorldChanged()` is on both door exits and not on the
   ...
   function owns the call (`world.js:4350`, DFU's `OnMapPixelChanged` /
...
3. *"`worldModes.js:8327` disposes the dungeon overlay that
   `dungeonCtx.destroy()` disposes again - HARD1's double free."* Already
   known, already written down, at `dungeonContext.js:6422-6423`:

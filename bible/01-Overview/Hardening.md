...
`worldModes.js` that pointed between them cited `world.js:7477` and
`dungeonContext.js:6498` - lines that had moved to `:5142` and `:6471`.
...
name at `world.js:561` - and two math helpers in the shot path. **No
drift.**
...
2. *"`npcSession.onWorldChanged()` is on both door exits and not on the
   ...
   function owns the call (`world.js:6236`, DFU's `OnMapPixelChanged` /
...
3. *"`worldModes.js:10214` disposes the dungeon overlay that
   ...
   known, already written down, at `dungeonContext.js:7313-7314`:

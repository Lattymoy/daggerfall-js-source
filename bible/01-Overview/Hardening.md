...
`worldModes.js` that pointed between them cited `world.js:4507` and
...
name at `world.js:360` - and two math helpers in the shot path. **No
drift.**
...
2. *"`npcSession.onWorldChanged()` is on both door exits and not on the
   ...
   function owns the call (`world.js:3603`, DFU's `OnMapPixelChanged` /

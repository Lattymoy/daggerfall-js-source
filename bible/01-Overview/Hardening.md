...
`worldModes.js` that pointed between them cited `world.js:5284` and
`dungeonContext.js:5713` - lines that had moved to `:4613` and `:5696`.
...
2. *"`npcSession.onWorldChanged()` is on both door exits and not on the
   ...
   function owns the call (`world.js:4280`, DFU's `OnMapPixelChanged` /
...
3. *"`worldModes.js:8304` disposes the dungeon overlay that
   ...
   known, already written down, at `dungeonContext.js:6372-6373`:

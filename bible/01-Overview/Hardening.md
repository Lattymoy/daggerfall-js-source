...
`worldModes.js` that pointed between them cited `world.js:6752` and
`dungeonContext.js:6345` - lines that had moved to `:4994` and `:6320`.
...
name at `world.js:502` - and two math helpers in the shot path. **No
drift.**
...
3. *"`worldModes.js:9630` disposes the dungeon overlay that
   `dungeonCtx.destroy()` disposes again - HARD1's double free."* Already
   known, already written down, at `dungeonContext.js:7147-7148`:

...
`worldModes.js` that pointed between them cited `world.js:4817` and
`dungeonContext.js:5602` - lines that had moved to `:4540` and `:5608`.
...
2. *"`npcSession.onWorldChanged()` is on both door exits and not on the
   ...
   function owns the call (`world.js:3841`, DFU's `OnMapPixelChanged` /
...
3. *"`worldModes.js:8133` disposes the dungeon overlay that
   `dungeonCtx.destroy()` disposes again - HARD1's double free."* Already
   known, already written down, at `dungeonContext.js:6227-6228`:

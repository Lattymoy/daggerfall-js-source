...
`worldModes.js` that pointed between them cited `world.js:5602` and
`dungeonContext.js:6080` - lines that had moved to `:4785` and `:6055`.
...
2. *"`npcSession.onWorldChanged()` is on both door exits and not on the
   ...
   function owns the call (`world.js:4597`, DFU's `OnMapPixelChanged` /
...
3. *"`worldModes.js:9047` disposes the dungeon overlay that
   `dungeonCtx.destroy()` disposes again - HARD1's double free."* Already
   known, already written down, at `dungeonContext.js:6833-6834`:

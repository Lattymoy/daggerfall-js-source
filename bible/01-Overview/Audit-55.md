...
| 6 | The bake's origin assumptions, after 4b-pre found one (the anchor at y=0) | `navmesh.js:119`'s "no bottom = solid from y=0" arena rule is dead for us: every triRaster box carries `bottom` (`triRaster.js:84`). The phantom ground is `minY - 10`, relative. `xmin`/`zmin` come from the input's extent. **clean**, with 4b-pre's fix pinned at 0, 25 and -40 |
...
(`enemyMotor.js:978`). A foe the watchdog fires on is stuck against

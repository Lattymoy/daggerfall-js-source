...
- **SL-1** `restoreWeather` lowered `_evolveHour` but not
  ...
  `weatherSim.js:463`; no envelope change. The CLK4 pin stayed green
...
- `TextProvider.GetSkillSummary` (`TextProvider.cs:489-529`) formats
  ...
  the next. `charsheet.js:704` and `enhancedCharSheet.js:152` read
  `skillValue` now, which moves the enhanced skin's meter with its
  number (`enhancedMenu.js:2000-2001`) - correctly, since the attribute
...
- `this.standing` and `movingLessThanHalfSpeed` are a port-side CACHE
  ...
  repeated - `worldTick.js:629-630` is climb-first, matching
...
spell), and two escaped-regex cites in tests (`dungeon\.js:483`,
`worldModes\.js:504`), which the mapper cannot see - the CS1 edge AUDIT

...
- **SL-1** `restoreWeather` lowered `_evolveHour` but not
  ...
  `weatherSim.js:405`; no envelope change. The CLK4 pin stayed green
...
- `TextProvider.GetSkillSummary` (`TextProvider.cs:489-529`) formats
  ...
  number (`enhancedMenu.js:1785-1786`) - correctly, since the attribute
...
`worldModes\.js:477`), which the mapper cannot see - the CS1 edge AUDIT

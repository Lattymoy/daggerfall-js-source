...
- `TextProvider.GetSkillSummary` (`TextProvider.cs:489-529`) formats
  ...
  the next. `charsheet.js:669` and `enhancedCharSheet.js:126` read
  `skillValue` now, which moves the enhanced skin's meter with its
  number (`enhancedMenu.js:1942-1943`) - correctly, since the attribute
...
spell), and two escaped-regex cites in tests (`dungeon\.js:481`,
`worldModes\.js:497`), which the mapper cannot see - the CS1 edge AUDIT

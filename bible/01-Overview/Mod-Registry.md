...
| `iliac-puddle-no-more` | manifest, settings and the seven fish item templates verbatim, the seven fish pictures (the author's own, the bundle's textures decoded at their first mip - DW-E3), and the compiled assembly byte for byte (the port's law - the bundle carries no C# source); NOT the two 356 MB coastline bakes, which the mod computes from WOODS.WLD, MAPS.BSA and BLOCKS.BSA and the port rebuilds from the player's own files (DW-A), nor the compiled shaders, restated as GLSL | jet082 | 1.2.2 | shipped `.7z` `Iliac_Puddle_No_More_1304_1.2.2_2026-07-11T17-49Z_psKKacHpq`; behaviour off the assembly read back to C#; the bake checked against the mod's own `DistanceBakeVanilla` by `test/dwa_bake.test.js` when it is at hand | granted (Mac handed the archive over 2026-09-25) - **RECORD OPEN** | DW-A to DW-E5, DW-F (the close: the sea at a distance, the audit pass) | 2026-09-26 | `03-World/Deep-Waters.md` |

...
> `vendor/` holds 33 directories and 19 of their READMEs still carry an
> unfilled permission line, each shown as `RECORD OPEN` in the table
> (SKIN2, 2026-09-25, added `class-skins`, the thirty-third, its record
> open; AUDIT BRANCH (WoD), 2026-09-23, counted them again after the merge
...
| `class-skins` | twenty redrawn sprite sheets of Daggerfall's enemy classes (1,682 PNGs) and a manifest of every record's frame count, worn as on-foot skins 16-35 | **not named for the pack** (one folder names Kamer, for the bounty hunter) | - | Mac's `ExistingClasses.rar` (one RAR per set); three sets given port archive numbers and two records' frame gaps closed on the way in (`vendor/class-skins/README.md`) | granted (Mac handed the archive over 2026-09-25) - **RECORD OPEN** | SKIN2 | 2026-09-25 | `06-Systems/Eye-Of-The-Beholder.md` |

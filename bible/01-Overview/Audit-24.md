...
and fixed by hand while the verifiers ran (commits `660618f`, `6732d22`):
...
(1) Its load-bearing premise is false. The claim says the port "deliberately preserves the identical C# quirk 25 lines above" at the foe==null throw, making 2029 inconsistent with the port's own doctrine. It does not preserve it: actions.js:2012 is `throw new Error(\`create foe could not find Foe with symbol name ${this.symbol?.name}\`)` — optional chaining, so the port renders "...symbol name undefined" and throws a normal Error. It never reproduces C#'s NullReferenceException (JS would have thrown a TypeError there anyway, a different failure). Same shape at CastSpellOnFoe (actions.js:2087, `this.symbol?.name`). So NEITHER port throw site reproduces the C# NRE — both render a message where C# blows up on the null argument. Line 2029 is not inconsistent with 2011; it is the same choice with different filler text.

(2) The port RECORDS the quirk and states the doctrine covering both sites. actions.js:2009-2011: "C# formats the action's own never-assigned Symbol (NREs before the message renders) - the same quirk as CastSpellOnFoe; either way the quest error-terminates", and the CastSpellOnFoe class docstring lists it among "Two C# QUIRKS KEPT". The recorded position is that these throws are equivalent because the quest error-terminates regardless of exception text/type — which is precisely the finding's own admitted consequence ("Message-text only"). The branch also sits behind the injected `world.createFoeGameObjects` seam and is reachable only when a host adapter mints the wrong count.
...
(`6c8d060`, `b289915`, `cf9089a`, `1c4c3f8` and this one). Each pin was
...
The audit did not stay closed. Twenty-seven more waves ran after the section
...
| 37 | Not an audit finding - a crash screenshot from the deployed build. Both browser hosts register their pointer and wheel listeners ~600 lines above `var modes = createWorldModes({...})`, and wrote `modes.pointerdown?.(e)`: an optional call on the METHOD, which dereferences the object unconditionally. The `var` hoist is deliberate (a `const` would make every earlier reference a TDZ `ReferenceError`) but it only pays off if the guard is on the object, and `?world` has one `await` in that window - `loadQuestPack()`, a network fetch - so a click during boot threw `TypeError: can't access property "pointerdown", it is undefined` and took the scene down. Every above-declaration reference is now `modes?.`, including the five already safe by short-circuit, because a rule with an exception list is not a gate. **AN OPTIONAL CALL IS NOT AN OPTIONAL DEREFERENCE** |
| 38 | `EnemyDeath.CompleteDeath` does five things and the two above-ground pools did one. `exteriorFoes` rolled its loot table, equipped the foe, drew a corpse - and exported no activation seam at all, so every encounter kill's loot was generated, drawn, and unreachable for the session. Neither pool asked `FindGroundPosition` (:817), so a flyer's body hung where it died; no pool anywhere played `BodyFall` (:126-129); none popped `"%s just died."` (:79-83). One home now (`scenes/corpseMarker.js`) carrying PlayerActivate's `CorpseMarker` arm too - the empty-body line the watch pool made unreachable by skipping empty corpses, and the arrows-only auto-pickup. `ENEMY_NAMES` moved to the leaf, which exposed the pacification line reading `ENEMY_BASICS[...].name` - **a field no row has ever had**, so every pacification in the game has said "The enemy". And `WeaponManager`'s knockback gate is a precedence trap: `&&` binds tighter, so it is `(speed <= 5/ratio && isClass) || Weight > 0`; the pool had only the first half of the first arm, which cost chain-knockback on every weighted monster and gave Ghost and Wraith - the only Weight-0 rows, which is exactly why DFU's gate spares them - a **NaN** knockback that no later hit could overwrite |
| 39 | Two whole DFU components unported. `EnemyBlood.ShowBloodSplash` - the port generated `bloodIndex` into the table CORRECTLY (the same six rows DFU marks) and then nothing read it, so nothing has bled in any pool since the first enemy shipped. And `ShowPlayerDamage`, thirty-seven lines of which three matter (0.4 alpha falling at 0.7 a second, red, full screen), so the player took every hit in the game with no feedback at all. Both were reachable the whole time - the one-shot billboard clock was ported for the missile impact and `drawScreenQuad(null, ...)` has been the blended solid since U10 - what was missing was wiring. **Which hits flash is a LAW**: the flash rides `PlayerHealth.RemoveHealth`, sent from exactly three places (an enemy's blow, the two dungeon damage traps, the fall arm), so spell damage, poison and disease do NOT flash. Adding the flash to the spell host is one of the wave's 32 mutants, and it is killed - **more responsive is a kind of more accurate**. Two further DFU call sites are deliberately not ported because they are dead in the shipped game (`showBlood` is passed false by every caller; `EnemyHealth` has no users), and `ShowMagicSparkles` is cut rather than half-wired - two of its three arms have a target to hang on and the CasterOnly arm does not |
| 40 | The second crash screenshot from the deployed build - `TypeError: can't access property "glyphWidth", s is undefined`. The local build was byte-identical to the deployed bundle, so the minified stack mapped straight onto source: open the character sheet, press LOGBOOK, dead. Every window in src/ui takes `draw(renderer, canvas, font, s)` where s is the HUD scale, and `CharSheet.draw` forwards its four arguments straight through to a pushed child - but `QuestJournalWindow` declared its fourth as `largeFont = null`. A number is not nullish, so `largeFont ?? font` picked the SCALE and measureText got `3 .fnt`. **Nothing had ever passed a real font there**: the parameter had never once done its job and existed only to be filled by mistake. The wave-37 shape one layer up - a thing that LOOKS considered. The law is real (`Font = DaggerfallUI.LargeFont` = FONT0000) so the font is a module warm now and the signature drops to three: **a parameter a function does not declare is a parameter nothing can fill by mistake** |
| 41 | `moveSound` and `barkSound` are in all 62 rows of the enemy table and had **zero consumers anywhere**; `attackSound` had one, inline in the dungeon frame body. Above ground nothing barked, nothing moved audibly and nothing made a noise when it swung - the watch's whole vocabulary was a single HALT on the detection rising edge, **which DFU does not have**: it runs a 3-9 second attract cadence while you are within 16m. The dungeon's inline copy had drifted three ways - its mute gate was `!isClass` where DFU carves the CITY WATCH out (:222), it never ran `SetVolumeScale` so barks came through walls at full volume, and it played on an inverse rolloff where DFU pins linear to the attract radius. Two DFU quirks kept because they are DFU's: the counter steps out of range (so an enemy greets you the moment you are near), and `volumeScale` is a FIELD only the attract sound writes - so a foe whose last bark was muffled keeps SWINGING quietly in the open. Also ONE DFU MEMBER ONE EXPORT: `146` was written out in five places |
| 42 | One the exterior hosts never did and one they did twice. `EnemySenses:504-527`'s first-encounter language roll was ported whole - both formulas verbatim, `justEncountered` raised by the motor for EVERY pool - and had exactly ONE consumer, inline in the dungeon frame body, so no monster in a field and no watchman in a street had ever been talked down. And `WeaponManager.cs:420` charges `swingWeaponFatigueLoss` once per swing, inside the single `isDamageFinished` block; `cityGuards.resolvePlayerHit` charged it a SECOND time on top of the host's, and its opening `if (!live.length) return false` meant the extra landed only while a guard was ALIVE - 11 fatigue a swing in an empty street, 22 the moment the watch turned up. The campaign needed two rounds: three mutants survived because `assert.match(src, /fn\(/)` is satisfied by `if (false) fn(...)`, and one FALSE kill came from a pin quoting a spelling that an equivalent rewrite changed. **A grep for a name proves the name is there; a pin that dies on a rename was never watching the behaviour** |
| 43 | `SetEnemyCareer` runs three rolls after the loot table (`EnemyEntity.cs:388-397`) that the port never had: `RandomlyAddMap(MapChance)` unconditionally, then `RandomlyAddPotion(3)` and `RandomlyAddPotionRecipe(2)` gated on the enemy having a table. `mapChance` is in all 62 rows - **thirty of them non-zero** - with zero readers, so no enemy had ever dropped a dungeon map, one of classic's main ways of finding new dungeons. `LootTables.GenerateLoot:147-159` runs a DIFFERENT trio for dungeon piles: the chance comes from a six-entry table, only keys J-O roll, and the potion chance is FOUR. `CreateRandomPotion` needed the potion mint that had never been built - DFU draws it from the effect broker's registry while the recipe roll draws from hardcoded `classicRecipeKeys`, and the vendored tree defines twenty recipes against a list of twenty, so unmodded they are one set. The campaign needed two rounds: the enemy recipe chance had no boundary pin, and `?? 0` -> `|| 0` was a FALSE KILL off a spelling assertion - **the same false kill wave 42 recorded one wave earlier**. Writing a law down is not the same as having learned it |
| 44 | `ShowMagicSparkles`, which wave 39 cut on a reading that was wrong twice over. Its ONE call site is inside `EnemyCastReadySpell()` - whose own comment says "For enemies this is equivalent to PlayerSpellCasting_OnReleaseFrame()" - so there is no sparkle on the player's path at all and no player position was ever needed; and `entityBehaviour` there is the MANAGER'S OWN entity (:158), so the sparkles bloom on the CASTER, not the target. Every pool has had that position since it had a foe. The gate is the target type: rangeType 0, 1 and 3 sparkle, the two ranged ones do not, because their visual is the missile. Cutting it was still right; the reason was wrong - a half-wired version would have shipped sparkles on the wrong entity in two arms of three. And the campaign produced a THIRD consecutive false kill off a spelling assertion, so the wave ends with a procedure rather than another paragraph: **when a mutant dies, ask what killed it** - if only a text pin failed, it is a suspect, not a kill |
| 45 | Reported from play: "whenever attempting to attack, it swings your entire screen." RMB is a WEAPON control, but the two streaming hosts are not the only listener - `worldModes.js:2139` registers its own global `mousemove` for the interior and dungeon modes it owns, and both fire on every move. The hosts gated their swing line on `modeNow() === 'exterior'`, which READS as "am I outdoors" when its job is "is anybody else eating this drag": indoors worldModes fed the modal rig and the host fell through to `cam.yaw -= movementX`, so every swing in a building or a town-entered dungeon turned the view with it. `dungeon.js:268` (standalone) always had the right shape - attack, then return, no mode in the test - because it has no modal sibling, which is why the difference never looked like a bug. Fixed with a THREE-answer router (`swing`/`modal`/`look`): the old code had no word for "somebody else is handling this" and used the nearest word it had. **Not a regression from waves 30-44** - no weapon file has changed since wave 24; wave 37's crash fix simply let the player reach an interior again |
| 46 | The player's side of being hit. C2 ported the voice tables whole and wired only the ENEMY side, so the player took every hit in silence - `PlayerFootsteps.RemoveHealth`'s 40% cry was unported. And the male High Elf -> Wood Elf swap is `EnemySounds.PlayCombatVoice`'s alone (its comment: "sound odd when coming from NPCs"); the player's two voices call `GetRaceGender*Sound` DIRECTLY, so `playerAttackGrunt` going through `combatVoice()` had a male High Elf player grunting as a Wood Elf since C2. A FALL flashes the screen and does NOT cry, because `PlayerHealth:57` calls its own `RemoveHealth` where the blows and traps SEND it and Unity's SendMessage reaches every component - a real distinction, pinned as one. Both arrow-on-player paths owed the flash and the cry (`BowDamage` routes through the same `ApplyDamageToPlayer`), and the dungeon's had none of the three. `PlayArrowSound` has NO sender in the tree - dead, unported; an enemy arrow makes the WEAPON hit sound. 20 mutants, 20 kills, zero equivalents, after two rounds in which the first found more about the PINS than the code |

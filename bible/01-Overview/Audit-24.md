...
Grep over src/ confirms only two RumorsPostSuccess uses remain and both are legitimate: src/systems/quest/quest.js:302 (the tombstone success/failure rumor pick) and the prose in the topicTree comment. Nothing in src/ reads a bare 1007 at quest start, so the claimed consequences (post-success text seeded at accept time, missing during-quest rumor, double-write with tombstone) do not occur.

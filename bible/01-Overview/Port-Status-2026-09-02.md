...
the same place - `Port-Ledger.md:723`, the struck keybinding-registry
...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:7603`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:4679` carries the sweep and `hostMagic.js:816` its missile half. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:3084`, `:2792`). Ledger row `:669`'s residue list is spent - see list 2. |
...
and `world.js:291` reaches it by mounting `worldModes`. But `grep -rn
...
- ~~**`src/scenes/dungeonContext.js:2126`** - the two window seams this
  ...
  carries the same closure against Ledger row `:732`. The two WINDOWS
  ...
  `test/x11b.test.js`. This was also Ledger row `:732`'s adjudication,
...
- ~~**`src/scenes/exterior.js:1943`** - PX3: this test host mounts no
  ...
  site. (The `src/scenes/worldModes.js:2296` row below is struck: ROAD-E
...
Section C's table is **247 rows** between `Port-Ledger.md:606` and `:852`
...
route** - plus `:723`, struck at its head but carrying a live PENDING
...
1. **`:641` FaceUVTool's 1,803-UV residual at matched precision** ->
...
2. **`:654` the custom builder's hidden `ResetBonusPool` control** (STRUCK at E2, landing after this list was written: the control is live) ->
...
3. ~~**`:624` remainder: the overlay mouse-UP seam** -> UI arc. A7 shipped
...
4. ~~**`:667` the quest machine** -> `playSound`'s busy-skip, the one
...
5. ~~**`:720` the classic `.SAV` reader** -> the phone path: no zip arm in
   the saves picker, a desktop-first charter call.~~ **CLOSED (OT1, 2026-09-10).**
6. **`:723` (struck, with a live clause) the keybinding registry** ->
   the mouse/advanced and joystick sub-windows, the two of DFU's 65
   game windows the port does not cite.
7. ~~**`:732` the standalone dungeon host has no trade window** -> a
...
8. ~~**`:733` three stale probes**~~ **CLOSED (E-group, 2026-09-02).**
...
9. **`:619` UseItem's unbuilt destinations.** Every arm the row names is
   built: `DrinkPotion` (`systems/useItem.js:205`, `:296-306`),
   `RecordLocationFromMap`/`DiscoverRandomLocation`
   (`ui/nativeInventory.js:834-838`, `scenes/world.js:4733`), the
...
10. **`:669` fast-travel residue.** Its three named survivors are spent:
    ...
    `scenes/world.js:3921` (the arrival clamp anchoring the encounter
...
11. **`:674` `PatchRegionIndex` legacy-save fix.** Ported verbatim at
    `src/formats/mapsFile.js:107`, with the C# line range cited.
12. **`:718` the magic crafting windows.** The row's FLAGGED residue is
    ...
    clause goes with it.**
13. **`:668` the talk manager.** The row's own tail already says both
...
14. **`:698` the small-residue trio** (biography GP arm, arrow roll
...
- **The voxel character engine** (Mac's system, Port-Doctrine). 86
  ...
  by a game host: `worldModes.js:5302` passes `voxelfolk`,
...
- **`FaceUVTool` arithmetic** - JS doubles for C# float mix, `Math.trunc`
  ...
  precision are ledger row `:641`, narrowed at E8 and the only part of

...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:4762`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:2826` carries the sweep and `hostMagic.js:601` its missile half. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:1866`, `:1672`). Ledger row `:552`'s residue list is spent - see list 2. |
...
and `world.js:182` reaches it by mounting `worldModes`. But `grep -rn
...
- ~~**`src/scenes/world.js:3075`** - the port's default landing stands
...
Quests tab has a machine to read), ~~`world.js:3121` -> `:2938`~~
...
9. **`:502` UseItem's unbuilt destinations.** Every arm the row names is
   ...
   (`ui/nativeInventory.js:703-707`, `scenes/world.js:2875`), the
   (`ui/nativeInventory.js:703-707`, `scenes/world.js:2821`), the
...
10. **`:552` fast-travel residue.** Its three named survivors are spent:
    ...
    `scenes/world.js:3835` (the arrival clamp anchoring the encounter
    clock, so the traveled window is not replayed) and `:1966` (the
    `scenes/world.js:3779` (the arrival clamp anchoring the encounter

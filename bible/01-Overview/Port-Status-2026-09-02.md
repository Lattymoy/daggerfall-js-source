...
the same place - `Port-Ledger.md:706`, the struck keybinding-registry
...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:7541`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:4649` carries the sweep and `hostMagic.js:765` its missile half. |
...
| **player** | Near-1:1 / one lethal seam | **Near-1:1** | `cancelMovement` is a real motor field set on the mode edges (`player/motor.js:373,:445,:453`). ROAD-A A6 took the -0.28 doorway head-dip, `PlayerMoveScanner`'s three probes, `controllerSwimHeight` 0.30 with Do(Un)Sinking, and `FreezeMotor` on teleports. |
...
| **characters-voxel** | Departure (deliberate) | **Departure, still editor-only** | 86 designs across seven tables - 42 of the 43 monster mobiles, 19 class, 25 villager. The rig is 1,791 lines across 7 modules and is still gated: `worldModes.js:5285` passes `voxelfolk`, `scenes/interiorContext.js:577` consumes it. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:3058`, `:2766`). Ledger row `:652`'s residue list is spent - see list 2. |
...
that own overlay slots - `worldModes.js:83` and `dungeonContext.js:44` -
and `world.js:275` reaches it by mounting `worldModes`. But `grep -rn
...
- ~~**`src/scenes/dungeonContext.js:2126`** - the two window seams this
  ...
  carries the same closure against Ledger row `:715`. The two WINDOWS
  ...
  `test/x11b.test.js`. This was also Ledger row `:715`'s adjudication,
  and that row is struck with it.
- **`src/ui/enhancedMenu.js:3140`** - the rest of the keyboard; the
...
- ~~**`src/scenes/exterior.js:1946`** - PX3: this test host mounts no
  ...
  site. (The `src/scenes/worldModes.js:2284` row below is struck: ROAD-E
...
- ~~**`src/scenes/world.js:4405`** - the port's default landing stands
...
- ~~**`src/scenes/worldModes.js:1959`** - above ground only:
  ...
  ported and idle at `systems/quest/machine.js:807` including the
...
- **`src/scenes/dungeonContext.js:1188`** - the dungeon's non-layout foe
...
Quests tab has a machine to read), ~~`world.js:4432` -> `:4455`~~
(**CLOSED at the ship landing**), ~~`worldModes.js:1922` -> `:1799`~~
...
- ~~**`src/scenes/worldModes.js:2029`** - above ground only: the GUILD
...
Section C's table is **247 rows** between `Port-Ledger.md:589` and `:835`
...
route** - plus `:706`, struck at its head but carrying a live PENDING
...
1. **`:624` FaceUVTool's 1,803-UV residual at matched precision** ->
...
2. **`:637` the custom builder's hidden `ResetBonusPool` control** (STRUCK at E2, landing after this list was written: the control is live) ->
...
3. ~~**`:607` remainder: the overlay mouse-UP seam** -> UI arc. A7 shipped
...
4. ~~**`:650` the quest machine** -> `playSound`'s busy-skip, the one
...
5. ~~**`:703` the classic `.SAV` reader** -> the phone path: no zip arm in
   the saves picker, a desktop-first charter call.~~ **CLOSED (OT1, 2026-09-10).**
6. **`:706` (struck, with a live clause) the keybinding registry** ->
   the mouse/advanced and joystick sub-windows, the two of DFU's 65
   game windows the port does not cite.
7. ~~**`:715` the standalone dungeon host has no trade window** -> a
...
8. ~~**`:716` three stale probes**~~ **CLOSED (E-group, 2026-09-02).**
...
9. **`:602` UseItem's unbuilt destinations.** Every arm the row names is
   built: `DrinkPotion` (`systems/useItem.js:205`, `:296-306`),
   `RecordLocationFromMap`/`DiscoverRandomLocation`
   (`ui/nativeInventory.js:841-845`, `scenes/world.js:4703`), the
...
10. **`:652` fast-travel residue.** Its three named survivors are spent:
    ...
    `scenes/world.js:3894` (the arrival clamp anchoring the encounter
...
11. **`:657` `PatchRegionIndex` legacy-save fix.** Ported verbatim at
    `src/formats/mapsFile.js:107`, with the C# line range cited.
12. **`:701` the magic crafting windows.** The row's FLAGGED residue is
    ...
    clause goes with it.**
13. **`:651` the talk manager.** The row's own tail already says both
...
14. **`:681` the small-residue trio** (biography GP arm, arrow roll
...
- **The voxel character engine** (Mac's system, Port-Doctrine). 86
  ...
  by a game host: `worldModes.js:5285` passes `voxelfolk`,
...
- **`FaceUVTool` arithmetic** - JS doubles for C# float mix, `Math.trunc`
  ...
  precision are ledger row `:624`, narrowed at E8 and the only part of

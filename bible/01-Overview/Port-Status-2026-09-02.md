...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:5020`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:2978` carries the sweep and `hostMagic.js:608` its missile half. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:1979`, `:1747`). Ledger row `:597`'s residue list is spent - see list 2. |
...
9. **`:547` UseItem's unbuilt destinations.** Every arm the row names is
   ...
   (`ui/nativeInventory.js:717-721`, `scenes/world.js:3027`), the
   (`ui/nativeInventory.js:717-721`, `scenes/world.js:2973`), the
...
10. **`:597` fast-travel residue.** Its three named survivors are spent:
    ...
    `scenes/world.js:4027` (the arrival clamp anchoring the encounter

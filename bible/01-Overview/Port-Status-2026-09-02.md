...
the same place - `Port-Ledger.md:600`, the struck keybinding-registry
...
| **formats-game** | Verbatim readers / broken consumer | **Verbatim** | The `ItemRecord` conversions were in the wave; ROAD-A A4 took the classic-import stragglers (building-level MAPSAVE, the native bank record, `LegacyArtifactIndexBitfieldCheck`). Ledger row `:597`'s one residue is the phone path - no zip arm in the saves picker, a desktop-first charter call. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:1637`, `:1667`). Ledger row `:546`'s residue list is spent - see list 2. |
...
- ~~**`src/scenes/dungeonContext.js:1688`** - the two window seams this
  ...
  carries the same closure against Ledger row `:609`. The two WINDOWS
  ...
  `test/x11b.test.js`. This was also Ledger row `:609`'s adjudication,
...
- **`src/systems/inputActions.js:513`** - STILL FLAGGED: axes and
  ...
  sub-windows are recorded pending at `Port-Ledger.md:600`. The flag's
...
`:1301`. `node tools/regenOpenFlags.mjs --check` and `Home.md` are the
only live inventory.
...
Section C's table is **247 rows** between `Port-Ledger.md:483` and
`:729` (`awk '/^\|/ && !/^\|---/'`). **226 are struck.** Of the 21 that
...
route** - plus `:600`, struck at its head but carrying a live PENDING
...
1. **`:518` FaceUVTool's 1,803-UV residual at matched precision** ->
...
2. **`:531` the custom builder's hidden `ResetBonusPool` control** (STRUCK at E2, landing after this list was written: the control is live) ->
...
3. **`:501` remainder: the overlay mouse-UP seam** -> UI arc. A7 shipped
...
4. ~~**`:544` the quest machine** -> `playSound`'s busy-skip, the one
...
5. **`:597` the classic `.SAV` reader** -> the phone path: no zip arm in
   the saves picker, a desktop-first charter call.
6. **`:600` (struck, with a live clause) the keybinding registry** ->
   the mouse/advanced and joystick sub-windows, the two of DFU's 65
   game windows the port does not cite.
7. ~~**`:609` the standalone dungeon host has no trade window** -> a
...
8. ~~**`:610` three stale probes**~~ **CLOSED (E-group, 2026-09-02).**
...
9. **`:496` UseItem's unbuilt destinations.** Every arm the row names is
...
10. **`:546` fast-travel residue.** Its three named survivors are spent:
...
11. **`:551` `PatchRegionIndex` legacy-save fix.** Ported verbatim at
    `src/formats/mapsFile.js:104`, with the C# line range cited.
12. **`:595` the magic crafting windows.** The row's FLAGGED residue is
    ...
    clause goes with it.**
13. **`:545` the talk manager.** The row's own tail already says both
...
14. **`:575` the small-residue trio** (biography GP arm, arrow roll
    ...
    `BIOG*.TXT` files; the faceUV guard rides row `:518`.

## 3. The deliberate departures, which are not on the road

`Road-To-1-1.md` names these at its head so nobody re-opens them, and
`Port-Ledger.md` section A carries **84 rows, 7 struck - 77 standing
approved departures** (the 70th is WIND1, 2026-09-02, added after this page's measurement and counted here so the tally follows the tree; the 4th is MOBILE TOUCH INPUT, TI1, 2026-09-05, inserted under the windmill row and likewise; the 8th is DS1 - Dynamic Skies vendored 1:1 with permission, 2026-09-04 - inserted directly under the enhanced sky's row it stands beside, which moved every row below it down one; the 9th is SIB1 - Seasons of the Iliac Bay ported 1:1 with permission, 2026-09-05, its textures the player's to supply - inserted directly under DS1, moving every row below down one more; the 10th is ECV1 - Enhanced Combat Visuals, 2026-09-07, the enhanced skin's draw of a concealed foe - inserted directly under SIB1, moving every row below down one more; the 2nd is the re-integrated road system, inserted directly under the struck ROADS row it supersedes, added by AUDIT 58 F5; the 72nd is the pause window's version line, appended by AUDIT 58's records lane, which found `src/ui/pauseWindow.js` citing a Ledger A approval that had never been written down; rows 73-80 are AUDIT 58's seams lane, which found EIGHT more files declaring a departure as already RECORDED - music replacement, the fuzzy find, travel weather, the conversation save, the colour picker, async art in two windows, the town map's plate and arrow, and the input ladder's E and swing - against a section A that rowed none of them; rows 81-83 are ROAD-G G7's records lane, which widened the doctrine gate from the DEPARTURE token to the `Ledger A` CITE and found seventeen files claiming an approval this page did not name - the `AdvancedClimbing` scaffolding, the merchant service popup's accelerators and the port's scalar clock are the three that were genuinely owed a row rather than a roster line or a corrected comment) plus the DUNGEON SEED bullet, now naming its two files, and the
...
- **`FaceUVTool` arithmetic** - JS doubles for C# float mix, `Math.trunc`
  ...
  precision are ledger row `:518`, narrowed at E8 and the only part of

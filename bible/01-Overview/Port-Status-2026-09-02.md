...
the same place - `Port-Ledger.md:639`, the struck keybinding-registry
...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:4912`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:2920` carries the sweep and `hostMagic.js:608` its missile half. |
| **scenes-modes** | Solid, pause parity broken | **Near-1:1** | ROAD-B B1 put `UserInterfaceManager`'s real stack under this host's slot (`ui/windowStack.js`, 295 lines, imported at `worldModes.js:65`). See "the pause primitive" below - the stack exists, its `paused()` member has no reader. |
...
| **player** | Near-1:1 / one lethal seam | **Near-1:1** | `cancelMovement` is a real motor field set on the mode edges (`player/motor.js:361,:439,:447`). ROAD-A A6 took the -0.28 doorway head-dip, `PlayerMoveScanner`'s three probes, `controllerSwimHeight` 0.30 with Do(Un)Sinking, and `FreezeMotor` on teleports. |
...
| **characters-voxel** | Departure (deliberate) | **Departure, still editor-only** | 86 designs across seven tables - 42 of the 43 monster mobiles, 19 class, 25 villager. The rig is 1,791 lines across 7 modules and is still gated: `worldModes.js:4272` passes `voxelfolk`, `scenes/interiorContext.js:483` consumes it. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:1949`, `:1728`). Ledger row `:585`'s residue list is spent - see list 2. |
...
that own overlay slots - `worldModes.js:65` and `dungeonContext.js:37` -
and `world.js:193` reaches it by mounting `worldModes`. But `grep -rn
...
- ~~**`src/scenes/dungeonContext.js:1868`** - the two window seams this
  ...
  carries the same closure against Ledger row `:648`. The two WINDOWS
  ...
  `test/x11b.test.js`. This was also Ledger row `:648`'s adjudication,
  and that row is struck with it.
- **`src/ui/enhancedMenu.js:2288`** - the rest of the keyboard; the
...
- ~~**`src/scenes/exterior.js:1622`** - PX3: this test host mounts no
  ...
  site. (The `src/scenes/worldModes.js:1916` row below is struck: ROAD-E
...
- ~~**`src/scenes/world.js:3152`** - the port's default landing stands
...
- ~~**`src/characters/enemyCasting.js:91`** - now exactly one term
  ...
  second.*
- ~~**`src/scenes/worldModes.js:1862`** - above ground only:
...
- ~~**`src/ui/hudLarge.js:75`** - the docked bar occludes, it does not
  ...
  (`render/renderer.js:1313, :1287, :1783, :1958`) and the 2D passes
...
regenerated onto the new sites: ~~`exterior.js:1290` -> `:1108`~~
(**CLOSED at TP2**: the Recall interim narrowed to the one arm this host
cannot take, `exterior.js:1625`), ~~`exterior.js:1668` -> `:1374`~~
(**CLOSED at QX1**: this host took the quest bridge, so the pause window's
Quests tab has a machine to read), ~~`world.js:3198` -> `:3006`~~
(**CLOSED at the ship landing**), ~~`worldModes.js:1859` -> `:1714`~~
...
- ~~**`src/scenes/worldModes.js:1932`** - above ground only: the GUILD
...
Section C's table is **247 rows** between `Port-Ledger.md:522` and `:768`
...
generation, to the enhanced lane)~~ (**it read 226 and 21 until `:549` was
...
route** - plus `:639`, struck at its head but carrying a live PENDING
...
1. **`:557` FaceUVTool's 1,803-UV residual at matched precision** ->
...
2. **`:570` the custom builder's hidden `ResetBonusPool` control** (STRUCK at E2, landing after this list was written: the control is live) ->
...
3. ~~**`:540` remainder: the overlay mouse-UP seam** -> UI arc. A7 shipped
...
4. ~~**`:583` the quest machine** -> `playSound`'s busy-skip, the one
...
5. ~~**`:636` the classic `.SAV` reader** -> the phone path: no zip arm in
   the saves picker, a desktop-first charter call.~~ **CLOSED (OT1, 2026-09-10).**
6. **`:639` (struck, with a live clause) the keybinding registry** ->
   the mouse/advanced and joystick sub-windows, the two of DFU's 65
   game windows the port does not cite.
7. ~~**`:648` the standalone dungeon host has no trade window** -> a
...
8. ~~**`:649` three stale probes**~~ **CLOSED (E-group, 2026-09-02).**
...
9. **`:535` UseItem's unbuilt destinations.** Every arm the row names is
   built: `DrinkPotion` (`systems/useItem.js:167`, `:245-255`),
   `RecordLocationFromMap`/`DiscoverRandomLocation`
   (`ui/nativeInventory.js:704-708`, `scenes/world.js:2969`), the
   (`ui/nativeInventory.js:704-708`, `scenes/world.js:2915`), the
...
10. **`:585` fast-travel residue.** Its three named survivors are spent:
    ...
    `scenes/world.js:3936` (the arrival clamp anchoring the encounter
    clock, so the traveled window is not replayed) and `:1966` (the
    `scenes/world.js:3880` (the arrival clamp anchoring the encounter
...
11. **`:590` `PatchRegionIndex` legacy-save fix.** Ported verbatim at
    `src/formats/mapsFile.js:104`, with the C# line range cited.
12. **`:634` the magic crafting windows.** The row's FLAGGED residue is
    ...
    clause goes with it.**
13. **`:584` the talk manager.** The row's own tail already says both
...
14. **`:614` the small-residue trio** (biography GP arm, arrow roll
...
- **The voxel character engine** (Mac's system, Port-Doctrine). 86
  ...
  by a game host: `worldModes.js:4272` passes `voxelfolk`,
  `scenes/interiorContext.js:483` gates on it, and no default route sets
...
- **`FaceUVTool` arithmetic** - JS doubles for C# float mix, `Math.trunc`
  ...
  precision are ledger row `:557`, narrowed at E8 and the only part of

...
| **formats-mw** | Departure lane, faithful within it | **Departure lane, now consumed** | `clipSweepTimes` has a production caller: `combat/fpArm.js:1700` runs the whole-clip reach sweep the superseded page said had never run in the game. 17 modules / 8,842 lines. |
...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:5028`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:2993` carries the sweep and `hostMagic.js:608` its missile half. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:1973`, `:1741`). Ledger row `:596`'s residue list is spent - see list 2. |
...
- **`src/scenes/exterior.js:1768`** - Recall pends here; the anchor
...
- ~~**`src/scenes/world.js:3221`** - the port's default landing stands
...
- ~~**`src/ui/hudLarge.js:75`** - the docked bar occludes, it does not
  ...
  (`render/renderer.js:1327, :1301, :1797, :1972`) and the 2D passes
...
Quests tab has a machine to read), ~~`world.js:3267` -> `:3072`~~
...
9. **`:546` UseItem's unbuilt destinations.** Every arm the row names is
   ...
   (`ui/nativeInventory.js:717-721`, `scenes/world.js:3042`), the
   (`ui/nativeInventory.js:717-721`, `scenes/world.js:2988`), the
...
10. **`:596` fast-travel residue.** Its three named survivors are spent:
    ...
    `scenes/world.js:3986` (the arrival clamp anchoring the encounter

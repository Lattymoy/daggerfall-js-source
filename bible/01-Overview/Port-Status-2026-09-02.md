...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:5331`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:3145` carries the sweep and `hostMagic.js:608` its missile half. |
...
- **`src/scenes/exterior.js:1855`** - Recall pends here; the anchor
...
9. **`:559` UseItem's unbuilt destinations.** Every arm the row names is
   ...
   (`ui/nativeInventory.js:736-740`, `scenes/world.js:3194`), the
   (`ui/nativeInventory.js:736-740`, `scenes/world.js:3140`), the
...
10. **`:609` fast-travel residue.** Its three named survivors are spent:
    ...
    `scenes/world.js:4270` (the arrival clamp anchoring the encounter
    clock, so the traveled window is not replayed) and `:1969` (the
    `scenes/world.js:4214` (the arrival clamp anchoring the encounter

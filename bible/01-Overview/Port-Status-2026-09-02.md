...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:7835`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:4857` carries the sweep and `hostMagic.js:792` its missile half. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:3246`, `:2943`). Ledger row `:668`'s residue list is spent - see list 2. |
...
and `world.js:283` reaches it by mounting `worldModes`. But `grep -rn
...
- **`src/ui/enhancedMenu.js:3351`** - the rest of the keyboard; the
...
- ~~**`src/scenes/world.js:4430`** - the port's default landing stands
...
- **`src/scenes/dungeonContext.js:1223`** - the dungeon's non-layout foe
...
Quests tab has a machine to read), ~~`world.js:4457` -> `:4480`~~
...
9. **`:618` UseItem's unbuilt destinations.** Every arm the row names is
   ...
   (`ui/nativeInventory.js:834-838`, `scenes/world.js:4911`), the
...
10. **`:668` fast-travel residue.** Its three named survivors are spent:
    ...
    `scenes/world.js:4091` (the arrival clamp anchoring the encounter

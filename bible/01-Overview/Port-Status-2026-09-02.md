...
the same place - `Port-Ledger.md:644`, the struck keybinding-registry
...
- ~~**`src/scenes/dungeonContext.js:1868`** - the two window seams this
  ...
  carries the same closure against Ledger row `:653`. The two WINDOWS
  ...
  `test/x11b.test.js`. This was also Ledger row `:653`'s adjudication,
...
Section C's table is **247 rows** between `Port-Ledger.md:527` and `:773`
...
route** - plus `:644`, struck at its head but carrying a live PENDING
...
1. **`:562` FaceUVTool's 1,803-UV residual at matched precision** ->
...
2. **`:575` the custom builder's hidden `ResetBonusPool` control** (STRUCK at E2, landing after this list was written: the control is live) ->
...
3. ~~**`:545` remainder: the overlay mouse-UP seam** -> UI arc. A7 shipped
...
4. ~~**`:588` the quest machine** -> `playSound`'s busy-skip, the one
...
5. ~~**`:641` the classic `.SAV` reader** -> the phone path: no zip arm in
   the saves picker, a desktop-first charter call.~~ **CLOSED (OT1, 2026-09-10).**
6. **`:644` (struck, with a live clause) the keybinding registry** ->
   the mouse/advanced and joystick sub-windows, the two of DFU's 65
   game windows the port does not cite.
7. ~~**`:653` the standalone dungeon host has no trade window** -> a
...
8. ~~**`:654` three stale probes**~~ **CLOSED (E-group, 2026-09-02).**
...
9. **`:540` UseItem's unbuilt destinations.** Every arm the row names is
...
10. **`:590` fast-travel residue.** Its three named survivors are spent:
...
11. **`:595` `PatchRegionIndex` legacy-save fix.** Ported verbatim at
    `src/formats/mapsFile.js:104`, with the C# line range cited.
12. **`:639` the magic crafting windows.** The row's FLAGGED residue is
    ...
    clause goes with it.**
13. **`:589` the talk manager.** The row's own tail already says both
...
14. **`:619` the small-residue trio** (biography GP arm, arrow roll
...
- **`FaceUVTool` arithmetic** - JS doubles for C# float mix, `Math.trunc`
  ...
  precision are ledger row `:562`, narrowed at E8 and the only part of

...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:4879`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:2891` carries the sweep and `hostMagic.js:608` its missile half. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:1932`, `:1711`). Ledger row `:580`'s residue list is spent - see list 2. |
...
and `world.js:189` reaches it by mounting `worldModes`. But `grep -rn
...
9. **`:530` UseItem's unbuilt destinations.** Every arm the row names is
   ...
   (`ui/nativeInventory.js:704-708`, `scenes/world.js:2940`), the
   (`ui/nativeInventory.js:704-708`, `scenes/world.js:2886`), the
...
10. **`:580` fast-travel residue.** Its three named survivors are spent:
    ...
    `scenes/world.js:3849` (the arrival clamp anchoring the encounter

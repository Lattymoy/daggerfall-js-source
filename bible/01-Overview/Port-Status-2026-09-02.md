...
the same place - `Port-Ledger.md:697`, the struck keybinding-registry
...
| **formats-game** | Verbatim readers / broken consumer | **Verbatim** | The `ItemRecord` conversions were in the wave; ROAD-A A4 took the classic-import stragglers (building-level MAPSAVE, the native bank record, `LegacyArtifactIndexBitfieldCheck`). ~~Ledger row `:694`'s one residue is the phone path - no zip arm in the saves picker, a desktop-first charter call.~~ **CLOSED (OT1, 2026-09-10): the saves picker took the ARENA2 door's zip arm, and the row is struck whole.** |
| **formats-mw** | Departure lane, faithful within it | **Departure lane, now consumed** | `clipSweepTimes` has a production caller: `combat/fpArm.js:1784` runs the whole-clip reach sweep the superseded page said had never run in the game. 17 modules / 8,842 lines. |
...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:7482`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:4619` carries the sweep and `hostMagic.js:751` its missile half. |
...
| **characters-voxel** | Departure (deliberate) | **Departure, still editor-only** | 86 designs across seven tables - 42 of the 43 monster mobiles, 19 class, 25 villager. The rig is 1,791 lines across 7 modules and is still gated: `worldModes.js:5290` passes `voxelfolk`, `scenes/interiorContext.js:513` consumes it. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:3048`, `:2756`). Ledger row `:643`'s residue list is spent - see list 2. |
...
| **ui-enhanced** | Departure lane | **Departure lane, scoped** | `ui/worldPlaque.js:316` puts the skin gate above `ensure()`, so the unscoped `*`/`html`/`body`/`button`/`#app` rules never reach the classic page. 9 modules / 8,940 lines. |
...
- ~~**`src/scenes/dungeonContext.js:2123`** - the two window seams this
  ...
  carries the same closure against Ledger row `:706`. The two WINDOWS
  ...
  `test/x11b.test.js`. This was also Ledger row `:706`'s adjudication,
...
- **`src/scenes/exterior.js:2247`** - Recall pends here; the anchor
...
- ~~**`src/scenes/exterior.js:1936`** - PX3: this test host mounts no
  ...
  site. (The `src/scenes/worldModes.js:2278` row below is struck: ROAD-E
...
- ~~**`src/scenes/world.js:4386`** - the port's default landing stands
...
- ~~**`src/characters/enemyCasting.js:91`** - now exactly one term
  ...
  second.*
- ~~**`src/scenes/worldModes.js:1958`** - above ground only:
...
- ~~**`src/ui/hudLarge.js:75`** - the docked bar occludes, it does not
  ...
  (`render/renderer.js:2120, :2046, :2867, :3190`) and the 2D passes
...
- **`src/scenes/dungeonContext.js:1174`** - the dungeon's non-layout foe
...
regenerated onto the new sites: ~~`exterior.js:1594` -> `:1282`~~
(**CLOSED at TP2**: the Recall interim narrowed to the one arm this host
cannot take, `exterior.js:1958`), ~~`exterior.js:2008` -> `:1674`~~
(**CLOSED at QX1**: this host took the quest bridge, so the pause window's
Quests tab has a machine to read), ~~`world.js:4413` -> `:4406`~~
(**CLOSED at the ship landing**), ~~`worldModes.js:1921` -> `:1798`~~
...
- ~~**`src/scenes/worldModes.js:2028`** - above ground only: the GUILD
...
Section C's table is **247 rows** between `Port-Ledger.md:580` and `:826`
...
route** - plus `:697`, struck at its head but carrying a live PENDING
...
1. **`:615` FaceUVTool's 1,803-UV residual at matched precision** ->
...
2. **`:628` the custom builder's hidden `ResetBonusPool` control** (STRUCK at E2, landing after this list was written: the control is live) ->
...
3. ~~**`:598` remainder: the overlay mouse-UP seam** -> UI arc. A7 shipped
...
4. ~~**`:641` the quest machine** -> `playSound`'s busy-skip, the one
...
5. ~~**`:694` the classic `.SAV` reader** -> the phone path: no zip arm in
   the saves picker, a desktop-first charter call.~~ **CLOSED (OT1, 2026-09-10).**
6. **`:697` (struck, with a live clause) the keybinding registry** ->
   the mouse/advanced and joystick sub-windows, the two of DFU's 65
   game windows the port does not cite.
7. ~~**`:706` the standalone dungeon host has no trade window** -> a
...
8. ~~**`:707` three stale probes**~~ **CLOSED (E-group, 2026-09-02).**
...
9. **`:593` UseItem's unbuilt destinations.** Every arm the row names is
   built: `DrinkPotion` (`systems/useItem.js:200`, `:291-301`),
   `RecordLocationFromMap`/`DiscoverRandomLocation`
   (`ui/nativeInventory.js:809-813`, `scenes/world.js:4673`), the
...
10. **`:643` fast-travel residue.** Its three named survivors are spent:
...
11. **`:648` `PatchRegionIndex` legacy-save fix.** Ported verbatim at
    `src/formats/mapsFile.js:105`, with the C# line range cited.
12. **`:692` the magic crafting windows.** The row's FLAGGED residue is
    ...
    clause goes with it.**
13. **`:642` the talk manager.** The row's own tail already says both
...
14. **`:672` the small-residue trio** (biography GP arm, arrow roll
...
- **`FaceUVTool` arithmetic** - JS doubles for C# float mix, `Math.trunc`
  ...
  precision are ledger row `:615`, narrowed at E8 and the only part of

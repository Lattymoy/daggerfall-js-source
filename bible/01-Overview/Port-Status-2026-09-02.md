...
the same place - `Port-Ledger.md:707`, the struck keybinding-registry
...
| **formats-game** | Verbatim readers / broken consumer | **Verbatim** | The `ItemRecord` conversions were in the wave; ROAD-A A4 took the classic-import stragglers (building-level MAPSAVE, the native bank record, `LegacyArtifactIndexBitfieldCheck`). ~~Ledger row `:704`'s one residue is the phone path - no zip arm in the saves picker, a desktop-first charter call.~~ **CLOSED (OT1, 2026-9-10): the saves picker took the ARENA2 door's zip arm, and the row is struck whole.** |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:3070`, `:2778`). Ledger row `:653`'s residue list is spent - see list 2. |
...
- ~~**`src/scenes/dungeonContext.js:2126`** - the two window seams this
  ...
  carries the same closure against Ledger row `:716`. The two WINDOWS
  ...
  `test/x11b.test.js`. This was also Ledger row `:716`'s adjudication,
...
Section C's table is **247 rows** between `Port-Ledger.md:590` and `:836`
...
route** - plus `:707`, struck at its head but carrying a live PENDING
...
1. **`:625` FaceUVTool's 1,803-UV residual at matched precision** ->
...
2. **`:638` the custom builder's hidden `ResetBonusPool` control** (STRUCK at E2, landing after this list was written: the control is live) ->
...
3. ~~**`:608` remainder: the overlay mouse-UP seam** -> UI arc. A7 shipped
...
4. ~~**`:651` the quest machine** -> `playSound`'s busy-skip, the one
...
5. ~~**`:704` the classic `.SAV` reader** -> the phone path: no zip arm in
   the saves picker, a desktop-first charter call.~~ **CLOSED (OT1, 2026-09-10).**
6. **`:707` (struck, with a live clause) the keybinding registry** ->
   the mouse/advanced and joystick sub-windows, the two of DFU's 65
   game windows the port does not cite.
7. ~~**`:716` the standalone dungeon host has no trade window** -> a
...
8. ~~**`:717` three stale probes**~~ **CLOSED (E-group, 2026-9-2).**
...
9. **`:603` UseItem's unbuilt destinations.** Every arm the row names is
...
10. **`:653` fast-travel residue.** Its three named survivors are spent:
...
11. **`:658` `PatchRegionIndex` legacy-save fix.** Ported verbatim at
    `src/formats/mapsFile.js:107`, with the C# line range cited.
12. **`:702` the magic crafting windows.** The row's FLAGGED residue is
    ...
    clause goes with it.**
13. **`:652` the talk manager.** The row's own tail already says both
...
14. **`:682` the small-residue trio** (biography GP arm, arrow roll
...
- **`FaceUVTool` arithmetic** - JS doubles for C# float mix, `Math.trunc`
  ...
  precision are ledger row `:625`, narrowed at E8 and the only part of

...
the same place - `Port-Ledger.md:693`, the struck keybinding-registry
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:2485`, `:2201`). Ledger row `:639`'s residue list is spent - see list 2. |
...
- ~~**`src/scenes/dungeonContext.js:2115`** - the two window seams this
  ...
  carries the same closure against Ledger row `:702`. The two WINDOWS
  ...
  `test/x11b.test.js`. This was also Ledger row `:702`'s adjudication,
...
Section C's table is **247 rows** between `Port-Ledger.md:576` and `:822`
...
route** - plus `:693`, struck at its head but carrying a live PENDING
...
1. **`:611` FaceUVTool's 1,803-UV residual at matched precision** ->
...
2. **`:624` the custom builder's hidden `ResetBonusPool` control** (STRUCK at E2, landing after this list was written: the control is live) ->
...
3. ~~**`:594` remainder: the overlay mouse-UP seam** -> UI arc. A7 shipped
...
4. ~~**`:637` the quest machine** -> `playSound`'s busy-skip, the one
...
5. ~~**`:690` the classic `.SAV` reader** -> the phone path: no zip arm in
   the saves picker, a desktop-first charter call.~~ **CLOSED (OT1, 2026-09-10).**
6. **`:693` (struck, with a live clause) the keybinding registry** ->
   the mouse/advanced and joystick sub-windows, the two of DFU's 65
   game windows the port does not cite.
7. ~~**`:702` the standalone dungeon host has no trade window** -> a
...
8. ~~**`:703` three stale probes**~~ **CLOSED (E-group, 2026-09-02).**
...
9. **`:589` UseItem's unbuilt destinations.** Every arm the row names is
...
10. **`:639` fast-travel residue.** Its three named survivors are spent:
...
11. **`:644` `PatchRegionIndex` legacy-save fix.** Ported verbatim at
    `src/formats/mapsFile.js:104`, with the C# line range cited.
12. **`:688` the magic crafting windows.** The row's FLAGGED residue is
    ...
    clause goes with it.**
13. **`:638` the talk manager.** The row's own tail already says both
...
14. **`:668` the small-residue trio** (biography GP arm, arrow roll
...
- **`FaceUVTool` arithmetic** - JS doubles for C# float mix, `Math.trunc`
  ...
  precision are ledger row `:611`, narrowed at E8 and the only part of

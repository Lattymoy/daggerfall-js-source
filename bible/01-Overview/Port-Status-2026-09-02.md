...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:5044`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:3002` carries the sweep and `hostMagic.js:608` its missile half. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:1982`, `:1750`). Ledger row `:597`'s residue list is spent - see list 2. |
...
9. **`:547` UseItem's unbuilt destinations.** Every arm the row names is
   ...
   (`ui/nativeInventory.js:717-721`, `scenes/world.js:3051`), the
   (`ui/nativeInventory.js:717-721`, `scenes/world.js:2997`), the
...
10. **`:597` fast-travel residue.** Its three named survivors are spent:
    ...
    `scenes/world.js:4051` (the arrival clamp anchoring the encounter
    clock, so the traveled window is not replayed) and `:1966` (the
    `scenes/world.js:3995` (the arrival clamp anchoring the encounter

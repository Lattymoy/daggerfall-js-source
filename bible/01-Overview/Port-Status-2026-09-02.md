...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:7475`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:4625` carries the sweep and `hostMagic.js:711` its missile half. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:3056`, `:2764`). Ledger row `:644`'s residue list is spent - see list 2. |
...
regenerated onto the new sites: ~~`exterior.js:1599` -> `:1292`~~
(**CLOSED at TP2**: the Recall interim narrowed to the one arm this host
cannot take, `exterior.js:1963`), ~~`exterior.js:2013` -> `:1684`~~
(**CLOSED at QX1**: this host took the quest bridge, so the pause window's
Quests tab has a machine to read), ~~`world.js:4414` -> `:4412`~~
...
9. **`:594` UseItem's unbuilt destinations.** Every arm the row names is
   ...
   (`ui/nativeInventory.js:809-813`, `scenes/world.js:4679`), the
...
10. **`:644` fast-travel residue.** Its three named survivors are spent:
    ...
    `scenes/world.js:3874` (the arrival clamp anchoring the encounter

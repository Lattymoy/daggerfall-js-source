...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:6332`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:3704` carries the sweep and `hostMagic.js:608` its missile half. |
...
| **characters-voxel** | Departure (deliberate) | **Departure, still editor-only** | 86 designs across seven tables - 42 of the 43 monster mobiles, 19 class, 25 villager. The rig is 1,791 lines across 7 modules and is still gated: `worldModes.js:4472` passes `voxelfolk`, `scenes/interiorContext.js:503` consumes it. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:2406`, `:2122`). Ledger row `:629`'s residue list is spent - see list 2. |
...
- **`src/scenes/exterior.js:1981`** - Recall pends here; the anchor
...
- ~~**`src/scenes/exterior.js:1753`** - PX3: this test host mounts no
  ...
  site. (The `src/scenes/worldModes.js:2022` row below is struck: ROAD-E
...
- ~~**`src/ui/hudLarge.js:75`** - the docked bar occludes, it does not
  ...
  (`render/renderer.js:1984, :1911, :2731, :3043`) and the 2D passes
...
9. **`:579` UseItem's unbuilt destinations.** Every arm the row names is
   ...
   (`ui/nativeInventory.js:778-782`, `scenes/world.js:3804`), the
...
10. **`:629` fast-travel residue.** Its three named survivors are spent:
    ...
    `scenes/world.js:4869` (the arrival clamp anchoring the encounter
...
- **The voxel character engine** (Mac's system, Port-Doctrine). 86
  ...
  by a game host: `worldModes.js:4472` passes `voxelfolk`,

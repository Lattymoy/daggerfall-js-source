...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:7589`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:4686` carries the sweep and `hostMagic.js:792` its missile half. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:3092`, `:2800`). Ledger row `:657`'s residue list is spent - see list 2. |
...
9. **`:607` UseItem's unbuilt destinations.** Every arm the row names is
   ...
   (`ui/nativeInventory.js:841-845`, `scenes/world.js:4740`), the
...
10. **`:657` fast-travel residue.** Its three named survivors are spent:
    ...
    `scenes/world.js:3928` (the arrival clamp anchoring the encounter

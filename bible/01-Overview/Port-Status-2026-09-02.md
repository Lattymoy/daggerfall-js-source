...
the same place - `Port-Ledger.md:738`, the struck keybinding-registry
...
written by both curses (`systems/vampirism.js:180`,
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:3696`, `:3390`). Ledger row `:684`'s residue list is spent - see list 2. |
...
- ~~**`src/scenes/dungeonContext.js:2126`** - the two window seams this
  ...
  carries the same closure against Ledger row `:747`. The two WINDOWS
  ...
  `test/x11b.test.js`. This was also Ledger row `:747`'s adjudication,
...
- ~~**`src/ui/hudLarge.js:75`** - the docked bar occludes, it does not
  ...
  (`render/renderer.js:2339, :2061, :3079, :3413`) and the 2D passes
...
Section C's table is **247 rows** between `Port-Ledger.md:621` and `:867`
...
route** - plus `:738`, struck at its head but carrying a live PENDING
...
1. **`:656` FaceUVTool's 1,803-UV residual at matched precision** ->
...
2. **`:669` the custom builder's hidden `ResetBonusPool` control** (STRUCK at E2, landing after this list was written: the control is live) ->
...
3. ~~**`:639` remainder: the overlay mouse-UP seam** -> UI arc. A7 shipped
...
4. ~~**`:682` the quest machine** -> `playSound`'s busy-skip, the one
...
5. ~~**`:735` the classic `.SAV` reader** -> the phone path: no zip arm in
   the saves picker, a desktop-first charter call.~~ **CLOSED (OT1, 2026-09-10).**
6. **`:738` (struck, with a live clause) the keybinding registry** ->
   the mouse/advanced and joystick sub-windows, the two of DFU's 65
   game windows the port does not cite.
7. ~~**`:747` the standalone dungeon host has no trade window** -> a
...
8. ~~**`:748` three stale probes**~~ **CLOSED (E-group, 2026-09-02).**
...
9. **`:634` UseItem's unbuilt destinations.** Every arm the row names is
...
10. **`:684` fast-travel residue.** Its three named survivors are spent:
...
11. **`:689` `PatchRegionIndex` legacy-save fix.** Ported verbatim at
    `src/formats/mapsFile.js:107`, with the C# line range cited.
12. **`:733` the magic crafting windows.** The row's FLAGGED residue is
    ...
    clause goes with it.**
13. **`:683` the talk manager.** The row's own tail already says both
...
14. **`:713` the small-residue trio** (biography GP arm, arrow roll
...
- **`FaceUVTool` arithmetic** - JS doubles for C# float mix, `Math.trunc`
  ...
  precision are ledger row `:656`, narrowed at E8 and the only part of

...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:6247`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:3728` carries the sweep and `hostMagic.js:608` its missile half. |
...
| **characters-voxel** | Departure (deliberate) | **Departure, still editor-only** | 86 designs across seven tables - 42 of the 43 monster mobiles, 19 class, 25 villager. The rig is 1,791 lines across 7 modules and is still gated: `worldModes.js:4389` passes `voxelfolk`, `scenes/interiorContext.js:513` consumes it. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:2384`, `:2094`). Ledger row `:622`'s residue list is spent - see list 2. |
...
and `world.js:238` reaches it by mounting `worldModes`. But `grep -rn
...
- ~~**`src/scenes/dungeonContext.js:1953`** - the two window seams this
...
- ~~**`src/scenes/exterior.js:1747`** - PX3: this test host mounts no
  ...
  site. (The `src/scenes/worldModes.js:1998` row below is struck: ROAD-E
...
- ~~**`src/characters/enemyCasting.js:91`** - now exactly one term
  ...
  second.*
- ~~**`src/scenes/worldModes.js:1913`** - above ground only:
...
Quests tab has a machine to read), ~~`world.js:3690` -> `:3686`~~
(**CLOSED at the ship landing**), ~~`worldModes.js:1876` -> `:1769`~~
...
- ~~**`src/scenes/worldModes.js:1983`** - above ground only: the GUILD
...
9. **`:572` UseItem's unbuilt destinations.** Every arm the row names is
   ...
   (`ui/nativeInventory.js:747-751`, `scenes/world.js:3813`), the
...
10. **`:622` fast-travel residue.** Its three named survivors are spent:
    ...
    `scenes/world.js:4861` (the arrival clamp anchoring the encounter
...
- **The voxel character engine** (Mac's system, Port-Doctrine). 86
  ...
  by a game host: `worldModes.js:4389` passes `voxelfolk`,

...
the same place - `Port-Ledger.md:614`, the struck keybinding-registry
...
| **formats-game** | Verbatim readers / broken consumer | **Verbatim** | The `ItemRecord` conversions were in the wave; ROAD-A A4 took the classic-import stragglers (building-level MAPSAVE, the native bank record, `LegacyArtifactIndexBitfieldCheck`). ~~Ledger row `:611`'s one residue is the phone path - no zip arm in the saves picker, a desktop-first charter call.~~ **CLOSED (OT1, 2026-09-10): the saves picker took the ARENA2 door's zip arm, and the row is struck whole.** |
| **formats-mw** | Departure lane, faithful within it | **Departure lane, now consumed** | `clipSweepTimes` has a production caller: `combat/fpArm.js:1532` runs the whole-clip reach sweep the superseded page said had never run in the game. 17 modules / 8,842 lines. |
...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:4809`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:2847` carries the sweep and `hostMagic.js:601` its missile half. |
...
| **characters-voxel** | Departure (deliberate) | **Departure, still editor-only** | 86 designs across seven tables - 42 of the 43 monster mobiles, 19 class, 25 villager. The rig is 1,791 lines across 7 modules and is still gated: `worldModes.js:4173` passes `voxelfolk`, `scenes/interiorContext.js:482` consumes it. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:1887`, `:1693`). Ledger row `:560`'s residue list is spent - see list 2. |
...
- ~~**`src/scenes/dungeonContext.js:1842`** - the two window seams this
  ...
  carries the same closure against Ledger row `:623`. The two WINDOWS
  ...
  `test/x11b.test.js`. This was also Ledger row `:623`'s adjudication,
...
- **`src/scenes/exterior.js:1720`** - Recall pends here; the anchor
...
- ~~**`src/scenes/exterior.js:1609`** - PX3: this test host mounts no
  ...
  site. (The `src/scenes/worldModes.js:1838` row below is struck: ROAD-E
...
- ~~**`src/scenes/world.js:3096`** - the port's default landing stands
...
- ~~**`src/characters/enemyCasting.js:91`** - now exactly one term
  ...
  second.*
- ~~**`src/scenes/worldModes.js:1838`** - above ground only:
...
- ~~**`src/ui/hudLarge.js:75`** - the docked bar occludes, it does not
  ...
  (`render/renderer.js:1271, :1245, :1741, :1916`) and the 2D passes
...
regenerated onto the new sites: ~~`exterior.js:1278` -> `:1106`~~
...
Quests tab has a machine to read), ~~`world.js:3142` -> `:2969`~~
...
- ~~**`src/scenes/worldModes.js:1908`** - above ground only: the GUILD
...
Section C's table is **247 rows** between `Port-Ledger.md:497` and `:743`
...
route** - plus `:614`, struck at its head but carrying a live PENDING
...
1. **`:532` FaceUVTool's 1,803-UV residual at matched precision** ->
...
2. **`:545` the custom builder's hidden `ResetBonusPool` control** (STRUCK at E2, landing after this list was written: the control is live) ->
...
3. ~~**`:515` remainder: the overlay mouse-UP seam** -> UI arc. A7 shipped
...
4. ~~**`:558` the quest machine** -> `playSound`'s busy-skip, the one
...
5. ~~**`:611` the classic `.SAV` reader** -> the phone path: no zip arm in
   the saves picker, a desktop-first charter call.~~ **CLOSED (OT1, 2026-09-10).**
6. **`:614` (struck, with a live clause) the keybinding registry** ->
   the mouse/advanced and joystick sub-windows, the two of DFU's 65
   game windows the port does not cite.
7. ~~**`:623` the standalone dungeon host has no trade window** -> a
...
8. ~~**`:624` three stale probes**~~ **CLOSED (E-group, 2026-09-02).**
...
9. **`:510` UseItem's unbuilt destinations.** Every arm the row names is
   built: `DrinkPotion` (`systems/useItem.js:167`, `:245-255`),
   `RecordLocationFromMap`/`DiscoverRandomLocation`
   (`ui/nativeInventory.js:703-707`, `scenes/world.js:2896`), the
   (`ui/nativeInventory.js:703-707`, `scenes/world.js:2842`), the
...
10. **`:560` fast-travel residue.** Its three named survivors are spent:
    ...
    `scenes/world.js:3861` (the arrival clamp anchoring the encounter
    clock, so the traveled window is not replayed) and `:1966` (the
    `scenes/world.js:3805` (the arrival clamp anchoring the encounter
...
11. **`:565` `PatchRegionIndex` legacy-save fix.** Ported verbatim at
    `src/formats/mapsFile.js:104`, with the C# line range cited.
12. **`:609` the magic crafting windows.** The row's FLAGGED residue is
    ...
    clause goes with it.**
13. **`:559` the talk manager.** The row's own tail already says both
...
14. **`:589` the small-residue trio** (biography GP arm, arrow roll
...
- **The voxel character engine** (Mac's system, Port-Doctrine). 86
  ...
  by a game host: `worldModes.js:4173` passes `voxelfolk`,
  `scenes/interiorContext.js:482` gates on it, and no default route sets
...
- **`FaceUVTool` arithmetic** - JS doubles for C# float mix, `Math.trunc`
  ...
  precision are ledger row `:532`, narrowed at E8 and the only part of

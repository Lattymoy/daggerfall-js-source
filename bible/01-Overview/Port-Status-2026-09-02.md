...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:8585`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:5544` carries the sweep and `hostMagic.js:816` its missile half. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:3927`, `:3621`). Ledger row `:682`'s residue list is spent - see list 2. |
...
- ~~**`src/ui/hudLarge.js:75`** - the docked bar occludes, it does not
  ...
  (`render/renderer.js:2351, :2073, :3091, :3416`) and the 2D passes
...
9. **`:632` UseItem's unbuilt destinations.** Every arm the row names is
   ...
   (`ui/nativeInventory.js:842-846`, `scenes/world.js:5598`), the
...
10. **`:682` fast-travel residue.** Its three named survivors are spent:
    ...
    `scenes/world.js:4778` (the arrival clamp anchoring the encounter

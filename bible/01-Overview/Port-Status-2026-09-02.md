...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:6793`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:4013` carries the sweep and `hostMagic.js:711` its missile half. |
...
| **ui-enhanced** | Departure lane | **Departure lane, scoped** | `ui/worldPlaque.js:315` puts the skin gate above `ensure()`, so the unscoped `*`/`html`/`body`/`button`/`#app` rules never reach the classic page. 9 modules / 8,940 lines. |
...
- **`src/scenes/exterior.js:2156`** - Recall pends here; the anchor
...
- ~~**`src/scenes/world.js:3780`** - the port's default landing stands
...
- ~~**`src/scenes/worldModes.js:1939`** - above ground only:
  ...
  ported and idle at `systems/quest/machine.js:784` including the
...
regenerated onto the new sites: ~~`exterior.js:1526` -> `:1218`~~
(**CLOSED at TP2**: the Recall interim narrowed to the one arm this host
cannot take, `exterior.js:1868`), ~~`exterior.js:1918` -> `:1606`~~
(**CLOSED at QX1**: this host took the quest bridge, so the pause window's
Quests tab has a machine to read), ~~`world.js:3807` -> `:3800`~~
...
9. **`:590` UseItem's unbuilt destinations.** Every arm the row names is
   ...
   (`ui/nativeInventory.js:798-802`, `scenes/world.js:4067`), the
...
10. **`:640` fast-travel residue.** Its three named survivors are spent:
    ...
    `scenes/world.js:3282` (the arrival clamp anchoring the encounter

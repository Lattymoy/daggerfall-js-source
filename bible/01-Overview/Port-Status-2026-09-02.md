...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:4504`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:2670` carries the sweep and `hostMagic.js:601` its missile half. |
...
| **characters-voxel** | Departure (deliberate) | **Departure, still editor-only** | 86 designs across seven tables - 42 of the 43 monster mobiles, 19 class, 25 villager. The rig is 1,791 lines across 7 modules and is still gated: `worldModes.js:4029` passes `voxelfolk`, `scenes/interiorContext.js:455` consumes it. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:1712`, `:1667`). Ledger row `:548`'s residue list is spent - see list 2. |
...
- **`src/ui/enhancedMenu.js:1762`** - the rest of the keyboard; the
...
- ~~**`src/scenes/exterior.js:1475`** - PX3: this test host mounts no
  ...
  site. (The `src/scenes/worldModes.js:1810` row below is struck: ROAD-E
...
- ~~**`src/scenes/world.js:2922`** - the port's default landing stands
...
- ~~**`src/characters/enemyCasting.js:91`** - now exactly one term
  ...
  second.*
- ~~**`src/scenes/worldModes.js:1810`** - above ground only:
...
pause window's Quests tab has a machine to read), `world.js:2968` ->
`:2934`, ~~`worldModes.js:1807` -> `:1687`~~ (**CLOSED at ROAD-F GS1**,
...
- ~~**`src/scenes/worldModes.js:1880`** - above ground only: the GUILD
...
9. **`:498` UseItem's unbuilt destinations.** Every arm the row names is
   ...
   (`ui/nativeInventory.js:670-674`, `scenes/world.js:2719`), the
   (`ui/nativeInventory.js:670-674`, `scenes/world.js:2665`), the
...
10. **`:548` fast-travel residue.** Its three named survivors are spent:
    ...
    `scenes/world.js:3651` (the arrival clamp anchoring the encounter
    clock, so the traveled window is not replayed) and `:1965` (the
    `scenes/world.js:3604` (the arrival clamp anchoring the encounter
...
- **The voxel character engine** (Mac's system, Port-Doctrine). 86
  ...
  by a game host: `worldModes.js:4029` passes `voxelfolk`,

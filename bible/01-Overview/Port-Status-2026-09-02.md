...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:5394`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:3205` carries the sweep and `hostMagic.js:608` its missile half. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:2048`, `:1807`). Ledger row `:610`'s residue list is spent - see list 2. |
...
- ~~**`src/scenes/world.js:3326`** - the port's default landing stands
...
Quests tab has a machine to read), ~~`world.js:3365` -> `:3213`~~
...
9. **`:560` UseItem's unbuilt destinations.** Every arm the row names is
   ...
   (`ui/nativeInventory.js:736-740`, `scenes/world.js:3254`), the
   (`ui/nativeInventory.js:736-740`, `scenes/world.js:3200`), the
...
10. **`:610` fast-travel residue.** Its three named survivors are spent:
    ...
    `scenes/world.js:4331` (the arrival clamp anchoring the encounter
    clock, so the traveled window is not replayed) and `:1969` (the
    `scenes/world.js:4275` (the arrival clamp anchoring the encounter

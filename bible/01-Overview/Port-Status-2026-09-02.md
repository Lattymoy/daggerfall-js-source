...
the same place - `Port-Ledger.md:716`, the struck keybinding-registry
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:3077`, `:2785`). Ledger row `:662`'s residue list is spent - see list 2. |
...
- ~~**`src/scenes/dungeonContext.js:2126`** - the two window seams this
  ...
  carries the same closure against Ledger row `:725`. The two WINDOWS
  ...
  `test/x11b.test.js`. This was also Ledger row `:725`'s adjudication,
...
Section C's table is **247 rows** between `Port-Ledger.md:599` and `:845`
...
route** - plus `:716`, struck at its head but carrying a live PENDING
...
1. **`:634` FaceUVTool's 1,803-UV residual at matched precision** ->
...
2. **`:647` the custom builder's hidden `ResetBonusPool` control** (STRUCK at E2, landing after this list was written: the control is live) ->
...
3. ~~**`:617` remainder: the overlay mouse-UP seam** -> UI arc. A7 shipped
...
4. ~~**`:660` the quest machine** -> `playSound`'s busy-skip, the one
...
5. ~~**`:713` the classic `.SAV` reader** -> the phone path: no zip arm in
   the saves picker, a desktop-first charter call.~~ **CLOSED (OT1, 2026-09-10).**
6. **`:716` (struck, with a live clause) the keybinding registry** ->
   the mouse/advanced and joystick sub-windows, the two of DFU's 65
   game windows the port does not cite.
7. ~~**`:725` the standalone dungeon host has no trade window** -> a
...
8. ~~**`:726` three stale probes**~~ **CLOSED (E-group, 2026-09-02).**
...
9. **`:612` UseItem's unbuilt destinations.** Every arm the row names is
...
10. **`:662` fast-travel residue.** Its three named survivors are spent:
...
11. **`:667` `PatchRegionIndex` legacy-save fix.** Ported verbatim at
    `src/formats/mapsFile.js:107`, with the C# line range cited.
12. **`:711` the magic crafting windows.** The row's FLAGGED residue is
    ...
    clause goes with it.**
13. **`:661` the talk manager.** The row's own tail already says both
...
14. **`:691` the small-residue trio** (biography GP arm, arrow roll
...
- **`FaceUVTool` arithmetic** - JS doubles for C# float mix, `Math.trunc`
  ...
  precision are ledger row `:634`, narrowed at E8 and the only part of

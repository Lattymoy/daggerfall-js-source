...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:7386`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:4564` carries the sweep and `hostMagic.js:711` its missile half. |
...
- ~~**`src/scenes/world.js:3790`** - the port's default landing stands
...
Quests tab has a machine to read), ~~`world.js:3817` -> `:4089`~~
...
9. **`:591` UseItem's unbuilt destinations.** Every arm the row names is
   ...
   (`ui/nativeInventory.js:798-802`, `scenes/world.js:4618`), the
...
10. **`:641` fast-travel residue.** Its three named survivors are spent:
    ...
    `scenes/world.js:3833` (the arrival clamp anchoring the encounter

...
the same place - `Port-Ledger.md:699`, the struck keybinding-registry
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:3067`, `:2775`). Ledger row `:645`'s residue list is spent - see list 2. |
...
- ~~**`src/scenes/dungeonContext.js:2126`** - the two window seams this
  ...
  carries the same closure against Ledger row `:708`. The two WINDOWS
  ...
  `test/x11b.test.js`. This was also Ledger row `:708`'s adjudication,
...
Section C's table is **247 rows** between `Port-Ledger.md:582` and `:828`
...
route** - plus `:699`, struck at its head but carrying a live PENDING
...
1. **`:617` FaceUVTool's 1,803-UV residual at matched precision** ->
...
2. **`:630` the custom builder's hidden `ResetBonusPool` control** (STRUCK at E2, landing after this list was written: the control is live) ->
...
3. ~~**`:600` remainder: the overlay mouse-UP seam** -> UI arc. A7 shipped
...
4. ~~**`:643` the quest machine** -> `playSound`'s busy-skip, the one
...
5. ~~**`:696` the classic `.SAV` reader** -> the phone path: no zip arm in
   the saves picker, a desktop-first charter call.~~ **CLOSED (OT1, 2026-09-10).**
6. **`:699` (struck, with a live clause) the keybinding registry** ->
   the mouse/advanced and joystick sub-windows, the two of DFU's 65
   game windows the port does not cite.
7. ~~**`:708` the standalone dungeon host has no trade window** -> a
...
8. ~~**`:709` three stale probes**~~ **CLOSED (E-group, 2026-09-02).**
...
9. **`:595` UseItem's unbuilt destinations.** Every arm the row names is
   built: `DrinkPotion` (`systems/useItem.js:205`, `:296-306`),
   `RecordLocationFromMap`/`DiscoverRandomLocation`
   (`ui/nativeInventory.js:832-836`, `scenes/world.js:4690`), the
...
10. **`:645` fast-travel residue.** Its three named survivors are spent:
...
12. **`:694` the magic crafting windows.** The row's FLAGGED residue is
    ...
    clause goes with it.**
13. **`:644` the talk manager.** The row's own tail already says both
...
14. **`:674` the small-residue trio** (biography GP arm, arrow roll
...
- **`FaceUVTool` arithmetic** - JS doubles for C# float mix, `Math.trunc`
  ...
  precision are ledger row `:617`, narrowed at E8 and the only part of

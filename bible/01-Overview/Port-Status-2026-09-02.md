...
the same place - `Port-Ledger.md:669`, the struck keybinding-registry
...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:5328`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:3131` carries the sweep and `hostMagic.js:608` its missile half. |
...
- ~~**`src/scenes/dungeonContext.js:1905`** - the two window seams this
  ...
  carries the same closure against Ledger row `:678`. The two WINDOWS
  ...
  `test/x11b.test.js`. This was also Ledger row `:678`'s adjudication,
...
- **`src/scenes/exterior.js:1844`** - Recall pends here; the anchor
...
- ~~**`src/scenes/exterior.js:1651`** - PX3: this test host mounts no
...
- ~~**`src/scenes/world.js:3300`** - the port's default landing stands
...
cannot take, `exterior.js:1653`), ~~`exterior.js:1696` -> `:1402`~~
(**CLOSED at QX1**: this host took the quest bridge, so the pause window's
Quests tab has a machine to read), ~~`world.js:3339` -> `:3184`~~
...
Section C's table is **247 rows** between `Port-Ledger.md:552` and `:798`
...
route** - plus `:669`, struck at its head but carrying a live PENDING
...
1. **`:587` FaceUVTool's 1,803-UV residual at matched precision** ->
...
2. **`:600` the custom builder's hidden `ResetBonusPool` control** (STRUCK at E2, landing after this list was written: the control is live) ->
...
3. ~~**`:570` remainder: the overlay mouse-UP seam** -> UI arc. A7 shipped
...
4. ~~**`:613` the quest machine** -> `playSound`'s busy-skip, the one
...
5. ~~**`:666` the classic `.SAV` reader** -> the phone path: no zip arm in
   the saves picker, a desktop-first charter call.~~ **CLOSED (OT1, 2026-09-10).**
6. **`:669` (struck, with a live clause) the keybinding registry** ->
   the mouse/advanced and joystick sub-windows, the two of DFU's 65
   game windows the port does not cite.
7. ~~**`:678` the standalone dungeon host has no trade window** -> a
...
8. ~~**`:679` three stale probes**~~ **CLOSED (E-group, 2026-09-02).**
...
9. **`:565` UseItem's unbuilt destinations.** Every arm the row names is
   ...
   (`ui/nativeInventory.js:726-730`, `scenes/world.js:3180`), the
   (`ui/nativeInventory.js:726-730`, `scenes/world.js:3126`), the
...
10. **`:615` fast-travel residue.** Its three named survivors are spent:
    ...
    `scenes/world.js:4269` (the arrival clamp anchoring the encounter
    clock, so the traveled window is not replayed) and `:1969` (the
    `scenes/world.js:4213` (the arrival clamp anchoring the encounter
...
11. **`:620` `PatchRegionIndex` legacy-save fix.** Ported verbatim at
...
12. **`:664` the magic crafting windows.** The row's FLAGGED residue is
    ...
    clause goes with it.**
13. **`:614` the talk manager.** The row's own tail already says both
...
14. **`:644` the small-residue trio** (biography GP arm, arrow roll
    ...
    `BIOG*.TXT` files; the faceUV guard rides row `:587`.
...
- **`FaceUVTool` arithmetic** - JS doubles for C# float mix, `Math.trunc`
  ...
  precision are ledger row `:587`, narrowed at E8 and the only part of

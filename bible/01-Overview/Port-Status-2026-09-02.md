...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:7393`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:4571` carries the sweep and `hostMagic.js:711` its missile half. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:3033`, `:2749`). Ledger row `:642`'s residue list is spent - see list 2. |
...
- ~~**`src/scenes/world.js:4338`** - the port's default landing stands
...
Quests tab has a machine to read), ~~`world.js:3866` -> `:4138`~~
...
9. **`:592` UseItem's unbuilt destinations.** Every arm the row names is
   ...
   (`ui/nativeInventory.js:798-802`, `scenes/world.js:4625`), the
...
10. **`:642` fast-travel residue.** Its three named survivors are spent:
    ...
    `scenes/world.js:3840` (the arrival clamp anchoring the encounter

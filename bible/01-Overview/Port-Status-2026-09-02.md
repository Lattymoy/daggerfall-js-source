...
the same place - `Port-Ledger.md:637`, the struck keybinding-registry
...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:4893`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:2905` carries the sweep and `hostMagic.js:608` its missile half. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:1946`, `:1725`). Ledger row `:583`'s residue list is spent - see list 2. |
...
- ~~**`src/scenes/dungeonContext.js:1867`** - the two window seams this
  ...
  carries the same closure against Ledger row `:646`. The two WINDOWS
  ...
  `test/x11b.test.js`. This was also Ledger row `:646`'s adjudication,
...
- **`src/scenes/exterior.js:1731`** - Recall pends here; the anchor
...
Section C's table is **247 rows** between `Port-Ledger.md:520` and `:766`
...
route** - plus `:637`, struck at its head but carrying a live PENDING
...
1. **`:555` FaceUVTool's 1,803-UV residual at matched precision** ->
...
2. **`:568` the custom builder's hidden `ResetBonusPool` control** (STRUCK at E2, landing after this list was written: the control is live) ->
...
3. ~~**`:538` remainder: the overlay mouse-UP seam** -> UI arc. A7 shipped
...
4. ~~**`:581` the quest machine** -> `playSound`'s busy-skip, the one
...
5. ~~**`:634` the classic `.SAV` reader** -> the phone path: no zip arm in
   the saves picker, a desktop-first charter call.~~ **CLOSED (OT1, 2026-09-10).**
6. **`:637` (struck, with a live clause) the keybinding registry** ->
   the mouse/advanced and joystick sub-windows, the two of DFU's 65
   game windows the port does not cite.
7. ~~**`:646` the standalone dungeon host has no trade window** -> a
...
8. ~~**`:647` three stale probes**~~ **CLOSED (E-group, 2026-09-02).**
...
9. **`:533` UseItem's unbuilt destinations.** Every arm the row names is
   built: `DrinkPotion` (`systems/useItem.js:167`, `:245-255`),
   `RecordLocationFromMap`/`DiscoverRandomLocation`
   (`ui/nativeInventory.js:704-708`, `scenes/world.js:2954`), the
   (`ui/nativeInventory.js:704-708`, `scenes/world.js:2900`), the
...
10. **`:583` fast-travel residue.** Its three named survivors are spent:
...
11. **`:588` `PatchRegionIndex` legacy-save fix.** Ported verbatim at
    `src/formats/mapsFile.js:104`, with the C# line range cited.
12. **`:632` the magic crafting windows.** The row's FLAGGED residue is
    ...
    clause goes with it.**
13. **`:582` the talk manager.** The row's own tail already says both
...
14. **`:612` the small-residue trio** (biography GP arm, arrow roll
...
- **`FaceUVTool` arithmetic** - JS doubles for C# float mix, `Math.trunc`
  ...
  precision are ledger row `:555`, narrowed at E8 and the only part of

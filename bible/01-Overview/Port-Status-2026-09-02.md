...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:6810`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:4027` carries the sweep and `hostMagic.js:711` its missile half. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:2489`, `:2205`). Ledger row `:641`'s residue list is spent - see list 2. |
...
- **`src/scenes/exterior.js:2161`** - Recall pends here; the anchor
...
- ~~**`src/scenes/exterior.js:1851`** - PX3: this test host mounts no
...
- ~~**`src/scenes/world.js:3794`** - the port's default landing stands
...
regenerated onto the new sites: ~~`exterior.js:1531` -> `:1223`~~
(**CLOSED at TP2**: the Recall interim narrowed to the one arm this host
cannot take, `exterior.js:1873`), ~~`exterior.js:1923` -> `:1611`~~
(**CLOSED at QX1**: this host took the quest bridge, so the pause window's
Quests tab has a machine to read), ~~`world.js:3821` -> `:3814`~~
...
9. **`:591` UseItem's unbuilt destinations.** Every arm the row names is
   ...
   (`ui/nativeInventory.js:798-802`, `scenes/world.js:4081`), the
...
10. **`:641` fast-travel residue.** Its three named survivors are spent:
    ...
    `scenes/world.js:3296` (the arrival clamp anchoring the encounter

...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:5043`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:3001` carries the sweep and `hostMagic.js:608` its missile half. |
...
9. **`:547` UseItem's unbuilt destinations.** Every arm the row names is
   ...
   (`ui/nativeInventory.js:717-721`, `scenes/world.js:3050`), the
   (`ui/nativeInventory.js:717-721`, `scenes/world.js:2996`), the
...
10. **`:597` fast-travel residue.** Its three named survivors are spent:
    ...
    `scenes/world.js:4050` (the arrival clamp anchoring the encounter
    clock, so the traveled window is not replayed) and `:1966` (the
    `scenes/world.js:3994` (the arrival clamp anchoring the encounter

...
the same place - `Port-Ledger.md:713`, the struck keybinding-registry
...
| **characters-voxel** | Departure (deliberate) | **Departure, still editor-only** | 86 designs across seven tables - 42 of the 43 monster mobiles, 19 class, 25 villager. The rig is 1,791 lines across 7 modules and is still gated: `worldModes.js:5565` passes `voxelfolk`, `scenes/interiorContext.js:578` consumes it. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:3097`, `:2805`). Ledger row `:659`'s residue list is spent - see list 2. |
...
- ~~**`src/scenes/dungeonContext.js:2126`** - the two window seams this
  ...
  carries the same closure against Ledger row `:722`. The two WINDOWS
  ...
  `test/x11b.test.js`. This was also Ledger row `:722`'s adjudication,
...
- ~~**`src/scenes/exterior.js:1946`** - PX3: this test host mounts no
  ...
  site. (The `src/scenes/worldModes.js:2360` row below is struck: ROAD-E
...
- ~~**`src/characters/enemyCasting.js:91`** - now exactly one term
  ...
  second.*
- ~~**`src/scenes/worldModes.js:1969`** - above ground only:
...
(**CLOSED at the ship landing**), ~~`worldModes.js:1932` -> `:1809`~~
...
- ~~**`src/scenes/worldModes.js:2044`** - above ground only: the GUILD
...
Section C's table is **247 rows** between `Port-Ledger.md:596` and `:842`
...
route** - plus `:713`, struck at its head but carrying a live PENDING
...
1. **`:631` FaceUVTool's 1,803-UV residual at matched precision** ->
...
2. **`:644` the custom builder's hidden `ResetBonusPool` control** (STRUCK at E2, landing after this list was written: the control is live) ->
...
3. ~~**`:614` remainder: the overlay mouse-UP seam** -> UI arc. A7 shipped
...
4. ~~**`:657` the quest machine** -> `playSound`'s busy-skip, the one
...
5. ~~**`:710` the classic `.SAV` reader** -> the phone path: no zip arm in
   the saves picker, a desktop-first charter call.~~ **CLOSED (OT1, 2026-09-10).**
6. **`:713` (struck, with a live clause) the keybinding registry** ->
   the mouse/advanced and joystick sub-windows, the two of DFU's 65
   game windows the port does not cite.
7. ~~**`:722` the standalone dungeon host has no trade window** -> a
...
8. ~~**`:723` three stale probes**~~ **CLOSED (E-group, 2026-09-02).**
...
9. **`:609` UseItem's unbuilt destinations.** Every arm the row names is
...
10. **`:659` fast-travel residue.** Its three named survivors are spent:
...
11. **`:664` `PatchRegionIndex` legacy-save fix.** Ported verbatim at
    `src/formats/mapsFile.js:107`, with the C# line range cited.
12. **`:708` the magic crafting windows.** The row's FLAGGED residue is
    ...
    clause goes with it.**
13. **`:658` the talk manager.** The row's own tail already says both
...
14. **`:688` the small-residue trio** (biography GP arm, arrow roll
...
- **The voxel character engine** (Mac's system, Port-Doctrine). 86
  ...
  by a game host: `worldModes.js:5565` passes `voxelfolk`,
...
- **`FaceUVTool` arithmetic** - JS doubles for C# float mix, `Math.trunc`
  ...
  precision are ledger row `:631`, narrowed at E8 and the only part of

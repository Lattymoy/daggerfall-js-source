...
the same place - `Port-Ledger.md:605`, the struck keybinding-registry
...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:4754`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:2820` carries the sweep and `hostMagic.js:601` its missile half. |
...
| **player** | Near-1:1 / one lethal seam | **Near-1:1** | `cancelMovement` is a real motor field set on the mode edges (`player/motor.js:346,:439,:447`). ROAD-A A6 took the -0.28 doorway head-dip, `PlayerMoveScanner`'s three probes, `controllerSwimHeight` 0.30 with Do(Un)Sinking, and `FreezeMotor` on teleports. |
...
| **characters-voxel** | Departure (deliberate) | **Departure, still editor-only** | 86 designs across seven tables - 42 of the 43 monster mobiles, 19 class, 25 villager. The rig is 1,791 lines across 7 modules and is still gated: `worldModes.js:4172` passes `voxelfolk`, `scenes/interiorContext.js:472` consumes it. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:1862`, `:1668`). Ledger row `:551`'s residue list is spent - see list 2. |
...
and `world.js:181` reaches it by mounting `worldModes`. But `grep -rn
...
- ~~**`src/scenes/dungeonContext.js:1817`** - the two window seams this
  ...
  carries the same closure against Ledger row `:614`. The two WINDOWS
  ...
  `test/x11b.test.js`. This was also Ledger row `:614`'s adjudication,
  and that row is struck with it.
- **`src/ui/enhancedMenu.js:1896`** - the rest of the keyboard; the
...
- **`src/scenes/exterior.js:1712`** - Recall pends here; the anchor
...
- ~~**`src/scenes/exterior.js:1602`** - PX3: this test host mounts no
  ...
  site. (The `src/scenes/worldModes.js:1837` row below is struck: ROAD-E
...
- ~~**`src/scenes/world.js:3071`** - the port's default landing stands
...
- **`src/systems/inputActions.js:543`** - STILL FLAGGED: axes and
  ...
  sub-windows are recorded pending at `Port-Ledger.md:605`. The flag's
...
- ~~**`src/characters/enemyCasting.js:91`** - now exactly one term
  ...
  second.*
- ~~**`src/scenes/worldModes.js:1837`** - above ground only:
...
- ~~**`src/ui/hudLarge.js:75`** - the docked bar occludes, it does not
  ...
  (`render/renderer.js:1272, :1219, :1701, :1876`) and the 2D passes
...
(**CLOSED at the ship landing**), ~~`worldModes.js:1834` -> `:1687`~~
...
- ~~**`src/scenes/worldModes.js:1907`** - above ground only: the GUILD
...
Section C's table is **247 rows** between `Port-Ledger.md:488` and `:734`
...
generation, to the enhanced lane)~~ (**it read 226 and 21 until `:548` was
...
route** - plus `:605`, struck at its head but carrying a live PENDING
...
1. **`:523` FaceUVTool's 1,803-UV residual at matched precision** ->
...
2. **`:536` the custom builder's hidden `ResetBonusPool` control** (STRUCK at E2, landing after this list was written: the control is live) ->
   ...
   `systems/dialogShortcuts.js:194`/`:314` carries `ResetBonusPool` with
...
3. **`:506` remainder: the overlay mouse-UP seam** -> UI arc. A7 shipped
...
4. ~~**`:549` the quest machine** -> `playSound`'s busy-skip, the one
...
5. ~~**`:602` the classic `.SAV` reader** -> the phone path: no zip arm in
   the saves picker, a desktop-first charter call.~~ **CLOSED (OT1, 2026-09-10).**
6. **`:605` (struck, with a live clause) the keybinding registry** ->
   the mouse/advanced and joystick sub-windows, the two of DFU's 65
   game windows the port does not cite.
7. ~~**`:614` the standalone dungeon host has no trade window** -> a
...
8. ~~**`:615` three stale probes**~~ **CLOSED (E-group, 2026-09-02).**
...
9. **`:501` UseItem's unbuilt destinations.** Every arm the row names is
   built: `DrinkPotion` (`systems/useItem.js:167`, `:245-255`),
   `RecordLocationFromMap`/`DiscoverRandomLocation`
   (`ui/nativeInventory.js:703-707`, `scenes/world.js:2869`), the
   (`ui/nativeInventory.js:703-707`, `scenes/world.js:2815`), the
...
10. **`:551` fast-travel residue.** Its three named survivors are spent:
    ...
    `scenes/world.js:3829` (the arrival clamp anchoring the encounter
...
11. **`:556` `PatchRegionIndex` legacy-save fix.** Ported verbatim at
    `src/formats/mapsFile.js:104`, with the C# line range cited.
12. **`:600` the magic crafting windows.** The row's FLAGGED residue is
    ...
    clause goes with it.**
13. **`:550` the talk manager.** The row's own tail already says both
...
14. **`:580` the small-residue trio** (biography GP arm, arrow roll
...
- **The voxel character engine** (Mac's system, Port-Doctrine). 86
  ...
  by a game host: `worldModes.js:4172` passes `voxelfolk`,
...
- **`FaceUVTool` arithmetic** - JS doubles for C# float mix, `Math.trunc`
  ...
  precision are ledger row `:523`, narrowed at E8 and the only part of

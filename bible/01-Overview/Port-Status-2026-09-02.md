...
the same place - `Port-Ledger.md:660`, the struck keybinding-registry
...
| **formats-game** | Verbatim readers / broken consumer | **Verbatim** | The `ItemRecord` conversions were in the wave; ROAD-A A4 took the classic-import stragglers (building-level MAPSAVE, the native bank record, `LegacyArtifactIndexBitfieldCheck`). ~~Ledger row `:657`'s one residue is the phone path - no zip arm in the saves picker, a desktop-first charter call.~~ **CLOSED (OT1, 2026-09-10): the saves picker took the ARENA2 door's zip arm, and the row is struck whole.** |
| **formats-mw** | Departure lane, faithful within it | **Departure lane, now consumed** | `clipSweepTimes` has a production caller: `combat/fpArm.js:1717` runs the whole-clip reach sweep the superseded page said had never run in the game. 17 modules / 8,842 lines. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:1992`, `:1760`). Ledger row `:606`'s residue list is spent - see list 2. |
...
- ~~**`src/scenes/dungeonContext.js:1905`** - the two window seams this
  ...
  carries the same closure against Ledger row `:669`. The two WINDOWS
  ...
  `test/x11b.test.js`. This was also Ledger row `:669`'s adjudication,
...
Section C's table is **247 rows** between `Port-Ledger.md:543` and `:789`
...
route** - plus `:660`, struck at its head but carrying a live PENDING
...
1. **`:578` FaceUVTool's 1,803-UV residual at matched precision** ->
...
2. **`:591` the custom builder's hidden `ResetBonusPool` control** (STRUCK at E2, landing after this list was written: the control is live) ->
...
3. ~~**`:561` remainder: the overlay mouse-UP seam** -> UI arc. A7 shipped
...
4. ~~**`:604` the quest machine** -> `playSound`'s busy-skip, the one
...
5. ~~**`:657` the classic `.SAV` reader** -> the phone path: no zip arm in
   the saves picker, a desktop-first charter call.~~ **CLOSED (OT1, 2026-09-10).**
6. **`:660` (struck, with a live clause) the keybinding registry** ->
   the mouse/advanced and joystick sub-windows, the two of DFU's 65
   game windows the port does not cite.
7. ~~**`:669` the standalone dungeon host has no trade window** -> a
...
8. ~~**`:670` three stale probes**~~ **CLOSED (E-group, 2026-09-02).**
...
9. **`:556` UseItem's unbuilt destinations.** Every arm the row names is
...
10. **`:606` fast-travel residue.** Its three named survivors are spent:
...
11. **`:611` `PatchRegionIndex` legacy-save fix.** Ported verbatim at
    `src/formats/mapsFile.js:104`, with the C# line range cited.
12. **`:655` the magic crafting windows.** The row's FLAGGED residue is
    ...
    clause goes with it.**
13. **`:605` the talk manager.** The row's own tail already says both
...
14. **`:635` the small-residue trio** (biography GP arm, arrow roll
...
- **`FaceUVTool` arithmetic** - JS doubles for C# float mix, `Math.trunc`
  ...
  precision are ledger row `:578`, narrowed at E8 and the only part of

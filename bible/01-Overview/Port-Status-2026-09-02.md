...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:6334`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:3706` carries the sweep and `hostMagic.js:608` its missile half. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:2408`, `:2124`). Ledger row `:629`'s residue list is spent - see list 2. |
...
- **`src/ui/enhancedMenu.js:2804`** - the rest of the keyboard; the
...
9. **`:579` UseItem's unbuilt destinations.** Every arm the row names is
   ...
   (`ui/nativeInventory.js:778-782`, `scenes/world.js:3806`), the
...
10. **`:629` fast-travel residue.** Its three named survivors are spent:
    ...
    `scenes/world.js:4871` (the arrival clamp anchoring the encounter

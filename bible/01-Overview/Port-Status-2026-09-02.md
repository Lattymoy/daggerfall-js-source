...
the same place - `Port-Ledger.md:686`, the struck keybinding-registry
...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:6485`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:3873` carries the sweep and `hostMagic.js:608` its missile half. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:2460`, `:2177`). Ledger row `:632`'s residue list is spent - see list 2. |
...
- ~~**`src/scenes/dungeonContext.js:2075`** - the two window seams this
  ...
  carries the same closure against Ledger row `:695`. The two WINDOWS
  ...
  `test/x11b.test.js`. This was also Ledger row `:695`'s adjudication,
...
- ~~**`src/scenes/world.js:3640`** - the port's default landing stands
...
Quests tab has a machine to read), ~~`world.js:3667` -> `:3660`~~
...
Section C's table is **247 rows** between `Port-Ledger.md:569` and `:815`
...
route** - plus `:686`, struck at its head but carrying a live PENDING
...
1. **`:604` FaceUVTool's 1,803-UV residual at matched precision** ->
...
2. **`:617` the custom builder's hidden `ResetBonusPool` control** (STRUCK at E2, landing after this list was written: the control is live) ->
...
3. ~~**`:587` remainder: the overlay mouse-UP seam** -> UI arc. A7 shipped
...
4. ~~**`:630` the quest machine** -> `playSound`'s busy-skip, the one
...
5. ~~**`:683` the classic `.SAV` reader** -> the phone path: no zip arm in
   the saves picker, a desktop-first charter call.~~ **CLOSED (OT1, 2026-09-10).**
6. **`:686` (struck, with a live clause) the keybinding registry** ->
   the mouse/advanced and joystick sub-windows, the two of DFU's 65
   game windows the port does not cite.
7. ~~**`:695` the standalone dungeon host has no trade window** -> a
...
8. ~~**`:696` three stale probes**~~ **CLOSED (E-group, 2026-09-02).**
...
9. **`:582` UseItem's unbuilt destinations.** Every arm the row names is
   built: `DrinkPotion` (`systems/useItem.js:197`, `:275-285`),
   `RecordLocationFromMap`/`DiscoverRandomLocation`
   (`ui/nativeInventory.js:778-782`, `scenes/world.js:3927`), the
...
10. **`:632` fast-travel residue.** Its three named survivors are spent:
    ...
    `scenes/world.js:3150` (the arrival clamp anchoring the encounter
...
11. **`:637` `PatchRegionIndex` legacy-save fix.** Ported verbatim at
    `src/formats/mapsFile.js:104`, with the C# line range cited.
12. **`:681` the magic crafting windows.** The row's FLAGGED residue is
    ...
    clause goes with it.**
13. **`:631` the talk manager.** The row's own tail already says both
...
14. **`:661` the small-residue trio** (biography GP arm, arrow roll
...
- **`FaceUVTool` arithmetic** - JS doubles for C# float mix, `Math.trunc`
  ...
  precision are ledger row `:604`, narrowed at E8 and the only part of

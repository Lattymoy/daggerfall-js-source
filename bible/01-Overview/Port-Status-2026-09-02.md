...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:6464`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:3825` carries the sweep and `hostMagic.js:608` its missile half. |
...
| **characters-voxel** | Departure (deliberate) | **Departure, still editor-only** | 86 designs across seven tables - 42 of the 43 monster mobiles, 19 class, 25 villager. The rig is 1,791 lines across 7 modules and is still gated: `worldModes.js:5688` passes `voxelfolk`, `scenes/interiorContext.js:503` consumes it. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:2428`, `:2144`). Ledger row `:630`'s residue list is spent - see list 2. |
...
| **ui-enhanced** | Departure lane | **Departure lane, scoped** | `ui/worldPlaque.js:218` puts the skin gate above `ensure()`, so the unscoped `*`/`html`/`body`/`button`/`#app` rules never reach the classic page. 9 modules / 8,940 lines. |
...
that own overlay slots - `worldModes.js:81` and `dungeonContext.js:43` -
...
- ~~**`src/scenes/dungeonContext.js:2052`** - the two window seams this
...
- **`src/ui/enhancedMenu.js:2866`** - the rest of the keyboard; the
...
- **`src/scenes/exterior.js:2062`** - Recall pends here; the anchor
...
- ~~**`src/scenes/exterior.js:1834`** - PX3: this test host mounts no
  ...
  site. (The `src/scenes/worldModes.js:2446` row below is struck: ROAD-E
...
- ~~**`src/scenes/world.js:3758`** - the port's default landing stands
...
- ~~**`src/systems/inputActions.js:837`** - STILL FLAGGED, NARROWED (GP1,
...
- ~~**`src/characters/enemyCasting.js:91`** - now exactly one term
  ...
  second.*
- ~~**`src/scenes/worldModes.js:2160`** - above ground only:
...
- **`src/scenes/dungeonContext.js:1138`** - the dungeon's non-layout foe
...
regenerated onto the new sites: ~~`exterior.js:1514` -> `:1209`~~
(**CLOSED at TP2**: the Recall interim narrowed to the one arm this host
cannot take, `exterior.js:1856`), ~~`exterior.js:1906` -> `:1594`~~
(**CLOSED at QX1**: this host took the quest bridge, so the pause window's
Quests tab has a machine to read), ~~`world.js:3785` -> `:3778`~~
(**CLOSED at the ship landing**), ~~`worldModes.js:2123` -> `:2004`~~
...
- ~~**`src/scenes/worldModes.js:2230`** - above ground only: the GUILD
...
9. **`:580` UseItem's unbuilt destinations.** Every arm the row names is
   ...
   (`ui/nativeInventory.js:778-782`, `scenes/world.js:3925`), the
...
10. **`:630` fast-travel residue.** Its three named survivors are spent:
    ...
    `scenes/world.js:3264` (the arrival clamp anchoring the encounter
...
- **The voxel character engine** (Mac's system, Port-Doctrine). 86
  ...
  by a game host: `worldModes.js:5688` passes `voxelfolk`,

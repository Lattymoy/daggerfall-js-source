...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:6219`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:3700` carries the sweep and `hostMagic.js:608` its missile half. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:2336`, `:2053`). Ledger row `:622`'s residue list is spent - see list 2. |
...
- ~~**`src/scenes/dungeonContext.js:1936`** - the two window seams this
...
- **`src/scenes/exterior.js:1964`** - Recall pends here; the anchor
...
- ~~**`src/scenes/exterior.js:1743`** - PX3: this test host mounts no
...
- ~~**`src/scenes/world.js:3635`** - the port's default landing stands
...
regenerated onto the new sites: ~~`exterior.js:1423` -> `:1200`~~
...
Quests tab has a machine to read), ~~`world.js:3662` -> `:3658`~~
...
9. **`:572` UseItem's unbuilt destinations.** Every arm the row names is
   ...
   (`ui/nativeInventory.js:736-740`, `scenes/world.js:3703`), the
...
10. **`:622` fast-travel residue.** Its three named survivors are spent:
    ...
    `scenes/world.js:4833` (the arrival clamp anchoring the encounter

...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:7350`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:4528` carries the sweep and `hostMagic.js:711` its missile half. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:2990`, `:2706`). Ledger row `:642`'s residue list is spent - see list 2. |
...
- ~~**`src/scenes/world.js:4295`** - the port's default landing stands
...
Quests tab has a machine to read), ~~`world.js:3823` -> `:4095`~~
...
9. **`:592` UseItem's unbuilt destinations.** Every arm the row names is
   ...
   (`ui/nativeInventory.js:798-802`, `scenes/world.js:4582`), the
...
10. **`:642` fast-travel residue.** Its three named survivors are spent:
    ...
    `scenes/world.js:3797` (the arrival clamp anchoring the encounter

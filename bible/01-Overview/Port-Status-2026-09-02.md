...
the same place - `Port-Ledger.md:684`, the struck keybinding-registry
...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:6405`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:3848` carries the sweep and `hostMagic.js:608` its missile half. |
...
| **characters-voxel** | Departure (deliberate) | **Departure, still editor-only** | 86 designs across seven tables - 42 of the 43 monster mobiles, 19 class, 25 villager. The rig is 1,791 lines across 7 modules and is still gated: `worldModes.js:4983` passes `voxelfolk`, `scenes/interiorContext.js:503` consumes it. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:2450`, `:2167`). Ledger row `:630`'s residue list is spent - see list 2. |
...
| **ui-enhanced** | Departure lane | **Departure lane, scoped** | `ui/worldPlaque.js:180` puts the skin gate above `ensure()`, so the unscoped `*`/`html`/`body`/`button`/`#app` rules never reach the classic page. 9 modules / 8,940 lines. |
...
that own overlay slots - `worldModes.js:79` and `dungeonContext.js:40` -
and `world.js:246` reaches it by mounting `worldModes`. But `grep -rn
...
- ~~**`src/scenes/dungeonContext.js:1988`** - the two window seams this
  ...
  carries the same closure against Ledger row `:693`. The two WINDOWS
  ...
  `test/x11b.test.js`. This was also Ledger row `:693`'s adjudication,
  and that row is struck with it.
- **`src/ui/enhancedMenu.js:2589`** - the rest of the keyboard; the
...
- **`src/scenes/exterior.js:2041`** - Recall pends here; the anchor
...
- ~~**`src/scenes/exterior.js:1744`** - PX3: this test host mounts no
  ...
  site. (The `src/scenes/worldModes.js:2205` row below is struck: ROAD-E
...
- ~~**`src/scenes/world.js:3625`** - the port's default landing stands
...
- ~~**`src/systems/inputActions.js:767`** - STILL FLAGGED, NARROWED (GP1,
...
- ~~**`src/characters/enemyCasting.js:91`** - now exactly one term
  ...
  second.*
- ~~**`src/scenes/worldModes.js:1919`** - above ground only:
...
- ~~**`src/ui/hudLarge.js:75`** - the docked bar occludes, it does not
  ...
  (`render/renderer.js:1963, :1890, :2710, :3022`) and the 2D passes
...
- **`src/scenes/dungeonContext.js:1132`** - the dungeon's non-layout foe
...
regenerated onto the new sites: ~~`exterior.js:1424` -> `:1202`~~
(**CLOSED at TP2**: the Recall interim narrowed to the one arm this host
cannot take, `exterior.js:1766`), ~~`exterior.js:1816` -> `:1505`~~
(**CLOSED at QX1**: this host took the quest bridge, so the pause window's
Quests tab has a machine to read), ~~`world.js:3652` -> `:3650`~~
(**CLOSED at the ship landing**), ~~`worldModes.js:1882` -> `:1769`~~
...
- ~~**`src/scenes/worldModes.js:1989`** - above ground only: the GUILD
...
Section C's table is **247 rows** between `Port-Ledger.md:567` and `:813`
...
generation, to the enhanced lane)~~ (**it read 226 and 21 until `:558` was
...
route** - plus `:684`, struck at its head but carrying a live PENDING
...
1. **`:602` FaceUVTool's 1,803-UV residual at matched precision** ->
...
2. **`:615` the custom builder's hidden `ResetBonusPool` control** (STRUCK at E2, landing after this list was written: the control is live) ->
   ...
   `systems/dialogShortcuts.js:194`/`:319` carries `ResetBonusPool` with
...
3. ~~**`:585` remainder: the overlay mouse-UP seam** -> UI arc. A7 shipped
...
4. ~~**`:628` the quest machine** -> `playSound`'s busy-skip, the one
...
5. ~~**`:681` the classic `.SAV` reader** -> the phone path: no zip arm in
   the saves picker, a desktop-first charter call.~~ **CLOSED (OT1, 2026-09-10).**
6. **`:684` (struck, with a live clause) the keybinding registry** ->
   the mouse/advanced and joystick sub-windows, the two of DFU's 65
   game windows the port does not cite.
7. ~~**`:693` the standalone dungeon host has no trade window** -> a
...
8. ~~**`:694` three stale probes**~~ **CLOSED (E-group, 2026-09-02).**
...
9. **`:580` UseItem's unbuilt destinations.** Every arm the row names is
   built: `DrinkPotion` (`systems/useItem.js:197`, `:275-285`),
   `RecordLocationFromMap`/`DiscoverRandomLocation`
   (`ui/nativeInventory.js:778-782`, `scenes/world.js:3902`), the
...
10. **`:630` fast-travel residue.** Its three named survivors are spent:
    ...
    `scenes/world.js:3128` (the arrival clamp anchoring the encounter
    clock, so the traveled window is not replayed) and `:3167` (the
...
11. **`:635` `PatchRegionIndex` legacy-save fix.** Ported verbatim at
    `src/formats/mapsFile.js:104`, with the C# line range cited.
12. **`:679` the magic crafting windows.** The row's FLAGGED residue is
    three items and all three are answered: spell icons ship and are
    drawn (`ui/spellIcons.js`, imported at
    `ui/spellbookWindow.js:136-138`, drawn at `:1009`), the icon picker
    ...
    clause goes with it.**
13. **`:629` the talk manager.** The row's own tail already says both
...
14. **`:659` the small-residue trio** (biography GP arm, arrow roll
...
- **The voxel character engine** (Mac's system, Port-Doctrine). 86
  ...
  by a game host: `worldModes.js:4983` passes `voxelfolk`,
...
- **`FaceUVTool` arithmetic** - JS doubles for C# float mix, `Math.trunc`
  ...
  precision are ledger row `:602`, narrowed at E8 and the only part of

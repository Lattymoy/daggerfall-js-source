...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:6482`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:3870` carries the sweep and `hostMagic.js:608` its missile half. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:2457`, `:2174`). Ledger row `:632`'s residue list is spent - see list 2. |
...
- **`src/ui/enhancedMenu.js:2602`** - the rest of the keyboard; the
...
- ~~**`src/scenes/world.js:3637`** - the port's default landing stands
...
Quests tab has a machine to read), ~~`world.js:3664` -> `:3657`~~
...
9. **`:582` UseItem's unbuilt destinations.** Every arm the row names is
   ...
   (`ui/nativeInventory.js:778-782`, `scenes/world.js:3924`), the
...
10. **`:632` fast-travel residue.** Its three named survivors are spent:
    ...
    `scenes/world.js:3147` (the arrival clamp anchoring the encounter

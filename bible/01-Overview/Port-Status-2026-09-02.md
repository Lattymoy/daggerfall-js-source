...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:7470`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:4620` carries the sweep and `hostMagic.js:711` its missile half. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:3051`, `:2759`). Ledger row `:644`'s residue list is spent - see list 2. |
...
- **`src/scenes/exterior.js:2252`** - Recall pends here; the anchor
...
- ~~**`src/scenes/exterior.js:1941`** - PX3: this test host mounts no
...
- ~~**`src/scenes/world.js:4387`** - the port's default landing stands
...
regenerated onto the new sites: ~~`exterior.js:1599` -> `:1287`~~
(**CLOSED at TP2**: the Recall interim narrowed to the one arm this host
cannot take, `exterior.js:1963`), ~~`exterior.js:2013` -> `:1679`~~
(**CLOSED at QX1**: this host took the quest bridge, so the pause window's
Quests tab has a machine to read), ~~`world.js:4414` -> `:4407`~~

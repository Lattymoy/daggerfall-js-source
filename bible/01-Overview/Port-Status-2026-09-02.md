...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:5005`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:2970` carries the sweep and `hostMagic.js:608` its missile half. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:1971`, `:1739`). Ledger row `:596`'s residue list is spent - see list 2. |
...
- ~~**`src/scenes/world.js:3206`** - the port's default landing stands
...
Quests tab has a machine to read), ~~`world.js:3252` -> `:3055`~~
...
9. **`:546` UseItem's unbuilt destinations.** Every arm the row names is
   ...
   (`ui/nativeInventory.js:717-721`, `scenes/world.js:3019`), the
   (`ui/nativeInventory.js:717-721`, `scenes/world.js:2965`), the
...
10. **`:596` fast-travel residue.** Its three named survivors are spent:
    ...
    `scenes/world.js:4019` (the arrival clamp anchoring the encounter
    clock, so the traveled window is not replayed) and `:1966` (the
    `scenes/world.js:3963` (the arrival clamp anchoring the encounter

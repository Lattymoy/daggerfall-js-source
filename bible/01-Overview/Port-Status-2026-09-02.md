...
the same place - `Port-Ledger.md:700`, the struck keybinding-registry
...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:7507`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:4638` carries the sweep and `hostMagic.js:751` its missile half. |
...
| **characters-voxel** | Departure (deliberate) | **Departure, still editor-only** | 86 designs across seven tables - 42 of the 43 monster mobiles, 19 class, 25 villager. The rig is 1,791 lines across 7 modules and is still gated: `worldModes.js:5291` passes `voxelfolk`, `scenes/interiorContext.js:513` consumes it. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:3067`, `:2775`). Ledger row `:646`'s residue list is spent - see list 2. |
...
| **ui-hud** | Verbatim (classic) / Partial (enhanced default) | **Verbatim on both skins** | The skin fork moved BELOW the game-state seams: `ui/hud.js:503` runs `updateHudVitals` and `:558` `drawNearDeathFlicker`, both above the enhanced branch at `:611` and above the `!art` return, and the enhanced HUD takes the Detect markers at `:627`. `lastHealthLost()` is no longer pinned at 0, so `CameraRecoiler` lives in all three hosts. |
| **ui-windows-a** | Rect parity excellent / two crash doors | **Verbatim** | ROAD-D D2 shipped both scroll thumbs (`chargenArt.js:781` over `RECTS.pickScroll`, `spellbookWindow.js:1016` over the 7-wide rail). |
...
that own overlay slots - `worldModes.js:82` and `dungeonContext.js:44` -
...
- ~~**`src/scenes/dungeonContext.js:2126`** - the two window seams this
  ...
  carries the same closure against Ledger row `:709`. The two WINDOWS
  ...
  `test/x11b.test.js`. This was also Ledger row `:709`'s adjudication,
  and that row is struck with it.
- **`src/ui/enhancedMenu.js:3113`** - the rest of the keyboard; the
...
- ~~**`src/scenes/exterior.js:1946`** - PX3: this test host mounts no
  ...
  site. (The `src/scenes/worldModes.js:2279` row below is struck: ROAD-E
...
- ~~**`src/ui/hudLarge.js:75`** - the docked bar occludes, it does not
  ...
  (`render/renderer.js:2149, :2066, :2896, :3220`) and the 2D passes
...
- **`src/scenes/dungeonContext.js:1175`** - the dungeon's non-layout foe
...
Quests tab has a machine to read), ~~`world.js:4432` -> `:4425`~~
...
Section C's table is **247 rows** between `Port-Ledger.md:583` and `:829`
...
route** - plus `:700`, struck at its head but carrying a live PENDING
...
1. **`:618` FaceUVTool's 1,803-UV residual at matched precision** ->
...
2. **`:631` the custom builder's hidden `ResetBonusPool` control** (STRUCK at E2, landing after this list was written: the control is live) ->
...
3. ~~**`:601` remainder: the overlay mouse-UP seam** -> UI arc. A7 shipped
...
4. ~~**`:644` the quest machine** -> `playSound`'s busy-skip, the one
...
5. ~~**`:697` the classic `.SAV` reader** -> the phone path: no zip arm in
   the saves picker, a desktop-first charter call.~~ **CLOSED (OT1, 2026-09-10).**
6. **`:700` (struck, with a live clause) the keybinding registry** ->
   the mouse/advanced and joystick sub-windows, the two of DFU's 65
   game windows the port does not cite.
7. ~~**`:709` the standalone dungeon host has no trade window** -> a
...
8. ~~**`:710` three stale probes**~~ **CLOSED (E-group, 2026-09-02).**
...
9. **`:596` UseItem's unbuilt destinations.** Every arm the row names is
   built: `DrinkPotion` (`systems/useItem.js:205`, `:296-306`),
   `RecordLocationFromMap`/`DiscoverRandomLocation`
   (`ui/nativeInventory.js:841-845`, `scenes/world.js:4692`), the
...
10. **`:646` fast-travel residue.** Its three named survivors are spent:
    ...
    `scenes/world.js:3885` (the arrival clamp anchoring the encounter
...
11. **`:651` `PatchRegionIndex` legacy-save fix.** Ported verbatim at
    `src/formats/mapsFile.js:105`, with the C# line range cited.
12. **`:695` the magic crafting windows.** The row's FLAGGED residue is
    ...
    clause goes with it.**
13. **`:645` the talk manager.** The row's own tail already says both
...
14. **`:675` the small-residue trio** (biography GP arm, arrow roll
...
- **The voxel character engine** (Mac's system, Port-Doctrine). 86
  ...
  by a game host: `worldModes.js:5291` passes `voxelfolk`,
...
- **`FaceUVTool` arithmetic** - JS doubles for C# float mix, `Math.trunc`
  ...
  precision are ledger row `:618`, narrowed at E8 and the only part of

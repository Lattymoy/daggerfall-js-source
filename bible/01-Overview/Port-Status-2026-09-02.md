...
the same place - `Port-Ledger.md:608`, the struck keybinding-registry
...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:4800`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:2843` carries the sweep and `hostMagic.js:601` its missile half. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:1883`, `:1689`). Ledger row `:554`'s residue list is spent - see list 2. |
...
and `world.js:185` reaches it by mounting `worldModes`. But `grep -rn
...
- ~~**`src/scenes/dungeonContext.js:1842`** - the two window seams this
  ...
  carries the same closure against Ledger row `:617`. The two WINDOWS
  ...
  `test/x11b.test.js`. This was also Ledger row `:617`'s adjudication,
...
- ~~**`src/scenes/world.js:3092`** - the port's default landing stands
...
- **`src/systems/inputActions.js:543`** - STILL FLAGGED: axes and
  ...
  sub-windows are recorded pending at `Port-Ledger.md:608`. The flag's
...
Quests tab has a machine to read), ~~`world.js:3138` -> `:2955`~~
...
Section C's table is **247 rows** between `Port-Ledger.md:491` and `:737`
...
route** - plus `:608`, struck at its head but carrying a live PENDING
...
1. **`:526` FaceUVTool's 1,803-UV residual at matched precision** ->
...
2. **`:539` the custom builder's hidden `ResetBonusPool` control** (STRUCK at E2, landing after this list was written: the control is live) ->
...
3. **`:509` remainder: the overlay mouse-UP seam** -> UI arc. A7 shipped
...
4. ~~**`:552` the quest machine** -> `playSound`'s busy-skip, the one
...
5. ~~**`:605` the classic `.SAV` reader** -> the phone path: no zip arm in
   the saves picker, a desktop-first charter call.~~ **CLOSED (OT1, 2026-09-10).**
6. **`:608` (struck, with a live clause) the keybinding registry** ->
   the mouse/advanced and joystick sub-windows, the two of DFU's 65
   game windows the port does not cite.
7. ~~**`:617` the standalone dungeon host has no trade window** -> a
...
8. ~~**`:618` three stale probes**~~ **CLOSED (E-group, 2026-09-02).**
...
9. **`:504` UseItem's unbuilt destinations.** Every arm the row names is
   built: `DrinkPotion` (`systems/useItem.js:167`, `:245-255`),
   `RecordLocationFromMap`/`DiscoverRandomLocation`
   (`ui/nativeInventory.js:703-707`, `scenes/world.js:2892`), the
   (`ui/nativeInventory.js:703-707`, `scenes/world.js:2838`), the
...
10. **`:554` fast-travel residue.** Its three named survivors are spent:
    ...
    `scenes/world.js:3852` (the arrival clamp anchoring the encounter
    clock, so the traveled window is not replayed) and `:1966` (the
    `scenes/world.js:3796` (the arrival clamp anchoring the encounter
...
11. **`:559` `PatchRegionIndex` legacy-save fix.** Ported verbatim at
    `src/formats/mapsFile.js:104`, with the C# line range cited.
12. **`:603` the magic crafting windows.** The row's FLAGGED residue is
    ...
    clause goes with it.**
13. **`:553` the talk manager.** The row's own tail already says both
...
14. **`:583` the small-residue trio** (biography GP arm, arrow roll
...
- **`FaceUVTool` arithmetic** - JS doubles for C# float mix, `Math.trunc`
  ...
  precision are ledger row `:526`, narrowed at E8 and the only part of

...
the same place - `Port-Ledger.md:670`, the struck keybinding-registry
...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:5463`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:3246` carries the sweep and `hostMagic.js:608` its missile half. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:2060`, `:1819`). Ledger row `:616`'s residue list is spent - see list 2. |
...
- ~~**`src/scenes/dungeonContext.js:1921`** - the two window seams this
  ...
  carries the same closure against Ledger row `:679`. The two WINDOWS
  ...
  `test/x11b.test.js`. This was also Ledger row `:679`'s adjudication,
...
Quests tab has a machine to read), ~~`world.js:3394` -> `:3254`~~
...
Section C's table is **247 rows** between `Port-Ledger.md:553` and `:799`
...
route** - plus `:670`, struck at its head but carrying a live PENDING
...
1. **`:588` FaceUVTool's 1,803-UV residual at matched precision** ->
...
2. **`:601` the custom builder's hidden `ResetBonusPool` control** (STRUCK at E2, landing after this list was written: the control is live) ->
...
3. ~~**`:571` remainder: the overlay mouse-UP seam** -> UI arc. A7 shipped
...
4. ~~**`:614` the quest machine** -> `playSound`'s busy-skip, the one
...
5. ~~**`:667` the classic `.SAV` reader** -> the phone path: no zip arm in
   the saves picker, a desktop-first charter call.~~ **CLOSED (OT1, 2026-09-10).**
6. **`:670` (struck, with a live clause) the keybinding registry** ->
   the mouse/advanced and joystick sub-windows, the two of DFU's 65
   game windows the port does not cite.
7. ~~**`:679` the standalone dungeon host has no trade window** -> a
...
8. ~~**`:680` three stale probes**~~ **CLOSED (E-group, 2026-09-02).**
...
9. **`:566` UseItem's unbuilt destinations.** Every arm the row names is
   built: `DrinkPotion` (`systems/useItem.js:197`, `:275-285`),
   `RecordLocationFromMap`/`DiscoverRandomLocation`
   (`ui/nativeInventory.js:736-740`, `scenes/world.js:3295`), the
   (`ui/nativeInventory.js:736-740`, `scenes/world.js:3241`), the
...
10. **`:616` fast-travel residue.** Its three named survivors are spent:
    ...
    `scenes/world.js:4388` (the arrival clamp anchoring the encounter
    clock, so the traveled window is not replayed) and `:1969` (the
    `scenes/world.js:4332` (the arrival clamp anchoring the encounter
...
11. **`:621` `PatchRegionIndex` legacy-save fix.** Ported verbatim at
    `src/formats/mapsFile.js:104`, with the C# line range cited.
12. **`:665` the magic crafting windows.** The row's FLAGGED residue is
    ...
    clause goes with it.**
13. **`:615` the talk manager.** The row's own tail already says both
...
14. **`:645` the small-residue trio** (biography GP arm, arrow roll
    ...
    `BIOG*.TXT` files; the faceUV guard rides row `:588`.
...
- **`FaceUVTool` arithmetic** - JS doubles for C# float mix, `Math.trunc`
  ...
  precision are ledger row `:588`, narrowed at E8 and the only part of

...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:7157`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:4374` carries the sweep and `hostMagic.js:711` its missile half. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:2846`, `:2562`). Ledger row `:638`'s residue list is spent - see list 2. |
...
- **`src/scenes/exterior.js:2204`** - Recall pends here; the anchor
...
9. **`:588` UseItem's unbuilt destinations.** Every arm the row names is
   ...
   (`ui/nativeInventory.js:798-802`, `scenes/world.js:4428`), the
...
10. **`:638` fast-travel residue.** Its three named survivors are spent:
    ...
    `scenes/world.js:3643` (the arrival clamp anchoring the encounter

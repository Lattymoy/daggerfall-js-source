...
the same place - `Port-Ledger.md:701`, the struck keybinding-registry
...
`systems/lycanthropy.js:207`), so vampire clan spells price at the
...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:7553`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:4660` carries the sweep and `hostMagic.js:762` its missile half. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:3067`, `:2775`). Ledger row `:647`'s residue list is spent - see list 2. |
...
and `world.js:276` reaches it by mounting `worldModes`. But `grep -rn
...
- ~~**`src/scenes/dungeonContext.js:2126`** - the two window seams this
  ...
  carries the same closure against Ledger row `:710`. The two WINDOWS
  ...
  `test/x11b.test.js`. This was also Ledger row `:710`'s adjudication,
...
Quests tab has a machine to read), ~~`world.js:4432` -> `:4445`~~
...
Section C's table is **247 rows** between `Port-Ledger.md:584` and `:830`
...
route** - plus `:701`, struck at its head but carrying a live PENDING
...
1. **`:619` FaceUVTool's 1,803-UV residual at matched precision** ->
...
2. **`:632` the custom builder's hidden `ResetBonusPool` control** (STRUCK at E2, landing after this list was written: the control is live) ->
...
3. ~~**`:602` remainder: the overlay mouse-UP seam** -> UI arc. A7 shipped
...
4. ~~**`:645` the quest machine** -> `playSound`'s busy-skip, the one
...
5. ~~**`:698` the classic `.SAV` reader** -> the phone path: no zip arm in
   the saves picker, a desktop-first charter call.~~ **CLOSED (OT1, 2026-09-10).**
6. **`:701` (struck, with a live clause) the keybinding registry** ->
   the mouse/advanced and joystick sub-windows, the two of DFU's 65
   game windows the port does not cite.
7. ~~**`:710` the standalone dungeon host has no trade window** -> a
...
8. ~~**`:711` three stale probes**~~ **CLOSED (E-group, 2026-09-02).**
...
9. **`:597` UseItem's unbuilt destinations.** Every arm the row names is
   built: `DrinkPotion` (`systems/useItem.js:205`, `:296-306`),
   `RecordLocationFromMap`/`DiscoverRandomLocation`
   (`ui/nativeInventory.js:841-845`, `scenes/world.js:4714`), the
...
10. **`:647` fast-travel residue.** Its three named survivors are spent:
    ...
    `scenes/world.js:3905` (the arrival clamp anchoring the encounter
...
11. **`:652` `PatchRegionIndex` legacy-save fix.** Ported verbatim at
    `src/formats/mapsFile.js:105`, with the C# line range cited.
12. **`:696` the magic crafting windows.** The row's FLAGGED residue is
    ...
    clause goes with it.**
13. **`:646` the talk manager.** The row's own tail already says both
...
14. **`:676` the small-residue trio** (biography GP arm, arrow roll
...
- **`FaceUVTool` arithmetic** - JS doubles for C# float mix, `Math.trunc`
  ...
  precision are ledger row `:619`, narrowed at E8 and the only part of

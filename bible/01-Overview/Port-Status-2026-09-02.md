...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:4881`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:2893` carries the sweep and `hostMagic.js:608` its missile half. |
...
9. **`:530` UseItem's unbuilt destinations.** Every arm the row names is
   ...
   (`ui/nativeInventory.js:704-708`, `scenes/world.js:2942`), the
   (`ui/nativeInventory.js:704-708`, `scenes/world.js:2888`), the
...
10. **`:580` fast-travel residue.** Its three named survivors are spent:
    ...
    `scenes/world.js:3907` (the arrival clamp anchoring the encounter

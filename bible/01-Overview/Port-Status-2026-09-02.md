...
in-game name: `combat/formulas.js:752` runs it at DFU's own dispatch
...
the same place - `Port-Ledger.md:736`, the struck keybinding-registry
...
| **formats-game** | Verbatim readers / broken consumer | **Verbatim** | The `ItemRecord` conversions were in the wave; ROAD-A A4 took the classic-import stragglers (building-level MAPSAVE, the native bank record, `LegacyArtifactIndexBitfieldCheck`). ~~Ledger row `:733`'s one residue is the phone path - no zip arm in the saves picker, a desktop-first charter call.~~ **CLOSED (OT1, 2026-09-10): the saves picker took the ARENA2 door's zip arm, and the row is struck whole.** |
| **formats-mw** | Departure lane, faithful within it | **Departure lane, now consumed** | `clipSweepTimes` has a production caller: `combat/fpArm.js:2036` runs the whole-clip reach sweep the superseded page said had never run in the game. 17 modules / 8,842 lines. |
...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:8369`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:5327` carries the sweep and `hostMagic.js:816` its missile half. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:3710`, `:3404`). Ledger row `:682`'s residue list is spent - see list 2. |
...
- ~~**`src/scenes/dungeonContext.js:2126`** - the two window seams this
  ...
  carries the same closure against Ledger row `:745`. The two WINDOWS
  ...
  `test/x11b.test.js`. This was also Ledger row `:745`'s adjudication,
...
Section C's table is **247 rows** between `Port-Ledger.md:619` and `:865`
...
route** - plus `:736`, struck at its head but carrying a live PENDING
...
1. **`:654` FaceUVTool's 1,803-UV residual at matched precision** ->
...
2. **`:667` the custom builder's hidden `ResetBonusPool` control** (STRUCK at E2, landing after this list was written: the control is live) ->
...
3. ~~**`:637` remainder: the overlay mouse-UP seam** -> UI arc. A7 shipped
...
4. ~~**`:680` the quest machine** -> `playSound`'s busy-skip, the one
...
5. ~~**`:733` the classic `.SAV` reader** -> the phone path: no zip arm in
   the saves picker, a desktop-first charter call.~~ **CLOSED (OT1, 2026-09-10).**
6. **`:736` (struck, with a live clause) the keybinding registry** ->
   the mouse/advanced and joystick sub-windows, the two of DFU's 65
   game windows the port does not cite.
7. ~~**`:745` the standalone dungeon host has no trade window** -> a
...
8. ~~**`:746` three stale probes**~~ **CLOSED (E-group, 2026-09-02).**
...
9. **`:632` UseItem's unbuilt destinations.** Every arm the row names is
   ...
   (`ui/nativeInventory.js:842-846`, `scenes/world.js:5381`), the
...
10. **`:682` fast-travel residue.** Its three named survivors are spent:
    ...
    `scenes/world.js:4561` (the arrival clamp anchoring the encounter
...
11. **`:687` `PatchRegionIndex` legacy-save fix.** Ported verbatim at
    `src/formats/mapsFile.js:107`, with the C# line range cited.
12. **`:731` the magic crafting windows.** The row's FLAGGED residue is
    ...
    clause goes with it.**
13. **`:681` the talk manager.** The row's own tail already says both
...
14. **`:711` the small-residue trio** (biography GP arm, arrow roll
...
- **`FaceUVTool` arithmetic** - JS doubles for C# float mix, `Math.trunc`
  ...
  precision are ledger row `:654`, narrowed at E8 and the only part of

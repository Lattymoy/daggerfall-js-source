...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:6229`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:3710` carries the sweep and `hostMagic.js:608` its missile half. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:2393`, `:2103`). Ledger row `:622`'s residue list is spent - see list 2. |
...
- **`src/scenes/exterior.js:1999`** - Recall pends here; the anchor
...
- ~~**`src/scenes/exterior.js:1778`** - PX3: this test host mounts no
...
cannot take, `exterior.js:1800`), ~~`exterior.js:1850` -> `:1538`~~
(**CLOSED at QX1**: this host took the quest bridge, so the pause window's
Quests tab has a machine to read), ~~`world.js:3672` -> `:3668`~~
...
9. **`:572` UseItem's unbuilt destinations.** Every arm the row names is
   ...
   (`ui/nativeInventory.js:747-751`, `scenes/world.js:3795`), the

...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:4861`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:2877` carries the sweep and `hostMagic.js:601` its missile half. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:1917`, `:1708`). Ledger row `:578`'s residue list is spent - see list 2. |
...
9. **`:528` UseItem's unbuilt destinations.** Every arm the row names is
   ...
   (`ui/nativeInventory.js:703-707`, `scenes/world.js:2926`), the
   (`ui/nativeInventory.js:703-707`, `scenes/world.js:2872`), the
...
10. **`:578` fast-travel residue.** Its three named survivors are spent:
    ...
    `scenes/world.js:3891` (the arrival clamp anchoring the encounter

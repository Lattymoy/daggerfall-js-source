...
the same place - `Port-Ledger.md:629`, the struck keybinding-registry
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:1902`, `:1708`). Ledger row `:575`'s residue list is spent - see list 2. |
...
- ~~**`src/scenes/dungeonContext.js:1854`** - the two window seams this
  ...
  carries the same closure against Ledger row `:638`. The two WINDOWS
  ...
  `test/x11b.test.js`. This was also Ledger row `:638`'s adjudication,
...
Section C's table is **247 rows** between `Port-Ledger.md:512` and `:758`
...
route** - plus `:629`, struck at its head but carrying a live PENDING
...
1. **`:547` FaceUVTool's 1,803-UV residual at matched precision** ->
...
2. **`:560` the custom builder's hidden `ResetBonusPool` control** (STRUCK at E2, landing after this list was written: the control is live) ->
...
3. ~~**`:530` remainder: the overlay mouse-UP seam** -> UI arc. A7 shipped
...
4. ~~**`:573` the quest machine** -> `playSound`'s busy-skip, the one
...
5. ~~**`:626` the classic `.SAV` reader** -> the phone path: no zip arm in
   the saves picker, a desktop-first charter call.~~ **CLOSED (OT1, 2026-09-10).**
6. **`:629` (struck, with a live clause) the keybinding registry** ->
   the mouse/advanced and joystick sub-windows, the two of DFU's 65
   game windows the port does not cite.
7. ~~**`:638` the standalone dungeon host has no trade window** -> a
...
8. ~~**`:639` three stale probes**~~ **CLOSED (E-group, 2026-09-02).**
...
9. **`:525` UseItem's unbuilt destinations.** Every arm the row names is
...
10. **`:575` fast-travel residue.** Its three named survivors are spent:
...
11. **`:580` `PatchRegionIndex` legacy-save fix.** Ported verbatim at
    `src/formats/mapsFile.js:104`, with the C# line range cited.
12. **`:624` the magic crafting windows.** The row's FLAGGED residue is
    ...
    clause goes with it.**
13. **`:574` the talk manager.** The row's own tail already says both
...
14. **`:604` the small-residue trio** (biography GP arm, arrow roll
...
- **`FaceUVTool` arithmetic** - JS doubles for C# float mix, `Math.trunc`
  ...
  precision are ledger row `:547`, narrowed at E8 and the only part of

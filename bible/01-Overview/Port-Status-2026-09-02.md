...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:4154`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:2455` carries the sweep and `hostMagic.js:594` its missile half. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:1532`, `:1667`). Ledger row `:544`'s residue list is spent - see list 2. |
...
- **`src/scenes/exterior.js:1485`** - Recall pends here; the anchor
...
- ~~**`src/scenes/exterior.js:1375`** - PX3: this test host mounts no
...
- ~~**`src/scenes/world.js:2707`** - the port's default landing stands
...
host cannot take, `exterior.js:1378`), ~~`exterior.js:1421` ->
`:1344`~~ (**CLOSED at QX1**: this host took the quest bridge, so the
pause window's Quests tab has a machine to read), `world.js:2753` ->
...
9. **`:494` UseItem's unbuilt destinations.** Every arm the row names is
   ...
   (`ui/nativeInventory.js:670-674`, `scenes/world.js:2504`), the
   (`ui/nativeInventory.js:670-674`, `scenes/world.js:2450`), the
...
10. **`:544` fast-travel residue.** Its three named survivors are spent:
    ...
    `scenes/world.js:3422` (the arrival clamp anchoring the encounter
    clock, so the traveled window is not replayed) and `:1965` (the
    `scenes/world.js:3375` (the arrival clamp anchoring the encounter

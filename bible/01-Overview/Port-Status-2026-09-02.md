...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:7357`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:4526` carries the sweep and `hostMagic.js:711` its missile half. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:2984`, `:2700`). Ledger row `:641`'s residue list is spent - see list 2. |
...
and `world.js:270` reaches it by mounting `worldModes`. But `grep -rn
...
- ~~**`src/scenes/world.js:4293`** - the port's default landing stands
...
9. **`:591` UseItem's unbuilt destinations.** Every arm the row names is
   ...
   (`ui/nativeInventory.js:798-802`, `scenes/world.js:4580`), the
...
10. **`:641` fast-travel residue.** Its three named survivors are spent:
    ...
    `scenes/world.js:3795` (the arrival clamp anchoring the encounter

...
the same place - `Port-Ledger.md:663`, the struck keybinding-registry
...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:5364`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:3175` carries the sweep and `hostMagic.js:608` its missile half. |
...
| **characters-voxel** | Departure (deliberate) | **Departure, still editor-only** | 86 designs across seven tables - 42 of the 43 monster mobiles, 19 class, 25 villager. The rig is 1,791 lines across 7 modules and is still gated: `worldModes.js:4306` passes `voxelfolk`, `scenes/interiorContext.js:503` consumes it. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:2018`, `:1778`). Ledger row `:609`'s residue list is spent - see list 2. |
...
and `world.js:211` reaches it by mounting `worldModes`. But `grep -rn
...
- ~~**`src/scenes/dungeonContext.js:1910`** - the two window seams this
  ...
  carries the same closure against Ledger row `:672`. The two WINDOWS
  ...
  `test/x11b.test.js`. This was also Ledger row `:672`'s adjudication,
  and that row is struck with it.
- **`src/ui/enhancedMenu.js:2477`** - the rest of the keyboard; the
...
- **`src/scenes/exterior.js:1859`** - Recall pends here; the anchor
...
- ~~**`src/scenes/exterior.js:1649`** - PX3: this test host mounts no
  ...
  site. (The `src/scenes/worldModes.js:1938` row below is struck: ROAD-E
...
- ~~**`src/scenes/world.js:3296`** - the port's default landing stands
...
- ~~**`src/characters/enemyCasting.js:91`** - now exactly one term
  ...
  second.*
- ~~**`src/scenes/worldModes.js:1867`** - above ground only:
...
- ~~**`src/ui/hudLarge.js:75`** - the docked bar occludes, it does not
  ...
  (`render/renderer.js:1672, :1646, :2298, :2474`) and the 2D passes
...
regenerated onto the new sites: ~~`exterior.js:1315` -> `:1151`~~
(**CLOSED at TP2**: the Recall interim narrowed to the one arm this host
cannot take, `exterior.js:1651`), ~~`exterior.js:1695` -> `:1403`~~
(**CLOSED at QX1**: this host took the quest bridge, so the pause window's
Quests tab has a machine to read), ~~`world.js:3335` -> `:3183`~~
(**CLOSED at the ship landing**), ~~`worldModes.js:1863` -> `:1721`~~
...
- ~~**`src/scenes/worldModes.js:1937`** - above ground only: the GUILD
...
Section C's table is **247 rows** between `Port-Ledger.md:546` and `:792`
...
route** - plus `:663`, struck at its head but carrying a live PENDING
...
1. **`:581` FaceUVTool's 1,803-UV residual at matched precision** ->
...
2. **`:594` the custom builder's hidden `ResetBonusPool` control** (STRUCK at E2, landing after this list was written: the control is live) ->
...
3. ~~**`:564` remainder: the overlay mouse-UP seam** -> UI arc. A7 shipped
...
4. ~~**`:607` the quest machine** -> `playSound`'s busy-skip, the one
...
5. ~~**`:660` the classic `.SAV` reader** -> the phone path: no zip arm in
   the saves picker, a desktop-first charter call.~~ **CLOSED (OT1, 2026-09-10).**
6. **`:663` (struck, with a live clause) the keybinding registry** ->
   the mouse/advanced and joystick sub-windows, the two of DFU's 65
   game windows the port does not cite.
7. ~~**`:672` the standalone dungeon host has no trade window** -> a
...
8. ~~**`:673` three stale probes**~~ **CLOSED (E-group, 2026-09-02).**
...
9. **`:559` UseItem's unbuilt destinations.** Every arm the row names is
   built: `DrinkPotion` (`systems/useItem.js:184`, `:262-272`),
   `RecordLocationFromMap`/`DiscoverRandomLocation`
   (`ui/nativeInventory.js:736-740`, `scenes/world.js:3224`), the
   (`ui/nativeInventory.js:736-740`, `scenes/world.js:3170`), the
   quest-item click (`useItem.js:216`, `:229-230`) and
...
10. **`:609` fast-travel residue.** Its three named survivors are spent:
    ...
    `scenes/world.js:4301` (the arrival clamp anchoring the encounter
    clock, so the traveled window is not replayed) and `:1969` (the
    `scenes/world.js:4245` (the arrival clamp anchoring the encounter
...
11. **`:614` `PatchRegionIndex` legacy-save fix.** Ported verbatim at
    `src/formats/mapsFile.js:104`, with the C# line range cited.
12. **`:658` the magic crafting windows.** The row's FLAGGED residue is
    ...
    clause goes with it.**
13. **`:608` the talk manager.** The row's own tail already says both
...
14. **`:638` the small-residue trio** (biography GP arm, arrow roll
...
- **The voxel character engine** (Mac's system, Port-Doctrine). 86
  ...
  by a game host: `worldModes.js:4306` passes `voxelfolk`,
...
- **`FaceUVTool` arithmetic** - JS doubles for C# float mix, `Math.trunc`
  ...
  precision are ledger row `:581`, narrowed at E8 and the only part of

...
in-game name: `combat/formulas.js:740` runs it at DFU's own dispatch
...
the same place - `Port-Ledger.md:722`, the struck keybinding-registry
...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:7672`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:4766` carries the sweep and `hostMagic.js:792` its missile half. |
...
| **player** | Near-1:1 / one lethal seam | **Near-1:1** | `cancelMovement` is a real motor field set on the mode edges (`player/motor.js:374,:451,:459`). ROAD-A A6 took the -0.28 doorway head-dip, `PlayerMoveScanner`'s three probes, `controllerSwimHeight` 0.30 with Do(Un)Sinking, and `FreezeMotor` on teleports. |
...
| **characters-voxel** | Departure (deliberate) | **Departure, still editor-only** | 86 designs across seven tables - 42 of the 43 monster mobiles, 19 class, 25 villager. The rig is 1,791 lines across 7 modules and is still gated: `worldModes.js:5329` passes `voxelfolk`, `scenes/interiorContext.js:577` consumes it. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:3172`, `:2880`). Ledger row `:668`'s residue list is spent - see list 2. |
...
| **ui-enhanced** | Departure lane | **Departure lane, scoped** | `ui/worldPlaque.js:326` puts the skin gate above `ensure()`, so the unscoped `*`/`html`/`body`/`button`/`#app` rules never reach the classic page. 9 modules / 8,940 lines. |
...
and `world.js:282` reaches it by mounting `worldModes`. But `grep -rn
...
- ~~**`src/scenes/dungeonContext.js:2126`** - the two window seams this
  ...
  carries the same closure against Ledger row `:731`. The two WINDOWS
  ...
  `test/x11b.test.js`. This was also Ledger row `:731`'s adjudication,
  and that row is struck with it.
- **`src/ui/enhancedMenu.js:3298`** - the rest of the keyboard; the
...
- ~~**`src/scenes/exterior.js:1943`** - PX3: this test host mounts no
  ...
  site. (The `src/scenes/worldModes.js:2371` row below is struck: ROAD-E
...
- ~~**`src/scenes/world.js:4432`** - the port's default landing stands
...
- ~~**`src/characters/enemyCasting.js:91`** - now exactly one term
  ...
  second.*
- ~~**`src/scenes/worldModes.js:1971`** - above ground only:
...
- ~~**`src/ui/hudLarge.js:75`** - the docked bar occludes, it does not
  ...
  (`render/renderer.js:2328, :2050, :3066, :3391`) and the 2D passes
...
- **`src/scenes/dungeonContext.js:1193`** - the dungeon's non-layout foe
...
regenerated onto the new sites: ~~`exterior.js:1604` -> `:1292`~~
(**CLOSED at TP2**: the Recall interim narrowed to the one arm this host
cannot take, `exterior.js:1965`), ~~`exterior.js:2015` -> `:1684`~~
(**CLOSED at QX1**: this host took the quest bridge, so the pause window's
Quests tab has a machine to read), ~~`world.js:4459` -> `:4482`~~
(**CLOSED at the ship landing**), ~~`worldModes.js:1934` -> `:1811`~~
...
- ~~**`src/scenes/worldModes.js:2046`** - above ground only: the GUILD
...
Section C's table is **247 rows** between `Port-Ledger.md:605` and `:851`
...
route** - plus `:722`, struck at its head but carrying a live PENDING
...
1. **`:640` FaceUVTool's 1,803-UV residual at matched precision** ->
...
2. **`:653` the custom builder's hidden `ResetBonusPool` control** (STRUCK at E2, landing after this list was written: the control is live) ->
...
3. ~~**`:623` remainder: the overlay mouse-UP seam** -> UI arc. A7 shipped
...
4. ~~**`:666` the quest machine** -> `playSound`'s busy-skip, the one
...
5. ~~**`:719` the classic `.SAV` reader** -> the phone path: no zip arm in
   the saves picker, a desktop-first charter call.~~ **CLOSED (OT1, 2026-09-10).**
6. **`:722` (struck, with a live clause) the keybinding registry** ->
   the mouse/advanced and joystick sub-windows, the two of DFU's 65
   game windows the port does not cite.
7. ~~**`:731` the standalone dungeon host has no trade window** -> a
...
8. ~~**`:732` three stale probes**~~ **CLOSED (E-group, 2026-09-02).**
...
9. **`:618` UseItem's unbuilt destinations.** Every arm the row names is
   built: `DrinkPotion` (`systems/useItem.js:205`, `:296-306`),
   `RecordLocationFromMap`/`DiscoverRandomLocation`
   (`ui/nativeInventory.js:834-838`, `scenes/world.js:4861`), the
...
10. **`:668` fast-travel residue.** Its three named survivors are spent:
    ...
    `scenes/world.js:4044` (the arrival clamp anchoring the encounter
...
11. **`:673` `PatchRegionIndex` legacy-save fix.** Ported verbatim at
    `src/formats/mapsFile.js:107`, with the C# line range cited.
12. **`:717` the magic crafting windows.** The row's FLAGGED residue is
    ...
    clause goes with it.**
13. **`:667` the talk manager.** The row's own tail already says both
...
14. **`:697` the small-residue trio** (biography GP arm, arrow roll
...
- **The voxel character engine** (Mac's system, Port-Doctrine). 86
  ...
  by a game host: `worldModes.js:5648` passes `voxelfolk`,
...
- **`FaceUVTool` arithmetic** - JS doubles for C# float mix, `Math.trunc`
  ...
  precision are ledger row `:640`, narrowed at E8 and the only part of

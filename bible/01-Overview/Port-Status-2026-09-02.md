...
the same place - `Port-Ledger.md:674`, the struck keybinding-registry
...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:5911`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:3412` carries the sweep and `hostMagic.js:608` its missile half. |
...
| **characters-voxel** | Departure (deliberate) | **Departure, still editor-only** | 86 designs across seven tables - 42 of the 43 monster mobiles, 19 class, 25 villager. The rig is 1,791 lines across 7 modules and is still gated: `worldModes.js:4322` passes `voxelfolk`, `scenes/interiorContext.js:503` consumes it. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:2186`, `:1911`). Ledger row `:620`'s residue list is spent - see list 2. |
...
| **ui-hud** | Verbatim (classic) / Partial (enhanced default) | **Verbatim on both skins** | The skin fork moved BELOW the game-state seams: `ui/hud.js:497` runs `updateHudVitals` and `:552` `drawNearDeathFlicker`, both above the enhanced branch at `:583` and above the `!art` return, and the enhanced HUD takes the Detect markers at `:597`. `lastHealthLost()` is no longer pinned at 0, so `CameraRecoiler` lives in all three hosts. |
...
and `world.js:227` reaches it by mounting `worldModes`. But `grep -rn
...
- ~~**`src/scenes/dungeonContext.js:1921`** - the two window seams this
  ...
  carries the same closure against Ledger row `:683`. The two WINDOWS
  ...
  `test/x11b.test.js`. This was also Ledger row `:683`'s adjudication,
  and that row is struck with it.
- **`src/ui/enhancedMenu.js:2505`** - the rest of the keyboard; the
...
- **`src/scenes/exterior.js:1911`** - Recall pends here; the anchor
...
- ~~**`src/scenes/exterior.js:1680`** - PX3: this test host mounts no
  ...
  site. (The `src/scenes/worldModes.js:1945` row below is struck: ROAD-E
...
- ~~**`src/scenes/world.js:3436`** - the port's default landing stands
...
- ~~**`src/ui/hudLarge.js:75`** - the docked bar occludes, it does not
  ...
  (`render/renderer.js:1676, :1650, :2302, :2478`) and the 2D passes
...
Quests tab has a machine to read), ~~`world.js:3463` -> `:3428`~~
...
Section C's table is **247 rows** between `Port-Ledger.md:557` and `:803`
...
route** - plus `:674`, struck at its head but carrying a live PENDING
...
1. **`:592` FaceUVTool's 1,803-UV residual at matched precision** ->
...
2. **`:605` the custom builder's hidden `ResetBonusPool` control** (STRUCK at E2, landing after this list was written: the control is live) ->
...
3. ~~**`:575` remainder: the overlay mouse-UP seam** -> UI arc. A7 shipped
...
4. ~~**`:618` the quest machine** -> `playSound`'s busy-skip, the one
...
5. ~~**`:671` the classic `.SAV` reader** -> the phone path: no zip arm in
   the saves picker, a desktop-first charter call.~~ **CLOSED (OT1, 2026-09-10).**
6. **`:674` (struck, with a live clause) the keybinding registry** ->
   the mouse/advanced and joystick sub-windows, the two of DFU's 65
   game windows the port does not cite.
7. ~~**`:683` the standalone dungeon host has no trade window** -> a
...
8. ~~**`:684` three stale probes**~~ **CLOSED (E-group, 2026-09-02).**
...
9. **`:570` UseItem's unbuilt destinations.** Every arm the row names is
   built: `DrinkPotion` (`systems/useItem.js:197`, `:275-285`),
   `RecordLocationFromMap`/`DiscoverRandomLocation`
   (`ui/nativeInventory.js:736-740`, `scenes/world.js:3477`), the
...
10. **`:620` fast-travel residue.** Its three named survivors are spent:
    ...
    `scenes/world.js:4498` (the arrival clamp anchoring the encounter
...
11. **`:625` `PatchRegionIndex` legacy-save fix.** Ported verbatim at
    `src/formats/mapsFile.js:104`, with the C# line range cited.
12. **`:669` the magic crafting windows.** The row's FLAGGED residue is
    ...
    clause goes with it.**
13. **`:619` the talk manager.** The row's own tail already says both
...
14. **`:649` the small-residue trio** (biography GP arm, arrow roll
...
- **The voxel character engine** (Mac's system, Port-Doctrine). 86
  ...
  by a game host: `worldModes.js:4322` passes `voxelfolk`,
...
- **`FaceUVTool` arithmetic** - JS doubles for C# float mix, `Math.trunc`
  ...
  precision are ledger row `:592`, narrowed at E8 and the only part of

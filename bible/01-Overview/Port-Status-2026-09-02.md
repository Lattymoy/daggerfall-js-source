...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:6648`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:3895` carries the sweep and `hostMagic.js:608` its missile half. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:2470`, `:2187`). Ledger row `:637`'s residue list is spent - see list 2. |
...
- **`src/scenes/exterior.js:2065`** - Recall pends here; the anchor
...
- ~~**`src/scenes/exterior.js:1755`** - PX3: this test host mounts no
...
- ~~**`src/scenes/world.js:3662`** - the port's default landing stands
...
regenerated onto the new sites: ~~`exterior.js:1435` -> `:1206`~~
(**CLOSED at TP2**: the Recall interim narrowed to the one arm this host
cannot take, `exterior.js:1777`), ~~`exterior.js:1827` -> `:1515`~~
...
9. **`:587` UseItem's unbuilt destinations.** Every arm the row names is
   ...
   (`ui/nativeInventory.js:796-800`, `scenes/world.js:3949`), the
...
10. **`:637` fast-travel residue.** Its three named survivors are spent:
    ...
    `scenes/world.js:3172` (the arrival clamp anchoring the encounter

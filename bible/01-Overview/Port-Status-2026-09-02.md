...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:5868`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:3391` carries the sweep and `hostMagic.js:608` its missile half. |
...
- **`src/ui/enhancedMenu.js:2478`** - the rest of the keyboard; the
...
- **`src/scenes/exterior.js:1890`** - Recall pends here; the anchor
...
Quests tab has a machine to read), ~~`world.js:3463` -> `:3408`~~
...
9. **`:568` UseItem's unbuilt destinations.** Every arm the row names is
   ...
   (`ui/nativeInventory.js:736-740`, `scenes/world.js:3456`), the
...
10. **`:618` fast-travel residue.** Its three named survivors are spent:
    ...
    `scenes/world.js:4533` (the arrival clamp anchoring the encounter
    clock, so the traveled window is not replayed) and `:1969` (the
    `scenes/world.js:4477` (the arrival clamp anchoring the encounter

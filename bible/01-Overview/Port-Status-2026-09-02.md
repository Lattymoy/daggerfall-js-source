...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:6463`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:3857` carries the sweep and `hostMagic.js:608` its missile half. |
...
| **characters-voxel** | Departure (deliberate) | **Departure, still editor-only** | 86 designs across seven tables - 42 of the 43 monster mobiles, 19 class, 25 villager. The rig is 1,791 lines across 7 modules and is still gated: `worldModes.js:5043` passes `voxelfolk`, `scenes/interiorContext.js:503` consumes it. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:2455`, `:2172`). Ledger row `:630`'s residue list is spent - see list 2. |
...
that own overlay slots - `worldModes.js:80` and `dungeonContext.js:43` -
and `world.js:251` reaches it by mounting `worldModes`. But `grep -rn
...
- ~~**`src/scenes/dungeonContext.js:2049`** - the two window seams this
...
- **`src/scenes/exterior.js:2050`** - Recall pends here; the anchor
...
- ~~**`src/scenes/exterior.js:1753`** - PX3: this test host mounts no
  ...
  site. (The `src/scenes/worldModes.js:2214` row below is struck: ROAD-E
...
- **`src/scenes/dungeonContext.js:1135`** - the dungeon's non-layout foe
...
(**CLOSED at the ship landing**), ~~`worldModes.js:1898` -> `:1778`~~
...
9. **`:580` UseItem's unbuilt destinations.** Every arm the row names is
   ...
   (`ui/nativeInventory.js:778-782`, `scenes/world.js:3911`), the
...
10. **`:630` fast-travel residue.** Its three named survivors are spent:
    ...
    `scenes/world.js:3133` (the arrival clamp anchoring the encounter
...
- **The voxel character engine** (Mac's system, Port-Doctrine). 86
  ...
  by a game host: `worldModes.js:5043` passes `voxelfolk`,

...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:7604`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:4680` carries the sweep and `hostMagic.js:816` its missile half. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:3085`, `:2793`). Ledger row `:670`'s residue list is spent - see list 2. |
...
and `world.js:292` reaches it by mounting `worldModes`. But `grep -rn
...
9. **`:620` UseItem's unbuilt destinations.** Every arm the row names is
   ...
   (`ui/nativeInventory.js:834-838`, `scenes/world.js:4734`), the
...
10. **`:670` fast-travel residue.** Its three named survivors are spent:
    ...
    `scenes/world.js:3922` (the arrival clamp anchoring the encounter

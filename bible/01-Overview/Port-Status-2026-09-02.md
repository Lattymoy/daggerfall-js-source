...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:6672`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:3911` carries the sweep and `hostMagic.js:711` its missile half. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:2478`, `:2195`). Ledger row `:638`'s residue list is spent - see list 2. |
...
- ~~**`src/scenes/world.js:3678`** - the port's default landing stands
...
Quests tab has a machine to read), ~~`world.js:3705` -> `:3698`~~
...
9. **`:588` UseItem's unbuilt destinations.** Every arm the row names is
   ...
   (`ui/nativeInventory.js:796-800`, `scenes/world.js:3965`), the
...
10. **`:638` fast-travel residue.** Its three named survivors are spent:
    ...
    `scenes/world.js:3180` (the arrival clamp anchoring the encounter

...
the same place - `Port-Ledger.md:694`, the struck keybinding-registry
...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:6818`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:4025` carries the sweep and `hostMagic.js:711` its missile half. |
| **scenes-modes** | Solid, pause parity broken | **Near-1:1** | ROAD-B B1 put `UserInterfaceManager`'s real stack under this host's slot (`ui/windowStack.js`, 295 lines, imported at `worldModes.js:78`). See "the pause primitive" below - the stack exists, its `paused()` member has no reader. |
...
| **characters-voxel** | Departure (deliberate) | **Departure, still editor-only** | 86 designs across seven tables - 42 of the 43 monster mobiles, 19 class, 25 villager. The rig is 1,791 lines across 7 modules and is still gated: `worldModes.js:5227` passes `voxelfolk`, `scenes/interiorContext.js:503` consumes it. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:2485`, `:2201`). Ledger row `:640`'s residue list is spent - see list 2. |
...
that own overlay slots - `worldModes.js:79` and `dungeonContext.js:44` -
and `world.js:264` reaches it by mounting `worldModes`. But `grep -rn
...
- ~~**`src/scenes/dungeonContext.js:2122`** - the two window seams this
  ...
  carries the same closure against Ledger row `:703`. The two WINDOWS
  ...
  `test/x11b.test.js`. This was also Ledger row `:703`'s adjudication,
  and that row is struck with it.
- **`src/ui/enhancedMenu.js:3013`** - the rest of the keyboard; the
...
- **`src/scenes/exterior.js:2160`** - Recall pends here; the anchor
...
- ~~**`src/scenes/exterior.js:1850`** - PX3: this test host mounts no
  ...
  site. (The `src/scenes/worldModes.js:2250` row below is struck: ROAD-E
...
- ~~**`src/scenes/world.js:3792`** - the port's default landing stands
...
- ~~**`src/characters/enemyCasting.js:91`** - now exactly one term
  ...
  second.*
- ~~**`src/scenes/worldModes.js:1946`** - above ground only:
...
regenerated onto the new sites: ~~`exterior.js:1529` -> `:1218`~~
(**CLOSED at TP2**: the Recall interim narrowed to the one arm this host
cannot take, `exterior.js:1872`), ~~`exterior.js:1922` -> `:1609`~~
(**CLOSED at QX1**: this host took the quest bridge, so the pause window's
Quests tab has a machine to read), ~~`world.js:3819` -> `:3812`~~
(**CLOSED at the ship landing**), ~~`worldModes.js:1909` -> `:1789`~~
...
- ~~**`src/scenes/worldModes.js:2016`** - above ground only: the GUILD
...
Section C's table is **247 rows** between `Port-Ledger.md:577` and `:823`
...
route** - plus `:694`, struck at its head but carrying a live PENDING
...
1. **`:612` FaceUVTool's 1,803-UV residual at matched precision** ->
...
2. **`:625` the custom builder's hidden `ResetBonusPool` control** (STRUCK at E2, landing after this list was written: the control is live) ->
...
3. ~~**`:595` remainder: the overlay mouse-UP seam** -> UI arc. A7 shipped
...
4. ~~**`:638` the quest machine** -> `playSound`'s busy-skip, the one
...
5. ~~**`:691` the classic `.SAV` reader** -> the phone path: no zip arm in
   the saves picker, a desktop-first charter call.~~ **CLOSED (OT1, 2026-09-10).**
6. **`:694` (struck, with a live clause) the keybinding registry** ->
   the mouse/advanced and joystick sub-windows, the two of DFU's 65
   game windows the port does not cite.
7. ~~**`:703` the standalone dungeon host has no trade window** -> a
...
8. ~~**`:704` three stale probes**~~ **CLOSED (E-group, 2026-09-02).**
...
9. **`:590` UseItem's unbuilt destinations.** Every arm the row names is
   built: `DrinkPotion` (`systems/useItem.js:198`, `:276-286`),
   `RecordLocationFromMap`/`DiscoverRandomLocation`
   (`ui/nativeInventory.js:798-802`, `scenes/world.js:4079`), the
...
10. **`:640` fast-travel residue.** Its three named survivors are spent:
    ...
    `scenes/world.js:3294` (the arrival clamp anchoring the encounter
...
11. **`:645` `PatchRegionIndex` legacy-save fix.** Ported verbatim at
    `src/formats/mapsFile.js:104`, with the C# line range cited.
12. **`:689` the magic crafting windows.** The row's FLAGGED residue is
    ...
    clause goes with it.**
13. **`:639` the talk manager.** The row's own tail already says both
...
14. **`:669` the small-residue trio** (biography GP arm, arrow roll
...
- **The voxel character engine** (Mac's system, Port-Doctrine). 86
  ...
  by a game host: `worldModes.js:5227` passes `voxelfolk`,
...
- **`FaceUVTool` arithmetic** - JS doubles for C# float mix, `Math.trunc`
  ...
  precision are ledger row `:612`, narrowed at E8 and the only part of

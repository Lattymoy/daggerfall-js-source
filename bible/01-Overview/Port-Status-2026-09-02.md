...
the same place - `Port-Ledger.md:726`, the struck keybinding-registry
...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:7611`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:4682` carries the sweep and `hostMagic.js:816` its missile half. |
...
| **characters-voxel** | Departure (deliberate) | **Departure, still editor-only** | 86 designs across seven tables - 42 of the 43 monster mobiles, 19 class, 25 villager. The rig is 1,791 lines across 7 modules and is still gated: `worldModes.js:5303` passes `voxelfolk`, `scenes/interiorContext.js:577` consumes it. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:3087`, `:2795`). Ledger row `:672`'s residue list is spent - see list 2. |
...
and `world.js:294` reaches it by mounting `worldModes`. But `grep -rn
...
- ~~**`src/scenes/dungeonContext.js:2126`** - the two window seams this
  ...
  carries the same closure against Ledger row `:735`. The two WINDOWS
  ...
  `test/x11b.test.js`. This was also Ledger row `:735`'s adjudication,
...
- ~~**`src/scenes/exterior.js:1943`** - PX3: this test host mounts no
  ...
  site. (The `src/scenes/worldModes.js:2297` row below is struck: ROAD-E
...
- ~~**`src/scenes/world.js:4424`** - the port's default landing stands
...
- ~~**`src/characters/enemyCasting.js:91`** - now exactly one term
  ...
  second.*
- ~~**`src/scenes/worldModes.js:1961`** - above ground only:
  `QuestMachine.SetupIndividualStaticNPC`. *Multi-host. The law is
  ported and idle at `systems/quest/machine.js:808` including the
...
- ~~**`src/ui/hudLarge.js:75`** - the docked bar occludes, it does not
  ...
  (`render/renderer.js:2242, :2038, :2980, :3305`) and the 2D passes
...
Quests tab has a machine to read), ~~`world.js:4451` -> `:4474`~~
(**CLOSED at the ship landing**), ~~`worldModes.js:1924` -> `:1801`~~
...
- ~~**`src/scenes/worldModes.js:2031`** - above ground only: the GUILD
...
Section C's table is **247 rows** between `Port-Ledger.md:609` and `:855`
...
route** - plus `:726`, struck at its head but carrying a live PENDING
...
1. **`:644` FaceUVTool's 1,803-UV residual at matched precision** ->
...
2. **`:657` the custom builder's hidden `ResetBonusPool` control** (STRUCK at E2, landing after this list was written: the control is live) ->
...
3. ~~**`:627` remainder: the overlay mouse-UP seam** -> UI arc. A7 shipped
...
4. ~~**`:670` the quest machine** -> `playSound`'s busy-skip, the one
...
5. ~~**`:723` the classic `.SAV` reader** -> the phone path: no zip arm in
   the saves picker, a desktop-first charter call.~~ **CLOSED (OT1, 2026-09-10).**
6. **`:726` (struck, with a live clause) the keybinding registry** ->
   the mouse/advanced and joystick sub-windows, the two of DFU's 65
   game windows the port does not cite.
7. ~~**`:735` the standalone dungeon host has no trade window** -> a
...
8. ~~**`:736` three stale probes**~~ **CLOSED (E-group, 2026-09-02).**
...
9. **`:622` UseItem's unbuilt destinations.** Every arm the row names is
   built: `DrinkPotion` (`systems/useItem.js:205`, `:296-306`),
   `RecordLocationFromMap`/`DiscoverRandomLocation`
   (`ui/nativeInventory.js:834-838`, `scenes/world.js:4736`), the
...
10. **`:672` fast-travel residue.** Its three named survivors are spent:
    ...
    `scenes/world.js:3924` (the arrival clamp anchoring the encounter
...
11. **`:677` `PatchRegionIndex` legacy-save fix.** Ported verbatim at
    `src/formats/mapsFile.js:107`, with the C# line range cited.
12. **`:721` the magic crafting windows.** The row's FLAGGED residue is
    ...
    clause goes with it.**
13. **`:671` the talk manager.** The row's own tail already says both
...
14. **`:701` the small-residue trio** (biography GP arm, arrow roll
...
- **The voxel character engine** (Mac's system, Port-Doctrine). 86
  ...
  by a game host: `worldModes.js:5303` passes `voxelfolk`,
  `scenes/interiorContext.js:577` gates on it, and no default route sets
...
- **`FaceUVTool` arithmetic** - JS doubles for C# float mix, `Math.trunc`
  ...
  precision are ledger row `:644`, narrowed at E8 and the only part of

...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:6299`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:3700` carries the sweep and `hostMagic.js:608` its missile half. |
...
| **characters-voxel** | Departure (deliberate) | **Departure, still editor-only** | 86 designs across seven tables - 42 of the 43 monster mobiles, 19 class, 25 villager. The rig is 1,791 lines across 7 modules and is still gated: `worldModes.js:4402` passes `voxelfolk`, `scenes/interiorContext.js:503` consumes it. |
...
- **`src/scenes/exterior.js:1974`** - Recall pends here; the anchor
...
- ~~**`src/scenes/world.js:3633`** - the port's default landing stands
...
- ~~**`src/characters/enemyCasting.js:91`** - now exactly one term
  ...
  second.*
- ~~**`src/scenes/worldModes.js:1927`** - above ground only:
...
- ~~**`src/ui/hudLarge.js:75`** - the docked bar occludes, it does not
  ...
  (`render/renderer.js:1983, :1910, :2730, :3042`) and the 2D passes
...
Quests tab has a machine to read), ~~`world.js:3660` -> `:3657`~~
(**CLOSED at the ship landing**), ~~`worldModes.js:1890` -> `:1777`~~
...
- ~~**`src/scenes/worldModes.js:1997`** - above ground only: the GUILD
...
9. **`:579` UseItem's unbuilt destinations.** Every arm the row names is
   ...
   (`ui/nativeInventory.js:778-782`, `scenes/world.js:3800`), the
...
10. **`:629` fast-travel residue.** Its three named survivors are spent:
    ...
    `scenes/world.js:4865` (the arrival clamp anchoring the encounter
...
- **The voxel character engine** (Mac's system, Port-Doctrine). 86
  ...
  by a game host: `worldModes.js:4402` passes `voxelfolk`,

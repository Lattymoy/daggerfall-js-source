...
and six rows** (five, since OT1 struck `:605`),
...
the same place - `Port-Ledger.md:665`, the struck keybinding-registry
...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:5281`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:3100` carries the sweep and `hostMagic.js:608` its missile half. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:2002`, `:1762`). Ledger row `:611`'s residue list is spent - see list 2. |
...
- ~~**`src/scenes/dungeonContext.js:1905`** - the two window seams this
  ...
  carries the same closure against Ledger row `:674`. The two WINDOWS
  ...
  `test/x11b.test.js`. This was also Ledger row `:674`'s adjudication,
...
Section C's table is **247 rows** between `Port-Ledger.md:548` and `:794`
...
route** - plus `:665`, struck at its head but carrying a live PENDING
...
1. **`:583` FaceUVTool's 1,803-UV residual at matched precision** ->
...
2. **`:596` the custom builder's hidden `ResetBonusPool` control** (STRUCK at E2, landing after this list was written: the control is live) ->
   ...
   `systems/dialogShortcuts.js:194`/`:318` carries `ResetBonusPool` with
...
3. ~~**`:566` remainder: the overlay mouse-UP seam** -> UI arc. A7 shipped
...
4. ~~**`:609` the quest machine** -> `playSound`'s busy-skip, the one
...
5. ~~**`:662` the classic `.SAV` reader** -> the phone path: no zip arm in
   the saves picker, a desktop-first charter call.~~ **CLOSED (OT1, 2026-09-10).**
6. **`:665` (struck, with a live clause) the keybinding registry** ->
   the mouse/advanced and joystick sub-windows, the two of DFU's 65
   game windows the port does not cite.
7. ~~**`:674` the standalone dungeon host has no trade window** -> a
...
8. ~~**`:675` three stale probes**~~ **CLOSED (E-group, 2026-09-02).**
...
9. **`:561` UseItem's unbuilt destinations.** Every arm the row names is
...
10. **`:611` fast-travel residue.** Its three named survivors are spent:
    ...
    `scenes/world.js:4222` (the arrival clamp anchoring the encounter
    clock, so the traveled window is not replayed) and `:1969` (the
    `scenes/world.js:4166` (the arrival clamp anchoring the encounter
...
11. **`:616` `PatchRegionIndex` legacy-save fix.** Ported verbatim at
    `src/formats/mapsFile.js:104`, with the C# line range cited.
12. **`:660` the magic crafting windows.** The row's FLAGGED residue is
    ...
    clause goes with it.**
13. **`:610` the talk manager.** The row's own tail already says both
...
14. **`:640` the small-residue trio** (biography GP arm, arrow roll
...
- **`FaceUVTool` arithmetic** - JS doubles for C# float mix, `Math.trunc`
  ...
  precision are ledger row `:583`, narrowed at E8 and the only part of

...
the same place - `Port-Ledger.md:659`, the struck keybinding-registry
...
| **formats-game** | Verbatim readers / broken consumer | **Verbatim** | The `ItemRecord` conversions were in the wave; ROAD-A A4 took the classic-import stragglers (building-level MAPSAVE, the native bank record, `LegacyArtifactIndexBitfieldCheck`). ~~Ledger row `:656`'s one residue is the phone path - no zip arm in the saves picker, a desktop-first charter call.~~ **CLOSED (OT1, 2026-09-10): the saves picker took the ARENA2 door's zip arm, and the row is struck whole.** |
| **formats-mw** | Departure lane, faithful within it | **Departure lane, now consumed** | `clipSweepTimes` has a production caller: `combat/fpArm.js:1715` runs the whole-clip reach sweep the superseded page said had never run in the game. 17 modules / 8,842 lines. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:1992`, `:1760`). Ledger row `:605`'s residue list is spent - see list 2. |
...
- ~~**`src/scenes/dungeonContext.js:1905`** - the two window seams this
  ...
  carries the same closure against Ledger row `:668`. The two WINDOWS
  ...
  `test/x11b.test.js`. This was also Ledger row `:668`'s adjudication,
  and that row is struck with it.
- **`src/ui/enhancedMenu.js:2457`** - the rest of the keyboard; the
...
- ~~**`src/ui/hudLarge.js:75`** - the docked bar occludes, it does not
  ...
  (`render/renderer.js:1618, :1592, :2140, :2316`) and the 2D passes
...
Section C's table is **247 rows** between `Port-Ledger.md:542` and `:788`
...
route** - plus `:659`, struck at its head but carrying a live PENDING
...
1. **`:577` FaceUVTool's 1,803-UV residual at matched precision** ->
...
2. **`:590` the custom builder's hidden `ResetBonusPool` control** (STRUCK at E2, landing after this list was written: the control is live) ->
...
3. ~~**`:560` remainder: the overlay mouse-UP seam** -> UI arc. A7 shipped
...
4. ~~**`:603` the quest machine** -> `playSound`'s busy-skip, the one
...
5. ~~**`:656` the classic `.SAV` reader** -> the phone path: no zip arm in
   the saves picker, a desktop-first charter call.~~ **CLOSED (OT1, 2026-09-10).**
6. **`:659` (struck, with a live clause) the keybinding registry** ->
   the mouse/advanced and joystick sub-windows, the two of DFU's 65
   game windows the port does not cite.
7. ~~**`:668` the standalone dungeon host has no trade window** -> a
...
8. ~~**`:669` three stale probes**~~ **CLOSED (E-group, 2026-09-02).**
...
9. **`:555` UseItem's unbuilt destinations.** Every arm the row names is
...
10. **`:605` fast-travel residue.** Its three named survivors are spent:
...
11. **`:610` `PatchRegionIndex` legacy-save fix.** Ported verbatim at
    `src/formats/mapsFile.js:104`, with the C# line range cited.
12. **`:654` the magic crafting windows.** The row's FLAGGED residue is
    ...
    clause goes with it.**
13. **`:604` the talk manager.** The row's own tail already says both
...
14. **`:634` the small-residue trio** (biography GP arm, arrow roll
...
- **`FaceUVTool` arithmetic** - JS doubles for C# float mix, `Math.trunc`
  ...
  precision are ledger row `:577`, narrowed at E8 and the only part of

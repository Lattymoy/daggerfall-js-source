...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:5004`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:2969` carries the sweep and `hostMagic.js:608` its missile half. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:1970`, `:1738`). Ledger row `:596`'s residue list is spent - see list 2. |
...
- ~~**`src/scenes/world.js:3204`** - the port's default landing stands
...
Quests tab has a machine to read), ~~`world.js:3250` -> `:3053`~~
...
9. **`:546` UseItem's unbuilt destinations.** Every arm the row names is
   ...
   (`ui/nativeInventory.js:704-708`, `scenes/world.js:3018`), the
   (`ui/nativeInventory.js:704-708`, `scenes/world.js:2964`), the
...
10. **`:596` fast-travel residue.** Its three named survivors are spent:
    ...
    `scenes/world.js:4018` (the arrival clamp anchoring the encounter
    clock, so the traveled window is not replayed) and `:1966` (the
    `scenes/world.js:3962` (the arrival clamp anchoring the encounter

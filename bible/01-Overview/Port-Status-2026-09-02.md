...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:7073`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:4292` carries the sweep and `hostMagic.js:711` its missile half. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:2764`, `:2480`). Ledger row `:638`'s residue list is spent - see list 2. |
...
9. **`:588` UseItem's unbuilt destinations.** Every arm the row names is
   ...
   (`ui/nativeInventory.js:798-802`, `scenes/world.js:4346`), the
...
10. **`:638` fast-travel residue.** Its three named survivors are spent:
    ...
    `scenes/world.js:3561` (the arrival clamp anchoring the encounter

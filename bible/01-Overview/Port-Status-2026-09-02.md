...
the same place - `Port-Ledger.md:705`, the struck keybinding-registry
...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:7538`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:4649` carries the sweep and `hostMagic.js:765` its missile half. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:3058`, `:2766`). Ledger row `:651`'s residue list is spent - see list 2. |
...
- ~~**`src/scenes/dungeonContext.js:2126`** - the two window seams this
  ...
  carries the same closure against Ledger row `:714`. The two WINDOWS
  ...
  `test/x11b.test.js`. This was also Ledger row `:714`'s adjudication,
  and that row is struck with it.
- **`src/ui/enhancedMenu.js:3116`** - the rest of the keyboard; the
...
- ~~**`src/ui/hudLarge.js:75`** - the docked bar occludes, it does not
  ...
  (`render/renderer.js:2235, :2038, :2972, :3297`) and the 2D passes
...
Section C's table is **247 rows** between `Port-Ledger.md:588` and `:834`
...
route** - plus `:705`, struck at its head but carrying a live PENDING
...
1. **`:623` FaceUVTool's 1,803-UV residual at matched precision** ->
...
2. **`:636` the custom builder's hidden `ResetBonusPool` control** (STRUCK at E2, landing after this list was written: the control is live) ->
...
3. ~~**`:606` remainder: the overlay mouse-UP seam** -> UI arc. A7 shipped
...
4. ~~**`:649` the quest machine** -> `playSound`'s busy-skip, the one
...
5. ~~**`:702` the classic `.SAV` reader** -> the phone path: no zip arm in
   the saves picker, a desktop-first charter call.~~ **CLOSED (OT1, 2026-09-10).**
6. **`:705` (struck, with a live clause) the keybinding registry** ->
   the mouse/advanced and joystick sub-windows, the two of DFU's 65
   game windows the port does not cite.
7. ~~**`:714` the standalone dungeon host has no trade window** -> a
...
8. ~~**`:715` three stale probes**~~ **CLOSED (E-group, 2026-09-02).**
...
9. **`:601` UseItem's unbuilt destinations.** Every arm the row names is
...
10. **`:651` fast-travel residue.** Its three named survivors are spent:
...
11. **`:656` `PatchRegionIndex` legacy-save fix.** Ported verbatim at
    `src/formats/mapsFile.js:107`, with the C# line range cited.
12. **`:700` the magic crafting windows.** The row's FLAGGED residue is
    ...
    clause goes with it.**
13. **`:650` the talk manager.** The row's own tail already says both
...
14. **`:680` the small-residue trio** (biography GP arm, arrow roll
...
- **`FaceUVTool` arithmetic** - JS doubles for C# float mix, `Math.trunc`
  ...
  precision are ledger row `:623`, narrowed at E8 and the only part of

...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:8020`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:5107` carries the sweep and `hostMagic.js:792` its missile half. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:3508`, `:3213`). Ledger row `:659`'s residue list is spent - see list 2. |
...
- **`src/scenes/exterior.js:2260`** - Recall pends here; the anchor
...
- ~~**`src/scenes/exterior.js:1944`** - PX3: this test host mounts no
...
- ~~**`src/scenes/world.js:4427`** - the port's default landing stands
...
regenerated onto the new sites: ~~`exterior.js:1605` -> `:1293`~~
(**CLOSED at TP2**: the Recall interim narrowed to the one arm this host
cannot take, `exterior.js:1966`), ~~`exterior.js:2015` -> `:1685`~~
(**CLOSED at QX1**: this host took the quest bridge, so the pause window's
Quests tab has a machine to read), ~~`world.js:4454` -> `:4477`~~
...
9. **`:609` UseItem's unbuilt destinations.** Every arm the row names is
   ...
   (`ui/nativeInventory.js:842-846`, `scenes/world.js:5161`), the
...
10. **`:659` fast-travel residue.** Its three named survivors are spent:
    ...
    `scenes/world.js:4349` (the arrival clamp anchoring the encounter

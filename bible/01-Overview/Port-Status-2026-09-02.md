...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:5729`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:3252` carries the sweep and `hostMagic.js:608` its missile half. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:2049`, `:1849`). Ledger row `:618`'s residue list is spent - see list 2. |
...
10. **`:618` fast-travel residue.** Its three named survivors are spent:
    ...
    `scenes/world.js:4338` (the arrival clamp anchoring the encounter

...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:7393`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:4568` carries the sweep and `hostMagic.js:711` its missile half. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:3030`, `:2746`). Ledger row `:642`'s residue list is spent - see list 2. |
...
- **`src/scenes/exterior.js:2211`** - Recall pends here; the anchor
...
- ~~**`src/scenes/exterior.js:1900`** - PX3: this test host mounts no
...
- ~~**`src/scenes/world.js:4335`** - the port's default landing stands
...
- ~~**`src/ui/hudLarge.js:75`** - the docked bar occludes, it does not
  ...
  (`render/renderer.js:2124, :2049, :2871, :3194`) and the 2D passes
...
regenerated onto the new sites: ~~`exterior.js:1531` -> `:1271`~~
(**CLOSED at TP2**: the Recall interim narrowed to the one arm this host
cannot take, `exterior.js:1873`), ~~`exterior.js:1923` -> `:1659`~~
(**CLOSED at QX1**: this host took the quest bridge, so the pause window's
Quests tab has a machine to read), ~~`world.js:3821` -> `:4093`~~
...
9. **`:592` UseItem's unbuilt destinations.** Every arm the row names is
   ...
   (`ui/nativeInventory.js:798-802`, `scenes/world.js:4622`), the
...
10. **`:642` fast-travel residue.** Its three named survivors are spent:
    ...
    `scenes/world.js:3837` (the arrival clamp anchoring the encounter

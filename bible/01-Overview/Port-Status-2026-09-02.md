...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:6486`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:3874` carries the sweep and `hostMagic.js:608` its missile half. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:2461`, `:2178`). Ledger row `:631`'s residue list is spent - see list 2. |
...
- ~~**`src/scenes/world.js:3641`** - the port's default landing stands
...
Quests tab has a machine to read), ~~`world.js:3668` -> `:3661`~~
...
9. **`:581` UseItem's unbuilt destinations.** Every arm the row names is
   ...
   (`ui/nativeInventory.js:778-782`, `scenes/world.js:3928`), the
...
10. **`:631` fast-travel residue.** Its three named survivors are spent:
    ...
    `scenes/world.js:3151` (the arrival clamp anchoring the encounter

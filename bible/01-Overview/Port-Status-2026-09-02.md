...
| **formats-mw** | Departure lane, faithful within it | **Departure lane, now consumed** | `clipSweepTimes` has a production caller: `combat/fpArm.js:1714` runs the whole-clip reach sweep the superseded page said had never run in the game. 17 modules / 8,842 lines. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:1992`, `:1760`). Ledger row `:602`'s residue list is spent - see list 2. |
...
- ~~**`src/ui/hudLarge.js:75`** - the docked bar occludes, it does not
  ...
  (`render/renderer.js:1607, :1581, :2081, :2257`) and the 2D passes
...
generation, to the enhanced lane)~~ (**it read 226 and 21 until `:552` was
...
2. **`:587` the custom builder's hidden `ResetBonusPool` control** (STRUCK at E2, landing after this list was written: the control is live) ->
   ...
   `systems/dialogShortcuts.js:194`/`:316` carries `ResetBonusPool` with
...
9. **`:552` UseItem's unbuilt destinations.** Every arm the row names is
   ...
   (`ui/nativeInventory.js:726-730`, `scenes/world.js:3065`), the
...
10. **`:602` fast-travel residue.** Its three named survivors are spent:
    ...
    clock, so the traveled window is not replayed) and `:1968` (the
    `scenes/world.js:4078` (the arrival clamp anchoring the encounter
    clock, so the traveled window is not replayed) and `:1914` (the
...
12. **`:651` the magic crafting windows.** The row's FLAGGED residue is
    ...
    `ui/spellbookWindow.js:135-137`, drawn at `:999`), the icon picker

...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:5058`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:3012` carries the sweep and `hostMagic.js:608` its missile half. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:1988`, `:1756`). Ledger row `:598`'s residue list is spent - see list 2. |
...
- ~~**`src/scenes/world.js:3222`** - the port's default landing stands
...
Quests tab has a machine to read), ~~`world.js:3268` -> `:3080`~~
(**CLOSED at the ship landing**), ~~`worldModes.js:1861` -> `:1717`~~
...
9. **`:548` UseItem's unbuilt destinations.** Every arm the row names is
   ...
   (`ui/nativeInventory.js:723-727`, `scenes/world.js:3061`), the
   (`ui/nativeInventory.js:723-727`, `scenes/world.js:3007`), the
...
10. **`:598` fast-travel residue.** Its three named survivors are spent:
    ...
    `scenes/world.js:4061` (the arrival clamp anchoring the encounter
    clock, so the traveled window is not replayed) and `:1966` (the
    `scenes/world.js:4005` (the arrival clamp anchoring the encounter

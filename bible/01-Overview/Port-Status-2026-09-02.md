...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:5954`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:3455` carries the sweep and `hostMagic.js:608` its missile half. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:2229`, `:1955`). Ledger row `:618`'s residue list is spent - see list 2. |
...
- ~~**`src/scenes/world.js:3500`** - the port's default landing stands
...
Quests tab has a machine to read), ~~`world.js:3507` -> `:3472`~~
...
9. **`:568` UseItem's unbuilt destinations.** Every arm the row names is
   ...
   (`ui/nativeInventory.js:736-740`, `scenes/world.js:3520`), the
...
10. **`:618` fast-travel residue.** Its three named survivors are spent:
    ...
    `scenes/world.js:4597` (the arrival clamp anchoring the encounter
    clock, so the traveled window is not replayed) and `:1969` (the
    `scenes/world.js:4541` (the arrival clamp anchoring the encounter

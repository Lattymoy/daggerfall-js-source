...
the same place - `Port-Ledger.md:712`, the struck keybinding-registry
...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:7602`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:4705` carries the sweep and `hostMagic.js:792` its missile half. |
...
| **characters-voxel** | Departure (deliberate) | **Departure, still editor-only** | 86 designs across seven tables - 42 of the 43 monster mobiles, 19 class, 25 villager. The rig is 1,791 lines across 7 modules and is still gated: `worldModes.js:5368` passes `voxelfolk`, `scenes/interiorContext.js:578` consumes it. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:3095`, `:2803`). Ledger row `:658`'s residue list is spent - see list 2. |
...
- ~~**`src/scenes/dungeonContext.js:2126`** - the two window seams this
  ...
  carries the same closure against Ledger row `:721`. The two WINDOWS
  ...
  `test/x11b.test.js`. This was also Ledger row `:721`'s adjudication,
...
- ~~**`src/scenes/exterior.js:1946`** - PX3: this test host mounts no
  ...
  site. (The `src/scenes/worldModes.js:2299` row below is struck: ROAD-E
...
Section C's table is **247 rows** between `Port-Ledger.md:595` and `:841`
...
route** - plus `:712`, struck at its head but carrying a live PENDING
...
1. **`:630` FaceUVTool's 1,803-UV residual at matched precision** ->
...
2. **`:643` the custom builder's hidden `ResetBonusPool` control** (STRUCK at E2, landing after this list was written: the control is live) ->
...
3. ~~**`:613` remainder: the overlay mouse-UP seam** -> UI arc. A7 shipped
...
4. ~~**`:656` the quest machine** -> `playSound`'s busy-skip, the one
...
5. ~~**`:709` the classic `.SAV` reader** -> the phone path: no zip arm in
   the saves picker, a desktop-first charter call.~~ **CLOSED (OT1, 2026-09-10).**
6. **`:712` (struck, with a live clause) the keybinding registry** ->
   the mouse/advanced and joystick sub-windows, the two of DFU's 65
   game windows the port does not cite.
7. ~~**`:721` the standalone dungeon host has no trade window** -> a
...
8. ~~**`:722` three stale probes**~~ **CLOSED (E-group, 2026-09-02).**
...
9. **`:608` UseItem's unbuilt destinations.** Every arm the row names is
   built: `DrinkPotion` (`systems/useItem.js:205`, `:296-306`),
   `RecordLocationFromMap`/`DiscoverRandomLocation`
   (`ui/nativeInventory.js:841-845`, `scenes/world.js:4759`), the
...
10. **`:658` fast-travel residue.** Its three named survivors are spent:
    ...
    `scenes/world.js:3940` (the arrival clamp anchoring the encounter
...
11. **`:663` `PatchRegionIndex` legacy-save fix.** Ported verbatim at
    `src/formats/mapsFile.js:107`, with the C# line range cited.
12. **`:707` the magic crafting windows.** The row's FLAGGED residue is
    ...
    clause goes with it.**
13. **`:657` the talk manager.** The row's own tail already says both
...
14. **`:687` the small-residue trio** (biography GP arm, arrow roll
...
- **The voxel character engine** (Mac's system, Port-Doctrine). 86
  ...
  by a game host: `worldModes.js:5368` passes `voxelfolk`,
...
- **`FaceUVTool` arithmetic** - JS doubles for C# float mix, `Math.trunc`
  ...
  precision are ledger row `:630`, narrowed at E8 and the only part of

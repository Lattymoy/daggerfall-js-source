...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:4835`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:2852` carries the sweep and `hostMagic.js:601` its missile half. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:1892`, `:1698`). Ledger row `:564`'s residue list is spent - see list 2. |
...
- ~~**`src/scenes/world.js:3101`** - the port's default landing stands
...
Quests tab has a machine to read), ~~`world.js:3147` -> `:2969`~~
...
9. **`:514` UseItem's unbuilt destinations.** Every arm the row names is
   ...
   (`ui/nativeInventory.js:703-707`, `scenes/world.js:2901`), the
   (`ui/nativeInventory.js:703-707`, `scenes/world.js:2847`), the

...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:6655`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:3899` carries the sweep and `hostMagic.js:608` its missile half. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:2474`, `:2191`). Ledger row `:637`'s residue list is spent - see list 2. |
...
- ~~**`src/scenes/dungeonContext.js:2113`** - the two window seams this
...
- **`src/ui/enhancedMenu.js:2992`** - the rest of the keyboard; the
...
- **`src/scenes/exterior.js:2070`** - Recall pends here; the anchor
...
- ~~**`src/scenes/exterior.js:1760`** - PX3: this test host mounts no
...
- ~~**`src/scenes/world.js:3666`** - the port's default landing stands
...
- ~~**`src/ui/hudLarge.js:75`** - the docked bar occludes, it does not
  ...
  (`render/renderer.js:2005, :1932, :2752, :3075`) and the 2D passes
...
regenerated onto the new sites: ~~`exterior.js:1440` -> `:1211`~~
(**CLOSED at TP2**: the Recall interim narrowed to the one arm this host
cannot take, `exterior.js:1782`), ~~`exterior.js:1832` -> `:1520`~~
(**CLOSED at QX1**: this host took the quest bridge, so the pause window's
Quests tab has a machine to read), ~~`world.js:3693` -> `:3686`~~
...
generation, to the enhanced lane)~~ (**it read 226 and 21 until `:562` was
...
9. **`:587` UseItem's unbuilt destinations.** Every arm the row names is
   ...
   (`ui/nativeInventory.js:796-800`, `scenes/world.js:3953`), the
...
10. **`:637` fast-travel residue.** Its three named survivors are spent:
    ...
    `scenes/world.js:3176` (the arrival clamp anchoring the encounter

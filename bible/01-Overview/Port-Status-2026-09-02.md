...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:4808`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:2846` carries the sweep and `hostMagic.js:601` its missile half. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:1886`, `:1692`). Ledger row `:558`'s residue list is spent - see list 2. |
...
- **`src/ui/enhancedMenu.js:2048`** - the rest of the keyboard; the
...
- **`src/scenes/exterior.js:1722`** - Recall pends here; the anchor
...
- ~~**`src/scenes/exterior.js:1611`** - PX3: this test host mounts no
...
- ~~**`src/scenes/world.js:3095`** - the port's default landing stands
...
regenerated onto the new sites: ~~`exterior.js:1280` -> `:1106`~~
(**CLOSED at TP2**: the Recall interim narrowed to the one arm this host
cannot take, `exterior.js:1614`), ~~`exterior.js:1657` -> `:1363`~~
(**CLOSED at QX1**: this host took the quest bridge, so the pause window's
Quests tab has a machine to read), ~~`world.js:3141` -> `:2969`~~
...
9. **`:508` UseItem's unbuilt destinations.** Every arm the row names is
   ...
   (`ui/nativeInventory.js:703-707`, `scenes/world.js:2895`), the
   (`ui/nativeInventory.js:703-707`, `scenes/world.js:2841`), the
...
10. **`:558` fast-travel residue.** Its three named survivors are spent:
    ...
    `scenes/world.js:3860` (the arrival clamp anchoring the encounter
    clock, so the traveled window is not replayed) and `:1966` (the
    `scenes/world.js:3804` (the arrival clamp anchoring the encounter
...
`Port-Ledger.md` section A carries **96 rows, 10 struck - 86 standing

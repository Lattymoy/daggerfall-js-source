...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:6781`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:4003` carries the sweep and `hostMagic.js:711` its missile half. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:2483`, `:2199`). Ledger row `:637`'s residue list is spent - see list 2. |
...
- **`src/scenes/exterior.js:2147`** - Recall pends here; the anchor
...
- ~~**`src/scenes/exterior.js:1837`** - PX3: this test host mounts no
...
- ~~**`src/scenes/world.js:3770`** - the port's default landing stands
...
regenerated onto the new sites: ~~`exterior.js:1517` -> `:1217`~~
(**CLOSED at TP2**: the Recall interim narrowed to the one arm this host
cannot take, `exterior.js:1859`), ~~`exterior.js:1909` -> `:1597`~~
(**CLOSED at QX1**: this host took the quest bridge, so the pause window's
Quests tab has a machine to read), ~~`world.js:3797` -> `:3790`~~
...
9. **`:587` UseItem's unbuilt destinations.** Every arm the row names is
   ...
   (`ui/nativeInventory.js:798-802`, `scenes/world.js:4057`), the
...
10. **`:637` fast-travel residue.** Its three named survivors are spent:
    ...
    `scenes/world.js:3272` (the arrival clamp anchoring the encounter

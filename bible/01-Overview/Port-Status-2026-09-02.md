...
the same place - `Port-Ledger.md:714`, the struck keybinding-registry
...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:7604`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:4707` carries the sweep and `hostMagic.js:792` its missile half. |
...
| **characters-voxel** | Departure (deliberate) | **Departure, still editor-only** | 86 designs across seven tables - 42 of the 43 monster mobiles, 19 class, 25 villager. The rig is 1,791 lines across 7 modules and is still gated: `worldModes.js:5600` passes `voxelfolk`, `scenes/interiorContext.js:578` consumes it. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:3097`, `:2805`). Ledger row `:660`'s residue list is spent - see list 2. |
...
| **ui-enhanced** | Departure lane | **Departure lane, scoped** | `ui/worldPlaque.js:333` puts the skin gate above `ensure()`, so the unscoped `*`/`html`/`body`/`button`/`#app` rules never reach the classic page. 9 modules / 8,940 lines. |
...
- ~~**`src/scenes/dungeonContext.js:2126`** - the two window seams this
  ...
  carries the same closure against Ledger row `:723`. The two WINDOWS
  ...
  `test/x11b.test.js`. This was also Ledger row `:723`'s adjudication,
  and that row is struck with it.
- **`src/ui/enhancedMenu.js:3175`** - the rest of the keyboard; the
...
- **`src/scenes/exterior.js:2258`** - Recall pends here; the anchor
...
- ~~**`src/scenes/exterior.js:1946`** - PX3: this test host mounts no
  ...
  site. (The `src/scenes/worldModes.js:2363` row below is struck: ROAD-E
...
- ~~**`src/scenes/world.js:4422`** - the port's default landing stands
...
- ~~**`src/ui/hudLarge.js:75`** - the docked bar occludes, it does not
  ...
  (`render/renderer.js:2112, :2027, :2839, :3163`) and the 2D passes
...
cannot take, `exterior.js:1968`), ~~`exterior.js:2018` -> `:1684`~~
(**CLOSED at QX1**: this host took the quest bridge, so the pause window's
Quests tab has a machine to read), ~~`world.js:4449` -> `:4472`~~
...
Section C's table is **247 rows** between `Port-Ledger.md:597` and `:843`
...
route** - plus `:714`, struck at its head but carrying a live PENDING
...
1. **`:632` FaceUVTool's 1,803-UV residual at matched precision** ->
...
2. **`:645` the custom builder's hidden `ResetBonusPool` control** (STRUCK at E2, landing after this list was written: the control is live) ->
   ...
   `systems/dialogShortcuts.js:194`/`:322` carries `ResetBonusPool` with
...
3. ~~**`:615` remainder: the overlay mouse-UP seam** -> UI arc. A7 shipped
...
4. ~~**`:658` the quest machine** -> `playSound`'s busy-skip, the one
...
5. ~~**`:711` the classic `.SAV` reader** -> the phone path: no zip arm in
   the saves picker, a desktop-first charter call.~~ **CLOSED (OT1, 2026-09-10).**
6. **`:714` (struck, with a live clause) the keybinding registry** ->
   the mouse/advanced and joystick sub-windows, the two of DFU's 65
   game windows the port does not cite.
7. ~~**`:723` the standalone dungeon host has no trade window** -> a
...
8. ~~**`:724` three stale probes**~~ **CLOSED (E-group, 2026-09-02).**
...
9. **`:610` UseItem's unbuilt destinations.** Every arm the row names is
   built: `DrinkPotion` (`systems/useItem.js:205`, `:296-306`),
   `RecordLocationFromMap`/`DiscoverRandomLocation`
   (`ui/nativeInventory.js:841-845`, `scenes/world.js:4761`), the
...
10. **`:660` fast-travel residue.** Its three named survivors are spent:
    ...
    `scenes/world.js:3942` (the arrival clamp anchoring the encounter
...
11. **`:665` `PatchRegionIndex` legacy-save fix.** Ported verbatim at
    `src/formats/mapsFile.js:107`, with the C# line range cited.
12. **`:709` the magic crafting windows.** The row's FLAGGED residue is
    ...
    clause goes with it.**
13. **`:659` the talk manager.** The row's own tail already says both
...
14. **`:689` the small-residue trio** (biography GP arm, arrow roll
...
- **The voxel character engine** (Mac's system, Port-Doctrine). 86
  ...
  by a game host: `worldModes.js:5600` passes `voxelfolk`,
...
- **`FaceUVTool` arithmetic** - JS doubles for C# float mix, `Math.trunc`
  ...
  precision are ledger row `:632`, narrowed at E8 and the only part of

...
| **formats-game** | Verbatim readers / broken consumer | **Verbatim** | The `ItemRecord` conversions were in the wave; ROAD-A A4 took the classic-import stragglers (building-level MAPSAVE, the native bank record, `LegacyArtifactIndexBitfieldCheck`). Ledger row `:593`'s one residue is the phone path - no zip arm in the saves picker, a desktop-first charter call. |
...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:4138`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:2439` carries the sweep and `hostMagic.js:594` its missile half. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:1519`, `:1667`). Ledger row `:542`'s residue list is spent - see list 2. |
...
- ~~**`src/scenes/dungeonContext.js:1684`** - the two window seams this
  ...
  carries the same closure against Ledger row `:605`. The two WINDOWS
  ...
  `test/x11b.test.js`. This was also Ledger row `:605`'s adjudication,
...
- **`src/scenes/exterior.js:1408`** - Recall pends here; the anchor
...
- ~~**`src/scenes/exterior.js:1298`** - PX3: this test host mounts no
...
- ~~**`src/scenes/world.js:2691`** - the port's default landing stands
...
regenerated onto the new sites: ~~`exterior.js:1077` -> `:1089`~~
(**CLOSED at TP2**: the Recall interim narrowed to the one arm this
host cannot take, `exterior.js:1301`), ~~`exterior.js:1344` ->
`:1344`~~ (**CLOSED at QX1**: this host took the quest bridge, so the
pause window's Quests tab has a machine to read), `world.js:2737` ->
...
Section C's table is **247 rows** between `Port-Ledger.md:479` and
`:725` (`awk '/^\|/ && !/^\|---/'`). **226 are struck.** Of the 21 that
...
route** - plus `:596`, struck at its head but carrying a live PENDING
...
1. **`:514` FaceUVTool's 1,803-UV residual at matched precision** ->
...
2. **`:527` the custom builder's hidden `ResetBonusPool` control** (STRUCK at E2, landing after this list was written: the control is live) ->
...
3. **`:497` remainder: the overlay mouse-UP seam** -> UI arc. A7 shipped
...
4. ~~**`:540` the quest machine** -> `playSound`'s busy-skip, the one
...
5. **`:593` the classic `.SAV` reader** -> the phone path: no zip arm in
   the saves picker, a desktop-first charter call.
6. **`:596` (struck, with a live clause) the keybinding registry** ->
   the mouse/advanced and joystick sub-windows, the two of DFU's 65
   game windows the port does not cite.
7. ~~**`:605` the standalone dungeon host has no trade window** -> a
...
8. ~~**`:606` three stale probes**~~ **CLOSED (E-group, 2026-09-02).**
...
9. **`:492` UseItem's unbuilt destinations.** Every arm the row names is
   ...
   (`ui/nativeInventory.js:670-674`, `scenes/world.js:2488`), the
   (`ui/nativeInventory.js:670-674`, `scenes/world.js:2434`), the
...
10. **`:542` fast-travel residue.** Its three named survivors are spent:
    ...
    `scenes/world.js:3406` (the arrival clamp anchoring the encounter
    clock, so the traveled window is not replayed) and `:1964` (the
    `scenes/world.js:3359` (the arrival clamp anchoring the encounter
...
11. **`:547` `PatchRegionIndex` legacy-save fix.** Ported verbatim at
    `src/formats/mapsFile.js:104`, with the C# line range cited.
12. **`:591` the magic crafting windows.** The row's FLAGGED residue is
    ...
    clause goes with it.**
13. **`:541` the talk manager.** The row's own tail already says both
...
14. **`:571` the small-residue trio** (biography GP arm, arrow roll
...
- **`FaceUVTool` arithmetic** - JS doubles for C# float mix, `Math.trunc`
  ...
  precision are ledger row `:514`, narrowed at E8 and the only part of

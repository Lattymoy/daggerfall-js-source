...
the same place - `Port-Ledger.md:676`, the struck keybinding-registry
...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:6131`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:3612` carries the sweep and `hostMagic.js:608` its missile half. |
| **scenes-modes** | Solid, pause parity broken | **Near-1:1** | ROAD-B B1 put `UserInterfaceManager`'s real stack under this host's slot (`ui/windowStack.js`, 295 lines, imported at `worldModes.js:76`). See "the pause primitive" below - the stack exists, its `paused()` member has no reader. |
...
| **characters-voxel** | Departure (deliberate) | **Departure, still editor-only** | 86 designs across seven tables - 42 of the 43 monster mobiles, 19 class, 25 villager. The rig is 1,791 lines across 7 modules and is still gated: `worldModes.js:4369` passes `voxelfolk`, `scenes/interiorContext.js:503` consumes it. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:2334`, `:2051`). Ledger row `:622`'s residue list is spent - see list 2. |
...
that own overlay slots - `worldModes.js:76` and `dungeonContext.js:39` -
and `world.js:236` reaches it by mounting `worldModes`. But `grep -rn
...
- ~~**`src/scenes/dungeonContext.js:1933`** - the two window seams this
  ...
  carries the same closure against Ledger row `:685`. The two WINDOWS
  ...
  `test/x11b.test.js`. This was also Ledger row `:685`'s adjudication,
...
- **`src/scenes/exterior.js:1955`** - Recall pends here; the anchor
...
- ~~**`src/scenes/exterior.js:1696`** - PX3: this test host mounts no
  ...
  site. (The `src/scenes/worldModes.js:1992` row below is struck: ROAD-E
...
- ~~**`src/scenes/world.js:3547`** - the port's default landing stands
...
- ~~**`src/characters/enemyCasting.js:91`** - now exactly one term
  ...
  second.*
- ~~**`src/scenes/worldModes.js:1907`** - above ground only:
...
- ~~**`src/ui/hudLarge.js:75`** - the docked bar occludes, it does not
  ...
  (`render/renderer.js:1751, :1685, :2498, :2642`) and the 2D passes
...
regenerated onto the new sites: ~~`exterior.js:1376` -> `:1198`~~
(**CLOSED at TP2**: the Recall interim narrowed to the one arm this host
cannot take, `exterior.js:1718`), ~~`exterior.js:1768` -> `:1456`~~
(**CLOSED at QX1**: this host took the quest bridge, so the pause window's
Quests tab has a machine to read), ~~`world.js:3574` -> `:3570`~~
(**CLOSED at the ship landing**), ~~`worldModes.js:1870` -> `:1763`~~
...
- ~~**`src/scenes/worldModes.js:1977`** - above ground only: the GUILD
...
Section C's table is **247 rows** between `Port-Ledger.md:559` and `:805`
...
route** - plus `:676`, struck at its head but carrying a live PENDING
...
1. **`:594` FaceUVTool's 1,803-UV residual at matched precision** ->
...
2. **`:607` the custom builder's hidden `ResetBonusPool` control** (STRUCK at E2, landing after this list was written: the control is live) ->
...
3. ~~**`:577` remainder: the overlay mouse-UP seam** -> UI arc. A7 shipped
...
4. ~~**`:620` the quest machine** -> `playSound`'s busy-skip, the one
...
5. ~~**`:673` the classic `.SAV` reader** -> the phone path: no zip arm in
   the saves picker, a desktop-first charter call.~~ **CLOSED (OT1, 2026-09-10).**
6. **`:676` (struck, with a live clause) the keybinding registry** ->
   the mouse/advanced and joystick sub-windows, the two of DFU's 65
   game windows the port does not cite.
7. ~~**`:685` the standalone dungeon host has no trade window** -> a
...
8. ~~**`:686` three stale probes**~~ **CLOSED (E-group, 2026-09-02).**
...
9. **`:572` UseItem's unbuilt destinations.** Every arm the row names is
   ...
   (`ui/nativeInventory.js:736-740`, `scenes/world.js:3682`), the
...
10. **`:622` fast-travel residue.** Its three named survivors are spent:
    ...
    `scenes/world.js:4745` (the arrival clamp anchoring the encounter
...
11. **`:627` `PatchRegionIndex` legacy-save fix.** Ported verbatim at
    `src/formats/mapsFile.js:104`, with the C# line range cited.
12. **`:671` the magic crafting windows.** The row's FLAGGED residue is
    ...
    clause goes with it.**
13. **`:621` the talk manager.** The row's own tail already says both
...
14. **`:651` the small-residue trio** (biography GP arm, arrow roll
...
- **The voxel character engine** (Mac's system, Port-Doctrine). 86
  ...
  by a game host: `worldModes.js:4369` passes `voxelfolk`,
...
- **`FaceUVTool` arithmetic** - JS doubles for C# float mix, `Math.trunc`
  ...
  precision are ledger row `:594`, narrowed at E8 and the only part of

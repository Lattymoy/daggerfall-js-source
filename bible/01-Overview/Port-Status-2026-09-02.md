...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:4866`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:2878` carries the sweep and `hostMagic.js:601` its missile half. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:1918`, `:1709`). Ledger row `:579`'s residue list is spent - see list 2. |
...
- **`src/ui/enhancedMenu.js:2273`** - the rest of the keyboard; the
...
- ~~**`src/scenes/world.js:3124`** - the port's default landing stands
...
Quests tab has a machine to read), ~~`world.js:3170` -> `:2976`~~
...
9. **`:529` UseItem's unbuilt destinations.** Every arm the row names is
   ...
   (`ui/nativeInventory.js:703-707`, `scenes/world.js:2927`), the
   (`ui/nativeInventory.js:703-707`, `scenes/world.js:2873`), the
...
10. **`:579` fast-travel residue.** Its three named survivors are spent:
    ...
    `scenes/world.js:3892` (the arrival clamp anchoring the encounter
    clock, so the traveled window is not replayed) and `:1966` (the
    `scenes/world.js:3836` (the arrival clamp anchoring the encounter

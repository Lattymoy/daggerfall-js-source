...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:6937`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:4176` carries the sweep and `hostMagic.js:662` its missile half. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:2747`, `:2464`). Ledger row `:637`'s residue list is spent - see list 2. |
...
and `world.js:261` reaches it by mounting `worldModes`. But `grep -rn
...
- **`src/ui/enhancedMenu.js:2995`** - the rest of the keyboard; the
...
- **`src/scenes/exterior.js:2114`** - Recall pends here; the anchor
...
- ~~**`src/scenes/world.js:3668`** - the port's default landing stands
...
Quests tab has a machine to read), ~~`world.js:3695` -> `:3688`~~
...
9. **`:587` UseItem's unbuilt destinations.** Every arm the row names is
   ...
   (`ui/nativeInventory.js:796-800`, `scenes/world.js:4230`), the
...
10. **`:637` fast-travel residue.** Its three named survivors are spent:
    ...
    `scenes/world.js:3449` (the arrival clamp anchoring the encounter

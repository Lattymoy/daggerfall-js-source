...
the same place - `Port-Ledger.md:610`, the struck keybinding-registry
...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:4818`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:2856` carries the sweep and `hostMagic.js:601` its missile half. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:1896`, `:1702`). Ledger row `:556`'s residue list is spent - see list 2. |
...
- ~~**`src/scenes/dungeonContext.js:1842`** - the two window seams this
  ...
  carries the same closure against Ledger row `:619`. The two WINDOWS
  ...
  `test/x11b.test.js`. This was also Ledger row `:619`'s adjudication,
...
- **`src/scenes/exterior.js:1729`** - Recall pends here; the anchor
...
- ~~**`src/scenes/exterior.js:1618`** - PX3: this test host mounts no
...
- **`src/systems/inputActions.js:543`** - STILL FLAGGED: axes and
  ...
  sub-windows are recorded pending at `Port-Ledger.md:610`. The flag's
...
- ~~**`src/ui/hudLarge.js:75`** - the docked bar occludes, it does not
  ...
  (`render/renderer.js:1277, :1245, :1741, :1916`) and the 2D passes
...
regenerated onto the new sites: ~~`exterior.js:1287` -> `:1105`~~
(**CLOSED at TP2**: the Recall interim narrowed to the one arm this host
cannot take, `exterior.js:1621`), ~~`exterior.js:1664` -> `:1362`~~
...
Section C's table is **247 rows** between `Port-Ledger.md:493` and `:739`
...
route** - plus `:610`, struck at its head but carrying a live PENDING
...
1. **`:528` FaceUVTool's 1,803-UV residual at matched precision** ->
...
2. **`:541` the custom builder's hidden `ResetBonusPool` control** (STRUCK at E2, landing after this list was written: the control is live) ->
...
3. **`:511` remainder: the overlay mouse-UP seam** -> UI arc. A7 shipped
...
4. ~~**`:554` the quest machine** -> `playSound`'s busy-skip, the one
...
5. ~~**`:607` the classic `.SAV` reader** -> the phone path: no zip arm in
   the saves picker, a desktop-first charter call.~~ **CLOSED (OT1, 2026-09-10).**
6. **`:610` (struck, with a live clause) the keybinding registry** ->
   the mouse/advanced and joystick sub-windows, the two of DFU's 65
   game windows the port does not cite.
7. ~~**`:619` the standalone dungeon host has no trade window** -> a
...
8. ~~**`:620` three stale probes**~~ **CLOSED (E-group, 2026-09-02).**
...
9. **`:506` UseItem's unbuilt destinations.** Every arm the row names is
...
10. **`:556` fast-travel residue.** Its three named survivors are spent:
    ...
    `scenes/world.js:3870` (the arrival clamp anchoring the encounter
    clock, so the traveled window is not replayed) and `:1966` (the
    `scenes/world.js:3814` (the arrival clamp anchoring the encounter
...
11. **`:561` `PatchRegionIndex` legacy-save fix.** Ported verbatim at
    `src/formats/mapsFile.js:104`, with the C# line range cited.
12. **`:605` the magic crafting windows.** The row's FLAGGED residue is
    ...
    clause goes with it.**
13. **`:555` the talk manager.** The row's own tail already says both
...
14. **`:585` the small-residue trio** (biography GP arm, arrow roll
...
- **`FaceUVTool` arithmetic** - JS doubles for C# float mix, `Math.trunc`
  ...
  precision are ledger row `:528`, narrowed at E8 and the only part of

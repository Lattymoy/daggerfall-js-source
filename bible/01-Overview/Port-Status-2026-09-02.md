...
the same place - `Port-Ledger.md:672`, the struck keybinding-registry
...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:5923`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:3424` carries the sweep and `hostMagic.js:608` its missile half. |
...
| **characters-voxel** | Departure (deliberate) | **Departure, still editor-only** | 86 designs across seven tables - 42 of the 43 monster mobiles, 19 class, 25 villager. The rig is 1,791 lines across 7 modules and is still gated: `worldModes.js:4319` passes `voxelfolk`, `scenes/interiorContext.js:503` consumes it. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:2198`, `:1924`). Ledger row `:618`'s residue list is spent - see list 2. |
...
- ~~**`src/scenes/dungeonContext.js:1921`** - the two window seams this
  ...
  carries the same closure against Ledger row `:681`. The two WINDOWS
  ...
  `test/x11b.test.js`. This was also Ledger row `:681`'s adjudication,
...
- **`src/scenes/exterior.js:1922`** - Recall pends here; the anchor
...
- ~~**`src/scenes/exterior.js:1680`** - PX3: this test host mounts no
  ...
  site. (The `src/scenes/worldModes.js:1942` row below is struck: ROAD-E
...
- ~~**`src/scenes/world.js:3448`** - the port's default landing stands
...
regenerated onto the new sites: ~~`exterior.js:1351` -> `:1188`~~
(**CLOSED at TP2**: the Recall interim narrowed to the one arm this host
cannot take, `exterior.js:1693`), ~~`exterior.js:1737` -> `:1440`~~
(**CLOSED at QX1**: this host took the quest bridge, so the pause window's
Quests tab has a machine to read), ~~`world.js:3475` -> `:3441`~~
...
Section C's table is **247 rows** between `Port-Ledger.md:555` and `:801`
...
route** - plus `:672`, struck at its head but carrying a live PENDING
...
1. **`:590` FaceUVTool's 1,803-UV residual at matched precision** ->
...
2. **`:603` the custom builder's hidden `ResetBonusPool` control** (STRUCK at E2, landing after this list was written: the control is live) ->
...
3. ~~**`:573` remainder: the overlay mouse-UP seam** -> UI arc. A7 shipped
...
4. ~~**`:616` the quest machine** -> `playSound`'s busy-skip, the one
...
5. ~~**`:669` the classic `.SAV` reader** -> the phone path: no zip arm in
   the saves picker, a desktop-first charter call.~~ **CLOSED (OT1, 2026-09-10).**
6. **`:672` (struck, with a live clause) the keybinding registry** ->
   the mouse/advanced and joystick sub-windows, the two of DFU's 65
   game windows the port does not cite.
7. ~~**`:681` the standalone dungeon host has no trade window** -> a
...
8. ~~**`:682` three stale probes**~~ **CLOSED (E-group, 2026-09-02).**
...
9. **`:568` UseItem's unbuilt destinations.** Every arm the row names is
   built: `DrinkPotion` (`systems/useItem.js:197`, `:275-285`),
   `RecordLocationFromMap`/`DiscoverRandomLocation`
   (`ui/nativeInventory.js:736-740`, `scenes/world.js:3496`), the
...
10. **`:618` fast-travel residue.** Its three named survivors are spent:
    ...
    `scenes/world.js:4510` (the arrival clamp anchoring the encounter
...
11. **`:623` `PatchRegionIndex` legacy-save fix.** Ported verbatim at
    `src/formats/mapsFile.js:104`, with the C# line range cited.
12. **`:667` the magic crafting windows.** The row's FLAGGED residue is
    ...
    clause goes with it.**
13. **`:617` the talk manager.** The row's own tail already says both
...
14. **`:647` the small-residue trio** (biography GP arm, arrow roll
...
- **The voxel character engine** (Mac's system, Port-Doctrine). 86
  ...
  by a game host: `worldModes.js:4319` passes `voxelfolk`,
...
- **`FaceUVTool` arithmetic** - JS doubles for C# float mix, `Math.trunc`
  ...
  precision are ledger row `:590`, narrowed at E8 and the only part of

...
the same place - `Port-Ledger.md:679`, the struck keybinding-registry
...
| **formats-game** | Verbatim readers / broken consumer | **Verbatim** | The `ItemRecord` conversions were in the wave; ROAD-A A4 took the classic-import stragglers (building-level MAPSAVE, the native bank record, `LegacyArtifactIndexBitfieldCheck`). ~~Ledger row `:676`'s one residue is the phone path - no zip arm in the saves picker, a desktop-first charter call.~~ **CLOSED (OT1, 2026-09-10): the saves picker took the ARENA2 door's zip arm, and the row is struck whole.** |
| **formats-mw** | Departure lane, faithful within it | **Departure lane, now consumed** | `clipSweepTimes` has a production caller: `combat/fpArm.js:1755` runs the whole-clip reach sweep the superseded page said had never run in the game. 17 modules / 8,842 lines. |
...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:6235`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:3690` carries the sweep and `hostMagic.js:608` its missile half. |
...
| **characters-voxel** | Departure (deliberate) | **Departure, still editor-only** | 86 designs across seven tables - 42 of the 43 monster mobiles, 19 class, 25 villager. The rig is 1,791 lines across 7 modules and is still gated: `worldModes.js:4393` passes `voxelfolk`, `scenes/interiorContext.js:503` consumes it. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:2396`, `:2113`). Ledger row `:625`'s residue list is spent - see list 2. |
...
| **ui-windows-a** | Rect parity excellent / two crash doors | **Verbatim** | ROAD-D D2 shipped both scroll thumbs (`chargenArt.js:777` over `RECTS.pickScroll`, `spellbookWindow.js:945` over the 7-wide rail). |
...
and `world.js:239` reaches it by mounting `worldModes`. But `grep -rn
...
- ~~**`src/scenes/dungeonContext.js:1974`** - the two window seams this
  ...
  carries the same closure against Ledger row `:688`. The two WINDOWS
  ...
  `test/x11b.test.js`. This was also Ledger row `:688`'s adjudication,
  and that row is struck with it.
- **`src/ui/enhancedMenu.js:2522`** - the rest of the keyboard; the
...
- ~~**`src/scenes/exterior.js:1744`** - PX3: this test host mounts no
  ...
  site. (The `src/scenes/worldModes.js:2005` row below is struck: ROAD-E
...
- ~~**`src/systems/inputActions.js:682`** - STILL FLAGGED, NARROWED (GP1,
...
- ~~**`src/ui/hudLarge.js:75`** - the docked bar occludes, it does not
  ...
  (`render/renderer.js:1902, :1836, :2649, :2941`) and the 2D passes
...
- **`src/scenes/dungeonContext.js:1113`** - the dungeon's non-layout foe
...
Quests tab has a machine to read), ~~`world.js:3652` -> `:3648`~~
...
Section C's table is **247 rows** between `Port-Ledger.md:562` and `:808`
...
route** - plus `:679`, struck at its head but carrying a live PENDING
...
1. **`:597` FaceUVTool's 1,803-UV residual at matched precision** ->
...
2. **`:610` the custom builder's hidden `ResetBonusPool` control** (STRUCK at E2, landing after this list was written: the control is live) ->
...
3. ~~**`:580` remainder: the overlay mouse-UP seam** -> UI arc. A7 shipped
...
4. ~~**`:623` the quest machine** -> `playSound`'s busy-skip, the one
...
5. ~~**`:676` the classic `.SAV` reader** -> the phone path: no zip arm in
   the saves picker, a desktop-first charter call.~~ **CLOSED (OT1, 2026-09-10).**
6. **`:679` (struck, with a live clause) the keybinding registry** ->
   the mouse/advanced and joystick sub-windows, the two of DFU's 65
   game windows the port does not cite.
7. ~~**`:688` the standalone dungeon host has no trade window** -> a
...
8. ~~**`:689` three stale probes**~~ **CLOSED (E-group, 2026-09-02).**
...
9. **`:575` UseItem's unbuilt destinations.** Every arm the row names is
   built: `DrinkPotion` (`systems/useItem.js:197`, `:275-285`),
   `RecordLocationFromMap`/`DiscoverRandomLocation`
   (`ui/nativeInventory.js:779-783`, `scenes/world.js:3775`), the
...
10. **`:625` fast-travel residue.** Its three named survivors are spent:
    ...
    `scenes/world.js:4840` (the arrival clamp anchoring the encounter
...
11. **`:630` `PatchRegionIndex` legacy-save fix.** Ported verbatim at
    `src/formats/mapsFile.js:104`, with the C# line range cited.
12. **`:674` the magic crafting windows.** The row's FLAGGED residue is
    ...
    clause goes with it.**
13. **`:624` the talk manager.** The row's own tail already says both
...
14. **`:654` the small-residue trio** (biography GP arm, arrow roll
...
- **The voxel character engine** (Mac's system, Port-Doctrine). 86
  ...
  by a game host: `worldModes.js:4393` passes `voxelfolk`,
...
- **`FaceUVTool` arithmetic** - JS doubles for C# float mix, `Math.trunc`
  ...
  precision are ledger row `:597`, narrowed at E8 and the only part of

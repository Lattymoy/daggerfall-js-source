...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:2456`, `:2173`). Ledger row `:631`'s residue list is spent - see list 2. |
...
| **ui-enhanced** | Departure lane | **Departure lane, scoped** | `ui/worldPlaque.js:228` puts the skin gate above `ensure()`, so the unscoped `*`/`html`/`body`/`button`/`#app` rules never reach the classic page. 9 modules / 8,940 lines. |
...
and `world.js:252` reaches it by mounting `worldModes`. But `grep -rn
...
- **`src/scenes/dungeonContext.js:1136`** - the dungeon's non-layout foe
...
10. **`:631` fast-travel residue.** Its three named survivors are spent:
    ...
    `scenes/world.js:3146` (the arrival clamp anchoring the encounter

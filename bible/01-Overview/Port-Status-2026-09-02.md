...
the same place - `Port-Ledger.md:691`, the struck keybinding-registry
...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:6666`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:3905` carries the sweep and `hostMagic.js:711` its missile half. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:2472`, `:2189`). Ledger row `:637`'s residue list is spent - see list 2. |
...
and `world.js:255` reaches it by mounting `worldModes`. But `grep -rn
...
- ~~**`src/scenes/dungeonContext.js:2114`** - the two window seams this
  ...
  carries the same closure against Ledger row `:700`. The two WINDOWS
  ...
  `test/x11b.test.js`. This was also Ledger row `:700`'s adjudication,
...
- **`src/scenes/exterior.js:2071`** - Recall pends here; the anchor
...
- ~~**`src/scenes/world.js:3672`** - the port's default landing stands
...
regenerated onto the new sites: ~~`exterior.js:1441` -> `:1212`~~
...
Quests tab has a machine to read), ~~`world.js:3699` -> `:3692`~~
...
Section C's table is **247 rows** between `Port-Ledger.md:574` and `:820`
...
route** - plus `:691`, struck at its head but carrying a live PENDING
...
1. **`:609` FaceUVTool's 1,803-UV residual at matched precision** ->
...
2. **`:622` the custom builder's hidden `ResetBonusPool` control** (STRUCK at E2, landing after this list was written: the control is live) ->
...
3. ~~**`:592` remainder: the overlay mouse-UP seam** -> UI arc. A7 shipped
...
4. ~~**`:635` the quest machine** -> `playSound`'s busy-skip, the one
...
5. ~~**`:688` the classic `.SAV` reader** -> the phone path: no zip arm in
   the saves picker, a desktop-first charter call.~~ **CLOSED (OT1, 2026-09-10).**
6. **`:691` (struck, with a live clause) the keybinding registry** ->
   the mouse/advanced and joystick sub-windows, the two of DFU's 65
   game windows the port does not cite.
7. ~~**`:700` the standalone dungeon host has no trade window** -> a
...
8. ~~**`:701` three stale probes**~~ **CLOSED (E-group, 2026-09-02).**
...
9. **`:587` UseItem's unbuilt destinations.** Every arm the row names is
   built: `DrinkPotion` (`systems/useItem.js:197`, `:275-285`),
   `RecordLocationFromMap`/`DiscoverRandomLocation`
   (`ui/nativeInventory.js:796-800`, `scenes/world.js:3959`), the
...
10. **`:637` fast-travel residue.** Its three named survivors are spent:
...
11. **`:642` `PatchRegionIndex` legacy-save fix.** Ported verbatim at
    `src/formats/mapsFile.js:104`, with the C# line range cited.
12. **`:686` the magic crafting windows.** The row's FLAGGED residue is
    ...
    clause goes with it.**
13. **`:636` the talk manager.** The row's own tail already says both
...
14. **`:666` the small-residue trio** (biography GP arm, arrow roll
    ...
    `BIOG*.TXT` files; the faceUV guard rides row `:609`.
...
- **`FaceUVTool` arithmetic** - JS doubles for C# float mix, `Math.trunc`
  ...
  precision are ledger row `:609`, narrowed at E8 and the only part of

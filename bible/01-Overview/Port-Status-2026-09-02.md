...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:4778`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:2842` carries the sweep and `hostMagic.js:601` its missile half. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:1882`, `:1688`). Ledger row `:552`'s residue list is spent - see list 2. |
...
and `world.js:184` reaches it by mounting `worldModes`. But `grep -rn
...
- ~~**`src/scenes/world.js:3091`** - the port's default landing stands
...
Quests tab has a machine to read), ~~`world.js:3137` -> `:2954`~~
...
9. **`:502` UseItem's unbuilt destinations.** Every arm the row names is
   ...
   (`ui/nativeInventory.js:703-707`, `scenes/world.js:2891`), the
   (`ui/nativeInventory.js:703-707`, `scenes/world.js:2837`), the
...
10. **`:552` fast-travel residue.** Its three named survivors are spent:
    ...
    `scenes/world.js:3851` (the arrival clamp anchoring the encounter

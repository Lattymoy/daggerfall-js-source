...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:8017`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:5104` carries the sweep and `hostMagic.js:792` its missile half. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:3505`, `:3210`). Ledger row `:659`'s residue list is spent - see list 2. |
...
9. **`:609` UseItem's unbuilt destinations.** Every arm the row names is
   ...
   (`ui/nativeInventory.js:842-846`, `scenes/world.js:5158`), the
...
10. **`:659` fast-travel residue.** Its three named survivors are spent:
    ...
    `scenes/world.js:4346` (the arrival clamp anchoring the encounter

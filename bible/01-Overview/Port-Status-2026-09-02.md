...
the same place - `Port-Ledger.md:580`, the struck keybinding-registry
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:1599`, `:1667`). Ledger row 511's residue list is spent - see list 2. |
...
- **`src/systems/inputActions.js:465`** - STILL FLAGGED: axes and
  ...
  sub-windows are recorded pending at `Port-Ledger.md:580`. The flag's
...
Section C's table is **247 rows** between `Port-Ledger.md:466` and
`:712` (`awk '/^\|/ && !/^\|---/'`). **226 are struck.** Of the 21 that
...
route** - plus `:583`, struck at its head but carrying a live PENDING
...
1. **`:501` FaceUVTool's 1,803-UV residual at matched precision** ->
...
2. **`:514` the custom builder's hidden `ResetBonusPool` control** (STRUCK at E2, landing after this list was written: the control is live) ->
...
3. **`:484` remainder: the overlay mouse-UP seam** -> UI arc. A7 shipped
...
4. ~~**`:527` the quest machine** -> `playSound`'s busy-skip, the one
...
5. **`:580` the classic `.SAV` reader** -> the phone path: no zip arm in
   the saves picker, a desktop-first charter call.
6. **`:583` (struck, with a live clause) the keybinding registry** ->
   the mouse/advanced and joystick sub-windows, the two of DFU's 65
   game windows the port does not cite.
7. ~~**`:592` the standalone dungeon host has no trade window** -> a
...
8. ~~**`:593` three stale probes**~~ **CLOSED (E-group, 2026-09-02).**
...
9. **`:479` UseItem's unbuilt destinations.** Every arm the row names is
   built: `DrinkPotion` (`systems/useItem.js:167`, `:245-255`),
   `RecordLocationFromMap`/`DiscoverRandomLocation`
   (`ui/nativeInventory.js:633-637`, `scenes/world.js:2404`), the
...
10. **`:529` fast-travel residue.** Its three named survivors are spent:
...
12. **`:578` the magic crafting windows.** The row's FLAGGED residue is
    ...
    clause goes with it.**
13. **`:528` the talk manager.** The row's own tail already says both
...
14. **`:558` the small-residue trio** (biography GP arm, arrow roll
    ...
    `BIOG*.TXT` files; the faceUV guard rides row `:501`.
...
- **`FaceUVTool` arithmetic** - JS doubles for C# float mix, `Math.trunc`
  ...
  precision are ledger row `:501`, narrowed at E8 and the only part of

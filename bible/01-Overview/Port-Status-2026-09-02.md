...
the same place - `Port-Ledger.md:715`, the struck keybinding-registry
...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:7786`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:4810` carries the sweep and `hostMagic.js:792` its missile half. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:3213`, `:2910`). Ledger row `:661`'s residue list is spent - see list 2. |
...
- ~~**`src/scenes/dungeonContext.js:2126`** - the two window seams this
  ...
  carries the same closure against Ledger row `:724`. The two WINDOWS
  ...
  `test/x11b.test.js`. This was also Ledger row `:724`'s adjudication,
...
- **`src/scenes/dungeonContext.js:1219`** - the dungeon's non-layout foe
...
Section C's table is **247 rows** between `Port-Ledger.md:598` and `:844`
...
route** - plus `:715`, struck at its head but carrying a live PENDING
...
1. **`:633` FaceUVTool's 1,803-UV residual at matched precision** ->
...
2. **`:646` the custom builder's hidden `ResetBonusPool` control** (STRUCK at E2, landing after this list was written: the control is live) ->
...
3. ~~**`:616` remainder: the overlay mouse-UP seam** -> UI arc. A7 shipped
...
4. ~~**`:659` the quest machine** -> `playSound`'s busy-skip, the one
...
5. ~~**`:712` the classic `.SAV` reader** -> the phone path: no zip arm in
   the saves picker, a desktop-first charter call.~~ **CLOSED (OT1, 2026-09-10).**
6. **`:715` (struck, with a live clause) the keybinding registry** ->
   the mouse/advanced and joystick sub-windows, the two of DFU's 65
   game windows the port does not cite.
7. ~~**`:724` the standalone dungeon host has no trade window** -> a
...
8. ~~**`:725` three stale probes**~~ **CLOSED (E-group, 2026-09-02).**
...
9. **`:611` UseItem's unbuilt destinations.** Every arm the row names is
   ...
   (`ui/nativeInventory.js:834-838`, `scenes/world.js:4864`), the
...
10. **`:661` fast-travel residue.** Its three named survivors are spent:
    ...
    `scenes/world.js:4049` (the arrival clamp anchoring the encounter
...
11. **`:666` `PatchRegionIndex` legacy-save fix.** Ported verbatim at
    `src/formats/mapsFile.js:107`, with the C# line range cited.
12. **`:710` the magic crafting windows.** The row's FLAGGED residue is
    ...
    clause goes with it.**
13. **`:660` the talk manager.** The row's own tail already says both
...
14. **`:690` the small-residue trio** (biography GP arm, arrow roll
...
- **`FaceUVTool` arithmetic** - JS doubles for C# float mix, `Math.trunc`
  ...
  precision are ledger row `:633`, narrowed at E8 and the only part of

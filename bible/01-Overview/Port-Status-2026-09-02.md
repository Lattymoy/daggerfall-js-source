...
the same place - `Port-Ledger.md:681`, the struck keybinding-registry
...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:6263`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:3697` carries the sweep and `hostMagic.js:608` its missile half. |
...
| **characters-voxel** | Departure (deliberate) | **Departure, still editor-only** | 86 designs across seven tables - 42 of the 43 monster mobiles, 19 class, 25 villager. The rig is 1,791 lines across 7 modules and is still gated: `worldModes.js:4395` passes `voxelfolk`, `scenes/interiorContext.js:503` consumes it. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:2403`, `:2120`). Ledger row `:627`'s residue list is spent - see list 2. |
...
that own overlay slots - `worldModes.js:78` and `dungeonContext.js:43` -
and `world.js:245` reaches it by mounting `worldModes`. But `grep -rn
...
- ~~**`src/scenes/dungeonContext.js:1991`** - the two window seams this
  ...
  carries the same closure against Ledger row `:690`. The two WINDOWS
  ...
  `test/x11b.test.js`. This was also Ledger row `:690`'s adjudication,
...
- **`src/scenes/exterior.js:1967`** - Recall pends here; the anchor
...
- ~~**`src/scenes/exterior.js:1746`** - PX3: this test host mounts no
  ...
  site. (The `src/scenes/worldModes.js:2007` row below is struck: ROAD-E
...
- ~~**`src/scenes/world.js:3630`** - the port's default landing stands
...
- ~~**`src/characters/enemyCasting.js:91`** - now exactly one term
  ...
  second.*
- ~~**`src/scenes/worldModes.js:1920`** - above ground only:
...
regenerated onto the new sites: ~~`exterior.js:1426` -> `:1203`~~
(**CLOSED at TP2**: the Recall interim narrowed to the one arm this host
cannot take, `exterior.js:1768`), ~~`exterior.js:1818` -> `:1506`~~
(**CLOSED at QX1**: this host took the quest bridge, so the pause window's
Quests tab has a machine to read), ~~`world.js:3657` -> `:3654`~~
(**CLOSED at the ship landing**), ~~`worldModes.js:1883` -> `:1770`~~
...
- ~~**`src/scenes/worldModes.js:1990`** - above ground only: the GUILD
...
Section C's table is **247 rows** between `Port-Ledger.md:564` and `:810`
...
generation, to the enhanced lane)~~ (**it read 226 and 21 until `:556` was
...
route** - plus `:681`, struck at its head but carrying a live PENDING
...
1. **`:599` FaceUVTool's 1,803-UV residual at matched precision** ->
...
2. **`:612` the custom builder's hidden `ResetBonusPool` control** (STRUCK at E2, landing after this list was written: the control is live) ->
   ...
   `systems/dialogShortcuts.js:194`/`:317` carries `ResetBonusPool` with
...
3. ~~**`:582` remainder: the overlay mouse-UP seam** -> UI arc. A7 shipped
...
4. ~~**`:625` the quest machine** -> `playSound`'s busy-skip, the one
...
5. ~~**`:678` the classic `.SAV` reader** -> the phone path: no zip arm in
   the saves picker, a desktop-first charter call.~~ **CLOSED (OT1, 2026-09-10).**
6. **`:681` (struck, with a live clause) the keybinding registry** ->
   the mouse/advanced and joystick sub-windows, the two of DFU's 65
   game windows the port does not cite.
7. ~~**`:690` the standalone dungeon host has no trade window** -> a
...
8. ~~**`:691` three stale probes**~~ **CLOSED (E-group, 2026-09-02).**
...
9. **`:577` UseItem's unbuilt destinations.** Every arm the row names is
   built: `DrinkPotion` (`systems/useItem.js:197`, `:275-285`),
   `RecordLocationFromMap`/`DiscoverRandomLocation`
   (`ui/nativeInventory.js:778-782`, `scenes/world.js:3782`), the
...
10. **`:627` fast-travel residue.** Its three named survivors are spent:
    ...
    `scenes/world.js:4847` (the arrival clamp anchoring the encounter
...
11. **`:632` `PatchRegionIndex` legacy-save fix.** Ported verbatim at
    `src/formats/mapsFile.js:104`, with the C# line range cited.
12. **`:676` the magic crafting windows.** The row's FLAGGED residue is
    three items and all three are answered: spell icons ship and are
    drawn (`ui/spellIcons.js`, imported at
    `ui/spellbookWindow.js:136-138`, drawn at `:1007`), the icon picker
    ...
    clause goes with it.**
13. **`:626` the talk manager.** The row's own tail already says both
...
14. **`:656` the small-residue trio** (biography GP arm, arrow roll
...
- **The voxel character engine** (Mac's system, Port-Doctrine). 86
  ...
  by a game host: `worldModes.js:4395` passes `voxelfolk`,
...
- **`FaceUVTool` arithmetic** - JS doubles for C# float mix, `Math.trunc`
  ...
  precision are ledger row `:599`, narrowed at E8 and the only part of

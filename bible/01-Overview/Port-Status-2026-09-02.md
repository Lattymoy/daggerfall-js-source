...
the same place - `Port-Ledger.md:653`, the struck keybinding-registry
...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:5104`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:3016` carries the sweep and `hostMagic.js:608` its missile half. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:1992`, `:1760`). Ledger row `:599`'s residue list is spent - see list 2. |
...
- ~~**`src/scenes/dungeonContext.js:1905`** - the two window seams this
  ...
  carries the same closure against Ledger row `:662`. The two WINDOWS
  ...
  `test/x11b.test.js`. This was also Ledger row `:662`'s adjudication,
...
Section C's table is **247 rows** between `Port-Ledger.md:536` and `:782`
...
route** - plus `:653`, struck at its head but carrying a live PENDING
...
1. **`:571` FaceUVTool's 1,803-UV residual at matched precision** ->
...
2. **`:584` the custom builder's hidden `ResetBonusPool` control** (STRUCK at E2, landing after this list was written: the control is live) ->
...
3. ~~**`:554` remainder: the overlay mouse-UP seam** -> UI arc. A7 shipped
...
4. ~~**`:597` the quest machine** -> `playSound`'s busy-skip, the one
...
5. ~~**`:650` the classic `.SAV` reader** -> the phone path: no zip arm in
   the saves picker, a desktop-first charter call.~~ **CLOSED (OT1, 2026-09-10).**
6. **`:653` (struck, with a live clause) the keybinding registry** ->
   the mouse/advanced and joystick sub-windows, the two of DFU's 65
   game windows the port does not cite.
7. ~~**`:662` the standalone dungeon host has no trade window** -> a
...
8. ~~**`:663` three stale probes**~~ **CLOSED (E-group, 2026-09-02).**
...
9. **`:549` UseItem's unbuilt destinations.** Every arm the row names is
...
10. **`:599` fast-travel residue.** Its three named survivors are spent:
    ...
    `scenes/world.js:4102` (the arrival clamp anchoring the encounter
    clock, so the traveled window is not replayed) and `:1966` (the
    `scenes/world.js:4046` (the arrival clamp anchoring the encounter
...
11. **`:604` `PatchRegionIndex` legacy-save fix.** Ported verbatim at
    `src/formats/mapsFile.js:104`, with the C# line range cited.
12. **`:648` the magic crafting windows.** The row's FLAGGED residue is
    ...
    clause goes with it.**
13. **`:598` the talk manager.** The row's own tail already says both
...
14. **`:628` the small-residue trio** (biography GP arm, arrow roll
...
- **`FaceUVTool` arithmetic** - JS doubles for C# float mix, `Math.trunc`
  ...
  precision are ledger row `:571`, narrowed at E8 and the only part of

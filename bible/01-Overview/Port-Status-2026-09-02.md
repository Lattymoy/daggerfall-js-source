...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:8397`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:5313` carries the sweep and `hostMagic.js:816` its missile half. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:3696`, `:3390`). Ledger row `:683`'s residue list is spent - see list 2. |
...
9. **`:633` UseItem's unbuilt destinations.** Every arm the row names is
   ...
   (`ui/nativeInventory.js:842-846`, `scenes/world.js:5367`), the
...
10. **`:683` fast-travel residue.** Its three named survivors are spent:
    ...
    `scenes/world.js:4547` (the arrival clamp anchoring the encounter

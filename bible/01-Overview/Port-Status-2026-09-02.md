...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:4266`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:2565` carries the sweep and `hostMagic.js:594` its missile half. |
...
| **characters-ai** | Verbatim | **Verbatim** | `stopDistance` picks `CLASSIC_MELEE_DISTANCE_VS_AI` per pass (`enemyMotor.js:1176`, `enemyAttack.js:177`). ROAD-A A5 took enemy levitation, invisibility/Shade as live sources, foe fall damage and the Seducer transform pair. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:1636`, `:1667`). Ledger row `:547`'s residue list is spent - see list 2. |
...
that own overlay slots - `worldModes.js:56` and `dungeonContext.js:35` -
...
- ~~**`src/scenes/dungeonContext.js:1687`** - the two window seams this
...
- **`src/scenes/exterior.js:1536`** - Recall pends here; the anchor
...
- ~~**`src/scenes/exterior.js:1426`** - PX3: this test host mounts no
...
- ~~**`src/scenes/world.js:2817`** - the port's default landing stands
...
regenerated onto the new sites: ~~`exterior.js:1120` -> `:1089`~~
(**CLOSED at TP2**: the Recall interim narrowed to the one arm this
host cannot take, `exterior.js:1429`), ~~`exterior.js:1472` ->
`:1347`~~ (**CLOSED at QX1**: this host took the quest bridge, so the
pause window's Quests tab has a machine to read), `world.js:2863` ->
`:2935`, ~~`worldModes.js:1659` -> `:1687`~~ (**CLOSED at ROAD-F GS1**,
...
9. **`:497` UseItem's unbuilt destinations.** Every arm the row names is
   ...
   (`ui/nativeInventory.js:670-674`, `scenes/world.js:2614`), the
   (`ui/nativeInventory.js:670-674`, `scenes/world.js:2560`), the
...
10. **`:547` fast-travel residue.** Its three named survivors are spent:
    ...
    `scenes/world.js:3534` (the arrival clamp anchoring the encounter
    clock, so the traveled window is not replayed) and `:1965` (the
    `scenes/world.js:3487` (the arrival clamp anchoring the encounter

...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:4811`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:2849` carries the sweep and `hostMagic.js:601` its missile half. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:1889`, `:1695`). Ledger row `:564`'s residue list is spent - see list 2. |
...
- ~~**`src/scenes/world.js:3098`** - the port's default landing stands
...
Quests tab has a machine to read), ~~`world.js:3144` -> `:2969`~~
...
9. **`:514` UseItem's unbuilt destinations.** Every arm the row names is
   ...
   (`ui/nativeInventory.js:703-707`, `scenes/world.js:2898`), the
   (`ui/nativeInventory.js:703-707`, `scenes/world.js:2844`), the
...
10. **`:564` fast-travel residue.** Its three named survivors are spent:
    ...
    `scenes/world.js:3807` (the arrival clamp anchoring the encounter

...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:7399`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:4568` carries the sweep and `hostMagic.js:711` its missile half. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:3026`, `:2742`). Ledger row `:641`'s residue list is spent - see list 2. |
...
- ~~**`src/scenes/dungeonContext.js:2116`** - the two window seams this
...
- **`src/scenes/exterior.js:2206`** - Recall pends here; the anchor
...
- ~~**`src/scenes/exterior.js:1846`** - PX3: this test host mounts no
...
regenerated onto the new sites: ~~`exterior.js:1526` -> `:1266`~~
(**CLOSED at TP2**: the Recall interim narrowed to the one arm this host
cannot take, `exterior.js:1868`), ~~`exterior.js:1918` -> `:1654`~~
(**CLOSED at QX1**: this host took the quest bridge, so the pause window's
Quests tab has a machine to read), ~~`world.js:3863` -> `:4135`~~

...
the same place - `Port-Ledger.md:638`, the struck keybinding-registry
...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:4911`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:2919` carries the sweep and `hostMagic.js:608` its missile half. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:1948`, `:1727`). Ledger row `:584`'s residue list is spent - see list 2. |
...
- ~~**`src/scenes/dungeonContext.js:1868`** - the two window seams this
  ...
  carries the same closure against Ledger row `:647`. The two WINDOWS
  ...
  `test/x11b.test.js`. This was also Ledger row `:647`'s adjudication,
...
- ~~**`src/scenes/exterior.js:1621`** - PX3: this test host mounts no
...
- ~~**`src/scenes/world.js:3151`** - the port's default landing stands
...
regenerated onto the new sites: ~~`exterior.js:1289` -> `:1107`~~
(**CLOSED at TP2**: the Recall interim narrowed to the one arm this host
cannot take, `exterior.js:1624`), ~~`exterior.js:1667` -> `:1373`~~
(**CLOSED at QX1**: this host took the quest bridge, so the pause window's
Quests tab has a machine to read), ~~`world.js:3197` -> `:3005`~~
...
Section C's table is **247 rows** between `Port-Ledger.md:521` and `:767`
...
route** - plus `:638`, struck at its head but carrying a live PENDING
...
1. **`:556` FaceUVTool's 1,803-UV residual at matched precision** ->
...
2. **`:569` the custom builder's hidden `ResetBonusPool` control** (STRUCK at E2, landing after this list was written: the control is live) ->
...
3. ~~**`:539` remainder: the overlay mouse-UP seam** -> UI arc. A7 shipped
...
4. ~~**`:582` the quest machine** -> `playSound`'s busy-skip, the one
...
5. ~~**`:635` the classic `.SAV` reader** -> the phone path: no zip arm in
   the saves picker, a desktop-first charter call.~~ **CLOSED (OT1, 2026-09-10).**
6. **`:638` (struck, with a live clause) the keybinding registry** ->
   the mouse/advanced and joystick sub-windows, the two of DFU's 65
   game windows the port does not cite.
7. ~~**`:647` the standalone dungeon host has no trade window** -> a
...
8. ~~**`:648` three stale probes**~~ **CLOSED (E-group, 2026-09-02).**
...
9. **`:534` UseItem's unbuilt destinations.** Every arm the row names is
   built: `DrinkPotion` (`systems/useItem.js:167`, `:245-255`),
   `RecordLocationFromMap`/`DiscoverRandomLocation`
   (`ui/nativeInventory.js:704-708`, `scenes/world.js:2968`), the
   (`ui/nativeInventory.js:704-708`, `scenes/world.js:2914`), the
...
10. **`:584` fast-travel residue.** Its three named survivors are spent:
    ...
    `scenes/world.js:3935` (the arrival clamp anchoring the encounter
    clock, so the traveled window is not replayed) and `:1966` (the
    `scenes/world.js:3879` (the arrival clamp anchoring the encounter
...
11. **`:589` `PatchRegionIndex` legacy-save fix.** Ported verbatim at
    `src/formats/mapsFile.js:104`, with the C# line range cited.
12. **`:633` the magic crafting windows.** The row's FLAGGED residue is
    ...
    clause goes with it.**
13. **`:583` the talk manager.** The row's own tail already says both
...
14. **`:613` the small-residue trio** (biography GP arm, arrow roll
...
- **`FaceUVTool` arithmetic** - JS doubles for C# float mix, `Math.trunc`
  ...
  precision are ledger row `:556`, narrowed at E8 and the only part of

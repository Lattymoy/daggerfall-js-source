...
| **characters-voxel** | Departure (deliberate) | **Departure, still editor-only** | 86 designs across seven tables - 42 of the 43 monster mobiles, 19 class, 25 villager. The rig is 1,791 lines across 7 modules and is still gated: `worldModes.js:5283` passes `voxelfolk`, `scenes/interiorContext.js:576` consumes it. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:3049`, `:2757`). Ledger row `:645`'s residue list is spent - see list 2. |
...
- ~~**`src/scenes/exterior.js:1947`** - PX3: this test host mounts no
  ...
  site. (The `src/scenes/worldModes.js:2282` row below is struck: ROAD-E
...
- **`src/scenes/dungeonContext.js:1172`** - the dungeon's non-layout foe
...
9. **`:595` UseItem's unbuilt destinations.** Every arm the row names is
   ...
   (`ui/nativeInventory.js:832-836`, `scenes/world.js:4669`), the
...
10. **`:645` fast-travel residue.** Its three named survivors are spent:
    ...
    `scenes/world.js:3864` (the arrival clamp anchoring the encounter
...
- **The voxel character engine** (Mac's system, Port-Doctrine). 86
  ...
  by a game host: `worldModes.js:5283` passes `voxelfolk`,

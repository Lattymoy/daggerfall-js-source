...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:4894`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:2906` carries the sweep and `hostMagic.js:608` its missile half. |
...
| **characters-voxel** | Departure (deliberate) | **Departure, still editor-only** | 86 designs across seven tables - 42 of the 43 monster mobiles, 19 class, 25 villager. The rig is 1,791 lines across 7 modules and is still gated: `worldModes.js:4260` passes `voxelfolk`, `scenes/interiorContext.js:483` consumes it. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:1947`, `:1726`). Ledger row `:584`'s residue list is spent - see list 2. |
...
and `world.js:192` reaches it by mounting `worldModes`. But `grep -rn
...
- ~~**`src/scenes/dungeonContext.js:1867`** - the two window seams this
...
- **`src/ui/enhancedMenu.js:2257`** - the rest of the keyboard; the
...
- **`src/scenes/exterior.js:1732`** - Recall pends here; the anchor
...
- ~~**`src/scenes/exterior.js:1610`** - PX3: this test host mounts no
  ...
  site. (The `src/scenes/worldModes.js:1906` row below is struck: ROAD-E
...
- ~~**`src/scenes/world.js:3137`** - the port's default landing stands
...
- ~~**`src/characters/enemyCasting.js:91`** - now exactly one term
  ...
  second.*
- ~~**`src/scenes/worldModes.js:1852`** - above ground only:
...
regenerated onto the new sites: ~~`exterior.js:1279` -> `:1106`~~
(**CLOSED at TP2**: the Recall interim narrowed to the one arm this host
cannot take, `exterior.js:1613`), ~~`exterior.js:1656` -> `:1363`~~
(**CLOSED at QX1**: this host took the quest bridge, so the pause window's
Quests tab has a machine to read), ~~`world.js:3183` -> `:2991`~~
(**CLOSED at the ship landing**), ~~`worldModes.js:1849` -> `:1704`~~
...
- ~~**`src/scenes/worldModes.js:1922`** - above ground only: the GUILD
...
9. **`:534` UseItem's unbuilt destinations.** Every arm the row names is
   ...
   (`ui/nativeInventory.js:704-708`, `scenes/world.js:2955`), the
   (`ui/nativeInventory.js:704-708`, `scenes/world.js:2901`), the

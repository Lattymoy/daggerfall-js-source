...
the same place - `Port-Ledger.md:664`, the struck keybinding-registry
...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:5392`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:3203` carries the sweep and `hostMagic.js:608` its missile half. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:2046`, `:1805`). Ledger row `:610`'s residue list is spent - see list 2. |
...
- ~~**`src/scenes/dungeonContext.js:1913`** - the two window seams this
  ...
  carries the same closure against Ledger row `:673`. The two WINDOWS
  ...
  `test/x11b.test.js`. This was also Ledger row `:673`'s adjudication,
...
- **`src/scenes/exterior.js:1883`** - Recall pends here; the anchor
...
- ~~**`src/scenes/exterior.js:1673`** - PX3: this test host mounts no
...
- ~~**`src/scenes/world.js:3324`** - the port's default landing stands
...
regenerated onto the new sites: ~~`exterior.js:1339` -> `:1175`~~
(**CLOSED at TP2**: the Recall interim narrowed to the one arm this host
cannot take, `exterior.js:1675`), ~~`exterior.js:1719` -> `:1427`~~
(**CLOSED at QX1**: this host took the quest bridge, so the pause window's
Quests tab has a machine to read), ~~`world.js:3363` -> `:3211`~~
...
Section C's table is **247 rows** between `Port-Ledger.md:547` and `:793`
...
route** - plus `:664`, struck at its head but carrying a live PENDING
...
1. **`:582` FaceUVTool's 1,803-UV residual at matched precision** ->
...
2. **`:595` the custom builder's hidden `ResetBonusPool` control** (STRUCK at E2, landing after this list was written: the control is live) ->
...
3. ~~**`:565` remainder: the overlay mouse-UP seam** -> UI arc. A7 shipped
...
4. ~~**`:608` the quest machine** -> `playSound`'s busy-skip, the one
...
5. ~~**`:661` the classic `.SAV` reader** -> the phone path: no zip arm in
   the saves picker, a desktop-first charter call.~~ **CLOSED (OT1, 2026-09-10).**
6. **`:664` (struck, with a live clause) the keybinding registry** ->
   the mouse/advanced and joystick sub-windows, the two of DFU's 65
   game windows the port does not cite.
7. ~~**`:673` the standalone dungeon host has no trade window** -> a
...
8. ~~**`:674` three stale probes**~~ **CLOSED (E-group, 2026-09-02).**
...
9. **`:560` UseItem's unbuilt destinations.** Every arm the row names is
   built: `DrinkPotion` (`systems/useItem.js:184`, `:262-272`),
   `RecordLocationFromMap`/`DiscoverRandomLocation`
   (`ui/nativeInventory.js:736-740`, `scenes/world.js:3252`), the
   (`ui/nativeInventory.js:736-740`, `scenes/world.js:3198`), the
...
10. **`:610` fast-travel residue.** Its three named survivors are spent:
    ...
    `scenes/world.js:4329` (the arrival clamp anchoring the encounter
    clock, so the traveled window is not replayed) and `:1969` (the
    `scenes/world.js:4273` (the arrival clamp anchoring the encounter
...
11. **`:615` `PatchRegionIndex` legacy-save fix.** Ported verbatim at
    `src/formats/mapsFile.js:104`, with the C# line range cited.
12. **`:659` the magic crafting windows.** The row's FLAGGED residue is
    ...
    clause goes with it.**
13. **`:609` the talk manager.** The row's own tail already says both
...
14. **`:639` the small-residue trio** (biography GP arm, arrow roll
...
- **`FaceUVTool` arithmetic** - JS doubles for C# float mix, `Math.trunc`
  ...
  precision are ledger row `:582`, narrowed at E8 and the only part of

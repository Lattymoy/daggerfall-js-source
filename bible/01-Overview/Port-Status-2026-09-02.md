...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:5007`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:2972` carries the sweep and `hostMagic.js:608` its missile half. |
...
9. **`:546` UseItem's unbuilt destinations.** Every arm the row names is
   ...
   (`ui/nativeInventory.js:717-721`, `scenes/world.js:3021`), the
   (`ui/nativeInventory.js:717-721`, `scenes/world.js:2967`), the
...
10. **`:596` fast-travel residue.** Its three named survivors are spent:
    ...
    `scenes/world.js:4021` (the arrival clamp anchoring the encounter
    clock, so the traveled window is not replayed) and `:1966` (the
    `scenes/world.js:3965` (the arrival clamp anchoring the encounter

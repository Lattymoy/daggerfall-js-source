...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:5337`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:3148` carries the sweep and `hostMagic.js:608` its missile half. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:2014`, `:1774`). Ledger row `:609`'s residue list is spent - see list 2. |
...
and `world.js:207` reaches it by mounting `worldModes`. But `grep -rn
...
- ~~**`src/scenes/world.js:3269`** - the port's default landing stands
...
Quests tab has a machine to read), ~~`world.js:3308` -> `:3156`~~
...
9. **`:559` UseItem's unbuilt destinations.** Every arm the row names is
   ...
   (`ui/nativeInventory.js:736-740`, `scenes/world.js:3197`), the
   (`ui/nativeInventory.js:736-740`, `scenes/world.js:3143`), the
...
10. **`:609` fast-travel residue.** Its three named survivors are spent:
    ...
    `scenes/world.js:4274` (the arrival clamp anchoring the encounter
    clock, so the traveled window is not replayed) and `:1969` (the
    `scenes/world.js:4218` (the arrival clamp anchoring the encounter

...
the same place - `Port-Ledger.md:604`, the struck keybinding-registry
...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:4753`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:2819` carries the sweep and `hostMagic.js:601` its missile half. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:1861`, `:1667`). Ledger row `:550`'s residue list is spent - see list 2. |
...
and `world.js:180` reaches it by mounting `worldModes`. But `grep -rn
...
- ~~**`src/scenes/dungeonContext.js:1817`** - the two window seams this
  ...
  carries the same closure against Ledger row `:613`. The two WINDOWS
...
- **`src/systems/inputActions.js:543`** - STILL FLAGGED: axes and
  ...
  sub-windows are recorded pending at `Port-Ledger.md:604`. The flag's
...
1. **`:522` FaceUVTool's 1,803-UV residual at matched precision** ->
...
2. **`:535` the custom builder's hidden `ResetBonusPool` control** (STRUCK at E2, landing after this list was written: the control is live) ->
...
3. **`:505` remainder: the overlay mouse-UP seam** -> UI arc. A7 shipped
...
4. ~~**`:548` the quest machine** -> `playSound`'s busy-skip, the one
...
7. ~~**`:613` the standalone dungeon host has no trade window** -> a
...
8. ~~**`:614` three stale probes**~~ **CLOSED (E-group, 2026-09-02).**
...
9. **`:500` UseItem's unbuilt destinations.** Every arm the row names is
   built: `DrinkPotion` (`systems/useItem.js:167`, `:245-255`),
   `RecordLocationFromMap`/`DiscoverRandomLocation`
   (`ui/nativeInventory.js:703-707`, `scenes/world.js:2868`), the
   (`ui/nativeInventory.js:703-707`, `scenes/world.js:2814`), the
...
10. **`:550` fast-travel residue.** Its three named survivors are spent:
    ...
    `scenes/world.js:3828` (the arrival clamp anchoring the encounter
    clock, so the traveled window is not replayed) and `:1966` (the
    `scenes/world.js:3772` (the arrival clamp anchoring the encounter
...
11. **`:555` `PatchRegionIndex` legacy-save fix.** Ported verbatim at
...
12. **`:599` the magic crafting windows.** The row's FLAGGED residue is
    ...
    clause goes with it.**
13. **`:549` the talk manager.** The row's own tail already says both
...
14. **`:579` the small-residue trio** (biography GP arm, arrow roll
...
- **`FaceUVTool` arithmetic** - JS doubles for C# float mix, `Math.trunc`
  ...
  precision are ledger row `:522`, narrowed at E8 and the only part of

...
the same place - `Port-Ledger.md:725`, the struck keybinding-registry
...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:7606`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:4681` carries the sweep and `hostMagic.js:816` its missile half. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:3086`, `:2794`). Ledger row `:671`'s residue list is spent - see list 2. |
...
and `world.js:293` reaches it by mounting `worldModes`. But `grep -rn
...
- ~~**`src/scenes/dungeonContext.js:2126`** - the two window seams this
  ...
  carries the same closure against Ledger row `:734`. The two WINDOWS
  ...
  `test/x11b.test.js`. This was also Ledger row `:734`'s adjudication,
...
Section C's table is **247 rows** between `Port-Ledger.md:608` and `:854`
...
route** - plus `:725`, struck at its head but carrying a live PENDING
...
1. **`:643` FaceUVTool's 1,803-UV residual at matched precision** ->
...
2. **`:656` the custom builder's hidden `ResetBonusPool` control** (STRUCK at E2, landing after this list was written: the control is live) ->
...
3. ~~**`:626` remainder: the overlay mouse-UP seam** -> UI arc. A7 shipped
...
4. ~~**`:669` the quest machine** -> `playSound`'s busy-skip, the one
...
5. ~~**`:722` the classic `.SAV` reader** -> the phone path: no zip arm in
   the saves picker, a desktop-first charter call.~~ **CLOSED (OT1, 2026-09-10).**
6. **`:725` (struck, with a live clause) the keybinding registry** ->
   the mouse/advanced and joystick sub-windows, the two of DFU's 65
   game windows the port does not cite.
7. ~~**`:734` the standalone dungeon host has no trade window** -> a
...
8. ~~**`:735` three stale probes**~~ **CLOSED (E-group, 2026-09-02).**
...
9. **`:621` UseItem's unbuilt destinations.** Every arm the row names is
   built: `DrinkPotion` (`systems/useItem.js:205`, `:296-306`),
   `RecordLocationFromMap`/`DiscoverRandomLocation`
   (`ui/nativeInventory.js:834-838`, `scenes/world.js:4735`), the
...
10. **`:671` fast-travel residue.** Its three named survivors are spent:
    ...
    `scenes/world.js:3923` (the arrival clamp anchoring the encounter
...
11. **`:676` `PatchRegionIndex` legacy-save fix.** Ported verbatim at
    `src/formats/mapsFile.js:107`, with the C# line range cited.
12. **`:720` the magic crafting windows.** The row's FLAGGED residue is
    ...
    clause goes with it.**
13. **`:670` the talk manager.** The row's own tail already says both
...
14. **`:700` the small-residue trio** (biography GP arm, arrow roll
...
- **`FaceUVTool` arithmetic** - JS doubles for C# float mix, `Math.trunc`
  ...
  precision are ledger row `:643`, narrowed at E8 and the only part of

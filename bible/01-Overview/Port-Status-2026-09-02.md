...
and six rows** (five, since OT1 struck `:676`),
...
the same place - `Port-Ledger.md:689`, the struck keybinding-registry
...
| **characters-voxel** | Departure (deliberate) | **Departure, still editor-only** | 86 designs across seven tables - 42 of the 43 monster mobiles, 19 class, 25 villager. The rig is 1,791 lines across 7 modules and is still gated: `worldModes.js:5172` passes `voxelfolk`, `scenes/interiorContext.js:503` consumes it. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:2470`, `:2187`). Ledger row `:635`'s residue list is spent - see list 2. |
...
- ~~**`src/scenes/dungeonContext.js:2113`** - the two window seams this
  ...
  carries the same closure against Ledger row `:698`. The two WINDOWS
  ...
  `test/x11b.test.js`. This was also Ledger row `:698`'s adjudication,
  and that row is struck with it.
- **`src/ui/enhancedMenu.js:2969`** - the rest of the keyboard; the
...
- ~~**`src/scenes/exterior.js:1754`** - PX3: this test host mounts no
  ...
  site. (The `src/scenes/worldModes.js:2227` row below is struck: ROAD-E
...
Section C's table is **247 rows** between `Port-Ledger.md:572` and `:818`
...
generation, to the enhanced lane)~~ (**it read 226 and 21 until `:560` was
...
route** - plus `:689`, struck at its head but carrying a live PENDING
...
1. **`:607` FaceUVTool's 1,803-UV residual at matched precision** ->
...
2. **`:620` the custom builder's hidden `ResetBonusPool` control** (STRUCK at E2, landing after this list was written: the control is live) ->
...
3. ~~**`:590` remainder: the overlay mouse-UP seam** -> UI arc. A7 shipped
...
4. ~~**`:633` the quest machine** -> `playSound`'s busy-skip, the one
...
5. ~~**`:686` the classic `.SAV` reader** -> the phone path: no zip arm in
   the saves picker, a desktop-first charter call.~~ **CLOSED (OT1, 2026-09-10).**
6. **`:689` (struck, with a live clause) the keybinding registry** ->
   the mouse/advanced and joystick sub-windows, the two of DFU's 65
   game windows the port does not cite.
7. ~~**`:698` the standalone dungeon host has no trade window** -> a
...
8. ~~**`:699` three stale probes**~~ **CLOSED (E-group, 2026-09-02).**
...
9. **`:585` UseItem's unbuilt destinations.** Every arm the row names is
   built: `DrinkPotion` (`systems/useItem.js:197`, `:275-285`),
   `RecordLocationFromMap`/`DiscoverRandomLocation`
   (`ui/nativeInventory.js:778-782`, `scenes/world.js:3949`), the
...
10. **`:635` fast-travel residue.** Its three named survivors are spent:
...
11. **`:640` `PatchRegionIndex` legacy-save fix.** Ported verbatim at
    `src/formats/mapsFile.js:104`, with the C# line range cited.
12. **`:684` the magic crafting windows.** The row's FLAGGED residue is
    three items and all three are answered: spell icons ship and are
    drawn (`ui/spellIcons.js`, imported at
    `ui/spellbookWindow.js:136-138`, drawn at `:1011`), the icon picker
    ...
    clause goes with it.**
13. **`:634` the talk manager.** The row's own tail already says both
...
14. **`:664` the small-residue trio** (biography GP arm, arrow roll
...
- **The voxel character engine** (Mac's system, Port-Doctrine). 86
  ...
  by a game host: `worldModes.js:5172` passes `voxelfolk`,
...
- **`FaceUVTool` arithmetic** - JS doubles for C# float mix, `Math.trunc`
  ...
  precision are ledger row `:607`, narrowed at E8 and the only part of

...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:7351`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:4529` carries the sweep and `hostMagic.js:711` its missile half. |
...
| **characters-voxel** | Departure (deliberate) | **Departure, still editor-only** | 86 designs across seven tables - 42 of the 43 monster mobiles, 19 class, 25 villager. The rig is 1,791 lines across 7 modules and is still gated: `worldModes.js:5224` passes `voxelfolk`, `scenes/interiorContext.js:503` consumes it. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:2991`, `:2707`). Ledger row `:642`'s residue list is spent - see list 2. |
...
and `world.js:271` reaches it by mounting `worldModes`. But `grep -rn
...
- ~~**`src/scenes/dungeonContext.js:2115`** - the two window seams this
...
- **`src/ui/enhancedMenu.js:3016`** - the rest of the keyboard; the
...
- **`src/scenes/exterior.js:2207`** - Recall pends here; the anchor
...
- ~~**`src/scenes/exterior.js:1896`** - PX3: this test host mounts no
  ...
  site. (The `src/scenes/worldModes.js:2237` row below is struck: ROAD-E
...
- ~~**`src/scenes/world.js:4296`** - the port's default landing stands
...
- ~~**`src/characters/enemyCasting.js:91`** - now exactly one term
  ...
  second.*
- ~~**`src/scenes/worldModes.js:1939`** - above ground only:
...
regenerated onto the new sites: ~~`exterior.js:1527` -> `:1267`~~
(**CLOSED at TP2**: the Recall interim narrowed to the one arm this host
cannot take, `exterior.js:1869`), ~~`exterior.js:1919` -> `:1655`~~
(**CLOSED at QX1**: this host took the quest bridge, so the pause window's
Quests tab has a machine to read), ~~`world.js:3824` -> `:4096`~~
...
- ~~**`src/scenes/worldModes.js:2009`** - above ground only: the GUILD
...
9. **`:592` UseItem's unbuilt destinations.** Every arm the row names is
   built: `DrinkPotion` (`systems/useItem.js:198`, `:276-286`),
   `RecordLocationFromMap`/`DiscoverRandomLocation`
   (`ui/nativeInventory.js:798-802`, `scenes/world.js:4583`), the
...
10. **`:642` fast-travel residue.** Its three named survivors are spent:
    ...
    `scenes/world.js:3798` (the arrival clamp anchoring the encounter
...
- **The voxel character engine** (Mac's system, Port-Doctrine). 86
  ...
  by a game host: `worldModes.js:5224` passes `voxelfolk`,

...
the same place - `Port-Ledger.md:632`, the struck keybinding-registry
...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:4868`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:2880` carries the sweep and `hostMagic.js:601` its missile half. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:1920`, `:1711`). Ledger row `:578`'s residue list is spent - see list 2. |
...
- ~~**`src/scenes/dungeonContext.js:1881`** - the two window seams this
  ...
  carries the same closure against Ledger row `:641`. The two WINDOWS
...
- **`src/ui/enhancedMenu.js:2166`** - the rest of the keyboard; the
...
- ~~**`src/scenes/world.js:3126`** - the port's default landing stands
...
Quests tab has a machine to read), ~~`world.js:3172` -> `:2978`~~
...
Section C's table is **247 rows** between `Port-Ledger.md:515` and `:761`
...
route** - plus `:632`, struck at its head but carrying a live PENDING
...
1. **`:550` FaceUVTool's 1,803-UV residual at matched precision** ->
...
2. **`:563` the custom builder's hidden `ResetBonusPool` control** (STRUCK at E2, landing after this list was written: the control is live) ->
...
3. ~~**`:533` remainder: the overlay mouse-UP seam** -> UI arc. A7 shipped
...
4. ~~**`:576` the quest machine** -> `playSound`'s busy-skip, the one
...
5. ~~**`:629` the classic `.SAV` reader** -> the phone path: no zip arm in
   the saves picker, a desktop-first charter call.~~ **CLOSED (OT1, 2026-09-10).**
6. **`:632` (struck, with a live clause) the keybinding registry** ->
   the mouse/advanced and joystick sub-windows, the two of DFU's 65
   game windows the port does not cite.
7. ~~**`:641` the standalone dungeon host has no trade window** -> a
...
8. ~~**`:642` three stale probes**~~ **CLOSED (E-group, 2026-09-02).**
...
9. **`:528` UseItem's unbuilt destinations.** Every arm the row names is
   built: `DrinkPotion` (`systems/useItem.js:167`, `:245-255`),
   `RecordLocationFromMap`/`DiscoverRandomLocation`
   (`ui/nativeInventory.js:703-707`, `scenes/world.js:2929`), the
...
10. **`:578` fast-travel residue.** Its three named survivors are spent:
...
11. **`:583` `PatchRegionIndex` legacy-save fix.** Ported verbatim at
    `src/formats/mapsFile.js:104`, with the C# line range cited.
12. **`:627` the magic crafting windows.** The row's FLAGGED residue is
    ...
    clause goes with it.**
13. **`:577` the talk manager.** The row's own tail already says both
...
14. **`:607` the small-residue trio** (biography GP arm, arrow roll
...
- **`FaceUVTool` arithmetic** - JS doubles for C# float mix, `Math.trunc`
  ...
  precision are ledger row `:550`, narrowed at E8 and the only part of

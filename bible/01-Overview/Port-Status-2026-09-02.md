...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:4913`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:2921` carries the sweep and `hostMagic.js:608` its missile half. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:1950`, `:1729`). Ledger row `:585`'s residue list is spent - see list 2. |
...
and `world.js:194` reaches it by mounting `worldModes`. But `grep -rn
...
- **`src/scenes/exterior.js:1744`** - Recall pends here; the anchor
...
- ~~**`src/ui/hudLarge.js:75`** - the docked bar occludes, it does not
  ...
  (`render/renderer.js:1325, :1299, :1795, :1970`) and the 2D passes
...
9. **`:535` UseItem's unbuilt destinations.** Every arm the row names is
   ...
   (`ui/nativeInventory.js:704-708`, `scenes/world.js:2970`), the
   (`ui/nativeInventory.js:704-708`, `scenes/world.js:2916`), the
...
10. **`:585` fast-travel residue.** Its three named survivors are spent:
    ...
    `scenes/world.js:3937` (the arrival clamp anchoring the encounter
    clock, so the traveled window is not replayed) and `:1966` (the
    `scenes/world.js:3881` (the arrival clamp anchoring the encounter

...
the same place - `Port-Ledger.md:620`, the struck keybinding-registry
...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:4834`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:2851` carries the sweep and `hostMagic.js:601` its missile half. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:1891`, `:1697`). Ledger row `:566`'s residue list is spent - see list 2. |
...
- ~~**`src/scenes/dungeonContext.js:1844`** - the two window seams this
  ...
  carries the same closure against Ledger row `:629`. The two WINDOWS
  ...
  `test/x11b.test.js`. This was also Ledger row `:629`'s adjudication,
  and that row is struck with it.
- **`src/ui/enhancedMenu.js:2138`** - the rest of the keyboard; the
...
- ~~**`src/scenes/world.js:3100`** - the port's default landing stands
...
Quests tab has a machine to read), ~~`world.js:3146` -> `:2969`~~
...
Section C's table is **247 rows** between `Port-Ledger.md:503` and `:749`
...
route** - plus `:620`, struck at its head but carrying a live PENDING
...
1. **`:538` FaceUVTool's 1,803-UV residual at matched precision** ->
...
2. **`:551` the custom builder's hidden `ResetBonusPool` control** (STRUCK at E2, landing after this list was written: the control is live) ->
...
3. ~~**`:521` remainder: the overlay mouse-UP seam** -> UI arc. A7 shipped
...
4. ~~**`:564` the quest machine** -> `playSound`'s busy-skip, the one
...
5. ~~**`:617` the classic `.SAV` reader** -> the phone path: no zip arm in
   the saves picker, a desktop-first charter call.~~ **CLOSED (OT1, 2026-09-10).**
6. **`:620` (struck, with a live clause) the keybinding registry** ->
   the mouse/advanced and joystick sub-windows, the two of DFU's 65
   game windows the port does not cite.
7. ~~**`:629` the standalone dungeon host has no trade window** -> a
...
8. ~~**`:630` three stale probes**~~ **CLOSED (E-group, 2026-09-02).**
...
9. **`:516` UseItem's unbuilt destinations.** Every arm the row names is
   built: `DrinkPotion` (`systems/useItem.js:167`, `:245-255`),
   `RecordLocationFromMap`/`DiscoverRandomLocation`
   (`ui/nativeInventory.js:703-707`, `scenes/world.js:2900`), the
   (`ui/nativeInventory.js:703-707`, `scenes/world.js:2846`), the
...
10. **`:566` fast-travel residue.** Its three named survivors are spent:
...
11. **`:571` `PatchRegionIndex` legacy-save fix.** Ported verbatim at
    `src/formats/mapsFile.js:104`, with the C# line range cited.
12. **`:615` the magic crafting windows.** The row's FLAGGED residue is
    ...
    clause goes with it.**
13. **`:565` the talk manager.** The row's own tail already says both
...
14. **`:595` the small-residue trio** (biography GP arm, arrow roll
...
- **`FaceUVTool` arithmetic** - JS doubles for C# float mix, `Math.trunc`
  ...
  precision are ledger row `:538`, narrowed at E8 and the only part of

...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:6264`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:3698` carries the sweep and `hostMagic.js:608` its missile half. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:2404`, `:2121`). Ledger row `:627`'s residue list is spent - see list 2. |
...
- **`src/scenes/exterior.js:1968`** - Recall pends here; the anchor
...
- ~~**`src/scenes/exterior.js:1747`** - PX3: this test host mounts no
...
- ~~**`src/scenes/world.js:3631`** - the port's default landing stands
...
regenerated onto the new sites: ~~`exterior.js:1427` -> `:1204`~~
(**CLOSED at TP2**: the Recall interim narrowed to the one arm this host
cannot take, `exterior.js:1769`), ~~`exterior.js:1819` -> `:1507`~~
(**CLOSED at QX1**: this host took the quest bridge, so the pause window's
Quests tab has a machine to read), ~~`world.js:3658` -> `:3655`~~
...
9. **`:577` UseItem's unbuilt destinations.** Every arm the row names is
   ...
   (`ui/nativeInventory.js:778-782`, `scenes/world.js:3783`), the
...
10. **`:627` fast-travel residue.** Its three named survivors are spent:
    ...
    `scenes/world.js:4848` (the arrival clamp anchoring the encounter

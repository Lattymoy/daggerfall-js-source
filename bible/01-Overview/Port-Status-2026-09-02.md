...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:1598`, `:1667`). Ledger row `:542`'s residue list is spent - see list 2. |
...
pause window's Quests tab has a machine to read), `world.js:2820` ->
...
10. **`:542` fast-travel residue.** Its three named survivors are spent:
    ...
    `scenes/world.js:3491` (the arrival clamp anchoring the encounter

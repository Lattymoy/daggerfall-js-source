...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:5002`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:2967` carries the sweep and `hostMagic.js:608` its missile half. |
...
that own overlay slots - `worldModes.js:65` and `dungeonContext.js:38` -
...
- **`src/scenes/exterior.js:1765`** - Recall pends here; the anchor
...
- ~~**`src/scenes/exterior.js:1644`** - PX3: this test host mounts no
...
- ~~**`src/scenes/world.js:3202`** - the port's default landing stands
...
regenerated onto the new sites: ~~`exterior.js:1312` -> `:1144`~~
(**CLOSED at TP2**: the Recall interim narrowed to the one arm this host
cannot take, `exterior.js:1647`), ~~`exterior.js:1690` -> `:1396`~~
(**CLOSED at QX1**: this host took the quest bridge, so the pause window's
Quests tab has a machine to read), ~~`world.js:3248` -> `:3051`~~
...
9. **`:546` UseItem's unbuilt destinations.** Every arm the row names is
   ...
   (`ui/nativeInventory.js:704-708`, `scenes/world.js:3016`), the
   (`ui/nativeInventory.js:704-708`, `scenes/world.js:2962`), the
...
10. **`:596` fast-travel residue.** Its three named survivors are spent:
    ...
    `scenes/world.js:4016` (the arrival clamp anchoring the encounter
    clock, so the traveled window is not replayed) and `:1966` (the
    `scenes/world.js:3960` (the arrival clamp anchoring the encounter

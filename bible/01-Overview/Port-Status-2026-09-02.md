...
the same place - `Port-Ledger.md:703`, the struck keybinding-registry
...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:7581`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:4685` carries the sweep and `hostMagic.js:789` its missile half. |
...
| **player** | Near-1:1 / one lethal seam | **Near-1:1** | `cancelMovement` is a real motor field set on the mode edges (`player/motor.js:367,:444,:452`). ROAD-A A6 took the -0.28 doorway head-dip, `PlayerMoveScanner`'s three probes, `controllerSwimHeight` 0.30 with Do(Un)Sinking, and `FreezeMotor` on teleports. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:3088`, `:2796`). Ledger row `:649`'s residue list is spent - see list 2. |
...
- ~~**`src/scenes/dungeonContext.js:2126`** - the two window seams this
  ...
  carries the same closure against Ledger row `:712`. The two WINDOWS
  ...
  `test/x11b.test.js`. This was also Ledger row `:712`'s adjudication,
  and that row is struck with it.
- **`src/ui/enhancedMenu.js:3021`** - the rest of the keyboard; the
...
- ~~**`src/scenes/world.js:4420`** - the port's default landing stands
...
Quests tab has a machine to read), ~~`world.js:4447` -> `:4470`~~
...
Section C's table is **247 rows** between `Port-Ledger.md:586` and `:832`
...
route** - plus `:703`, struck at its head but carrying a live PENDING
...
1. **`:621` FaceUVTool's 1,803-UV residual at matched precision** ->
...
2. **`:634` the custom builder's hidden `ResetBonusPool` control** (STRUCK at E2, landing after this list was written: the control is live) ->
...
3. ~~**`:604` remainder: the overlay mouse-UP seam** -> UI arc. A7 shipped
...
4. ~~**`:647` the quest machine** -> `playSound`'s busy-skip, the one
...
5. ~~**`:700` the classic `.SAV` reader** -> the phone path: no zip arm in
   the saves picker, a desktop-first charter call.~~ **CLOSED (OT1, 2026-09-10).**
6. **`:703` (struck, with a live clause) the keybinding registry** ->
   the mouse/advanced and joystick sub-windows, the two of DFU's 65
   game windows the port does not cite.
7. ~~**`:712` the standalone dungeon host has no trade window** -> a
...
8. ~~**`:713` three stale probes**~~ **CLOSED (E-group, 2026-09-02).**
...
9. **`:599` UseItem's unbuilt destinations.** Every arm the row names is
   built: `DrinkPotion` (`systems/useItem.js:205`, `:296-306`),
   `RecordLocationFromMap`/`DiscoverRandomLocation`
   (`ui/nativeInventory.js:841-845`, `scenes/world.js:4739`), the
...
10. **`:649` fast-travel residue.** Its three named survivors are spent:
...
11. **`:654` `PatchRegionIndex` legacy-save fix.** Ported verbatim at
    `src/formats/mapsFile.js:105`, with the C# line range cited.
12. **`:698` the magic crafting windows.** The row's FLAGGED residue is
    ...
    clause goes with it.**
13. **`:648` the talk manager.** The row's own tail already says both
...
14. **`:678` the small-residue trio** (biography GP arm, arrow roll
...
- **`FaceUVTool` arithmetic** - JS doubles for C# float mix, `Math.trunc`
  ...
  precision are ledger row `:621`, narrowed at E8 and the only part of

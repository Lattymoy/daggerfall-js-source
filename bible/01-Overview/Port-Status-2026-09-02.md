...
the same place - `Port-Ledger.md:667`, the struck keybinding-registry
...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:5309`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:3112` carries the sweep and `hostMagic.js:608` its missile half. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:2014`, `:1774`). Ledger row `:613`'s residue list is spent - see list 2. |
...
- ~~**`src/scenes/dungeonContext.js:1905`** - the two window seams this
  ...
  carries the same closure against Ledger row `:676`. The two WINDOWS
  ...
  `test/x11b.test.js`. This was also Ledger row `:676`'s adjudication,
...
- **`src/scenes/exterior.js:1840`** - Recall pends here; the anchor
...
- ~~**`src/scenes/world.js:3281`** - the port's default landing stands
...
cannot take, `exterior.js:1649`), ~~`exterior.js:1692` -> `:1402`~~
(**CLOSED at QX1**: this host took the quest bridge, so the pause window's
Quests tab has a machine to read), ~~`world.js:3320` -> `:3165`~~
...
Section C's table is **247 rows** between `Port-Ledger.md:550` and `:796`
...
route** - plus `:667`, struck at its head but carrying a live PENDING
...
1. **`:585` FaceUVTool's 1,803-UV residual at matched precision** ->
...
2. **`:598` the custom builder's hidden `ResetBonusPool` control** (STRUCK at E2, landing after this list was written: the control is live) ->
...
3. ~~**`:568` remainder: the overlay mouse-UP seam** -> UI arc. A7 shipped
...
4. ~~**`:611` the quest machine** -> `playSound`'s busy-skip, the one
...
5. ~~**`:664` the classic `.SAV` reader** -> the phone path: no zip arm in
   the saves picker, a desktop-first charter call.~~ **CLOSED (OT1, 2026-09-10).**
6. **`:667` (struck, with a live clause) the keybinding registry** ->
   the mouse/advanced and joystick sub-windows, the two of DFU's 65
   game windows the port does not cite.
7. ~~**`:676` the standalone dungeon host has no trade window** -> a
...
8. ~~**`:677` three stale probes**~~ **CLOSED (E-group, 2026-09-02).**
...
9. **`:563` UseItem's unbuilt destinations.** Every arm the row names is
   built: `DrinkPotion` (`systems/useItem.js:182`, `:260-270`),
   `RecordLocationFromMap`/`DiscoverRandomLocation`
   (`ui/nativeInventory.js:726-730`, `scenes/world.js:3161`), the
   (`ui/nativeInventory.js:726-730`, `scenes/world.js:3107`), the
...
10. **`:613` fast-travel residue.** Its three named survivors are spent:
    ...
    `scenes/world.js:4250` (the arrival clamp anchoring the encounter
    clock, so the traveled window is not replayed) and `:1969` (the
    `scenes/world.js:4194` (the arrival clamp anchoring the encounter
...
11. **`:618` `PatchRegionIndex` legacy-save fix.** Ported verbatim at
    `src/formats/mapsFile.js:104`, with the C# line range cited.
12. **`:662` the magic crafting windows.** The row's FLAGGED residue is
    ...
    clause goes with it.**
13. **`:612` the talk manager.** The row's own tail already says both
...
14. **`:642` the small-residue trio** (biography GP arm, arrow roll
...
- **`FaceUVTool` arithmetic** - JS doubles for C# float mix, `Math.trunc`
  ...
  precision are ledger row `:585`, narrowed at E8 and the only part of

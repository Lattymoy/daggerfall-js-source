...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:6645`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:3892` carries the sweep and `hostMagic.js:608` its missile half. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:2468`, `:2185`). Ledger row `:633`'s residue list is spent - see list 2. |
...
- ~~**`src/scenes/dungeonContext.js:2091`** - the two window seams this
...
- ~~**`src/scenes/world.js:3659`** - the port's default landing stands
...
- **`src/scenes/dungeonContext.js:1164`** - the dungeon's non-layout foe
...
Quests tab has a machine to read), ~~`world.js:3686` -> `:3679`~~
...
9. **`:583` UseItem's unbuilt destinations.** Every arm the row names is
   ...
   (`ui/nativeInventory.js:778-782`, `scenes/world.js:3946`), the
...
10. **`:633` fast-travel residue.** Its three named survivors are spent:
    ...
    `scenes/world.js:3169` (the arrival clamp anchoring the encounter

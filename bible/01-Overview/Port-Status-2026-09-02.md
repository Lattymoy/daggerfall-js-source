...
the same place - `Port-Ledger.md:640`, the struck keybinding-registry
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:1949`, `:1728`). Ledger row `:586`'s residue list is spent - see list 2. |
...
- ~~**`src/scenes/dungeonContext.js:1868`** - the two window seams this
  ...
  carries the same closure against Ledger row `:649`. The two WINDOWS
  ...
  `test/x11b.test.js`. This was also Ledger row `:649`'s adjudication,
...
- ~~**`src/ui/hudLarge.js:75`** - the docked bar occludes, it does not
  ...
  (`render/renderer.js:1314, :1288, :1784, :1959`) and the 2D passes
...
Section C's table is **247 rows** between `Port-Ledger.md:523` and `:769`
...
route** - plus `:640`, struck at its head but carrying a live PENDING
...
1. **`:558` FaceUVTool's 1,803-UV residual at matched precision** ->
...
2. **`:571` the custom builder's hidden `ResetBonusPool` control** (STRUCK at E2, landing after this list was written: the control is live) ->
...
3. ~~**`:541` remainder: the overlay mouse-UP seam** -> UI arc. A7 shipped
...
4. ~~**`:584` the quest machine** -> `playSound`'s busy-skip, the one
...
5. ~~**`:637` the classic `.SAV` reader** -> the phone path: no zip arm in
   the saves picker, a desktop-first charter call.~~ **CLOSED (OT1, 2026-09-10).**
6. **`:640` (struck, with a live clause) the keybinding registry** ->
   the mouse/advanced and joystick sub-windows, the two of DFU's 65
   game windows the port does not cite.
7. ~~**`:649` the standalone dungeon host has no trade window** -> a
...
8. ~~**`:650` three stale probes**~~ **CLOSED (E-group, 2026-09-02).**
...
9. **`:536` UseItem's unbuilt destinations.** Every arm the row names is
...
10. **`:586` fast-travel residue.** Its three named survivors are spent:
...
11. **`:591` `PatchRegionIndex` legacy-save fix.** Ported verbatim at
    `src/formats/mapsFile.js:104`, with the C# line range cited.
12. **`:635` the magic crafting windows.** The row's FLAGGED residue is
    ...
    clause goes with it.**
13. **`:585` the talk manager.** The row's own tail already says both
...
14. **`:615` the small-residue trio** (biography GP arm, arrow roll
...
- **`FaceUVTool` arithmetic** - JS doubles for C# float mix, `Math.trunc`
  ...
  precision are ledger row `:558`, narrowed at E8 and the only part of

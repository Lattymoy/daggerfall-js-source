...
the same place - `Port-Ledger.md:690`, the struck keybinding-registry
...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:6774`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:3997` carries the sweep and `hostMagic.js:662` its missile half. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:2481`, `:2197`). Ledger row `:636`'s residue list is spent - see list 2. |
...
and `world.js:263` reaches it by mounting `worldModes`. But `grep -rn
...
- ~~**`src/scenes/dungeonContext.js:2114`** - the two window seams this
  ...
  carries the same closure against Ledger row `:699`. The two WINDOWS
  ...
  `test/x11b.test.js`. This was also Ledger row `:699`'s adjudication,
  and that row is struck with it.
- **`src/ui/enhancedMenu.js:2998`** - the rest of the keyboard; the
...
- **`src/scenes/exterior.js:2146`** - Recall pends here; the anchor
...
- ~~**`src/scenes/exterior.js:1836`** - PX3: this test host mounts no
...
- ~~**`src/scenes/world.js:3764`** - the port's default landing stands
...
regenerated onto the new sites: ~~`exterior.js:1516` -> `:1216`~~
(**CLOSED at TP2**: the Recall interim narrowed to the one arm this host
cannot take, `exterior.js:1858`), ~~`exterior.js:1908` -> `:1596`~~
(**CLOSED at QX1**: this host took the quest bridge, so the pause window's
Quests tab has a machine to read), ~~`world.js:3791` -> `:3784`~~
...
Section C's table is **247 rows** between `Port-Ledger.md:573` and `:819`
...
route** - plus `:690`, struck at its head but carrying a live PENDING
...
1. **`:608` FaceUVTool's 1,803-UV residual at matched precision** ->
...
2. **`:621` the custom builder's hidden `ResetBonusPool` control** (STRUCK at E2, landing after this list was written: the control is live) ->
...
3. ~~**`:591` remainder: the overlay mouse-UP seam** -> UI arc. A7 shipped
...
4. ~~**`:634` the quest machine** -> `playSound`'s busy-skip, the one
...
5. ~~**`:687` the classic `.SAV` reader** -> the phone path: no zip arm in
   the saves picker, a desktop-first charter call.~~ **CLOSED (OT1, 2026-09-10).**
6. **`:690` (struck, with a live clause) the keybinding registry** ->
   the mouse/advanced and joystick sub-windows, the two of DFU's 65
   game windows the port does not cite.
7. ~~**`:699` the standalone dungeon host has no trade window** -> a
...
8. ~~**`:700` three stale probes**~~ **CLOSED (E-group, 2026-09-02).**
...
9. **`:586` UseItem's unbuilt destinations.** Every arm the row names is
   ...
   (`ui/nativeInventory.js:798-802`, `scenes/world.js:4051`), the
...
10. **`:636` fast-travel residue.** Its three named survivors are spent:
    ...
    `scenes/world.js:3270` (the arrival clamp anchoring the encounter
...
11. **`:641` `PatchRegionIndex` legacy-save fix.** Ported verbatim at
    `src/formats/mapsFile.js:104`, with the C# line range cited.
12. **`:685` the magic crafting windows.** The row's FLAGGED residue is
    ...
    clause goes with it.**
13. **`:635` the talk manager.** The row's own tail already says both
...
14. **`:665` the small-residue trio** (biography GP arm, arrow roll
...
- **`FaceUVTool` arithmetic** - JS doubles for C# float mix, `Math.trunc`
  ...
  precision are ledger row `:608`, narrowed at E8 and the only part of

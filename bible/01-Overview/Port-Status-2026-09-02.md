...
and six rows** (five, since OT1 struck `:604`),
...
the same place - `Port-Ledger.md:677`, the struck keybinding-registry
...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:6205`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:3669` carries the sweep and `hostMagic.js:608` its missile half. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:2375`, `:2092`). Ledger row `:623`'s residue list is spent - see list 2. |
...
| **ui-hud** | Verbatim (classic) / Partial (enhanced default) | **Verbatim on both skins** | The skin fork moved BELOW the game-state seams: `ui/hud.js:498` runs `updateHudVitals` and `:553` `drawNearDeathFlicker`, both above the enhanced branch at `:590` and above the `!art` return, and the enhanced HUD takes the Detect markers at `:605`. `lastHealthLost()` is no longer pinned at 0, so `CameraRecoiler` lives in all three hosts. |
...
- ~~**`src/scenes/dungeonContext.js:1974`** - the two window seams this
  ...
  carries the same closure against Ledger row `:686`. The two WINDOWS
  ...
  `test/x11b.test.js`. This was also Ledger row `:686`'s adjudication,
...
Section C's table is **247 rows** between `Port-Ledger.md:560` and `:806`
...
route** - plus `:677`, struck at its head but carrying a live PENDING
...
1. **`:595` FaceUVTool's 1,803-UV residual at matched precision** ->
...
2. **`:608` the custom builder's hidden `ResetBonusPool` control** (STRUCK at E2, landing after this list was written: the control is live) ->
...
3. ~~**`:578` remainder: the overlay mouse-UP seam** -> UI arc. A7 shipped
...
4. ~~**`:621` the quest machine** -> `playSound`'s busy-skip, the one
...
5. ~~**`:674` the classic `.SAV` reader** -> the phone path: no zip arm in
   the saves picker, a desktop-first charter call.~~ **CLOSED (OT1, 2026-09-10).**
6. **`:677` (struck, with a live clause) the keybinding registry** ->
   the mouse/advanced and joystick sub-windows, the two of DFU's 65
   game windows the port does not cite.
7. ~~**`:686` the standalone dungeon host has no trade window** -> a
...
8. ~~**`:687` three stale probes**~~ **CLOSED (E-group, 2026-09-02).**
...
9. **`:573` UseItem's unbuilt destinations.** Every arm the row names is
...
10. **`:623` fast-travel residue.** Its three named survivors are spent:
...
11. **`:628` `PatchRegionIndex` legacy-save fix.** Ported verbatim at
    `src/formats/mapsFile.js:104`, with the C# line range cited.
12. **`:672` the magic crafting windows.** The row's FLAGGED residue is
    ...
    clause goes with it.**
13. **`:622` the talk manager.** The row's own tail already says both
...
14. **`:652` the small-residue trio** (biography GP arm, arrow roll
...
- **`FaceUVTool` arithmetic** - JS doubles for C# float mix, `Math.trunc`
  ...
  precision are ledger row `:595`, narrowed at E8 and the only part of

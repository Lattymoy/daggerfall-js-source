...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:4899`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:2907` carries the sweep and `hostMagic.js:608` its missile half. |
...
| **characters-voxel** | Departure (deliberate) | **Departure, still editor-only** | 86 designs across seven tables - 42 of the 43 monster mobiles, 19 class, 25 villager. The rig is 1,791 lines across 7 modules and is still gated: `worldModes.js:4274` passes `voxelfolk`, `scenes/interiorContext.js:483` consumes it. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:1934`, `:1713`). Ledger row `:581`'s residue list is spent - see list 2. |
...
and `world.js:190` reaches it by mounting `worldModes`. But `grep -rn
...
- **`src/scenes/exterior.js:1733`** - Recall pends here; the anchor
...
- ~~**`src/scenes/exterior.js:1610`** - PX3: this test host mounts no
  ...
  site. (The `src/scenes/worldModes.js:1918` row below is struck: ROAD-E
...
9. **`:531` UseItem's unbuilt destinations.** Every arm the row names is
   ...
   (`ui/nativeInventory.js:704-708`, `scenes/world.js:2956`), the
   (`ui/nativeInventory.js:704-708`, `scenes/world.js:2902`), the
...
- **The voxel character engine** (Mac's system, Port-Doctrine). 86
  ...
  by a game host: `worldModes.js:4274` passes `voxelfolk`,

...
`NewLocationAlert` is `systems/discovery.js:176`'s `discoverLocation`;
...
the same place - `Port-Ledger.md:601`, the struck keybinding-registry
...
| **formats-game** | Verbatim readers / broken consumer | **Verbatim** | The `ItemRecord` conversions were in the wave; ROAD-A A4 took the classic-import stragglers (building-level MAPSAVE, the native bank record, `LegacyArtifactIndexBitfieldCheck`). Ledger row `:598`'s one residue is the phone path - no zip arm in the saves picker, a desktop-first charter call. |
...
| **world-layout** | Near-1:1 | **Near-1:1** | `rmbLayout`'s shared-block mutation is gated: `attachWindmillRecord` runs only when `enhanced` is true, is idempotent by a `subs.findIndex(r => r?.windmill)` guard, and the header names `subRecords.length` as the count three subsystems bind on (`world/rmbLayout.js:123-131` the gated call, `:174` the guard). |
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:4445`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:2636` carries the sweep and `hostMagic.js:598` its missile half. |
| **scenes-modes** | Solid, pause parity broken | **Near-1:1** | ROAD-B B1 put `UserInterfaceManager`'s real stack under this host's slot (`ui/windowStack.js`, 295 lines, imported at `worldModes.js:62`). See "the pause primitive" below - the stack exists, its `paused()` member has no reader. |
...
| **characters-voxel** | Departure (deliberate) | **Departure, still editor-only** | 86 designs across seven tables - 42 of the 43 monster mobiles, 19 class, 25 villager. The rig is 1,791 lines across 7 modules and is still gated: `worldModes.js:4021` passes `voxelfolk`, `scenes/interiorContext.js:455` consumes it. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:1678`, `:1667`). Ledger row `:547`'s residue list is spent - see list 2. |
...
that own overlay slots - `worldModes.js:62` and `dungeonContext.js:35` -
and `world.js:177` reaches it by mounting `worldModes`. But `grep -rn
...
- ~~**`src/scenes/dungeonContext.js:1727`** - the two window seams this
  ...
  carries the same closure against Ledger row `:610`. The two WINDOWS
  ...
  `test/x11b.test.js`. This was also Ledger row `:610`'s adjudication,
  and that row is struck with it.
- **`src/ui/enhancedMenu.js:1734`** - the rest of the keyboard; the
...
- **`src/scenes/exterior.js:1585`** - Recall pends here; the anchor
...
- ~~**`src/scenes/exterior.js:1475`** - PX3: this test host mounts no
  ...
  site. (The `src/scenes/worldModes.js:1802` row below is struck: ROAD-E
...
- ~~**`src/scenes/world.js:2888`** - the port's default landing stands
...
- **`src/systems/inputActions.js:513`** - STILL FLAGGED: axes and
  ...
  sub-windows are recorded pending at `Port-Ledger.md:601`. The flag's
...
- ~~**`src/characters/enemyCasting.js:91`** - now exactly one term
  ...
  second.*
- ~~**`src/scenes/worldModes.js:1802`** - above ground only:
...
regenerated onto the new sites: ~~`exterior.js:1144` -> `:1089`~~
(**CLOSED at TP2**: the Recall interim narrowed to the one arm this
host cannot take, `exterior.js:1478`), ~~`exterior.js:1521` ->
`:1346`~~ (**CLOSED at QX1**: this host took the quest bridge, so the
pause window's Quests tab has a machine to read), `world.js:2934` ->
`:2934`, ~~`worldModes.js:1799` -> `:1687`~~ (**CLOSED at ROAD-F GS1**,
...
- ~~**`src/scenes/worldModes.js:1872`** - above ground only: the GUILD
...
Section C's table is **247 rows** between `Port-Ledger.md:484` and
`:730` (`awk '/^\|/ && !/^\|---/'`). **226 are struck.** Of the 21 that
...
route** - plus `:601`, struck at its head but carrying a live PENDING
...
1. **`:519` FaceUVTool's 1,803-UV residual at matched precision** ->
...
2. **`:532` the custom builder's hidden `ResetBonusPool` control** (STRUCK at E2, landing after this list was written: the control is live) ->
...
3. **`:502` remainder: the overlay mouse-UP seam** -> UI arc. A7 shipped
...
4. ~~**`:545` the quest machine** -> `playSound`'s busy-skip, the one
...
5. **`:598` the classic `.SAV` reader** -> the phone path: no zip arm in
   the saves picker, a desktop-first charter call.
6. **`:601` (struck, with a live clause) the keybinding registry** ->
   the mouse/advanced and joystick sub-windows, the two of DFU's 65
   game windows the port does not cite.
7. ~~**`:610` the standalone dungeon host has no trade window** -> a
...
8. ~~**`:611` three stale probes**~~ **CLOSED (E-group, 2026-09-02).**
...
9. **`:497` UseItem's unbuilt destinations.** Every arm the row names is
   built: `DrinkPotion` (`systems/useItem.js:167`, `:245-255`),
   `RecordLocationFromMap`/`DiscoverRandomLocation`
   (`ui/nativeInventory.js:670-674`, `scenes/world.js:2685`), the
   (`ui/nativeInventory.js:670-674`, `scenes/world.js:2631`), the
...
10. **`:547` fast-travel residue.** Its three named survivors are spent:
    ...
    `scenes/world.js:3606` (the arrival clamp anchoring the encounter
    clock, so the traveled window is not replayed) and `:1965` (the
    `scenes/world.js:3559` (the arrival clamp anchoring the encounter
...
11. **`:552` `PatchRegionIndex` legacy-save fix.** Ported verbatim at
    `src/formats/mapsFile.js:104`, with the C# line range cited.
12. **`:596` the magic crafting windows.** The row's FLAGGED residue is
    ...
    clause goes with it.**
13. **`:546` the talk manager.** The row's own tail already says both
...
14. **`:576` the small-residue trio** (biography GP arm, arrow roll
    ...
    `BIOG*.TXT` files; the faceUV guard rides row `:519`.

## 3. The deliberate departures, which are not on the road

`Road-To-1-1.md` names these at its head so nobody re-opens them, and
`Port-Ledger.md` section A carries **86 rows, 7 struck - 79 standing
approved departures** (the 71st is WIND1, 2026-09-02, added after this page's measurement and counted here so the tally follows the tree; the 4th is MOBILE TOUCH INPUT, TI1, 2026-09-05, inserted under the windmill row and likewise; the 9th is DS1 - Dynamic Skies vendored 1:1 with permission, 2026-09-04 - inserted directly under the enhanced sky's row it stands beside, which moved every row below it down one; the 10th is SIB1 - Seasons of the Iliac Bay ported 1:1 with permission, 2026-09-05, its textures the player's to supply - inserted directly under DS1, moving every row below down one more; the 11th is ECV1 - Enhanced Combat Visuals, 2026-09-07, the enhanced skin's draw of a concealed foe - inserted directly under SIB1, moving every row below down one more; the 12th is VC - Volumetric Clouds, 2026-09-07, the arc's row, inserted directly under ECV1, moving every row below down one more; the 2nd is the re-integrated road system, inserted directly under the struck ROADS row it supersedes, added by AUDIT 58 F5; the 74th is the pause window's version line, appended by AUDIT 58's records lane, which found `src/ui/pauseWindow.js` citing a Ledger A approval that had never been written down; rows 75-82 are AUDIT 58's seams lane, which found EIGHT more files declaring a departure as already RECORDED - music replacement, the fuzzy find, travel weather, the conversation save, the colour picker, async art in two windows, the town map's plate and arrow, and the input ladder's E and swing - against a section A that rowed none of them; rows 83-85 are ROAD-G G7's records lane, which widened the doctrine gate from the DEPARTURE token to the `Ledger A` CITE and found seventeen files claiming an approval this page did not name - the `AdvancedClimbing` scaffolding, the merchant service popup's accelerators and the port's scalar clock are the three that were genuinely owed a row rather than a roster line or a corrected comment; the 86th is AUDIT 64 F29's biography name seed, 2026-09-08, appended at the table's foot - `(uint)BiogFile.GetHashCode()` is a CLR identity hash and belongs to neither arm of the ENGINE-PRNG rule) plus the DUNGEON SEED bullet, now naming its two files, and the
...
- **The voxel character engine** (Mac's system, Port-Doctrine). 86
  ...
  by a game host: `worldModes.js:4021` passes `voxelfolk`,
  `scenes/interiorContext.js:455` gates on it, and no default route sets
...
- **`FaceUVTool` arithmetic** - JS doubles for C# float mix, `Math.trunc`
  ...
  precision are ledger row `:519`, narrowed at E8 and the only part of

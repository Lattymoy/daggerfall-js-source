...
the same place - `Port-Ledger.md:683`, the struck keybinding-registry
...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:6273`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:3694` carries the sweep and `hostMagic.js:608` its missile half. |
...
| **characters-voxel** | Departure (deliberate) | **Departure, still editor-only** | 86 designs across seven tables - 42 of the 43 monster mobiles, 19 class, 25 villager. The rig is 1,791 lines across 7 modules and is still gated: `worldModes.js:4401` passes `voxelfolk`, `scenes/interiorContext.js:503` consumes it. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:2400`, `:2116`). Ledger row `:629`'s residue list is spent - see list 2. |
...
| **ui-hud** | Verbatim (classic) / Partial (enhanced default) | **Verbatim on both skins** | The skin fork moved BELOW the game-state seams: `ui/hud.js:486` runs `updateHudVitals` and `:541` `drawNearDeathFlicker`, both above the enhanced branch at `:593` and above the `!art` return, and the enhanced HUD takes the Detect markers at `:608`. `lastHealthLost()` is no longer pinned at 0, so `CameraRecoiler` lives in all three hosts. |
...
that own overlay slots - `worldModes.js:78` and `dungeonContext.js:40` -
...
- ~~**`src/scenes/dungeonContext.js:1988`** - the two window seams this
  ...
  carries the same closure against Ledger row `:692`. The two WINDOWS
  ...
  `test/x11b.test.js`. This was also Ledger row `:692`'s adjudication,
  and that row is struck with it.
- **`src/ui/enhancedMenu.js:2528`** - the rest of the keyboard; the
...
- **`src/scenes/exterior.js:1971`** - Recall pends here; the anchor
...
- ~~**`src/scenes/exterior.js:1744`** - PX3: this test host mounts no
  ...
  site. (The `src/scenes/worldModes.js:2013` row below is struck: ROAD-E
...
- ~~**`src/ui/hudLarge.js:75`** - the docked bar occludes, it does not
  ...
  (`render/renderer.js:1925, :1859, :2672, :2984`) and the 2D passes
...
- **`src/scenes/dungeonContext.js:1129`** - the dungeon's non-layout foe
...
regenerated onto the new sites: ~~`exterior.js:1424` -> `:1201`~~
(**CLOSED at TP2**: the Recall interim narrowed to the one arm this host
cannot take, `exterior.js:1766`), ~~`exterior.js:1816` -> `:1504`~~
(**CLOSED at QX1**: this host took the quest bridge, so the pause window's
Quests tab has a machine to read), ~~`world.js:3652` -> `:3649`~~
...
Section C's table is **247 rows** between `Port-Ledger.md:566` and `:812`
...
route** - plus `:683`, struck at its head but carrying a live PENDING
...
1. **`:601` FaceUVTool's 1,803-UV residual at matched precision** ->
...
2. **`:614` the custom builder's hidden `ResetBonusPool` control** (STRUCK at E2, landing after this list was written: the control is live) ->
...
3. ~~**`:584` remainder: the overlay mouse-UP seam** -> UI arc. A7 shipped
...
4. ~~**`:627` the quest machine** -> `playSound`'s busy-skip, the one
...
5. ~~**`:680` the classic `.SAV` reader** -> the phone path: no zip arm in
   the saves picker, a desktop-first charter call.~~ **CLOSED (OT1, 2026-09-10).**
6. **`:683` (struck, with a live clause) the keybinding registry** ->
   the mouse/advanced and joystick sub-windows, the two of DFU's 65
   game windows the port does not cite.
7. ~~**`:692` the standalone dungeon host has no trade window** -> a
...
8. ~~**`:693` three stale probes**~~ **CLOSED (E-group, 2026-09-02).**
...
9. **`:579` UseItem's unbuilt destinations.** Every arm the row names is
   ...
   (`ui/nativeInventory.js:778-782`, `scenes/world.js:3794`), the
...
10. **`:629` fast-travel residue.** Its three named survivors are spent:
    ...
    `scenes/world.js:4859` (the arrival clamp anchoring the encounter
    clock, so the traveled window is not replayed) and `:1971` (the
    `scenes/world.js:4859` (the arrival clamp anchoring the encounter
    clock, so the traveled window is not replayed) and `:1917` (the
    ":524-525" clear that lets spawns resume). AUDIT 58 re-resolved this
    pair - both cites had drifted off the lines they name.
11. **`:634` `PatchRegionIndex` legacy-save fix.** Ported verbatim at
    `src/formats/mapsFile.js:104`, with the C# line range cited.
12. **`:678` the magic crafting windows.** The row's FLAGGED residue is
    ...
    clause goes with it.**
13. **`:628` the talk manager.** The row's own tail already says both
...
14. **`:658` the small-residue trio** (biography GP arm, arrow roll
    ...
    `BIOG*.TXT` files; the faceUV guard rides row `:601`.
...
- **The voxel character engine** (Mac's system, Port-Doctrine). 86
  ...
  by a game host: `worldModes.js:4401` passes `voxelfolk`,
...
- **`FaceUVTool` arithmetic** - JS doubles for C# float mix, `Math.trunc`
  ...
  precision are ledger row `:601`, narrowed at E8 and the only part of

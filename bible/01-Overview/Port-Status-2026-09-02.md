...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:7671`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:4765` carries the sweep and `hostMagic.js:792` its missile half. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:3171`, `:2879`). Ledger row `:668`'s residue list is spent - see list 2. |
...
- ~~**`src/scenes/world.js:4431`** - the port's default landing stands
...
Quests tab has a machine to read), ~~`world.js:4458` -> `:4481`~~
...
9. **`:618` UseItem's unbuilt destinations.** Every arm the row names is
   ...
   (`ui/nativeInventory.js:834-838`, `scenes/world.js:4860`), the
...
10. **`:668` fast-travel residue.** Its three named survivors are spent:
    ...
    `scenes/world.js:4043` (the arrival clamp anchoring the encounter

...
the same place - `Port-Ledger.md:655`, the struck keybinding-registry
...
| **formats-game** | Verbatim readers / broken consumer | **Verbatim** | The `ItemRecord` conversions were in the wave; ROAD-A A4 took the classic-import stragglers (building-level MAPSAVE, the native bank record, `LegacyArtifactIndexBitfieldCheck`). ~~Ledger row `:652`'s one residue is the phone path - no zip arm in the saves picker, a desktop-first charter call.~~ **CLOSED (OT1, 2026-09-10): the saves picker took the ARENA2 door's zip arm, and the row is struck whole.** |
| **formats-mw** | Departure lane, faithful within it | **Departure lane, now consumed** | `clipSweepTimes` has a production caller: `combat/fpArm.js:1678` runs the whole-clip reach sweep the superseded page said had never run in the game. 17 modules / 8,842 lines. |
...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:5138`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:3016` carries the sweep and `hostMagic.js:608` its missile half. |
...
| **characters-voxel** | Departure (deliberate) | **Departure, still editor-only** | 86 designs across seven tables - 42 of the 43 monster mobiles, 19 class, 25 villager. The rig is 1,791 lines across 7 modules and is still gated: `worldModes.js:4286` passes `voxelfolk`, `scenes/interiorContext.js:503` consumes it. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:1992`, `:1760`). Ledger row `:601`'s residue list is spent - see list 2. |
...
and `world.js:200` reaches it by mounting `worldModes`. But `grep -rn
...
- ~~**`src/scenes/dungeonContext.js:1905`** - the two window seams this
  ...
  carries the same closure against Ledger row `:664`. The two WINDOWS
  ...
  `test/x11b.test.js`. This was also Ledger row `:664`'s adjudication,
  and that row is struck with it.
- **`src/ui/enhancedMenu.js:2423`** - the rest of the keyboard; the
...
- **`src/scenes/exterior.js:1774`** - Recall pends here; the anchor
...
- ~~**`src/scenes/exterior.js:1646`** - PX3: this test host mounts no
  ...
  site. (The `src/scenes/worldModes.js:1923` row below is struck: ROAD-E
...
- ~~**`src/scenes/world.js:3266`** - the port's default landing stands
...
- ~~**`src/ui/hudLarge.js:75`** - the docked bar occludes, it does not
  ...
  (`render/renderer.js:1574, :1548, :2048, :2224`) and the 2D passes
...
regenerated onto the new sites: ~~`exterior.js:1313` -> `:1149`~~
...
Section C's table is **247 rows** between `Port-Ledger.md:538` and `:784`
...
route** - plus `:655`, struck at its head but carrying a live PENDING
...
1. **`:573` FaceUVTool's 1,803-UV residual at matched precision** ->
...
2. **`:586` the custom builder's hidden `ResetBonusPool` control** (STRUCK at E2, landing after this list was written: the control is live) ->
...
3. ~~**`:556` remainder: the overlay mouse-UP seam** -> UI arc. A7 shipped
...
4. ~~**`:599` the quest machine** -> `playSound`'s busy-skip, the one
...
5. ~~**`:652` the classic `.SAV` reader** -> the phone path: no zip arm in
   the saves picker, a desktop-first charter call.~~ **CLOSED (OT1, 2026-09-10).**
6. **`:655` (struck, with a live clause) the keybinding registry** ->
   the mouse/advanced and joystick sub-windows, the two of DFU's 65
   game windows the port does not cite.
7. ~~**`:664` the standalone dungeon host has no trade window** -> a
...
8. ~~**`:665` three stale probes**~~ **CLOSED (E-group, 2026-09-02).**
...
9. **`:551` UseItem's unbuilt destinations.** Every arm the row names is
   built: `DrinkPotion` (`systems/useItem.js:182`, `:260-270`),
   `RecordLocationFromMap`/`DiscoverRandomLocation`
   (`ui/nativeInventory.js:726-730`, `scenes/world.js:3088`), the
   (`ui/nativeInventory.js:726-730`, `scenes/world.js:3011`), the
...
10. **`:601` fast-travel residue.** Its three named survivors are spent:
    ...
    `scenes/world.js:4134` (the arrival clamp anchoring the encounter
    clock, so the traveled window is not replayed) and `:1966` (the
    `scenes/world.js:4078` (the arrival clamp anchoring the encounter
...
11. **`:606` `PatchRegionIndex` legacy-save fix.** Ported verbatim at
    `src/formats/mapsFile.js:104`, with the C# line range cited.
12. **`:650` the magic crafting windows.** The row's FLAGGED residue is
    three items and all three are answered: spell icons ship and are
    drawn (`ui/spellIcons.js`, imported at
    `ui/spellbookWindow.js:135-137`, drawn at `:997`), the icon picker
    ...
    clause goes with it.**
13. **`:600` the talk manager.** The row's own tail already says both
...
14. **`:630` the small-residue trio** (biography GP arm, arrow roll
...
- **The voxel character engine** (Mac's system, Port-Doctrine). 86
  ...
  by a game host: `worldModes.js:4286` passes `voxelfolk`,
...
- **`FaceUVTool` arithmetic** - JS doubles for C# float mix, `Math.trunc`
  ...
  precision are ledger row `:573`, narrowed at E8 and the only part of

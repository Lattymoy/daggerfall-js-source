...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:6858`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:4098` carries the sweep and `hostMagic.js:662` its missile half. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:2669`, `:2386`). Ledger row `:637`'s residue list is spent - see list 2. |
...
and `world.js:260` reaches it by mounting `worldModes`. But `grep -rn
...
9. **`:587` UseItem's unbuilt destinations.** Every arm the row names is
   ...
   (`ui/nativeInventory.js:796-800`, `scenes/world.js:4152`), the
...
10. **`:637` fast-travel residue.** Its three named survivors are spent:
    ...
    `scenes/world.js:3371` (the arrival clamp anchoring the encounter

...
the same place - `Port-Ledger.md:724`, the struck keybinding-registry
...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:8294`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:5298` carries the sweep and `hostMagic.js:792` its missile half. |
...
| **characters-voxel** | Departure (deliberate) | **Departure, still editor-only** | 86 designs across seven tables - 42 of the 43 monster mobiles, 19 class, 25 villager. The rig is 1,791 lines across 7 modules and is still gated: `worldModes.js:5302` passes `voxelfolk`, `scenes/interiorContext.js:577` consumes it. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:3682`, `:3376`). Ledger row `:670`'s residue list is spent - see list 2. |
...
and `world.js:287` reaches it by mounting `worldModes`. But `grep -rn
...
- ~~**`src/scenes/dungeonContext.js:2126`** - the two window seams this
  ...
  carries the same closure against Ledger row `:733`. The two WINDOWS
  ...
  `test/x11b.test.js`. This was also Ledger row `:733`'s adjudication,
...
- ~~**`src/scenes/exterior.js:1945`** - PX3: this test host mounts no
  ...
  site. (The `src/scenes/worldModes.js:2373` row below is struck: ROAD-E
...
- **`src/scenes/dungeonContext.js:1230`** - the dungeon's non-layout foe
...
Section C's table is **247 rows** between `Port-Ledger.md:607` and `:853`
...
route** - plus `:724`, struck at its head but carrying a live PENDING
...
1. **`:642` FaceUVTool's 1,803-UV residual at matched precision** ->
...
2. **`:655` the custom builder's hidden `ResetBonusPool` control** (STRUCK at E2, landing after this list was written: the control is live) ->
...
3. ~~**`:625` remainder: the overlay mouse-UP seam** -> UI arc. A7 shipped
...
4. ~~**`:668` the quest machine** -> `playSound`'s busy-skip, the one
...
5. ~~**`:721` the classic `.SAV` reader** -> the phone path: no zip arm in
   the saves picker, a desktop-first charter call.~~ **CLOSED (OT1, 2026-09-10).**
6. **`:724` (struck, with a live clause) the keybinding registry** ->
   the mouse/advanced and joystick sub-windows, the two of DFU's 65
   game windows the port does not cite.
7. ~~**`:733` the standalone dungeon host has no trade window** -> a
...
8. ~~**`:734` three stale probes**~~ **CLOSED (E-group, 2026-09-02).**
...
9. **`:620` UseItem's unbuilt destinations.** Every arm the row names is
   built: `DrinkPotion` (`systems/useItem.js:205`, `:296-306`),
   `RecordLocationFromMap`/`DiscoverRandomLocation`
   (`ui/nativeInventory.js:842-846`, `scenes/world.js:5352`), the
...
10. **`:670` fast-travel residue.** Its three named survivors are spent:
    ...
    `scenes/world.js:4532` (the arrival clamp anchoring the encounter
...
11. **`:675` `PatchRegionIndex` legacy-save fix.** Ported verbatim at
    `src/formats/mapsFile.js:107`, with the C# line range cited.
12. **`:719` the magic crafting windows.** The row's FLAGGED residue is
    ...
    clause goes with it.**
13. **`:669` the talk manager.** The row's own tail already says both
...
14. **`:699` the small-residue trio** (biography GP arm, arrow roll
...
- **The voxel character engine** (Mac's system, Port-Doctrine). 86
  ...
  by a game host: `worldModes.js:5612` passes `voxelfolk`,
...
- **`FaceUVTool` arithmetic** - JS doubles for C# float mix, `Math.trunc`
  ...
  precision are ledger row `:642`, narrowed at E8 and the only part of

...
the same place - `Port-Ledger.md:719`, the struck keybinding-registry
...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:7600`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:4676` carries the sweep and `hostMagic.js:792` its missile half. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:3081`, `:2789`). Ledger row `:665`'s residue list is spent - see list 2. |
...
- ~~**`src/scenes/dungeonContext.js:2126`** - the two window seams this
  ...
  carries the same closure against Ledger row `:728`. The two WINDOWS
  ...
  `test/x11b.test.js`. This was also Ledger row `:728`'s adjudication,
...
Section C's table is **247 rows** between `Port-Ledger.md:602` and `:848`
...
route** - plus `:719`, struck at its head but carrying a live PENDING
...
1. **`:637` FaceUVTool's 1,803-UV residual at matched precision** ->
...
2. **`:650` the custom builder's hidden `ResetBonusPool` control** (STRUCK at E2, landing after this list was written: the control is live) ->
...
3. ~~**`:620` remainder: the overlay mouse-UP seam** -> UI arc. A7 shipped
...
4. ~~**`:663` the quest machine** -> `playSound`'s busy-skip, the one
...
5. ~~**`:716` the classic `.SAV` reader** -> the phone path: no zip arm in
   the saves picker, a desktop-first charter call.~~ **CLOSED (OT1, 2026-09-10).**
6. **`:719` (struck, with a live clause) the keybinding registry** ->
   the mouse/advanced and joystick sub-windows, the two of DFU's 65
   game windows the port does not cite.
7. ~~**`:728` the standalone dungeon host has no trade window** -> a
...
8. ~~**`:729` three stale probes**~~ **CLOSED (E-group, 2026-09-02).**
...
9. **`:615` UseItem's unbuilt destinations.** Every arm the row names is
...
10. **`:665` fast-travel residue.** Its three named survivors are spent:
...
11. **`:670` `PatchRegionIndex` legacy-save fix.** Ported verbatim at
    `src/formats/mapsFile.js:107`, with the C# line range cited.
12. **`:714` the magic crafting windows.** The row's FLAGGED residue is
    ...
    clause goes with it.**
13. **`:664` the talk manager.** The row's own tail already says both
...
14. **`:694` the small-residue trio** (biography GP arm, arrow roll
...
- **`FaceUVTool` arithmetic** - JS doubles for C# float mix, `Math.trunc`
  ...
  precision are ledger row `:637`, narrowed at E8 and the only part of

...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:6141`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:3622` carries the sweep and `hostMagic.js:608` its missile half. |
| **scenes-modes** | Solid, pause parity broken | **Near-1:1** | ROAD-B B1 put `UserInterfaceManager`'s real stack under this host's slot (`ui/windowStack.js`, 295 lines, imported at `worldModes.js:75`). See "the pause primitive" below - the stack exists, its `paused()` member has no reader. |
...
| **characters-voxel** | Departure (deliberate) | **Departure, still editor-only** | 86 designs across seven tables - 42 of the 43 monster mobiles, 19 class, 25 villager. The rig is 1,791 lines across 7 modules and is still gated: `worldModes.js:4365` passes `voxelfolk`, `scenes/interiorContext.js:503` consumes it. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:2333`, `:2050`). Ledger row `:622`'s residue list is spent - see list 2. |
...
that own overlay slots - `worldModes.js:75` and `dungeonContext.js:39` -
and `world.js:235` reaches it by mounting `worldModes`. But `grep -rn
...
- **`src/scenes/exterior.js:1949`** - Recall pends here; the anchor
...
- ~~**`src/scenes/exterior.js:1690`** - PX3: this test host mounts no
  ...
  site. (The `src/scenes/worldModes.js:1988` row below is struck: ROAD-E
...
- ~~**`src/scenes/world.js:3557`** - the port's default landing stands
...
- ~~**`src/characters/enemyCasting.js:91`** - now exactly one term
  ...
  second.*
- ~~**`src/scenes/worldModes.js:1903`** - above ground only:
...
- ~~**`src/ui/hudLarge.js:75`** - the docked bar occludes, it does not
  ...
  (`render/renderer.js:1784, :1718, :2531, :2675`) and the 2D passes
...
regenerated onto the new sites: ~~`exterior.js:1370` -> `:1197`~~
(**CLOSED at TP2**: the Recall interim narrowed to the one arm this host
cannot take, `exterior.js:1712`), ~~`exterior.js:1762` -> `:1450`~~
(**CLOSED at QX1**: this host took the quest bridge, so the pause window's
Quests tab has a machine to read), ~~`world.js:3584` -> `:3580`~~
(**CLOSED at the ship landing**), ~~`worldModes.js:1866` -> `:1759`~~
...
- ~~**`src/scenes/worldModes.js:1973`** - above ground only: the GUILD
...
9. **`:572` UseItem's unbuilt destinations.** Every arm the row names is
   ...
   (`ui/nativeInventory.js:736-740`, `scenes/world.js:3692`), the
...
10. **`:622` fast-travel residue.** Its three named survivors are spent:
    ...
    `scenes/world.js:4755` (the arrival clamp anchoring the encounter
...
- **The voxel character engine** (Mac's system, Port-Doctrine). 86
  ...
  by a game host: `worldModes.js:4365` passes `voxelfolk`,

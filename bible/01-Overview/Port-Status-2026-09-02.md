...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:4228`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:2527` carries the sweep and `hostMagic.js:594` its missile half. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:1607`, `:1667`). Ledger row `:544`'s residue list is spent - see list 2. |
...
- ~~**`src/scenes/dungeonContext.js:1684`** - the two window seams this
...
- **`src/scenes/exterior.js:1449`** - Recall pends here; the anchor
...
- ~~**`src/scenes/exterior.js:1339`** - PX3: this test host mounts no
...
- ~~**`src/scenes/world.js:2779`** - the port's default landing stands
...
regenerated onto the new sites: ~~`exterior.js:1118` -> `:1089`~~
(**CLOSED at TP2**: the Recall interim narrowed to the one arm this
host cannot take, `exterior.js:1342`), ~~`exterior.js:1385` ->
`:1344`~~ (**CLOSED at QX1**: this host took the quest bridge, so the
pause window's Quests tab has a machine to read), `world.js:2825` ->
...
9. **`:494` UseItem's unbuilt destinations.** Every arm the row names is
   ...
   (`ui/nativeInventory.js:670-674`, `scenes/world.js:2576`), the
   (`ui/nativeInventory.js:670-674`, `scenes/world.js:2522`), the
...
10. **`:544` fast-travel residue.** Its three named survivors are spent:
    ...
    `scenes/world.js:3496` (the arrival clamp anchoring the encounter

...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:4516`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:2682` carries the sweep and `hostMagic.js:601` its missile half. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:1724`, `:1667`). Ledger row `:549`'s residue list is spent - see list 2. |
...
and `world.js:178` reaches it by mounting `worldModes`. But `grep -rn
...
- **`src/ui/enhancedMenu.js:1768`** - the rest of the keyboard; the
...
- **`src/scenes/exterior.js:1591`** - Recall pends here; the anchor
...
- ~~**`src/scenes/exterior.js:1481`** - PX3: this test host mounts no
...
- ~~**`src/scenes/world.js:2934`** - the port's default landing stands
...
regenerated onto the new sites: ~~`exterior.js:1150` -> `:1089`~~
(**CLOSED at TP2**: the Recall interim narrowed to the one arm this
host cannot take, `exterior.js:1484`), ~~`exterior.js:1527` ->
`:1346`~~ (**CLOSED at QX1**: this host took the quest bridge, so the
pause window's Quests tab has a machine to read), `world.js:2980` ->
...
9. **`:499` UseItem's unbuilt destinations.** Every arm the row names is
   ...
   (`ui/nativeInventory.js:670-674`, `scenes/world.js:2731`), the
   (`ui/nativeInventory.js:670-674`, `scenes/world.js:2677`), the
...
10. **`:549` fast-travel residue.** Its three named survivors are spent:
    ...
    `scenes/world.js:3663` (the arrival clamp anchoring the encounter
    clock, so the traveled window is not replayed) and `:1966` (the
    `scenes/world.js:3616` (the arrival clamp anchoring the encounter

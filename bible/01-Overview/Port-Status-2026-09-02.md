...
the same place - `Port-Ledger.md:687`, the struck keybinding-registry
...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:6490`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:3878` carries the sweep and `hostMagic.js:608` its missile half. |
...
| **characters-voxel** | Departure (deliberate) | **Departure, still editor-only** | 86 designs across seven tables - 42 of the 43 monster mobiles, 19 class, 25 villager. The rig is 1,791 lines across 7 modules and is still gated: `worldModes.js:5194` passes `voxelfolk`, `scenes/interiorContext.js:503` consumes it. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:2464`, `:2181`). Ledger row `:633`'s residue list is spent - see list 2. |
...
and `world.js:253` reaches it by mounting `worldModes`. But `grep -rn
...
- ~~**`src/scenes/dungeonContext.js:2097`** - the two window seams this
  ...
  carries the same closure against Ledger row `:696`. The two WINDOWS
  ...
  `test/x11b.test.js`. This was also Ledger row `:696`'s adjudication,
  and that row is struck with it.
- **`src/ui/enhancedMenu.js:2967`** - the rest of the keyboard; the
...
- ~~**`src/scenes/exterior.js:1754`** - PX3: this test host mounts no
  ...
  site. (The `src/scenes/worldModes.js:2230` row below is struck: ROAD-E
...
- ~~**`src/scenes/world.js:3645`** - the port's default landing stands
...
- ~~**`src/characters/enemyCasting.js:91`** - now exactly one term
  ...
  second.*
- ~~**`src/scenes/worldModes.js:1934`** - above ground only:
  `QuestMachine.SetupIndividualStaticNPC`. *Multi-host. The law is
  ported and idle at `systems/quest/machine.js:753` including the
...
- ~~**`src/scenes/worldModes.js:2004`** - above ground only: the GUILD
...
Section C's table is **247 rows** between `Port-Ledger.md:570` and `:816`
...
generation, to the enhanced lane)~~ (**it read 226 and 21 until `:559` was
...
route** - plus `:687`, struck at its head but carrying a live PENDING
...
1. **`:605` FaceUVTool's 1,803-UV residual at matched precision** ->
...
2. **`:618` the custom builder's hidden `ResetBonusPool` control** (STRUCK at E2, landing after this list was written: the control is live) ->
   ...
   `systems/dialogShortcuts.js:194`/`:320` carries `ResetBonusPool` with
...
3. ~~**`:588` remainder: the overlay mouse-UP seam** -> UI arc. A7 shipped
...
4. ~~**`:631` the quest machine** -> `playSound`'s busy-skip, the one
...
5. ~~**`:684` the classic `.SAV` reader** -> the phone path: no zip arm in
   the saves picker, a desktop-first charter call.~~ **CLOSED (OT1, 2026-09-10).**
6. **`:687` (struck, with a live clause) the keybinding registry** ->
   the mouse/advanced and joystick sub-windows, the two of DFU's 65
   game windows the port does not cite.
7. ~~**`:696` the standalone dungeon host has no trade window** -> a
...
8. ~~**`:697` three stale probes**~~ **CLOSED (E-group, 2026-09-02).**
...
9. **`:583` UseItem's unbuilt destinations.** Every arm the row names is
   built: `DrinkPotion` (`systems/useItem.js:197`, `:275-285`),
   `RecordLocationFromMap`/`DiscoverRandomLocation`
   (`ui/nativeInventory.js:796-800`, `scenes/world.js:3932`), the
...
10. **`:633` fast-travel residue.** Its three named survivors are spent:
    ...
    `scenes/world.js:3155` (the arrival clamp anchoring the encounter
...
11. **`:638` `PatchRegionIndex` legacy-save fix.** Ported verbatim at
    `src/formats/mapsFile.js:104`, with the C# line range cited.
12. **`:682` the magic crafting windows.** The row's FLAGGED residue is
    three items and all three are answered: spell icons ship and are
    drawn (`ui/spellIcons.js`, imported at
    `ui/spellbookWindow.js:136-138`, drawn at `:1014`), the icon picker
    ...
    clause goes with it.**
13. **`:632` the talk manager.** The row's own tail already says both
...
14. **`:662` the small-residue trio** (biography GP arm, arrow roll
...
- **The voxel character engine** (Mac's system, Port-Doctrine). 86
  ...
  by a game host: `worldModes.js:5194` passes `voxelfolk`,
...
- **`FaceUVTool` arithmetic** - JS doubles for C# float mix, `Math.trunc`
  ...
  precision are ledger row `:605`, narrowed at E8 and the only part of

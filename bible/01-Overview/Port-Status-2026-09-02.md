...
the same place - `Port-Ledger.md:696`, the struck keybinding-registry
...
written by both curses (`systems/vampirism.js:163`,
...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:7444`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:4620` carries the sweep and `hostMagic.js:711` its missile half. |
...
| **characters-voxel** | Departure (deliberate) | **Departure, still editor-only** | 86 designs across seven tables - 42 of the 43 monster mobiles, 19 class, 25 villager. The rig is 1,791 lines across 7 modules and is still gated: `worldModes.js:5261` passes `voxelfolk`, `scenes/interiorContext.js:513` consumes it. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:3058`, `:2766`). Ledger row `:642`'s residue list is spent - see list 2. |
...
- ~~**`src/scenes/dungeonContext.js:2117`** - the two window seams this
  ...
  carries the same closure against Ledger row `:705`. The two WINDOWS
  ...
  `test/x11b.test.js`. This was also Ledger row `:705`'s adjudication,
...
- **`src/scenes/exterior.js:2243`** - Recall pends here; the anchor
...
- ~~**`src/scenes/exterior.js:1883`** - PX3: this test host mounts no
  ...
  site. (The `src/scenes/worldModes.js:2262` row below is struck: ROAD-E
...
- ~~**`src/scenes/world.js:3826`** - the port's default landing stands
...
- ~~**`src/characters/enemyCasting.js:91`** - now exactly one term
  ...
  second.*
- ~~**`src/scenes/worldModes.js:1948`** - above ground only:
...
- **`src/scenes/dungeonContext.js:1173`** - the dungeon's non-layout foe
...
regenerated onto the new sites: ~~`exterior.js:1542` -> `:1282`~~
(**CLOSED at TP2**: the Recall interim narrowed to the one arm this host
cannot take, `exterior.js:1905`), ~~`exterior.js:1955` -> `:1670`~~
(**CLOSED at QX1**: this host took the quest bridge, so the pause window's
Quests tab has a machine to read), ~~`world.js:3853` -> `:4145`~~
(**CLOSED at the ship landing**), ~~`worldModes.js:1911` -> `:1788`~~
...
- ~~**`src/scenes/worldModes.js:2018`** - above ground only: the GUILD
...
Section C's table is **247 rows** between `Port-Ledger.md:579` and `:825`
...
route** - plus `:696`, struck at its head but carrying a live PENDING
...
1. **`:614` FaceUVTool's 1,803-UV residual at matched precision** ->
...
2. **`:627` the custom builder's hidden `ResetBonusPool` control** (STRUCK at E2, landing after this list was written: the control is live) ->
...
3. ~~**`:597` remainder: the overlay mouse-UP seam** -> UI arc. A7 shipped
...
4. ~~**`:640` the quest machine** -> `playSound`'s busy-skip, the one
...
5. ~~**`:693` the classic `.SAV` reader** -> the phone path: no zip arm in
   the saves picker, a desktop-first charter call.~~ **CLOSED (OT1, 2026-09-10).**
6. **`:696` (struck, with a live clause) the keybinding registry** ->
   the mouse/advanced and joystick sub-windows, the two of DFU's 65
   game windows the port does not cite.
7. ~~**`:705` the standalone dungeon host has no trade window** -> a
...
8. ~~**`:706` three stale probes**~~ **CLOSED (E-group, 2026-09-02).**
...
9. **`:592` UseItem's unbuilt destinations.** Every arm the row names is
   built: `DrinkPotion` (`systems/useItem.js:198`, `:285-295`),
   `RecordLocationFromMap`/`DiscoverRandomLocation`
   (`ui/nativeInventory.js:809-813`, `scenes/world.js:4674`), the
...
10. **`:642` fast-travel residue.** Its three named survivors are spent:
    ...
    `scenes/world.js:3869` (the arrival clamp anchoring the encounter
...
11. **`:647` `PatchRegionIndex` legacy-save fix.** Ported verbatim at
    `src/formats/mapsFile.js:105`, with the C# line range cited.
12. **`:691` the magic crafting windows.** The row's FLAGGED residue is
    ...
    clause goes with it.**
13. **`:641` the talk manager.** The row's own tail already says both
...
14. **`:671` the small-residue trio** (biography GP arm, arrow roll
...
- **The voxel character engine** (Mac's system, Port-Doctrine). 86
  ...
  by a game host: `worldModes.js:5261` passes `voxelfolk`,
...
- **`FaceUVTool` arithmetic** - JS doubles for C# float mix, `Math.trunc`
  ...
  precision are ledger row `:614`, narrowed at E8 and the only part of

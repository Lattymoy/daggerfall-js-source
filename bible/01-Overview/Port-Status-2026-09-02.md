...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:5057`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:3011` carries the sweep and `hostMagic.js:608` its missile half. |
| **scenes-modes** | Solid, pause parity broken | **Near-1:1** | ROAD-B B1 put `UserInterfaceManager`'s real stack under this host's slot (`ui/windowStack.js`, 295 lines, imported at `worldModes.js:67`). See "the pause primitive" below - the stack exists, its `paused()` member has no reader. |
...
| **characters-voxel** | Departure (deliberate) | **Departure, still editor-only** | 86 designs across seven tables - 42 of the 43 monster mobiles, 19 class, 25 villager. The rig is 1,791 lines across 7 modules and is still gated: `worldModes.js:4284` passes `voxelfolk`, `scenes/interiorContext.js:503` consumes it. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:1987`, `:1755`). Ledger row `:598`'s residue list is spent - see list 2. |
...
that own overlay slots - `worldModes.js:67` and `dungeonContext.js:38` -
...
- **`src/scenes/exterior.js:1772`** - Recall pends here; the anchor
...
- ~~**`src/scenes/exterior.js:1644`** - PX3: this test host mounts no
  ...
  site. (The `src/scenes/worldModes.js:1921` row below is struck: ROAD-E
...
- ~~**`src/ui/hudLarge.js:75`** - the docked bar occludes, it does not
  ...
  (`render/renderer.js:1523, :1497, :1995, :2171`) and the 2D passes
...
9. **`:548` UseItem's unbuilt destinations.** Every arm the row names is
   ...
   (`ui/nativeInventory.js:723-727`, `scenes/world.js:3060`), the
   (`ui/nativeInventory.js:723-727`, `scenes/world.js:3006`), the
...
10. **`:598` fast-travel residue.** Its three named survivors are spent:
    ...
    `scenes/world.js:4060` (the arrival clamp anchoring the encounter
    clock, so the traveled window is not replayed) and `:1966` (the
    `scenes/world.js:4004` (the arrival clamp anchoring the encounter
...
- **The voxel character engine** (Mac's system, Port-Doctrine). 86
  ...
  by a game host: `worldModes.js:4284` passes `voxelfolk`,

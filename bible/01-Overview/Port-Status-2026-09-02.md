...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:7517`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:4648` carries the sweep and `hostMagic.js:751` its missile half. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:3076`, `:2784`). Ledger row `:647`'s residue list is spent - see list 2. |
...
9. **`:597` UseItem's unbuilt destinations.** Every arm the row names is
   ...
   (`ui/nativeInventory.js:841-845`, `scenes/world.js:4702`), the
...
10. **`:647` fast-travel residue.** Its three named survivors are spent:
    ...
    `scenes/world.js:3895` (the arrival clamp anchoring the encounter

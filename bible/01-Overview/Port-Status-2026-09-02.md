...
and six rows** (five, since OT1 struck `:675`),
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:2397`, `:2114`). Ledger row `:625`'s residue list is spent - see list 2. |
...
generation, to the enhanced lane)~~ (**it read 226 and 21 until `:555` was
...
9. **`:575` UseItem's unbuilt destinations.** Every arm the row names is
   ...
   (`ui/nativeInventory.js:737-741`, `scenes/world.js:3776`), the

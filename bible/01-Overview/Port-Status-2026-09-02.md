...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:7602`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:4678` carries the sweep and `hostMagic.js:816` its missile half. |
...
| **characters-voxel** | Departure (deliberate) | **Departure, still editor-only** | 86 designs across seven tables - 42 of the 43 monster mobiles, 19 class, 25 villager. The rig is 1,791 lines across 7 modules and is still gated: `worldModes.js:5301` passes `voxelfolk`, `scenes/interiorContext.js:577` consumes it. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:3083`, `:2791`). Ledger row `:668`'s residue list is spent - see list 2. |
...
and `world.js:290` reaches it by mounting `worldModes`. But `grep -rn
...
- ~~**`src/scenes/exterior.js:1943`** - PX3: this test host mounts no
  ...
  site. (The `src/scenes/worldModes.js:2295` row below is struck: ROAD-E
...
9. **`:618` UseItem's unbuilt destinations.** Every arm the row names is
   ...
   (`ui/nativeInventory.js:834-838`, `scenes/world.js:4732`), the
...
10. **`:668` fast-travel residue.** Its three named survivors are spent:
    ...
    `scenes/world.js:3920` (the arrival clamp anchoring the encounter
...
- **The voxel character engine** (Mac's system, Port-Doctrine). 86
  ...
  by a game host: `worldModes.js:5301` passes `voxelfolk`,

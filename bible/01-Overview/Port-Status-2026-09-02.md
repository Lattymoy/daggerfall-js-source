...
the same place - `Port-Ledger.md:692`, the struck keybinding-registry
...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:6670`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:3909` carries the sweep and `hostMagic.js:711` its missile half. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:2476`, `:2193`). Ledger row `:638`'s residue list is spent - see list 2. |
...
| **ui-enhanced** | Departure lane | **Departure lane, scoped** | `ui/worldPlaque.js:305` puts the skin gate above `ensure()`, so the unscoped `*`/`html`/`body`/`button`/`#app` rules never reach the classic page. 9 modules / 8,940 lines. |
...
and `world.js:256` reaches it by mounting `worldModes`. But `grep -rn
...
- ~~**`src/scenes/dungeonContext.js:2114`** - the two window seams this
  ...
  carries the same closure against Ledger row `:701`. The two WINDOWS
  ...
  `test/x11b.test.js`. This was also Ledger row `:701`'s adjudication,
  and that row is struck with it.
- **`src/ui/enhancedMenu.js:3007`** - the rest of the keyboard; the
...
- **`src/scenes/exterior.js:2067`** - Recall pends here; the anchor
...
- ~~**`src/scenes/exterior.js:1757`** - PX3: this test host mounts no
...
- ~~**`src/scenes/world.js:3676`** - the port's default landing stands
...
regenerated onto the new sites: ~~`exterior.js:1437` -> `:1208`~~
...
Quests tab has a machine to read), ~~`world.js:3703` -> `:3696`~~
...
Section C's table is **247 rows** between `Port-Ledger.md:575` and `:821`
...
route** - plus `:692`, struck at its head but carrying a live PENDING
...
1. **`:610` FaceUVTool's 1,803-UV residual at matched precision** ->
...
2. **`:623` the custom builder's hidden `ResetBonusPool` control** (STRUCK at E2, landing after this list was written: the control is live) ->
...
3. ~~**`:593` remainder: the overlay mouse-UP seam** -> UI arc. A7 shipped
...
4. ~~**`:636` the quest machine** -> `playSound`'s busy-skip, the one
...
5. ~~**`:689` the classic `.SAV` reader** -> the phone path: no zip arm in
   the saves picker, a desktop-first charter call.~~ **CLOSED (OT1, 2026-09-10).**
6. **`:692` (struck, with a live clause) the keybinding registry** ->
   the mouse/advanced and joystick sub-windows, the two of DFU's 65
   game windows the port does not cite.
7. ~~**`:701` the standalone dungeon host has no trade window** -> a
...
8. ~~**`:702` three stale probes**~~ **CLOSED (E-group, 2026-09-02).**
...
9. **`:588` UseItem's unbuilt destinations.** Every arm the row names is
   built: `DrinkPotion` (`systems/useItem.js:197`, `:275-285`),
   `RecordLocationFromMap`/`DiscoverRandomLocation`
   (`ui/nativeInventory.js:796-800`, `scenes/world.js:3963`), the
...
10. **`:638` fast-travel residue.** Its three named survivors are spent:
    ...
    `scenes/world.js:3178` (the arrival clamp anchoring the encounter
...
11. **`:643` `PatchRegionIndex` legacy-save fix.** Ported verbatim at
    `src/formats/mapsFile.js:104`, with the C# line range cited.
12. **`:687` the magic crafting windows.** The row's FLAGGED residue is
    ...
    clause goes with it.**
13. **`:637` the talk manager.** The row's own tail already says both
...
14. **`:667` the small-residue trio** (biography GP arm, arrow roll
...
- **`FaceUVTool` arithmetic** - JS doubles for C# float mix, `Math.trunc`
  ...
  precision are ledger row `:610`, narrowed at E8 and the only part of

...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:5097`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:3009` carries the sweep and `hostMagic.js:608` its missile half. |
...
| **characters-voxel** | Departure (deliberate) | **Departure, still editor-only** | 86 designs across seven tables - 42 of the 43 monster mobiles, 19 class, 25 villager. The rig is 1,791 lines across 7 modules and is still gated: `worldModes.js:4283` passes `voxelfolk`, `scenes/interiorContext.js:503` consumes it. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:1985`, `:1753`). Ledger row `:598`'s residue list is spent - see list 2. |
...
and `world.js:198` reaches it by mounting `worldModes`. But `grep -rn
...
- **`src/scenes/exterior.js:1771`** - Recall pends here; the anchor
...
- ~~**`src/scenes/exterior.js:1645`** - PX3: this test host mounts no
  ...
  site. (The `src/scenes/worldModes.js:1920` row below is struck: ROAD-E
...
- ~~**`src/scenes/world.js:3259`** - the port's default landing stands
...
- ~~**`src/ui/hudLarge.js:75`** - the docked bar occludes, it does not
  ...
  (`render/renderer.js:1335, :1309, :1805, :1980`) and the 2D passes
...
Quests tab has a machine to read), ~~`world.js:3305` -> `:3120`~~
...
9. **`:548` UseItem's unbuilt destinations.** Every arm the row names is
   ...
   (`ui/nativeInventory.js:726-730`, `scenes/world.js:3058`), the
   (`ui/nativeInventory.js:726-730`, `scenes/world.js:3004`), the
...
10. **`:598` fast-travel residue.** Its three named survivors are spent:
    ...
    `scenes/world.js:4095` (the arrival clamp anchoring the encounter
    clock, so the traveled window is not replayed) and `:1966` (the
    `scenes/world.js:4039` (the arrival clamp anchoring the encounter
...
- **The voxel character engine** (Mac's system, Port-Doctrine). 86
  ...
  by a game host: `worldModes.js:4283` passes `voxelfolk`,

...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:6746`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:3981` carries the sweep and `hostMagic.js:662` its missile half. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:2480`, `:2196`). Ledger row `:636`'s residue list is spent - see list 2. |
...
and `world.js:262` reaches it by mounting `worldModes`. But `grep -rn
...
- **`src/scenes/exterior.js:2135`** - Recall pends here; the anchor
...
- ~~**`src/scenes/exterior.js:1825`** - PX3: this test host mounts no
...
- ~~**`src/scenes/world.js:3748`** - the port's default landing stands
...
regenerated onto the new sites: ~~`exterior.js:1505` -> `:1215`~~
(**CLOSED at TP2**: the Recall interim narrowed to the one arm this host
cannot take, `exterior.js:1847`), ~~`exterior.js:1897` -> `:1585`~~
(**CLOSED at QX1**: this host took the quest bridge, so the pause window's
Quests tab has a machine to read), ~~`world.js:3775` -> `:3768`~~
...
9. **`:586` UseItem's unbuilt destinations.** Every arm the row names is
   ...
   (`ui/nativeInventory.js:798-802`, `scenes/world.js:4035`), the
...
10. **`:636` fast-travel residue.** Its three named survivors are spent:
    ...
    `scenes/world.js:3254` (the arrival clamp anchoring the encounter

...
the same place - `Port-Ledger.md:652`, the struck keybinding-registry
...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:5061`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:3015` carries the sweep and `hostMagic.js:608` its missile half. |
| **scenes-modes** | Solid, pause parity broken | **Near-1:1** | ROAD-B B1 put `UserInterfaceManager`'s real stack under this host's slot (`ui/windowStack.js`, 295 lines, imported at `worldModes.js:68`). See "the pause primitive" below - the stack exists, its `paused()` member has no reader. |
...
| **characters-voxel** | Departure (deliberate) | **Departure, still editor-only** | 86 designs across seven tables - 42 of the 43 monster mobiles, 19 class, 25 villager. The rig is 1,791 lines across 7 modules and is still gated: `worldModes.js:4285` passes `voxelfolk`, `scenes/interiorContext.js:503` consumes it. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:1991`, `:1759`). Ledger row `:598`'s residue list is spent - see list 2. |
...
that own overlay slots - `worldModes.js:68` and `dungeonContext.js:38` -
and `world.js:199` reaches it by mounting `worldModes`. But `grep -rn
...
- ~~**`src/scenes/dungeonContext.js:1881`** - the two window seams this
  ...
  carries the same closure against Ledger row `:661`. The two WINDOWS
  ...
  `test/x11b.test.js`. This was also Ledger row `:661`'s adjudication,
  and that row is struck with it.
- **`src/ui/enhancedMenu.js:2413`** - the rest of the keyboard; the
...
- **`src/scenes/exterior.js:1773`** - Recall pends here; the anchor
...
- ~~**`src/scenes/exterior.js:1645`** - PX3: this test host mounts no
  ...
  site. (The `src/scenes/worldModes.js:1922` row below is struck: ROAD-E
...
- ~~**`src/scenes/world.js:3225`** - the port's default landing stands
...
- ~~**`src/ui/hudLarge.js:75`** - the docked bar occludes, it does not
  ...
  (`render/renderer.js:1550, :1524, :2022, :2198`) and the 2D passes
...
regenerated onto the new sites: ~~`exterior.js:1313` -> `:1148`~~
(**CLOSED at TP2**: the Recall interim narrowed to the one arm this host
cannot take, `exterior.js:1648`), ~~`exterior.js:1691` -> `:1400`~~
(**CLOSED at QX1**: this host took the quest bridge, so the pause window's
Quests tab has a machine to read), ~~`world.js:3274` -> `:3089`~~
(**CLOSED at the ship landing**), ~~`worldModes.js:1862` -> `:1718`~~
...
Section C's table is **247 rows** between `Port-Ledger.md:535` and `:781`
...
route** - plus `:652`, struck at its head but carrying a live PENDING
...
1. **`:570` FaceUVTool's 1,803-UV residual at matched precision** ->
...
2. **`:583` the custom builder's hidden `ResetBonusPool` control** (STRUCK at E2, landing after this list was written: the control is live) ->
...
3. ~~**`:553` remainder: the overlay mouse-UP seam** -> UI arc. A7 shipped
...
4. ~~**`:596` the quest machine** -> `playSound`'s busy-skip, the one
...
5. ~~**`:649` the classic `.SAV` reader** -> the phone path: no zip arm in
   the saves picker, a desktop-first charter call.~~ **CLOSED (OT1, 2026-09-10).**
6. **`:652` (struck, with a live clause) the keybinding registry** ->
   the mouse/advanced and joystick sub-windows, the two of DFU's 65
   game windows the port does not cite.
7. ~~**`:661` the standalone dungeon host has no trade window** -> a
...
8. ~~**`:662` three stale probes**~~ **CLOSED (E-group, 2026-09-02).**
...
9. **`:548` UseItem's unbuilt destinations.** Every arm the row names is
   built: `DrinkPotion` (`systems/useItem.js:167`, `:245-255`),
   `RecordLocationFromMap`/`DiscoverRandomLocation`
   (`ui/nativeInventory.js:723-727`, `scenes/world.js:3064`), the
   (`ui/nativeInventory.js:723-727`, `scenes/world.js:3010`), the
...
10. **`:598` fast-travel residue.** Its three named survivors are spent:
    ...
    `scenes/world.js:4064` (the arrival clamp anchoring the encounter
    clock, so the traveled window is not replayed) and `:1966` (the
    `scenes/world.js:4008` (the arrival clamp anchoring the encounter
...
11. **`:603` `PatchRegionIndex` legacy-save fix.** Ported verbatim at
    `src/formats/mapsFile.js:104`, with the C# line range cited.
12. **`:647` the magic crafting windows.** The row's FLAGGED residue is
    ...
    clause goes with it.**
13. **`:597` the talk manager.** The row's own tail already says both
...
14. **`:627` the small-residue trio** (biography GP arm, arrow roll
...
- **The voxel character engine** (Mac's system, Port-Doctrine). 86
  ...
  by a game host: `worldModes.js:4285` passes `voxelfolk`,
...
- **`FaceUVTool` arithmetic** - JS doubles for C# float mix, `Math.trunc`
  ...
  precision are ledger row `:570`, narrowed at E8 and the only part of

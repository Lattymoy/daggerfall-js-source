...
the same place - `Port-Ledger.md:643`, the struck keybinding-registry
...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:4907`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:2922` carries the sweep and `hostMagic.js:608` its missile half. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:1951`, `:1730`). Ledger row `:589`'s residue list is spent - see list 2. |
...
- ~~**`src/scenes/dungeonContext.js:1868`** - the two window seams this
  ...
  carries the same closure against Ledger row `:652`. The two WINDOWS
  ...
  `test/x11b.test.js`. This was also Ledger row `:652`'s adjudication,
...
- ~~**`src/scenes/exterior.js:1623`** - PX3: this test host mounts no
...
- ~~**`src/scenes/world.js:3153`** - the port's default landing stands
...
- ~~**`src/ui/hudLarge.js:75`** - the docked bar occludes, it does not
  ...
  (`render/renderer.js:1326, :1300, :1796, :1971`) and the 2D passes
...
regenerated onto the new sites: ~~`exterior.js:1291` -> `:1109`~~
(**CLOSED at TP2**: the Recall interim narrowed to the one arm this host
cannot take, `exterior.js:1626`), ~~`exterior.js:1669` -> `:1375`~~
(**CLOSED at QX1**: this host took the quest bridge, so the pause window's
Quests tab has a machine to read), ~~`world.js:3199` -> `:3007`~~
...
Section C's table is **247 rows** between `Port-Ledger.md:526` and `:772`
...
route** - plus `:643`, struck at its head but carrying a live PENDING
...
1. **`:561` FaceUVTool's 1,803-UV residual at matched precision** ->
...
2. **`:574` the custom builder's hidden `ResetBonusPool` control** (STRUCK at E2, landing after this list was written: the control is live) ->
...
3. ~~**`:544` remainder: the overlay mouse-UP seam** -> UI arc. A7 shipped
...
4. ~~**`:587` the quest machine** -> `playSound`'s busy-skip, the one
...
5. ~~**`:640` the classic `.SAV` reader** -> the phone path: no zip arm in
   the saves picker, a desktop-first charter call.~~ **CLOSED (OT1, 2026-09-10).**
6. **`:643` (struck, with a live clause) the keybinding registry** ->
   the mouse/advanced and joystick sub-windows, the two of DFU's 65
   game windows the port does not cite.
7. ~~**`:652` the standalone dungeon host has no trade window** -> a
...
8. ~~**`:653` three stale probes**~~ **CLOSED (E-group, 2026-09-02).**
...
9. **`:539` UseItem's unbuilt destinations.** Every arm the row names is
   built: `DrinkPotion` (`systems/useItem.js:167`, `:245-255`),
   `RecordLocationFromMap`/`DiscoverRandomLocation`
   (`ui/nativeInventory.js:704-708`, `scenes/world.js:2971`), the
   (`ui/nativeInventory.js:704-708`, `scenes/world.js:2917`), the
...
10. **`:589` fast-travel residue.** Its three named survivors are spent:
    ...
    `scenes/world.js:3882` (the arrival clamp anchoring the encounter
...
11. **`:594` `PatchRegionIndex` legacy-save fix.** Ported verbatim at
    `src/formats/mapsFile.js:104`, with the C# line range cited.
12. **`:638` the magic crafting windows.** The row's FLAGGED residue is
    ...
    clause goes with it.**
13. **`:588` the talk manager.** The row's own tail already says both
...
14. **`:618` the small-residue trio** (biography GP arm, arrow roll
...
- **`FaceUVTool` arithmetic** - JS doubles for C# float mix, `Math.trunc`
  ...
  precision are ledger row `:561`, narrowed at E8 and the only part of

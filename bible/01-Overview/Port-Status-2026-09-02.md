...
the same place - `Port-Ledger.md:609`, the struck keybinding-registry
...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:4805`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:2843` carries the sweep and `hostMagic.js:601` its missile half. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:1883`, `:1689`). Ledger row `:555`'s residue list is spent - see list 2. |
...
- ~~**`src/scenes/dungeonContext.js:1842`** - the two window seams this
  ...
  carries the same closure against Ledger row `:618`. The two WINDOWS
  ...
  `test/x11b.test.js`. This was also Ledger row `:618`'s adjudication,
...
- **`src/scenes/exterior.js:1719`** - Recall pends here; the anchor
...
- ~~**`src/scenes/exterior.js:1608`** - PX3: this test host mounts no
...
- ~~**`src/ui/hudLarge.js:75`** - the docked bar occludes, it does not
  ...
  (`render/renderer.js:1275, :1244, :1740, :1915`) and the 2D passes
...
regenerated onto the new sites: ~~`exterior.js:1277` -> `:1095`~~
(**CLOSED at TP2**: the Recall interim narrowed to the one arm this host
cannot take, `exterior.js:1611`), ~~`exterior.js:1654` -> `:1352`~~
...
Section C's table is **247 rows** between `Port-Ledger.md:492` and `:738`
...
route** - plus `:609`, struck at its head but carrying a live PENDING
...
1. **`:527` FaceUVTool's 1,803-UV residual at matched precision** ->
...
5. ~~**`:606` the classic `.SAV` reader** -> the phone path: no zip arm in
   the saves picker, a desktop-first charter call.~~ **CLOSED (OT1, 2026-09-10).**
6. **`:609` (struck, with a live clause) the keybinding registry** ->
   the mouse/advanced and joystick sub-windows, the two of DFU's 65
   game windows the port does not cite.
7. ~~**`:618` the standalone dungeon host has no trade window** -> a
...
8. ~~**`:619` three stale probes**~~ **CLOSED (E-group, 2026-09-02).**
...
10. **`:555` fast-travel residue.** Its three named survivors are spent:
    ...
    `scenes/world.js:3857` (the arrival clamp anchoring the encounter
    clock, so the traveled window is not replayed) and `:1966` (the
    `scenes/world.js:3801` (the arrival clamp anchoring the encounter
...
11. **`:560` `PatchRegionIndex` legacy-save fix.** Ported verbatim at
    `src/formats/mapsFile.js:104`, with the C# line range cited.
12. **`:604` the magic crafting windows.** The row's FLAGGED residue is
    ...
    clause goes with it.**
13. **`:554` the talk manager.** The row's own tail already says both
...
14. **`:584` the small-residue trio** (biography GP arm, arrow roll
...
- **`FaceUVTool` arithmetic** - JS doubles for C# float mix, `Math.trunc`
  ...
  precision are ledger row `:527`, narrowed at E8 and the only part of

...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:5042`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:3000` carries the sweep and `hostMagic.js:608` its missile half. |
...
| **characters-voxel** | Departure (deliberate) | **Departure, still editor-only** | 86 designs across seven tables - 42 of the 43 monster mobiles, 19 class, 25 villager. The rig is 1,791 lines across 7 modules and is still gated: `worldModes.js:4282` passes `voxelfolk`, `scenes/interiorContext.js:503` consumes it. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:1980`, `:1748`). Ledger row `:597`'s residue list is spent - see list 2. |
...
and `world.js:196` reaches it by mounting `worldModes`. But `grep -rn
...
- **`src/scenes/exterior.js:1769`** - Recall pends here; the anchor
...
- ~~**`src/scenes/exterior.js:1644`** - PX3: this test host mounts no
  ...
  site. (The `src/scenes/worldModes.js:1919` row below is struck: ROAD-E
...
9. **`:547` UseItem's unbuilt destinations.** Every arm the row names is
   ...
   (`ui/nativeInventory.js:723-727`, `scenes/world.js:3049`), the
   (`ui/nativeInventory.js:723-727`, `scenes/world.js:2995`), the
...
10. **`:597` fast-travel residue.** Its three named survivors are spent:
    ...
    `scenes/world.js:3993` (the arrival clamp anchoring the encounter
...
- **The voxel character engine** (Mac's system, Port-Doctrine). 86
  ...
  by a game host: `worldModes.js:4282` passes `voxelfolk`,

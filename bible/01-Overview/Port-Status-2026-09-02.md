...
the same place - `Port-Ledger.md:634`, the struck keybinding-registry
...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:4880`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:2892` carries the sweep and `hostMagic.js:608` its missile half. |
...
| **characters-voxel** | Departure (deliberate) | **Departure, still editor-only** | 86 designs across seven tables - 42 of the 43 monster mobiles, 19 class, 25 villager. The rig is 1,791 lines across 7 modules and is still gated: `worldModes.js:4262` passes `voxelfolk`, `scenes/interiorContext.js:483` consumes it. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:1931`, `:1710`). Ledger row `:580`'s residue list is spent - see list 2. |
...
- ~~**`src/scenes/dungeonContext.js:1866`** - the two window seams this
  ...
  carries the same closure against Ledger row `:643`. The two WINDOWS
  ...
  `test/x11b.test.js`. This was also Ledger row `:643`'s adjudication,
...
- **`src/scenes/exterior.js:1721`** - Recall pends here; the anchor
...
- ~~**`src/scenes/exterior.js:1608`** - PX3: this test host mounts no
  ...
  site. (The `src/scenes/worldModes.js:1908` row below is struck: ROAD-E
...
- ~~**`src/scenes/world.js:3135`** - the port's default landing stands
...
- ~~**`src/characters/enemyCasting.js:91`** - now exactly one term
  ...
  second.*
- ~~**`src/scenes/worldModes.js:1850`** - above ground only:
...
cannot take, `exterior.js:1611`), ~~`exterior.js:1654` -> `:1363`~~
(**CLOSED at QX1**: this host took the quest bridge, so the pause window's
Quests tab has a machine to read), ~~`world.js:3181` -> `:2989`~~
(**CLOSED at the ship landing**), ~~`worldModes.js:1847` -> `:1702`~~
...
- ~~**`src/scenes/worldModes.js:1920`** - above ground only: the GUILD
...
Section C's table is **247 rows** between `Port-Ledger.md:517` and `:763`
...
route** - plus `:634`, struck at its head but carrying a live PENDING
...
1. **`:552` FaceUVTool's 1,803-UV residual at matched precision** ->
...
2. **`:565` the custom builder's hidden `ResetBonusPool` control** (STRUCK at E2, landing after this list was written: the control is live) ->
...
3. ~~**`:535` remainder: the overlay mouse-UP seam** -> UI arc. A7 shipped
...
4. ~~**`:578` the quest machine** -> `playSound`'s busy-skip, the one
...
5. ~~**`:631` the classic `.SAV` reader** -> the phone path: no zip arm in
   the saves picker, a desktop-first charter call.~~ **CLOSED (OT1, 2026-09-10).**
6. **`:634` (struck, with a live clause) the keybinding registry** ->
   the mouse/advanced and joystick sub-windows, the two of DFU's 65
   game windows the port does not cite.
7. ~~**`:643` the standalone dungeon host has no trade window** -> a
...
8. ~~**`:644` three stale probes**~~ **CLOSED (E-group, 2026-09-02).**
...
9. **`:530` UseItem's unbuilt destinations.** Every arm the row names is
   built: `DrinkPotion` (`systems/useItem.js:167`, `:245-255`),
   `RecordLocationFromMap`/`DiscoverRandomLocation`
   (`ui/nativeInventory.js:704-708`, `scenes/world.js:2941`), the
   (`ui/nativeInventory.js:704-708`, `scenes/world.js:2887`), the
...
10. **`:580` fast-travel residue.** Its three named survivors are spent:
    ...
    `scenes/world.js:3850` (the arrival clamp anchoring the encounter
...
11. **`:585` `PatchRegionIndex` legacy-save fix.** Ported verbatim at
    `src/formats/mapsFile.js:104`, with the C# line range cited.
12. **`:629` the magic crafting windows.** The row's FLAGGED residue is
    ...
    clause goes with it.**
13. **`:579` the talk manager.** The row's own tail already says both
...
14. **`:609` the small-residue trio** (biography GP arm, arrow roll
...
- **The voxel character engine** (Mac's system, Port-Doctrine). 86
  ...
  by a game host: `worldModes.js:4262` passes `voxelfolk`,
...
- **`FaceUVTool` arithmetic** - JS doubles for C# float mix, `Math.trunc`
  ...
  precision are ledger row `:552`, narrowed at E8 and the only part of

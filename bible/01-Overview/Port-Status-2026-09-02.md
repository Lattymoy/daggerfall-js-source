...
and six rows** (five, since OT1 struck `:678`),
...
| open flags | 151 | **6** | `node tools/regenOpenFlags.mjs --check` answers 7 ("6 entries, up to date"). It answered 19 when this table was taken - 17 at `c3c12ee`, plus the two the closeout tail's spell-hand port added - Wave E then retired six, named in list 1, the ship landing a seventh, ROAD-F three more (GS1 `scenes/worldModes.js`, GS2 `systems/skills.js`, DR1 `scenes/dungeonContext.js`), and QX1 the next (`scenes/exterior.js`'s PX3, struck in list 1); the same grep over `git show 6881171:bible/Home.md` returns 151 |
...
in-game name: `combat/formulas.js:757` runs it at DFU's own dispatch
...
`NewLocationAlert` is `systems/discovery.js:185`'s `discoverLocation`;
...
the same place - `Port-Ledger.md:737`, the struck keybinding-registry
...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:8351`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:5315` carries the sweep and `hostMagic.js:817` its missile half. |
...
| **characters-voxel** | Departure (deliberate) | **Departure, still editor-only** | 86 designs across seven tables - 42 of the 43 monster mobiles, 19 class, 25 villager. The rig is 1,791 lines across 7 modules and is still gated: `worldModes.js:5306` passes `voxelfolk`, `scenes/interiorContext.js:577` consumes it. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:3698`, `:3392`). Ledger row `:683`'s residue list is spent - see list 2. |
...
and `world.js:301` reaches it by mounting `worldModes`. But `grep -rn
...
unstruck here is the seven the tool answers today (GP3, 2026-09-11, retired the joystick flag; ONLINE-DUNGEON-FOES, 2026-09-20, added one after this page's measurement).
...
- ~~**`src/scenes/dungeonContext.js:2126`** - the two window seams this
  ...
  carries the same closure against Ledger row `:746`. The two WINDOWS
  ...
  `test/x11b.test.js`. This was also Ledger row `:746`'s adjudication,
  and that row is struck with it.
- **`src/ui/enhancedMenu.js:3357`** - the rest of the keyboard; the
...
- **`src/scenes/exterior.js:2262`** - Recall pends here; the anchor
  ...
  live claim has to name a live line. The struck rows keep the
  measurement's numbers, which is what the note below says they are.)* **NARROWED (TP2,
...
- ~~**`src/scenes/exterior.js:1945`** - PX3: this test host mounts no
...
- ~~**`src/scenes/world.js:4871`** - the port's default landing stands
...
- ~~**`src/scenes/worldModes.js:1973`** - above ground only:
  ...
  ported and idle at `systems/quest/machine.js:810` including the
...
- ~~**`src/systems/inventory.js:50`** - gold as a bag stack.~~ **SHIPPED
  ...
  (`:189`) came back whole as `inventory.carriedWeight` and reaches its
...
- ~~**`src/ui/chargenArt.js:749`** - the picker's scroll bar has no HIT.
...
- ~~**`src/ui/hudLarge.js:75`** - the docked bar occludes, it does not
  ...
  (`render/renderer.js:2334, :2056, :3074, :3399`) and the 2D passes
...
- ~~**`src/systems/skills.js:165`** - "AUDIT 18: the +10% used to be
...
- **`src/scenes/dungeonContext.js:1234`** - the dungeon's non-layout foe
  ...
  measured once, and every foe appended past it (a quest foe, an
  encounter, a summon) is outside BOTH the stream (`foesFrame` loops
  `i < _layoutFoes`) and the target machine's peer candidates
  (`_authority && streamed`, where `streamed` is that same bound). So a
  quest-placed foe exists only on the client that placed it, and no
  non-layout foe ever sees another player as a target. RECORDED, NOT
  CLOSED: the two halves must be paid together - arming an unsynced foe
  against peers is worse than the bug, because it would chase and swing
  at a player who cannot see it and has no damage frame to resolve the
  blow. The answer is WORLD6b's owner law (`scenes/exteriorFoes.js`, a
  cell's foe is its spawner's) brought into the dungeon, which is a
...
still standing there are the six `bible/Home.md` listed then (GP3 retired the seventh), plus the one
added above after this page was measured - seven in all, which is what the tool answers. **As of Wave E, the ship
...
regenerated onto the new sites: ~~`exterior.js:1606` -> `:1294`~~
...
`:NNN` inside a STRUCK row here is the measurement's line and is not a
...
**ROAD-G G7 (2026-09-04) - THE RECORDS SWEEP, and what it closed.** Not
a host slice: the subject is this page's own class of defect, a `:NNN`
that was a claim once and is a pointer at a stranger now.

- ~~The `src/scenes/exterior.js` citations.~~ **CLOSED.** *Thirty-six
  `exterior.js:NNN` cites stood across `src/` comments, bible pages and
  test headers and every one was stale BEFORE Wave F - the fixed-city
  host has taken a slice in nearly every wave since the numbers were
  written, so a cite exact at U44 was six hundred lines out at HEAD, and
  `characters/playerEntity.js`'s was never right at all (the commit that
  wrote it landed on the town-talk topics table, not on a chargen line).
  Each is re-resolved BY CONTENT and pinned into `test/citedrift.test.js`
  CD4, with this page's one still-open `exterior.js` row into CD6. Only
  `ui/hudLarge.js`'s activation-ray cite and `Home.md`'s generated flag
  were already exact.*
- ~~The doctrine gate's blind half.~~ **CLOSED.** *`test/doctrine.test.js`
  scanned only files shouting a DEPARTURE token, so AUDIT 21 F7's other
  half - a `src/` site citing "Ledger A" where no row on that page names
  the file - went on recurring unwatched. Widening the population to
  `/Ledger A/` reddened SEVENTEEN files at HEAD. Three were owed a
  section-A row and have one (the `AdvancedClimbing` scaffolding, the
  merchant service popup's accelerators, the port's scalar clock - which
  is why the section A tally below reads 80); the rest resolved against a
...
- ~~`ui/transportWindow.js`'s "the port's own letters".~~ **SHIPPED.**
  *The one of the seventeen that was a live defect rather than a missing
  ...
  out of `DialogShortcuts.txt`, which ROAD-A8 ported whole, so the
  letters are wired to the table now - and the EXIT, which had no letter
  at all where DFU binds `TransportExit`, answers `E`. This is the D1
  correction reaching the window that pass left behind. UI-Arc carries
  the slice.*
- ~~`Settings-Screen-Spec.md`'s FieldOfView bullet.~~ **STRUCK.** *It
  still named five `Math.PI/3` sites that the SETT/MENU view-settings
  slice had already replaced with `ui/viewSettings.js`'s `fieldOfView()`
  - the cites had rotted precisely because nobody struck the row that
  made them.*
...
Section C's table is **247 rows** between `Port-Ledger.md:620` and `:866`
...
generation, to the enhanced lane)~~ (**it read 226 and 21 until `:564` was
...
route** - plus `:737`, struck at its head but carrying a live PENDING
...
1. **`:655` FaceUVTool's 1,803-UV residual at matched precision** ->
...
2. **`:668` the custom builder's hidden `ResetBonusPool` control** (STRUCK at E2, landing after this list was written: the control is live) ->
...
3. ~~**`:638` remainder: the overlay mouse-UP seam** -> UI arc. A7 shipped
...
4. ~~**`:681` the quest machine** -> `playSound`'s busy-skip, the one
...
5. ~~**`:734` the classic `.SAV` reader** -> the phone path: no zip arm in
   the saves picker, a desktop-first charter call.~~ **CLOSED (OT1, 2026-09-10).**
6. **`:737` (struck, with a live clause) the keybinding registry** ->
   the mouse/advanced and joystick sub-windows, the two of DFU's 65
   game windows the port does not cite.
7. ~~**`:746` the standalone dungeon host has no trade window** -> a
...
8. ~~**`:747` three stale probes**~~ **CLOSED (E-group, 2026-09-02).**
...
9. **`:633` UseItem's unbuilt destinations.** Every arm the row names is
   built: `DrinkPotion` (`systems/useItem.js:205`, `:296-306`),
   `RecordLocationFromMap`/`DiscoverRandomLocation`
   (`ui/nativeInventory.js:842-846`, `scenes/world.js:5369`), the
...
10. **`:683` fast-travel residue.** Its three named survivors are spent:
    ...
    `scenes/world.js:4549` (the arrival clamp anchoring the encounter
...
11. **`:688` `PatchRegionIndex` legacy-save fix.** Ported verbatim at
    `src/formats/mapsFile.js:107`, with the C# line range cited.
12. **`:732` the magic crafting windows.** The row's FLAGGED residue is
    ...
    clause goes with it.**
13. **`:682` the talk manager.** The row's own tail already says both
...
14. **`:712` the small-residue trio** (biography GP arm, arrow roll
    ...
    `BIOG*.TXT` files; the faceUV guard rides row `:655`.

## 3. The deliberate departures, which are not on the road

`Road-To-1-1.md` names these at its head so nobody re-opens them, and
`Port-Ledger.md` section A carries **218 rows, 15 struck - 203 standing
approved departures** (the 74th is WIND1, 2026-09-02, added after this page's measurement and counted here so the tally follows the tree; the 137th is THE QUICKSLOT DIAMOND, QS1-QS3, 2026-09-17; the 138th is GetKeyDown AND GetKeyUp, THE READS THE PORT NEVER HAD, MWCROUCH, 2026-09-17, inserted directly under it; the 139th is ONE MODE, ONE DISPATCH - AND A DEBUG SENTINEL OUT OF THE PLAYER'S MOUTH, QS7 + TALK-UNKNOWN, 2026-09-17, inserted directly under THAT, ; the 140th is A BODY IS OPENED NOT EMPTIED..., MAC-A/C/D/E, 2026-09-17, inserted directly under THAT; the 141st is A CARD THAT FOLDS, AND A PAGE THAT WAS NEVER DRAWN, MAC-F/MAC-G, 2026-09-17, inserted directly under THAT; the 142nd is THE VIEWMODEL TAKES THE ROOM'S LIGHT, AND A NAME IS NOT A BUTTON, MAC-H/I/J/P, 2026-09-17, inserted directly under THAT; the 143rd is THE TORCH'S FIRE - MORROWIND'S PARTICLE SYSTEMS, PORTED, MAC-Q, 2026-09-17, inserted directly under THAT; the 144th is BETTER RESTING, WILDLIFE GROUPS, FOES UNDER A WINDOW, AND AN ONLINE DEATH THAT RESPAWNS, RESTX2 / CAMP1 / WINFOE1 / D-ONLINE1, 2026-09-17, inserted directly under THAT; the 146th is FOUR OF MAC'S BUGS - THE RAISED BLADE, THE DOUBLED LINE, THE HAND CELLS, THE HOLD UNDER A REPAINT, MAC-R1/R2/R3/R4, 2026-09-17, inserted directly under THAT; the 147th is THE TARGET BAR'S BLADE FACE, FOEBAR1, 2026-09-17, inserted directly under THAT, the 148th is THE CRIMINAL'S WATCH RIDES THE CELL, WATCH1 + AUDIT WATCH1, 2026-09-17, inserted directly under THAT, the 149th is SHOPS STAFFED AROUND THE CLOCK ONLINE, OL4, 2026-09-17, inserted directly under THAT, with the 159th, THE TOWN GATE AND THE GUILD HALL, OPEN AT NIGHT ONLINE, OL5, 2026-09-20, inserted directly under OL4's because it is OL4's own reasoning applied to the two subjects OL4 left out, which moved every row below it - sections B, C and D - down one more line, the 150th is THE REGION'S PRICES ARE THE WORLD'S, ECON1, 2026-09-17, inserted directly under THAT, the 151st is EIGHT OF JANOME'S REPORTS - THE HAND THAT SWITCHED SIDES, THE FIST UNDER THE TORCH, THE WALKERS IN THE SKY, THE GLARE THROUGH THE ATTIC, THE CLASSIC TALK BOX, JAN1, 2026-09-18, inserted directly under THAT, the 152nd is THE LIGHT IN THE HAND SAYS SO, MAC-T1/T2, 2026-09-18, inserted directly under THAT, the 153rd is CLIMATES & CALORIES, OVERHAULED WITH PERMISSION, SURV1-SURV7, 2026-09-18, inserted directly under THAT, the 154th is NO GROUP ROLL UNDER A REST, AND THE TUTORIAL DUNGEON IS NOT A RESPAWN DOOR, CAMP1-REST / D-ONLINE2, 2026-09-18, inserted directly under THAT, the 157th is THE ASCENSION - THE ENHANCED SKIN'S LEVEL-UP WINDOW, LV1, 2026-09-18, appended at the FOOT of the section, with the 158th, THE RISING - THE ENHANCED SKIN ANNOUNCES A LEVEL INSTEAD OF SEIZING THE SCREEN, LV2, 2026-09-19, appended directly under it, which moved sections B, C and D down two lines and is why every `:NNN` below was re-resolved, and the 161st is THE SPELLBOOK'S FOUR STANDING DEPARTURES, ROWED (CM3-CM11, 2026-09-21: striking TB1 left `ui/spellbookWindow.js`'s recorded departures approved by no row), the 162nd is THE NOTICE AT THE EDGE (ENH-NOTICE1, 2026-09-21: the enhanced skin's message box slides in from the right) and the 163rd is THE PAD LAYOUT, PAD1, 2026-09-21 - all three appended at the FOOT of the section under LV2's, which moved sections B, C and D down three lines more, with the 164th, THE ONE DOOR EVERY MESSAGE GOES THROUGH, and the 165th, THE HUD LINE AS A TOAST, both ENH-NOTICE3, 2026-09-21, appended directly under PAD1's, which moved sections B, C and D down two lines more, and the eighteen together moved every row below - sections B, C and D - down eighteen lines; the 105th is HANDHELD TORCHES, HT1, 2026-09-14, under Weapon Widget's row, with ONE SHAPE FOR EVERY TEXTURE DOOR (TEX1) under it; the 106th to 109th are NO RAIN OVER SNOW, CLOUD TYPES BY PLACE, THE WEATHER FIELD and THE SANDSTORM, WEATHER2a/c/b/d, 2026-09-14, inserted after WX2's row; the 4th is MOBILE TOUCH INPUT, TI1, 2026-09-05, inserted under the windmill row and likewise; the 10th is DS1 - Dynamic Skies vendored 1:1 with permission, 2026-09-04 - inserted directly under the enhanced sky's row it stands beside, which moved every row below it down one; the 11th is SIB1 - Seasons of the Iliac Bay ported 1:1 with permission, 2026-09-05, its textures the player's to supply - inserted directly under DS1, moving every row below down one more; the 12th is ECV1 - Enhanced Combat Visuals, 2026-09-07, the enhanced skin's draw of a concealed foe - inserted directly under SIB1, moving every row below down one more; the 13th is VC - Volumetric Clouds, 2026-09-07, the arc's row, inserted directly under ECV1, moving every row below down one more; the 14th is CLK2 - the weather evolving within the day on the enhanced lane, 2026-09-08, the Clock arc's row, inserted directly under VC, moving every row below down one more; the 2nd is the re-integrated road system, inserted directly under the struck ROADS row it supersedes, added by AUDIT 58 F5; the 77th is the pause window's version line, appended by AUDIT 58's records lane, which found `src/ui/pauseWindow.js` citing a Ledger A approval that had never been written down; rows 78-85 are AUDIT 58's seams lane, which found EIGHT more files declaring a departure as already RECORDED - music replacement, the fuzzy find, travel weather, the conversation save, the colour picker, async art in two windows, the town map's plate and arrow, and the input ladder's E and swing - against a section A that rowed none of them; rows 86-88 are ROAD-G G7's records lane, which widened the doctrine gate from the DEPARTURE token to the `Ledger A` CITE and found seventeen files claiming an approval this page did not name - the `AdvancedClimbing` scaffolding, the merchant service popup's accelerators and the port's scalar clock are the three that were genuinely owed a row rather than a roster line or a corrected comment; the 89th is AUDIT 64 F29's biography name seed, 2026-09-08, appended at the table's foot - `(uint)BiogFile.GetHashCode()` is a CLR identity hash and belongs to neither arm of the ENGINE-PRNG rule; the 90th is WATER1 - Enhanced Water, 2026-09-08, the water surface over the exterior water tiles on the enhanced lane, appended at the foot of the section; the 91st is MAC1 - THE LOOK'S FLOOR, 2026-09-10, the one law departure in Mac's play report, appended below WATER1, which moved sections B, C and D down one line; the 5th is TI2 - THE PHONE IN HAND, TUNED, 2026-09-11, the second touch slice, inserted directly under TI1's row, which moved every row below it - every ordinal from DS1's on in this sentence - down one more; the 28th is THE SMALLER-DUNGEON SAVE STAMP IS THE BUILD, FT1, 2026-09-14, appended at the foot of the first table, which moved every ordinal from WIND1's on down one more; the 118th is WEAPON WIDGET, WW1, 2026-09-14, RedRoryOTheGlen's mod ported 1:1 off its IL with the Morrowind arms riding its channels, appended at the foot of the section; the 109th is THE PACK COULD NOT LIGHT A TORCH, HT2, 2026-09-14, inserted directly under TEX1's row, which moved every row below it down one; the 110th is MODS AND THEIR MODULES ARE ON, MODS-ON, 2026-09-14, inserted directly under HT2's, moving every row below down one more; the 111th is THE WISPS' COUNT, THE SKY'S DIRECTION, THE GRASS AT NIGHT, WIND4, 2026-09-15, inserted directly under MODS-ON's, moving every row below down one more; the 112th is THE HELD SPRITE WAS UPSIDE DOWN, HT3, 2026-09-15, inserted directly under WIND4's, moving every row below down one more; the 113th is THE LOOT LADDER IS THE DEFAULT, LR1-LR5, 2026-09-14/15, inserted directly under HT3's at the foot of the section - the ladder shipped OFF and was dormant, and LR5 making it the DEFAULT is what a departure a player meets without asking owes section A; the 135th is SOCIAL - FRIENDS, PRESENCE AND THE FOUR-SEAT PARTY, SOC1-SOC7, 2026-09-16, inserted directly under AUDIT RESTX + AUDIT OQ's at the foot of the first table, which moved every row below it - sections B, C and D - down one line; the 167th is THE STATUS READOUT DOES NOT STOP THE WORLD, AND THE PANEL IS AS WIDE AS WHAT IT SAYS, STATUS-LIVE + NOTICE-FIT, 2026-09-22, inserted directly ABOVE THE NOTICE AT THE EDGE's - the departure it is a consequence of - which moved every row below it down one line; the 168th is THE MOUSE BUTTONS ARE NAMED, NOT NUMBERED, SWING-LABEL, 2026-09-22, inserted directly under the input ladder's E ACTIVATES BESIDE MOUSE0 row it stands beside, which moved every row below it - sections B, C and D - down one line more; the 169th is THE NIGHT'S CRICKETS SING IN CHORUSES, CRICKET-QUIET, 2026-09-22, inserted directly under SWING-LABEL's, which moved every row below it down one line more; the 171st is A HELD TWO-HANDER TAKES BOTH HANDS - RELAXED TWO-HANDED WEAPONS SHIPS OFF, 3ARMS, 2026-09-22, appended under the drops row at the section's foot, which moved sections B, C and D down one line more; the 172nd is THE WORLD WEATHER MAP, WEATHER3, 2026-09-22, inserted directly under THE SANDSTORM's row with the WEATHER2 rows it supersedes on the lane, which moved every row below it - sections B, C and D - down one line more; the 173rd is ONLINE, A NO-REGEN CAREER'S REST PAYS MAGICKA; A HEALER KNOWS A TOUCH HEAL; THE CRICKETS AND THE HOWL HAVE SWITCHES, FRIENDLY-SPELLS, 2026-09-23, appended directly under 3ARMS's, which moved sections B, C and D down one line more; the 174th and 175th are THE STREET'S AMBIENCE INDOORS and THE EARS, DISC6-C/D, 2026-09-23, inserted directly under CRICKET-QUIET's, which moved every row below them - sections B, C and D - down two lines more; the 176th is THE HORSE'S VERBS ON THE PLAQUE, ACT-MENU, DISC7, 2026-09-23, inserted directly under THE EARS, which moved every row below it down one line more; the 177th is WORLD OF DAGGERFALL, 1:1, WOD1-WOD5, 2026-09-23, appended directly under FRIENDLY-SPELLS's at the section's foot, which moved sections B, C and D down one line more; the 178th is THE PILE OF BODIES - THE LOOT WINDOW HOLDS IT, A TAB PER BODY, LOOT-STACK, 2026-09-23, inserted directly under QUICK-LOOT's row - the take a press on the pile still makes - which moved every row below it, sections B, C and D with them, down one line more and is why section 2's `:NNN` identifiers were re-resolved; and the 179th and 180th are DISC10-B's A PERSON'S "I'LL MARK IT ON YOUR MAP" MARKS THEIR HOUSE and DISC10-A's ONE CART FOR THE CART TRANSPORT, 2026-09-23, inserted directly under ACT-MENU's row (THE HORSE'S VERBS ON THE PLAQUE), which moved every row below them down two lines, and THE KEYBINDING STANDARD, KB1, 2026-09-23, inserted directly under E ACTIVATES BESIDE MOUSE0 (which it closed), which moved every row below it down one line more; and the 182nd is THE WATCH DEFENDS THE TOWN, DISC19-F, 2026-09-24, and the 183rd A MUSIC SWITCH FADES, DISC20-B, 2026-09-24, both appended at the section's foot, which moved sections B, C and D down two lines more; and the 184th is DFU'S RETRO MODE, PORTED, RETRO1, 2026-09-24, appended at the section's foot after them, which moved them down one line more; and the 185th is THE FIGHTERS GUILD TAKES A BARE HAND, FGH2H, 2026-09-24, appended at the section's foot after RETRO1's, which moved them down one line more; and the 186th is ENCHANTED ITEMS ARE MENDED, AND ONLINE IT IS THE ROOM'S RULE, DISC22-A, 2026-09-24, appended at the section's foot after FGH2H's, which moved sections B, C and D down one line more; and the 187th is THE PARTY ON THE PLANS; EYE OF THE BEHOLDER'S SPRITES, CHOSEN AND SEEN, DISC23-A/B, 2026-09-24, appended at the section's foot after DISC22-A's, which moved them down one line more; and the 188th is A DUEL BETWEEN TWO PLAYERS, INSIDE A RING OF LIGHT, DUEL1, 2026-09-24, and the 189th REPAIR ACTIVE QUESTS, QREPAIR, 2026-09-24, both appended at the section's foot after DISC23's, which moved sections B, C and D down two lines more; and the 190th is THE HORSE'S OWN PICTURE AND THE CART'S MODEL; THE PLAYER'S OWN SHADOW; A REVIVAL THAT STANDS A ZEROED STAT BACK UP, DISC24-B/C/D, 2026-09-24, appended at the section's foot after QREPAIR's, which moved sections B, C and D down one line more; and the 191st is A LANTERN HANGS AT THE WAIST, HT-WAIST, 2026-09-24, appended after DISC24's, which moved them down one line more; and the 192nd is THE DUNGEON MAP'S FLOORS ARE SHEETS AND ITS STAIRS ARE DRAWN; A STACK'S HOW-MANY ON THE ENHANCED CARDS, DISC25-A/F, 2026-09-25, appended after HT-WAIST's, which moved them down one line more; and the 193rd is THE FRAME RATE CAP HOLDS UNDER VSYNC; WAIT FOR SCREEN REFRESH IS UNAVAILABLE, FPS-CAP1, 2026-09-25, appended after DISC25's, which moved them down one line more; and the 194th is THE UX BACKLOG, UXB1, 2026-09-25, appended after FPS-CAP1's, which moved them down one line more; and the 195th is DAGGERFALL'S CLASSES AS SKINS; THE SKIN CARD'S TWO PANELS, SKIN2, 2026-09-25, appended after UXB1's, which moved them down one line more; and the 196th is A RENDER SCALE, AND THE COUNTER NAMES THE GPU, PERF-SCALE, 2026-09-25, appended after SKIN2's, which moved them down one line more; and the 197th is RENOWN, A LEVEL THAT EXISTS ONLY ONLINE, RENOWN1, 2026-09-24, appended at the section's foot after PERF-SCALE's, which moved sections B, C and D down one line more; and the 198th is A FIGHT WEIGHS WHAT THE PARTY WEIGHS, PSCALE1, 2026-09-25, appended at the section's foot after RENOWN1's, which moved sections B, C and D down one line more; the 199th is SOME WEAPONS WON ONLINE CARRY A SIGIL, AND IT GROWS, SIGIL1, 2026-09-25, appended at the section's foot after PSCALE1's, which moved sections B, C and D down one line more; the 200th is EVERY REGION'S MAIN CITY IS ITS HUB, HUB1, 2026-09-25, appended at the section's foot after SIGIL1's, which moved sections B, C and D down one line more; the 201st is AN ONLINE HOME IS ONE PLAYER'S, AND ITS DOOR IS THEIRS TO OPEN, HOME1, 2026-09-25, appended at the section's foot after HUB1's, which moved sections B, C and D down one line more; the 202nd is A ROOM IS FURNISHED BY ITS OWNER, A PIECE AT A TIME, FOR GOLD, DECOR1, 2026-09-25, appended at the section's foot after HOME1's, which moved sections B, C and D down one line more; the 203rd is A GUILD IS ITS PLAYERS', GUILD1, 2026-09-25, appended at the section's foot after DECOR1's; the 204th is WARM ASHES - SHIPS' THREE PORT-SIDE CHOICES, WA1, 2026-09-25, appended after GUILD1's at the sea update's merge with main; and the 205th is ILIAC PUDDLE NO MORE'S SEVEN (THE COASTLINE BUILT, THE FOG ON EVERY FRAGMENT, THE COLUMN'S TWO DRAWS, THE SWIMMER WITHOUT THE FORGED DUNGEON, THE COLLIDER GATE, THE PERIPHERAL-LOCATION SKIP, THE RECENTRE THAT CARRIES THE SCHOOLS), DW-A to DW-E4, 2026-09-25, appended after WA1's, which moved sections B, C and D down one line more; the 206th is THE OBLIVION GATE'S OMEN, WB1, 2026-09-25, appended after DW-A to DW-E4's at the merge with main, which moved sections B, C and D down one line more; the 207th is THE OBLIVION GATE STANDS IN THE WORLD, WB2, the same day, after WB1's; the 208th is THE GATE'S BOSS ROOM ON THE RELAY, WB3a, the same day, after WB2's; the 209th is THE BURNING COURT, WB3b, the same day, after WB3a's; the 210th is THE BOSS FIGHTS BACK, WB4a, the same day, after WB3b's; the 211th is THE BOSS IS FOUGHT, WB4b, the same day, after WB4a's; the 212th is THE SPOILS, WB5a, the same day, after WB4b's; the 213th is THE GATES CLOSED, WB5b, the same day, after WB5a's; the 214th is THE DEADLANDS, WB6a, the same day, after WB5b's; the 215th is THE DEADLANDS' LIFE, WB6b, the same day, after WB6a's; the 216th is THE STEP THROUGH THE GATE, WB6c, the same day, after WB6b's; the 217th is HIS VOICE AND HIS MUSIC, WB7, the same day, after WB6c's; the 218th is THE OBLIVION GATE AFTER ITS FIRST FIGHTS, WBX, 2026-09-26, after WB7's, which moved sections B, C and D down one line more and is why section 2's `:NNN` identifiers were re-resolved) plus the DUNGEON SEED bullet, now naming its two files, and the
...
- **The voxel character engine** (Mac's system, Port-Doctrine). 86
  ...
  `scenes/interiorContext.js:578` gates on it, and no default route sets
...
- **`FaceUVTool` arithmetic** - JS doubles for C# float mix, `Math.trunc`
  ...
  precision are ledger row `:655`, narrowed at E8 and the only part of
...
**seven sites carry a blocker** - the six `node

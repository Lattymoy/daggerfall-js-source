...
the same place - `Port-Ledger.md:611`, the struck keybinding-registry
...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:4833`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:2869` carries the sweep and `hostMagic.js:601` its missile half. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:1909`, `:1715`). Ledger row `:557`'s residue list is spent - see list 2. |
...
and `world.js:188` reaches it by mounting `worldModes`. But `grep -rn
...
- ~~**`src/scenes/dungeonContext.js:1842`** - the two window seams this
  ...
  carries the same closure against Ledger row `:620`. The two WINDOWS
  ...
  `test/x11b.test.js`. This was also Ledger row `:620`'s adjudication,
...
- **`src/scenes/exterior.js:1742`** - Recall pends here; the anchor
...
- ~~**`src/scenes/exterior.js:1631`** - PX3: this test host mounts no
...
- ~~**`src/scenes/world.js:3118`** - the port's default landing stands
...
- ~~**`src/ui/hudLarge.js:75`** - the docked bar occludes, it does not
  ...
  (`render/renderer.js:1279, :1245, :1741, :1916`) and the 2D passes
...
regenerated onto the new sites: ~~`exterior.js:1300` -> `:1106`~~
(**CLOSED at TP2**: the Recall interim narrowed to the one arm this host
cannot take, `exterior.js:1634`), ~~`exterior.js:1677` -> `:1363`~~
(**CLOSED at QX1**: this host took the quest bridge, so the pause window's
Quests tab has a machine to read), ~~`world.js:3164` -> `:2969`~~
...
Section C's table is **247 rows** between `Port-Ledger.md:494` and `:740`
...
route** - plus `:611`, struck at its head but carrying a live PENDING
...
1. **`:529` FaceUVTool's 1,803-UV residual at matched precision** ->
...
2. **`:542` the custom builder's hidden `ResetBonusPool` control** (STRUCK at E2, landing after this list was written: the control is live) ->
...
3. ~~**`:512` remainder: the overlay mouse-UP seam** -> UI arc. A7 shipped
...
4. ~~**`:555` the quest machine** -> `playSound`'s busy-skip, the one
...
5. ~~**`:608` the classic `.SAV` reader** -> the phone path: no zip arm in
   the saves picker, a desktop-first charter call.~~ **CLOSED (OT1, 2026-09-10).**
6. **`:611` (struck, with a live clause) the keybinding registry** ->
   the mouse/advanced and joystick sub-windows, the two of DFU's 65
   game windows the port does not cite.
7. ~~**`:620` the standalone dungeon host has no trade window** -> a
...
8. ~~**`:621` three stale probes**~~ **CLOSED (E-group, 2026-09-02).**
...
9. **`:507` UseItem's unbuilt destinations.** Every arm the row names is
   built: `DrinkPotion` (`systems/useItem.js:167`, `:245-255`),
   `RecordLocationFromMap`/`DiscoverRandomLocation`
   (`ui/nativeInventory.js:703-707`, `scenes/world.js:2918`), the
   (`ui/nativeInventory.js:703-707`, `scenes/world.js:2864`), the
...
10. **`:557` fast-travel residue.** Its three named survivors are spent:
...
11. **`:562` `PatchRegionIndex` legacy-save fix.** Ported verbatim at
    `src/formats/mapsFile.js:104`, with the C# line range cited.
12. **`:606` the magic crafting windows.** The row's FLAGGED residue is
    ...
    clause goes with it.**
13. **`:556` the talk manager.** The row's own tail already says both
...
14. **`:586` the small-residue trio** (biography GP arm, arrow roll
...
- **`FaceUVTool` arithmetic** - JS doubles for C# float mix, `Math.trunc`
  ...
  precision are ledger row `:529`, narrowed at E8 and the only part of

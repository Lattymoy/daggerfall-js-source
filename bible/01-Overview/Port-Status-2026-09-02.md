...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:4853`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:2870` carries the sweep and `hostMagic.js:601` its missile half. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:1910`, `:1708`). Ledger row `:579`'s residue list is spent - see list 2. |
...
- **`src/ui/enhancedMenu.js:2277`** - the rest of the keyboard; the
...
10. **`:579` fast-travel residue.** Its three named survivors are spent:
    ...
    `scenes/world.js:3884` (the arrival clamp anchoring the encounter
...
`Port-Ledger.md` section A carries **117 rows, 10 struck - 107 standing

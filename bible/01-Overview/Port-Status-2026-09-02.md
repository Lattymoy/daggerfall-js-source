...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:5333`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:3146` carries the sweep and `hostMagic.js:608` its missile half. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:2012`, `:1772`). Ledger row `:609`'s residue list is spent - see list 2. |
...
and `world.js:205` reaches it by mounting `worldModes`. But `grep -rn
...
- ~~**`src/scenes/dungeonContext.js:1907`** - the two window seams this
...
- **`src/scenes/exterior.js:1857`** - Recall pends here; the anchor
...
- ~~**`src/scenes/exterior.js:1647`** - PX3: this test host mounts no
...
- ~~**`src/scenes/world.js:3267`** - the port's default landing stands
...
cannot take, `exterior.js:1649`), ~~`exterior.js:1693` -> `:1401`~~
(**CLOSED at QX1**: this host took the quest bridge, so the pause window's
Quests tab has a machine to read), ~~`world.js:3306` -> `:3154`~~
...
9. **`:559` UseItem's unbuilt destinations.** Every arm the row names is
   ...
   (`ui/nativeInventory.js:736-740`, `scenes/world.js:3195`), the
   (`ui/nativeInventory.js:736-740`, `scenes/world.js:3141`), the
...
10. **`:609` fast-travel residue.** Its three named survivors are spent:
    ...
    `scenes/world.js:4272` (the arrival clamp anchoring the encounter
    clock, so the traveled window is not replayed) and `:1969` (the
    `scenes/world.js:4216` (the arrival clamp anchoring the encounter

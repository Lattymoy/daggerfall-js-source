...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:6941`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:4180` carries the sweep and `hostMagic.js:662` its missile half. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:2751`, `:2468`). Ledger row `:637`'s residue list is spent - see list 2. |
...
9. **`:587` UseItem's unbuilt destinations.** Every arm the row names is
   ...
   (`ui/nativeInventory.js:796-800`, `scenes/world.js:4234`), the
...
10. **`:637` fast-travel residue.** Its three named survivors are spent:
    ...
    `scenes/world.js:3453` (the arrival clamp anchoring the encounter

...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:6078`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:3559` carries the sweep and `hostMagic.js:608` its missile half. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:2300`, `:2017`). Ledger row `:622`'s residue list is spent - see list 2. |
...
- ~~**`src/scenes/exterior.js:1680`** - PX3: this test host mounts no
...
- ~~**`src/scenes/world.js:3494`** - the port's default landing stands
...
regenerated onto the new sites: ~~`exterior.js:1360` -> `:1188`~~
...
Quests tab has a machine to read), ~~`world.js:3521` -> `:3478`~~
...
9. **`:572` UseItem's unbuilt destinations.** Every arm the row names is
   ...
   (`ui/nativeInventory.js:736-740`, `scenes/world.js:3629`), the
...
10. **`:622` fast-travel residue.** Its three named survivors are spent:
    ...
    `scenes/world.js:4692` (the arrival clamp anchoring the encounter
    clock, so the traveled window is not replayed) and `:1969` (the
    `scenes/world.js:4654` (the arrival clamp anchoring the encounter

...
the same place - `Port-Ledger.md:628`, the struck keybinding-registry
...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:4845`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:2862` carries the sweep and `hostMagic.js:601` its missile half. |
...
| **characters-voxel** | Departure (deliberate) | **Departure, still editor-only** | 86 designs across seven tables - 42 of the 43 monster mobiles, 19 class, 25 villager. The rig is 1,791 lines across 7 modules and is still gated: `worldModes.js:4187` passes `voxelfolk`, `scenes/interiorContext.js:482` consumes it. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:1902`, `:1708`). Ledger row `:574`'s residue list is spent - see list 2. |
...
- ~~**`src/scenes/dungeonContext.js:1854`** - the two window seams this
  ...
  carries the same closure against Ledger row `:637`. The two WINDOWS
  ...
  `test/x11b.test.js`. This was also Ledger row `:637`'s adjudication,
...
Section C's table is **247 rows** between `Port-Ledger.md:511` and `:757`
...
route** - plus `:628`, struck at its head but carrying a live PENDING
...
1. **`:546` FaceUVTool's 1,803-UV residual at matched precision** ->
...
2. **`:559` the custom builder's hidden `ResetBonusPool` control** (STRUCK at E2, landing after this list was written: the control is live) ->
...
3. ~~**`:529` remainder: the overlay mouse-UP seam** -> UI arc. A7 shipped
...
4. ~~**`:572` the quest machine** -> `playSound`'s busy-skip, the one
...
5. ~~**`:625` the classic `.SAV` reader** -> the phone path: no zip arm in
   the saves picker, a desktop-first charter call.~~ **CLOSED (OT1, 2026-09-10).**
6. **`:628` (struck, with a live clause) the keybinding registry** ->
   the mouse/advanced and joystick sub-windows, the two of DFU's 65
   game windows the port does not cite.
7. ~~**`:637` the standalone dungeon host has no trade window** -> a
...
8. ~~**`:638` three stale probes**~~ **CLOSED (E-group, 2026-09-02).**
...
9. **`:524` UseItem's unbuilt destinations.** Every arm the row names is
   built: `DrinkPotion` (`systems/useItem.js:167`, `:245-255`),
   `RecordLocationFromMap`/`DiscoverRandomLocation`
   (`ui/nativeInventory.js:703-707`, `scenes/world.js:2911`), the
   (`ui/nativeInventory.js:703-707`, `scenes/world.js:2857`), the
...
10. **`:574` fast-travel residue.** Its three named survivors are spent:
    ...
    `scenes/world.js:3820` (the arrival clamp anchoring the encounter
...
11. **`:579` `PatchRegionIndex` legacy-save fix.** Ported verbatim at
    `src/formats/mapsFile.js:104`, with the C# line range cited.
12. **`:623` the magic crafting windows.** The row's FLAGGED residue is
    ...
    clause goes with it.**
13. **`:573` the talk manager.** The row's own tail already says both
...
14. **`:603` the small-residue trio** (biography GP arm, arrow roll
...
- **The voxel character engine** (Mac's system, Port-Doctrine). 86
  ...
  by a game host: `worldModes.js:4187` passes `voxelfolk`,
...
- **`FaceUVTool` arithmetic** - JS doubles for C# float mix, `Math.trunc`
  ...
  precision are ledger row `:546`, narrowed at E8 and the only part of

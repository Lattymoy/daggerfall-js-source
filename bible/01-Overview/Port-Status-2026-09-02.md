...
the same place - `Port-Ledger.md:698`, the struck keybinding-registry
...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:7468`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:4618` carries the sweep and `hostMagic.js:751` its missile half. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:3047`, `:2755`). Ledger row `:644`'s residue list is spent - see list 2. |
...
- ~~**`src/scenes/dungeonContext.js:2123`** - the two window seams this
  ...
  carries the same closure against Ledger row `:707`. The two WINDOWS
  ...
  `test/x11b.test.js`. This was also Ledger row `:707`'s adjudication,
...
- **`src/scenes/exterior.js:2246`** - Recall pends here; the anchor
...
- ~~**`src/scenes/exterior.js:1935`** - PX3: this test host mounts no
...
- ~~**`src/scenes/world.js:4385`** - the port's default landing stands
...
regenerated onto the new sites: ~~`exterior.js:1593` -> `:1281`~~
(**CLOSED at TP2**: the Recall interim narrowed to the one arm this host
cannot take, `exterior.js:1957`), ~~`exterior.js:2007` -> `:1673`~~
(**CLOSED at QX1**: this host took the quest bridge, so the pause window's
Quests tab has a machine to read), ~~`world.js:4412` -> `:4405`~~
...
Section C's table is **247 rows** between `Port-Ledger.md:581` and `:827`
...
route** - plus `:698`, struck at its head but carrying a live PENDING
...
1. **`:616` FaceUVTool's 1,803-UV residual at matched precision** ->
...
2. **`:629` the custom builder's hidden `ResetBonusPool` control** (STRUCK at E2, landing after this list was written: the control is live) ->
...
3. ~~**`:599` remainder: the overlay mouse-UP seam** -> UI arc. A7 shipped
...
4. ~~**`:642` the quest machine** -> `playSound`'s busy-skip, the one
...
5. ~~**`:695` the classic `.SAV` reader** -> the phone path: no zip arm in
   the saves picker, a desktop-first charter call.~~ **CLOSED (OT1, 2026-09-10).**
6. **`:698` (struck, with a live clause) the keybinding registry** ->
   the mouse/advanced and joystick sub-windows, the two of DFU's 65
   game windows the port does not cite.
7. ~~**`:707` the standalone dungeon host has no trade window** -> a
...
8. ~~**`:708` three stale probes**~~ **CLOSED (E-group, 2026-09-02).**
...
10. **`:644` fast-travel residue.** Its three named survivors are spent:
...
11. **`:649` `PatchRegionIndex` legacy-save fix.** Ported verbatim at
    `src/formats/mapsFile.js:105`, with the C# line range cited.
12. **`:693` the magic crafting windows.** The row's FLAGGED residue is
    ...
    clause goes with it.**
13. **`:643` the talk manager.** The row's own tail already says both
...
14. **`:673` the small-residue trio** (biography GP arm, arrow roll
...
- **`FaceUVTool` arithmetic** - JS doubles for C# float mix, `Math.trunc`
  ...
  precision are ledger row `:616`, narrowed at E8 and the only part of

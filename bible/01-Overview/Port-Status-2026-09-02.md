...
the same place - `Port-Ledger.md:656`, the struck keybinding-registry
...
| **formats-game** | Verbatim readers / broken consumer | **Verbatim** | The `ItemRecord` conversions were in the wave; ROAD-A A4 took the classic-import stragglers (building-level MAPSAVE, the native bank record, `LegacyArtifactIndexBitfieldCheck`). ~~Ledger row `:653`'s one residue is the phone path - no zip arm in the saves picker, a desktop-first charter call.~~ **CLOSED (OT1, 2026-09-10): the saves picker took the ARENA2 door's zip arm, and the row is struck whole.** |
| **formats-mw** | Departure lane, faithful within it | **Departure lane, now consumed** | `clipSweepTimes` has a production caller: `combat/fpArm.js:1681` runs the whole-clip reach sweep the superseded page said had never run in the game. 17 modules / 8,842 lines. |
...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:5246`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:3043` carries the sweep and `hostMagic.js:608` its missile half. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:2007`, `:1774`). Ledger row `:602`'s residue list is spent - see list 2. |
...
- ~~**`src/scenes/dungeonContext.js:1905`** - the two window seams this
  ...
  carries the same closure against Ledger row `:665`. The two WINDOWS
  ...
  `test/x11b.test.js`. This was also Ledger row `:665`'s adjudication,
...
Section C's table is **247 rows** between `Port-Ledger.md:539` and `:785`
...
route** - plus `:656`, struck at its head but carrying a live PENDING
...
1. **`:574` FaceUVTool's 1,803-UV residual at matched precision** ->
...
2. **`:587` the custom builder's hidden `ResetBonusPool` control** (STRUCK at E2, landing after this list was written: the control is live) ->
...
3. ~~**`:557` remainder: the overlay mouse-UP seam** -> UI arc. A7 shipped
...
4. ~~**`:600` the quest machine** -> `playSound`'s busy-skip, the one
...
5. ~~**`:653` the classic `.SAV` reader** -> the phone path: no zip arm in
   the saves picker, a desktop-first charter call.~~ **CLOSED (OT1, 2026-09-10).**
6. **`:656` (struck, with a live clause) the keybinding registry** ->
   the mouse/advanced and joystick sub-windows, the two of DFU's 65
   game windows the port does not cite.
7. ~~**`:665` the standalone dungeon host has no trade window** -> a
...
8. ~~**`:666` three stale probes**~~ **CLOSED (E-group, 2026-09-02).**
...
9. **`:552` UseItem's unbuilt destinations.** Every arm the row names is
   built: `DrinkPotion` (`systems/useItem.js:182`, `:260-270`),
   `RecordLocationFromMap`/`DiscoverRandomLocation`
   (`ui/nativeInventory.js:726-730`, `scenes/world.js:3115`), the
   (`ui/nativeInventory.js:726-730`, `scenes/world.js:3038`), the
...
10. **`:602` fast-travel residue.** Its three named survivors are spent:
    ...
    `scenes/world.js:4169` (the arrival clamp anchoring the encounter
    clock, so the traveled window is not replayed) and `:1966` (the
    `scenes/world.js:4113` (the arrival clamp anchoring the encounter
...
11. **`:607` `PatchRegionIndex` legacy-save fix.** Ported verbatim at
    `src/formats/mapsFile.js:104`, with the C# line range cited.
12. **`:651` the magic crafting windows.** The row's FLAGGED residue is
    ...
    clause goes with it.**
13. **`:601` the talk manager.** The row's own tail already says both
...
14. **`:631` the small-residue trio** (biography GP arm, arrow roll
...
- **`FaceUVTool` arithmetic** - JS doubles for C# float mix, `Math.trunc`
  ...
  precision are ledger row `:574`, narrowed at E8 and the only part of

...
| **characters-voxel** | Departure (deliberate) | **Departure, still editor-only** | 86 designs across seven tables - 42 of the 43 monster mobiles, 19 class, 25 villager. The rig is 1,791 lines across 7 modules and is still gated: `worldModes.js:5290` passes `voxelfolk`, `scenes/interiorContext.js:577` consumes it. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:3073`, `:2781`). Ledger row `:659`'s residue list is spent - see list 2. |
...
- **`src/ui/enhancedMenu.js:3324`** - the rest of the keyboard; the
...
- ~~**`src/scenes/exterior.js:1943`** - PX3: this test host mounts no
  ...
  site. (The `src/scenes/worldModes.js:2286` row below is struck: ROAD-E
...
- **The voxel character engine** (Mac's system, Port-Doctrine). 86
  ...
  by a game host: `worldModes.js:5290` passes `voxelfolk`,

...
the same place - `Port-Ledger.md:636`, the struck keybinding-registry
...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:4886`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:2898` carries the sweep and `hostMagic.js:608` its missile half. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:1939`, `:1718`). Ledger row `:582`'s residue list is spent - see list 2. |
...
- ~~**`src/scenes/dungeonContext.js:1867`** - the two window seams this
  ...
  carries the same closure against Ledger row `:645`. The two WINDOWS
  ...
  `test/x11b.test.js`. This was also Ledger row `:645`'s adjudication,
...
- **`src/scenes/exterior.js:1726`** - Recall pends here; the anchor
...
Section C's table is **247 rows** between `Port-Ledger.md:519` and `:765`
...
route** - plus `:636`, struck at its head but carrying a live PENDING
...
1. **`:554` FaceUVTool's 1,803-UV residual at matched precision** ->
...
2. **`:567` the custom builder's hidden `ResetBonusPool` control** (STRUCK at E2, landing after this list was written: the control is live) ->
...
3. ~~**`:537` remainder: the overlay mouse-UP seam** -> UI arc. A7 shipped
...
4. ~~**`:580` the quest machine** -> `playSound`'s busy-skip, the one
...
5. ~~**`:633` the classic `.SAV` reader** -> the phone path: no zip arm in
   the saves picker, a desktop-first charter call.~~ **CLOSED (OT1, 2026-09-10).**
6. **`:636` (struck, with a live clause) the keybinding registry** ->
   the mouse/advanced and joystick sub-windows, the two of DFU's 65
   game windows the port does not cite.
7. ~~**`:645` the standalone dungeon host has no trade window** -> a
...
8. ~~**`:646` three stale probes**~~ **CLOSED (E-group, 2026-09-02).**
...
9. **`:532` UseItem's unbuilt destinations.** Every arm the row names is
   built: `DrinkPotion` (`systems/useItem.js:167`, `:245-255`),
   `RecordLocationFromMap`/`DiscoverRandomLocation`
   (`ui/nativeInventory.js:704-708`, `scenes/world.js:2947`), the
   (`ui/nativeInventory.js:704-708`, `scenes/world.js:2893`), the
...
10. **`:582` fast-travel residue.** Its three named survivors are spent:
    ...
    `scenes/world.js:3912` (the arrival clamp anchoring the encounter
    clock, so the traveled window is not replayed) and `:1966` (the
    `scenes/world.js:3856` (the arrival clamp anchoring the encounter
...
11. **`:587` `PatchRegionIndex` legacy-save fix.** Ported verbatim at
    `src/formats/mapsFile.js:104`, with the C# line range cited.
12. **`:631` the magic crafting windows.** The row's FLAGGED residue is
    ...
    clause goes with it.**
13. **`:581` the talk manager.** The row's own tail already says both
...
14. **`:611` the small-residue trio** (biography GP arm, arrow roll
...
- **`FaceUVTool` arithmetic** - JS doubles for C# float mix, `Math.trunc`
  ...
  precision are ledger row `:554`, narrowed at E8 and the only part of

...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:4855`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:2872` carries the sweep and `hostMagic.js:601` its missile half. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:1912`, `:1708`). Ledger row `:578`'s residue list is spent - see list 2. |
...
- **`src/ui/enhancedMenu.js:2234`** - the rest of the keyboard; the
...
9. **`:528` UseItem's unbuilt destinations.** Every arm the row names is
   ...
   (`ui/nativeInventory.js:703-707`, `scenes/world.js:2921`), the
   (`ui/nativeInventory.js:703-707`, `scenes/world.js:2867`), the

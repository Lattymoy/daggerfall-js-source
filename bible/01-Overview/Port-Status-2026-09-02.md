...
the same place - `Port-Ledger.md:720`, the struck keybinding-registry
...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:7600`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:4676` carries the sweep and `hostMagic.js:816` its missile half. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:3081`, `:2789`). Ledger row `:666`'s residue list is spent - see list 2. |
...
and `world.js:288` reaches it by mounting `worldModes`. But `grep -rn
...
- ~~**`src/scenes/dungeonContext.js:2126`** - the two window seams this
  ...
  carries the same closure against Ledger row `:729`. The two WINDOWS
  ...
  `test/x11b.test.js`. This was also Ledger row `:729`'s adjudication,
...
Section C's table is **247 rows** between `Port-Ledger.md:603` and `:849`
...
route** - plus `:720`, struck at its head but carrying a live PENDING
...
1. **`:638` FaceUVTool's 1,803-UV residual at matched precision** ->
...
2. **`:651` the custom builder's hidden `ResetBonusPool` control** (STRUCK at E2, landing after this list was written: the control is live) ->
...
3. ~~**`:621` remainder: the overlay mouse-UP seam** -> UI arc. A7 shipped
...
4. ~~**`:664` the quest machine** -> `playSound`'s busy-skip, the one
...
5. ~~**`:717` the classic `.SAV` reader** -> the phone path: no zip arm in
   the saves picker, a desktop-first charter call.~~ **CLOSED (OT1, 2026-09-10).**
6. **`:720` (struck, with a live clause) the keybinding registry** ->
   the mouse/advanced and joystick sub-windows, the two of DFU's 65
   game windows the port does not cite.
7. ~~**`:729` the standalone dungeon host has no trade window** -> a
...
8. ~~**`:730` three stale probes**~~ **CLOSED (E-group, 2026-09-02).**
...
9. **`:616` UseItem's unbuilt destinations.** Every arm the row names is
   built: `DrinkPotion` (`systems/useItem.js:205`, `:296-306`),
   `RecordLocationFromMap`/`DiscoverRandomLocation`
   (`ui/nativeInventory.js:834-838`, `scenes/world.js:4730`), the
...
10. **`:666` fast-travel residue.** Its three named survivors are spent:
    ...
    `scenes/world.js:3918` (the arrival clamp anchoring the encounter
...
11. **`:671` `PatchRegionIndex` legacy-save fix.** Ported verbatim at
    `src/formats/mapsFile.js:107`, with the C# line range cited.
12. **`:715` the magic crafting windows.** The row's FLAGGED residue is
    ...
    clause goes with it.**
13. **`:665` the talk manager.** The row's own tail already says both
...
14. **`:695` the small-residue trio** (biography GP arm, arrow roll
...
- **`FaceUVTool` arithmetic** - JS doubles for C# float mix, `Math.trunc`
  ...
  precision are ledger row `:638`, narrowed at E8 and the only part of

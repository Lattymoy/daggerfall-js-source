...
| **scenes-world** | Law 1:1 / seams broken | **Near-1:1** | `currentWeatherKey` reads a live getter (`world.js:7578`). Region identity, the quest region/vampire faction seams and `CleanupUntrackedObjects` were the wave; `world.js:4668` carries the sweep and `hostMagic.js:792` its missile half. |
...
| **sys-sim** | Law line-for-line / clock seam broken | **Near-1:1** | `preventEnemySpawns` is live on the fast-travel path (`world.js:3074`, `:2782`). Ledger row `:658`'s residue list is spent - see list 2. |
...
9. **`:608` UseItem's unbuilt destinations.** Every arm the row names is
   ...
   (`ui/nativeInventory.js:834-838`, `scenes/world.js:4722`), the
...
10. **`:658` fast-travel residue.** Its three named survivors are spent:
    ...
    `scenes/world.js:3910` (the arrival clamp anchoring the encounter

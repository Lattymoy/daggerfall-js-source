...
(`dungeonContext.js:2265`). This is the one page whose live-queue
...
- `:340` residue "a static NPC currently answers with the
  ...
  `:446` (B7); `worldModes.js:1173-1184` opens the real talk window.
...
- *Systems-Arc*: `:1445-1454` (S27) "Open and Lock are still not
  ...
  triggerOpen" - both are called from `world/actionSystem.js:821-822`
  ...
  foes, `dungeonContext.js:666-726`); the mid-file Queue
...
- *UI-Arc*: a dozen closed flags stand (talk pages B5-6 at
  ...
  (`worldModes.js:4185-4186` routes them). UI-Arc carries no records
...
- `src/ui/deathScreen.js:42-43` claims "`drop` is read by each host's
  frame" - no host reads it (the Ledger row `:379` is right, the
  comment is wrong).
- `src/systems/mysticism.js:53` header "OPEN AND LOCK ARE NOT WIRED" -
  they are (X1, `actionSystem.js:821-822`).
...
`Port-Ledger.md:480` (save.js:28/:472/:481 → :28/:498/:528), `:486`
(world.js:1907 → :2409); `Quest-Arc.md:719`/`:2905`
(worldModes.js:476 → :903); `Player-Arc.md:955` (worldModes.js:621 →

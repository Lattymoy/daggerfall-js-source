...
(`dungeonContext.js:2662`). This is the one page whose live-queue
...
- *Systems-Arc*: `:1445-1454` (S27) "Open and Lock are still not
  ...
  foes, `dungeonContext.js:820-871`); the mid-file Queue
...
- *UI-Arc*: a dozen closed flags stand (talk pages B5-6 at
  ...
  (`worldModes.js:5278-5279` routes them). UI-Arc carries no records
...
- `src/ui/deathScreen.js:87-88` claims "`drop` is read by each host's
  frame" - no host reads it (the Ledger row `:470` is right, the
  comment is wrong).
...
`Port-Ledger.md:566` (save.js:32/:549/:558 → :28/:607/:637), `:589`
(world.js:3543 → :2412); `Quest-Arc.md:719`/`:2906`
...
(interiorContext.js:206 → :199); `Rendering.md:122`

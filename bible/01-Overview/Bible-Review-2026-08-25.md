...
- *Systems-Arc*: `:1445-1454` (S27) "Open and Lock are still not
  ...
  foes, `dungeonContext.js:688-747`); the mid-file Queue
...
- *UI-Arc*: a dozen closed flags stand (talk pages B5-6 at
  ...
  (`worldModes.js:4273-4274` routes them). UI-Arc carries no records
...
- `src/ui/deathScreen.js:42-43` claims "`drop` is read by each host's
  frame" - no host reads it (the Ledger row `:393` is right, the
  comment is wrong).
...
`Port-Ledger.md:495` (save.js:28/:472/:481 → :28/:498/:528), `:490`
(world.js:1950 → :2412); `Quest-Arc.md:719`/`:2905`
(worldModes.js:479 → :903); `Player-Arc.md:955` (worldModes.js:686 →

...
(`dungeonContext.js:2296`). This is the one page whose live-queue
...
- `:340` residue "a static NPC currently answers with the
  ...
  `:446` (B7); `worldModes.js:1224-1235` opens the real talk window.
...
- *Systems-Arc*: `:1445-1454` (S27) "Open and Lock are still not
  ...
  foes, `dungeonContext.js:687-746`); the mid-file Queue
...
- *UI-Arc*: a dozen closed flags stand (talk pages B5-6 at
  ...
  (`worldModes.js:4259-4260` routes them). UI-Arc carries no records
...
- `src/ui/deathScreen.js:42-43` claims "`drop` is read by each host's
  frame" - no host reads it (the Ledger row `:392` is right, the
  comment is wrong).
...
`Port-Ledger.md:493` (save.js:28/:472/:481 → :28/:498/:528), `:490`
(world.js:1948 → :2412); `Quest-Arc.md:719`/`:2905`
(worldModes.js:478 → :903); `Player-Arc.md:955` (worldModes.js:674 →

...
(`dungeonContext.js:2264`). This is the one page whose live-queue
...
- *Systems-Arc*: `:1445-1454` (S27) "Open and Lock are still not
  ...
  foes, `dungeonContext.js:665-725`); the mid-file Queue
...
- `src/ui/deathScreen.js:42-43` claims "`drop` is read by each host's
  frame" - no host reads it (the Ledger row `:378` is right, the
  comment is wrong).
...
`Port-Ledger.md:479` (save.js:28/:472/:481 → :28/:498/:528), `:485`

...
- `:340` residue "a static NPC currently answers with the
  ...
  `:446` (B7); `worldModes.js:1376-1387` opens the real talk window.
...
- *Systems-Arc*: `:1445-1454` (S27) "Open and Lock are still not
  ...
  foes, `dungeonContext.js:800-853`); the mid-file Queue
...
- *UI-Arc*: a dozen closed flags stand (talk pages B5-6 at
  ...
  (`worldModes.js:5268-5269` routes them). UI-Arc carries no records
...
- `src/ui/deathScreen.js:42-43` claims "`drop` is read by each host's
  frame" - no host reads it (the Ledger row `:456` is right, the
  comment is wrong).
...
`Port-Ledger.md:552` (save.js:30/:547/:556 → :28/:586/:616), `:573`
(world.js:3064 → :2412); `Quest-Arc.md:719`/`:2906`
(worldModes.js:561 → :903); `Player-Arc.md:955` (worldModes.js:789 →

...
(`dungeonContext.js:2461`). This is the one page whose live-queue
...
- `:340` residue "a static NPC currently answers with the
  ...
  `:446` (B7); `worldModes.js:1302-1313` opens the real talk window.
...
- *Systems-Arc*: `:1445-1454` (S27) "Open and Lock are still not
  ...
  foes, `dungeonContext.js:740-799`); the mid-file Queue
...
- *UI-Arc*: a dozen closed flags stand (talk pages B5-6 at
  ...
  (`worldModes.js:4372-4373` routes them). UI-Arc carries no records
...
- `src/ui/deathScreen.js:42-43` claims "`drop` is read by each host's
  frame" - no host reads it (the Ledger row `:437` is right, the
  comment is wrong).
...
`Port-Ledger.md:534` (save.js:29/:524/:533 → :28/:557/:587), `:544`
(world.js:2391 → :2412); `Quest-Arc.md:719`/`:2905`
(worldModes.js:519 → :903); `Player-Arc.md:955` (worldModes.js:740 →

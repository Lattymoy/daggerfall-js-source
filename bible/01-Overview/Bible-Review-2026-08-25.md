...
(`dungeonContext.js:2371`). This is the one page whose live-queue
...
- `:340` residue "a static NPC currently answers with the
  ...
  `:446` (B7); `worldModes.js:1239-1250` opens the real talk window.
...
- *UI-Arc*: a dozen closed flags stand (talk pages B5-6 at
  ...
  (`worldModes.js:4283-4284` routes them). UI-Arc carries no records
...
- `src/ui/deathScreen.js:42-43` claims "`drop` is read by each host's
  frame" - no host reads it (the Ledger row `:423` is right, the
  comment is wrong).
...
`Port-Ledger.md:524` (save.js:29/:505/:514 → :28/:531/:561), `:523`
(world.js:2018 → :2412); `Quest-Arc.md:719`/`:2905`
(worldModes.js:486 → :903); `Player-Arc.md:955` (worldModes.js:689 →

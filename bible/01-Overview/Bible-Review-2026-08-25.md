...
(`dungeonContext.js:2420`). This is the one page whose live-queue
...
- `:340` residue "a static NPC currently answers with the
  ...
  `:446` (B7); `worldModes.js:1292-1303` opens the real talk window.
...
- *Systems-Arc*: `:1445-1454` (S27) "Open and Lock are still not
  ...
  foes, `dungeonContext.js:719-778`); the mid-file Queue
...
- *UI-Arc*: a dozen closed flags stand (talk pages B5-6 at
  ...
  (`worldModes.js:4356-4357` routes them). UI-Arc carries no records
...
- `src/ui/deathScreen.js:42-43` claims "`drop` is read by each host's
  frame" - no host reads it (the Ledger row `:436` is right, the
  comment is wrong).
...
(world.js:2349 → :2412); `Quest-Arc.md:719`/`:2905`
(worldModes.js:512 → :903); `Player-Arc.md:955` (worldModes.js:730 →
:2764), `:304` (world.js "531 lines" → 3,564); `Characters-Arc.md:190`
(CHAR_PIXEL "7" - `renderer.js:581` ships 9, and the doc missed two

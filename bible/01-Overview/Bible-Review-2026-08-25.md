...
(`dungeonContext.js:2624`). This is the one page whose live-queue
...
- `:340` residue "a static NPC currently answers with the
  ...
  `:446` (B7); `worldModes.js:1356-1367` opens the real talk window.
...
- *UI-Arc*: a dozen closed flags stand (talk pages B5-6 at
  ...
  (`worldModes.js:5209-5210` routes them). UI-Arc carries no records
...
- `src/ui/deathScreen.js:42-43` claims "`drop` is read by each host's
  frame" - no host reads it (the Ledger row `:452` is right, the
  comment is wrong).
...
`Port-Ledger.md:548` (save.js:29/:539/:548 → :28/:585/:615), `:572`
(world.js:3041 → :2412); `Quest-Arc.md:719`/`:2906`
(worldModes.js:557 → :903); `Player-Arc.md:955` (worldModes.js:785 →
...
(interiorContext.js:205 → :199); `Rendering.md:100`

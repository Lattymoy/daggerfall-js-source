...
(`dungeonContext.js:2250`). This is the one page whose live-queue
...
- `src/ui/deathScreen.js:42-43` claims "`drop` is read by each host's
  frame" - no host reads it (the Ledger row `:367` is right, the
  comment is wrong).
...
`Port-Ledger.md:468` (save.js:28/:445/:454 → :28/:471/:501), `:451`

...
(`dungeonContext.js:2656`). This is the one page whose live-queue
...
- *Systems-Arc*: `:1445-1454` (S27) "Open and Lock are still not
  ...
  foes, `dungeonContext.js:814-865`); the mid-file Queue
...
- `src/ui/deathScreen.js:87-88` claims "`drop` is read by each host's
  frame" - no host reads it (the Ledger row `:476` is right, the
  comment is wrong).
...
`Port-Ledger.md:572` (save.js:32/:549/:558 → :28/:607/:637), `:589`

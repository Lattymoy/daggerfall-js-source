...
(`dungeonContext.js:2263`). This is the one page whose live-queue
...
- `:340` residue "a static NPC currently answers with the
  ...
  `:446` (B7); `worldModes.js:1160-1171` opens the real talk window.
...
- *Systems-Arc*: `:1445-1454` (S27) "Open and Lock are still not
  ...
  foes, `dungeonContext.js:664-724`); the mid-file Queue
...
- `src/ui/deathScreen.js:42-43` claims "`drop` is read by each host's
  frame" - no host reads it (the Ledger row `:369` is right, the
  comment is wrong).
...
`Port-Ledger.md:470` (save.js:28/:465/:474 → :28/:491/:521), `:472`
(world.js:1913 → :2409); `Quest-Arc.md:719`/`:2905`
(worldModes.js:475 → :903); `Player-Arc.md:947` (worldModes.js:620 →

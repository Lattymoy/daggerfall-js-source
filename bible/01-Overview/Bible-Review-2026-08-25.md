...
- `:340` residue "a static NPC currently answers with the
  ...
  `:446` (B7); `worldModes.js:1234-1245` opens the real talk window.
...
- *UI-Arc*: a dozen closed flags stand (talk pages B5-6 at
  ...
  (`worldModes.js:4271-4272` routes them). UI-Arc carries no records
...
- `src/ui/deathScreen.js:42-43` claims "`drop` is read by each host's
  frame" - no host reads it (the Ledger row `:404` is right, the
  comment is wrong).
...
`Port-Ledger.md:505` (save.js:29/:473/:482 → :28/:499/:529), `:491`
(world.js:1967 → :2412); `Quest-Arc.md:719`/`:2905`

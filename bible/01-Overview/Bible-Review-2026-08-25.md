...
- `:340` residue "a static NPC currently answers with the
  ...
  `:446` (B7); `worldModes.js:1383-1394` opens the real talk window.
...
- *UI-Arc*: a dozen closed flags stand (talk pages B5-6 at
  ...
  (`worldModes.js:5286-5287` routes them). UI-Arc carries no records
...
- `src/ui/deathScreen.js:87-88` claims "`drop` is read by each host's
  frame" - no host reads it (the Ledger row `:477` is right, the
  comment is wrong).
...
`Port-Ledger.md:573` (save.js:32/:549/:558 → :28/:607/:637), `:589`
(world.js:3116 → :2412); `Quest-Arc.md:719`/`:2906`
(worldModes.js:568 → :903); `Player-Arc.md:955` (worldModes.js:796 →

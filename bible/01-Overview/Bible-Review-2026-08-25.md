...
(`dungeonContext.js:2547`). This is the one page whose live-queue
...
- `:340` residue "a static NPC currently answers with the
  ...
  `:446` (B7); `worldModes.js:1350-1361` opens the real talk window.
...
- *Systems-Arc*: `:1445-1454` (S27) "Open and Lock are still not
  ...
  foes, `dungeonContext.js:763-822`); the mid-file Queue
...
- *UI-Arc*: a dozen closed flags stand (talk pages B5-6 at
  ...
  (`worldModes.js:5152-5153` routes them). UI-Arc carries no records
...
- `src/ui/deathScreen.js:42-43` claims "`drop` is read by each host's
  frame" - no host reads it (the Ledger row `:441` is right, the
  comment is wrong).
...
`Port-Ledger.md:537` (save.js:29/:528/:537 → :28/:574/:604), `:561`
(world.js:2471 → :2412); `Quest-Arc.md:719`/`:2906`
(worldModes.js:552 → :903); `Player-Arc.md:955` (worldModes.js:780 →

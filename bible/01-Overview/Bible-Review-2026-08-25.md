...
music - is asserted absent or pending in five places (`Audio.md:22`
...
(`dungeonContext.js:2390`). This is the one page whose live-queue
...
- `:340` residue "a static NPC currently answers with the
  ...
  `:446` (B7); `worldModes.js:1244-1255` opens the real talk window.
...
- *Systems-Arc*: `:1445-1454` (S27) "Open and Lock are still not
  ...
  foes, `dungeonContext.js:706-765`); the mid-file Queue
...
- *Quest-Arc*: the Q4-v RECORDED seams (`:1346-1360`) and the closing
  ...
  `questMacros.js:427-438` ("MacroHelper.cs:240-245 registers ALL SIX
...
- *UI-Arc*: a dozen closed flags stand (talk pages B5-6 at
  ...
  (`worldModes.js:4302-4303` routes them). UI-Arc carries no records
...
- `src/ui/deathScreen.js:42-43` claims "`drop` is read by each host's
  frame" - no host reads it (the Ledger row `:427` is right, the
  comment is wrong).
...
`Port-Ledger.md:528` (save.js:29/:508/:517 → :28/:537/:567), `:524`
(world.js:2076 → :2412); `Quest-Arc.md:719`/`:2905`
(worldModes.js:491 → :903); `Player-Arc.md:955` (worldModes.js:694 →
:2764), `:304` (world.js "531 lines" → 3,564); `Characters-Arc.md:190`
(CHAR_PIXEL "7" - `renderer.js:559` ships 9, and the doc missed two
later revisions recorded in `paperdollViewer.js:138`), `:2114`
(interiorContext.js:205 → :199); `Rendering.md:76`

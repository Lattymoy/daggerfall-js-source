...
(`dungeonContext.js:2637`). This is the one page whose live-queue
...
- `:340` residue "a static NPC currently answers with the
  ...
  `:446` (B7); `worldModes.js:1371-1382` opens the real talk window.
...
- *Systems-Arc*: `:1445-1454` (S27) "Open and Lock are still not
  ...
  foes, `dungeonContext.js:801-860`); the mid-file Queue
...
- `src/ui/deathScreen.js:87-88` claims "`drop` is read by each host's
  frame" - no host reads it (the Ledger row `:454` is right, the
  comment is wrong).
...
`Port-Ledger.md:550` (save.js:30/:547/:556 → :28/:594/:624), `:581`
(world.js:3063 → :2412); `Quest-Arc.md:719`/`:2906`
...
(interiorContext.js:206 → :199); `Rendering.md:100`

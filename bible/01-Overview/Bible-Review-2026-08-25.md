...
(`dungeonContext.js:2625`). This is the one page whose live-queue
...
- `:340` residue "a static NPC currently answers with the
  ...
  `:446` (B7); `worldModes.js:1362-1373` opens the real talk window.
...
- *Systems-Arc*: `:1445-1454` (S27) "Open and Lock are still not
  ...
  foes, `dungeonContext.js:800-859`); the mid-file Queue
...
- *UI-Arc*: a dozen closed flags stand (talk pages B5-6 at
  ...
  (`worldModes.js:5246-5247` routes them). UI-Arc carries no records
...
- `src/ui/deathScreen.js:42-43` claims "`drop` is read by each host's
  frame" - no host reads it (the Ledger row `:453` is right, the
  comment is wrong).
...
`Port-Ledger.md:549` (save.js:30/:541/:550 → :28/:587/:617), `:574`
(world.js:3073 → :2412); `Quest-Arc.md:719`/`:2906`
(worldModes.js:562 → :903); `Player-Arc.md:955` (worldModes.js:790 →

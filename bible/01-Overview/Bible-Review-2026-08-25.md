...
(`dungeonContext.js:2612`). This is the one page whose live-queue
...
- `src/ui/deathScreen.js:42-43` claims "`drop` is read by each host's
  frame" - no host reads it (the Ledger row `:446` is right, the
  comment is wrong).
...
`Port-Ledger.md:542` (save.js:29/:528/:537 → :28/:574/:604), `:561`

...
- `:340` residue "a static NPC currently answers with the
  ...
  `:446` (B7); `worldModes.js:1381-1392` opens the real talk window.
...
- *UI-Arc*: a dozen closed flags stand (talk pages B5-6 at
  ...
  (`worldModes.js:5284-5285` routes them). UI-Arc carries no records
...
- `src/ui/deathScreen.js:87-88` claims "`drop` is read by each host's
  frame" - no host reads it (the Ledger row `:471` is right, the
  comment is wrong).
...
`Port-Ledger.md:567` (save.js:32/:549/:558 → :28/:607/:637), `:589`
(world.js:3114 → :2412); `Quest-Arc.md:719`/`:2906`

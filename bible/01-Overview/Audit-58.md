...
three (`src/scenes/worldModes.js:1061-1134`). **The exterior host mounted no
...
between them destructured two (`src/systems/sceneCache.js:119-123`), so
...
- ~~`createCityGuards` takes no `makeAreaHostile` dep at all, so striking a
  ...
  **G1's review closed the arm the lane missed**: an ARROW reaches a pool
  ...
  `exteriorFoes.js:2018`) and all three seams route by pool membership.

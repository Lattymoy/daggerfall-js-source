...
`src/world/roadsCache.js:17`, `src/scenes/world.js:131`,
...
three (`src/scenes/worldModes.js:1024-1096`). **The exterior host mounted no
...
- ~~`createCityGuards` takes no `makeAreaHostile` dep at all, so striking a
  ...
  **G1's review closed the arm the lane missed**: an ARROW reaches a pool
  ...
  `exteriorFoes.js:1960`) and all three seams route by pool membership.

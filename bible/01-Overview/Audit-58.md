...
`src/world/roadsCache.js:17`, `src/scenes/world.js:120`,
...
three (`src/scenes/worldModes.js:975-1043`). **The exterior host mounted no
...
`src/scenes/exterior.js:52` now, and the pool it answers with is the
...
in (`src/ui/input.js:162-183`) and no production call site supplied one,
...
- ~~`createCityGuards` takes no `makeAreaHostile` dep at all, so striking a
  ...
  **G1's review closed the arm the lane missed**: an ARROW reaches a pool
  ...
  `exteriorFoes.js:1802`) and all three seams route by pool membership.

...
three (`src/scenes/worldModes.js:895-963`). **The exterior host mounted no
...
`src/scenes/exterior.js:48` now, and the pool it answers with is the
...
- ~~`createCityGuards` takes no `makeAreaHostile` dep at all, so striking a
  ...
  **G1's review closed the arm the lane missed**: an ARROW reaches a pool
  ...
  `exteriorFoes.js:1660`) and all three seams route by pool membership.

...
three (`src/scenes/worldModes.js:1025-1093`). **The exterior host mounted no
...
`src/scenes/exterior.js:61` now, and the pool it answers with is the
...
in (`src/ui/input.js:249-270`) and no production call site supplied one,
...
- ~~`createCityGuards` takes no `makeAreaHostile` dep at all, so striking a
  ...
  **G1's review closed the arm the lane missed**: an ARROW reaches a pool
  ...
  seam, which is where :630 actually lives (`arrowFlight.js:315`), still
  ...
  `exteriorFoes.js:1959`) and all three seams route by pool membership.

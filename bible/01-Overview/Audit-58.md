...
`src/world/roadsCache.js:17`, `src/scenes/world.js:118`,
...
three (`src/scenes/worldModes.js:956-1024`). **The exterior host mounted no
...
- ~~`createCityGuards` takes no `makeAreaHostile` dep at all, so striking a
  ...
  **G1's review closed the arm the lane missed**: an ARROW reaches a pool
  ...
  seam, which is where :630 actually lives (`arrowFlight.js:231`), still

...
documented absence - `worldModes.js:295` "standFoe is absent",
`machine.js:167` "ABSENT createFoeGameObjects = the spawn law idles".
...
(IsPlayerHere). The interior adapter ships (`worldModes.js:354-366`,
...
response." (`worldModes.js:837`). Again the engine is complete - only
...
which have no classic key). `applySpell` (`effects.js:471-826`) lands

...
documented absence - `worldModes.js:302` "standFoe is absent",
`machine.js:169` "ABSENT createFoeGameObjects = the spawn law idles".
...
(IsPlayerHere). The interior adapter ships (`worldModes.js:361-373`,
...
response." (`worldModes.js:844`). Again the engine is complete - only
...
which have no classic key). `applySpell` (`effects.js:471-820`) lands

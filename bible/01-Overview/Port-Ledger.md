...
| The TITLE SCREEN is OUR logo, not TITL00I0 (U21c) | Classic's title art is TITL00I0.IMG. DFU does not draw it either - TITL00I0 appears nowhere in the DFU source except ImgFile's palettized file list, because DFU replaced classic's title with its OWN branding. Our analogue of that is our own logo, so ui/titleScreen.js draws public/logo.png (the first non-ARENA2 image the port loads - OUR artwork is ours and ships with the build; game data still never enters the repo). Centred, aspect-preserved, capped at 86% of the canvas, any click or key advances. If the file is absent the title screen does not draw AT ALL and the boot goes straight to the menu - a missing asset costs a splash, never a game. This retires the C row that said showing a title needed an A row first. The art ALSO needed two opt-ins the classic path does not want: alpha BLENDING (every screen quad in the port discards a<0.5 and forces alpha 1, which is exactly right for classic art because classic art IS a 1-bit cutout, and exactly wrong for anti-aliased serifs and a soft shadow) and a LINEAR/CLAMP upload (NEAREST is for pixel-exact 320x200 art at integer scale; the logo is high-resolution at a non-integer one). Both are caller opt-ins - ui/titleScreen.js is the only caller and no game art path changed | Mac ("Let's tackle all of it. This is also our logo") |
...
| The TITLE SCREEN (TITL00I0.IMG) | RETIRED by U21c - see section A. The art still reads and is still pinned byte-exact (imgcif.test.js pins its x4 embedded-palette expansion); nothing draws it, because DFU does not draw it either. The title slot is filled by OUR logo | Closed (Ledger A) |
...
| MUSIC on the menu and in the world | HALF DONE. The READER shipped - src/formats/hmiFile.js decodes all 131 songs to a merged tick-ordered event stream (see section A). NOTHING RENDERS IT yet, and the remaining half has no source law at all: there is no soundfont in ARENA2 and DFU's SF2 is DFU's own asset, so instrument mapping is a DEPARTURE DECISION, not a port question. The archive uses 52 distinct GM programs and 76 of 131 songs put notes on channel 9, so GM percussion is needed too. Options cheapest first: synthesised per-program approximations, a small procedural sample set, or a user-supplied SF2 loaded at runtime. Scheduling is trivial from here - one tempo per song, secondsPerTick constant | Audio arc, its own slice - needs a Ledger A row for the instrument source before it can start |

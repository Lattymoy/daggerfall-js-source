...
| MUSIC on the menu and in the world | U21 measured the real shape: DFU does NOT read MIDI.BSA either. DaggerfallSongPlayer loads 133 pre-converted .mid files plus a SoundFont from Unity Resources (:278, :303, :314-322) and renders them with the vendored AudioSynthesis synth. So there is no reader to port AND no reusable asset - their .mid/SF2 are DFU's own, not the user's ARENA2. Doing it our way means XMI parsing off the user's MIDI.BSA (present, 1.2MB) + a WebAudio synth + an instrument source. The existing Ledger A music row is accurate; this row records the SCOPE | Audio arc, its own slice |

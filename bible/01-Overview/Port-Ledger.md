...
| THE DUNGEON HOST STILL OPENS THE OLD KEYED INVENTORY (U25). `scenes/dungeonContext.js` constructs `ui/inventory.js`'s InventoryWindow, not the native one, so a dungeon has no Use mode, no paperdoll, no tab filter and no real info panel - it is the last host without the real inventory. The swap needs a dungeon ground-pile mint for Remove-mode drops (droppedLoot is mounted in the exterior hosts only), which is the piece that makes it a slice rather than an edit | DaggerfallInventoryWindow in every context | UI arc (U26) |
...
| ONE CLOCK PER HOST (U23 noticed it; not introduced by it). scenes/shared.js's createPlayerTicker and dungeonContext.js each keep their OWN classicMinutes, so the interior/exterior clock and the dungeon clock advance independently and a date read in one does not match the other. DFU has a single WorldTime. The guild rank gate reads the interior host's, which is correct for a guild hall and wrong the moment anything in a dungeon needs a date | DaggerfallUnity.Instance.WorldTime | Systems arc |

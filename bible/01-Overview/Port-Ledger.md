...
| Interior furniture actions, loot, spawn points (NARROWED by the 2026-08-19 sweep: MakeHouseContainer SHIPPED in S2b - the 418 group + the 13-entry index list, TextureRecord = modelId % 100 - and the AddFurnitureAction SHELF-FIRST chain shipped in E2 with lazy shop stocking. STILL OPEN: containers spawn EMPTY, so house-container loot pends; Library/Guild/Temple bookshelves and owned-house storage pend their slices; AddSpawnPoints has no port at all) | DaggerfallInterior AddFurnitureAction/AddSpawnPoints | Systems arc |
...
| ~~Transition + activation sounds: ladder climb, enter/exit stingers~~ CLOSED as verbatim N/A (2026-08-17, recorded in Audio.md but never struck here - found by the 2026-08-19 sweep): DFU plays NOTHING on building/dungeon enter-exit, and DaggerfallLadder has no climb sound. The row was A1-era speculative naming with no source law behind it. Door open/close shipped in A1, action PlaySound in A2, the bash 28 and ActivateLockUnlock via P10/C9 | DaggerfallActionDoor, DaggerfallAudioSource | Audio arc - now MUSIC ONLY |
...
| ~~Breath/drowning + crouch motor~~ SHIPPED (P12: MaxBreath = LiveEndurance / 2 fortify-aware, the submersion test at controller centre + 76*GlobalScale, drowning damage, currentBreath riding the save envelope, and the PlayerHeightChanger crouch - the row was left unstruck at the P12 close and found by the 2026-08-19 section-C sweep) | PlayerEnterExit, AcrobatMotor | Player arc |

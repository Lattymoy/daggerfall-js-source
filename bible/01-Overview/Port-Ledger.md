...
| The WAGON as a second inventory target (U26 leaves the button answering "You don't own a wagon.", which is correct while no Transportation item exists). ShowWagon swaps the REMOTE list to the cart's own 750kg collection, with WagonCanHoldAmount gating transfers and a dungeon rule of its own - allowDungeonWagonAccess, which DFU grants only near the entrance ("exitTooFar" otherwise) | DaggerfallInventoryWindow.ShowWagon + ItemHelper.WagonKgLimit | Systems arc (with Transportation items) |
...
| ONE CLOCK PER HOST (U23 noticed it; AUDIT 22 F10 measured the cost). scenes/shared.js's createPlayerTicker and dungeonContext.js each keep their OWN classicMinutes, so the interior/exterior clock and the dungeon clock advance independently and a date read in one does not match the other. DFU has a single WorldTime. Concretely (AUDIT 22): the guild windows read the INTERIOR host's counter, so the rank gate's 28 days, the training cooldown's 12 hours and the temple's holiday date are all short by however long the player spent outdoors or underground. Nothing is wrong inside one hall; everything drifts across a session | DaggerfallUnity.Instance.WorldTime | Systems arc |

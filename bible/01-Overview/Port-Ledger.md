...
| MUSIC: the two selection inputs the port cannot supply | AUDIT 19 closed the rest of SongManager - all 24 playlists and every AssignPlaylist arm are ported and wired. TWO inputs remain unavailable rather than unported. (1) The DUNGEON seed wants the dungeon block's Unknown2 header field XOR the region; mapsFile parses that field but nothing threads it to a music layer, so the pick falls to DFU's own day-seeded branch. (2) The BUILDING FACTION id is not carried on the interior building record, so a GuildHall cannot be recognised as the Mages Guild and a Temple cannot take its god's alignment; both fall to DFU's own Interior default rather than guessing. Flagged at both sites and listed in Home.md's open flags - a wrong seed is a wrong song EVERY time, which is worse than the general rule | Routed (needs a faction id on the building record, and the dungeon header threaded) |

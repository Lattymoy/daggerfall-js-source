...
| The TITLE SCREEN (TITL00I0.IMG) | U21: the art reads and is pinned byte-exact (imgcif.test.js pins its x4 embedded-palette expansion) but nothing draws it. NOT a port: TITL00I0 appears nowhere in DFU except its palettized file list - DFU replaced classic's title with its own branding, so there is no source law here and showing it is a DEPARTURE needing Mac's approval, not a 1:1 slice | UI arc (needs a Ledger A row first) |

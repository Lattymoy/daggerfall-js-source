...
| MUSIC on the menu and in the world | LARGELY RETIRED by A5 - see section A. The reader shipped, the synth shipped, SongManager's playlists and SelectCurrentSong are ported verbatim, and the dungeon host plays. What REMAINS: the exterior/interior hosts are wired to the seam but do not yet choose a weather/time playlist, the two unportable selection arms above, and the sound quality itself - oscillators are the floor, not the ceiling, and an SF2 path can land behind the same interface | Audio arc - a wiring slice and, if wanted, a better instrument source |

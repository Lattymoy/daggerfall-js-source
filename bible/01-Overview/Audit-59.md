...
| 2 | The host lifecycle: `buildDungeonContext` is built per entry (`worldModes.js:6154` / `:8433` destroy) and `enhancedNav` with it - so a second dungeon never reads the first's bake. But `destroy()` never touched `enhancedNav.client`. | **F2** |
...
| 6 | Every foe construction site in src against the switch. | The two dungeon sites read the pref; `cityGuards.js:239` and `exteriorFoes.js:300` construct classic `EnemyAI` unconditionally - the exterior hosts are slice 5, recorded on the arc page, not a defect. **clean** |

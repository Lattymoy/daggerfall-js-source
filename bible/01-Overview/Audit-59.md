...
| 2 | The host lifecycle: `buildDungeonContext` is built per entry (`worldModes.js:4906` / `:6828` destroy) and `enhancedNav` with it - so a second dungeon never reads the first's bake. But `destroy()` never touched `enhancedNav.client`. | **F2** |
...
| 6 | Every foe construction site in src against the switch. | The two dungeon sites read the pref; `cityGuards.js:230` and `exteriorFoes.js:201` construct classic `EnemyAI` unconditionally - the exterior hosts are slice 5, recorded on the arc page, not a defect. **clean** |
...
| 8 | The three bake sites (`navBake`, `navClient.bakeHere`, `navWorker`) for a drift in coarsening: `bakeHere` calls `coarsenAgent(cols, agent)` with no budget/target where the other two pass 250000/80000. | `coarsenAgent`'s own defaults (`navmesh.js:59`) are 250000/80000 - identical. **clean** |

...
| 2 | The host lifecycle: `buildDungeonContext` is built per entry (`worldModes.js:6640` / `:8935` destroy) and `enhancedNav` with it - so a second dungeon never reads the first's bake. But `destroy()` never touched `enhancedNav.client`. | **F2** |

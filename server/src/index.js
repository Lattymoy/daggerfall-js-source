...
export class Room {
  ...
  async _named(m, now) {
    ...
    // ADV1: and the Adventuring Level, off the same signature - `lv`
  ...
  async _message(ws, message) {
    ...
    if (m.t === 'hello') {
      ...
      if (!this._setAttach(ws, { ...a, id: m.id, name: who.name, title: who.title, glyphs: who.glyphs, lv: who.lv, sub: who.subject, mu: who.mu, pose: chat ? null : m.pose, since: replaced?.since ?? now })) { this._refuse(ws, 'hello too large'); return; }   // MOD1: `sub` the verified account (what a mute names), `mu` until when it may not talk   // ADV1: `lv` the Adventuring Level the token carried
    ...
    if (m.t === 'level') {
      // ═══ ADV1 — A LEVEL THAT ROSE, CARRIED IN ══════════════════════
      //
      ...
      // The account service signs a LEVEL ORDER when it derives a new
      ...
      const now = Date.now();
      if (!this._spend(ws, now, levelGate, 'lvbucket', 'lvdrops', 'too many level orders')) return;
      await this._loadKey();
      if (!this._verifyKey) return;
      const r = await verifyOrder(m.order, this._verifyKey, { subtle: crypto.subtle, nowS: Math.floor(now / 1000), kind: 'level' });
      ...
      if (cur.lv === r.claims.lv) return;   // the same level twice is one level
      if (!this._setAttach(ws, { ...cur, lv: r.claims.lv })) return;
      const said = JSON.stringify({ t: 'level', id: cur.id, lv: r.claims.lv });

...
export class OnlineSession {
  constructor({ url = DEFAULT_SERVER, name = 'Traveller', look = null, id = null, secret = null, presence = true, acct = null, asecret = null, mintToken = null, WebSocketImpl = globalThis.WebSocket, now = () => Date.now(), rand = Math.random } = {}) {
    ...
    this._inDuelBuckets = new Map();   // DUEL1: the gate on duel frames coming in, per sender - the card gate's shape
    this._inDuelSaid = false;
    ...
    if (m.t === 'welcome') {
      ...
      if (typeof m.id === 'string' && m.id !== this.id) {
        ...
        if (!g.pass) {
          ...
          if (d && d.to === this.id) this._deliver('card', () => this.onCard?.(m.id, d, subOf(m)));   // DUEL1: and the answerer's account as the relay verified it (null from an older relay)
        }
      }
    } else if (m.t === 'page') {
      ...
      // (the card arm's law), projected by the wire, addressed to ME, with the sender's account as the relay verified it.
      // The duel's law decides what it means; nothing is read from it here.
      if (typeof m.id === 'string' && m.id !== this.id) {
        if (this._inDuelBuckets.size > TRADE_IN_SENDERS_MAX) this._inDuelBuckets.clear();
        const g = duelInGate(this._inDuelBuckets.get(m.id) ?? null, now);
        this._inDuelBuckets.set(m.id, g.bucket);
        if (!g.pass) {
          if (!this._inDuelSaid) { this._inDuelSaid = true; console.warn(`[online] duel frames are arriving faster than ${DUEL_IN_HZ_MAX}/s - frames are being dropped.`); }
        } else {
          const d = validDuelData(m.data);
          if (d && d.to === this.id) this._deliver('duel', () => this.onDuel?.(m.id, d, subOf(m)));
        }
      }
    } else if (m.t === 'park') {

...
//                    {t:'narrate', text}                TITLE-N: THE DUNGEON MASTER SPEAKING - only from a socket whose
...
//                    {t:'guild', order}                 GUILD1c: a GUILD order the account service signed - the guild the carrier's
//                                                       character is in NOW, or none; taken only from the account it names, the newest
//                                                       winning (GUILD_ORDER_HZ_MAX, one bucket with guildout)
//                    {t:'guildout', order}              GUILD1c: a member removed, or a guild disbanded, carried by whoever acted; the
//                                                       room believes the signature, never the carrier, as a mute order
...
//                    GUILD1c: and `gt`, the guild tag the token (or a guild order since) carried, absent for none
//                    {t:'guild', id, gt?}               GUILD1c: a player's guild tag changed (`gt` absent: none) - in a place to
//                                                       everyone, in a channel or the hub to that player alone
...
/** GUILD1c: guild orders a socket may carry a second, `guild` and `guildout` on one bucket - RENOWN_HZ_MAX's reason: a
 *  membership moves a few times in a character's life, and the gate is there so a socket cannot make the room verify
 *  signatures without limit. */
export const GUILD_ORDER_HZ_MAX = 1;
/** GUILD1c: the ROOM's guild-tag fans a second, every carrier together (RENOWN_ROOM_HZ_MAX's reason). Over it the
 *  change still lands and its carrier hears it; the rest of the room reads the tag off its next roster. */
export const GUILD_ROOM_HZ_MAX = 4;
/** GUILD1c: how long the client keeps a guild order to carry down a socket welcomed after it - RENOWN_ORDER_KEEP_MS's
 *  number and reason: inside the order's own minute, with ten seconds for the clocks and the wire. */
export const GUILD_ORDER_KEEP_MS = 50_000;
/**
...
/** AUDIT WORLD6b-iii(c) C3: a DESTINATION's HIT bytes a second (AUDIT 68, beside the destination's frame funnel) - the hit frame
...
export const CHAT_LINE_CHANNELS = Object.freeze(['party', 'guild']);   // GUILD1c: a guild's line, said on the hub link and heard by the guild's members online alone
...
/** GUILD1c: the hub's budget for GUILD lines, lines a second across every guild - PARTY_CHAT_ROOM_HZ_MAX's reason: a
 *  guild's fan is its members online, priced apart from the World's everyone and from the parties'. The client gates
 *  guild lines coming in on this number too (guildChatInGate). */
export const GUILD_CHAT_ROOM_HZ_MAX = 40;
 ...
 *  emoji presentation needs. EMOTE1: bar the one joiner sanitizeChat
 *  keeps between two pictographs, which is how an emoji is joined. */
const INVISIBLE = /[\p{Cf}\uFE00-\uFE0E\u{E0100}-\u{E01EF}]/u;
/** EMOTE1: a pictograph - an emoji's base (Unicode Extended_Pictographic: the faces, the people, the hearts, the
 *  signs a joiner makes a profession of) - and what may follow one inside an emoji: its presentation selector, a skin
 *  tone, the keycap's enclosing mark. */
const EMOJI_PICTO = /\p{Extended_Pictographic}/u;
const EMOJI_TAIL = /[\uFE0F\u20E3\u{1F3FB}-\u{1F3FF}]/u;

/** WHAT A PLAYER'S WORDS MAY CARRY, whatever surface draws them: control and format characters gone (a line cannot
 *  rewrite the line before it, or hide in zero width), a lone surrogate gone (B3: half a character is not a character),
 *  a stack of combining marks cut to three (A4: two hundred on one letter paint over the game), and the one joiner
 *  EMOTE1 keeps between two pictographs. Every other character is a person's own. No whitespace is collapsed and
 *  nothing is bounded here: those are the surface's (a chat line is one line of CHAT_MAX; MAIL1's letter keeps its
 *  lines - net/letterLaw.js), which is why MAIL1 took this loop out of sanitizeChat rather than copying it - one law
 *  for the characters, not two that drift. A newline is a control character and goes, so a caller that keeps lines
 *  splits on them first. Idempotent. */
export function visibleText(text) {
  let s = '';
  const cps = [...String(text ?? '')];
  let inEmoji = false;   // EMOTE1: the last thing kept was a pictograph, or its selector or skin tone
  for (let i = 0; i < cps.length; i++) {
    const ch = cps[i];
    const c = ch.codePointAt(0);
    // EMOTE1 (2026-09-23, Addison Knox: "Emotes, be it emojis or additional animations"): THE ONE JOINER KEPT. A zero
    // width joiner BETWEEN TWO PICTOGRAPHS is what makes a family, a profession, a rainbow flag - one emoji, drawn as
    // one glyph - and it hides nothing there: a pictograph stands on each side of it, in plain sight. Anywhere else it
    // is the invisible it always was (between letters it splits a word a filter reads), and it goes.
    if (c === 0x200d) {
      if (inEmoji && i + 1 < cps.length && EMOJI_PICTO.test(cps[i + 1])) s += ch;
      continue;
    }
    if (c < 32 || (c >= 0x7f && c <= 0x9f) || (c >= 0xd800 && c <= 0xdfff) || INVISIBLE.test(ch)) continue;
    s += ch;
    inEmoji = EMOJI_PICTO.test(ch) || (inEmoji && EMOJI_TAIL.test(ch));
  }
  return s.replace(/(\p{M}{3})\p{M}+/gu, '$1');
}

/** A chat line the room will relay: the characters a player's words may carry (visibleText), whitespace collapsed,
 *  trimmed, bounded - or '' when nothing is left to say. Idempotent, so what the client sends the relay takes (the
 *  panel is DOM text: nothing here is markup). */
export function sanitizeChat(text) {
  let s = visibleText(text).replace(/\s+/g, ' ').trim().slice(0, CHAT_MAX);
  // the bound can fall inside a pair (no half of a character) or just past a joiner, whose pictograph it cut away
  // (EMOTE1: a joiner with nothing after it joins nothing) - either leaves a tail a second pass would take; it goes now
  while (/[\uD800-\uDBFF]$/.test(s) || s.endsWith('\u200d')) s = s.slice(0, -1);
  return s.trim();
}
...
export function poseChanged(a, b, eps = 0.01) {
  ...
  return Math.abs(a.x - b.x) > eps || Math.abs(a.y - b.y) > eps || Math.abs(a.z - b.z) > eps
    ...
    || (a.lc | 0) !== (b.lc | 0)   // PEERLIGHT2: a Light spell cast or run out goes out at once
    || (a.lt | 0) !== (b.lt | 0)   // PEERLIGHT1: a torch lit, put out or guttering goes out at once
...
export function validPose(p) {
  ...
  const { x, y, z, yaw, pitch, mv, wd, an, as, am, sr, cn, cr, ce, ar, fk, rd, rv, hs, lh, wb, dd, hl, lt, lc, hk, hp, hb, hq, hu, uq } = p;
  ...
  return {
    ...
    ...rideOf(rd, rv, hs),
    // DISC12 (2026-09-23, Discord: "Weapons when swapped into left hand dont work showing fists"; Mac: werewolves
    // "not wired correctly"): `lh` - the player fights with the LEFT hand (WeaponManager.UsingRightHand false), so the
    // others draw and hear the weapon in THAT hand (the look carries both); `wb` - the player is in beast form, 1 the
    // werewolf and 2 the wereboar (LycanthropyTypes), so the others see the beast, not the person. Both OMITTED at 0
    ...
    // your way as well"): `lt` - the lit light's steady radius x2, +1 while it gutters (systems/playerTorch.js
    // torchPoseByte), so the others put a point light at this player's hand. OMITTED while nothing burns.
    ...(uint(lt, 127) > 1 ? { lt: uint(lt, 127) } : {}),
    // PEERLIGHT2 (2026-09-26, the player: "can the candle spell of mages also make light for others?"): `lc` 1 while
    // the player's Light spell burns (hasActiveEffect 'light'), so the others hang its candle - sprite and light -
    // before them. OMITTED otherwise.
    ...(uint(lc, 1) ? { lc: 1 } : {}),
    // PEERFX1 (2026-09-26): a player's landed weapon blows (`hk`, a count) with the struck point (`hp`, this frame's
    // own world coordinates, bounded as x/y/z are), the struck body's blood record (`hb`) and the blow's share of its
    // health (`hq`, 0-100); and the times the player was struck (`hu`, a count). The others ring and splash them
    // (net/peerFx.js). OMITTED at 0 (validLook's `class` law): a player who never fought keeps the bytes it had.
    ...peerFxOf(hk, hp, hb, hq, hu, uq),
  };
}
...
function peerFxOf(hk, hp, hb, hq, hu, uq) {
  const out = {};
  const k = uint(hk, 65535) ?? 0;
  const pt = Array.isArray(hp) && hp.length === 3 && hp.every(finite) && Math.abs(hp[0]) <= POSE_BOUND && Math.abs(hp[2]) <= POSE_BOUND && Math.abs(hp[1]) <= POSE_Y_BOUND ? [hp[0], hp[1], hp[2]] : null;
  if (k && pt) { out.hk = k; out.hp = pt; out.hb = uint(hb, 63) ?? 0; out.hq = uint(hq, 100) ?? 0; }
  const u = uint(hu, 65535) ?? 0;
  if (u) { out.hu = u; out.uq = uint(uq, 100) ?? 20; }   // PEERFX2: and what that blow cost, 0-100 (the splash's size)
  return out;
}
...
export function validLook(look) {
  ...
  // on the class"): `eo` - the Eye Of The Beholder on-foot set the player chose (Graphics.OnFoot, 0-15), sent while
...
/** One client frame, parsed and checked: {t:'hello'|'pose'|'ping'|'chat'|'roll'|'say'|'narrate'|'stage'|'mute'|'renown'|'guild'|'guildout'|'world'|'foes'|'hit'|'act'|'who'|'quest'|'social'|'party'|'trade'|'cast'|'card'|'page'|'duel'|'park'|'look'|'gate', ...}
...
export function parseClient(text, { hasHello = false } = {}) {
  ...
  if (m.t === 'chat') {
    ...
    // EMOTE1: an ACTION - `/me waves` - is a line said as what the speaker DOES ("Bran waves"): `me: true`, and
    // nothing else is admitted in its place (a truthy junk value is refused, never read as an action)
    if (m.me !== undefined && m.me !== true) return { error: 'bad chat' };
    const me = m.me === true ? { me: true } : {};
    if (m.ch === undefined) return { t: 'chat', text, ...me };
    return CHAT_LINE_CHANNELS.includes(m.ch) ? { t: 'chat', text, ch: m.ch, ...me } : { error: 'bad chat' };
  }
  if (m.t === 'roll') {
    // DICE1: A ROLL ASKED OF THE RELAY - how many dice, how many sides, what to add, inside the dice's own bounds
    // (net/dice.js). The relay rolls them; nothing on this frame is a result. A channel is the chat line's law.
    if (!hasHello) return { error: 'roll before hello' };
    const spec = { n: m.n, m: m.m, k: m.k };
    if (!validRollSpec(spec)) return { error: 'bad roll' };
    if (m.ch === undefined) return { t: 'roll', ...spec };
    return CHAT_LINE_CHANNELS.includes(m.ch) ? { t: 'roll', ...spec, ch: m.ch } : { error: 'bad roll' };
  }
  if (m.t === 'say') {
  ...
  if (m.t === 'narrate') {
    // TITLE-N: THE DUNGEON MASTER'S LINE, ASKED FOR - `say`'s law exactly: the shape here, and whether this socket
    // may is the relay's question, asked of the signed token it alone holds the key to.
    if (!hasHello) return { error: 'narrate before hello' };
    const text = typeof m.text === 'string' ? sanitizeChat(m.text) : '';
  ...
  if (m.t === 'guild' || m.t === 'guildout') {
    // GUILD1c: THE SHAPE ONLY, as the renown order's. Whether it is real, and (for `guild`) whether it names the socket
    // that carried it, are the relay's questions - a signature, and the account the hello's token verified.
    if (!hasHello) return { error: `${m.t} before hello` };
    return typeof m.order === 'string' && m.order.length > 0 && m.order.length <= 1024
      ? { t: m.t, order: m.order } : { error: `bad ${m.t}` };
  }
...
/** GUILD1c: the guild orders' own bucket - see GUILD_ORDER_HZ_MAX. */
export const guildGate = (bucket, nowMs) => tokenGate(bucket, nowMs, GUILD_ORDER_HZ_MAX);
/** GUILD1c: the room's own guild-tag budget - see GUILD_ROOM_HZ_MAX. */
export const guildRoomGate = (bucket, nowMs) => tokenGate(bucket, nowMs, GUILD_ROOM_HZ_MAX);
...
/** GUILD1c: the gate on GUILD lines coming in - the hub's own guild budget (GUILD_CHAT_ROOM_HZ_MAX). */
export const guildChatInGate = (bucket, nowMs) => tokenGate(bucket, nowMs, GUILD_CHAT_ROOM_HZ_MAX);
...
export function badged(row, from) {
  ...
  // GUILD1c: AND THE GUILD'S TAG, by the same law - off the verified token (or a signed guild order since), absent for
  // none. The tag alone: which guild, and which member of it, are the relay's to route a guild's chat and a removal
  // by, and nobody else's to read.
  const gt = from?.gt;
  if (typeof gt === 'string' && GUILD_TAG_RE.test(gt)) row.gt = gt;
  return row;
}
...
/** GUILD1c: the guild tag a row carries, as a client reads it back - `badged`'s other half; null for none, and for
 *  anything the law's shape does not admit (a stranger's word about themselves). */
export const readGuildTag = (row) => (typeof row?.gt === 'string' && GUILD_TAG_RE.test(row.gt) ? row.gt : null);
 ...
 *  socket), the same deploy as the channels (CHAN_RELAY_MIN). A roll is asked only of a relay that says it rolls. */
export const ROLL_RELAY_MIN = 102;   // MERGE: the arc's deploy is world102 - main's world99 (HCC-PARK), world100 (DISC7) and world101 (DISC12) roll nothing
/** EMOTE1: the relay that first carries an ACTION line (`me: true`) - an older one projects the line to `{t:'chat',
...
/** TITLE-N: the relay that first carries the Dungeon Master's line (`{t:'narrate'}` in, `{t:'dm'}` out) - an older one
 *  answers the frame with 'unknown message' and CLOSES the socket, so /dm is sent only to a relay that says it can. */
...
export const CHAN_RELAY_MIN = 102;   // MERGE: world102 - main's world99, world100 and world101 route no channel of the arc's
...
export const CARD_RELAY_MIN = 102;   // MERGE: world102, as CHAN_RELAY_MIN
...
export const PAGE_RELAY_MIN = 102;   // MERGE: world102, as CARD_RELAY_MIN
...
/** GUILD1c: the guild frames and the guild's chat line are world115's (world113 on the branch; main's AUDIT WB took
 *  world113 and the Enhanced Plus patch world114 first, and neither routes them) - an older relay closes the socket on a
 *  frame type it does not know and refuses a line naming a channel it does not route, so neither goes to a relay whose
 *  welcome said less (DM_RELAY_MIN's reason). */
export const GUILD_RELAY_MIN = 115;
export const relaySupportsGuild = (v) => { const m = /^world(\d+)$/.exec(typeof v === 'string' ? v : ''); return !!m && Number(m[1]) >= GUILD_RELAY_MIN; };

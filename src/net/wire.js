...
/** One client frame, parsed and checked: {t:'hello'|'pose'|'ping'|'chat'|'roll'|'say'|'narrate'|'mute'|'renown'|'world'|'foes'|'hit'|'act'|'who'|'quest'|'social'|'party'|'trade'|'cast'|'card'|'page'|'duel'|'park', ...}
...
/** RENOWN1: the renown frame is world108's - an older relay closes the socket on a frame type it does not know, so a level
 *  order goes only to a relay whose welcome said world108 or later (DM_RELAY_MIN's reason). */
export const RENOWN_RELAY_MIN = 108;

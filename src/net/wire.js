...
/** One client frame, parsed and checked: {t:'hello'|'pose'|'ping'|'chat'|'roll'|'say'|'narrate'|'stage'|'mute'|'renown'|'guild'|'guildout'|'world'|'foes'|'hit'|'act'|'who'|'quest'|'social'|'party'|'trade'|'cast'|'card'|'page'|'duel'|'park'|'look', ...}
...
/** GUILD1c: the guild frames and the guild's chat line are world113's - an older relay closes the socket on a frame
 *  type it does not know and refuses a line naming a channel it does not route, so neither goes to a relay whose welcome
 *  said less (DM_RELAY_MIN's reason). */
export const GUILD_RELAY_MIN = 113;

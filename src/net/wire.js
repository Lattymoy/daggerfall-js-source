...
//                    {t:'level', order}                 ADV1: a LEVEL order the account service signed when the carrier's own
//                                                       Adventuring Level rose; the room takes it only from the account it names (LEVEL_HZ_MAX)
...
//                    ADV1: and `lv`, the Adventuring Level the token carried, absent when it carried none
//                    {t:'level', id, lv}                ADV1: a player's Adventuring Level rose - to everyone in the room, the carrier included
...
import { TITLES, GLYPHS, GLYPHS_MAX, levelIssuable } from './identityToken.js';   // ACC3: the badge vocabulary, closed - `badged` writes it and `readBadge` checks it back   // ADV1: and the level's bound, `badged` and `readLevel` alike
...
/** ADV1: level orders a socket may carry a second. One, and far more than a level rises: the gate is only there so a
 *  socket cannot make the room verify signatures without limit. */
export const LEVEL_HZ_MAX = 1;
...
/** One client frame, parsed and checked: {t:'hello'|'pose'|'ping'|'chat'|'roll'|'say'|'narrate'|'mute'|'level'|'world'|'foes'|'hit'|'act'|'who'|'quest'|'social'|'party'|'trade'|'cast'|'card'|'page'|'duel'|'park', ...}
...
export function parseClient(text, { hasHello = false } = {}) {
  ...
  if (m.t === 'level') {
    // ADV1: THE SHAPE ONLY, as the mute order's. Whether it is real, and whether it names the socket that carried it,
    // are the relay's questions - a signature and the account the hello's token verified.
    if (!hasHello) return { error: 'level before hello' };
    return typeof m.order === 'string' && m.order.length > 0 && m.order.length <= 1024
      ? { t: 'level', order: m.order } : { error: 'bad level' };
  }
...
/** ADV1: the level order's own bucket - see LEVEL_HZ_MAX. */
export const levelGate = (bucket, nowMs) => tokenGate(bucket, nowMs, LEVEL_HZ_MAX);
...
export function badged(row, from) {
  ...
  // ADV1: AND THE ADVENTURING LEVEL, beside them and by the same law: read off the verified token (or a signed level
  // order since), absent when there is none. It is a number, so it is checked against the one bound both ends share.
  const lv = from?.lv;
  if (levelIssuable(lv)) row.lv = lv;
  return row;
}

/** ADV1: the level a row carries, as a client reads it back - `badged`'s other half, as `readBadge` is the badge's;
 *  null for none, and for anything the bound does not admit (a stranger's word about themselves). */
export const readLevel = (row) => (levelIssuable(row?.lv) ? row.lv : null);
...
/** ADV1: the level frame is world108's - an older relay closes the socket on a frame type it does not know, so a level
 *  order goes only to a relay whose welcome said world108 or later (DM_RELAY_MIN's reason). */
export const LEVEL_RELAY_MIN = 108;
export const relaySupportsLevel = (v) => { const m = /^world(\d+)$/.exec(typeof v === 'string' ? v : ''); return !!m && Number(m[1]) >= LEVEL_RELAY_MIN; };

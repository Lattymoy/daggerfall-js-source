...
export function lastOnlineText(online, seen, now) {
  ...
  if (ago < min) return 'Last online just now';
  if (ago < hour) return `Last online ${Math.floor(ago / min)} min ago`;
  if (ago < day) return `Last online ${Math.floor(ago / hour)} h ago`;
  if (ago < 2 * day) return 'Last online yesterday';
  if (ago < week * 2) return `Last online ${Math.floor(ago / day)} days ago`;
  if (ago < 365 * day) return `Last online ${Math.floor(ago / week)} weeks ago`;

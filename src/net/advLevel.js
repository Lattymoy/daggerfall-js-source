// @ts-check
// ═══════════════════════════════════════════════════════════════════
// ADV1 (2026-09-24) — THE ADVENTURING LEVEL: online's own level, kept
// beside Daggerfall's and never in its place.
//
...
// the curve is a long "Grind", and offline play earns "No" Adventuring
// XP - "Plus having their level appear on the left side of character
// name and profile main menu + ingame profile".
//
// ═══ WHAT IT IS, AND WHAT IT NEVER TOUCHES ═════════════════════════
//
// A second track PER CHARACTER, kept by the account service
// (server-account/src/adventuring.js) and never in the save. The
...
//
// Online, and only online, the track adds `advBonus(level)` to the
...
// holds the bounds: at most ADV_XP_REPORT_MAX a report and
// ADV_XP_HOUR_MAX an hour per ACCOUNT, across all its characters. The
...
import { ADV_LEVEL_MAX } from './identityToken.js';

export { ADV_LEVEL_MAX };
...
export function advXpForLevel(level) {
  const L = Math.max(1, Math.min(ADV_LEVEL_MAX, Math.trunc(Number(level) || 1)));
  const n = L - 1;
...
export const ADV_XP_MAX = advXpForLevel(ADV_LEVEL_MAX);

/** The level a total makes: the highest level whose total it has reached. */
export function advLevelForXp(xp) {
  const x = Number.isFinite(xp) ? Math.max(0, Math.trunc(xp)) : 0;
  let L = 1;
  while (L < ADV_LEVEL_MAX && advXpForLevel(L + 1) <= x) L++;
  return L;
}
 ...
 * and `frac` is 1.
 */
export function advProgress(xp) {
  const total = Number.isFinite(xp) ? Math.max(0, Math.min(ADV_XP_MAX, Math.trunc(xp))) : 0;
  const level = advLevelForXp(total);
  if (level >= ADV_LEVEL_MAX) return { level, xp: total, into: 0, need: 0, frac: 1 };
  const from = advXpForLevel(level);
  const need = advXpForLevel(level + 1) - from;
 ...
 * room earns the whole kill, plus ADV_PARTY_BONUS_PCT a head beyond the
 ...
 */
export const ADV_KILL_XP_PER_LEVEL = 10;
/** A foe's level is read up to here - Daggerfall's own foes stop near 21 and a career foe follows the player. */
export const ADV_KILL_LEVEL_MAX = 30;
export const ADV_QUEST_XP_BASE = 100;
export const ADV_QUEST_XP_PER_LEVEL = 40;
/** A quest's character level is read up to here, as a foe's is. */
export const ADV_QUEST_LEVEL_MAX = 30;
/** Each partymate in the room beyond the first adds this much to every kill, per cent. */
export const ADV_PARTY_BONUS_PCT = 10;
/** The most partymates the bonus counts - the party's own seats. */
export const ADV_PARTY_COUNT_MAX = 8;

const clampInt = (v, lo, hi) => Math.max(lo, Math.min(hi, Math.round(Number(v) || 0)));

/** A kill's XP, by the foe's level. */
export const advKillXp = (foeLevel) => ADV_KILL_XP_PER_LEVEL * clampInt(foeLevel, 1, ADV_KILL_LEVEL_MAX);

/** A quest's XP, by the character's Daggerfall level when it was done. */
export const advQuestXp = (characterLevel) => ADV_QUEST_XP_BASE + ADV_QUEST_XP_PER_LEVEL * clampInt(characterLevel, 1, ADV_QUEST_LEVEL_MAX);
...
export function advPartyXp(xp, present) {
  const n = clampInt(present, 1, ADV_PARTY_COUNT_MAX);
  return Math.round((Math.max(0, Math.trunc(Number(xp) || 0)) * (100 + ADV_PARTY_BONUS_PCT * (n - 1))) / 100);
}
 ...
 */
export const ADV_HP_PER_LEVEL = 3;
export const ADV_MP_PER_LEVEL = 2;

/** The health and magicka a level adds online. */
export function advBonus(level) {
  const L = clampInt(level, 1, ADV_LEVEL_MAX);
  return { hp: ADV_HP_PER_LEVEL * (L - 1), mp: ADV_MP_PER_LEVEL * (L - 1) };
}
 ...
 * carries at most ADV_XP_REPORT_MAX (a minute of the fiercest fighting a
 * group does, several times over), and an ACCOUNT earns at most
 * ADV_XP_HOUR_MAX in a clock hour, across all its characters - roughly
 ...
 */
export const ADV_XP_REPORT_MAX = 5_000;
export const ADV_XP_HOUR_MAX = 20_000;
/** How often the client sends what it has earned. */
export const ADV_REPORT_MS = 60_000;
/** Tracks per account - one a character; a new character past this is refused (SAVES_MAX's reason: a bound on rows). */
export const ADV_TRACKS_MAX = 60;
/** A character's name as the service keeps it for the account card - display only, bounded. */
export const ADV_NAME_MAX = 32;

/* ═══ THE WORDS ═══════════════════════════════════════════════════════ */

/** The level as it sits left of a name: "Lv 12". Null for none. */
export function advLevelText(level) {
  if (!Number.isSafeInteger(level) || level < 1 || level > ADV_LEVEL_MAX) return null;
  return `Lv ${level}`;
}

const grouped = (n) => String(n).replace(/\B(?=(\d{3})+(?!\d))/g, ',');

/** A track's progress in words: "1,234 / 5,510 XP to level 10", or "Level 50 - the highest" at the cap. */
export function advProgressText(xp) {
  const p = advProgress(xp);
  if (p.level >= ADV_LEVEL_MAX) return `Level ${ADV_LEVEL_MAX} - the highest`;
  return `${grouped(p.into)} / ${grouped(p.need)} XP to level ${p.level + 1}`;
}

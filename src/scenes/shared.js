...
export function createSkyController(gl, params) {
  ...
  // generated by drawing, a draw path before the first frame).
  const cloudsDoor = params.get('clouds');
  const clouds = enhancedSky && cloudsDoor !== 'off'
    ? new VolumetricClouds(gl, cloudsDoor in CLOUD_QUALITY ? cloudsDoor : 'default', [0, 0, gl.drawingBufferWidth, gl.drawingBufferHeight]) : null;

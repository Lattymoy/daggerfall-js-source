...
export function createWorldModes(host) {
   ...
   * record these hosts mint spells it `name` (exterior.js:3490 hands
   ...
   *  the C11 law dungeonContext.js:1747 states), so the same visual
   *  result needs the shift on the DUNGEON side - which is exactly the
   *  shift the dungeon's own RDB flats already take
   *  (dungeonContext.js:1645, `y - size.h / 2`), and which a building's
...
// ROAD-B B1: the whole stack leaves with the interior, not just its
    ...
    try {
      const ctx = await buildDungeonContext(
        ...
        dfLocation, blocks, dfLocation.climate.climateType, {
          ...
          // (dungeonContext.js:6105), so the OUTER host's one rides in.
  ...
  function frame(dt, now) {
    ...
    // which is what dungeon.js:306's `held` already implements.
    ...
    if (mode === 'dungeon' && dungeonCtx) {
      ...
      // only dungeon.js:368 fed it, so in a world-hosted dungeon the
    ...
    if (mode === 'dungeon') {
      ...
      if (!overlayHeld) dungeonCtx.actions.update(dt);   // dungeon.js:307's `if (!held)` - a paused game advances no movers
...
// AUDIT 39 (#132): the flag the other three hosts pass and
  ...
  function installShotProbes() {
    ...
    // scenes/dungeon.js:385-439 carries, so a probe could take the
  ...
  addEventListener('mousedown', (e) => {
    ...
    // host bag (`exterior.js:3551`, `world.js`'s twin), and the OUTER

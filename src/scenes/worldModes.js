...
export function createWorldModes(host) {
   ...
   * record these hosts mint spells it `name` (exterior.js:3710 hands
   ...
   *  the C11 law dungeonContext.js:1816 states), so the same visual
   *  result needs the shift on the DUNGEON side - which is exactly the
   *  shift the dungeon's own RDB flats already take
   *  (dungeonContext.js:1701, `y - size.h / 2`), and which a building's
...
// ROAD-B B1: the whole stack leaves with the interior, not just its
  ...
  function frame(dt, now) {
    ...
    // which is what dungeon.js:324's `held` already implements.
    ...
    if (mode === 'dungeon') {
      ...
      if (!overlayHeld) dungeonCtx.actions.update(dt);   // dungeon.js:325's `if (!held)` - a paused game advances no movers
    ...
    interiorArrows.update(dt, {
      ...
      onPlayerHit: (m) => {
        ...
        if (dmg > 0) {
          ...
          // a blow (world.js:9185's own wave-46 note); the interior
...
// AUDIT 39 (#132): the flag the other three hosts pass and
  ...
  addEventListener('mousedown', (e) => {
    ...
    // host bag (`exterior.js:3772`, `world.js`'s twin), and the OUTER
  ...
  return {
     ...
     *  unconditionally (world.js:6362). So an F9 pressed in a shop
     ...
     *  worldQuickLoad calls forceExitToExterior FIRST (world.js:6455)
     ...
     *  HARD2c: this used to spell them out, and named `world.js:6616`

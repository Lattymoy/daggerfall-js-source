...
export function createWorldModes(host) {
   ...
   * record these hosts mint spells it `name` (exterior.js:3527 hands
   ...
   *  READ the effect list every frame (exteriorFoes.js:862-866 and
   ...
   *  the C11 law dungeonContext.js:1805 states), so the same visual
   *  result needs the shift on the DUNGEON side - which is exactly the
   *  shift the dungeon's own RDB flats already take
   *  (dungeonContext.js:1690, `y - size.h / 2`), and which a building's
    ...
    try {
      const ctx = await buildDungeonContext(
        ...
        dfLocation, blocks, dfLocation.climate.climateType, {
          ...
          // (dungeonContext.js:6240), so the OUTER host's one rides in.
    ...
    interiorArrows.update(dt, {
      ...
      onPlayerHit: (m) => {
        ...
        if (dmg > 0) {
          ...
          // a blow (world.js:8378's own wave-46 note); the interior
...
// AUDIT 39 (#132): the flag the other three hosts pass and
  ...
  addEventListener('mousedown', (e) => {
    ...
    // host bag (`exterior.js:3588`, `world.js`'s twin), and the OUTER
  ...
  return {
     ...
     *  unconditionally (world.js:5586). So an F9 pressed in a shop
     ...
     *  worldQuickLoad calls forceExitToExterior FIRST (world.js:5678)
     ...
     *  HARD2c: this used to spell them out, and named `world.js:5823`
     *  and `dungeonContext.js:6249` for its two sibling copies - lines

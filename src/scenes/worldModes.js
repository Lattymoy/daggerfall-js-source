...
export function createWorldModes(host) {
   ...
   *  READ the effect list every frame (exteriorFoes.js:787-788 and
    ...
    try {
      const ctx = await buildDungeonContext(
        ...
        dfLocation, blocks, dfLocation.climate.climateType, { activateHeld: () => held(keys, 'ActivateCenterObject') || !!host.activateDown?.(), useMagicItem: (item) => host.useMagicItem?.(item), onFoeHit: (hit) => host.onFoeHit?.(hit),   // WORLD2: a puppet's blow goes to the host
          ...
          // (dungeonContext.js:5263), so the OUTER host's one rides in.
    ...
    interiorArrows.update(dt, {
      ...
      onPlayerHit: (m) => {
        ...
        if (dmg > 0) {
          ...
          // a blow (world.js:6326's own wave-46 note); the interior
...
// AUDIT 39 (#132): the flag the other three hosts pass and
  ...
  return {
     ...
     *  unconditionally (world.js:4203). So an F9 pressed in a shop
     ...
     *  worldQuickLoad calls forceExitToExterior FIRST (world.js:4263)
     ...
     *
     *  FLAG ONLY, presence-gated, exactly as world.js:4369/:4371 and
     *  dungeonContext.js:5336/:5337 are: the C# restore sets the

    ...
    try {
      const ctx = await buildDungeonContext(
        ...
        dfLocation, blocks, dfLocation.climate.climateType, { activateHeld: () => held(keys, 'ActivateCenterObject') || !!host.activateDown?.(), useMagicItem: (item) => host.useMagicItem?.(item), onFoeHit: (hit) => host.onFoeHit?.(hit),   // WORLD2: a puppet's blow goes to the host
          ...
          // (dungeonContext.js:5363), so the OUTER host's one rides in.
    ...
    interiorArrows.update(dt, {
      ...
      onPlayerHit: (m) => {
        ...
        if (dmg > 0) {
          ...
          // a blow (world.js:6342's own wave-46 note); the interior
...
// AUDIT 39 (#132): the flag the other three hosts pass and
  ...
  return {
     ...
     *  and exterior.js's - which this seam does not reach: that host has no save path at all, its charter exterior.js:2597-2619), and IS1 routed the inside-a-building save to
     *  the WORLD host's composer - which reads its own exterior rig
     *  unconditionally (world.js:4219). So an F9 pressed in a shop
     ...
     *  worldQuickLoad calls forceExitToExterior FIRST (world.js:4279)
     ...
     *
     *  FLAG ONLY, presence-gated, exactly as world.js:4385/:4387 and

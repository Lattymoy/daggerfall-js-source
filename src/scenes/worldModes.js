...
export function createWorldModes(host) {
   ...
   * record these hosts mint spells it `name` (exterior.js:3668 hands
   ...
   *  the C11 law dungeonContext.js:1808 states), so the same visual
   *  result needs the shift on the DUNGEON side - which is exactly the
   *  shift the dungeon's own RDB flats already take
   *  (dungeonContext.js:1693, `y - size.h / 2`), and which a building's
...
// ROAD-B B1: the whole stack leaves with the interior, not just its
    ...
    try {
      const ctx = await buildDungeonContext(
        ...
        dfLocation, blocks, dfLocation.climate.climateType, {
          ...
          // (dungeonContext.js:6243), so the OUTER host's one rides in.
  ...
  function tryExitDungeon({ pressCast = false } = {}) {
    ...
    // scenes/dungeon.js:242's, line for line, over this context's pool.
    ...
    const _enemyArm = (reach, nearerThan = Infinity) => (dungeonCtx ? tryMobileEnemyActivate(eye, dir, dungeonCtx.foes, dungeonCtx.collider,
      reach, getInteractionMode(), playerEntity, {
        ...
        // dungeon's own. scenes/dungeon.js:256-257 is the same pair.
  ...
  function frame(dt, now) {
    ...
    // which is what dungeon.js:320's `held` already implements.
    ...
    // the crouch edge still toggled. dungeon.js:550 is this same gate
    // ("no movers, no motor").
    if (!overlayHeld) {
    ...
    if (mode === 'dungeon' && dungeonCtx) {
      ...
      // only dungeon.js:393 fed it, so in a world-hosted dungeon the
    ...
    if (mode === 'dungeon') {
      ...
      if (!overlayHeld) dungeonCtx.actions.update(dt);   // dungeon.js:321's `if (!held)` - a paused game advances no movers
    ...
    interiorArrows.update(dt, {
      ...
      onPlayerHit: (m) => {
        ...
        if (dmg > 0) {
          ...
          // a blow (world.js:9107's own wave-46 note); the interior
      ...
      onPlayerArrowHitFoe: (m, t) => playerArrowHitFoe(m, t, {
        ...
        // (cityGuards.js:590-595), so this seam splits by pool exactly
        // as `dealDamage` above it does rather than dropping the
        // non-encounter half - the zero-damage SWING already reaches
        // that door (cityGuards.js:1082) and the shaft owes the same.
...
// AUDIT 39 (#132): the flag the other three hosts pass and
  ...
  function installShotProbes() {
    ...
    // scenes/dungeon.js:410-464 carries, so a probe could take the
  ...
  addEventListener('mousedown', (e) => {
    ...
    // host bag (`exterior.js:3730`, `world.js`'s twin), and the OUTER
    ...
    // gesture), never a swing - dungeon.js:240 and both exterior slots
  ...
  return {
     ...
     *  and exterior.js's - which this seam does not reach: that host has no save path at all, its charter exterior.js:3316-3338), and IS1 routed the inside-a-building save to
     ...
     *  HARD2c: this used to spell them out, and named `world.js:6539`
     *  and `dungeonContext.js:6252` for its two sibling copies - lines

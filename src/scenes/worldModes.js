...
export function createWorldModes(host) {
   ...
   *  READ the effect list every frame (exteriorFoes.js:906-910 and
   ...
   *  the C11 law dungeonContext.js:1853 states), so the same visual
   *  result needs the shift on the DUNGEON side - which is exactly the
   *  shift the dungeon's own RDB flats already take
   *  (dungeonContext.js:1738, `y - size.h / 2`), and which a building's
    ...
    try {
      ...
      const ctx = await buildDungeonContext(
        ...
        dfLocation, blocks, dfLocation.climate.climateType, {
          ...
          // (dungeonContext.js:6337), so the OUTER host's one rides in.
    ...
    interiorArrows.update(dt, {
      ...
      onPlayerHit: (m) => {
        ...
        if (dmg > 0) {
          ...
          // a blow (world.js:9537's own wave-46 note); the interior
...
// AUDIT 39 (#132): the flag the other three hosts pass and
  ...
  return {
     ...
     *  unconditionally (world.js:6655). So an F9 pressed in a shop
     ...
     *  worldQuickLoad calls forceExitToExterior FIRST (world.js:6755)
     ...
     *  HARD2c: this used to spell them out, and named `world.js:6922`
     *  and `dungeonContext.js:6346` for its two sibling copies - lines

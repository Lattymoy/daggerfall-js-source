...
// ROAD-B B1: the whole stack leaves with the interior, not just its
  ...
  function frame(dt, now) {
    ...
    // which is what dungeon.js:242's `held` already implements.
    ...
    // the crouch edge still toggled. dungeon.js:393 is this same gate
    // ("no movers, no motor").
    if (!overlayHeld) {
    ...
    if (mode === 'dungeon' && dungeonCtx) {
      ...
      // only dungeon.js:294 fed it, so in a world-hosted dungeon the
...
// a12: SwitchHand (H) - ActionComplete's RELEASE edge
    ...
    if (mode === 'dungeon') {
      ...
      if (!overlayHeld) dungeonCtx.actions.update(dt);   // dungeon.js:243's `if (!held)` - a paused game advances no movers
    ...
    interiorArrows.update(dt, {
      ...
      onPlayerHit: (m) => {
        ...
        if (dmg > 0) {
          ...
          // a blow (world.js:5463's own wave-46 note); the interior
...
// AUDIT 39 (#132): the flag the other three hosts pass and
  ...
  function installShotProbes() {
    ...
    // scenes/dungeon.js:303-329 carries, so a probe could take the
  ...
  addEventListener('mousedown', (e) => {
    ...
    // gesture), never a swing - dungeon.js:207 and both exterior slots

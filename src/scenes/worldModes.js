...
// ROAD-B B1: the whole stack leaves with the interior, not just its
  ...
  function frame(dt, now) {
    ...
    // which is what dungeon.js:291's `held` already implements.
    ...
    // the crouch edge still toggled. dungeon.js:493 is this same gate
    // ("no movers, no motor").
    if (!overlayHeld) {
    ...
    if (mode === 'dungeon' && dungeonCtx) {
      ...
      // only dungeon.js:345 fed it, so in a world-hosted dungeon the
...
// a12: SwitchHand (H) - ActionComplete's RELEASE edge
    ...
    if (mode === 'dungeon') {
      ...
      if (!overlayHeld) dungeonCtx.actions.update(dt);   // dungeon.js:292's `if (!held)` - a paused game advances no movers
    ...
    interiorArrows.update(dt, {
      ...
      onPlayerHit: (m) => {
        ...
        if (dmg > 0) {
          ...
          // a blow (world.js:6545's own wave-46 note); the interior
...
// AUDIT 39 (#132): the flag the other three hosts pass and
  ...
  function installShotProbes() {
    ...
    // scenes/dungeon.js:362-412 carries, so a probe could take the
  ...
  addEventListener('mousedown', (e) => {
    ...
    // host bag (`exterior.js:3132`, `world.js`'s twin), and the OUTER
  ...
  return {
     ...
     *  and exterior.js's - which this seam does not reach: that host has no save path at all, its charter exterior.js:2709-2731), and IS1 routed the inside-a-building save to
     *  the WORLD host's composer - which reads its own exterior rig
     *  unconditionally (world.js:4359). So an F9 pressed in a shop
     ...
     *  worldQuickLoad calls forceExitToExterior FIRST (world.js:4441)
     ...
     *  HARD2c: this used to spell them out, and named `world.js:4553`

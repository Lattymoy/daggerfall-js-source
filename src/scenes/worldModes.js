...
export function createWorldModes(host) {
   ...
   * record these hosts mint spells it `name` (exterior.js:3518 hands
   ...
   *  the C11 law dungeonContext.js:1782 states), so the same visual
   *  result needs the shift on the DUNGEON side - which is exactly the
   *  shift the dungeon's own RDB flats already take
   *  (dungeonContext.js:1680, `y - size.h / 2`), and which a building's
...
// AUDIT 39 (#132): the flag the other three hosts pass and
  ...
  addEventListener('mousedown', (e) => {
    ...
    // host bag (`exterior.js:3579`, `world.js`'s twin), and the OUTER

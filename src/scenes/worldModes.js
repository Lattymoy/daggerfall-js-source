...
export function createWorldModes(host) {
   ...
   * record these hosts mint spells it `name` (exterior.js:3744 hands
   ...
   *  the C11 law dungeonContext.js:1833 states), so the same visual
   *  result needs the shift on the DUNGEON side - which is exactly the
   *  shift the dungeon's own RDB flats already take
   *  (dungeonContext.js:1718, `y - size.h / 2`), and which a building's
    ...
    try {
      const ctx = await buildDungeonContext(
        ...
        dfLocation, blocks, dfLocation.climate.climateType, {
          ...
          // (dungeonContext.js:6290), so the OUTER host's one rides in.
...
// AUDIT 39 (#132): the flag the other three hosts pass and
  ...
  addEventListener('mousedown', (e) => {
    ...
    // host bag (`exterior.js:3806`, `world.js`'s twin), and the OUTER
  ...
  return {
     ...
     *  unconditionally (world.js:6480). So an F9 pressed in a shop
     ...
     *  worldQuickLoad calls forceExitToExterior FIRST (world.js:6573)
     ...
     *  HARD2c: this used to spell them out, and named `world.js:6734`
     *  and `dungeonContext.js:6299` for its two sibling copies - lines

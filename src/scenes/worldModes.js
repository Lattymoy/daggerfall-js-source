...
export function createWorldModes(host) {
   ...
   *  the C11 law dungeonContext.js:1492 states), so the same visual
   *  result needs the shift on the DUNGEON side - which is exactly the
   *  shift the dungeon's own RDB flats already take
   *  (dungeonContext.js:1397, `y - size.h / 2`), and which a building's
...
// ROAD-B B1: the whole stack leaves with the interior, not just its
    ...
    try {
      const ctx = await buildDungeonContext(
        ...
        dfLocation, blocks, dfLocation.climate.climateType, { activateHeld: () => held(keys, 'ActivateCenterObject') || !!host.activateDown?.(), useMagicItem: (item) => host.useMagicItem?.(item),
          ...
          // (dungeonContext.js:4706), so the OUTER host's one rides in.
  ...
  function tryExitDungeon() {
    ...
    // scenes/dungeon.js:224's, line for line, over this context's pool.
    ...
    const _enemyArm = (reach, nearerThan = Infinity) => (dungeonCtx ? tryMobileEnemyActivate(eye, dir, dungeonCtx.foes, dungeonCtx.collider,
      reach, getInteractionMode(), playerEntity, {
        ...
        // dungeon's own. scenes/dungeon.js:237-238 is the same pair.
  ...
  function frame(dt, now) {
    ...
    // which is what dungeon.js:276's `held` already implements.
    ...
    // the crouch edge still toggled. dungeon.js:454 is this same gate
    // ("no movers, no motor").
    if (!overlayHeld) {
    ...
    if (mode === 'dungeon' && dungeonCtx) {
      ...
      // only dungeon.js:330 fed it, so in a world-hosted dungeon the
...
// a12: SwitchHand (H) - ActionComplete's RELEASE edge
    ...
    if (mode === 'dungeon') {
      ...
      if (!overlayHeld) dungeonCtx.actions.update(dt);   // dungeon.js:277's `if (!held)` - a paused game advances no movers
    ...
    interiorArrows.update(dt, {
      ...
      onPlayerHit: (m) => {
        ...
        if (dmg > 0) {
          ...
          // a blow (world.js:6225's own wave-46 note); the interior
...
// AUDIT 39 (#132): the flag the other three hosts pass and
  ...
  function installShotProbes() {
    ...
    // scenes/dungeon.js:347-381 carries, so a probe could take the
  ...
  addEventListener('mousedown', (e) => {
    ...
    // gesture), never a swing - dungeon.js:222 and both exterior slots
  ...
  return {
     ...
     *  and exterior.js's - which this seam does not reach: that host has no save path at all, its charter exterior.js:2575-2597), and IS1 routed the inside-a-building save to
     ...
     *  dungeonContext.js:4771/:4777 are: the C# restore sets the

    ...
    try {
      ...
      const ctx = await buildDungeonContext(
        ...
        dfLocation, hit.blocksFile ?? blocks, dfLocation.climate.climateType, {   // WB3b: the court's blocks file answers its one made block
          ...
          // (dungeonContext.js:6504), so the OUTER host's one rides in.
    ...
    interiorArrows.update(dt, {
      ...
      onPlayerHit: (m) => {
        ...
        if (dmg > 0) {
          ...
          // a blow (world.js:10160's own wave-46 note); the interior
...
// AUDIT 39 (#132): the flag the other three hosts pass and
  ...
  return {
     ...
     *  unconditionally (world.js:7211). So an F9 pressed in a shop
     ...
     *  worldQuickLoad calls forceExitToExterior FIRST (world.js:7311)
     ...
     *  HARD2c: this used to spell them out, and named `world.js:7479`
     *  and `dungeonContext.js:6515` for its two sibling copies - lines

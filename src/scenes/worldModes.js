...
export function createWorldModes(host) {
  ...
  function openServiceFlow(destination, { guild, memberships, store, rows, route }) {
    ...
    // G4: the four remaining trade-mode services. U40 built every
    // mode and X6 proved the shelf pattern; these were destination
    // strings and nothing else. All four pass the guild's OWN faction
    // id, which is what makes the holiday clause reachable.
    if (destination === 'guildServiceIdentify' && tradeArtLoaded()) {
      // Identify and SellMagic work off the PLAYER'S PACK - there is
      // no shelf to stock, which is why DFU passes the trade window a
      // mode and a guild and nothing else (:347-349, :409-411).

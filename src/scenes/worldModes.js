...
// ROAD-B B1: the whole stack leaves with the interior, not just its
  ...
  function tryExitDungeon() {
    ...
    const _enemyArm = (reach, nearerThan = Infinity) => (dungeonCtx ? tryMobileEnemyActivate(eye, dir, dungeonCtx.foes, dungeonCtx.collider,
      reach, getInteractionMode(), playerEntity, {
        ...
        // dungeon's own. scenes/dungeon.js:241-242 is the same pair.
  ...
  function frame(dt, now) {
    ...
    // which is what dungeon.js:280's `held` already implements.
    ...
    // the crouch edge still toggled. dungeon.js:460 is this same gate
    // ("no movers, no motor").
    if (!overlayHeld) {
    ...
    if (mode === 'dungeon' && dungeonCtx) {
      ...
      // only dungeon.js:334 fed it, so in a world-hosted dungeon the
...
// a12: SwitchHand (H) - ActionComplete's RELEASE edge
    ...
    if (mode === 'dungeon') {
      ...
      if (!overlayHeld) dungeonCtx.actions.update(dt);   // dungeon.js:281's `if (!held)` - a paused game advances no movers
    ...
    interiorArrows.update(dt, {
      ...
      onPlayerHit: (m) => {
        ...
        if (dmg > 0) {
          ...
          // a blow (world.js:6271's own wave-46 note); the interior
...
// AUDIT 39 (#132): the flag the other three hosts pass and
  ...
  function installShotProbes() {
    ...
    // scenes/dungeon.js:351-385 carries, so a probe could take the
  ...
  return {
     ...
     *  and exterior.js's - which this seam does not reach: that host has no save path at all, its charter exterior.js:2595-2617), and IS1 routed the inside-a-building save to
     *  the WORLD host's composer - which reads its own exterior rig
     *  unconditionally (world.js:4167). So an F9 pressed in a shop
     ...
     *  worldQuickLoad calls forceExitToExterior FIRST (world.js:4227)
     ...
     *
     *  FLAG ONLY, presence-gated, exactly as world.js:4312/:4312 and

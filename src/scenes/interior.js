...
export async function bootInterior(canvas, renderer, params, status) {
  ...
  addEventListener('keydown', (e) => {
    ...
    // returns before its own preventDefault - worldModes.js:8196 sits
    ...
    if (overlay) { overlay.input(e.code, e); drainOverlay(); if (!overlay && !gamePaused()) requestLook(canvas); e.preventDefault(); return; }
  ...
  addEventListener('keyup', (e) => {
    ...
    drainOverlay(); if (!overlay && !gamePaused()) requestLook(canvas);
  });
  ...
  function frame(now) {
    ...
    // driver. dungeon.js:733 and worldModes.js:6182/:6210 gate the same

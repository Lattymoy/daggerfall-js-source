...
export function createPlayerMagic({
  ...
  function applySpellToFoe(spell, casterLevel, foe, caster = null, ctx = undefined) {
    const r = applySpell(spell, casterLevel, foe.entity, foeSinks(foe, !caster || caster.entity === playerEntity), rolls, caster, ctx);   // AUDIT WORLD2 B7: a foe's spell is not the player's blow (AUDIT 68 X4: every host's sinks read the second arg)

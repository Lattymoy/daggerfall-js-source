...
export async function bootDungeon(canvas, renderer, params, status) {
  ...
  addEventListener('keyup', (e) => { keys.delete(e.code); noteKeyUp(keyEdge, e.code); if (e.code === 'AltLeft') e.preventDefault(); const hadOverlay = !!ctx.uiOverlayActive; routeKeyUp(e, ctx); if (hadOverlay && !ctx.uiOverlayActive) requestLook(canvas); });   // MWCROUCH: ...and the release, for SwitchHand's ActionComplete edge
  ...
  addEventListener('keydown', (e) => {
    ...
    const hadOverlay = !!ctx.uiOverlayActive; if (routeKey(e, ctx, (p) => player.spawn(p[0], p[1], p[2]), keys)) e.preventDefault(); if (hadOverlay && !ctx.uiOverlayActive) requestLook(canvas);   // P14: a load clears motion state (DFU CancelMovement + ClearFallingDamage)   // AUDIT 58 (f3/input): + the held-keys Set, so a rebound combo reaches the dispatch (InputManager.cs:1666-1712)
  });

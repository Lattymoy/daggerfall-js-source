...
import { TreeModelRenderer } from '../render/treeModels.js';   // TR1: our partner's tree meshes, wearing the player's own sprite; TR6: integrated with the streaming world
...
export async function bootWorld(canvas, renderer, params, status) {
  ...
  const _treeDraw = [];   // TR6: this frame's visible tree batches, collected in the pixel walk with the flats
  ...
  async function buildPixelNow(px, py, { roadsRetry = false } = {}) {
    ...
    const batches = [];
    const treeBatches = [];   // TR1: this pixel's tree batches, freed with the pixel (TR6: per pixel, never per record)
    ...
    for (const [k, centers] of groups) {
      ...
      // TR1: a nature flat with a mesh is drawn as one. TR6: the flat's
      // stored position IS its base - flatBatchAabb spans minY..minY + h
      // and the billboard stands on it - so the mesh stands on the same
      // point, in the PIXEL'S frame like the flat (the translation is the
      // draw's); `size` is {w, h}; the batch keeps the flat's EV3 box and
      // joins the pixel's bounds as the flat would have. Anything that
      // fails here falls through to the flat.
      const rec = treeModels && archive === natureArchive ? treeModels.modelFor(archive, record) : null;
      if (rec) {
        try {
          const bm = t.getDFBitmap(record, 0);
          const tb = treeModels.build(key, archive, record, rec, centers, size.h, bm, {
            ...
            upload: (k2, raster) => renderer.uploadTexture(archive, k2, raster),
          });
          if (tb) { tb._box = flatBatchAabb(centers, size); unionBox(tb._box); treeBatches.push(tb); continue; }
  ...
  function destroyPixel(px, py, { collectLoose = true } = {}) {
    ...
    treeModels?.disposePixel(key);   // TR1: the pixel's trees leave with it - TR6: its OWN, and no other pixel's
...
// AUDIT 39 (#21): a GETTER, not startLoc's number. LowerRepForCrime
  ...
  function frame(now) {
    ...
    for (const p of built.values()) {
      ...
      // TR6: the trees, collected exactly as the flats are - the pixel's
      // translation for the instances (`t` is the pixel's own scratch, so
      // the alias holds for the frame), and the grid translation WITHOUT
      // the floating origin's compensation for the gust's wave, so a
      // recentre moves the trees with the world and never twitches a crown.
      for (const tb of p.treeBatches ?? []) {
        if (!pixelVisible || (cullOn && aabbOutside(_planes, tb._box, t[0], t[1], t[2]))) continue;   // EV3
        tb.origin = t;
        (tb.gustOrigin ??= [0, 0])[0] = t[0] - state.compensation[0]; tb.gustOrigin[1] = t[2] - state.compensation[2];
        _treeDraw.push(tb);
      }
    }
    ...
    // (drawBillboards' law), full-bright in a clockless scene. TR6: the
    // batches the pixel walk found visible, each with its translation.
    if (treeModels && _treeDraw.length) {
      const r = renderer;
      const tint = r._clockLit
        ...
        : [1, 1, 1];
      treeModels.draw(r, proj, view, now / 1000, sceneWind(), tint, _treeDraw);

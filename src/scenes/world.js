...
import { basinDepths, carveBasin, waterMesh, flatDepths } from '../render/waterBasin.js';   // WATER2: the ground carved under the water, the water on its own uncarved grid
...
export async function bootWorld(canvas, renderer, params, status) {
  ...
  async function buildPixelNow(px, py, { roadsRetry = false } = {}) {
    ...
    // WATER4: the archive's own water ART, read once off its bitmaps and
    // kept on the renderer beside its tile array - the water's quads,
    // the basin and the feet all take its tables, the shader its cells.
    ...
    const waterIndices = waterOn ? buildWaterIndices(tilemapBytes, stride, waterArt?.any) : null;
    const water = waterIndices ? buildWater(positions, normals, tilemapBytes, stride, waterIndices, waterArt) : null;
   ...
   *  carved into the grid (in place) for the ground pass. WATER4: the
   *  art's own corners carve it, and where the water reaches no vertex
   *  (a shore tile's lone corner, a puddle) the sheet lies flat on the
   *  ground and the shader gives it its bed. */
  function buildWater(positions, normals, tilemapBytes, stride, waterIndices, art = null) {
    const depths = basinDepths(tilemapBytes, stride, TERRAIN_TILE_DIM, art?.corners) ?? flatDepths(stride, TERRAIN_TILE_DIM);
  ...
  function restrideTerrain(p, stride) {
    ...
    const art = renderer.waterArtOf(p.groundArchive);   // WATER4
    const waterIndices = waterOn ? buildWaterIndices(p.tilemapBytes, stride, art?.any) : null;
    p.water = waterIndices ? buildWater(grid.positions, grid.normals, p.tilemapBytes, stride, waterIndices, art) : null;   // WATER2: before the carved grid is uploaded
...
// AUDIT 39 (#21): a GETTER, not startLoc's number. LowerRepForCrime
  ...
  const enchantReplaceFoe = (targetEntity, mobileType) => {
    ...
    // got exactly what removeGuard (cityGuards.js:1214-1218) gives it -
    // batch freed, `dead = true`, no corpse, skipped by the next AI pass
    // (cityGuards.js:758) and spliced out at the end of it (:940).
  ...
  const playerGroundSample = () => {
    ...
    return { tile: built_.tilemap[tx + ty * TERRAIN_TILE_DIM], feet: [u - tx, v - ty], art: renderer.waterArtOf(built_.groundArchive) };
  };

...
// AUDIT 39 (#21): a GETTER, not startLoc's number. LowerRepForCrime
  ...
  // and dungeonContext.js:2347 mounts the same one, gated on
  // `opts.enchantCtx !== false` because setDefaultEnchantCtx is a
  // session singleton and EC1 already routes THIS host's mount into
  // that context through modes.dungeonCtx - so worldModes.js:4739
  ...
  function worldQuickSave(saveName = QUICK_SAVE_NAME) {
    ...
    // composer, dungeonContext.js:5872), so exterior mode and a
  ...
  const hudCtx = {
     ...
     *  on. Here routeAction's arm is optional (ui/input.js:625) and
     ...
     *  POLLED_ACTIONS (ui/input.js:486), so a Z press reaches the
  ...
  addEventListener('keydown', (e) => {
    ...
    if (!townTalk.overlayActive && (modes?.mode ?? 'exterior') === 'exterior') {
      ...
      // `POLLED_ACTIONS.has(act)` decline (ui/input.js:588) never
  ...
  // worldModes answers it in BOTH modes (worldModes.js:7646-7709 -

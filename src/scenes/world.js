...
export async function bootWorld(canvas, renderer, params, status) {
  ...
  // U43: ui/input.js's routeAction contract for this host - the same
  ...
  addEventListener('keydown', (e) => {
    ...
    // U43 - THE ONE DOOR PER DESTINATION: this ladder and the large
    // HUD's eleven panels open the same windows, so they read the same
    // object. QuickLoad keeps its own arm below because it is the one
    // action that works with a window UP (the death screen's F11).
  ...
  // U43: Actions.ActivateCursor (Enter) - PlayerMouseLook.cursorActive,
  ...
  canvas.addEventListener('pointerdown', (e) => {
    ...
    // U43: the large HUD's panels, BEFORE the relock.

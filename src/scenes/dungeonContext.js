...
import { isStaleChunk, STALE_CHUNK_IN_PLAY_TEXT, STALE_CHUNK_IN_PLAY_SECONDS } from '../systems/staleChunk.js';   // DISC17-D: a chunk gone mid-session is said, not swallowed
...
export async function buildDungeonContext(deps, dfLocation, blocks, climateBaseType, opts = {}) {
  ...
  const foes = [];
  let foeDeps = null;
  let _staleChunkNotice = false;   // DISC17-D (AUDIT DISC17): the foe subsystem's chunk was gone - said on the first frame
  ...
  if (opts.foes && palette) {
   try {
    ...
    const [shared, engineRig, { buildRaceCharacter },
      ...
      // MT-iv: dynamic, as the rest of this block. (AUDIT DISC17: the
    ...
    enhancedNav.world = makeNavWorld();
     ...
     if (isStaleChunk(err)) _staleChunkNotice = true;   // said once the level is up - drawFoes below (AUDIT DISC17: set here, mid-build, it had timed out before the first frame)
     foeDeps = null;
   }
  }

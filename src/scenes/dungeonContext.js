...
// through the kill door. The exterior twin has carried this bound since WORLD6b (exteriorFoes.js:1969); the dungeon
...
export async function buildDungeonContext(deps, dfLocation, blocks, climateBaseType, opts = {}) {
  ...
  // host's own townTalk sink (world.js:9505 / exterior.js:3667), set
  ...
  async function ensureArrowModel(m) {
    ...
    // (dungeon.js:1080 against :1109; worldModes.js:7080 against :7104).   // QS6: both pairs' SECOND half was stale before this slice - they named neither `drawFoes` call, and a positional bump would have moved a wrong number by the right offset; re-resolved by content
        ...
        if (m.fromPlayer) {
          for (const f of foes) {
            ...
            if (missileHitsFoe(m.pos, f)) {   // ROAD-H tail: DaggerfallMissile.cs:339's SphereCast meets the foe's CAPSULE (REVIEW 2026-09-05 had its centre as a point)
              ...
              // playerArrowHitFoe is the one copy world.js:14923,
              // exterior.js:5234 and worldModes.js:7289 already ran;
  ...
  function applyHit(id, data) {
    ...
    const f = foes[i];
    // AUDIT FOES FOE5: BOUNDED, as the exterior twin's applyHit is (exteriorFoes.js:1969). The number is a peer's
  ...
  function applyWorld(w, { truncate = true, wire = false } = {}) {
    ...
    // construction (exteriorFoes.js:1544's restoreWorld goes through

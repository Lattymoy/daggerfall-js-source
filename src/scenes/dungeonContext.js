...
export async function buildDungeonContext(deps, dfLocation, blocks, climateBaseType, opts = {}) {
  ...
  // host's own townTalk sink (world.js:6825 / exterior.js:3079), set
   ...
   *  mountSpellWindow DUNGEON ARM (worldModes.js:1047,
        ...
        if (m.fromPlayer) {
          for (const f of foes) {
            ...
            if (missileHitsFoe(m.pos, f)) {   // ROAD-H tail: DaggerfallMissile.cs:339's SphereCast meets the foe's CAPSULE (REVIEW 2026-09-05 had its centre as a point)
              ...
              // playerArrowHitFoe is the one copy world.js:9381,
              // exterior.js:4459 and worldModes.js:6114 already ran;
  ...
  const api = {
    ...
    destroy() {
      ...
      // no `onSpawn` (worldModes.js:507), so it owns its batches and

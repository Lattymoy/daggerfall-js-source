...
export async function buildDungeonContext(deps, dfLocation, blocks, climateBaseType, opts = {}) {
  ...
  async function ensureArrowModel(m) {
    ...
    // (dungeon.js:1002 against :1039; worldModes.js:6139 against :6163).   // QS6: both pairs' SECOND half was stale before this slice - they named neither `drawFoes` call, and a positional bump would have moved a wrong number by the right offset; re-resolved by content
  ...
  const api = {
    ...
    destroy() {
      ...
      // no `onSpawn` (worldModes.js:575), so it owns its batches and

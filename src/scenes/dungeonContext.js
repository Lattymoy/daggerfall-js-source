...
// through the kill door. The exterior twin has carried this bound since WORLD6b (exteriorFoes.js:1677); the dungeon
...
export async function buildDungeonContext(deps, dfLocation, blocks, climateBaseType, opts = {}) {
  ...
  // host's own townTalk sink (world.js:7026 / exterior.js:3176), set
   ...
   *  mountSpellWindow DUNGEON ARM (worldModes.js:1048,
  ...
  async function ensureArrowModel(m) {
    ...
    // (dungeon.js:996 against :1032; worldModes.js:5986 against :6008).   // QS6: both pairs' SECOND half was stale before this slice - they named neither `drawFoes` call, and a positional bump would have moved a wrong number by the right offset; re-resolved by content
        ...
        if (m.fromPlayer) {
          for (const f of foes) {
            ...
            if (missileHitsFoe(m.pos, f)) {   // ROAD-H tail: DaggerfallMissile.cs:339's SphereCast meets the foe's CAPSULE (REVIEW 2026-09-05 had its centre as a point)
              ...
              // playerArrowHitFoe is the one copy world.js:9662,
              // exterior.js:4589 and worldModes.js:6140 already ran;
  ...
  function applyHit(id, data) {
    ...
    const f = foes[i];
    // AUDIT FOES FOE5: BOUNDED, as the exterior twin's applyHit is (exteriorFoes.js:1677). The number is a peer's
  ...
  const api = {
    ...
    togglePause(doorOpts = {}) {
      ...
      openPauseFlow((w) => { activeOverlay = w; }, {
        ...
        // pause door ui/input.js:619 reaches underground).
        relock: () => opts.relock?.(),

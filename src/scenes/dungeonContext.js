...
export async function buildDungeonContext(deps, dfLocation, blocks, climateBaseType, opts = {}) {
   ...
   *  mountSpellWindow DUNGEON ARM (worldModes.js:1107,
  ...
  async function ensureArrowModel(m) {
    ...
    // (dungeon.js:1002 against :1039; worldModes.js:6152 against :6163).   // QS6: both pairs' SECOND half was stale before this slice - they named neither `drawFoes` call, and a positional bump would have moved a wrong number by the right offset; re-resolved by content
        ...
        if (m.fromPlayer) {
          for (const f of foes) {
            ...
            if (missileHitsFoe(m.pos, f)) {   // ROAD-H tail: DaggerfallMissile.cs:339's SphereCast meets the foe's CAPSULE (REVIEW 2026-09-05 had its centre as a point)
              ...
              // exterior.js:4752 and worldModes.js:6306 already ran;
  ...
  const api = {
    ...
    destroy() {
      ...
      // no `onSpawn` (worldModes.js:576), so it owns its batches and

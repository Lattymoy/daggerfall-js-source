...
export async function buildDungeonContext(deps, dfLocation, blocks, climateBaseType, opts = {}) {
   ...
   *  mountSpellWindow DUNGEON ARM (worldModes.js:1110,
  ...
  async function ensureArrowModel(m) {
    ...
    // (dungeon.js:1002 against :1039; worldModes.js:6166 against :6177).   // QS6: both pairs' SECOND half was stale before this slice - they named neither `drawFoes` call, and a positional bump would have moved a wrong number by the right offset; re-resolved by content
        ...
        if (m.fromPlayer) {
          for (const f of foes) {
            ...
            if (missileHitsFoe(m.pos, f)) {   // ROAD-H tail: DaggerfallMissile.cs:339's SphereCast meets the foe's CAPSULE (REVIEW 2026-09-05 had its centre as a point)
              ...
              // exterior.js:4752 and worldModes.js:6320 already ran;
  ...
  const api = {
    ...
    destroy() {
      ...
      // no `onSpawn` (worldModes.js:578), so it owns its batches and

...
export async function buildDungeonContext(deps, dfLocation, blocks, climateBaseType, opts = {}) {
  ...
  // host's own townTalk sink (world.js:6778 / exterior.js:3043), set
  ...
  async function ensureArrowModel(m) {
    ...
    // (dungeon.js:978 against :981; worldModes.js:5974 against :5975).
        ...
        if (m.fromPlayer) {
          for (const f of foes) {
            ...
            if (missileHitsFoe(m.pos, f)) {   // ROAD-H tail: DaggerfallMissile.cs:339's SphereCast meets the foe's CAPSULE (REVIEW 2026-09-05 had its centre as a point)
              ...
              // playerArrowHitFoe is the one copy world.js:9281,
              // exterior.js:4416 and worldModes.js:6101 already ran;
  ...
  const api = {
    ...
    destroy() {
      ...
      // no `onSpawn` (worldModes.js:508), so it owns its batches and

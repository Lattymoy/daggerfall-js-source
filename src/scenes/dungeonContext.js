...
export async function buildDungeonContext(deps, dfLocation, blocks, climateBaseType, opts = {}) {
  ...
  if (opts.foes && palette) {
   try {
    ...
    const [shared, engineRig, { buildRaceCharacter },
      ...
      { runTargetMachine, isPlayerTarget, PLAYER_TARGET, resetAllyTeamOnPlayerAttack, targetAimPoint },
  ...
  async function ensureArrowModel(m) {
    ...
    // (dungeon.js:784 against :815; worldModes.js:5104 against :5113).
        ...
        if (m.fromPlayer) {
          for (const f of foes) {
            ...
            if (Math.hypot(fx, fy, fz) <= MISSILE_COLLIDER_RADIUS + 0.45) {
              ...
              // playerArrowHitFoe is the one copy world.js:6484,
              // exterior.js:3795 and worldModes.js:4627 already ran;

...
export async function bootExterior(canvas, renderer, params, status) {
  ...
  // pool the mode machine holds, which is world.js:2050's line.
  ...
  const cityGuards = createCityGuards({
    ...
    // own note describes. Written in world.js:2138's shape, so one pin
   ...
   *  the other two exterior hosts mount - world.js:2090 over the street
   ...
   *  it (world.js:2136-2220) and is its own slice; this pool does not
   *  wait on it. */
  ...
  const outdoorRestDeps = createRestDeps(playerEntity, {
    ...
    advanceMinutes: (n) => { playerTicker.advance(n); runEncounterTick(walkMode ? player.pos : cam.pos); },   // ROAD-G TAIL: the catch-up loop rides the rest's minutes, as world.js:3020 has it
   ...
   *  (effects.js:881/:895) - world.js:2278 hoisted its copy for exactly
  ...
  const questWorld = {
    ...
    // other. world.js:5503 is the same line.
     ...
     *  mode-aware (worldModes:1258), so this is world.js:5394's line
     ...
     *  Bridge-only, no host state - world.js:5402's call verbatim. */
     ...
     *  Everything else is world.js:5506's arm term for term: the cast
  ...
  questBridge = createQuestBridge({
    ...
    // closure S40 gave this host, and world.js:6159's line.
    ...
    // encounter half is world.js:6081's line and the watch half is this
  ...
  // here, so the sink is handed down at this moment - world.js:6443's
  // line for this host.
   ...
   *  second ctx here is the shape FS1 was written about. */
  {
     ...
     *  encounter mount above, and world.js:2632-2648 is the shape.
     ...
     *  world.js:2574-2575 does it. ROAD-G TAIL: a WATCHMAN transforms
  ...
  function frame(now) {
    ...
    // through the player for ever. world.js:7875-7949 is the shape.
    arrows.update(dt, {
    ...
    if (walkMode && !tpMode) {
      ...
      for (const ev of weaponRig.frame(dt, { paralyzed })) {
        ...
        if (!cityGuards.resolvePlayerHit(weaponRig.playerWeapon, eye, fwd, player.pos, makeInView(proj, view, multiply), guardHitSound)) {
          ...
          // BEFORE civilians - world.js:8018's order, and the order

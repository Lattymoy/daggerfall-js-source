...
export async function bootExterior(canvas, renderer, params, status) {
  ...
  //     continue;`, world.js:2226) and stands them in batches of their
  ...
  function onExhaustedExterior() {
    ...
    try {
      const out = exhaustionOutcome({
        ...
        enemiesNearby: areEnemiesNearby([...(cityGuards?.guards ?? []), ...(exteriorFoes?.foes ?? [])]),   // ROAD-G G2: BOTH street pools, world.js:3109's line
  ...
  // pool the mode machine holds, which is world.js:3707's line.
  ...
  const cityGuards = createCityGuards({
    ...
    // own note describes. Written in world.js:4042's shape, so one pin
   ...
   *  the other two exterior hosts mount - world.js:3768 over the street
   ...
   *  it (world.js:4040-4249) and is its own slice; this pool does not
   *  wait on it. */
  ...
  const outdoorRestDeps = createRestDeps(playerEntity, {
    ...
    advanceMinutes: (n, sharedEnd) => { playerTicker.advance(n); runEncounterTick(walkMode ? player.pos : cam.pos, sharedEnd, true); },   // CAMP1-REST: every tick this drives IS a rest   // ROAD-G TAIL: the catch-up loop rides the rest's minutes, as world.js:5444 has it   // RESTX2: sharedEnd is the session's local sim-minutes online, so the roll still gets a fresh `now` while the real clock stands
   ...
   *  (effects.js:885/:899) - world.js:4599 hoisted its copy for exactly
  ...
  const magic = createPlayerMagic({
    ...
    // two engine-owning hosts wire the identical pair (world.js:4547-4548,
  ...
  const questWorld = {
    ...
    // other. world.js:9142 is the same line.
     ...
     *  %rn/%rt's producer - world.js:9041-9054. */
    ...
    // QG1: CastSpellDo's two world reads, world.js:8957-8960's pair.
     ...
     *  mode-aware (worldModes:1258), so this is world.js:8930's line
     ...
     *  Bridge-only, no host state - world.js:8938's call verbatim. */
     ...
     *  Everything else is world.js:9042's arm term for term: the cast
  ...
  questBridge = createQuestBridge({
    ...
    // closure S40 gave this host, and world.js:9702's line.
    ...
    // encounter half is world.js:9794's line and the watch half is this
  ...
  // same order as world.js:10008/:10013, for the same reason: the
  ...
  // here, so the sink is handed down at this moment - world.js:9992's
  // line for this host.
  ...
  var modes = createWorldModes({
    ...
    buildingDirectory: () => ({
      ...
      // two inputs the world host uses (world.js:12928).
   ...
   *  encounter mount above, and world.js:4779-4795 is the shape.
   ...
   *  second ctx here is the shape FS1 was written about. */
  {
     ...
     *  world.js:4726-4727 does it. ROAD-G TAIL: a WATCHMAN transforms
  ...
  function frame(now) {
    ...
    // through the player for ever. world.js:14804-15052 is the shape.
    arrows.update(dt, {
    ...
    if (walkMode && !tpMode) {
      ...
      for (const ev of weaponRig.frame(dt, { paralyzed })) {
        ...
        if (!cityGuards.resolvePlayerHit(weaponRig.playerWeapon, eye, fwd, player.pos, makeInView(proj, view, multiply), guardHitSound, { swing })) {
          ...
          // BEFORE civilians - world.js:15167's order, and the order

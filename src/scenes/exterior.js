...
export async function bootExterior(canvas, renderer, params, status) {
   ...
   *  (effects.js:881/:895) - world.js:2801 hoisted its copy for exactly
  ...
  const magic = createPlayerMagic({
    ...
    // two engine-owning hosts wire the identical pair (world.js:2714-2715,
  ...
  const questWorld = {
    ...
    // other. world.js:6318 is the same line.
     ...
     *  %rn/%rt's producer - world.js:6217-6230. */
    ...
    // QG1: CastSpellDo's two world reads, world.js:6133-6136's pair.
     ...
     *  mode-aware (worldModes:1258), so this is world.js:6106's line
     ...
     *  Bridge-only, no host state - world.js:6114's call verbatim. */
     ...
     *  Everything else is world.js:6218's arm term for term: the cast
  ...
  questBridge = createQuestBridge({
    ...
    // closure S40 gave this host, and world.js:6871's line.
    ...
    // encounter half is world.js:6951's line and the watch half is this
  ...
  // same order as world.js:7172/:7177, for the same reason: the
  ...
  // here, so the sink is handed down at this moment - world.js:7156's
  // line for this host.
  ...
  var modes = createWorldModes({
    ...
    buildingDirectory: () => ({
      ...
      // two inputs the world host uses (world.js:7995).
   ...
   *  second ctx here is the shape FS1 was written about. */
  {
     ...
     *  encounter mount above, and world.js:2983-2999 is the shape.
     ...
     *  world.js:2922-2923 does it. ROAD-G TAIL: a WATCHMAN transforms
  ...
  function frame(now) {
    ...
    // through the player for ever. world.js:9401-9522 is the shape.
    arrows.update(dt, {
    ...
    if (walkMode && !tpMode) {
      ...
      for (const ev of weaponRig.frame(dt, { paralyzed })) {
        ...
        if (!cityGuards.resolvePlayerHit(weaponRig.playerWeapon, eye, fwd, player.pos, makeInView(proj, view, multiply), guardHitSound)) {
          ...
          // BEFORE civilians - world.js:9592's order, and the order

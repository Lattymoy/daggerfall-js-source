...
import { spellRecordOfIndex } from '../systems/loot.js';   // QG1: CastSpellDo's classic-record read (the G4 registry) - world.js:165's import
...
export async function bootExterior(canvas, renderer, params, status) {
  ...
  //     continue;`, world.js:1231) and stands them in batches of their
  ...
  function onExhaustedExterior() {
    ...
    try {
      const out = exhaustionOutcome({
        ...
        enemiesNearby: areEnemiesNearby([...(cityGuards?.guards ?? []), ...(exteriorFoes?.foes ?? [])]),   // ROAD-G G2: BOTH street pools, world.js:1935's line
  ...
  // pool the mode machine holds, which is world.js:2502's line.
  ...
  const cityGuards = createCityGuards({
    ...
    // own note describes. Written in world.js:2680's shape, so one pin
   ...
   *  the other two exterior hosts mount - world.js:2563 over the street
   ...
   *  it (world.js:2678-2760) and is its own slice; this pool does not
   *  wait on it. */
  ...
  const outdoorRestDeps = createRestDeps(playerEntity, {
    ...
    advanceMinutes: (n, sharedEnd) => { playerTicker.advance(n); runEncounterTick(walkMode ? player.pos : cam.pos, sharedEnd, true); },   // CAMP1-REST: every tick this drives IS a rest   // ROAD-G TAIL: the catch-up loop rides the rest's minutes, as world.js:3809 has it   // RESTX2: sharedEnd is the session's local sim-minutes online, so the roll still gets a fresh `now` while the real clock stands
   ...
   *  (effects.js:881/:895) - world.js:3073 hoisted its copy for exactly
  ...
  const magic = createPlayerMagic({
    ...
    // two engine-owning hosts wire the identical pair (world.js:2986-2987,
    // dungeonContext.js:2080-2081); `questBridge` is assigned below this
  ...
  const makeJournalWindow = (mode) => {
    ...
    // gate could never run. dungeonContext.js:1303-1308 is the shape:
  ...
  const questWorld = {
    ...
    // other. world.js:7051 is the same line.
     ...
     *  %rn/%rt's producer - world.js:6950-6963. */
    ...
    // QG1: CastSpellDo's two world reads, world.js:6866-6869's pair.
     ...
     *  mode-aware (worldModes:1258), so this is world.js:6839's line
     ...
     *  Bridge-only, no host state - world.js:6847's call verbatim. */
     ...
     *  Everything else is world.js:6951's arm term for term: the cast
  ...
  questBridge = createQuestBridge({
    ...
    // closure S40 gave this host, and world.js:7604's line.
    ...
    // encounter half is world.js:7684's line and the watch half is this
  ...
  // same order as world.js:7905/:7910, for the same reason: the
  ...
  // here, so the sink is handed down at this moment - world.js:7889's
  // line for this host.
  ...
  var modes = createWorldModes({
    ...
    buildingDirectory: () => ({
      ...
      // two inputs the world host uses (world.js:8771).
   ...
   *  second ctx here is the shape FS1 was written about. */
  {
     ...
     *  encounter mount above, and world.js:3255-3271 is the shape.
     ...
     *  world.js:3185-3186 does it. ROAD-G TAIL: a WATCHMAN transforms
  ...
  function frame(now) {
    ...
    // through the player for ever. world.js:10334-10497 is the shape.
    arrows.update(dt, {
    ...
    if (walkMode && !tpMode) {
      ...
      for (const ev of weaponRig.frame(dt, { paralyzed })) {
        ...
        if (!cityGuards.resolvePlayerHit(weaponRig.playerWeapon, eye, fwd, player.pos, makeInView(proj, view, multiply), guardHitSound)) {
          ...
          // BEFORE civilians - world.js:10592's order, and the order

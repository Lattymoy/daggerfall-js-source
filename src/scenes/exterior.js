...
import { spellRecordOfIndex } from '../systems/loot.js';   // QG1: CastSpellDo's classic-record read (the G4 registry) - world.js:223's import
...
export async function bootExterior(canvas, renderer, params, status) {
  ...
  //     continue;`, world.js:1819) and stands them in batches of their
  ...
  function onExhaustedExterior() {
    ...
    try {
      const out = exhaustionOutcome({
        ...
        enemiesNearby: areEnemiesNearby([...(cityGuards?.guards ?? []), ...(exteriorFoes?.foes ?? [])]),   // ROAD-G G2: BOTH street pools, world.js:2694's line
  ...
  // pool the mode machine holds, which is world.js:3289's line.
  ...
  const cityGuards = createCityGuards({
    ...
    // own note describes. Written in world.js:3624's shape, so one pin
   ...
   *  the other two exterior hosts mount - world.js:3350 over the street
   ...
   *  it (world.js:3622-3827) and is its own slice; this pool does not
   *  wait on it. */
  ...
  const outdoorRestDeps = createRestDeps(playerEntity, {
    ...
    advanceMinutes: (n, sharedEnd) => { playerTicker.advance(n); runEncounterTick(walkMode ? player.pos : cam.pos, sharedEnd, true); },   // CAMP1-REST: every tick this drives IS a rest   // ROAD-G TAIL: the catch-up loop rides the rest's minutes, as world.js:5022 has it   // RESTX2: sharedEnd is the session's local sim-minutes online, so the roll still gets a fresh `now` while the real clock stands
   ...
   *  (effects.js:885/:899) - world.js:4177 hoisted its copy for exactly
  ...
  const magic = createPlayerMagic({
    ...
    // two engine-owning hosts wire the identical pair (world.js:4125-4126,
    // dungeonContext.js:2252-2253); `questBridge` is assigned below this
  ...
  const makeJournalWindow = (mode) => {
    ...
    // gate could never run. dungeonContext.js:1456-1461 is the shape:
  ...
  addEventListener('keydown', (e) => {
    ...
    // "use for both") answers each of them, owner first, and the ladder - `exteriorKeyAction` below, word for word, each `return`
    ...
    function exteriorKeyAction(act, first) {
      ...
      if (!townTalk.overlayActive && quickLootArm(act)) { _tapArmed = 2; e.preventDefault(); return true; }
  ...
  const questWorld = {
    ...
    // other. world.js:8723 is the same line.
     ...
     *  %rn/%rt's producer - world.js:8622-8635. */
    ...
    // QG1: CastSpellDo's two world reads, world.js:8538-8541's pair.
     ...
     *  mode-aware (worldModes:1258), so this is world.js:8511's line
     ...
     *  Bridge-only, no host state - world.js:8841's call verbatim. */
     ...
     *  Everything else is world.js:8623's arm term for term: the cast
  ...
  questBridge = createQuestBridge({
    ...
    // closure S40 gave this host, and world.js:9283's line.
    ...
    // encounter half is world.js:9375's line and the watch half is this
  ...
  // same order as world.js:9589/:9594, for the same reason: the
  ...
  // here, so the sink is handed down at this moment - world.js:9573's
  // line for this host.
  ...
  var modes = createWorldModes({
    ...
    buildingDirectory: () => ({
      ...
      // two inputs the world host uses (world.js:12739).
   ...
   *  encounter mount above, and world.js:4357-4373 is the shape.
   ...
   *  second ctx here is the shape FS1 was written about. */
  {
     ...
     *  world.js:4304-4305 does it. ROAD-G TAIL: a WATCHMAN transforms
  ...
  function frame(now) {
    ...
    // through the player for ever. world.js:14553-14796 is the shape.
    arrows.update(dt, {
    ...
    if (walkMode && !tpMode) {
      ...
      for (const ev of weaponRig.frame(dt, { paralyzed })) {
        ...
        if (!cityGuards.resolvePlayerHit(weaponRig.playerWeapon, eye, fwd, player.pos, makeInView(proj, view, multiply), guardHitSound, { swing })) {
          ...
          // BEFORE civilians - world.js:14906's order, and the order

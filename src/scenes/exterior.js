...
export async function bootExterior(canvas, renderer, params, status) {
  ...
  //     continue;`, world.js:1039) and stands them in batches of their
  ...
  function onExhaustedExterior() {
    ...
    try {
      const out = exhaustionOutcome({
        ...
        enemiesNearby: areEnemiesNearby([...(cityGuards?.guards ?? []), ...(exteriorFoes?.foes ?? [])]),   // ROAD-G G2: BOTH street pools, world.js:1634's line
  ...
  // pool the mode machine holds, which is world.js:2089's line.
  ...
  const cityGuards = createCityGuards({
    ...
    // own note describes. Written in world.js:2177's shape, so one pin
   ...
   *  the other two exterior hosts mount - world.js:2129 over the street
   ...
   *  it (world.js:2175-2259) and is its own slice; this pool does not
   *  wait on it. */
  ...
  const outdoorRestDeps = createRestDeps(playerEntity, {
    ...
    advanceMinutes: (n) => { playerTicker.advance(n); runEncounterTick(walkMode ? player.pos : cam.pos); },   // ROAD-G TAIL: the catch-up loop rides the rest's minutes, as world.js:3060 has it
   ...
   *  (effects.js:881/:895) - world.js:2317 hoisted its copy for exactly
  ...
  const magic = createPlayerMagic({
    ...
    // two engine-owning hosts wire the identical pair (world.js:2425-2426,
    // dungeonContext.js:1952-1953); `questBridge` is assigned below this
  ...
  const makeJournalWindow = (mode) => {
    ...
    // gate could never run. dungeonContext.js:1261-1263 is the shape:
  ...
  const questWorld = {
    ...
    // other. world.js:5574 is the same line.
     ...
     *  %rn/%rt's producer - world.js:5576-5589. */
    ...
    // QG1: CastSpellDo's two world reads, world.js:5492-5495's pair.
     ...
     *  mode-aware (worldModes:1258), so this is world.js:5465's line
     ...
     *  Bridge-only, no host state - world.js:5473's call verbatim. */
     ...
     *  Everything else is world.js:5577's arm term for term: the cast
  ...
  questBridge = createQuestBridge({
    ...
    // closure S40 gave this host, and world.js:6230's line.
    ...
    // encounter half is world.js:6152's line and the watch half is this
  ...
  // same order as world.js:6530/:6532, for the same reason: the
  ...
  // here, so the sink is handed down at this moment - world.js:6514's
  // line for this host.
  ...
  var modes = createWorldModes({
    ...
    buildingDirectory: () => ({
      ...
      // two inputs the world host uses (world.js:6752).
   ...
   *  second ctx here is the shape FS1 was written about. */
  {
     ...
     *  encounter mount above, and world.js:2672-2688 is the shape.
     ...
     *  world.js:2614-2615 does it. ROAD-G TAIL: a WATCHMAN transforms
  ...
  function frame(now) {
    ...
    // through the player for ever. world.js:8051-8170 is the shape.
    arrows.update(dt, {
    ...
    if (walkMode && !tpMode) {
      ...
      for (const ev of weaponRig.frame(dt, { paralyzed })) {
        ...
        if (!cityGuards.resolvePlayerHit(weaponRig.playerWeapon, eye, fwd, player.pos, makeInView(proj, view, multiply), guardHitSound)) {
          ...
          // BEFORE civilians - world.js:8239's order, and the order

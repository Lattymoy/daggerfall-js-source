...
import { spellRecordOfIndex } from '../systems/loot.js';   // QG1: CastSpellDo's classic-record read (the G4 registry) - world.js:185's import
...
export async function bootExterior(canvas, renderer, params, status) {
  ...
  //     continue;`, world.js:1387) and stands them in batches of their
  ...
  function onExhaustedExterior() {
    ...
    try {
      const out = exhaustionOutcome({
        ...
        enemiesNearby: areEnemiesNearby([...(cityGuards?.guards ?? []), ...(exteriorFoes?.foes ?? [])]),   // ROAD-G G2: BOTH street pools, world.js:2102's line
  ...
  const cityGuards = createCityGuards({
    ...
    // own note describes. Written in world.js:3015's shape, so one pin
   ...
   *  it (world.js:3013-3095) and is its own slice; this pool does not
   *  wait on it. */
  ...
  const outdoorRestDeps = createRestDeps(playerEntity, {
    ...
    advanceMinutes: (n, sharedEnd) => { playerTicker.advance(n); runEncounterTick(walkMode ? player.pos : cam.pos, sharedEnd, true); },   // CAMP1-REST: every tick this drives IS a rest   // ROAD-G TAIL: the catch-up loop rides the rest's minutes, as world.js:4166 has it   // RESTX2: sharedEnd is the session's local sim-minutes online, so the roll still gets a fresh `now` while the real clock stands
   ...
   *  (effects.js:881/:895) - world.js:3426 hoisted its copy for exactly
  ...
  const magic = createPlayerMagic({
    ...
    // two engine-owning hosts wire the identical pair (world.js:3327-3328,
    // dungeonContext.js:2155-2156); `questBridge` is assigned below this
  ...
  const makeJournalWindow = (mode) => {
    ...
    // gate could never run. dungeonContext.js:1401-1406 is the shape:
  ...
  addEventListener('keydown', (e) => {
    ...
    // held-keys Set (ui/input.js:162-183), and no host passed one - so
    ...
    if (!townTalk.overlayActive && (modes?.mode ?? 'exterior') === 'exterior') {
      ...
      // own `POLLED_ACTIONS.has(act)` decline (ui/input.js:588) never
  ...
  const questWorld = {
    ...
    // other. world.js:7526 is the same line.
     ...
     *  %rn/%rt's producer - world.js:7425-7438. */
    ...
    // QG1: CastSpellDo's two world reads, world.js:7341-7344's pair.
     ...
     *  mode-aware (worldModes:1258), so this is world.js:7314's line
     ...
     *  Bridge-only, no host state - world.js:7322's call verbatim. */
     ...
     *  Everything else is world.js:7426's arm term for term: the cast
  ...
  questBridge = createQuestBridge({
    ...
    // closure S40 gave this host, and world.js:8080's line.
    ...
    // encounter half is world.js:8160's line and the watch half is this
  ...
  // same order as world.js:8381/:8386, for the same reason: the
  ...
  // here, so the sink is handed down at this moment - world.js:8365's
  // line for this host.
  ...
  var modes = createWorldModes({
    ...
    buildingDirectory: () => ({
      ...
      // two inputs the world host uses (world.js:9287).
   ...
   *  second ctx here is the shape FS1 was written about. */
  {
     ...
     *  encounter mount above, and world.js:3608-3624 is the shape.
     ...
     *  world.js:3538-3539 does it. ROAD-G TAIL: a WATCHMAN transforms
  ...
  function frame(now) {
    ...
    if (modes.frame(dt, now)) {
      ...
      // (townTalk.js:632, :640). So every HUD line raised in a modal
    ...
    // through the player for ever. world.js:10929-11129 is the shape.
    arrows.update(dt, {
    ...
    if (walkMode && !tpMode) {
      ...
      for (const ev of weaponRig.frame(dt, { paralyzed })) {
        ...
        if (!cityGuards.resolvePlayerHit(weaponRig.playerWeapon, eye, fwd, player.pos, makeInView(proj, view, multiply), guardHitSound)) {
          ...
          // BEFORE civilians - world.js:11236's order, and the order

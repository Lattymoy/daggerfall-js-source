...
export async function bootExterior(canvas, renderer, params, status) {
  ...
  const outdoorRestDeps = createRestDeps(playerEntity, {
    ...
    advanceMinutes: (n, sharedEnd) => { playerTicker.advance(n); runEncounterTick(walkMode ? player.pos : cam.pos, sharedEnd); },   // ROAD-G TAIL: the catch-up loop rides the rest's minutes, as world.js:3425 has it   // RESTX2: sharedEnd is the session's local sim-minutes online, so the roll still gets a fresh `now` while the real clock stands
   ...
   *  (effects.js:881/:895) - world.js:2730 hoisted its copy for exactly
  ...
  const magic = createPlayerMagic({
    ...
    // two engine-owning hosts wire the identical pair (world.js:2643-2644,
    // dungeonContext.js:2060-2061); `questBridge` is assigned below this
  ...
  const questWorld = {
    ...
    // other. world.js:6229 is the same line.
     ...
     *  %rn/%rt's producer - world.js:6128-6141. */
    ...
    // QG1: CastSpellDo's two world reads, world.js:6044-6047's pair.
     ...
     *  mode-aware (worldModes:1258), so this is world.js:6017's line
     ...
     *  Bridge-only, no host state - world.js:6025's call verbatim. */
     ...
     *  Everything else is world.js:6129's arm term for term: the cast
  ...
  questBridge = createQuestBridge({
    ...
    // closure S40 gave this host, and world.js:6782's line.
    ...
    // encounter half is world.js:6862's line and the watch half is this
  ...
  // same order as world.js:7083/:7088, for the same reason: the
  ...
  // here, so the sink is handed down at this moment - world.js:7067's
  // line for this host.
  ...
  var modes = createWorldModes({
    ...
    buildingDirectory: () => ({
      ...
      // two inputs the world host uses (world.js:7893).
   ...
   *  second ctx here is the shape FS1 was written about. */
  {
     ...
     *  encounter mount above, and world.js:2912-2928 is the shape.
     ...
     *  world.js:2851-2852 does it. ROAD-G TAIL: a WATCHMAN transforms
  ...
  function frame(now) {
    ...
    // through the player for ever. world.js:9300-9421 is the shape.
    arrows.update(dt, {
    ...
    if (walkMode && !tpMode) {
      ...
      for (const ev of weaponRig.frame(dt, { paralyzed })) {
        ...
        if (!cityGuards.resolvePlayerHit(weaponRig.playerWeapon, eye, fwd, player.pos, makeInView(proj, view, multiply), guardHitSound)) {
          ...
          // BEFORE civilians - world.js:9490's order, and the order

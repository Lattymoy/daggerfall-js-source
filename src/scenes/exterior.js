...
import { spellRecordOfIndex } from '../systems/loot.js';   // QG1: CastSpellDo's classic-record read (the G4 registry) - world.js:123's import
...
export async function bootExterior(canvas, renderer, params, status) {
  ...
  function onExhaustedExterior() {
    ...
    try {
      const out = exhaustionOutcome({
        ...
        enemiesNearby: areEnemiesNearby([...(cityGuards?.guards ?? []), ...(exteriorFoes?.foes ?? [])]),   // ROAD-G G2: BOTH street pools, world.js:1631's line
  ...
  // pool the mode machine holds, which is world.js:2086's line.
  ...
  const cityGuards = createCityGuards({
    ...
    // own note describes. Written in world.js:2174's shape, so one pin
   ...
   *  the other two exterior hosts mount - world.js:2126 over the street
   ...
   *  it (world.js:2172-2256) and is its own slice; this pool does not
   *  wait on it. */
  ...
  const outdoorRestDeps = createRestDeps(playerEntity, {
    ...
    advanceMinutes: (n) => { playerTicker.advance(n); runEncounterTick(walkMode ? player.pos : cam.pos); },   // ROAD-G TAIL: the catch-up loop rides the rest's minutes, as world.js:3057 has it
   ...
   *  (effects.js:881/:895) - world.js:2314 hoisted its copy for exactly
  ...
  const magic = createPlayerMagic({
    ...
    // two engine-owning hosts wire the identical pair (world.js:2422-2423,
  ...
  const questWorld = {
    ...
    // other. world.js:5549 is the same line.
     ...
     *  %rn/%rt's producer - world.js:5551-5564. */
    ...
    // QG1: CastSpellDo's two world reads, world.js:5467-5470's pair.
     ...
     *  mode-aware (worldModes:1258), so this is world.js:5440's line
     ...
     *  Bridge-only, no host state - world.js:5448's call verbatim. */
     ...
     *  Everything else is world.js:5552's arm term for term: the cast
  ...
  questBridge = createQuestBridge({
    ...
    // closure S40 gave this host, and world.js:6205's line.
    ...
    // encounter half is world.js:6127's line and the watch half is this
  ...
  // same order as world.js:6505/:6507, for the same reason: the
  ...
  // here, so the sink is handed down at this moment - world.js:6489's
  // line for this host.
  ...
  var modes = createWorldModes({
    ...
    buildingDirectory: () => ({
      ...
      // two inputs the world host uses (world.js:6603).
   ...
   *  second ctx here is the shape FS1 was written about. */
  {
     ...
     *  encounter mount above, and world.js:2669-2685 is the shape.
     ...
     *  world.js:2611-2612 does it. ROAD-G TAIL: a WATCHMAN transforms
  ...
  function frame(now) {
    ...
    // through the player for ever. world.js:7894-8013 is the shape.
    arrows.update(dt, {
    ...
    if (walkMode && !tpMode) {
      ...
      for (const ev of weaponRig.frame(dt, { paralyzed })) {
        ...
        if (!cityGuards.resolvePlayerHit(weaponRig.playerWeapon, eye, fwd, player.pos, makeInView(proj, view, multiply), guardHitSound)) {
          ...
          // BEFORE civilians - world.js:8082's order, and the order

...
export async function bootExterior(canvas, renderer, params, status) {
  ...
  //     continue;`, world.js:1102) and stands them in batches of their
  ...
  function onExhaustedExterior() {
    ...
    try {
      const out = exhaustionOutcome({
        ...
        enemiesNearby: areEnemiesNearby([...(cityGuards?.guards ?? []), ...(exteriorFoes?.foes ?? [])]),   // ROAD-G G2: BOTH street pools, world.js:1715's line
  ...
  // pool the mode machine holds, which is world.js:2214's line.
  ...
  const cityGuards = createCityGuards({
    ...
    // own note describes. Written in world.js:2313's shape, so one pin
   ...
   *  the other two exterior hosts mount - world.js:2254 over the street
   ...
   *  it (world.js:2311-2393) and is its own slice; this pool does not
   *  wait on it. */
  ...
  const outdoorRestDeps = createRestDeps(playerEntity, {
    ...
    advanceMinutes: (n) => { playerTicker.advance(n); runEncounterTick(walkMode ? player.pos : cam.pos); },   // ROAD-G TAIL: the catch-up loop rides the rest's minutes, as world.js:3258 has it
   ...
   *  (effects.js:881/:895) - world.js:2646 hoisted its copy for exactly
  ...
  const magic = createPlayerMagic({
    ...
    // two engine-owning hosts wire the identical pair (world.js:2559-2560,
    // dungeonContext.js:2028-2029); `questBridge` is assigned below this
  ...
  const questWorld = {
    ...
    // other. world.js:5979 is the same line.
     ...
     *  %rn/%rt's producer - world.js:5878-5891. */
    ...
    // QG1: CastSpellDo's two world reads, world.js:5794-5797's pair.
     ...
     *  mode-aware (worldModes:1258), so this is world.js:5767's line
     ...
     *  Bridge-only, no host state - world.js:5775's call verbatim. */
     ...
     *  Everything else is world.js:5879's arm term for term: the cast
  ...
  questBridge = createQuestBridge({
    ...
    // closure S40 gave this host, and world.js:6532's line.
    ...
    // encounter half is world.js:6612's line and the watch half is this
  ...
  // same order as world.js:6833/:6838, for the same reason: the
  ...
  // here, so the sink is handed down at this moment - world.js:6817's
  // line for this host.
  ...
  var modes = createWorldModes({
    ...
    buildingDirectory: () => ({
      ...
      // two inputs the world host uses (world.js:7612).
   ...
   *  second ctx here is the shape FS1 was written about. */
  {
     ...
     *  encounter mount above, and world.js:2828-2844 is the shape.
     ...
     *  world.js:2767-2768 does it. ROAD-G TAIL: a WATCHMAN transforms
  ...
  function frame(now) {
    ...
    // through the player for ever. world.js:8993-9113 is the shape.
    arrows.update(dt, {
    ...
    if (walkMode && !tpMode) {
      ...
      for (const ev of weaponRig.frame(dt, { paralyzed })) {
        ...
        if (!cityGuards.resolvePlayerHit(weaponRig.playerWeapon, eye, fwd, player.pos, makeInView(proj, view, multiply), guardHitSound)) {
          ...
          // BEFORE civilians - world.js:9182's order, and the order

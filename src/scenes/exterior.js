...
export async function bootExterior(canvas, renderer, params, status) {
  ...
  //     continue;`, world.js:1113) and stands them in batches of their
  ...
  function onExhaustedExterior() {
    ...
    try {
      const out = exhaustionOutcome({
        ...
        enemiesNearby: areEnemiesNearby([...(cityGuards?.guards ?? []), ...(exteriorFoes?.foes ?? [])]),   // ROAD-G G2: BOTH street pools, world.js:1726's line
  ...
  // pool the mode machine holds, which is world.js:2229's line.
  ...
  const cityGuards = createCityGuards({
    ...
    // own note describes. Written in world.js:2328's shape, so one pin
   ...
   *  the other two exterior hosts mount - world.js:2269 over the street
   ...
   *  it (world.js:2326-2408) and is its own slice; this pool does not
   *  wait on it. */
  ...
  const outdoorRestDeps = createRestDeps(playerEntity, {
    ...
    advanceMinutes: (n) => { playerTicker.advance(n); runEncounterTick(walkMode ? player.pos : cam.pos); },   // ROAD-G TAIL: the catch-up loop rides the rest's minutes, as world.js:3305 has it
   ...
   *  (effects.js:881/:895) - world.js:2661 hoisted its copy for exactly
  ...
  const magic = createPlayerMagic({
    ...
    // two engine-owning hosts wire the identical pair (world.js:2574-2575,
  ...
  const questWorld = {
    ...
    // other. world.js:6244 is the same line.
     ...
     *  %rn/%rt's producer - world.js:6143-6156. */
    ...
    // QG1: CastSpellDo's two world reads, world.js:6059-6062's pair.
     ...
     *  mode-aware (worldModes:1258), so this is world.js:6032's line
     ...
     *  Bridge-only, no host state - world.js:6040's call verbatim. */
     ...
     *  Everything else is world.js:6144's arm term for term: the cast
  ...
  questBridge = createQuestBridge({
    ...
    // closure S40 gave this host, and world.js:6797's line.
    ...
    // encounter half is world.js:6877's line and the watch half is this
  ...
  // same order as world.js:7098/:7103, for the same reason: the
  ...
  // here, so the sink is handed down at this moment - world.js:7082's
  // line for this host.
  ...
  var modes = createWorldModes({
    ...
    buildingDirectory: () => ({
      ...
      // two inputs the world host uses (world.js:7877).
   ...
   *  second ctx here is the shape FS1 was written about. */
  {
     ...
     *  encounter mount above, and world.js:2843-2859 is the shape.
     ...
     *  world.js:2782-2783 does it. ROAD-G TAIL: a WATCHMAN transforms

...
export async function bootExterior(canvas, renderer, params, status) {
  ...
  function onExhaustedExterior() {
    ...
    try {
      const out = exhaustionOutcome({
        ...
        enemiesNearby: areEnemiesNearby([...(cityGuards?.guards ?? []), ...(exteriorFoes?.foes ?? [])]),   // ROAD-G G2: BOTH street pools, world.js:1850's line
  ...
  // pool the mode machine holds, which is world.js:2415's line.
  ...
  const cityGuards = createCityGuards({
    ...
    // own note describes. Written in world.js:2584's shape, so one pin
   ...
   *  the other two exterior hosts mount - world.js:2467 over the street
   ...
   *  it (world.js:2582-2664) and is its own slice; this pool does not
   *  wait on it. */
   ...
   *  (effects.js:881/:895) - world.js:2975 hoisted its copy for exactly
  ...
  const magic = createPlayerMagic({
    ...
    // two engine-owning hosts wire the identical pair (world.js:2888-2889,
  ...
  const questWorld = {
    ...
    // other. world.js:6895 is the same line.
     ...
     *  %rn/%rt's producer - world.js:6794-6807. */
    ...
    // QG1: CastSpellDo's two world reads, world.js:6710-6713's pair.
     ...
     *  mode-aware (worldModes:1258), so this is world.js:6683's line
     ...
     *  Bridge-only, no host state - world.js:6691's call verbatim. */
     ...
     *  Everything else is world.js:6795's arm term for term: the cast
  ...
  questBridge = createQuestBridge({
    ...
    // closure S40 gave this host, and world.js:7448's line.
    ...
    // encounter half is world.js:7528's line and the watch half is this
  ...
  // same order as world.js:7749/:7754, for the same reason: the
  ...
  // here, so the sink is handed down at this moment - world.js:7733's
  // line for this host.
  ...
  var modes = createWorldModes({
    ...
    buildingDirectory: () => ({
      ...
      // two inputs the world host uses (world.js:8595).
   ...
   *  second ctx here is the shape FS1 was written about. */
  {
     ...
     *  encounter mount above, and world.js:3157-3173 is the shape.
     ...
     *  world.js:3096-3097 does it. ROAD-G TAIL: a WATCHMAN transforms
  ...
  function frame(now) {
    ...
    // through the player for ever. world.js:10142-10263 is the shape.
    arrows.update(dt, {
    ...
    if (walkMode && !tpMode) {
      ...
      for (const ev of weaponRig.frame(dt, { paralyzed })) {
        ...
        if (!cityGuards.resolvePlayerHit(weaponRig.playerWeapon, eye, fwd, player.pos, makeInView(proj, view, multiply), guardHitSound)) {
          ...
          // BEFORE civilians - world.js:10336's order, and the order

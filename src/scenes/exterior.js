...
export async function bootExterior(canvas, renderer, params, status) {
  ...
  //     continue;`, world.js:1901) and stands them in batches of their
  ...
  function onExhaustedExterior() {
    ...
    try {
      const out = exhaustionOutcome({
        ...
        enemiesNearby: areEnemiesNearby([...(cityGuards?.guards ?? []), ...(exteriorFoes?.foes ?? [])]),   // ROAD-G G2: BOTH street pools, world.js:2842's line
  ...
  // pool the mode machine holds, which is world.js:3448's line.
  ...
  const cityGuards = createCityGuards({
    ...
    // own note describes. Written in world.js:3783's shape, so one pin
   ...
   *  the other two exterior hosts mount - world.js:3509 over the street
   ...
   *  it (world.js:3781-3985) and is its own slice; this pool does not
   *  wait on it. */
  ...
  const outdoorRestDeps = createRestDeps(playerEntity, {
    ...
    advanceMinutes: (n, sharedEnd) => { playerTicker.advance(n); runEncounterTick(walkMode ? player.pos : cam.pos, sharedEnd, true); },   // CAMP1-REST: every tick this drives IS a rest   // ROAD-G TAIL: the catch-up loop rides the rest's minutes, as world.js:5197 has it   // RESTX2: sharedEnd is the session's local sim-minutes online, so the roll still gets a fresh `now` while the real clock stands
   ...
   *  (effects.js:891/:905) - world.js:4345 hoisted its copy for exactly
  ...
  const magic = createPlayerMagic({
    ...
    // two engine-owning hosts wire the identical pair (world.js:4293-4294,
    // dungeonContext.js:2282-2283); `questBridge` is assigned below this
  ...
  const makeJournalWindow = (mode) => {
    ...
    // gate could never run. dungeonContext.js:1486-1491 is the shape:
  ...
  const questWorld = {
    ...
    // other. world.js:8951 is the same line.
     ...
     *  %rn/%rt's producer - world.js:8847-8860. */
    ...
    // QG1: CastSpellDo's two world reads, world.js:8763-8766's pair.
     ...
     *  mode-aware (worldModes:1258), so this is world.js:8736's line
     ...
     *  Bridge-only, no host state - world.js:8744's call verbatim. */
     ...
     *  Everything else is world.js:8848's arm term for term: the cast
  ...
  questBridge = createQuestBridge({
    ...
    // closure S40 gave this host, and world.js:9511's line.
    ...
    // encounter half is world.js:9607's line and the watch half is this
  ...
  // same order as world.js:9821/:9826, for the same reason: the
  ...
  // here, so the sink is handed down at this moment - world.js:9805's
  // line for this host.
  ...
  var modes = createWorldModes({
    ...
    buildingDirectory: () => ({
      ...
      // two inputs the world host uses (world.js:12940).
   ...
   *  encounter mount above, and world.js:4525-4541 is the shape.
   ...
   *  second ctx here is the shape FS1 was written about. */
  {
     ...
     *  world.js:4472-4473 does it. ROAD-G TAIL: a WATCHMAN transforms
  ...
  function frame(now) {
    ...
    // through the player for ever. world.js:14777-15017 is the shape.
    arrows.update(dt, {
    ...
    if (walkMode && !tpMode) {
      ...
      for (const ev of weaponRig.frame(dt, { paralyzed })) {
        ...
        if (!cityGuards.resolvePlayerHit(weaponRig.playerWeapon, eye, fwd, player.pos, makeInView(proj, view, multiply), guardHitSound, { swing })) {
          ...
          // BEFORE civilians - world.js:15131's order, and the order

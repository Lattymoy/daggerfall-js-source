...
export async function bootExterior(canvas, renderer, params, status) {
  ...
  //     continue;`, world.js:1128) and stands them in batches of their
  ...
  function onExhaustedExterior() {
    ...
    try {
      const out = exhaustionOutcome({
        ...
        enemiesNearby: areEnemiesNearby([...(cityGuards?.guards ?? []), ...(exteriorFoes?.foes ?? [])]),   // ROAD-G G2: BOTH street pools, world.js:1780's line
  ...
  // pool the mode machine holds, which is world.js:2268's line.
  ...
  const cityGuards = createCityGuards({
    ...
    // own note describes. Written in world.js:2437's shape, so one pin
   ...
   *  the other two exterior hosts mount - world.js:2320 over the street
   ...
   *  it (world.js:2435-2517) and is its own slice; this pool does not
   *  wait on it. */
  ...
  const outdoorRestDeps = createRestDeps(playerEntity, {
    ...
    advanceMinutes: (n, sharedEnd) => { playerTicker.advance(n); runEncounterTick(walkMode ? player.pos : cam.pos, sharedEnd, true); },   // CAMP1-REST: every tick this drives IS a rest   // ROAD-G TAIL: the catch-up loop rides the rest's minutes, as world.js:3553 has it   // RESTX2: sharedEnd is the session's local sim-minutes online, so the roll still gets a fresh `now` while the real clock stands
   ...
   *  (effects.js:881/:895) - world.js:2828 hoisted its copy for exactly
  ...
  const magic = createPlayerMagic({
    ...
    // two engine-owning hosts wire the identical pair (world.js:2741-2742,
  ...
  const questWorld = {
    ...
    // other. world.js:6661 is the same line.
     ...
     *  %rn/%rt's producer - world.js:6560-6573. */
    ...
    // QG1: CastSpellDo's two world reads, world.js:6476-6479's pair.
     ...
     *  mode-aware (worldModes:1258), so this is world.js:6449's line
     ...
     *  Bridge-only, no host state - world.js:6457's call verbatim. */
     ...
     *  Everything else is world.js:6561's arm term for term: the cast
  ...
  questBridge = createQuestBridge({
    ...
    // closure S40 gave this host, and world.js:7214's line.
    ...
    // encounter half is world.js:7294's line and the watch half is this
  ...
  // same order as world.js:7515/:7520, for the same reason: the
  ...
  // here, so the sink is handed down at this moment - world.js:7499's
  // line for this host.
  ...
  var modes = createWorldModes({
    ...
    buildingDirectory: () => ({
      ...
      // two inputs the world host uses (world.js:8338).
   ...
   *  second ctx here is the shape FS1 was written about. */
  {
     ...
     *  encounter mount above, and world.js:3010-3026 is the shape.
     ...
     *  world.js:2949-2950 does it. ROAD-G TAIL: a WATCHMAN transforms
  ...
  function frame(now) {
    ...
    // through the player for ever. world.js:9821-9942 is the shape.
    arrows.update(dt, {
    ...
    if (walkMode && !tpMode) {
      ...
      for (const ev of weaponRig.frame(dt, { paralyzed })) {
        ...
        if (!cityGuards.resolvePlayerHit(weaponRig.playerWeapon, eye, fwd, player.pos, makeInView(proj, view, multiply), guardHitSound)) {
          ...
          // BEFORE civilians - world.js:10015's order, and the order

...
export async function bootExterior(canvas, renderer, params, status) {
   ...
   *  it (world.js:2995-3198) and is its own slice; this pool does not
   *  wait on it. */
   ...
   *  (effects.js:887/:901) - world.js:3529 hoisted its copy for exactly
  ...
  const questWorld = {
    ...
    // other. world.js:7874 is the same line.
     ...
     *  %rn/%rt's producer - world.js:7773-7786. */
    ...
    // QG1: CastSpellDo's two world reads, world.js:7689-7692's pair.
     ...
     *  mode-aware (worldModes:1258), so this is world.js:7662's line
     ...
     *  Bridge-only, no host state - world.js:7670's call verbatim. */
     ...
     *  Everything else is world.js:7774's arm term for term: the cast
  ...
  questBridge = createQuestBridge({
    ...
    // closure S40 gave this host, and world.js:8433's line.
    ...
    // encounter half is world.js:8524's line and the watch half is this
  ...
  // same order as world.js:8739/:8744, for the same reason: the
  ...
  // here, so the sink is handed down at this moment - world.js:8723's
  // line for this host.
  ...
  var modes = createWorldModes({
    ...
    buildingDirectory: () => ({
      ...
      // two inputs the world host uses (world.js:10896).
   ...
   *  second ctx here is the shape FS1 was written about. */
  {
     ...
     *  encounter mount above, and world.js:3719-3735 is the shape.
     ...
     *  world.js:3649-3650 does it. ROAD-G TAIL: a WATCHMAN transforms
  ...
  function frame(now) {
    ...
    // through the player for ever. world.js:12610-12839 is the shape.
    arrows.update(dt, {
    ...
    if (walkMode && !tpMode) {
      ...
      for (const ev of weaponRig.frame(dt, { paralyzed })) {
        ...
        if (!cityGuards.resolvePlayerHit(weaponRig.playerWeapon, eye, fwd, player.pos, makeInView(proj, view, multiply), guardHitSound)) {
          ...
          // BEFORE civilians - world.js:12949's order, and the order

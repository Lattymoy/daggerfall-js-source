...
export async function bootExterior(canvas, renderer, params, status) {
  ...
  //     continue;`, world.js:1056) and stands them in batches of their
  ...
  function onExhaustedExterior() {
    ...
    try {
      const out = exhaustionOutcome({
        ...
        enemiesNearby: areEnemiesNearby([...(cityGuards?.guards ?? []), ...(exteriorFoes?.foes ?? [])]),   // ROAD-G G2: BOTH street pools, world.js:1651's line
  ...
  // pool the mode machine holds, which is world.js:2106's line.
  ...
  const cityGuards = createCityGuards({
    ...
    // own note describes. Written in world.js:2194's shape, so one pin
   ...
   *  the other two exterior hosts mount - world.js:2146 over the street
   ...
   *  it (world.js:2192-2276) and is its own slice; this pool does not
   *  wait on it. */
  ...
  const outdoorRestDeps = createRestDeps(playerEntity, {
    ...
    advanceMinutes: (n) => { playerTicker.advance(n); runEncounterTick(walkMode ? player.pos : cam.pos); },   // ROAD-G TAIL: the catch-up loop rides the rest's minutes, as world.js:3077 has it
   ...
   *  (effects.js:881/:895) - world.js:2334 hoisted its copy for exactly
  ...
  const magic = createPlayerMagic({
    ...
    // two engine-owning hosts wire the identical pair (world.js:2442-2443,
  ...
  const questWorld = {
    ...
    // other. world.js:5592 is the same line.
     ...
     *  %rn/%rt's producer - world.js:5594-5607. */
    ...
    // QG1: CastSpellDo's two world reads, world.js:5510-5513's pair.
     ...
     *  mode-aware (worldModes:1258), so this is world.js:5483's line
     ...
     *  Bridge-only, no host state - world.js:5491's call verbatim. */
     ...
     *  Everything else is world.js:5595's arm term for term: the cast
  ...
  questBridge = createQuestBridge({
    ...
    // closure S40 gave this host, and world.js:6248's line.
    ...
    // encounter half is world.js:6170's line and the watch half is this
  ...
  // same order as world.js:6549/:6551, for the same reason: the
  ...
  // here, so the sink is handed down at this moment - world.js:6533's
  // line for this host.
  ...
  var modes = createWorldModes({
    ...
    buildingDirectory: () => ({
      ...
      // two inputs the world host uses (world.js:6900).
   ...
   *  second ctx here is the shape FS1 was written about. */
  {
     ...
     *  encounter mount above, and world.js:2689-2705 is the shape.
  ...
  function frame(now) {
    ...
    // through the player for ever. world.js:8199-8318 is the shape.
    arrows.update(dt, {
    ...
    if (walkMode && !tpMode) {
      ...
      for (const ev of weaponRig.frame(dt, { paralyzed })) {
        ...
        if (!cityGuards.resolvePlayerHit(weaponRig.playerWeapon, eye, fwd, player.pos, makeInView(proj, view, multiply), guardHitSound)) {
          ...
          // BEFORE civilians - world.js:8387's order, and the order

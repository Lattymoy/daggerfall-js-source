...
export async function bootExterior(canvas, renderer, params, status) {
  ...
  // pool the mode machine holds, which is world.js:2425's line.
  ...
  const cityGuards = createCityGuards({
    ...
    // own note describes. Written in world.js:2594's shape, so one pin
   ...
   *  the other two exterior hosts mount - world.js:2477 over the street
   ...
   *  it (world.js:2592-2674) and is its own slice; this pool does not
   *  wait on it. */
   ...
   *  (effects.js:881/:895) - world.js:2985 hoisted its copy for exactly
  ...
  const magic = createPlayerMagic({
    ...
    // two engine-owning hosts wire the identical pair (world.js:2898-2899,
  ...
  const questWorld = {
    ...
    // other. world.js:6896 is the same line.
     ...
     *  %rn/%rt's producer - world.js:6795-6808. */
    ...
    // QG1: CastSpellDo's two world reads, world.js:6711-6714's pair.
     ...
     *  mode-aware (worldModes:1258), so this is world.js:6684's line
     ...
     *  Bridge-only, no host state - world.js:6692's call verbatim. */
     ...
     *  Everything else is world.js:6796's arm term for term: the cast
  ...
  questBridge = createQuestBridge({
    ...
    // closure S40 gave this host, and world.js:7449's line.
    ...
    // encounter half is world.js:7529's line and the watch half is this
  ...
  // same order as world.js:7750/:7755, for the same reason: the
  ...
  // here, so the sink is handed down at this moment - world.js:7734's
  // line for this host.
  ...
  var modes = createWorldModes({
    ...
    buildingDirectory: () => ({
      ...
      // two inputs the world host uses (world.js:8596).
   ...
   *  second ctx here is the shape FS1 was written about. */
  {
     ...
     *  encounter mount above, and world.js:3167-3183 is the shape.
     ...
     *  world.js:3106-3107 does it. ROAD-G TAIL: a WATCHMAN transforms
  ...
  function frame(now) {
    ...
    // through the player for ever. world.js:10143-10264 is the shape.
    arrows.update(dt, {
    ...
    if (walkMode && !tpMode) {
      ...
      for (const ev of weaponRig.frame(dt, { paralyzed })) {
        ...
        if (!cityGuards.resolvePlayerHit(weaponRig.playerWeapon, eye, fwd, player.pos, makeInView(proj, view, multiply), guardHitSound)) {
          ...
          // BEFORE civilians - world.js:10337's order, and the order

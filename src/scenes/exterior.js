...
import { spellRecordOfIndex } from '../systems/loot.js';   // QG1: CastSpellDo's classic-record read (the G4 registry) - world.js:145's import
...
export async function bootExterior(canvas, renderer, params, status) {
  ...
  //     continue;`, world.js:1115) and stands them in batches of their
  ...
  function onExhaustedExterior() {
    ...
    try {
      const out = exhaustionOutcome({
        ...
        enemiesNearby: areEnemiesNearby([...(cityGuards?.guards ?? []), ...(exteriorFoes?.foes ?? [])]),   // ROAD-G G2: BOTH street pools, world.js:1733's line
  ...
  // pool the mode machine holds, which is world.js:2240's line.
  ...
  const cityGuards = createCityGuards({
    ...
    // own note describes. Written in world.js:2397's shape, so one pin
   ...
   *  the other two exterior hosts mount - world.js:2280 over the street
   ...
   *  it (world.js:2395-2477) and is its own slice; this pool does not
   *  wait on it. */
  ...
  const outdoorRestDeps = createRestDeps(playerEntity, {
    ...
    advanceMinutes: (n, sharedEnd) => { playerTicker.advance(n); runEncounterTick(walkMode ? player.pos : cam.pos, sharedEnd); },   // ROAD-G TAIL: the catch-up loop rides the rest's minutes, as world.js:3454 has it   // RESTX2: sharedEnd is the session's local sim-minutes online, so the roll still gets a fresh `now` while the real clock stands
   ...
   *  (effects.js:881/:895) - world.js:2759 hoisted its copy for exactly
  ...
  const magic = createPlayerMagic({
    ...
    // two engine-owning hosts wire the identical pair (world.js:2672-2673,
    // dungeonContext.js:2063-2064); `questBridge` is assigned below this
  ...
  const makeJournalWindow = (mode) => {
    ...
    // gate could never run. dungeonContext.js:1298-1300 is the shape:
  ...
  const questWorld = {
    ...
    // other. world.js:6260 is the same line.
     ...
     *  %rn/%rt's producer - world.js:6159-6172. */
    ...
    // QG1: CastSpellDo's two world reads, world.js:6075-6078's pair.
     ...
     *  mode-aware (worldModes:1258), so this is world.js:6048's line
     ...
     *  Bridge-only, no host state - world.js:6056's call verbatim. */
     ...
     *  Everything else is world.js:6160's arm term for term: the cast
  ...
  questBridge = createQuestBridge({
    ...
    // closure S40 gave this host, and world.js:6813's line.
    ...
    // encounter half is world.js:6893's line and the watch half is this
  ...
  // same order as world.js:7114/:7119, for the same reason: the
  ...
  // here, so the sink is handed down at this moment - world.js:7098's
  // line for this host.
  ...
  var modes = createWorldModes({
    ...
    buildingDirectory: () => ({
      ...
      // two inputs the world host uses (world.js:7925).
   ...
   *  second ctx here is the shape FS1 was written about. */
  {
     ...
     *  encounter mount above, and world.js:2941-2957 is the shape.
     ...
     *  world.js:2880-2881 does it. ROAD-G TAIL: a WATCHMAN transforms
  ...
  function frame(now) {
    ...
    // through the player for ever. world.js:9332-9453 is the shape.
    arrows.update(dt, {
    ...
    if (walkMode && !tpMode) {
      ...
      for (const ev of weaponRig.frame(dt, { paralyzed })) {
        ...
        if (!cityGuards.resolvePlayerHit(weaponRig.playerWeapon, eye, fwd, player.pos, makeInView(proj, view, multiply), guardHitSound)) {
          ...
          // BEFORE civilians - world.js:9522's order, and the order

...
export async function bootExterior(canvas, renderer, params, status) {
  ...
  //     continue;`, world.js:1807) and stands them in batches of their
  ...
  // pool the mode machine holds, which is world.js:3277's line.
  ...
  const cityGuards = createCityGuards({
    ...
    // own note describes. Written in world.js:3612's shape, so one pin
   ...
   *  the other two exterior hosts mount - world.js:3338 over the street
   ...
   *  it (world.js:3610-3815) and is its own slice; this pool does not
   *  wait on it. */
  ...
  const magic = createPlayerMagic({
    ...
    // dungeonContext.js:2251-2252); `questBridge` is assigned below this
  ...
  const makeJournalWindow = (mode) => {
    ...
    // gate could never run. dungeonContext.js:1455-1460 is the shape:
  ...
  const questWorld = {
    ...
    // other. world.js:8664 is the same line.
     ...
     *  %rn/%rt's producer - world.js:8563-8576. */
    ...
    // QG1: CastSpellDo's two world reads, world.js:8479-8482's pair.
     ...
     *  mode-aware (worldModes:1258), so this is world.js:8452's line
     ...
     *  Bridge-only, no host state - world.js:8460's call verbatim. */
     ...
     *  Everything else is world.js:8564's arm term for term: the cast
  ...
  questBridge = createQuestBridge({
    ...
    // closure S40 gave this host, and world.js:9224's line.
    ...
    // encounter half is world.js:9320's line and the watch half is this
  ...
  var modes = createWorldModes({
    ...
    buildingDirectory: () => ({
      ...
      // two inputs the world host uses (world.js:12502).
  ...
  function frame(now) {
    ...
    // through the player for ever. world.js:14298-14538 is the shape.
    arrows.update(dt, {
    ...
    if (walkMode && !tpMode) {
      ...
      for (const ev of weaponRig.frame(dt, { paralyzed })) {
        ...
        if (!cityGuards.resolvePlayerHit(weaponRig.playerWeapon, eye, fwd, player.pos, makeInView(proj, view, multiply), guardHitSound, { swing })) {
          ...
          // BEFORE civilians - world.js:14648's order, and the order

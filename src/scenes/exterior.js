...
export async function bootExterior(canvas, renderer, params, status) {
  ...
  const outdoorRestDeps = createRestDeps(playerEntity, {
    ...
    advanceMinutes: (n, sharedEnd) => { playerTicker.advance(n); runEncounterTick(walkMode ? player.pos : cam.pos, sharedEnd, true); },   // CAMP1-REST: every tick this drives IS a rest   // ROAD-G TAIL: the catch-up loop rides the rest's minutes, as world.js:4345 has it   // RESTX2: sharedEnd is the session's local sim-minutes online, so the roll still gets a fresh `now` while the real clock stands
  ...
  const questWorld = {
    ...
    // other. world.js:7866 is the same line.
     ...
     *  %rn/%rt's producer - world.js:7765-7778. */
    ...
    // QG1: CastSpellDo's two world reads, world.js:7681-7684's pair.
     ...
     *  mode-aware (worldModes:1258), so this is world.js:7654's line
     ...
     *  Bridge-only, no host state - world.js:7662's call verbatim. */
     ...
     *  Everything else is world.js:7766's arm term for term: the cast
  ...
  questBridge = createQuestBridge({
    ...
    // closure S40 gave this host, and world.js:8425's line.
    ...
    // encounter half is world.js:8516's line and the watch half is this
  ...
  // same order as world.js:8731/:8736, for the same reason: the
  ...
  // here, so the sink is handed down at this moment - world.js:8715's
  // line for this host.
  ...
  var modes = createWorldModes({
    ...
    buildingDirectory: () => ({
      ...
      // two inputs the world host uses (world.js:10892).
  ...
  function frame(now) {
    ...
    // through the player for ever. world.js:12598-12827 is the shape.
    arrows.update(dt, {
    ...
    if (walkMode && !tpMode) {
      ...
      for (const ev of weaponRig.frame(dt, { paralyzed })) {
        ...
        if (!cityGuards.resolvePlayerHit(weaponRig.playerWeapon, eye, fwd, player.pos, makeInView(proj, view, multiply), guardHitSound)) {
          ...
          // BEFORE civilians - world.js:12937's order, and the order

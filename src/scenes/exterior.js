...
export async function bootExterior(canvas, renderer, params, status) {
  ...
  const outdoorRestDeps = createRestDeps(playerEntity, {
    ...
    advanceMinutes: (n, sharedEnd) => { playerTicker.advance(n); runEncounterTick(walkMode ? player.pos : cam.pos, sharedEnd, true); },   // CAMP1-REST: every tick this drives IS a rest   // ROAD-G TAIL: the catch-up loop rides the rest's minutes, as world.js:4232 has it   // RESTX2: sharedEnd is the session's local sim-minutes online, so the roll still gets a fresh `now` while the real clock stands
  ...
  const magic = createPlayerMagic({
    ...
    // two engine-owning hosts wire the identical pair (world.js:3359-3360,
  ...
  const questWorld = {
    ...
    // other. world.js:7737 is the same line.
     ...
     *  %rn/%rt's producer - world.js:7636-7649. */
    ...
    // QG1: CastSpellDo's two world reads, world.js:7552-7555's pair.
     ...
     *  mode-aware (worldModes:1258), so this is world.js:7525's line
     ...
     *  Bridge-only, no host state - world.js:7533's call verbatim. */
     ...
     *  Everything else is world.js:7637's arm term for term: the cast
  ...
  questBridge = createQuestBridge({
    ...
    // closure S40 gave this host, and world.js:8296's line.
    ...
    // encounter half is world.js:8387's line and the watch half is this
  ...
  // same order as world.js:8602/:8607, for the same reason: the
  ...
  // here, so the sink is handed down at this moment - world.js:8586's
  // line for this host.
  ...
  var modes = createWorldModes({
    ...
    buildingDirectory: () => ({
      ...
      // two inputs the world host uses (world.js:10672).
   ...
   *  second ctx here is the shape FS1 was written about. */
  {
     ...
     *  encounter mount above, and world.js:3601-3617 is the shape.
     ...
     *  world.js:3531-3532 does it. ROAD-G TAIL: a WATCHMAN transforms
  ...
  function frame(now) {
    ...
    // through the player for ever. world.js:12365-12591 is the shape.
    arrows.update(dt, {
    ...
    if (walkMode && !tpMode) {
      ...
      for (const ev of weaponRig.frame(dt, { paralyzed })) {
        ...
        if (!cityGuards.resolvePlayerHit(weaponRig.playerWeapon, eye, fwd, player.pos, makeInView(proj, view, multiply), guardHitSound)) {
          ...
          // BEFORE civilians - world.js:12701's order, and the order

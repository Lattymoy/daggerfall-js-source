...
export async function bootExterior(canvas, renderer, params, status) {
  ...
  const magic = createPlayerMagic({
    ...
    // dungeonContext.js:2250-2251); `questBridge` is assigned below this
  ...
  const makeJournalWindow = (mode) => {
    ...
    // gate could never run. dungeonContext.js:1454-1459 is the shape:
  ...
  const questWorld = {
    ...
    // other. world.js:8652 is the same line.
     ...
     *  %rn/%rt's producer - world.js:8551-8564. */
    ...
    // QG1: CastSpellDo's two world reads, world.js:8467-8470's pair.
     ...
     *  mode-aware (worldModes:1258), so this is world.js:8440's line
     ...
     *  Bridge-only, no host state - world.js:8448's call verbatim. */
     ...
     *  Everything else is world.js:8552's arm term for term: the cast
  ...
  questBridge = createQuestBridge({
    ...
    // closure S40 gave this host, and world.js:9212's line.
    ...
    // encounter half is world.js:9308's line and the watch half is this
  ...
  // same order as world.js:9522/:9527, for the same reason: the
  ...
  // here, so the sink is handed down at this moment - world.js:9506's
  // line for this host.
  ...
  var modes = createWorldModes({
    ...
    buildingDirectory: () => ({
      ...
      // two inputs the world host uses (world.js:12474).
  ...
  function frame(now) {
    ...
    // through the player for ever. world.js:14270-14510 is the shape.
    arrows.update(dt, {
    ...
    if (walkMode && !tpMode) {
      ...
      for (const ev of weaponRig.frame(dt, { paralyzed })) {
        ...
        if (!cityGuards.resolvePlayerHit(weaponRig.playerWeapon, eye, fwd, player.pos, makeInView(proj, view, multiply), guardHitSound, { swing })) {
          ...
          // BEFORE civilians - world.js:14620's order, and the order

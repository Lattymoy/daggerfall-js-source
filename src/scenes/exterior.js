...
export async function bootExterior(canvas, renderer, params, status) {
   ...
   *  it (world.js:2193-2277) and is its own slice; this pool does not
   *  wait on it. */
  ...
  const outdoorRestDeps = createRestDeps(playerEntity, {
    ...
    advanceMinutes: (n) => { playerTicker.advance(n); runEncounterTick(walkMode ? player.pos : cam.pos); },   // ROAD-G TAIL: the catch-up loop rides the rest's minutes, as world.js:3078 has it
   ...
   *  (effects.js:881/:895) - world.js:2335 hoisted its copy for exactly
  ...
  const magic = createPlayerMagic({
    ...
    // two engine-owning hosts wire the identical pair (world.js:2443-2444,
  ...
  const questWorld = {
    ...
    // other. world.js:5597 is the same line.
     ...
     *  %rn/%rt's producer - world.js:5599-5612. */
    ...
    // QG1: CastSpellDo's two world reads, world.js:5515-5518's pair.
     ...
     *  mode-aware (worldModes:1258), so this is world.js:5488's line
     ...
     *  Bridge-only, no host state - world.js:5496's call verbatim. */
     ...
     *  Everything else is world.js:5600's arm term for term: the cast
  ...
  questBridge = createQuestBridge({
    ...
    // closure S40 gave this host, and world.js:6253's line.
    ...
    // encounter half is world.js:6175's line and the watch half is this
  ...
  // same order as world.js:6554/:6555, for the same reason: the
  ...
  // here, so the sink is handed down at this moment - world.js:6538's
  // line for this host.
  ...
  var modes = createWorldModes({
    ...
    buildingDirectory: () => ({
      ...
      // two inputs the world host uses (world.js:6936).
   ...
   *  second ctx here is the shape FS1 was written about. */
  {
     ...
     *  encounter mount above, and world.js:2690-2706 is the shape.
     ...
     *  world.js:2629-2630 does it. ROAD-G TAIL: a WATCHMAN transforms
  ...
  function frame(now) {
    ...
    // through the player for ever. world.js:8235-8354 is the shape.
    arrows.update(dt, {
    ...
    if (walkMode && !tpMode) {
      ...
      for (const ev of weaponRig.frame(dt, { paralyzed })) {
        ...
        if (!cityGuards.resolvePlayerHit(weaponRig.playerWeapon, eye, fwd, player.pos, makeInView(proj, view, multiply), guardHitSound)) {
          ...
          // BEFORE civilians - world.js:8423's order, and the order

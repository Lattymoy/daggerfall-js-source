...
export async function bootExterior(canvas, renderer, params, status) {
  ...
  const outdoorRestDeps = createRestDeps(playerEntity, {
    ...
    advanceMinutes: (n, sharedEnd) => { playerTicker.advance(n); runEncounterTick(walkMode ? player.pos : cam.pos, sharedEnd, true); },   // CAMP1-REST: every tick this drives IS a rest   // ROAD-G TAIL: the catch-up loop rides the rest's minutes, as world.js:4011 has it   // RESTX2: sharedEnd is the session's local sim-minutes online, so the roll still gets a fresh `now` while the real clock stands
   ...
   *  (effects.js:881/:895) - world.js:3256 hoisted its copy for exactly
  ...
  const questWorld = {
    ...
    // other. world.js:7338 is the same line.
     ...
     *  %rn/%rt's producer - world.js:7237-7250. */
    ...
    // QG1: CastSpellDo's two world reads, world.js:7150-7153's pair.
     ...
     *  mode-aware (worldModes:1258), so this is world.js:7123's line
     ...
     *  Bridge-only, no host state - world.js:7131's call verbatim. */
     ...
     *  Everything else is world.js:7238's arm term for term: the cast
  ...
  questBridge = createQuestBridge({
    ...
    // closure S40 gave this host, and world.js:7901's line.
    ...
    // encounter half is world.js:7981's line and the watch half is this
  ...
  // same order as world.js:8202/:8207, for the same reason: the
  ...
  // here, so the sink is handed down at this moment - world.js:8186's
  // line for this host.
  ...
  var modes = createWorldModes({
    ...
    buildingDirectory: () => ({
      ...
      // two inputs the world host uses (world.js:9108).
   ...
   *  second ctx here is the shape FS1 was written about. */
  {
     ...
     *  encounter mount above, and world.js:3438-3454 is the shape.
     ...
     *  world.js:3368-3369 does it. ROAD-G TAIL: a WATCHMAN transforms
  ...
  function frame(now) {
    ...
    // through the player for ever. world.js:10705-10905 is the shape.
    arrows.update(dt, {
    ...
    if (walkMode && !tpMode) {
      ...
      for (const ev of weaponRig.frame(dt, { paralyzed })) {
        ...
        if (!cityGuards.resolvePlayerHit(weaponRig.playerWeapon, eye, fwd, player.pos, makeInView(proj, view, multiply), guardHitSound)) {
          ...
          // BEFORE civilians - world.js:11012's order, and the order

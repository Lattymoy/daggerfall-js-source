...
import { spellRecordOfIndex } from '../systems/loot.js';   // QG1: CastSpellDo's classic-record read (the G4 registry) - world.js:171's import
...
export async function bootExterior(canvas, renderer, params, status) {
  ...
  //     continue;`, world.js:1270) and stands them in batches of their
  ...
  function onExhaustedExterior() {
    ...
    try {
      const out = exhaustionOutcome({
        ...
        enemiesNearby: areEnemiesNearby([...(cityGuards?.guards ?? []), ...(exteriorFoes?.foes ?? [])]),   // ROAD-G G2: BOTH street pools, world.js:1981's line
  ...
  const _livePersons = [];
  // PERF-TOWN1 (2026-09-20): world.js's own scratch, for the other
  // outdoor host - this one minted a little MORE, rebuilding
  // `_livePersons` and a row per person with a `.map` every frame. See
  // bible/07-Rendering/Performance-Town.md.
  const EMPTY_LIVE = Object.freeze([]);
  const _personBatches = [];
  const _personSeats = [];
  const _personSeat = (i) => (_personSeats[i] ??= { person: null, pos: null });
  const _stopFrom = [0, 0, 0];
  const _stopOpts = {
  ...
  const _personStops = (person) => {
    const dx = person.pos[0] - _stopFrom[0], dz = person.pos[2] - _stopFrom[2];
  ...
  // pool the mode machine holds, which is world.js:2548's line.
  ...
  const cityGuards = createCityGuards({
    ...
    // own note describes. Written in world.js:2870's shape, so one pin
   ...
   *  the other two exterior hosts mount - world.js:2609 over the street
   ...
   *  it (world.js:2868-2950) and is its own slice; this pool does not
   *  wait on it. */
  ...
  const outdoorRestDeps = createRestDeps(playerEntity, {
    ...
    advanceMinutes: (n, sharedEnd) => { playerTicker.advance(n); runEncounterTick(walkMode ? player.pos : cam.pos, sharedEnd, true); },   // CAMP1-REST: every tick this drives IS a rest   // ROAD-G TAIL: the catch-up loop rides the rest's minutes, as world.js:4013 has it   // RESTX2: sharedEnd is the session's local sim-minutes online, so the roll still gets a fresh `now` while the real clock stands
   ...
   *  (effects.js:881/:895) - world.js:3277 hoisted its copy for exactly
  ...
  const magic = createPlayerMagic({
    ...
    // two engine-owning hosts wire the identical pair (world.js:3182-3183,
    // dungeonContext.js:2095-2096); `questBridge` is assigned below this
  ...
  const makeJournalWindow = (mode) => {
    ...
    // gate could never run. dungeonContext.js:1318-1323 is the shape:
  ...
  const questWorld = {
    ...
    // other. world.js:7276 is the same line.
     ...
     *  %rn/%rt's producer - world.js:7175-7188. */
    ...
    // QG1: CastSpellDo's two world reads, world.js:7091-7094's pair.
     ...
     *  mode-aware (worldModes:1258), so this is world.js:7064's line
     ...
     *  Bridge-only, no host state - world.js:7072's call verbatim. */
     ...
     *  Everything else is world.js:7176's arm term for term: the cast
  ...
  var modes = createWorldModes({
    ...
    buildingDirectory: () => ({
      ...
      // two inputs the world host uses (world.js:9011).
   ...
   *  second ctx here is the shape FS1 was written about. */
  {
     ...
     *  encounter mount above, and world.js:3459-3475 is the shape.
     ...
     *  world.js:3389-3390 does it. ROAD-G TAIL: a WATCHMAN transforms
  ...
  function frame(now) {
    ...
    if (walkMode) {
      ...
      // PERF-TOWN1: the stop question's options are HOISTED - world.js's
      // own lines, and for its reason: this object and the closure
      // beside it were minted per person per frame for a predicate that
      // reads them once and keeps nothing.
      ...
      _stopFrom[0] = cam.pos[0]; _stopFrom[2] = cam.pos[2];
      const live = !population ? EMPTY_LIVE : population.update(popDt, cam.pos, cam.yaw, eye, !isNight(minute), _personStops);
      ...
      for (const { person, out } of live) {
        ...
        const seat = _personSeat(_livePersons.length);
    ...
    if (walkMode && !tpMode) {
      ...
      for (const ev of weaponRig.frame(dt, { paralyzed })) {
        ...
        if (!cityGuards.resolvePlayerHit(weaponRig.playerWeapon, eye, fwd, player.pos, makeInView(proj, view, multiply), guardHitSound)) {
          ...
          // BEFORE civilians - world.js:10856's order, and the order

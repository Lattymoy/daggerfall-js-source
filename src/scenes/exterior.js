...
export async function bootExterior(canvas, renderer, params, status) {
  ...
  //     continue;`, world.js:1782) and stands them in batches of their
  ...
  function onExhaustedExterior() {
    ...
    try {
      const out = exhaustionOutcome({
        ...
        enemiesNearby: areEnemiesNearby([...(cityGuards?.guards ?? []), ...(exteriorFoes?.foes ?? [])]),   // ROAD-G G2: BOTH street pools, world.js:2651's line
  ...
  // pool the mode machine holds, which is world.js:3220's line.
  ...
  const cityGuards = createCityGuards({
    ...
    // own note describes. Written in world.js:3545's shape, so one pin
   ...
   *  the other two exterior hosts mount - world.js:3281 over the street
   ...
   *  it (world.js:3543-3746) and is its own slice; this pool does not
   *  wait on it. */
   ...
   *  (effects.js:887/:901) - world.js:4077 hoisted its copy for exactly
  ...
  const magic = createPlayerMagic({
    ...
    // two engine-owning hosts wire the identical pair (world.js:4025-4026,
  ...
  const questWorld = {
    ...
    // other. world.js:8470 is the same line.
     ...
     *  %rn/%rt's producer - world.js:8369-8382. */
    ...
    // QG1: CastSpellDo's two world reads, world.js:8285-8288's pair.
     ...
     *  mode-aware (worldModes:1258), so this is world.js:8258's line
     ...
     *  Bridge-only, no host state - world.js:8266's call verbatim. */
     ...
     *  Everything else is world.js:8370's arm term for term: the cast
  ...
  questBridge = createQuestBridge({
    ...
    // closure S40 gave this host, and world.js:9029's line.
    ...
    // encounter half is world.js:9120's line and the watch half is this
  ...
  // same order as world.js:9335/:9340, for the same reason: the
  ...
  // here, so the sink is handed down at this moment - world.js:9319's
  // line for this host.
  ...
  var modes = createWorldModes({
    ...
    buildingDirectory: () => ({
      ...
      // two inputs the world host uses (world.js:11801).
   ...
   *  second ctx here is the shape FS1 was written about. */
  {
     ...
     *  encounter mount above, and world.js:4267-4283 is the shape.
     ...
     *  world.js:4197-4198 does it. ROAD-G TAIL: a WATCHMAN transforms
  ...
  function frame(now) {
    ...
    // through the player for ever. world.js:13563-13802 is the shape.
    arrows.update(dt, {
    ...
    if (walkMode && !tpMode) {
      ...
      for (const ev of weaponRig.frame(dt, { paralyzed })) {
        ...
        if (!cityGuards.resolvePlayerHit(weaponRig.playerWeapon, eye, fwd, player.pos, makeInView(proj, view, multiply), guardHitSound)) {
          ...
          // BEFORE civilians - world.js:13912's order, and the order

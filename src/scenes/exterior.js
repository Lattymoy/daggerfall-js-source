...
export async function bootExterior(canvas, renderer, params, status) {
  ...
  // pool the mode machine holds, which is world.js:2408's line.
  ...
  const cityGuards = createCityGuards({
    ...
    // own note describes. Written in world.js:2577's shape, so one pin
   ...
   *  the other two exterior hosts mount - world.js:2460 over the street
   ...
   *  it (world.js:2575-2657) and is its own slice; this pool does not
   *  wait on it. */
  ...
  const outdoorRestDeps = createRestDeps(playerEntity, {
    ...
    advanceMinutes: (n, sharedEnd) => { playerTicker.advance(n); runEncounterTick(walkMode ? player.pos : cam.pos, sharedEnd, true); },   // CAMP1-REST: every tick this drives IS a rest   // ROAD-G TAIL: the catch-up loop rides the rest's minutes, as world.js:3693 has it   // RESTX2: sharedEnd is the session's local sim-minutes online, so the roll still gets a fresh `now` while the real clock stands
   ...
   *  (effects.js:881/:895) - world.js:2968 hoisted its copy for exactly
  ...
  const magic = createPlayerMagic({
    ...
    // two engine-owning hosts wire the identical pair (world.js:2881-2882,
    // dungeonContext.js:2074-2075); `questBridge` is assigned below this
  ...
  const makeJournalWindow = (mode) => {
    ...
    // gate could never run. dungeonContext.js:1302-1304 is the shape:
  ...
  const questWorld = {
    ...
    // other. world.js:6801 is the same line.
     ...
     *  %rn/%rt's producer - world.js:6700-6713. */
    ...
    // QG1: CastSpellDo's two world reads, world.js:6616-6619's pair.
     ...
     *  mode-aware (worldModes:1258), so this is world.js:6589's line
     ...
     *  Bridge-only, no host state - world.js:6597's call verbatim. */
     ...
     *  Everything else is world.js:6701's arm term for term: the cast
  ...
  questBridge = createQuestBridge({
    ...
    // closure S40 gave this host, and world.js:7354's line.
    ...
    // encounter half is world.js:7434's line and the watch half is this
  ...
  // same order as world.js:7655/:7660, for the same reason: the
  ...
  // here, so the sink is handed down at this moment - world.js:7639's
  // line for this host.
  ...
  var modes = createWorldModes({
    ...
    buildingDirectory: () => ({
      ...
      // two inputs the world host uses (world.js:8478).
   ...
   *  second ctx here is the shape FS1 was written about. */
  {
     ...
     *  encounter mount above, and world.js:3150-3166 is the shape.
     ...
     *  world.js:3089-3090 does it. ROAD-G TAIL: a WATCHMAN transforms
  ...
  function frame(now) {
    ...
    // through the player for ever. world.js:10015-10136 is the shape.
    arrows.update(dt, {
    ...
    if (walkMode && !tpMode) {
      ...
      for (const ev of weaponRig.frame(dt, { paralyzed })) {
        ...
        if (!cityGuards.resolvePlayerHit(weaponRig.playerWeapon, eye, fwd, player.pos, makeInView(proj, view, multiply), guardHitSound)) {
          ...
          // BEFORE civilians - world.js:10209's order, and the order

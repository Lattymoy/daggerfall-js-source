...
import { mwViewFrame, mwViewWheel, mwViewDrawBody, mwViewFootstep, mwViewAttachWagon, mwViewDrawWagon, mwViewWagonTargets, mwViewWagonActivate, mwViewFirstPerson } from '../player/mwView.js';   // MW-D25: the Morrowind camera; AUDIT-EOTB2: the sprite's stride
...
export async function bootExterior(canvas, renderer, params, status) {
  ...
  // pool the mode machine holds, which is world.js:3234's line.
  ...
  const cityGuards = createCityGuards({
    ...
    // own note describes. Written in world.js:3563's shape, so one pin
   ...
   *  the other two exterior hosts mount - world.js:3295 over the street
   ...
   *  it (world.js:3561-3764) and is its own slice; this pool does not
   *  wait on it. */
  ...
  const outdoorRestDeps = createRestDeps(playerEntity, {
    ...
    advanceMinutes: (n, sharedEnd) => { playerTicker.advance(n); runEncounterTick(walkMode ? player.pos : cam.pos, sharedEnd, true); },   // CAMP1-REST: every tick this drives IS a rest   // ROAD-G TAIL: the catch-up loop rides the rest's minutes, as world.js:4934 has it   // RESTX2: sharedEnd is the session's local sim-minutes online, so the roll still gets a fresh `now` while the real clock stands
   ...
   *  (effects.js:887/:901) - world.js:4115 hoisted its copy for exactly
  ...
  const magic = createPlayerMagic({
    ...
    // two engine-owning hosts wire the identical pair (world.js:4063-4064,
  ...
  const questWorld = {
    ...
    // other. world.js:8502 is the same line.
     ...
     *  %rn/%rt's producer - world.js:8401-8414. */
    ...
    // QG1: CastSpellDo's two world reads, world.js:8317-8320's pair.
     ...
     *  mode-aware (worldModes:1258), so this is world.js:8290's line
     ...
     *  Bridge-only, no host state - world.js:8298's call verbatim. */
     ...
     *  Everything else is world.js:8402's arm term for term: the cast
  ...
  questBridge = createQuestBridge({
    ...
    // closure S40 gave this host, and world.js:9061's line.
    ...
    // encounter half is world.js:9152's line and the watch half is this
  ...
  // same order as world.js:9367/:9372, for the same reason: the
  ...
  // here, so the sink is handed down at this moment - world.js:9351's
  // line for this host.
  ...
  var modes = createWorldModes({
    ...
    buildingDirectory: () => ({
      ...
      // two inputs the world host uses (world.js:11534).
   ...
   *  second ctx here is the shape FS1 was written about. */
  {
     ...
     *  encounter mount above, and world.js:4305-4321 is the shape.
     ...
     *  world.js:4235-4236 does it. ROAD-G TAIL: a WATCHMAN transforms
  ...
  function frame(now) {
    ...
    // through the player for ever. world.js:13309-13548 is the shape.
    arrows.update(dt, {
    ...
    if (walkMode && !tpMode) {
      ...
      for (const ev of weaponRig.frame(dt, { paralyzed })) {
        ...
        if (!cityGuards.resolvePlayerHit(weaponRig.playerWeapon, eye, fwd, player.pos, makeInView(proj, view, multiply), guardHitSound)) {
          ...
          // BEFORE civilians - world.js:13658's order, and the order

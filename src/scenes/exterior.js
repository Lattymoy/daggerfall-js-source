...
export async function bootExterior(canvas, renderer, params, status) {
  ...
  function onExhaustedExterior() {
    ...
    try {
      const out = exhaustionOutcome({
        ...
        enemiesNearby: areEnemiesNearby([...(cityGuards?.guards ?? []), ...(exteriorFoes?.foes ?? [])]),   // ROAD-G G2: BOTH street pools, world.js:1729's line
  ...
  // pool the mode machine holds, which is world.js:2236's line.
  ...
  const cityGuards = createCityGuards({
    ...
    // own note describes. Written in world.js:2335's shape, so one pin
   ...
   *  the other two exterior hosts mount - world.js:2276 over the street
   ...
   *  it (world.js:2333-2415) and is its own slice; this pool does not
   *  wait on it. */
  ...
  const outdoorRestDeps = createRestDeps(playerEntity, {
    ...
    advanceMinutes: (n, sharedEnd) => { playerTicker.advance(n); runEncounterTick(walkMode ? player.pos : cam.pos, sharedEnd, true); },   // CAMP1-REST: every tick this drives IS a rest   // ROAD-G TAIL: the catch-up loop rides the rest's minutes, as world.js:3412 has it   // RESTX2: sharedEnd is the session's local sim-minutes online, so the roll still gets a fresh `now` while the real clock stands
   ...
   *  (effects.js:881/:895) - world.js:2715 hoisted its copy for exactly
  ...
  const magic = createPlayerMagic({
    ...
    // two engine-owning hosts wire the identical pair (world.js:2628-2629,
  ...
  const questWorld = {
    ...
    // other. world.js:6224 is the same line.
     ...
     *  %rn/%rt's producer - world.js:6123-6136. */
    ...
    // QG1: CastSpellDo's two world reads, world.js:6039-6042's pair.
     ...
     *  mode-aware (worldModes:1258), so this is world.js:6012's line
     ...
     *  Bridge-only, no host state - world.js:6020's call verbatim. */
     ...
     *  Everything else is world.js:6124's arm term for term: the cast
  ...
  questBridge = createQuestBridge({
    ...
    // closure S40 gave this host, and world.js:6777's line.
    ...
    // encounter half is world.js:6857's line and the watch half is this
  ...
  // same order as world.js:7078/:7083, for the same reason: the
  ...
  // here, so the sink is handed down at this moment - world.js:7062's
  // line for this host.
  ...
  var modes = createWorldModes({
    ...
    buildingDirectory: () => ({
      ...
      // two inputs the world host uses (world.js:7889).
   ...
   *  second ctx here is the shape FS1 was written about. */
  {
     ...
     *  encounter mount above, and world.js:2897-2913 is the shape.
     ...
     *  world.js:2836-2837 does it. ROAD-G TAIL: a WATCHMAN transforms
  ...
  function frame(now) {
    ...
    // through the player for ever. world.js:9288-9408 is the shape.
    arrows.update(dt, {
    ...
    if (walkMode && !tpMode) {
      ...
      for (const ev of weaponRig.frame(dt, { paralyzed })) {
        ...
        if (!cityGuards.resolvePlayerHit(weaponRig.playerWeapon, eye, fwd, player.pos, makeInView(proj, view, multiply), guardHitSound)) {
          ...
          // BEFORE civilians - world.js:9477's order, and the order

...
import { domCodeForKeyCode } from '../systems/keyCodes.js';   // HCC: the mod's KeyCode hotkeys
...
import { spellRecordOfIndex } from '../systems/loot.js';   // QG1: CastSpellDo's classic-record read (the G4 registry) - world.js:203's import
...
import { keyEdges, noteKeyDown, noteKeyUp, beginInputFrame, pressed, released, pressedCode, actionOf, held, moveHeld, anyMove, swallowBrowserKey, mouseCode, isSwingButton, keyboardLook, routeAction, installContextMenuGuard, POLLED_ACTIONS, QUICKSLOT_ACTIONS, swingKeyHeld } from '../ui/input.js';
...
export async function bootExterior(canvas, renderer, params, status) {
  ...
  function onExhaustedExterior() {
    ...
    try {
      const out = exhaustionOutcome({
        ...
        enemiesNearby: areEnemiesNearby([...(cityGuards?.guards ?? []), ...(exteriorFoes?.foes ?? [])]),   // ROAD-G G2: BOTH street pools, world.js:2673's line
  ...
  // pool the mode machine holds, which is world.js:3251's line.
  ...
  const cityGuards = createCityGuards({
    ...
    // own note describes. Written in world.js:3585's shape, so one pin
   ...
   *  the other two exterior hosts mount - world.js:3312 over the street
   ...
   *  it (world.js:3583-3787) and is its own slice; this pool does not
   *  wait on it. */
  ...
  const outdoorRestDeps = createRestDeps(playerEntity, {
    ...
    advanceMinutes: (n, sharedEnd) => { playerTicker.advance(n); runEncounterTick(walkMode ? player.pos : cam.pos, sharedEnd, true); },   // CAMP1-REST: every tick this drives IS a rest   // ROAD-G TAIL: the catch-up loop rides the rest's minutes, as world.js:4975 has it   // RESTX2: sharedEnd is the session's local sim-minutes online, so the roll still gets a fresh `now` while the real clock stands
  ...
  const weaponRig = createWeaponRig({
    ...
    keyDown: (code) => keys.has(code), torches: () => droppedTorches,   // HT1
    say: (l) => townTalk.say(l),
   ...
   *  (effects.js:887/:901) - world.js:4138 hoisted its copy for exactly
  ...
  const magic = createPlayerMagic({
    ...
    // (hostMagic.js:77-78), which are the ONLY route into the quest
    ...
    // two engine-owning hosts wire the identical pair (world.js:4086-4087,
  ...
  addEventListener('keydown', (e) => {
    ...
    // held-keys Set (ui/input.js:249-270), and no host passed one - so
    ...
    if (!townTalk.overlayActive && (modes?.mode ?? 'exterior') === 'exterior') {
      ...
      // own `POLLED_ACTIONS.has(act)` decline (ui/input.js:675) never
  ...
  const questWorld = {
    ...
    // other. world.js:8567 is the same line.
     ...
     *  %rn/%rt's producer - world.js:8466-8479. */
    ...
    // QG1: CastSpellDo's two world reads, world.js:8382-8385's pair.
     ...
     *  mode-aware (worldModes:1258), so this is world.js:8355's line
     ...
     *  Bridge-only, no host state - world.js:8363's call verbatim. */
     ...
     *  Everything else is world.js:8467's arm term for term: the cast
  ...
  questBridge = createQuestBridge({
    ...
    // closure S40 gave this host, and world.js:9126's line.
    ...
    // encounter half is world.js:9217's line and the watch half is this
  ...
  // same order as world.js:9432/:9437, for the same reason: the
  ...
  // here, so the sink is handed down at this moment - world.js:9416's
  // line for this host.
  ...
  var modes = createWorldModes({
    ...
    buildingDirectory: () => ({
      ...
      // two inputs the world host uses (world.js:11912).
   ...
   *  second ctx here is the shape FS1 was written about. */
  {
     ...
     *  encounter mount above, and world.js:4328-4344 is the shape.
     ...
     *  world.js:4258-4259 does it. ROAD-G TAIL: a WATCHMAN transforms
  ...
  function frame(now) {
    ...
    // through the player for ever. world.js:13693-13932 is the shape.
    arrows.update(dt, {
    ...
    if (walkMode && !tpMode) {
      ...
      for (const ev of weaponRig.frame(dt, { paralyzed })) {
        ...
        if (!cityGuards.resolvePlayerHit(weaponRig.playerWeapon, eye, fwd, player.pos, makeInView(proj, view, multiply), guardHitSound)) {
          ...
          // BEFORE civilians - world.js:14042's order, and the order

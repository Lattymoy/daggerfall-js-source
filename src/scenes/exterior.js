...
import { spellRecordOfIndex } from '../systems/loot.js';   // QG1: CastSpellDo's classic-record read (the G4 registry) - world.js:141's import
...
export async function bootExterior(canvas, renderer, params, status) {
  ...
  //     continue;`, world.js:1111) and stands them in batches of their
  ...
  const cityGuards = createCityGuards({
    ...
    // own note describes. Written in world.js:2370's shape, so one pin
   ...
   *  it (world.js:2368-2450) and is its own slice; this pool does not
   *  wait on it. */
  ...
  const outdoorRestDeps = createRestDeps(playerEntity, {
    ...
    advanceMinutes: (n, sharedEnd) => { playerTicker.advance(n); runEncounterTick(walkMode ? player.pos : cam.pos, sharedEnd); },   // ROAD-G TAIL: the catch-up loop rides the rest's minutes, as world.js:3427 has it   // RESTX2: sharedEnd is the session's local sim-minutes online, so the roll still gets a fresh `now` while the real clock stands
   ...
   *  (effects.js:881/:895) - world.js:2732 hoisted its copy for exactly
  ...
  const magic = createPlayerMagic({
    ...
    // two engine-owning hosts wire the identical pair (world.js:2645-2646,
  ...
  const questWorld = {
    ...
    // other. world.js:6233 is the same line.
     ...
     *  %rn/%rt's producer - world.js:6132-6145. */
    ...
    // QG1: CastSpellDo's two world reads, world.js:6048-6051's pair.
     ...
     *  mode-aware (worldModes:1258), so this is world.js:6021's line
     ...
     *  Bridge-only, no host state - world.js:6029's call verbatim. */
     ...
     *  Everything else is world.js:6133's arm term for term: the cast
  ...
  questBridge = createQuestBridge({
    ...
    // closure S40 gave this host, and world.js:6786's line.
    ...
    // encounter half is world.js:6866's line and the watch half is this
  ...
  // same order as world.js:7087/:7092, for the same reason: the
  ...
  // here, so the sink is handed down at this moment - world.js:7071's
  // line for this host.
  ...
  var modes = createWorldModes({
    ...
    buildingDirectory: () => ({
      ...
      // two inputs the world host uses (world.js:7898).
   ...
   *  second ctx here is the shape FS1 was written about. */
  {
     ...
     *  encounter mount above, and world.js:2914-2930 is the shape.
     ...
     *  world.js:2853-2854 does it. ROAD-G TAIL: a WATCHMAN transforms
  ...
  function frame(now) {
    ...
    // through the player for ever. world.js:9305-9426 is the shape.
    arrows.update(dt, {
    ...
    if (walkMode && !tpMode) {
      ...
      for (const ev of weaponRig.frame(dt, { paralyzed })) {
        ...
        if (!cityGuards.resolvePlayerHit(weaponRig.playerWeapon, eye, fwd, player.pos, makeInView(proj, view, multiply), guardHitSound)) {
          ...
          // BEFORE civilians - world.js:9495's order, and the order

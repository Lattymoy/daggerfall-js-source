...
export async function bootExterior(canvas, renderer, params, status) {
  ...
  // pool the mode machine holds, which is world.js:2613's line.
  ...
  const cityGuards = createCityGuards({
    ...
    // own note describes. Written in world.js:2843's shape, so one pin
   ...
   *  the other two exterior hosts mount - world.js:2674 over the street
   ...
   *  it (world.js:2841-2923) and is its own slice; this pool does not
   *  wait on it. */
  ...
  const outdoorRestDeps = createRestDeps(playerEntity, {
    ...
    advanceMinutes: (n, sharedEnd) => { playerTicker.advance(n); runEncounterTick(walkMode ? player.pos : cam.pos, sharedEnd, true); },   // CAMP1-REST: every tick this drives IS a rest   // ROAD-G TAIL: the catch-up loop rides the rest's minutes, as world.js:4005 has it   // RESTX2: sharedEnd is the session's local sim-minutes online, so the roll still gets a fresh `now` while the real clock stands
   ...
   *  (effects.js:881/:895) - world.js:3250 hoisted its copy for exactly
  ...
  const magic = createPlayerMagic({
    ...
    // dungeonContext.js:2206-2207); `questBridge` is assigned below this
  ...
  const questWorld = {
    ...
    // other. world.js:7312 is the same line.
     ...
     *  %rn/%rt's producer - world.js:7211-7224. */
    ...
    // QG1: CastSpellDo's two world reads, world.js:7124-7127's pair.
     ...
     *  mode-aware (worldModes:1258), so this is world.js:7097's line
     ...
     *  Bridge-only, no host state - world.js:7105's call verbatim. */
     ...
     *  Everything else is world.js:7212's arm term for term: the cast
  ...
  questBridge = createQuestBridge({
    ...
    // closure S40 gave this host, and world.js:7875's line.
    ...
    // encounter half is world.js:7955's line and the watch half is this
  ...
  // same order as world.js:8176/:8181, for the same reason: the
  ...
  // here, so the sink is handed down at this moment - world.js:8160's
  // line for this host.
  ...
  var modes = createWorldModes({
    ...
    buildingDirectory: () => ({
      ...
      // two inputs the world host uses (world.js:9082).
   ...
   *  second ctx here is the shape FS1 was written about. */
  {
     ...
     *  encounter mount above, and world.js:3432-3448 is the shape.
     ...
     *  world.js:3362-3363 does it. ROAD-G TAIL: a WATCHMAN transforms
  ...
  function frame(now) {
    ...
    // through the player for ever. world.js:10678-10853 is the shape.
    arrows.update(dt, {
    ...
    if (walkMode && !tpMode) {
      ...
      for (const ev of weaponRig.frame(dt, { paralyzed })) {
        ...
        if (!cityGuards.resolvePlayerHit(weaponRig.playerWeapon, eye, fwd, player.pos, makeInView(proj, view, multiply), guardHitSound)) {
          ...
          // BEFORE civilians - world.js:10950's order, and the order

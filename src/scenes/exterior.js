...
export async function bootExterior(canvas, renderer, params, status) {
  ...
  for (const b of loc.blocks) {
    ...
    for (const light of collectCityLights(b.dfBlock, lightSize)) {
      ...
      if (isHearthFlat(LIGHTS_ARCHIVE, light.record)) cityHearths.push(l);
    }
  }
  ...
  function onExhaustedExterior() {
    ...
    try {
      const out = exhaustionOutcome({
        ...
        enemiesNearby: areEnemiesNearby([...(cityGuards?.guards ?? []), ...(exteriorFoes?.foes ?? [])]),   // ROAD-G G2: BOTH street pools, world.js:2083's line
  ...
  // pool the mode machine holds, which is world.js:2651's line.
  ...
  const cityGuards = createCityGuards({
    ...
    // own note describes. Written in world.js:2881's shape, so one pin
   ...
   *  the other two exterior hosts mount - world.js:2712 over the street
   ...
   *  it (world.js:2879-3071) and is its own slice; this pool does not
   *  wait on it. */
  ...
  const outdoorRestDeps = createRestDeps(playerEntity, {
    ...
    advanceMinutes: (n, sharedEnd) => { playerTicker.advance(n); runEncounterTick(walkMode ? player.pos : cam.pos, sharedEnd, true); },   // CAMP1-REST: every tick this drives IS a rest   // ROAD-G TAIL: the catch-up loop rides the rest's minutes, as world.js:4215 has it   // RESTX2: sharedEnd is the session's local sim-minutes online, so the roll still gets a fresh `now` while the real clock stands
   ...
   *  (effects.js:881/:895) - world.js:3402 hoisted its copy for exactly
  ...
  const magic = createPlayerMagic({
    ...
    // two engine-owning hosts wire the identical pair (world.js:3350-3351,
    // dungeonContext.js:2198-2199); `questBridge` is assigned below this
  ...
  const makeJournalWindow = (mode) => {
    ...
    // gate could never run. dungeonContext.js:1429-1434 is the shape:
  ...
  const questWorld = {
    ...
    // other. world.js:7709 is the same line.
     ...
     *  %rn/%rt's producer - world.js:7608-7621. */
    ...
    // QG1: CastSpellDo's two world reads, world.js:7524-7527's pair.
     ...
     *  mode-aware (worldModes:1258), so this is world.js:7497's line
     ...
     *  Bridge-only, no host state - world.js:7505's call verbatim. */
     ...
     *  Everything else is world.js:7609's arm term for term: the cast
  ...
  questBridge = createQuestBridge({
    ...
    // closure S40 gave this host, and world.js:8263's line.
    ...
    // encounter half is world.js:8354's line and the watch half is this
  ...
  // same order as world.js:8569/:8574, for the same reason: the
  ...
  // here, so the sink is handed down at this moment - world.js:8553's
  // line for this host.
  ...
  var modes = createWorldModes({
    ...
    buildingDirectory: () => ({
      ...
      // two inputs the world host uses (world.js:10420).
   ...
   *  second ctx here is the shape FS1 was written about. */
  {
     ...
     *  encounter mount above, and world.js:3584-3600 is the shape.
     ...
     *  world.js:3514-3515 does it. ROAD-G TAIL: a WATCHMAN transforms
  ...
  function frame(now) {
    ...
    // through the player for ever. world.js:12089-12289 is the shape.
    arrows.update(dt, {
    ...
    if (walkMode && !tpMode) {
      ...
      for (const ev of weaponRig.frame(dt, { paralyzed })) {
        ...
        if (!cityGuards.resolvePlayerHit(weaponRig.playerWeapon, eye, fwd, player.pos, makeInView(proj, view, multiply), guardHitSound)) {
          ...
          // BEFORE civilians - world.js:12396's order, and the order

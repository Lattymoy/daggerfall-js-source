...
export async function bootExterior(canvas, renderer, params, status) {
  ...
  const outdoorRestDeps = createRestDeps(playerEntity, {
    ...
    advanceMinutes: (n, sharedEnd) => { playerTicker.advance(n); runEncounterTick(walkMode ? player.pos : cam.pos, sharedEnd, true); },   // CAMP1-REST: every tick this drives IS a rest   // ROAD-G TAIL: the catch-up loop rides the rest's minutes, as world.js:3858 has it   // RESTX2: sharedEnd is the session's local sim-minutes online, so the roll still gets a fresh `now` while the real clock stands
   ...
   *  (effects.js:881/:895) - world.js:3122 hoisted its copy for exactly
  ...
  const questWorld = {
    ...
    // other. world.js:7121 is the same line.
     ...
     *  %rn/%rt's producer - world.js:7020-7033. */
    ...
    // QG1: CastSpellDo's two world reads, world.js:6936-6939's pair.
     ...
     *  mode-aware (worldModes:1258), so this is world.js:6909's line
     ...
     *  Bridge-only, no host state - world.js:6917's call verbatim. */
     ...
     *  Everything else is world.js:7021's arm term for term: the cast
  ...
  questBridge = createQuestBridge({
    ...
    // closure S40 gave this host, and world.js:7674's line.
    ...
    // encounter half is world.js:7754's line and the watch half is this
  ...
  // same order as world.js:7975/:7980, for the same reason: the
  ...
  // here, so the sink is handed down at this moment - world.js:7959's
  // line for this host.
  ...
  var modes = createWorldModes({
    ...
    buildingDirectory: () => ({
      ...
      // two inputs the world host uses (world.js:8856).
   ...
   *  second ctx here is the shape FS1 was written about. */
  {
     ...
     *  encounter mount above, and world.js:3304-3320 is the shape.
     ...
     *  world.js:3234-3235 does it. ROAD-G TAIL: a WATCHMAN transforms
  ...
  function frame(now) {
    ...
    // through the player for ever. world.js:10426-10600 is the shape.
    arrows.update(dt, {
    ...
    if (walkMode && !tpMode) {
      ...
      for (const ev of weaponRig.frame(dt, { paralyzed })) {
        ...
        if (!cityGuards.resolvePlayerHit(weaponRig.playerWeapon, eye, fwd, player.pos, makeInView(proj, view, multiply), guardHitSound)) {
          ...
          // BEFORE civilians - world.js:10695's order, and the order

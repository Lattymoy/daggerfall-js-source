...
import { spellRecordOfIndex } from '../systems/loot.js';   // QG1: CastSpellDo's classic-record read (the G4 registry) - world.js:131's import
...
export async function bootExterior(canvas, renderer, params, status) {
  ...
  //     continue;`, world.js:1090) and stands them in batches of their
  ...
  function onExhaustedExterior() {
    ...
    try {
      const out = exhaustionOutcome({
        ...
        enemiesNearby: areEnemiesNearby([...(cityGuards?.guards ?? []), ...(exteriorFoes?.foes ?? [])]),   // ROAD-G G2: BOTH street pools, world.js:1703's line
  ...
  // pool the mode machine holds, which is world.js:2198's line.
  ...
  const cityGuards = createCityGuards({
    ...
    // own note describes. Written in world.js:2297's shape, so one pin
   ...
   *  the other two exterior hosts mount - world.js:2238 over the street
   ...
   *  it (world.js:2295-2377) and is its own slice; this pool does not
   *  wait on it. */
  ...
  const outdoorRestDeps = createRestDeps(playerEntity, {
    ...
    advanceMinutes: (n) => { playerTicker.advance(n); runEncounterTick(walkMode ? player.pos : cam.pos); },   // ROAD-G TAIL: the catch-up loop rides the rest's minutes, as world.js:3205 has it
   ...
   *  (effects.js:881/:895) - world.js:2630 hoisted its copy for exactly
  ...
  const magic = createPlayerMagic({
    ...
    // two engine-owning hosts wire the identical pair (world.js:2543-2544,
  ...
  const questWorld = {
    ...
    // other. world.js:5895 is the same line.
     ...
     *  %rn/%rt's producer - world.js:5794-5807. */
    ...
    // QG1: CastSpellDo's two world reads, world.js:5710-5713's pair.
     ...
     *  mode-aware (worldModes:1258), so this is world.js:5683's line
     ...
     *  Bridge-only, no host state - world.js:5691's call verbatim. */
     ...
     *  Everything else is world.js:5795's arm term for term: the cast
  ...
  questBridge = createQuestBridge({
    ...
    // closure S40 gave this host, and world.js:6448's line.
    ...
    // encounter half is world.js:6528's line and the watch half is this
  ...
  // same order as world.js:6749/:6754, for the same reason: the
  ...
  // here, so the sink is handed down at this moment - world.js:6733's
  // line for this host.
  ...
  var modes = createWorldModes({
    ...
    buildingDirectory: () => ({
      ...
      // two inputs the world host uses (world.js:7463).
   ...
   *  second ctx here is the shape FS1 was written about. */
  {
     ...
     *  encounter mount above, and world.js:2812-2828 is the shape.
     ...
     *  world.js:2751-2752 does it. ROAD-G TAIL: a WATCHMAN transforms
  ...
  function frame(now) {
    ...
    // through the player for ever. world.js:8828-8948 is the shape.
    arrows.update(dt, {
    ...
    if (walkMode && !tpMode) {
      ...
      for (const ev of weaponRig.frame(dt, { paralyzed })) {
        ...
        if (!cityGuards.resolvePlayerHit(weaponRig.playerWeapon, eye, fwd, player.pos, makeInView(proj, view, multiply), guardHitSound)) {
          ...
          // BEFORE civilians - world.js:9017's order, and the order

...
import { spellRecordOfIndex } from '../systems/loot.js';   // QG1: CastSpellDo's classic-record read (the G4 registry) - world.js:121's import
...
export async function bootExterior(canvas, renderer, params, status) {
  ...
  //     continue;`, world.js:1022) and stands them in batches of their
  ...
  function onExhaustedExterior() {
    ...
    try {
      const out = exhaustionOutcome({
        ...
        enemiesNearby: areEnemiesNearby([...(cityGuards?.guards ?? []), ...(exteriorFoes?.foes ?? [])]),   // ROAD-G G2: BOTH street pools, world.js:1617's line
  ...
  // pool the mode machine holds, which is world.js:2072's line.
  ...
  const cityGuards = createCityGuards({
    ...
    // own note describes. Written in world.js:2160's shape, so one pin
   ...
   *  the other two exterior hosts mount - world.js:2112 over the street
   ...
   *  it (world.js:2158-2242) and is its own slice; this pool does not
   *  wait on it. */
  ...
  const outdoorRestDeps = createRestDeps(playerEntity, {
    ...
    advanceMinutes: (n) => { playerTicker.advance(n); runEncounterTick(walkMode ? player.pos : cam.pos); },   // ROAD-G TAIL: the catch-up loop rides the rest's minutes, as world.js:3043 has it
   ...
   *  (effects.js:881/:895) - world.js:2300 hoisted its copy for exactly
  ...
  const magic = createPlayerMagic({
    ...
    // two engine-owning hosts wire the identical pair (world.js:2408-2409,
  ...
  const questWorld = {
    ...
    // other. world.js:5533 is the same line.
     ...
     *  %rn/%rt's producer - world.js:5535-5548. */
    ...
    // QG1: CastSpellDo's two world reads, world.js:5451-5454's pair.
     ...
     *  mode-aware (worldModes:1258), so this is world.js:5424's line
     ...
     *  Bridge-only, no host state - world.js:5432's call verbatim. */
     ...
     *  Everything else is world.js:5536's arm term for term: the cast
  ...
  questBridge = createQuestBridge({
    ...
    // closure S40 gave this host, and world.js:6189's line.
    ...
    // encounter half is world.js:6111's line and the watch half is this
  ...
  // same order as world.js:6489/:6489, for the same reason: the
  ...
  // here, so the sink is handed down at this moment - world.js:6473's
  // line for this host.
  ...
  var modes = createWorldModes({
    ...
    buildingDirectory: () => ({
      ...
      // two inputs the world host uses (world.js:6587).
   ...
   *  second ctx here is the shape FS1 was written about. */
  {
     ...
     *  encounter mount above, and world.js:2655-2671 is the shape.
     ...
     *  world.js:2597-2598 does it. ROAD-G TAIL: a WATCHMAN transforms
  ...
  function frame(now) {
    ...
    // through the player for ever. world.js:7877-7996 is the shape.
    arrows.update(dt, {
    ...
    if (walkMode && !tpMode) {
      ...
      for (const ev of weaponRig.frame(dt, { paralyzed })) {
        ...
        if (!cityGuards.resolvePlayerHit(weaponRig.playerWeapon, eye, fwd, player.pos, makeInView(proj, view, multiply), guardHitSound)) {
          ...
          // BEFORE civilians - world.js:8065's order, and the order

...
export async function bootExterior(canvas, renderer, params, status) {
   ...
   *  it (world.js:2991-3194) and is its own slice; this pool does not
   *  wait on it. */
  ...
  const outdoorRestDeps = createRestDeps(playerEntity, {
    ...
    advanceMinutes: (n, sharedEnd) => { playerTicker.advance(n); runEncounterTick(walkMode ? player.pos : cam.pos, sharedEnd, true); },   // CAMP1-REST: every tick this drives IS a rest   // ROAD-G TAIL: the catch-up loop rides the rest's minutes, as world.js:4347 has it   // RESTX2: sharedEnd is the session's local sim-minutes online, so the roll still gets a fresh `now` while the real clock stands
   ...
   *  (effects.js:887/:901) - world.js:3525 hoisted its copy for exactly
  ...
  const magic = createPlayerMagic({
    ...
    // two engine-owning hosts wire the identical pair (world.js:3473-3474,
  ...
  const questWorld = {
    ...
    // other. world.js:7867 is the same line.
     ...
     *  %rn/%rt's producer - world.js:7766-7779. */
    ...
    // QG1: CastSpellDo's two world reads, world.js:7682-7685's pair.
     ...
     *  mode-aware (worldModes:1258), so this is world.js:7655's line
     ...
     *  Bridge-only, no host state - world.js:7663's call verbatim. */
     ...
     *  Everything else is world.js:7767's arm term for term: the cast
  ...
  questBridge = createQuestBridge({
    ...
    // closure S40 gave this host, and world.js:8426's line.
    ...
    // encounter half is world.js:8517's line and the watch half is this
  ...
  // same order as world.js:8732/:8737, for the same reason: the
  ...
  // here, so the sink is handed down at this moment - world.js:8716's
  // line for this host.
  ...
  var modes = createWorldModes({
    ...
    buildingDirectory: () => ({
      ...
      // two inputs the world host uses (world.js:10882).
   ...
   *  second ctx here is the shape FS1 was written about. */
  {
     ...
     *  encounter mount above, and world.js:3715-3731 is the shape.
     ...
     *  world.js:3645-3646 does it. ROAD-G TAIL: a WATCHMAN transforms
  ...
  function frame(now) {
    ...
    // through the player for ever. world.js:12583-12812 is the shape.
    arrows.update(dt, {
    ...
    if (walkMode && !tpMode) {
      ...
      for (const ev of weaponRig.frame(dt, { paralyzed })) {
        ...
        if (!cityGuards.resolvePlayerHit(weaponRig.playerWeapon, eye, fwd, player.pos, makeInView(proj, view, multiply), guardHitSound)) {
          ...
          // BEFORE civilians - world.js:12922's order, and the order

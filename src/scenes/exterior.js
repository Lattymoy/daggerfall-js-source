...
export async function bootExterior(canvas, renderer, params, status) {
  ...
  //     continue;`, world.js:1101) and stands them in batches of their
  ...
  function onExhaustedExterior() {
    ...
    try {
      const out = exhaustionOutcome({
        ...
        enemiesNearby: areEnemiesNearby([...(cityGuards?.guards ?? []), ...(exteriorFoes?.foes ?? [])]),   // ROAD-G G2: BOTH street pools, world.js:1714's line
  ...
  // pool the mode machine holds, which is world.js:2213's line.
  ...
  const cityGuards = createCityGuards({
    ...
    // own note describes. Written in world.js:2312's shape, so one pin
   ...
   *  the other two exterior hosts mount - world.js:2253 over the street
   ...
   *  it (world.js:2310-2392) and is its own slice; this pool does not
   *  wait on it. */
  ...
  const outdoorRestDeps = createRestDeps(playerEntity, {
    ...
    advanceMinutes: (n) => { playerTicker.advance(n); runEncounterTick(walkMode ? player.pos : cam.pos); },   // ROAD-G TAIL: the catch-up loop rides the rest's minutes, as world.js:3220 has it
   ...
   *  (effects.js:881/:895) - world.js:2645 hoisted its copy for exactly
  ...
  const magic = createPlayerMagic({
    ...
    // two engine-owning hosts wire the identical pair (world.js:2558-2559,
  ...
  const questWorld = {
    ...
    // other. world.js:5930 is the same line.
     ...
     *  %rn/%rt's producer - world.js:5829-5842. */
    ...
    // QG1: CastSpellDo's two world reads, world.js:5745-5748's pair.
     ...
     *  mode-aware (worldModes:1258), so this is world.js:5718's line
     ...
     *  Bridge-only, no host state - world.js:5726's call verbatim. */
     ...
     *  Everything else is world.js:5830's arm term for term: the cast
  ...
  questBridge = createQuestBridge({
    ...
    // closure S40 gave this host, and world.js:6483's line.
    ...
    // encounter half is world.js:6563's line and the watch half is this
  ...
  // same order as world.js:6784/:6789, for the same reason: the
  ...
  // here, so the sink is handed down at this moment - world.js:6768's
  // line for this host.
  ...
  var modes = createWorldModes({
    ...
    buildingDirectory: () => ({
      ...
      // two inputs the world host uses (world.js:7563).
   ...
   *  second ctx here is the shape FS1 was written about. */
  {
     ...
     *  encounter mount above, and world.js:2827-2843 is the shape.
     ...
     *  world.js:2766-2767 does it. ROAD-G TAIL: a WATCHMAN transforms
  ...
  function frame(now) {
    ...
    // through the player for ever. world.js:8937-9057 is the shape.
    arrows.update(dt, {
    ...
    if (walkMode && !tpMode) {
      ...
      for (const ev of weaponRig.frame(dt, { paralyzed })) {
        ...
        if (!cityGuards.resolvePlayerHit(weaponRig.playerWeapon, eye, fwd, player.pos, makeInView(proj, view, multiply), guardHitSound)) {
          ...
          // BEFORE civilians - world.js:9126's order, and the order

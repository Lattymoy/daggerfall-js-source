...
import { createMountRig } from '../player/mountRig.js';   // MAC-K3: this host had NO transport surface at all - see the note at the build
...
import { spellRecordOfIndex } from '../systems/loot.js';   // QG1: CastSpellDo's classic-record read (the G4 registry) - world.js:199's import
...
export async function bootExterior(canvas, renderer, params, status) {
  ...
  //     host's static NPCs take it (interiorContext.js:417-435).
  ...
  //     continue;`, world.js:1775) and stands them in batches of their
  ...
  function onExhaustedExterior() {
    ...
    try {
      const out = exhaustionOutcome({
        ...
        enemiesNearby: areEnemiesNearby([...(cityGuards?.guards ?? []), ...(exteriorFoes?.foes ?? [])]),   // ROAD-G G2: BOTH street pools, world.js:2644's line
  ...
  // pool the mode machine holds, which is world.js:3213's line.
  ...
  const cityGuards = createCityGuards({
    ...
    // own note describes. Written in world.js:3538's shape, so one pin
   ...
   *  the other two exterior hosts mount - world.js:3274 over the street
   ...
   *  it (world.js:3536-3739) and is its own slice; this pool does not
   *  wait on it. */
  ...
  const outdoorRestDeps = createRestDeps(playerEntity, {
    ...
    advanceMinutes: (n, sharedEnd) => { playerTicker.advance(n); runEncounterTick(walkMode ? player.pos : cam.pos, sharedEnd, true); },   // CAMP1-REST: every tick this drives IS a rest   // ROAD-G TAIL: the catch-up loop rides the rest's minutes, as world.js:4889 has it   // RESTX2: sharedEnd is the session's local sim-minutes online, so the roll still gets a fresh `now` while the real clock stands
   ...
   *  (effects.js:887/:901) - world.js:4070 hoisted its copy for exactly
  ...
  const magic = createPlayerMagic({
    ...
    // two engine-owning hosts wire the identical pair (world.js:4018-4019,
  ...
  const questWorld = {
    ...
    // other. world.js:8457 is the same line.
     ...
     *  %rn/%rt's producer - world.js:8356-8369. */
    ...
    // QG1: CastSpellDo's two world reads, world.js:8272-8275's pair.
     ...
     *  mode-aware (worldModes:1258), so this is world.js:8245's line
     ...
     *  Bridge-only, no host state - world.js:8253's call verbatim. */
     ...
     *  Everything else is world.js:8357's arm term for term: the cast
  ...
  questBridge = createQuestBridge({
    ...
    // closure S40 gave this host, and world.js:9016's line.
    ...
    // encounter half is world.js:9107's line and the watch half is this
  ...
  // same order as world.js:9322/:9327, for the same reason: the
  ...
  // here, so the sink is handed down at this moment - world.js:9306's
  // line for this host.
  ...
  var modes = createWorldModes({
    ...
    buildingDirectory: () => ({
      ...
      // two inputs the world host uses (world.js:11489).
   ...
   *  session (enchantments.js:253-255) and every arm that folds it in
   ...
   *  (enchantments.js:337-345, :372-393), HealthLeech never billed the
   ...
   *  second ctx here is the shape FS1 was written about. */
  {
     ...
     *  encounter mount above, and world.js:4260-4276 is the shape.
     ...
     *  world.js:4190-4191 does it. ROAD-G TAIL: a WATCHMAN transforms
  ...
  function frame(now) {
    ...
    // through the player for ever. world.js:13263-13502 is the shape.
    arrows.update(dt, {
    ...
    if (walkMode && !tpMode) {
      ...
      for (const ev of weaponRig.frame(dt, { paralyzed })) {
        ...
        if (!cityGuards.resolvePlayerHit(weaponRig.playerWeapon, eye, fwd, player.pos, makeInView(proj, view, multiply), guardHitSound)) {
          ...
          // BEFORE civilians - world.js:13612's order, and the order

...
export async function bootExterior(canvas, renderer, params, status) {
  ...
  // pool the mode machine holds, which is world.js:3230's line.
  ...
  const cityGuards = createCityGuards({
    ...
    // own note describes. Written in world.js:3564's shape, so one pin
   ...
   *  the other two exterior hosts mount - world.js:3291 over the street
   ...
   *  it (world.js:3562-3765) and is its own slice; this pool does not
   *  wait on it. */
  ...
  const outdoorRestDeps = createRestDeps(playerEntity, {
    ...
    advanceMinutes: (n, sharedEnd) => { playerTicker.advance(n); runEncounterTick(walkMode ? player.pos : cam.pos, sharedEnd, true); },   // CAMP1-REST: every tick this drives IS a rest   // ROAD-G TAIL: the catch-up loop rides the rest's minutes, as world.js:4935 has it   // RESTX2: sharedEnd is the session's local sim-minutes online, so the roll still gets a fresh `now` while the real clock stands
   ...
   *  (effects.js:887/:901) - world.js:4116 hoisted its copy for exactly
  ...
  const magic = createPlayerMagic({
    ...
    // two engine-owning hosts wire the identical pair (world.js:4064-4065,
  ...
  const questWorld = {
    ...
    // other. world.js:8509 is the same line.
     ...
     *  %rn/%rt's producer - world.js:8408-8421. */
    ...
    // QG1: CastSpellDo's two world reads, world.js:8324-8327's pair.
     ...
     *  mode-aware (worldModes:1258), so this is world.js:8297's line
     ...
     *  Bridge-only, no host state - world.js:8305's call verbatim. */
     ...
     *  Everything else is world.js:8409's arm term for term: the cast
  ...
  questBridge = createQuestBridge({
    ...
    // closure S40 gave this host, and world.js:9068's line.
    ...
    // encounter half is world.js:9159's line and the watch half is this
  ...
  // same order as world.js:9374/:9379, for the same reason: the
  ...
  // here, so the sink is handed down at this moment - world.js:9358's
  // line for this host.
   ...
   *  second ctx here is the shape FS1 was written about. */
  {
     ...
     *  encounter mount above, and world.js:4306-4322 is the shape.
     ...
     *  world.js:4236-4237 does it. ROAD-G TAIL: a WATCHMAN transforms
  ...
  function frame(now) {
    ...
    // through the player for ever. world.js:13296-13535 is the shape.
    arrows.update(dt, {
    ...
    if (walkMode && !tpMode) {
      ...
      for (const ev of weaponRig.frame(dt, { paralyzed })) {
        ...
        if (!cityGuards.resolvePlayerHit(weaponRig.playerWeapon, eye, fwd, player.pos, makeInView(proj, view, multiply), guardHitSound)) {
          ...
          // BEFORE civilians - world.js:13645's order, and the order

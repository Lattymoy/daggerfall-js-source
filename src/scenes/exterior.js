...
export async function bootExterior(canvas, renderer, params, status) {
  ...
  //     continue;`, world.js:1384) and stands them in batches of their
  ...
  function onExhaustedExterior() {
    ...
    try {
      const out = exhaustionOutcome({
        ...
        enemiesNearby: areEnemiesNearby([...(cityGuards?.guards ?? []), ...(exteriorFoes?.foes ?? [])]),   // ROAD-G G2: BOTH street pools, world.js:2099's line
  ...
  // pool the mode machine holds, which is world.js:2668's line.
  ...
  const cityGuards = createCityGuards({
    ...
    // own note describes. Written in world.js:2993's shape, so one pin
   ...
   *  the other two exterior hosts mount - world.js:2729 over the street
   ...
   *  it (world.js:2991-3184) and is its own slice; this pool does not
   *  wait on it. */
  ...
  const outdoorRestDeps = createRestDeps(playerEntity, {
    ...
    advanceMinutes: (n, sharedEnd) => { playerTicker.advance(n); runEncounterTick(walkMode ? player.pos : cam.pos, sharedEnd, true); },   // CAMP1-REST: every tick this drives IS a rest   // ROAD-G TAIL: the catch-up loop rides the rest's minutes, as world.js:4337 has it   // RESTX2: sharedEnd is the session's local sim-minutes online, so the roll still gets a fresh `now` while the real clock stands
   ...
   *  (effects.js:887/:901) - world.js:3515 hoisted its copy for exactly
  ...
  const magic = createPlayerMagic({
    ...
    // two engine-owning hosts wire the identical pair (world.js:3463-3464,
  ...
  const questWorld = {
    ...
    // other. world.js:7857 is the same line.
     ...
     *  %rn/%rt's producer - world.js:7756-7769. */
    ...
    // QG1: CastSpellDo's two world reads, world.js:7672-7675's pair.
     ...
     *  mode-aware (worldModes:1258), so this is world.js:7645's line
     ...
     *  Bridge-only, no host state - world.js:7653's call verbatim. */
     ...
     *  Everything else is world.js:7757's arm term for term: the cast
  ...
  questBridge = createQuestBridge({
    ...
    // closure S40 gave this host, and world.js:8416's line.
    ...
    // encounter half is world.js:8507's line and the watch half is this
  ...
  // same order as world.js:8722/:8727, for the same reason: the
  ...
  // here, so the sink is handed down at this moment - world.js:8706's
  // line for this host.
  ...
  var modes = createWorldModes({
    ...
    buildingDirectory: () => ({
      ...
      // two inputs the world host uses (world.js:10866).
   ...
   *  second ctx here is the shape FS1 was written about. */
  {
     ...
     *  encounter mount above, and world.js:3705-3721 is the shape.
     ...
     *  world.js:3635-3636 does it. ROAD-G TAIL: a WATCHMAN transforms
  ...
  function frame(now) {
    ...
    // through the player for ever. world.js:12564-12793 is the shape.
    arrows.update(dt, {
    ...
    if (walkMode && !tpMode) {
      ...
      for (const ev of weaponRig.frame(dt, { paralyzed })) {
        ...
        if (!cityGuards.resolvePlayerHit(weaponRig.playerWeapon, eye, fwd, player.pos, makeInView(proj, view, multiply), guardHitSound)) {
          ...
          // BEFORE civilians - world.js:12903's order, and the order

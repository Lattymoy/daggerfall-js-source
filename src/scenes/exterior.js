...
export async function bootExterior(canvas, renderer, params, status) {
  ...
  // pool the mode machine holds, which is world.js:2186's line.
  ...
  const cityGuards = createCityGuards({
    ...
    // own note describes. Written in world.js:2285's shape, so one pin
   ...
   *  the other two exterior hosts mount - world.js:2226 over the street
   ...
   *  it (world.js:2283-2365) and is its own slice; this pool does not
   *  wait on it. */
  ...
  const outdoorRestDeps = createRestDeps(playerEntity, {
    ...
    advanceMinutes: (n) => { playerTicker.advance(n); runEncounterTick(walkMode ? player.pos : cam.pos); },   // ROAD-G TAIL: the catch-up loop rides the rest's minutes, as world.js:3172 has it
   ...
   *  (effects.js:881/:895) - world.js:2597 hoisted its copy for exactly
  ...
  const magic = createPlayerMagic({
    ...
    // two engine-owning hosts wire the identical pair (world.js:2531-2532,
  ...
  const questWorld = {
    ...
    // other. world.js:5841 is the same line.
     ...
     *  %rn/%rt's producer - world.js:5740-5753. */
    ...
    // QG1: CastSpellDo's two world reads, world.js:5656-5659's pair.
     ...
     *  mode-aware (worldModes:1258), so this is world.js:5629's line
     ...
     *  Bridge-only, no host state - world.js:5637's call verbatim. */
     ...
     *  Everything else is world.js:5741's arm term for term: the cast
  ...
  questBridge = createQuestBridge({
    ...
    // closure S40 gave this host, and world.js:6394's line.
    ...
    // encounter half is world.js:6474's line and the watch half is this
  ...
  // same order as world.js:6695/:6700, for the same reason: the
  ...
  // here, so the sink is handed down at this moment - world.js:6679's
  // line for this host.
  ...
  var modes = createWorldModes({
    ...
    buildingDirectory: () => ({
      ...
      // two inputs the world host uses (world.js:7142).
   ...
   *  second ctx here is the shape FS1 was written about. */
  {
     ...
     *  encounter mount above, and world.js:2779-2795 is the shape.
     ...
     *  world.js:2718-2719 does it. ROAD-G TAIL: a WATCHMAN transforms
  ...
  function frame(now) {
    ...
    // through the player for ever. world.js:8483-8603 is the shape.
    arrows.update(dt, {
    ...
    if (walkMode && !tpMode) {
      ...
      for (const ev of weaponRig.frame(dt, { paralyzed })) {
        ...
        if (!cityGuards.resolvePlayerHit(weaponRig.playerWeapon, eye, fwd, player.pos, makeInView(proj, view, multiply), guardHitSound)) {
          ...
          // BEFORE civilians - world.js:8672's order, and the order

...
import { spellRecordOfIndex } from '../systems/loot.js';   // QG1: CastSpellDo's classic-record read (the G4 registry) - world.js:228's import
...
export async function bootExterior(canvas, renderer, params, status) {
  ...
  //     continue;`, world.js:2347) and stands them in batches of their
  ...
  function onExhaustedExterior() {
    ...
    try {
      const out = exhaustionOutcome({
        ...
        enemiesNearby: areEnemiesNearby([...(cityGuards?.guards ?? []), ...(exteriorFoes?.foes ?? [])]),   // ROAD-G G2: BOTH street pools, world.js:3296's line
  ...
  // pool the mode machine holds, which is world.js:3905's line.
  ...
  const cityGuards = createCityGuards({
    ...
    // own note describes. Written in world.js:4240's shape, so one pin
   ...
   *  the other two exterior hosts mount - world.js:3966 over the street
   ...
   *  it (world.js:4238-4448) and is its own slice; this pool does not
   *  wait on it. */
  ...
  const outdoorRestDeps = createRestDeps(playerEntity, {
    ...
    advanceMinutes: (n, sharedEnd) => { playerTicker.advance(n); runEncounterTick(walkMode ? player.pos : cam.pos, sharedEnd, true); },   // CAMP1-REST: every tick this drives IS a rest   // ROAD-G TAIL: the catch-up loop rides the rest's minutes, as world.js:5660 has it   // RESTX2: sharedEnd is the session's local sim-minutes online, so the roll still gets a fresh `now` while the real clock stands
   ...
   *  (effects.js:891/:905) - world.js:4808 hoisted its copy for exactly
  ...
  const magic = createPlayerMagic({
    ...
    // two engine-owning hosts wire the identical pair (world.js:4756-4757,
    // dungeonContext.js:2294-2295); `questBridge` is assigned below this
  ...
  const makeJournalWindow = (mode) => {
    ...
    // gate could never run. dungeonContext.js:1498-1503 is the shape:
  ...
  const questWorld = {
    ...
    // other. world.js:9528 is the same line.
     ...
     *  %rn/%rt's producer - world.js:9424-9437. */
    ...
    // QG1: CastSpellDo's two world reads, world.js:9340-9343's pair.
     ...
     *  mode-aware (worldModes:1258), so this is world.js:9313's line
     ...
     *  Bridge-only, no host state - world.js:9321's call verbatim. */
     ...
     *  Everything else is world.js:9425's arm term for term: the cast
  ...
  questBridge = createQuestBridge({
    ...
    // closure S40 gave this host, and world.js:10088's line.
    ...
    // encounter half is world.js:10184's line and the watch half is this
  ...
  // same order as world.js:10398/:10403, for the same reason: the
  ...
  // here, so the sink is handed down at this moment - world.js:10382's
  // line for this host.
  ...
  var modes = createWorldModes({
    ...
    buildingDirectory: () => ({
      ...
      // two inputs the world host uses (world.js:13975).
   ...
   *  encounter mount above, and world.js:4988-5004 is the shape.
   ...
   *  second ctx here is the shape FS1 was written about. */
  {
     ...
     *  world.js:4935-4936 does it. ROAD-G TAIL: a WATCHMAN transforms
  ...
  function frame(now) {
    ...
    // through the player for ever. world.js:15907-16158 is the shape.
    arrows.update(dt, {
    ...
    if (walkMode && !tpMode) {
      ...
      for (const ev of weaponRig.frame(dt, { paralyzed })) {
        ...
        if (!cityGuards.resolvePlayerHit(weaponRig.playerWeapon, eye, fwd, player.pos, makeInView(proj, view, multiply), guardHitSound, { swing })) {
          ...
          // BEFORE civilians - world.js:16277's order, and the order

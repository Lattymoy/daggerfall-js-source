...
export async function bootExterior(canvas, renderer, params, status) {
  ...
  function onExhaustedExterior() {
    ...
    try {
      const out = exhaustionOutcome({
        ...
        enemiesNearby: areEnemiesNearby([...(cityGuards?.guards ?? []), ...(exteriorFoes?.foes ?? [])]),   // ROAD-G G2: BOTH street pools, world.js:1736's line
  ...
  const cityGuards = createCityGuards({
    ...
    // own note describes. Written in world.js:2425's shape, so one pin
   ...
   *  the other two exterior hosts mount - world.js:2308 over the street
   ...
   *  it (world.js:2423-2505) and is its own slice; this pool does not
   *  wait on it. */
  ...
  const outdoorRestDeps = createRestDeps(playerEntity, {
    ...
    advanceMinutes: (n, sharedEnd) => { playerTicker.advance(n); runEncounterTick(walkMode ? player.pos : cam.pos, sharedEnd); },   // ROAD-G TAIL: the catch-up loop rides the rest's minutes, as world.js:3482 has it   // RESTX2: sharedEnd is the session's local sim-minutes online, so the roll still gets a fresh `now` while the real clock stands
   ...
   *  (effects.js:881/:895) - world.js:2787 hoisted its copy for exactly
  ...
  const magic = createPlayerMagic({
    ...
    // two engine-owning hosts wire the identical pair (world.js:2700-2701,
    // dungeonContext.js:2066-2067); `questBridge` is assigned below this
  ...
  const makeJournalWindow = (mode) => {
    ...
    // gate could never run. dungeonContext.js:1301-1303 is the shape:
  ...
  const questWorld = {
    ...
    // other. world.js:6288 is the same line.
     ...
     *  %rn/%rt's producer - world.js:6187-6200. */
    ...
    // QG1: CastSpellDo's two world reads, world.js:6103-6106's pair.
     ...
     *  mode-aware (worldModes:1258), so this is world.js:6076's line
     ...
     *  Bridge-only, no host state - world.js:6084's call verbatim. */
     ...
     *  Everything else is world.js:6188's arm term for term: the cast
  ...
  questBridge = createQuestBridge({
    ...
    // closure S40 gave this host, and world.js:6841's line.
    ...
    // encounter half is world.js:6921's line and the watch half is this
  ...
  // same order as world.js:7142/:7147, for the same reason: the
  ...
  // here, so the sink is handed down at this moment - world.js:7126's
  // line for this host.
  ...
  var modes = createWorldModes({
    ...
    buildingDirectory: () => ({
      ...
      // two inputs the world host uses (world.js:7955).
   ...
   *  second ctx here is the shape FS1 was written about. */
  {
     ...
     *  encounter mount above, and world.js:2969-2985 is the shape.
     ...
     *  world.js:2908-2909 does it. ROAD-G TAIL: a WATCHMAN transforms
  ...
  function frame(now) {
    ...
    // through the player for ever. world.js:9362-9483 is the shape.
    arrows.update(dt, {
    ...
    if (walkMode && !tpMode) {
      ...
      for (const ev of weaponRig.frame(dt, { paralyzed })) {
        ...
        if (!cityGuards.resolvePlayerHit(weaponRig.playerWeapon, eye, fwd, player.pos, makeInView(proj, view, multiply), guardHitSound)) {
          ...
          // BEFORE civilians - world.js:9552's order, and the order

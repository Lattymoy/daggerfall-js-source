...
export async function bootExterior(canvas, renderer, params, status) {
  ...
  function onExhaustedExterior() {
    ...
    try {
      const out = exhaustionOutcome({
        ...
        enemiesNearby: areEnemiesNearby([...(cityGuards?.guards ?? []), ...(exteriorFoes?.foes ?? [])]),   // ROAD-G G2: BOTH street pools, world.js:1685's line
  ...
  // pool the mode machine holds, which is world.js:2141's line.
  ...
  const cityGuards = createCityGuards({
    ...
    // own note describes. Written in world.js:2240's shape, so one pin
   ...
   *  the other two exterior hosts mount - world.js:2181 over the street
   ...
   *  it (world.js:2238-2320) and is its own slice; this pool does not
   *  wait on it. */
  ...
  const outdoorRestDeps = createRestDeps(playerEntity, {
    ...
    advanceMinutes: (n) => { playerTicker.advance(n); runEncounterTick(walkMode ? player.pos : cam.pos); },   // ROAD-G TAIL: the catch-up loop rides the rest's minutes, as world.js:3122 has it
   ...
   *  (effects.js:881/:895) - world.js:2552 hoisted its copy for exactly
  ...
  const magic = createPlayerMagic({
    ...
    // two engine-owning hosts wire the identical pair (world.js:2486-2487,
  ...
  const questWorld = {
    ...
    // other. world.js:5736 is the same line.
     ...
     *  %rn/%rt's producer - world.js:5635-5648. */
    ...
    // QG1: CastSpellDo's two world reads, world.js:5551-5554's pair.
     ...
     *  mode-aware (worldModes:1258), so this is world.js:5524's line
     ...
     *  Bridge-only, no host state - world.js:5532's call verbatim. */
     ...
     *  Everything else is world.js:5636's arm term for term: the cast
  ...
  questBridge = createQuestBridge({
    ...
    // closure S40 gave this host, and world.js:6289's line.
    ...
    // encounter half is world.js:6369's line and the watch half is this
  ...
  // same order as world.js:6590/:6595, for the same reason: the
  ...
  // here, so the sink is handed down at this moment - world.js:6574's
  // line for this host.
  ...
  var modes = createWorldModes({
    ...
    buildingDirectory: () => ({
      ...
      // two inputs the world host uses (world.js:7013).
   ...
   *  second ctx here is the shape FS1 was written about. */
  {
     ...
     *  encounter mount above, and world.js:2734-2750 is the shape.
     ...
     *  world.js:2673-2674 does it. ROAD-G TAIL: a WATCHMAN transforms
  ...
  function frame(now) {
    ...
    // through the player for ever. world.js:8334-8454 is the shape.
    arrows.update(dt, {
    ...
    if (walkMode && !tpMode) {
      ...
      for (const ev of weaponRig.frame(dt, { paralyzed })) {
        ...
        if (!cityGuards.resolvePlayerHit(weaponRig.playerWeapon, eye, fwd, player.pos, makeInView(proj, view, multiply), guardHitSound)) {
          ...
          // BEFORE civilians - world.js:8523's order, and the order

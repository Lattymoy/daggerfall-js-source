...
import { spellRecordOfIndex } from '../systems/loot.js';   // QG1: CastSpellDo's classic-record read (the G4 registry) - world.js:198's import
...
export async function bootExterior(canvas, renderer, params, status) {
  ...
  //     continue;`, world.js:1721) and stands them in batches of their
  ...
  function onExhaustedExterior() {
    ...
    try {
      const out = exhaustionOutcome({
        ...
        enemiesNearby: areEnemiesNearby([...(cityGuards?.guards ?? []), ...(exteriorFoes?.foes ?? [])]),   // ROAD-G G2: BOTH street pools, world.js:2586's line
  ...
  // pool the mode machine holds, which is world.js:3155's line.
  ...
  const cityGuards = createCityGuards({
    ...
    // own note describes. Written in world.js:3480's shape, so one pin
   ...
   *  the other two exterior hosts mount - world.js:3216 over the street
   ...
   *  it (world.js:3478-3671) and is its own slice; this pool does not
   *  wait on it. */
  ...
  const outdoorRestDeps = createRestDeps(playerEntity, {
    ...
    advanceMinutes: (n, sharedEnd) => { playerTicker.advance(n); runEncounterTick(walkMode ? player.pos : cam.pos, sharedEnd, true); },   // CAMP1-REST: every tick this drives IS a rest   // ROAD-G TAIL: the catch-up loop rides the rest's minutes, as world.js:4824 has it   // RESTX2: sharedEnd is the session's local sim-minutes online, so the roll still gets a fresh `now` while the real clock stands
   ...
   *  (effects.js:887/:901) - world.js:4002 hoisted its copy for exactly
  ...
  const magic = createPlayerMagic({
    ...
    // machine's CastSpellDo / CastEffectDo latches (machine.js:844/:850;
    ...
    // two engine-owning hosts wire the identical pair (world.js:3950-3951,
  ...
  const makeInventoryWindow = (extra = {}) => createInventoryWindow({
    ...
    weaponPose: () => weaponPoseOf(weaponRig.playerWeapon),   // JAN1: the pose is the player's - a door in or out hands the pair through these
  ...
  const questWorld = {
    ...
    // other. world.js:8367 is the same line.
     ...
     *  %rn/%rt's producer - world.js:8266-8279. */
    ...
    // QG1: CastSpellDo's two world reads, world.js:8182-8185's pair.
     ...
     *  mode-aware (worldModes:1258), so this is world.js:8155's line
     ...
     *  Bridge-only, no host state - world.js:8163's call verbatim. */
     ...
     *  Everything else is world.js:8267's arm term for term: the cast
  ...
  questBridge = createQuestBridge({
    ...
    // closure S40 gave this host, and world.js:8926's line.
    ...
    // encounter half is world.js:9017's line and the watch half is this
  ...
  // same order as world.js:9232/:9237, for the same reason: the
  ...
  // here, so the sink is handed down at this moment - world.js:9216's
  // line for this host.
  ...
  var modes = createWorldModes({
    ...
    buildingDirectory: () => ({
      ...
      // two inputs the world host uses (world.js:11346).
   ...
   *  second ctx here is the shape FS1 was written about. */
  {
     ...
     *  encounter mount above, and world.js:4192-4208 is the shape.
     ...
     *  world.js:4122-4123 does it. ROAD-G TAIL: a WATCHMAN transforms
  ...
  function frame(now) {
    ...
    if (walkMode) {
      ...
      if ((_act.activate || useEdge) && !modes.transitioning) {
        ...
        if (_lockFoe) lockOn.toggle(_lockFoe);
          ...
          else if (_race.horseCartWins) { hcc.activate(_hccPick.key, _hccPick.distance, (l) => townTalk.say(l), () => setMidScreenText(TOO_FAR_AWAY_TEXT)); }   // HCC: the runtime's own reach test and refusals
    ...
    // through the player for ever. world.js:13090-13329 is the shape.
    arrows.update(dt, {
    ...
    if (walkMode && !tpMode) {
      ...
      for (const ev of weaponRig.frame(dt, { paralyzed })) {
        ...
        if (!cityGuards.resolvePlayerHit(weaponRig.playerWeapon, eye, fwd, player.pos, makeInView(proj, view, multiply), guardHitSound)) {
          ...
          // BEFORE civilians - world.js:13439's order, and the order

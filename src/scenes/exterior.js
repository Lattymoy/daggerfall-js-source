...
export async function bootExterior(canvas, renderer, params, status) {
  ...
  //     continue;`, world.js:1371) and stands them in batches of their
  ...
  function onExhaustedExterior() {
    ...
    try {
      const out = exhaustionOutcome({
        ...
        enemiesNearby: areEnemiesNearby([...(cityGuards?.guards ?? []), ...(exteriorFoes?.foes ?? [])]),   // ROAD-G G2: BOTH street pools, world.js:2086's line
  ...
  // pool the mode machine holds, which is world.js:2654's line.
  ...
  const cityGuards = createCityGuards({
    ...
    // own note describes. Written in world.js:2885's shape, so one pin
   ...
   *  the other two exterior hosts mount - world.js:2715 over the street
   ...
   *  it (world.js:2883-3075) and is its own slice; this pool does not
   *  wait on it. */
   ...
   *  (effects.js:881/:895) - world.js:3406 hoisted its copy for exactly
  ...
  const magic = createPlayerMagic({
    ...
    // two engine-owning hosts wire the identical pair (world.js:3354-3355,
  ...
  const questWorld = {
    ...
    // other. world.js:7720 is the same line.
     ...
     *  %rn/%rt's producer - world.js:7619-7632. */
    ...
    // QG1: CastSpellDo's two world reads, world.js:7535-7538's pair.
     ...
     *  mode-aware (worldModes:1258), so this is world.js:7508's line
     ...
     *  Bridge-only, no host state - world.js:7516's call verbatim. */
     ...
     *  Everything else is world.js:7620's arm term for term: the cast
  ...
  questBridge = createQuestBridge({
    ...
    // closure S40 gave this host, and world.js:8279's line.
    ...
    // encounter half is world.js:8370's line and the watch half is this
  ...
  // same order as world.js:8585/:8590, for the same reason: the
  ...
  // here, so the sink is handed down at this moment - world.js:8569's
  // line for this host.
  ...
  var modes = createWorldModes({
    ...
    buildingDirectory: () => ({
      ...
      // two inputs the world host uses (world.js:10516).
   ...
   *  second ctx here is the shape FS1 was written about. */
  {
     ...
     *  encounter mount above, and world.js:3588-3604 is the shape.
     ...
     *  world.js:3518-3519 does it. ROAD-G TAIL: a WATCHMAN transforms
  ...
  function frame(now) {
    ...
    // through the player for ever. world.js:12195-12395 is the shape.
    arrows.update(dt, {
    ...
    if (walkMode && !tpMode) {
      ...
      for (const ev of weaponRig.frame(dt, { paralyzed })) {
        ...
        if (!cityGuards.resolvePlayerHit(weaponRig.playerWeapon, eye, fwd, player.pos, makeInView(proj, view, multiply), guardHitSound)) {
          ...
          // BEFORE civilians - world.js:12502's order, and the order

...
import { spellRecordOfIndex } from '../systems/loot.js';   // QG1: CastSpellDo's classic-record read (the G4 registry) - world.js:116's import
...
export async function bootExterior(canvas, renderer, params, status) {
  ...
  //     continue;`, world.js:970) and stands them in batches of their
  ...
  function onExhaustedExterior() {
    ...
    try {
      const out = exhaustionOutcome({
        ...
        enemiesNearby: areEnemiesNearby([...(cityGuards?.guards ?? []), ...(exteriorFoes?.foes ?? [])]),   // ROAD-G G2: BOTH street pools, world.js:1561's line
  ...
  // pool the mode machine holds, which is world.js:2015's line.
  ...
  const cityGuards = createCityGuards({
    ...
    // own note describes. Written in world.js:2103's shape, so one pin
   ...
   *  the other two exterior hosts mount - world.js:2055 over the street
   ...
   *  it (world.js:2101-2185) and is its own slice; this pool does not
   *  wait on it. */
  ...
  const outdoorRestDeps = createRestDeps(playerEntity, {
    ...
    advanceMinutes: (n) => { playerTicker.advance(n); runEncounterTick(walkMode ? player.pos : cam.pos); },   // ROAD-G TAIL: the catch-up loop rides the rest's minutes, as world.js:2985 has it
   ...
   *  (effects.js:881/:895) - world.js:2243 hoisted its copy for exactly
  ...
  const magic = createPlayerMagic({
    ...
    // (hostMagic.js:74-75), which are the ONLY route into the quest
    ...
    // two engine-owning hosts wire the identical pair (world.js:2351-2352,
  ...
  const makeJournalWindow = (mode) => {
    ...
    // gate could never run. dungeonContext.js:1246-1248 is the shape:
  ...
  const questWorld = {
    ...
    // other. world.js:5425 is the same line.
     ...
     *  %rn/%rt's producer - world.js:5427-5440. */
    ...
    // QG1: CastSpellDo's two world reads, world.js:5343-5346's pair.
     ...
     *  mode-aware (worldModes:1258), so this is world.js:5316's line
     ...
     *  Bridge-only, no host state - world.js:5324's call verbatim. */
     ...
     *  Everything else is world.js:5428's arm term for term: the cast
  ...
  questBridge = createQuestBridge({
    ...
    // closure S40 gave this host, and world.js:6081's line.
    ...
    // encounter half is world.js:6003's line and the watch half is this
  ...
  // here, so the sink is handed down at this moment - world.js:6365's
  // line for this host.
  ...
  var modes = createWorldModes({
    ...
    buildingDirectory: () => ({
      ...
      // two inputs the world host uses (world.js:6478).
   ...
   *  second ctx here is the shape FS1 was written about. */
  {
     ...
     *  encounter mount above, and world.js:2597-2613 is the shape.
     ...
     *  world.js:2539-2540 does it. ROAD-G TAIL: a WATCHMAN transforms
  ...
  function frame(now) {
    ...
    // through the player for ever. world.js:7744-7808 is the shape.
    arrows.update(dt, {
    ...
    if (walkMode && !tpMode) {
      ...
      for (const ev of weaponRig.frame(dt, { paralyzed })) {
        ...
        if (!cityGuards.resolvePlayerHit(weaponRig.playerWeapon, eye, fwd, player.pos, makeInView(proj, view, multiply), guardHitSound)) {
          ...
          // BEFORE civilians - world.js:7868's order, and the order

...
export async function bootExterior(canvas, renderer, params, status) {
  ...
  const questWorld = {
    ...
    // other. world.js:6888 is the same line.
     ...
     *  %rn/%rt's producer - world.js:6787-6800. */
    ...
    // QG1: CastSpellDo's two world reads, world.js:6703-6706's pair.
     ...
     *  mode-aware (worldModes:1258), so this is world.js:6676's line
     ...
     *  Bridge-only, no host state - world.js:6684's call verbatim. */
     ...
     *  Everything else is world.js:6788's arm term for term: the cast
  ...
  questBridge = createQuestBridge({
    ...
    // closure S40 gave this host, and world.js:7441's line.
    ...
    // encounter half is world.js:7521's line and the watch half is this
  ...
  // same order as world.js:7742/:7747, for the same reason: the
  ...
  // here, so the sink is handed down at this moment - world.js:7726's
  // line for this host.
  ...
  var modes = createWorldModes({
    ...
    buildingDirectory: () => ({
      ...
      // two inputs the world host uses (world.js:8588).
  ...
  function frame(now) {
    ...
    // through the player for ever. world.js:10135-10256 is the shape.
    arrows.update(dt, {
    ...
    if (walkMode && !tpMode) {
      ...
      for (const ev of weaponRig.frame(dt, { paralyzed })) {
        ...
        if (!cityGuards.resolvePlayerHit(weaponRig.playerWeapon, eye, fwd, player.pos, makeInView(proj, view, multiply), guardHitSound)) {
          ...
          // BEFORE civilians - world.js:10329's order, and the order

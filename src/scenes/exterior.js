...
export async function bootExterior(canvas, renderer, params, status) {
  ...
  const outdoorRestDeps = createRestDeps(playerEntity, {
    ...
    advanceMinutes: (n) => { playerTicker.advance(n); runEncounterTick(walkMode ? player.pos : cam.pos); },   // ROAD-G TAIL: the catch-up loop rides the rest's minutes, as world.js:3198 has it
   ...
   *  (effects.js:881/:895) - world.js:2623 hoisted its copy for exactly
  ...
  const questWorld = {
    ...
    // other. world.js:5881 is the same line.
     ...
     *  %rn/%rt's producer - world.js:5780-5793. */
    ...
    // QG1: CastSpellDo's two world reads, world.js:5696-5699's pair.
     ...
     *  mode-aware (worldModes:1258), so this is world.js:5669's line
     ...
     *  Bridge-only, no host state - world.js:5677's call verbatim. */
     ...
     *  Everything else is world.js:5781's arm term for term: the cast
  ...
  questBridge = createQuestBridge({
    ...
    // closure S40 gave this host, and world.js:6434's line.
    ...
    // encounter half is world.js:6514's line and the watch half is this
  ...
  // same order as world.js:6735/:6740, for the same reason: the
  ...
  // here, so the sink is handed down at this moment - world.js:6719's
  // line for this host.
  ...
  var modes = createWorldModes({
    ...
    buildingDirectory: () => ({
      ...
      // two inputs the world host uses (world.js:7244).
   ...
   *  second ctx here is the shape FS1 was written about. */
  {
     ...
     *  encounter mount above, and world.js:2805-2821 is the shape.
     ...
     *  world.js:2744-2745 does it. ROAD-G TAIL: a WATCHMAN transforms
  ...
  function frame(now) {
    ...
    // through the player for ever. world.js:8600-8720 is the shape.
    arrows.update(dt, {
    ...
    if (walkMode && !tpMode) {
      ...
      for (const ev of weaponRig.frame(dt, { paralyzed })) {
        ...
        if (!cityGuards.resolvePlayerHit(weaponRig.playerWeapon, eye, fwd, player.pos, makeInView(proj, view, multiply), guardHitSound)) {
          ...
          // BEFORE civilians - world.js:8789's order, and the order

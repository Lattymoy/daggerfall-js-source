...
import { spellRecordOfIndex } from '../systems/loot.js';   // QG1: CastSpellDo's classic-record read (the G4 registry) - world.js:186's import
...
export async function bootExterior(canvas, renderer, params, status) {
  ...
  // pool the mode machine holds, which is world.js:2670's line.
  ...
  const cityGuards = createCityGuards({
    ...
    // own note describes. Written in world.js:3016's shape, so one pin
   ...
   *  the other two exterior hosts mount - world.js:2731 over the street
   ...
   *  it (world.js:3014-3096) and is its own slice; this pool does not
   *  wait on it. */
  ...
  const outdoorRestDeps = createRestDeps(playerEntity, {
    ...
    advanceMinutes: (n, sharedEnd) => { playerTicker.advance(n); runEncounterTick(walkMode ? player.pos : cam.pos, sharedEnd, true); },   // CAMP1-REST: every tick this drives IS a rest   // ROAD-G TAIL: the catch-up loop rides the rest's minutes, as world.js:4167 has it   // RESTX2: sharedEnd is the session's local sim-minutes online, so the roll still gets a fresh `now` while the real clock stands
   ...
   *  (effects.js:881/:895) - world.js:3427 hoisted its copy for exactly
  ...
  const magic = createPlayerMagic({
    ...
    // two engine-owning hosts wire the identical pair (world.js:3328-3329,
    // dungeonContext.js:2181-2182); `questBridge` is assigned below this
  ...
  const makeJournalWindow = (mode) => {
    ...
    // gate could never run. dungeonContext.js:1427-1432 is the shape:
  ...
  const questWorld = {
    ...
    // other. world.js:7546 is the same line.
     ...
     *  %rn/%rt's producer - world.js:7445-7458. */
    ...
    // QG1: CastSpellDo's two world reads, world.js:7361-7364's pair.
     ...
     *  mode-aware (worldModes:1258), so this is world.js:7334's line
     ...
     *  Bridge-only, no host state - world.js:7342's call verbatim. */
     ...
     *  Everything else is world.js:7446's arm term for term: the cast
  ...
  questBridge = createQuestBridge({
    ...
    // closure S40 gave this host, and world.js:8100's line.
    ...
    // encounter half is world.js:8180's line and the watch half is this
  ...
  // same order as world.js:8401/:8406, for the same reason: the
  ...
  // here, so the sink is handed down at this moment - world.js:8385's
  // line for this host.
  ...
  var modes = createWorldModes({
    ...
    buildingDirectory: () => ({
      ...
      // two inputs the world host uses (world.js:9307).
   ...
   *  second ctx here is the shape FS1 was written about. */
  {
     ...
     *  encounter mount above, and world.js:3609-3625 is the shape.
     ...
     *  world.js:3539-3540 does it. ROAD-G TAIL: a WATCHMAN transforms
  ...
  function frame(now) {
    ...
    // through the player for ever. world.js:10949-11149 is the shape.
    arrows.update(dt, {
    ...
    if (walkMode && !tpMode) {
      ...
      for (const ev of weaponRig.frame(dt, { paralyzed })) {
        ...
        if (!cityGuards.resolvePlayerHit(weaponRig.playerWeapon, eye, fwd, player.pos, makeInView(proj, view, multiply), guardHitSound)) {
          ...
          // BEFORE civilians - world.js:11256's order, and the order

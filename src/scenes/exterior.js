...
export async function bootExterior(canvas, renderer, params, status) {
  ...
  const outdoorRestDeps = createRestDeps(playerEntity, {
    ...
    advanceMinutes: (n) => { playerTicker.advance(n); runEncounterTick(walkMode ? player.pos : cam.pos); },   // ROAD-G TAIL: the catch-up loop rides the rest's minutes, as world.js:3183 has it
   ...
   *  (effects.js:881/:895) - world.js:2608 hoisted its copy for exactly
  ...
  const questWorld = {
    ...
    // other. world.js:5859 is the same line.
     ...
     *  %rn/%rt's producer - world.js:5758-5771. */
    ...
    // QG1: CastSpellDo's two world reads, world.js:5674-5677's pair.
     ...
     *  mode-aware (worldModes:1258), so this is world.js:5647's line
     ...
     *  Bridge-only, no host state - world.js:5655's call verbatim. */
     ...
     *  Everything else is world.js:5759's arm term for term: the cast
  ...
  questBridge = createQuestBridge({
    ...
    // closure S40 gave this host, and world.js:6412's line.
    ...
    // encounter half is world.js:6492's line and the watch half is this
  ...
  // same order as world.js:6713/:6718, for the same reason: the
  ...
  // here, so the sink is handed down at this moment - world.js:6697's
  // line for this host.
  ...
  var modes = createWorldModes({
    ...
    buildingDirectory: () => ({
      ...
      // two inputs the world host uses (world.js:7427).
   ...
   *  second ctx here is the shape FS1 was written about. */
  {
     ...
     *  encounter mount above, and world.js:2790-2806 is the shape.
     ...
     *  world.js:2729-2730 does it. ROAD-G TAIL: a WATCHMAN transforms
  ...
  function frame(now) {
    ...
    // through the player for ever. world.js:8783-8903 is the shape.
    arrows.update(dt, {
    ...
    if (walkMode && !tpMode) {
      ...
      for (const ev of weaponRig.frame(dt, { paralyzed })) {
        ...
        if (!cityGuards.resolvePlayerHit(weaponRig.playerWeapon, eye, fwd, player.pos, makeInView(proj, view, multiply), guardHitSound)) {
          ...
          // BEFORE civilians - world.js:8972's order, and the order

...
export async function bootExterior(canvas, renderer, params, status) {
  ...
  function onExhaustedExterior() {
    ...
    try {
      const out = exhaustionOutcome({
        ...
        enemiesNearby: areEnemiesNearby([...(cityGuards?.guards ?? []), ...(exteriorFoes?.foes ?? [])]),   // ROAD-G G2: BOTH street pools, world.js:1907's line
  ...
  // pool the mode machine holds, which is world.js:2474's line.
  ...
  const cityGuards = createCityGuards({
    ...
    // own note describes. Written in world.js:2652's shape, so one pin
   ...
   *  the other two exterior hosts mount - world.js:2535 over the street
   ...
   *  it (world.js:2650-2732) and is its own slice; this pool does not
   *  wait on it. */
  ...
  const outdoorRestDeps = createRestDeps(playerEntity, {
    ...
    advanceMinutes: (n, sharedEnd) => { playerTicker.advance(n); runEncounterTick(walkMode ? player.pos : cam.pos, sharedEnd, true); },   // CAMP1-REST: every tick this drives IS a rest   // ROAD-G TAIL: the catch-up loop rides the rest's minutes, as world.js:3781 has it   // RESTX2: sharedEnd is the session's local sim-minutes online, so the roll still gets a fresh `now` while the real clock stands
   ...
   *  (effects.js:881/:895) - world.js:3045 hoisted its copy for exactly
  ...
  const magic = createPlayerMagic({
    ...
    // two engine-owning hosts wire the identical pair (world.js:2958-2959,
  ...
  const questWorld = {
    ...
    // other. world.js:6977 is the same line.
     ...
     *  %rn/%rt's producer - world.js:6876-6889. */
    ...
    // QG1: CastSpellDo's two world reads, world.js:6792-6795's pair.
     ...
     *  mode-aware (worldModes:1258), so this is world.js:6765's line
     ...
     *  Bridge-only, no host state - world.js:6773's call verbatim. */
     ...
     *  Everything else is world.js:6877's arm term for term: the cast
  ...
  questBridge = createQuestBridge({
    ...
    // closure S40 gave this host, and world.js:7530's line.
    ...
    // encounter half is world.js:7610's line and the watch half is this
  ...
  // same order as world.js:7831/:7836, for the same reason: the
  ...
  // here, so the sink is handed down at this moment - world.js:7815's
  // line for this host.
  ...
  var modes = createWorldModes({
    ...
    buildingDirectory: () => ({
      ...
      // two inputs the world host uses (world.js:8682).
   ...
   *  second ctx here is the shape FS1 was written about. */
  {
     ...
     *  encounter mount above, and world.js:3227-3243 is the shape.
     ...
     *  world.js:3157-3158 does it. ROAD-G TAIL: a WATCHMAN transforms
  ...
  function frame(now) {
    ...
    // through the player for ever. world.js:10224-10345 is the shape.
    arrows.update(dt, {
    ...
    if (walkMode && !tpMode) {
      ...
      for (const ev of weaponRig.frame(dt, { paralyzed })) {
        ...
        if (!cityGuards.resolvePlayerHit(weaponRig.playerWeapon, eye, fwd, player.pos, makeInView(proj, view, multiply), guardHitSound)) {
          ...
          // BEFORE civilians - world.js:10439's order, and the order

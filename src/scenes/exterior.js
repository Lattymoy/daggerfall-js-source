...
export async function bootExterior(canvas, renderer, params, status) {
  ...
  //     continue;`, world.js:2566) and stands them in batches of their
  ...
  function onExhaustedExterior() {
    ...
    try {
      const out = exhaustionOutcome({
        ...
        enemiesNearby: areEnemiesNearby([...(cityGuards?.guards ?? []), ...(exteriorFoes?.foes ?? [])]),   // ROAD-G G2: BOTH street pools, world.js:3520's line
  ...
  // pool the mode machine holds, which is world.js:4129's line.
  ...
  const cityGuards = createCityGuards({
    ...
    // own note describes. Written in world.js:4464's shape, so one pin
   ...
   *  the other two exterior hosts mount - world.js:4190 over the street
   ...
   *  it (world.js:4462-4672) and is its own slice; this pool does not
   *  wait on it. */
  ...
  const outdoorRestDeps = createRestDeps(playerEntity, {
    ...
    advanceMinutes: (n, sharedEnd) => { playerTicker.advance(n); runEncounterTick(walkMode ? player.pos : cam.pos, sharedEnd, true); },   // CAMP1-REST: every tick this drives IS a rest   // ROAD-G TAIL: the catch-up loop rides the rest's minutes, as world.js:5884 has it   // RESTX2: sharedEnd is the session's local sim-minutes online, so the roll still gets a fresh `now` while the real clock stands
   ...
   *  (effects.js:891/:905) - world.js:5032 hoisted its copy for exactly
  ...
  const magic = createPlayerMagic({
    ...
    // two engine-owning hosts wire the identical pair (world.js:4980-4981,
  ...
  const questWorld = {
    ...
    // other. world.js:9752 is the same line.
     ...
     *  %rn/%rt's producer - world.js:9648-9661. */
    ...
    // QG1: CastSpellDo's two world reads, world.js:9564-9567's pair.
     ...
     *  mode-aware (worldModes:1258), so this is world.js:9537's line
     ...
     *  Bridge-only, no host state - world.js:9545's call verbatim. */
     ...
     *  Everything else is world.js:9649's arm term for term: the cast
  ...
  questBridge = createQuestBridge({
    ...
    // closure S40 gave this host, and world.js:10312's line.
    ...
    // encounter half is world.js:10408's line and the watch half is this
  ...
  // same order as world.js:10622/:10627, for the same reason: the
  ...
  // here, so the sink is handed down at this moment - world.js:10606's
  // line for this host.
  ...
  var modes = createWorldModes({
    ...
    buildingDirectory: () => ({
      ...
      // two inputs the world host uses (world.js:14001).
   ...
   *  encounter mount above, and world.js:5212-5228 is the shape.
   ...
   *  second ctx here is the shape FS1 was written about. */
  {
     ...
     *  world.js:5159-5160 does it. ROAD-G TAIL: a WATCHMAN transforms
  ...
  function frame(now) {
    ...
    // through the player for ever. world.js:15963-16214 is the shape.
    arrows.update(dt, {
    ...
    if (walkMode && !tpMode) {
      ...
      for (const ev of weaponRig.frame(dt, { paralyzed })) {
        ...
        if (!cityGuards.resolvePlayerHit(weaponRig.playerWeapon, eye, fwd, player.pos, makeInView(proj, view, multiply), guardHitSound, { swing })) {
          ...
          // BEFORE civilians - world.js:16334's order, and the order

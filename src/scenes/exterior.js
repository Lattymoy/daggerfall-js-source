...
import { spellRecordOfIndex } from '../systems/loot.js';   // QG1: CastSpellDo's classic-record read (the G4 registry) - world.js:162's import
...
export async function bootExterior(canvas, renderer, params, status) {
  ...
  //     continue;`, world.js:1204) and stands them in batches of their
  ...
  function onExhaustedExterior() {
    ...
    try {
      const out = exhaustionOutcome({
        ...
        enemiesNearby: areEnemiesNearby([...(cityGuards?.guards ?? []), ...(exteriorFoes?.foes ?? [])]),   // ROAD-G G2: BOTH street pools, world.js:1892's line
  ...
  // pool the mode machine holds, which is world.js:2456's line.
  ...
  const cityGuards = createCityGuards({
    ...
    // own note describes. Written in world.js:2634's shape, so one pin
   ...
   *  the other two exterior hosts mount - world.js:2517 over the street
   ...
   *  it (world.js:2632-2714) and is its own slice; this pool does not
   *  wait on it. */
  ...
  const outdoorRestDeps = createRestDeps(playerEntity, {
    ...
    advanceMinutes: (n, sharedEnd) => { playerTicker.advance(n); runEncounterTick(walkMode ? player.pos : cam.pos, sharedEnd, true); },   // CAMP1-REST: every tick this drives IS a rest   // ROAD-G TAIL: the catch-up loop rides the rest's minutes, as world.js:3761 has it   // RESTX2: sharedEnd is the session's local sim-minutes online, so the roll still gets a fresh `now` while the real clock stands
   ...
   *  (effects.js:881/:895) - world.js:3025 hoisted its copy for exactly
  ...
  const magic = createPlayerMagic({
    ...
    // two engine-owning hosts wire the identical pair (world.js:2938-2939,
  ...
  const hudCtx = {
    ...
    togglePause: (doorOpts = {}) => {
      ...
      openPauseFlow((w) => townTalk.showOverlay(w), {
        ...
        relock: () => requestLook(canvas),   // MAC1 J: the pointer comes back with the resume gesture (ui/pauseDoor.js:207-224)
  ...
  const questWorld = {
    ...
    // other. world.js:6957 is the same line.
     ...
     *  %rn/%rt's producer - world.js:6856-6869. */
    ...
    // QG1: CastSpellDo's two world reads, world.js:6772-6775's pair.
     ...
     *  mode-aware (worldModes:1258), so this is world.js:6745's line
     ...
     *  Bridge-only, no host state - world.js:6753's call verbatim. */
     ...
     *  Everything else is world.js:6857's arm term for term: the cast
  ...
  questBridge = createQuestBridge({
    ...
    // closure S40 gave this host, and world.js:7510's line.
    ...
    // encounter half is world.js:7590's line and the watch half is this
  ...
  // same order as world.js:7811/:7816, for the same reason: the
  ...
  // here, so the sink is handed down at this moment - world.js:7795's
  // line for this host.
  ...
  var modes = createWorldModes({
    ...
    // double-click MAC1 J closed for ?world (ui/pauseDoor.js:207-224).
    ...
    buildingDirectory: () => ({
      ...
      // two inputs the world host uses (world.js:8657).
   ...
   *  second ctx here is the shape FS1 was written about. */
  {
     ...
     *  encounter mount above, and world.js:3207-3223 is the shape.
     ...
     *  world.js:3137-3138 does it. ROAD-G TAIL: a WATCHMAN transforms
  ...
  function frame(now) {
    ...
    // through the player for ever. world.js:10199-10320 is the shape.
    arrows.update(dt, {
    ...
    if (walkMode && !tpMode) {
      ...
      for (const ev of weaponRig.frame(dt, { paralyzed })) {
        ...
        if (!cityGuards.resolvePlayerHit(weaponRig.playerWeapon, eye, fwd, player.pos, makeInView(proj, view, multiply), guardHitSound)) {
          ...
          // BEFORE civilians - world.js:10414's order, and the order

...
export async function bootExterior(canvas, renderer, params, status) {
  ...
  //     continue;`, world.js:868) and stands them in batches of their
  ...
  function onExhaustedExterior() {
    ...
    try {
      const out = exhaustionOutcome({
        ...
        enemiesNearby: areEnemiesNearby([...(cityGuards?.guards ?? []), ...(exteriorFoes?.foes ?? [])]),   // ROAD-G G2: BOTH street pools, world.js:1483's line
  ...
  // pool the mode machine holds, which is world.js:1888's line.
   ...
   *  the other two exterior hosts mount - world.js:1928 over the street
   ...
   *  it (world.js:1974-2058) and is its own slice; this pool does not
   *  wait on it. */
  ...
  const outdoorRestDeps = createRestDeps(playerEntity, {
    ...
    advanceMinutes: (n) => { playerTicker.advance(n); runEncounterTick(walkMode ? player.pos : cam.pos); },   // ROAD-G TAIL: the catch-up loop rides the rest's minutes, as world.js:2829 has it
   ...
   *  (effects.js:828/:842) - world.js:2113 hoisted its copy for exactly
  ...
  const magic = createPlayerMagic({
    ...
    // two engine-owning hosts wire the identical pair (world.js:2200-2201,
  ...
  const questWorld = {
    ...
    // other. world.js:5010 is the same line.
     ...
     *  %rn/%rt's producer - world.js:5012-5025. */
    ...
    // QG1: CastSpellDo's two world reads, world.js:4928-4931's pair.
     ...
     *  mode-aware (worldModes:1258), so this is world.js:4901's line
     ...
     *  Bridge-only, no host state - world.js:4909's call verbatim. */
     ...
     *  Everything else is world.js:5013's arm term for term: the cast
   ...
   *  second ctx here is the shape FS1 was written about. */
  {
     ...
     *  encounter mount above, and world.js:2446-2462 is the shape.
     ...
     *  world.js:2388-2389 does it. ROAD-G TAIL: a WATCHMAN transforms
  ...
  function frame(now) {
    ...
    if (walkMode) {
      ...
      if ((_act.activate || (useHeld && !latch.use)) && !modes.transitioning) {
        ...
        if (_tapDir && tapLock(_tapOrigin ?? cam.pos, useFwd, exteriorFoePool(), collider)) { /* TI1c: the tap was the lock */ }
    ...
    // through the player for ever. world.js:7129-7181 is the shape.
    arrows.update(dt, {
    ...
    if (walkMode && !tpMode) {
      ...
      for (const ev of weaponRig.frame(dt, { paralyzed })) {
        ...
        if (!cityGuards.resolvePlayerHit(weaponRig.playerWeapon, eye, fwd, player.pos, makeInView(proj, view, multiply), guardHitSound)) {
          ...
          // BEFORE civilians - world.js:7239's order, and the order

...
export async function bootExterior(canvas, renderer, params, status) {
  ...
  const cityGuards = createCityGuards({
    ...
    // own note describes. Written in world.js:3534's shape, so one pin
   ...
   *  it (world.js:3532-3735) and is its own slice; this pool does not
   *  wait on it. */
  ...
  const outdoorRestDeps = createRestDeps(playerEntity, {
    ...
    advanceMinutes: (n, sharedEnd) => { playerTicker.advance(n); runEncounterTick(walkMode ? player.pos : cam.pos, sharedEnd, true); },   // CAMP1-REST: every tick this drives IS a rest   // ROAD-G TAIL: the catch-up loop rides the rest's minutes, as world.js:4885 has it   // RESTX2: sharedEnd is the session's local sim-minutes online, so the roll still gets a fresh `now` while the real clock stands
   ...
   *  (effects.js:887/:901) - world.js:4066 hoisted its copy for exactly
  ...
  const magic = createPlayerMagic({
    ...
    // two engine-owning hosts wire the identical pair (world.js:4014-4015,
  ...
  const questWorld = {
    ...
    // other. world.js:8450 is the same line.
     ...
     *  %rn/%rt's producer - world.js:8349-8362. */
    ...
    // QG1: CastSpellDo's two world reads, world.js:8265-8268's pair.
     ...
     *  mode-aware (worldModes:1258), so this is world.js:8238's line
     ...
     *  Bridge-only, no host state - world.js:8246's call verbatim. */
     ...
     *  Everything else is world.js:8350's arm term for term: the cast
  ...
  questBridge = createQuestBridge({
    ...
    // closure S40 gave this host, and world.js:9009's line.
    ...
    // encounter half is world.js:9100's line and the watch half is this
  ...
  // same order as world.js:9315/:9320, for the same reason: the
  ...
  // here, so the sink is handed down at this moment - world.js:9299's
  // line for this host.
  ...
  var modes = createWorldModes({
    ...
    buildingDirectory: () => ({
      ...
      // two inputs the world host uses (world.js:11475).
   ...
   *  second ctx here is the shape FS1 was written about. */
  {
     ...
     *  encounter mount above, and world.js:4256-4272 is the shape.
     ...
     *  world.js:4186-4187 does it. ROAD-G TAIL: a WATCHMAN transforms
  ...
  function frame(now) {
    ...
    // through the player for ever. world.js:13236-13475 is the shape.
    arrows.update(dt, {
    ...
    if (walkMode && !tpMode) {
      ...
      for (const ev of weaponRig.frame(dt, { paralyzed })) {
        ...
        if (!cityGuards.resolvePlayerHit(weaponRig.playerWeapon, eye, fwd, player.pos, makeInView(proj, view, multiply), guardHitSound)) {
          ...
          // BEFORE civilians - world.js:13585's order, and the order

...
import { spellRecordOfIndex } from '../systems/loot.js';   // QG1: CastSpellDo's classic-record read (the G4 registry) - world.js:202's import
...
export async function bootExterior(canvas, renderer, params, status) {
  ...
  //     continue;`, world.js:1793) and stands them in batches of their
  ...
  function onExhaustedExterior() {
    ...
    try {
      const out = exhaustionOutcome({
        ...
        enemiesNearby: areEnemiesNearby([...(cityGuards?.guards ?? []), ...(exteriorFoes?.foes ?? [])]),   // ROAD-G G2: BOTH street pools, world.js:2662's line
  ...
  // pool the mode machine holds, which is world.js:3240's line.
  ...
  const cityGuards = createCityGuards({
    ...
    // own note describes. Written in world.js:3574's shape, so one pin
   ...
   *  the other two exterior hosts mount - world.js:3301 over the street
   ...
   *  it (world.js:3572-3776) and is its own slice; this pool does not
   *  wait on it. */
  ...
  const outdoorRestDeps = createRestDeps(playerEntity, {
    ...
    advanceMinutes: (n, sharedEnd) => { playerTicker.advance(n); runEncounterTick(walkMode ? player.pos : cam.pos, sharedEnd, true); },   // CAMP1-REST: every tick this drives IS a rest   // ROAD-G TAIL: the catch-up loop rides the rest's minutes, as world.js:4947 has it   // RESTX2: sharedEnd is the session's local sim-minutes online, so the roll still gets a fresh `now` while the real clock stands
   ...
   *  (effects.js:887/:901) - world.js:4127 hoisted its copy for exactly
  ...
  const magic = createPlayerMagic({
    ...
    // two engine-owning hosts wire the identical pair (world.js:4075-4076,
    // dungeonContext.js:2230-2231); `questBridge` is assigned below this
  ...
  const makeJournalWindow = (mode) => {
    ...
    // gate could never run. dungeonContext.js:1445-1450 is the shape:
  ...
  const questWorld = {
    ...
    // other. world.js:8540 is the same line.
     ...
     *  %rn/%rt's producer - world.js:8439-8452. */
    ...
    // QG1: CastSpellDo's two world reads, world.js:8355-8358's pair.
     ...
     *  mode-aware (worldModes:1258), so this is world.js:8328's line
     ...
     *  Bridge-only, no host state - world.js:8336's call verbatim. */
     ...
     *  Everything else is world.js:8440's arm term for term: the cast
  ...
  questBridge = createQuestBridge({
    ...
    // closure S40 gave this host, and world.js:9099's line.
    ...
    // encounter half is world.js:9190's line and the watch half is this
  ...
  // same order as world.js:9405/:9410, for the same reason: the
  ...
  // here, so the sink is handed down at this moment - world.js:9389's
  // line for this host.
  ...
  var modes = createWorldModes({
    ...
    buildingDirectory: () => ({
      ...
      // two inputs the world host uses (world.js:11578).
   ...
   *  second ctx here is the shape FS1 was written about. */
  {
     ...
     *  encounter mount above, and world.js:4317-4333 is the shape.
     ...
     *  world.js:4247-4248 does it. ROAD-G TAIL: a WATCHMAN transforms
  ...
  function frame(now) {
    ...
    if (isEnhanced() && !weatherOverride) {
      ...
      for (const s of ds.sounds) { const h = pixelLocalOfField(_locPixel.x, _locPixel.y, s.x, s.z); audio.play3d(s.clip, thunderSourceAt(eye, h[0], h[1]), s.volume, { refDistance: THUNDER_SOURCE_M, maxDistance: THUNDER_SOURCE_M * 8 }); }
    ...
    const lit = withPlayerLights(holdSel ? holdSel.data : lightsOn
      ...
      : new Float32Array(0),
    magic?.candleLight(), playerTorchLight(playerEntity, player.pos, cam.yaw), thunderlockMuzzleLight(playerEntity, player.pos, cam.yaw), ...camps.lights(), ...droppedTorches.lights());   // X11: the candle burns by day too - the effect has no time gate; T1: so does the torch; HT1 the dropped lights; FIELD-GUN13 the muzzle flash
    ...
    // through the player for ever. world.js:13364-13603 is the shape.
    arrows.update(dt, {
    ...
    if (walkMode && !tpMode) {
      ...
      for (const ev of weaponRig.frame(dt, { paralyzed })) {
        ...
        if (!cityGuards.resolvePlayerHit(weaponRig.playerWeapon, eye, fwd, player.pos, makeInView(proj, view, multiply), guardHitSound)) {
          ...
          // BEFORE civilians - world.js:13713's order, and the order

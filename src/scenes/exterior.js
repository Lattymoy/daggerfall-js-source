...
export async function bootExterior(canvas, renderer, params, status) {
  ...
  const outdoorRestDeps = createRestDeps(playerEntity, {
    ...
    advanceMinutes: (n, sharedEnd) => { playerTicker.advance(n); runEncounterTick(walkMode ? player.pos : cam.pos, sharedEnd, true); },   // CAMP1-REST: every tick this drives IS a rest   // ROAD-G TAIL: the catch-up loop rides the rest's minutes, as world.js:4004 has it   // RESTX2: sharedEnd is the session's local sim-minutes online, so the roll still gets a fresh `now` while the real clock stands
  ...
  const magic = createPlayerMagic({
    ...
    // two engine-owning hosts wire the identical pair (world.js:3153-3154,
    // dungeonContext.js:2175-2176); `questBridge` is assigned below this
  ...
  const makeJournalWindow = (mode) => {
    ...
    // gate could never run. dungeonContext.js:1378-1383 is the shape:
  ...
  const questWorld = {
    ...
    // other. world.js:7308 is the same line.
     ...
     *  %rn/%rt's producer - world.js:7207-7220. */
    ...
    // QG1: CastSpellDo's two world reads, world.js:7123-7126's pair.
     ...
     *  mode-aware (worldModes:1258), so this is world.js:7096's line
     ...
     *  Bridge-only, no host state - world.js:7104's call verbatim. */
     ...
     *  Everything else is world.js:7208's arm term for term: the cast
  ...
  questBridge = createQuestBridge({
    ...
    // closure S40 gave this host, and world.js:7871's line.
    ...
    // encounter half is world.js:7951's line and the watch half is this
  ...
  // same order as world.js:8172/:8177, for the same reason: the
  ...
  // here, so the sink is handed down at this moment - world.js:8156's
  // line for this host.
  ...
  var modes = createWorldModes({
    ...
    buildingDirectory: () => ({
      ...
      // two inputs the world host uses (world.js:9078).
   ...
   *  second ctx here is the shape FS1 was written about. */
  {
    ...
    setDefaultEnchantCtx(createEnchantCtx({
      ...
      // (DaggerfallUI.cs:1346-1358), which is PushWindow
  ...
  function frame(now) {
    ...
    if (modes.frame(dt, now)) {
      ...
      // (townTalk.js:618, :599). So every HUD line raised in a modal
    ...
    // through the player for ever. world.js:10674-10849 is the shape.
    arrows.update(dt, {
    ...
    if (walkMode && !tpMode) {
      ...
      for (const ev of weaponRig.frame(dt, { paralyzed })) {
        ...
        if (!cityGuards.resolvePlayerHit(weaponRig.playerWeapon, eye, fwd, player.pos, makeInView(proj, view, multiply), guardHitSound)) {
          ...
          // BEFORE civilians - world.js:10946's order, and the order

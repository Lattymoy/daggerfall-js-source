...
import { spellRecordOfIndex } from '../systems/loot.js';   // QG1: CastSpellDo's classic-record read (the G4 registry) - world.js:122's import
...
export async function bootExterior(canvas, renderer, params, status) {
  ...
  //     continue;`, world.js:1025) and stands them in batches of their
  ...
  function onExhaustedExterior() {
    ...
    try {
      const out = exhaustionOutcome({
        ...
        enemiesNearby: areEnemiesNearby([...(cityGuards?.guards ?? []), ...(exteriorFoes?.foes ?? [])]),   // ROAD-G G2: BOTH street pools, world.js:1620's line
  ...
  // pool the mode machine holds, which is world.js:2075's line.
  ...
  const cityGuards = createCityGuards({
    ...
    // own note describes. Written in world.js:2163's shape, so one pin
   ...
   *  the other two exterior hosts mount - world.js:2115 over the street
   ...
   *  it (world.js:2161-2245) and is its own slice; this pool does not
   *  wait on it. */
  ...
  const outdoorRestDeps = createRestDeps(playerEntity, {
    ...
    advanceMinutes: (n) => { playerTicker.advance(n); runEncounterTick(walkMode ? player.pos : cam.pos); },   // ROAD-G TAIL: the catch-up loop rides the rest's minutes, as world.js:3046 has it
   ...
   *  (effects.js:881/:895) - world.js:2303 hoisted its copy for exactly
  ...
  const magic = createPlayerMagic({
    ...
    // two engine-owning hosts wire the identical pair (world.js:2411-2412,
  ...
  const questWorld = {
    ...
    // other. world.js:5538 is the same line.
     ...
     *  %rn/%rt's producer - world.js:5540-5553. */
    ...
    // QG1: CastSpellDo's two world reads, world.js:5456-5459's pair.
     ...
     *  mode-aware (worldModes:1258), so this is world.js:5429's line
     ...
     *  Bridge-only, no host state - world.js:5437's call verbatim. */
     ...
     *  Everything else is world.js:5541's arm term for term: the cast
  ...
  questBridge = createQuestBridge({
    ...
    // closure S40 gave this host, and world.js:6194's line.
    ...
    // encounter half is world.js:6116's line and the watch half is this
  ...
  // same order as world.js:6494/:6496, for the same reason: the
  ...
  // here, so the sink is handed down at this moment - world.js:6478's
  // line for this host.
  ...
  const onlineFrame = (now) => {
    ...
    const key = roomKeyFor({
      host: 'exterior', mode,
      regionIndex: ident?.kind === 'dungeon' ? ident.regionIndex : dfLocation.regionIndex,
      locationName: ident?.kind === 'dungeon' ? ident.name : dfLocation.name,
      buildingKey: ident?.buildingKey ?? 0,
    });
    // the fixed city has no floating origin: every room's pose is the scene's own
    const pose = { x: player.pos[0], y: player.pos[1], z: player.pos[2], yaw: cam.yaw, pitch: cam.pitch, mv: 0 };
    onlineToScene = (p) => [p.x, p.y, p.z];
  ...
  var modes = createWorldModes({
    ...
    buildingDirectory: () => ({
      ...
      // two inputs the world host uses (world.js:6641).
   ...
   *  second ctx here is the shape FS1 was written about. */
  {
     ...
     *  encounter mount above, and world.js:2658-2674 is the shape.
     ...
     *  world.js:2600-2601 does it. ROAD-G TAIL: a WATCHMAN transforms
  ...
  function frame(now) {
    ...
    last = now;
    if (onlineOn) { if (!online) onlineStart(); onlineFrame(now); }   // ONLINE1: the pose out, the peers in - after the look is paid, before the camera is read and any mode draws
    ...
    // through the player for ever. world.js:7934-8053 is the shape.
    arrows.update(dt, {
    ...
    if (remotePlayers) for (const b of remotePlayers.batches()) _visBatches.push(b);   // ONLINE1: the others, at their feet
    ...
    if (walkMode && !tpMode) {
      ...
      for (const ev of weaponRig.frame(dt, { paralyzed })) {
        ...
        if (!cityGuards.resolvePlayerHit(weaponRig.playerWeapon, eye, fwd, player.pos, makeInView(proj, view, multiply), guardHitSound)) {
          ...
          // BEFORE civilians - world.js:8122's order, and the order
    ...
    // forever.
    {
      ...
      drawPeerNames(proj, view, eye);   // ONLINE1: the names over the heads

...
export async function bootExterior(canvas, renderer, params, status) {
  ...
  //     continue;`, world.js:1271) and stands them in batches of their
  ...
  function onExhaustedExterior() {
    ...
    try {
      const out = exhaustionOutcome({
        ...
        enemiesNearby: areEnemiesNearby([...(cityGuards?.guards ?? []), ...(exteriorFoes?.foes ?? [])]),   // ROAD-G G2: BOTH street pools, world.js:1982's line
  ...
  // pool the mode machine holds, which is world.js:2549's line.
   ...
   *  the other two exterior hosts mount - world.js:2610 over the street
  ...
  const outdoorRestDeps = createRestDeps(playerEntity, {
    ...
    advanceMinutes: (n, sharedEnd) => { playerTicker.advance(n); runEncounterTick(walkMode ? player.pos : cam.pos, sharedEnd, true); },   // CAMP1-REST: every tick this drives IS a rest   // ROAD-G TAIL: the catch-up loop rides the rest's minutes, as world.js:3992 has it   // RESTX2: sharedEnd is the session's local sim-minutes online, so the roll still gets a fresh `now` while the real clock stands
  ...
  const magic = createPlayerMagic({
    ...
    // two engine-owning hosts wire the identical pair (world.js:3161-3162,
  ...
  const questWorld = {
    ...
    // other. world.js:7255 is the same line.
     ...
     *  %rn/%rt's producer - world.js:7154-7167. */
    ...
    // QG1: CastSpellDo's two world reads, world.js:7070-7073's pair.
     ...
     *  mode-aware (worldModes:1258), so this is world.js:7043's line
     ...
     *  Bridge-only, no host state - world.js:7051's call verbatim. */
     ...
     *  Everything else is world.js:7155's arm term for term: the cast
  ...
  questBridge = createQuestBridge({
    ...
    // closure S40 gave this host, and world.js:7808's line.
    ...
    // encounter half is world.js:7888's line and the watch half is this
  ...
  // same order as world.js:8109/:8114, for the same reason: the
  ...
  // here, so the sink is handed down at this moment - world.js:8093's
  // line for this host.
  ...
  var modes = createWorldModes({
    ...
    buildingDirectory: () => ({
      ...
      // two inputs the world host uses (world.js:8990).
  ...
  function frame(now) {
    ...
    // through the player for ever. world.js:10560-10734 is the shape.
    arrows.update(dt, {
    ...
    if (walkMode && !tpMode) {
      ...
      for (const ev of weaponRig.frame(dt, { paralyzed })) {
        ...
        if (!cityGuards.resolvePlayerHit(weaponRig.playerWeapon, eye, fwd, player.pos, makeInView(proj, view, multiply), guardHitSound)) {
          ...
          // BEFORE civilians - world.js:10827's order, and the order

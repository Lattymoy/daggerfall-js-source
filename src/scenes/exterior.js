...
export async function bootExterior(canvas, renderer, params, status) {
  ...
  const questWorld = {
    ...
    // other. world.js:7264 is the same line.
     ...
     *  %rn/%rt's producer - world.js:7163-7176. */
    ...
    // QG1: CastSpellDo's two world reads, world.js:7079-7082's pair.
     ...
     *  mode-aware (worldModes:1258), so this is world.js:7052's line
     ...
     *  Bridge-only, no host state - world.js:7060's call verbatim. */
     ...
     *  Everything else is world.js:7164's arm term for term: the cast
  ...
  function frame(now) {
    ...
    // through the player for ever. world.js:10608-10782 is the shape.
    arrows.update(dt, {
    ...
    if (walkMode && !tpMode) {
      ...
      for (const ev of weaponRig.frame(dt, { paralyzed })) {
        ...
        if (!cityGuards.resolvePlayerHit(weaponRig.playerWeapon, eye, fwd, player.pos, makeInView(proj, view, multiply), guardHitSound)) {
          ...
          // BEFORE civilians - world.js:10878's order, and the order

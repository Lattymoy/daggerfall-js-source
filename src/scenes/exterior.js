...
import { pickActivatableHit, pickQuestFoe, pickFoe } from '../player/activate.js';
import { raceActivation } from '../player/activationRace.js';   // HARD2: one home for "the nearest thing under the one ray takes the click"   // G3: corpse loot; QG1/ROAD-G G2: the foe-click door; TI1: the lock-on pick
...
import { spellRecordOfIndex } from '../systems/loot.js';   // QG1: CastSpellDo's classic-record read (the G4 registry) - world.js:128's import
...
export async function bootExterior(canvas, renderer, params, status) {
  ...
  //     continue;`, world.js:1074) and stands them in batches of their
  ...
  function onExhaustedExterior() {
    ...
    try {
      const out = exhaustionOutcome({
        ...
        enemiesNearby: areEnemiesNearby([...(cityGuards?.guards ?? []), ...(exteriorFoes?.foes ?? [])]),   // ROAD-G G2: BOTH street pools, world.js:1684's line
  ...
  // pool the mode machine holds, which is world.js:2140's line.
  ...
  const cityGuards = createCityGuards({
    ...
    // own note describes. Written in world.js:2239's shape, so one pin
   ...
   *  the other two exterior hosts mount - world.js:2180 over the street
   ...
   *  it (world.js:2237-2319) and is its own slice; this pool does not
   *  wait on it. */
  ...
  const outdoorRestDeps = createRestDeps(playerEntity, {
    ...
    advanceMinutes: (n) => { playerTicker.advance(n); runEncounterTick(walkMode ? player.pos : cam.pos); },   // ROAD-G TAIL: the catch-up loop rides the rest's minutes, as world.js:3121 has it
   ...
   *  (effects.js:881/:895) - world.js:2551 hoisted its copy for exactly
  ...
  const magic = createPlayerMagic({
    ...
    // two engine-owning hosts wire the identical pair (world.js:2485-2486,
    // dungeonContext.js:1984-1985); `questBridge` is assigned below this
  ...
  const makeJournalWindow = (mode) => {
    ...
    // gate could never run. dungeonContext.js:1284-1286 is the shape:
  ...
  const questWorld = {
    ...
    // other. world.js:5742 is the same line.
     ...
     *  %rn/%rt's producer - world.js:5641-5654. */
    ...
    // QG1: CastSpellDo's two world reads, world.js:5557-5560's pair.
     ...
     *  mode-aware (worldModes:1258), so this is world.js:5530's line
     ...
     *  Bridge-only, no host state - world.js:5538's call verbatim. */
     ...
     *  Everything else is world.js:5642's arm term for term: the cast
  ...
  questBridge = createQuestBridge({
    ...
    // closure S40 gave this host, and world.js:6295's line.
    ...
    // encounter half is world.js:6375's line and the watch half is this
  ...
  // same order as world.js:6596/:6601, for the same reason: the
  ...
  // here, so the sink is handed down at this moment - world.js:6580's
  // line for this host.
  ...
  var modes = createWorldModes({
    ...
    buildingDirectory: () => ({
      ...
      // two inputs the world host uses (world.js:7019).
   ...
   *  second ctx here is the shape FS1 was written about. */
  {
     ...
     *  encounter mount above, and world.js:2733-2749 is the shape.
     ...
     *  world.js:2672-2673 does it. ROAD-G TAIL: a WATCHMAN transforms
  ...
  function frame(now) {
    ...
    // through the player for ever. world.js:8340-8460 is the shape.
    arrows.update(dt, {
    ...
    if (walkMode && !tpMode) {
      ...
      for (const ev of weaponRig.frame(dt, { paralyzed })) {
        ...
        if (!cityGuards.resolvePlayerHit(weaponRig.playerWeapon, eye, fwd, player.pos, makeInView(proj, view, multiply), guardHitSound)) {
          ...
          // BEFORE civilians - world.js:8529's order, and the order

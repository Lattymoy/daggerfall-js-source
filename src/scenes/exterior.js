...
export async function bootExterior(canvas, renderer, params, status) {
  ...
  //     continue;`, world.js:1790) and stands them in batches of their
  ...
  function onExhaustedExterior() {
    ...
    try {
      const out = exhaustionOutcome({
        ...
        enemiesNearby: areEnemiesNearby([...(cityGuards?.guards ?? []), ...(exteriorFoes?.foes ?? [])]),   // ROAD-G G2: BOTH street pools, world.js:2665's line
   ...
   *  (effects.js:880/:894) - world.js:4147 hoisted its copy for exactly
  ...
  const magic = createPlayerMagic({
    ...
    // two engine-owning hosts wire the identical pair (world.js:4095-4096,
    // dungeonContext.js:2245-2246); `questBridge` is assigned below this
  ...
  const makeJournalWindow = (mode) => {
    ...
    // gate could never run. dungeonContext.js:1450-1455 is the shape:
  ...
  const questWorld = {
    ...
    // other. world.js:8619 is the same line.
     ...
     *  %rn/%rt's producer - world.js:8518-8531. */
    ...
    // QG1: CastSpellDo's two world reads, world.js:8434-8437's pair.
     ...
     *  mode-aware (worldModes:1258), so this is world.js:8407's line
     ...
     *  Bridge-only, no host state - world.js:8415's call verbatim. */
     ...
     *  Everything else is world.js:8519's arm term for term: the cast
  ...
  questBridge = createQuestBridge({
    ...
    // closure S40 gave this host, and world.js:9179's line.
    ...
    // encounter half is world.js:9270's line and the watch half is this
  ...
  // same order as world.js:9484/:9489, for the same reason: the
  ...
  // here, so the sink is handed down at this moment - world.js:9468's
  // line for this host.
  ...
  var modes = createWorldModes({
    ...
    buildingDirectory: () => ({
      ...
      // two inputs the world host uses (world.js:12046).
   ...
   *  second ctx here is the shape FS1 was written about. */
  {
     ...
     *  world.js:4269-4270 does it. ROAD-G TAIL: a WATCHMAN transforms
  ...
  function frame(now) {
    ...
    // through the player for ever. world.js:13841-14080 is the shape.
    arrows.update(dt, {
    ...
    if (walkMode && !tpMode) {
      ...
      for (const ev of weaponRig.frame(dt, { paralyzed })) {
        ...
        if (!cityGuards.resolvePlayerHit(weaponRig.playerWeapon, eye, fwd, player.pos, makeInView(proj, view, multiply), guardHitSound, { swing })) {
          ...
          // BEFORE civilians - world.js:14190's order, and the order

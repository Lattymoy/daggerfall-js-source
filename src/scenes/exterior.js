...
import { spellRecordOfIndex } from '../systems/loot.js';   // QG1: CastSpellDo's classic-record read (the G4 registry) - world.js:169's import
...
export async function bootExterior(canvas, renderer, params, status) {
  ...
  //     continue;`, world.js:1268) and stands them in batches of their
  ...
  function onExhaustedExterior() {
    ...
    try {
      const out = exhaustionOutcome({
        ...
        enemiesNearby: areEnemiesNearby([...(cityGuards?.guards ?? []), ...(exteriorFoes?.foes ?? [])]),   // ROAD-G G2: BOTH street pools, world.js:1979's line
  ...
  // pool the mode machine holds, which is world.js:2546's line.
  ...
  const cityGuards = createCityGuards({
    ...
    // own note describes. Written in world.js:2771's shape, so one pin
   ...
   *  the other two exterior hosts mount - world.js:2607 over the street
   ...
   *  it (world.js:2769-2851) and is its own slice; this pool does not
   *  wait on it. */
   ...
   *  (effects.js:881/:895) - world.js:3178 hoisted its copy for exactly
  ...
  const magic = createPlayerMagic({
    ...
    // two engine-owning hosts wire the identical pair (world.js:3083-3084,
    // dungeonContext.js:2090-2091); `questBridge` is assigned below this
  ...
  const makeJournalWindow = (mode) => {
    ...
    // gate could never run. dungeonContext.js:1313-1318 is the shape:
  ...
  const questWorld = {
    ...
    // other. world.js:7177 is the same line.
     ...
     *  %rn/%rt's producer - world.js:7076-7089. */
    ...
    // QG1: CastSpellDo's two world reads, world.js:6992-6995's pair.
     ...
     *  mode-aware (worldModes:1258), so this is world.js:6965's line
     ...
     *  Bridge-only, no host state - world.js:6973's call verbatim. */
     ...
     *  Everything else is world.js:7077's arm term for term: the cast
  ...
  questBridge = createQuestBridge({
    ...
    // closure S40 gave this host, and world.js:7730's line.
    ...
    // encounter half is world.js:7810's line and the watch half is this
  ...
  // same order as world.js:8031/:8036, for the same reason: the
  ...
  // here, so the sink is handed down at this moment - world.js:8015's
  // line for this host.
  ...
  var modes = createWorldModes({
    ...
    buildingDirectory: () => ({
      ...
      // two inputs the world host uses (world.js:8912).
   ...
   *  second ctx here is the shape FS1 was written about. */
  {
     ...
     *  encounter mount above, and world.js:3360-3376 is the shape.
     ...
     *  world.js:3290-3291 does it. ROAD-G TAIL: a WATCHMAN transforms
  ...
  function frame(now) {
    ...
    // through the player for ever. world.js:10482-10656 is the shape.
    arrows.update(dt, {
    ...
    if (walkMode && !tpMode) {
      ...
      for (const ev of weaponRig.frame(dt, { paralyzed })) {
        ...
        if (!cityGuards.resolvePlayerHit(weaponRig.playerWeapon, eye, fwd, player.pos, makeInView(proj, view, multiply), guardHitSound)) {
          ...
          // BEFORE civilians - world.js:10751's order, and the order

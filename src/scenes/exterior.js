...
export async function bootExterior(canvas, renderer, params, status) {
  ...
  const questWorld = {
    ...
    // other. world.js:7509 is the same line.
     ...
     *  %rn/%rt's producer - world.js:7408-7421. */
    ...
    // QG1: CastSpellDo's two world reads, world.js:7321-7324's pair.
     ...
     *  mode-aware (worldModes:1258), so this is world.js:7294's line
     ...
     *  Bridge-only, no host state - world.js:7302's call verbatim. */
     ...
     *  Everything else is world.js:7409's arm term for term: the cast
  ...
  questBridge = createQuestBridge({
    ...
    // closure S40 gave this host, and world.js:8072's line.
    ...
    // encounter half is world.js:8152's line and the watch half is this
  ...
  // same order as world.js:8373/:8378, for the same reason: the
  ...
  // here, so the sink is handed down at this moment - world.js:8357's
  // line for this host.
  ...
  var modes = createWorldModes({
    ...
    buildingDirectory: () => ({
      ...
      // two inputs the world host uses (world.js:9309).
  ...
  function frame(now) {
    ...
    // through the player for ever. world.js:10925-11125 is the shape.
    arrows.update(dt, {
    ...
    if (walkMode && !tpMode) {
      ...
      for (const ev of weaponRig.frame(dt, { paralyzed })) {
        ...
        if (!cityGuards.resolvePlayerHit(weaponRig.playerWeapon, eye, fwd, player.pos, makeInView(proj, view, multiply), guardHitSound)) {
          ...
          // BEFORE civilians - world.js:11232's order, and the order

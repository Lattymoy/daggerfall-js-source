...
export async function bootExterior(canvas, renderer, params, status) {
   ...
   *  it (world.js:2205-2290) and is its own slice; this pool does not
   *  wait on it. */
   ...
   *  (effects.js:881/:895) - world.js:2348 hoisted its copy for exactly
  ...
  const questWorld = {
    ...
    // other. world.js:5605 is the same line.
  ...
  questBridge = createQuestBridge({
    ...
    // encounter half is world.js:6183's line and the watch half is this
  ...
  // same order as world.js:6562/:6562, for the same reason: the
  ...
  var modes = createWorldModes({
    ...
    buildingDirectory: () => ({
      ...
      // two inputs the world host uses (world.js:6950).
  ...
  function frame(now) {
    ...
    // through the player for ever. world.js:8249-8368 is the shape.
    arrows.update(dt, {
    ...
    if (walkMode && !tpMode) {
      ...
      for (const ev of weaponRig.frame(dt, { paralyzed })) {
        ...
        if (!cityGuards.resolvePlayerHit(weaponRig.playerWeapon, eye, fwd, player.pos, makeInView(proj, view, multiply), guardHitSound)) {
          ...
          // BEFORE civilians - world.js:8437's order, and the order

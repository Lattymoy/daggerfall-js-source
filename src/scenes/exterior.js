...
export async function bootExterior(canvas, renderer, params, status) {
  ...
  const questWorld = {
    ...
    // other. world.js:7228 is the same line.
     ...
     *  %rn/%rt's producer - world.js:7127-7140. */
    ...
    // QG1: CastSpellDo's two world reads, world.js:7043-7046's pair.
     ...
     *  mode-aware (worldModes:1258), so this is world.js:7016's line
     ...
     *  Bridge-only, no host state - world.js:7024's call verbatim. */
     ...
     *  Everything else is world.js:7128's arm term for term: the cast
  ...
  questBridge = createQuestBridge({
    ...
    // closure S40 gave this host, and world.js:7782's line.
    ...
    // encounter half is world.js:7862's line and the watch half is this
  ...
  // same order as world.js:8083/:8088, for the same reason: the
  ...
  // here, so the sink is handed down at this moment - world.js:8067's
  // line for this host.
  ...
  var modes = createWorldModes({
    ...
    buildingDirectory: () => ({
      ...
      // two inputs the world host uses (world.js:8989).
  ...
  function frame(now) {
    ...
    // through the player for ever. world.js:10571-10745 is the shape.
    arrows.update(dt, {
    ...
    if (walkMode && !tpMode) {
      ...
      for (const ev of weaponRig.frame(dt, { paralyzed })) {
        ...
        if (!cityGuards.resolvePlayerHit(weaponRig.playerWeapon, eye, fwd, player.pos, makeInView(proj, view, multiply), guardHitSound)) {
          ...
          // BEFORE civilians - world.js:10841's order, and the order

...
import { spellRecordOfIndex } from '../systems/loot.js';   // QG1: CastSpellDo's classic-record read (the G4 registry) - world.js:120's import
...
export async function bootExterior(canvas, renderer, params, status) {
  ...
  //     continue;`, world.js:1021) and stands them in batches of their
  ...
  function onExhaustedExterior() {
    ...
    try {
      const out = exhaustionOutcome({
        ...
        enemiesNearby: areEnemiesNearby([...(cityGuards?.guards ?? []), ...(exteriorFoes?.foes ?? [])]),   // ROAD-G G2: BOTH street pools, world.js:1616's line
  ...
  // pool the mode machine holds, which is world.js:2071's line.
  ...
  const cityGuards = createCityGuards({
    ...
    // own note describes. Written in world.js:2159's shape, so one pin
   ...
   *  the other two exterior hosts mount - world.js:2111 over the street
   ...
   *  it (world.js:2157-2241) and is its own slice; this pool does not
   *  wait on it. */
  ...
  const outdoorRestDeps = createRestDeps(playerEntity, {
    ...
    advanceMinutes: (n) => { playerTicker.advance(n); runEncounterTick(walkMode ? player.pos : cam.pos); },   // ROAD-G TAIL: the catch-up loop rides the rest's minutes, as world.js:3042 has it
   ...
   *  (effects.js:881/:895) - world.js:2299 hoisted its copy for exactly
  ...
  const magic = createPlayerMagic({
    ...
    // two engine-owning hosts wire the identical pair (world.js:2407-2408,
  ...
  const questWorld = {
    ...
    // other. world.js:5506 is the same line.
     ...
     *  %rn/%rt's producer - world.js:5508-5521. */
    ...
    // QG1: CastSpellDo's two world reads, world.js:5424-5427's pair.
     ...
     *  mode-aware (worldModes:1258), so this is world.js:5397's line
     ...
     *  Bridge-only, no host state - world.js:5405's call verbatim. */
     ...
     *  Everything else is world.js:5509's arm term for term: the cast
  ...
  questBridge = createQuestBridge({
    ...
    // closure S40 gave this host, and world.js:6162's line.
    ...
    // encounter half is world.js:6084's line and the watch half is this
  ...
  // here, so the sink is handed down at this moment - world.js:6446's
  // line for this host.
  ...
  var modes = createWorldModes({
    ...
    buildingDirectory: () => ({
      ...
      // two inputs the world host uses (world.js:6560).
   ...
   *  second ctx here is the shape FS1 was written about. */
  {
     ...
     *  encounter mount above, and world.js:2654-2670 is the shape.
     ...
     *  world.js:2596-2597 does it. ROAD-G TAIL: a WATCHMAN transforms
  ...
  function frame(now) {
    ...
    // through the player for ever. world.js:7832-7951 is the shape.
    arrows.update(dt, {
    ...
    if (walkMode && !tpMode) {
      ...
      for (const ev of weaponRig.frame(dt, { paralyzed })) {
        ...
        if (!cityGuards.resolvePlayerHit(weaponRig.playerWeapon, eye, fwd, player.pos, makeInView(proj, view, multiply), guardHitSound)) {
          ...
          // BEFORE civilians - world.js:8020's order, and the order

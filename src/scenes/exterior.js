...
export async function bootExterior(canvas, renderer, params, status) {
  ...
  const _hoverNamers = [
    ...
    (key) => (typeof key === 'string' && key.startsWith('water:')
      ? waterSourceHoverName(!!springs[Number(key.split(':')[1])]?.dry) : null),
  ...
  const cityGuards = createCityGuards({
    ...
    // own note describes. Written in world.js:2975's shape, so one pin
   ...
   *  it (world.js:2973-3055) and is its own slice; this pool does not
   *  wait on it. */
  ...
  const outdoorRestDeps = createRestDeps(playerEntity, {
    ...
    advanceMinutes: (n, sharedEnd) => { playerTicker.advance(n); runEncounterTick(walkMode ? player.pos : cam.pos, sharedEnd, true); },   // CAMP1-REST: every tick this drives IS a rest   // ROAD-G TAIL: the catch-up loop rides the rest's minutes, as world.js:4122 has it   // RESTX2: sharedEnd is the session's local sim-minutes online, so the roll still gets a fresh `now` while the real clock stands
  ...
  const magic = createPlayerMagic({
    ...
    // two engine-owning hosts wire the identical pair (world.js:3287-3288,
  ...
  const questWorld = {
    ...
    // other. world.js:7408 is the same line.
     ...
     *  %rn/%rt's producer - world.js:7307-7320. */
    ...
    // QG1: CastSpellDo's two world reads, world.js:7223-7226's pair.
     ...
     *  mode-aware (worldModes:1258), so this is world.js:7196's line
     ...
     *  Bridge-only, no host state - world.js:7204's call verbatim. */
     ...
     *  Everything else is world.js:7308's arm term for term: the cast
  ...
  questBridge = createQuestBridge({
    ...
    // closure S40 gave this host, and world.js:7962's line.
    ...
    // encounter half is world.js:8042's line and the watch half is this
  ...
  // same order as world.js:8263/:8268, for the same reason: the
  ...
  // here, so the sink is handed down at this moment - world.js:8247's
  // line for this host.
  ...
  var modes = createWorldModes({
    ...
    buildingDirectory: () => ({
      ...
      // two inputs the world host uses (world.js:9169).
  ...
  function frame(now) {
    ...
    // through the player for ever. world.js:10787-10987 is the shape.
    arrows.update(dt, {
    ...
    if (walkMode && !tpMode) {
      ...
      for (const ev of weaponRig.frame(dt, { paralyzed })) {
        ...
        if (!cityGuards.resolvePlayerHit(weaponRig.playerWeapon, eye, fwd, player.pos, makeInView(proj, view, multiply), guardHitSound)) {
          ...
          // BEFORE civilians - world.js:11094's order, and the order

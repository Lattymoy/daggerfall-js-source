...
export async function bootExterior(canvas, renderer, params, status) {
  ...
  const questWorld = {
    ...
    // other. world.js:7250 is the same line.
     ...
     *  %rn/%rt's producer - world.js:7149-7162. */
    ...
    // QG1: CastSpellDo's two world reads, world.js:7065-7068's pair.
     ...
     *  mode-aware (worldModes:1258), so this is world.js:7038's line
     ...
     *  Bridge-only, no host state - world.js:7046's call verbatim. */
     ...
     *  Everything else is world.js:7150's arm term for term: the cast
  ...
  questBridge = createQuestBridge({
    ...
    // closure S40 gave this host, and world.js:7804's line.
    ...
    // encounter half is world.js:7884's line and the watch half is this
  ...
  // same order as world.js:8105/:8110, for the same reason: the
  ...
  // here, so the sink is handed down at this moment - world.js:8089's
  // line for this host.
  ...
  function frame(now) {
    ...
    // through the player for ever. world.js:10593-10767 is the shape.
    arrows.update(dt, {
    ...
    if (walkMode && !tpMode) {
      ...
      for (const ev of weaponRig.frame(dt, { paralyzed })) {
        ...
        if (!cityGuards.resolvePlayerHit(weaponRig.playerWeapon, eye, fwd, player.pos, makeInView(proj, view, multiply), guardHitSound)) {
          ...
          // BEFORE civilians - world.js:10863's order, and the order

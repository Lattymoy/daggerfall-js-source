...
import { spellRecordOfIndex } from '../systems/loot.js';   // QG1: CastSpellDo's classic-record read (the G4 registry) - world.js:204's import
...
export async function bootExterior(canvas, renderer, params, status) {
  ...
  //     continue;`, world.js:1805) and stands them in batches of their
  ...
  function onExhaustedExterior() {
    ...
    try {
      const out = exhaustionOutcome({
        ...
        enemiesNearby: areEnemiesNearby([...(cityGuards?.guards ?? []), ...(exteriorFoes?.foes ?? [])]),   // ROAD-G G2: BOTH street pools, world.js:2682's line
  ...
  // pool the mode machine holds, which is world.js:3260's line.
  ...
  const cityGuards = createCityGuards({
    ...
    // own note describes. Written in world.js:3595's shape, so one pin
   ...
   *  the other two exterior hosts mount - world.js:3321 over the street
   ...
   *  it (world.js:3593-3797) and is its own slice; this pool does not
   *  wait on it. */
  ...
  const outdoorRestDeps = createRestDeps(playerEntity, {
    ...
    advanceMinutes: (n, sharedEnd) => { playerTicker.advance(n); runEncounterTick(walkMode ? player.pos : cam.pos, sharedEnd, true); },   // CAMP1-REST: every tick this drives IS a rest   // ROAD-G TAIL: the catch-up loop rides the rest's minutes, as world.js:4987 has it   // RESTX2: sharedEnd is the session's local sim-minutes online, so the roll still gets a fresh `now` while the real clock stands
   ...
   *  (effects.js:887/:901) - world.js:4148 hoisted its copy for exactly
  ...
  const magic = createPlayerMagic({
    ...
    // two engine-owning hosts wire the identical pair (world.js:4096-4097,
    // dungeonContext.js:2233-2234); `questBridge` is assigned below this
  ...
  const makeJournalWindow = (mode) => {
    ...
    // gate could never run. dungeonContext.js:1446-1451 is the shape:
  ...
  const questWorld = {
    ...
    // other. world.js:8592 is the same line.
     ...
     *  %rn/%rt's producer - world.js:8491-8504. */
    ...
    // QG1: CastSpellDo's two world reads, world.js:8407-8410's pair.
     ...
     *  mode-aware (worldModes:1258), so this is world.js:8380's line
     ...
     *  Bridge-only, no host state - world.js:8388's call verbatim. */
     ...
     *  Everything else is world.js:8492's arm term for term: the cast
  ...
  questBridge = createQuestBridge({
    ...
    // closure S40 gave this host, and world.js:9151's line.
    ...
    // encounter half is world.js:9242's line and the watch half is this
  ...
  // same order as world.js:9457/:9462, for the same reason: the
  ...
  // here, so the sink is handed down at this moment - world.js:9441's
  // line for this host.
  ...
  var modes = createWorldModes({
    ...
    buildingDirectory: () => ({
      ...
      // two inputs the world host uses (world.js:12022).
   ...
   *  second ctx here is the shape FS1 was written about. */
  {
     ...
     *  encounter mount above, and world.js:4340-4356 is the shape.
     ...
     *  world.js:4270-4271 does it. ROAD-G TAIL: a WATCHMAN transforms
  ...
  function frame(now) {
    ...
    // through the player for ever. world.js:13805-14044 is the shape.
    arrows.update(dt, {
    ...
    if (walkMode && !tpMode) {
      ...
      for (const ev of weaponRig.frame(dt, { paralyzed })) {
        ...
        if (!cityGuards.resolvePlayerHit(weaponRig.playerWeapon, eye, fwd, player.pos, makeInView(proj, view, multiply), guardHitSound)) {
          ...
          // BEFORE civilians - world.js:14154's order, and the order

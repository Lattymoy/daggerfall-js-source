...
import { spellRecordOfIndex } from '../systems/loot.js';   // QG1: CastSpellDo's classic-record read (the G4 registry) - world.js:172's import
...
export async function bootExterior(canvas, renderer, params, status) {
  ...
  //     continue;`, world.js:1314) and stands them in batches of their
  ...
  function onExhaustedExterior() {
    ...
    try {
      const out = exhaustionOutcome({
        ...
        enemiesNearby: areEnemiesNearby([...(cityGuards?.guards ?? []), ...(exteriorFoes?.foes ?? [])]),   // ROAD-G G2: BOTH street pools, world.js:2025's line
  ...
  // pool the mode machine holds, which is world.js:2599's line.
  ...
  const cityGuards = createCityGuards({
    ...
    // own note describes. Written in world.js:2833's shape, so one pin
   ...
   *  the other two exterior hosts mount - world.js:2660 over the street
   ...
   *  it (world.js:2831-2913) and is its own slice; this pool does not
   *  wait on it. */
   ...
   *  (effects.js:881/:895) - world.js:3290 hoisted its copy for exactly
  ...
  const magic = createPlayerMagic({
    ...
    // two engine-owning hosts wire the identical pair (world.js:3145-3146,
  ...
  const questWorld = {
    ...
    // other. world.js:7289 is the same line.
     ...
     *  %rn/%rt's producer - world.js:7188-7201. */
    ...
    // QG1: CastSpellDo's two world reads, world.js:7104-7107's pair.
     ...
     *  mode-aware (worldModes:1258), so this is world.js:7077's line
     ...
     *  Bridge-only, no host state - world.js:7085's call verbatim. */
     ...
     *  Everything else is world.js:7189's arm term for term: the cast
  ...
  questBridge = createQuestBridge({
    ...
    // closure S40 gave this host, and world.js:7842's line.
    ...
    // encounter half is world.js:7922's line and the watch half is this
  ...
  // same order as world.js:8143/:8148, for the same reason: the
  ...
  // here, so the sink is handed down at this moment - world.js:8127's
  // line for this host.
  ...
  var modes = createWorldModes({
    ...
    buildingDirectory: () => ({
      ...
      // two inputs the world host uses (world.js:9031).
   ...
   *  second ctx here is the shape FS1 was written about. */
  {
     ...
     *  encounter mount above, and world.js:3472-3488 is the shape.
     ...
     *  world.js:3402-3403 does it. ROAD-G TAIL: a WATCHMAN transforms
  ...
  function frame(now) {
    ...
    // through the player for ever. world.js:10601-10775 is the shape.
    arrows.update(dt, {
    ...
    if (walkMode && !tpMode) {
      ...
      for (const ev of weaponRig.frame(dt, { paralyzed })) {
        ...
        if (!cityGuards.resolvePlayerHit(weaponRig.playerWeapon, eye, fwd, player.pos, makeInView(proj, view, multiply), guardHitSound)) {
          ...
          // BEFORE civilians - world.js:10871's order, and the order

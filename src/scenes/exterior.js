...
export async function bootExterior(canvas, renderer, params, status) {
  ...
  const outdoorRestDeps = createRestDeps(playerEntity, {
    ...
    advanceMinutes: (n, sharedEnd) => { playerTicker.advance(n); runEncounterTick(walkMode ? player.pos : cam.pos, sharedEnd, true); },   // CAMP1-REST: every tick this drives IS a rest   // ROAD-G TAIL: the catch-up loop rides the rest's minutes, as world.js:3710 has it   // RESTX2: sharedEnd is the session's local sim-minutes online, so the roll still gets a fresh `now` while the real clock stands
  ...
  const questWorld = {
    ...
    // other. world.js:6818 is the same line.
     ...
     *  %rn/%rt's producer - world.js:6717-6730. */
    ...
    // QG1: CastSpellDo's two world reads, world.js:6633-6636's pair.
     ...
     *  mode-aware (worldModes:1258), so this is world.js:6606's line
     ...
     *  Bridge-only, no host state - world.js:6614's call verbatim. */
     ...
     *  Everything else is world.js:6718's arm term for term: the cast
  ...
  questBridge = createQuestBridge({
    ...
    // closure S40 gave this host, and world.js:7371's line.
    ...
    // encounter half is world.js:7451's line and the watch half is this
  ...
  // same order as world.js:7672/:7677, for the same reason: the
  ...
  // here, so the sink is handed down at this moment - world.js:7656's
  // line for this host.
  ...
  var modes = createWorldModes({
    ...
    buildingDirectory: () => ({
      ...
      // two inputs the world host uses (world.js:8495).

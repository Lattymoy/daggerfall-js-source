...
export async function bootExterior(canvas, renderer, params, status) {
  ...
  const outdoorRestDeps = createRestDeps(playerEntity, {
    ...
    advanceMinutes: (n, sharedEnd) => { playerTicker.advance(n); runEncounterTick(walkMode ? player.pos : cam.pos, sharedEnd); },   // ROAD-G TAIL: the catch-up loop rides the rest's minutes, as world.js:3393 has it   // RESTX2: sharedEnd is the session's local sim-minutes online, so the roll still gets a fresh `now` while the real clock stands
   ...
   *  (effects.js:881/:895) - world.js:2696 hoisted its copy for exactly
  ...
  const magic = createPlayerMagic({
    ...
    // two engine-owning hosts wire the identical pair (world.js:2609-2610,
  ...
  const questWorld = {
    ...
    // other. world.js:6205 is the same line.
     ...
     *  %rn/%rt's producer - world.js:6104-6117. */
    ...
    // QG1: CastSpellDo's two world reads, world.js:6020-6023's pair.
     ...
     *  mode-aware (worldModes:1258), so this is world.js:5993's line
     ...
     *  Bridge-only, no host state - world.js:6001's call verbatim. */
     ...
     *  Everything else is world.js:6105's arm term for term: the cast
  ...
  questBridge = createQuestBridge({
    ...
    // closure S40 gave this host, and world.js:6758's line.
    ...
    // encounter half is world.js:6838's line and the watch half is this
  ...
  // same order as world.js:7059/:7064, for the same reason: the
  ...
  // here, so the sink is handed down at this moment - world.js:7043's
  // line for this host.
  ...
  var modes = createWorldModes({
    ...
    buildingDirectory: () => ({
      ...
      // two inputs the world host uses (world.js:7870).
   ...
   *  second ctx here is the shape FS1 was written about. */
  {
     ...
     *  encounter mount above, and world.js:2878-2894 is the shape.
     ...
     *  world.js:2817-2818 does it. ROAD-G TAIL: a WATCHMAN transforms
  ...
  function frame(now) {
    ...
    // through the player for ever. world.js:9269-9389 is the shape.
    arrows.update(dt, {
    ...
    if (walkMode && !tpMode) {
      ...
      for (const ev of weaponRig.frame(dt, { paralyzed })) {
        ...
        if (!cityGuards.resolvePlayerHit(weaponRig.playerWeapon, eye, fwd, player.pos, makeInView(proj, view, multiply), guardHitSound)) {
          ...
          // BEFORE civilians - world.js:9458's order, and the order

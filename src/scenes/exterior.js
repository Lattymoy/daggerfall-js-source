...
export async function bootExterior(canvas, renderer, params, status) {
  ...
  //     continue;`, world.js:1873) and stands them in batches of their
  ...
  function onExhaustedExterior() {
    ...
    try {
      const out = exhaustionOutcome({
        ...
        enemiesNearby: areEnemiesNearby([...(cityGuards?.guards ?? []), ...(exteriorFoes?.foes ?? [])]),   // ROAD-G G2: BOTH street pools, world.js:2779's line
  ...
  // pool the mode machine holds, which is world.js:3401's line.
  ...
  const cityGuards = createCityGuards({
    ...
    // own note describes. Written in world.js:3736's shape, so one pin
   ...
   *  the other two exterior hosts mount - world.js:3462 over the street
   ...
   *  it (world.js:3734-3938) and is its own slice; this pool does not
   *  wait on it. */
   ...
   *  (effects.js:885/:899) - world.js:4298 hoisted its copy for exactly
  ...
  const magic = createPlayerMagic({
    ...
    // two engine-owning hosts wire the identical pair (world.js:4246-4247,
  ...
  const questWorld = {
    ...
    // other. world.js:8831 is the same line.
     ...
     *  %rn/%rt's producer - world.js:8727-8740. */
    ...
    // QG1: CastSpellDo's two world reads, world.js:8643-8646's pair.
     ...
     *  mode-aware (worldModes:1258), so this is world.js:8616's line
     ...
     *  Bridge-only, no host state - world.js:8624's call verbatim. */
     ...
     *  Everything else is world.js:8728's arm term for term: the cast
  ...
  questBridge = createQuestBridge({
    ...
    // closure S40 gave this host, and world.js:9391's line.
    ...
    // encounter half is world.js:9487's line and the watch half is this
  ...
  // same order as world.js:9701/:9706, for the same reason: the
  ...
  // here, so the sink is handed down at this moment - world.js:9685's
  // line for this host.
  ...
  var modes = createWorldModes({
    ...
    buildingDirectory: () => ({
      ...
      // two inputs the world host uses (world.js:12745).
   ...
   *  encounter mount above, and world.js:4478-4494 is the shape.
   ...
   *  second ctx here is the shape FS1 was written about. */
  {
     ...
     *  world.js:4425-4426 does it. ROAD-G TAIL: a WATCHMAN transforms
  ...
  function frame(now) {
    ...
    // through the player for ever. world.js:14565-14805 is the shape.
    arrows.update(dt, {
    ...
    if (walkMode && !tpMode) {
      ...
      for (const ev of weaponRig.frame(dt, { paralyzed })) {
        ...
        if (!cityGuards.resolvePlayerHit(weaponRig.playerWeapon, eye, fwd, player.pos, makeInView(proj, view, multiply), guardHitSound, { swing })) {
          ...
          // BEFORE civilians - world.js:14919's order, and the order

...
export async function bootExterior(canvas, renderer, params, status) {
  ...
  questBridge = createQuestBridge({
    ...
    // closure S40 gave this host, and world.js:8113's line.
    ...
    // encounter half is world.js:8204's line and the watch half is this
  ...
  // same order as world.js:8419/:8424, for the same reason: the
  ...
  // here, so the sink is handed down at this moment - world.js:8403's
  // line for this host.
  ...
  var modes = createWorldModes({
    ...
    buildingDirectory: () => ({
      ...
      // two inputs the world host uses (world.js:9434).
  ...
  function frame(now) {
    ...
    // through the player for ever. world.js:11090-11290 is the shape.
    arrows.update(dt, {
    ...
    if (walkMode && !tpMode) {
      ...
      for (const ev of weaponRig.frame(dt, { paralyzed })) {
        ...
        if (!cityGuards.resolvePlayerHit(weaponRig.playerWeapon, eye, fwd, player.pos, makeInView(proj, view, multiply), guardHitSound)) {
          ...
          // BEFORE civilians - world.js:11397's order, and the order

...
import { TOWN_WATER_DEPTH } from '../render/waterBasin.js';   // WATER2: the town's flat sheet at one depth
import { buildWaterArt } from '../world/waterArt.js';   // WATER4: the archive's own water art - the shore is the record's outline
...
export async function bootExterior(canvas, renderer, params, status) {
  ...
  // WATER4: the archive's own water ART, read once off its bitmaps and kept
  // on the renderer beside its tile array (world.js does the same)
  if (!renderer.waterArts.has(groundArchive)) {
    const groundTex = textureFiles.get(groundArchive) ?? await getTexture(groundArchive);
    const bitmaps = [];
    for (let r = 0; r < groundTex.recordCount; r++) bitmaps.push(groundTex.getDFBitmap(r, 0));
    renderer.uploadWaterArt(groundArchive, buildWaterArt(bitmaps));
  }
  const waterArt = renderer.waterArtOf(groundArchive);
  ...
  const waterOn = isEnhanced() && getPref('enhancedWater') && new URLSearchParams(globalThis.location?.search ?? '').get('water') !== 'off'
    && tilemapRectHasWater(tilemapBytes, tilemapDim, loc.width * GROUND_TILE_DIM, loc.height * GROUND_TILE_DIM, waterArt?.any);   // the padding past the town is zero, and zero is water; WATER4: by the art
  const groundSurface = (() => {
  ...
  // WATER2: the town's water on the same flat quad at one depth - a
  // moat or a dock's water has no grid to carve a bed into
  const townWater = waterOn ? (() => {
    const gy = GROUND_OFFSET * 0.025;
    const gw = loc.width * RMB_SIDE;
    const gh = loc.height * RMB_SIDE;
    return renderer.createWaterSurface(new Float32Array([0, gy, 0, gw, gy, 0, 0, gy, gh, gw, gy, gh]),
      new Float32Array(4).fill(TOWN_WATER_DEPTH), new Uint32Array([0, 2, 3, 0, 3, 1]));
  })() : null;
  ...
  const playerGroundSampleRaw = () => {
    ...
    // WATER4: and the archive's art, so the feet read the shore the eye sees
    return { tile: locationTilemap[tx + ty * tilemapDim], feet: [u - tx, v - ty], art: renderer.waterArtOf(groundArchive) };
  };
  ...
  const exteriorSurfaceNow = () => {
    ...
    return exteriorSurfaces({
      ...
      art: _ground?.art ?? null,     // WATER4: ...and the surface is the art's
  ...
  //     host's static NPCs take it (interiorContext.js:396-414).
  ...
  //     continue;`, world.js:1036) and stands them in batches of their
  ...
  function onExhaustedExterior() {
    ...
    try {
      const out = exhaustionOutcome({
        ...
        enemiesNearby: areEnemiesNearby([...(cityGuards?.guards ?? []), ...(exteriorFoes?.foes ?? [])]),   // ROAD-G G2: BOTH street pools, world.js:1644's line
  ...
  // pool the mode machine holds, which is world.js:2099's line.
  ...
  const cityGuards = createCityGuards({
    ...
    // own note describes. Written in world.js:2187's shape, so one pin
   ...
   *  the other two exterior hosts mount - world.js:2139 over the street
   ...
   *  it (world.js:2185-2269) and is its own slice; this pool does not
   *  wait on it. */
  ...
  const outdoorRestDeps = createRestDeps(playerEntity, {
    ...
    advanceMinutes: (n) => { playerTicker.advance(n); runEncounterTick(walkMode ? player.pos : cam.pos); },   // ROAD-G TAIL: the catch-up loop rides the rest's minutes, as world.js:3070 has it
   ...
   *  (effects.js:881/:895) - world.js:2327 hoisted its copy for exactly
  ...
  const magic = createPlayerMagic({
    ...
    // two engine-owning hosts wire the identical pair (world.js:2435-2436,
    // dungeonContext.js:1950-1951); `questBridge` is assigned below this
  ...
  const makeJournalWindow = (mode) => {
    ...
    // gate could never run. dungeonContext.js:1259-1261 is the shape:
  ...
  const questWorld = {
    ...
    // other. world.js:5564 is the same line.
     ...
     *  %rn/%rt's producer - world.js:5566-5579. */
    ...
    // QG1: CastSpellDo's two world reads, world.js:5482-5485's pair.
     ...
     *  mode-aware (worldModes:1258), so this is world.js:5455's line
     ...
     *  Bridge-only, no host state - world.js:5463's call verbatim. */
     ...
     *  Everything else is world.js:5567's arm term for term: the cast
  ...
  questBridge = createQuestBridge({
    ...
    // closure S40 gave this host, and world.js:6220's line.
    ...
    // encounter half is world.js:6142's line and the watch half is this
  ...
  // same order as world.js:6520/:6522, for the same reason: the
  ...
  // here, so the sink is handed down at this moment - world.js:6504's
  // line for this host.
  ...
  var modes = createWorldModes({
    ...
    buildingDirectory: () => ({
      ...
      // two inputs the world host uses (world.js:6618).
   ...
   *  session (enchantments.js:251-253) and every arm that folds it in
   ...
   *  second ctx here is the shape FS1 was written about. */
  {
     ...
     *  encounter mount above, and world.js:2682-2698 is the shape.
     ...
     *  world.js:2624-2625 does it. ROAD-G TAIL: a WATCHMAN transforms
  ...
  function frame(now) {
    ...
    // through the player for ever. world.js:7909-8028 is the shape.
    arrows.update(dt, {
      ...
      onPlayerArrowHitFoe: (m, t) => playerArrowHitFoe(m, t, {
        ...
        // (cityGuards.js:1005). DFU makes no pool distinction:
    ...
    if (waterOn) {
      renderer.drawWaterSurface(townWater, identityMatrix, renderer.tileArrays.get(groundArchive), tilemapTex, 6.4,
        waterUniforms({ seconds: now / 1000, wind: sky.wind(), rain: precipMode === 'rain' || precipMode === 'storm' ? fx.intensity : 0, sky: sky.waterSky(), debug: waterMaskDebug }),
        tilemapDim, renderer.waterArts.get(groundArchive));   // WATER-AUDIT: the town's own tilemap side, not 128; WATER4: the archive's art rides
    }
    ...
    if (walkMode && !tpMode) {
      ...
      for (const ev of weaponRig.frame(dt, { paralyzed })) {
        ...
        if (!cityGuards.resolvePlayerHit(weaponRig.playerWeapon, eye, fwd, player.pos, makeInView(proj, view, multiply), guardHitSound)) {
          ...
          // BEFORE civilians - world.js:8097's order, and the order

...
export async function bootExterior(canvas, renderer, params, status) {
  ...
  const magic = createPlayerMagic({
    ...
    // dungeonContext.js:2246-2247); `questBridge` is assigned below this
  ...
  var modes = createWorldModes({
    ...
    buildingDirectory: () => ({
      ...
      // two inputs the world host uses (world.js:12049).
  ...
  function frame(now) {
    ...
    // through the player for ever. world.js:13843-14082 is the shape.
    arrows.update(dt, {
    ...
    if (walkMode && !tpMode) {
      ...
      for (const ev of weaponRig.frame(dt, { paralyzed })) {
        ...
        if (!cityGuards.resolvePlayerHit(weaponRig.playerWeapon, eye, fwd, player.pos, makeInView(proj, view, multiply), guardHitSound, { swing })) {
          ...
          // BEFORE civilians - world.js:14192's order, and the order

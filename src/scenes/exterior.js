...
export async function bootExterior(canvas, renderer, params, status) {
  ...
  //     continue;`, world.js:2345) and stands them in batches of their
  ...
  function onExhaustedExterior() {
    ...
    try {
      const out = exhaustionOutcome({
        ...
        enemiesNearby: areEnemiesNearby([...(cityGuards?.guards ?? []), ...(exteriorFoes?.foes ?? [])]),   // ROAD-G G2: BOTH street pools, world.js:3294's line
  ...
  // pool the mode machine holds, which is world.js:3903's line.
  ...
  const cityGuards = createCityGuards({
    ...
    // own note describes. Written in world.js:4238's shape, so one pin
   ...
   *  the other two exterior hosts mount - world.js:3964 over the street
   ...
   *  it (world.js:4236-4446) and is its own slice; this pool does not
   *  wait on it. */
  ...
  const outdoorRestDeps = createRestDeps(playerEntity, {
    ...
    advanceMinutes: (n, sharedEnd) => { playerTicker.advance(n); runEncounterTick(walkMode ? player.pos : cam.pos, sharedEnd, true); },   // CAMP1-REST: every tick this drives IS a rest   // ROAD-G TAIL: the catch-up loop rides the rest's minutes, as world.js:5658 has it   // RESTX2: sharedEnd is the session's local sim-minutes online, so the roll still gets a fresh `now` while the real clock stands
   ...
   *  (effects.js:891/:905) - world.js:4806 hoisted its copy for exactly
  ...
  const magic = createPlayerMagic({
    ...
    // two engine-owning hosts wire the identical pair (world.js:4754-4755,
  ...
  const questWorld = {
    ...
    // other. world.js:9526 is the same line.
     ...
     *  %rn/%rt's producer - world.js:9422-9435. */
    ...
    // QG1: CastSpellDo's two world reads, world.js:9338-9341's pair.
     ...
     *  mode-aware (worldModes:1258), so this is world.js:9311's line
     ...
     *  Bridge-only, no host state - world.js:9319's call verbatim. */
     ...
     *  Everything else is world.js:9423's arm term for term: the cast
  ...
  questBridge = createQuestBridge({
    ...
    // closure S40 gave this host, and world.js:10086's line.
    ...
    // encounter half is world.js:10182's line and the watch half is this
  ...
  // same order as world.js:10396/:10401, for the same reason: the
  ...
  // here, so the sink is handed down at this moment - world.js:10380's
  // line for this host.
   ...
   *  encounter mount above, and world.js:4986-5002 is the shape.
   ...
   *  second ctx here is the shape FS1 was written about. */
  {
     ...
     *  world.js:4933-4934 does it. ROAD-G TAIL: a WATCHMAN transforms
  ...
  function frame(now) {
    ...
    // through the player for ever. world.js:15848-16099 is the shape.
    arrows.update(dt, {
    ...
    if (walkMode && !tpMode) {
      ...
      for (const ev of weaponRig.frame(dt, { paralyzed })) {
        ...
        if (!cityGuards.resolvePlayerHit(weaponRig.playerWeapon, eye, fwd, player.pos, makeInView(proj, view, multiply), guardHitSound, { swing })) {
          ...
          // BEFORE civilians - world.js:16218's order, and the order

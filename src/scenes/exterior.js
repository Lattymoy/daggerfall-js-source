...
export async function bootExterior(canvas, renderer, params, status) {
  ...
  function onExhaustedExterior() {
    ...
    try {
      const out = exhaustionOutcome({
        ...
        enemiesNearby: areEnemiesNearby([...(cityGuards?.guards ?? []), ...(exteriorFoes?.foes ?? [])]),   // ROAD-G G2: BOTH street pools, world.js:2043's line
  ...
  // pool the mode machine holds, which is world.js:2610's line.
   ...
   *  the other two exterior hosts mount - world.js:2671 over the street
  ...
  const outdoorRestDeps = createRestDeps(playerEntity, {
    ...
    advanceMinutes: (n, sharedEnd) => { playerTicker.advance(n); runEncounterTick(walkMode ? player.pos : cam.pos, sharedEnd, true); },   // CAMP1-REST: every tick this drives IS a rest   // ROAD-G TAIL: the catch-up loop rides the rest's minutes, as world.js:3987 has it   // RESTX2: sharedEnd is the session's local sim-minutes online, so the roll still gets a fresh `now` while the real clock stands
   ...
   *  (effects.js:881/:895) - world.js:3247 hoisted its copy for exactly
  ...
  const questWorld = {
    ...
    // other. world.js:7273 is the same line.
     ...
     *  %rn/%rt's producer - world.js:7172-7185. */
    ...
    // QG1: CastSpellDo's two world reads, world.js:7088-7091's pair.
     ...
     *  mode-aware (worldModes:1258), so this is world.js:7061's line
     ...
     *  Bridge-only, no host state - world.js:7069's call verbatim. */
     ...
     *  Everything else is world.js:7173's arm term for term: the cast
  ...
  questBridge = createQuestBridge({
    ...
    // closure S40 gave this host, and world.js:7827's line.
    ...
    // encounter half is world.js:7907's line and the watch half is this
  ...
  // same order as world.js:8128/:8133, for the same reason: the
  ...
  // here, so the sink is handed down at this moment - world.js:8112's
  // line for this host.
  ...
  var modes = createWorldModes({
    ...
    buildingDirectory: () => ({
      ...
      // two inputs the world host uses (world.js:9034).
   ...
   *  second ctx here is the shape FS1 was written about. */
  {
     ...
     *  encounter mount above, and world.js:3429-3445 is the shape.
     ...
     *  world.js:3359-3360 does it. ROAD-G TAIL: a WATCHMAN transforms
  ...
  function frame(now) {
    ...
    // through the player for ever. world.js:10617-10792 is the shape.
    arrows.update(dt, {
    ...
    if (walkMode && !tpMode) {
      ...
      for (const ev of weaponRig.frame(dt, { paralyzed })) {
        ...
        if (!cityGuards.resolvePlayerHit(weaponRig.playerWeapon, eye, fwd, player.pos, makeInView(proj, view, multiply), guardHitSound)) {
          ...
          // BEFORE civilians - world.js:10889's order, and the order

...
export async function bootExterior(canvas, renderer, params, status) {
  ...
  function onExhaustedExterior() {
    ...
    try {
      const out = exhaustionOutcome({
        ...
        enemiesNearby: areEnemiesNearby([...(cityGuards?.guards ?? []), ...(exteriorFoes?.foes ?? [])]),   // ROAD-G G2: BOTH street pools, world.js:2674's line
  ...
  // pool the mode machine holds, which is world.js:3271's line.
  ...
  const cityGuards = createCityGuards({
    ...
    // own note describes. Written in world.js:3605's shape, so one pin
   ...
   *  the other two exterior hosts mount - world.js:3332 over the street
   ...
   *  it (world.js:3603-3807) and is its own slice; this pool does not
   *  wait on it. */
  ...
  const outdoorRestDeps = createRestDeps(playerEntity, {
    ...
    advanceMinutes: (n, sharedEnd) => { playerTicker.advance(n); runEncounterTick(walkMode ? player.pos : cam.pos, sharedEnd, true); },   // CAMP1-REST: every tick this drives IS a rest   // ROAD-G TAIL: the catch-up loop rides the rest's minutes, as world.js:4999 has it   // RESTX2: sharedEnd is the session's local sim-minutes online, so the roll still gets a fresh `now` while the real clock stands
   ...
   *  (effects.js:887/:901) - world.js:4158 hoisted its copy for exactly
  ...
  const magic = createPlayerMagic({
    ...
    // two engine-owning hosts wire the identical pair (world.js:4106-4107,
  ...
  const questWorld = {
    ...
    // other. world.js:8634 is the same line.
     ...
     *  %rn/%rt's producer - world.js:8533-8546. */
    ...
    // QG1: CastSpellDo's two world reads, world.js:8449-8452's pair.
     ...
     *  mode-aware (worldModes:1258), so this is world.js:8422's line
     ...
     *  Bridge-only, no host state - world.js:8430's call verbatim. */
     ...
     *  Everything else is world.js:8534's arm term for term: the cast
  ...
  questBridge = createQuestBridge({
    ...
    // closure S40 gave this host, and world.js:9193's line.
    ...
    // encounter half is world.js:9284's line and the watch half is this
  ...
  // same order as world.js:9499/:9504, for the same reason: the
  ...
  // here, so the sink is handed down at this moment - world.js:9483's
  // line for this host.
   ...
   *  second ctx here is the shape FS1 was written about. */
  {
     ...
     *  world.js:4280-4281 does it. ROAD-G TAIL: a WATCHMAN transforms

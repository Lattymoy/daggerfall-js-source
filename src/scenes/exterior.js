...
export async function bootExterior(canvas, renderer, params, status) {
  ...
  //     continue;`, world.js:1372) and stands them in batches of their
  ...
  function onExhaustedExterior() {
    ...
    try {
      const out = exhaustionOutcome({
        ...
        enemiesNearby: areEnemiesNearby([...(cityGuards?.guards ?? []), ...(exteriorFoes?.foes ?? [])]),   // ROAD-G G2: BOTH street pools, world.js:2087's line
  ...
  // pool the mode machine holds, which is world.js:2655's line.
  ...
  const cityGuards = createCityGuards({
    ...
    // own note describes. Written in world.js:2886's shape, so one pin
   ...
   *  the other two exterior hosts mount - world.js:2716 over the street
   ...
   *  it (world.js:2884-3076) and is its own slice; this pool does not
   *  wait on it. */
  ...
  const outdoorRestDeps = createRestDeps(playerEntity, {
    ...
    advanceMinutes: (n, sharedEnd) => { playerTicker.advance(n); runEncounterTick(walkMode ? player.pos : cam.pos, sharedEnd, true); },   // CAMP1-REST: every tick this drives IS a rest   // ROAD-G TAIL: the catch-up loop rides the rest's minutes, as world.js:4220 has it   // RESTX2: sharedEnd is the session's local sim-minutes online, so the roll still gets a fresh `now` while the real clock stands
   ...
   *  (effects.js:881/:895) - world.js:3407 hoisted its copy for exactly
  ...
  const questWorld = {
    ...
    // other. world.js:7714 is the same line.
     ...
     *  %rn/%rt's producer - world.js:7613-7626. */
    ...
    // QG1: CastSpellDo's two world reads, world.js:7529-7532's pair.
     ...
     *  mode-aware (worldModes:1258), so this is world.js:7502's line
     ...
     *  Bridge-only, no host state - world.js:7510's call verbatim. */
     ...
     *  Everything else is world.js:7614's arm term for term: the cast
  ...
  questBridge = createQuestBridge({
    ...
    // closure S40 gave this host, and world.js:8273's line.
    ...
    // encounter half is world.js:8364's line and the watch half is this
  ...
  // same order as world.js:8579/:8584, for the same reason: the
  ...
  // here, so the sink is handed down at this moment - world.js:8563's
  // line for this host.
  ...
  var modes = createWorldModes({
    ...
    buildingDirectory: () => ({
      ...
      // two inputs the world host uses (world.js:10430).
   ...
   *  second ctx here is the shape FS1 was written about. */
  {
     ...
     *  encounter mount above, and world.js:3589-3605 is the shape.
     ...
     *  world.js:3519-3520 does it. ROAD-G TAIL: a WATCHMAN transforms
  ...
  function frame(now) {
    ...
    if (isEnhanced() && !weatherOverride) {
      const ds = distantStorms.tick({ systems: currentMapSystems(), at: fieldOfPixelLocal(_locPixel.x, _locPixel.y, eye[0], eye[2]), minutes: playerTicker.classicMinutes, seconds: now / 1000 });
    ...
    // through the player for ever. world.js:12111-12311 is the shape.
    arrows.update(dt, {
    ...
    if (walkMode && !tpMode) {
      ...
      for (const ev of weaponRig.frame(dt, { paralyzed })) {
        ...
        if (!cityGuards.resolvePlayerHit(weaponRig.playerWeapon, eye, fwd, player.pos, makeInView(proj, view, multiply), guardHitSound)) {
          ...
          // BEFORE civilians - world.js:12418's order, and the order

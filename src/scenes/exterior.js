...
export async function bootExterior(canvas, renderer, params, status) {
  ...
  //     continue;`, world.js:2354) and stands them in batches of their
  ...
  function onExhaustedExterior() {
    ...
    try {
      const out = exhaustionOutcome({
        ...
        enemiesNearby: areEnemiesNearby([...(cityGuards?.guards ?? []), ...(exteriorFoes?.foes ?? [])]),   // ROAD-G G2: BOTH street pools, world.js:3303's line
  ...
  // pool the mode machine holds, which is world.js:3912's line.
  ...
  const cityGuards = createCityGuards({
    ...
    // own note describes. Written in world.js:4247's shape, so one pin
   ...
   *  the other two exterior hosts mount - world.js:3973 over the street
   ...
   *  it (world.js:4245-4455) and is its own slice; this pool does not
   *  wait on it. */
  ...
  const outdoorRestDeps = createRestDeps(playerEntity, {
    ...
    advanceMinutes: (n, sharedEnd) => { playerTicker.advance(n); runEncounterTick(walkMode ? player.pos : cam.pos, sharedEnd, true); },   // CAMP1-REST: every tick this drives IS a rest   // ROAD-G TAIL: the catch-up loop rides the rest's minutes, as world.js:5667 has it   // RESTX2: sharedEnd is the session's local sim-minutes online, so the roll still gets a fresh `now` while the real clock stands
   ...
   *  (effects.js:891/:905) - world.js:4815 hoisted its copy for exactly
  ...
  const magic = createPlayerMagic({
    ...
    // two engine-owning hosts wire the identical pair (world.js:4763-4764,
  ...
  const questWorld = {
    ...
    // QG1: CastSpellDo's two world reads, world.js:9348-9351's pair.
     ...
     *  mode-aware (worldModes:1258), so this is world.js:9321's line
     ...
     *  Bridge-only, no host state - world.js:9329's call verbatim. */
  ...
  questBridge = createQuestBridge({
    ...
    // closure S40 gave this host, and world.js:10096's line.
    ...
    // encounter half is world.js:10192's line and the watch half is this
  ...
  var modes = createWorldModes({
    ...
    buildingDirectory: () => ({
      ...
      // two inputs the world host uses (world.js:13785).
   ...
   *  encounter mount above, and world.js:4995-5011 is the shape.
   ...
   *  second ctx here is the shape FS1 was written about. */
  {
     ...
     *  world.js:4942-4943 does it. ROAD-G TAIL: a WATCHMAN transforms
  ...
  function frame(now) {
    ...
    // through the player for ever. world.js:15716-15967 is the shape.
    arrows.update(dt, {
    ...
    if (walkMode && !tpMode) {
      ...
      for (const ev of weaponRig.frame(dt, { paralyzed })) {
        ...
        if (!cityGuards.resolvePlayerHit(weaponRig.playerWeapon, eye, fwd, player.pos, makeInView(proj, view, multiply), guardHitSound, { swing })) {
          ...
          // BEFORE civilians - world.js:16086's order, and the order

...
export async function bootExterior(canvas, renderer, params, status) {
  ...
  //     continue;`, world.js:1057) and stands them in batches of their
  ...
  function onExhaustedExterior() {
    ...
    try {
      const out = exhaustionOutcome({
        ...
        enemiesNearby: areEnemiesNearby([...(cityGuards?.guards ?? []), ...(exteriorFoes?.foes ?? [])]),   // ROAD-G G2: BOTH street pools, world.js:1652's line
  ...
  // pool the mode machine holds, which is world.js:2107's line.
  ...
  const cityGuards = createCityGuards({
    ...
    // own note describes. Written in world.js:2195's shape, so one pin
   ...
   *  the other two exterior hosts mount - world.js:2147 over the street
   ...
   *  it (world.js:2193-2278) and is its own slice; this pool does not
   *  wait on it. */
  ...
  const outdoorRestDeps = createRestDeps(playerEntity, {
    ...
    advanceMinutes: (n) => { playerTicker.advance(n); runEncounterTick(walkMode ? player.pos : cam.pos); },   // ROAD-G TAIL: the catch-up loop rides the rest's minutes, as world.js:3079 has it
   ...
   *  (effects.js:881/:895) - world.js:2336 hoisted its copy for exactly
  ...
  const magic = createPlayerMagic({
    ...
    // two engine-owning hosts wire the identical pair (world.js:2444-2445,
    // dungeonContext.js:1997-1998); `questBridge` is assigned below this
  ...
  const makeJournalWindow = (mode) => {
    ...
    // gate could never run. dungeonContext.js:1297-1299 is the shape:
  ...
  const questWorld = {
    ...
    // other. world.js:5598 is the same line.
     ...
     *  %rn/%rt's producer - world.js:5600-5613. */
    ...
    // QG1: CastSpellDo's two world reads, world.js:5516-5519's pair.
     ...
     *  mode-aware (worldModes:1258), so this is world.js:5489's line
     ...
     *  Bridge-only, no host state - world.js:5497's call verbatim. */
     ...
     *  Everything else is world.js:5601's arm term for term: the cast
  ...
  questBridge = createQuestBridge({
    ...
    questClocksStoodDown: () => sharedClockOn(),   // AUDIT WORLD5 C10: the same word as world.js's - the bridge's fallback is false, and a host that says nothing charges every clock
    ...
    // closure S40 gave this host, and world.js:6254's line.
    ...
    // encounter half is world.js:6176's line and the watch half is this
  ...
  // same order as world.js:6555/:6555, for the same reason: the
  ...
  // here, so the sink is handed down at this moment - world.js:6539's
  // line for this host.
  ...
  var modes = createWorldModes({
    ...
    buildingDirectory: () => ({
      ...
      // two inputs the world host uses (world.js:6943).
   ...
   *  second ctx here is the shape FS1 was written about. */
  {
     ...
     *  encounter mount above, and world.js:2691-2707 is the shape.
     ...
     *  world.js:2630-2631 does it. ROAD-G TAIL: a WATCHMAN transforms
  ...
  function frame(now) {
    ...
    // through the player for ever. world.js:8242-8361 is the shape.
    arrows.update(dt, {
    ...
    if (walkMode && !tpMode) {
      ...
      for (const ev of weaponRig.frame(dt, { paralyzed })) {
        ...
        if (!cityGuards.resolvePlayerHit(weaponRig.playerWeapon, eye, fwd, player.pos, makeInView(proj, view, multiply), guardHitSound)) {
          ...
          // BEFORE civilians - world.js:8430's order, and the order

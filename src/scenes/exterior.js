...
export async function bootExterior(canvas, renderer, params, status) {
  ...
  //     continue;`, world.js:1085) and stands them in batches of their
  ...
  function onExhaustedExterior() {
    ...
    try {
      const out = exhaustionOutcome({
        ...
        enemiesNearby: areEnemiesNearby([...(cityGuards?.guards ?? []), ...(exteriorFoes?.foes ?? [])]),   // ROAD-G G2: BOTH street pools, world.js:1698's line
  ...
  // pool the mode machine holds, which is world.js:2193's line.
  ...
  const cityGuards = createCityGuards({
    ...
    // own note describes. Written in world.js:2292's shape, so one pin
   ...
   *  the other two exterior hosts mount - world.js:2233 over the street
   ...
   *  it (world.js:2290-2372) and is its own slice; this pool does not
   *  wait on it. */
  ...
  const outdoorRestDeps = createRestDeps(playerEntity, {
    ...
    advanceMinutes: (n) => { playerTicker.advance(n); runEncounterTick(walkMode ? player.pos : cam.pos); },   // ROAD-G TAIL: the catch-up loop rides the rest's minutes, as world.js:3179 has it
   ...
   *  (effects.js:881/:895) - world.js:2604 hoisted its copy for exactly
  ...
  const magic = createPlayerMagic({
    ...
    // two engine-owning hosts wire the identical pair (world.js:2538-2539,
  ...
  const questWorld = {
    ...
    // other. world.js:5848 is the same line.
     ...
     *  %rn/%rt's producer - world.js:5747-5760. */
    ...
    // QG1: CastSpellDo's two world reads, world.js:5663-5666's pair.
     ...
     *  mode-aware (worldModes:1258), so this is world.js:5636's line
     ...
     *  Bridge-only, no host state - world.js:5644's call verbatim. */
     ...
     *  Everything else is world.js:5748's arm term for term: the cast
  ...
  questBridge = createQuestBridge({
    ...
    // closure S40 gave this host, and world.js:6401's line.
    ...
    // encounter half is world.js:6481's line and the watch half is this
  ...
  // same order as world.js:6702/:6707, for the same reason: the
  ...
  // here, so the sink is handed down at this moment - world.js:6686's
  // line for this host.
  ...
  var modes = createWorldModes({
    ...
    buildingDirectory: () => ({
      ...
      // two inputs the world host uses (world.js:7268).
   ...
   *  second ctx here is the shape FS1 was written about. */
  {
     ...
     *  encounter mount above, and world.js:2786-2802 is the shape.
     ...
     *  world.js:2725-2726 does it. ROAD-G TAIL: a WATCHMAN transforms
  ...
  function frame(now) {
    ...
    // through the player for ever. world.js:8624-8744 is the shape.
    arrows.update(dt, {
    ...
    if (walkMode && !tpMode) {
      ...
      for (const ev of weaponRig.frame(dt, { paralyzed })) {
        ...
        if (!cityGuards.resolvePlayerHit(weaponRig.playerWeapon, eye, fwd, player.pos, makeInView(proj, view, multiply), guardHitSound)) {
          ...
          // BEFORE civilians - world.js:8813's order, and the order

...
import { spellRecordOfIndex } from '../systems/loot.js';   // QG1: CastSpellDo's classic-record read (the G4 registry) - world.js:179's import
...
export async function bootExterior(canvas, renderer, params, status) {
  ...
  //     continue;`, world.js:1381) and stands them in batches of their
  ...
  function onExhaustedExterior() {
    ...
    try {
      const out = exhaustionOutcome({
        ...
        enemiesNearby: areEnemiesNearby([...(cityGuards?.guards ?? []), ...(exteriorFoes?.foes ?? [])]),   // ROAD-G G2: BOTH street pools, world.js:2096's line
   ...
   *  it (world.js:2995-3077) and is its own slice; this pool does not
   *  wait on it. */
   ...
   *  (effects.js:881/:895) - world.js:3404 hoisted its copy for exactly
  ...
  const magic = createPlayerMagic({
    ...
    // two engine-owning hosts wire the identical pair (world.js:3309-3310,
    // dungeonContext.js:2151-2152); `questBridge` is assigned below this
  ...
  const makeJournalWindow = (mode) => {
    ...
    // gate could never run. dungeonContext.js:1397-1402 is the shape:
  ...
  const questWorld = {
    ...
    // other. world.js:7441 is the same line.
     ...
     *  %rn/%rt's producer - world.js:7340-7353. */
    ...
    // QG1: CastSpellDo's two world reads, world.js:7256-7259's pair.
     ...
     *  mode-aware (worldModes:1258), so this is world.js:7229's line
     ...
     *  Bridge-only, no host state - world.js:7237's call verbatim. */
     ...
     *  Everything else is world.js:7341's arm term for term: the cast
  ...
  questBridge = createQuestBridge({
    ...
    // closure S40 gave this host, and world.js:7995's line.
    ...
    // encounter half is world.js:8075's line and the watch half is this
  ...
  // same order as world.js:8296/:8301, for the same reason: the
  ...
  // here, so the sink is handed down at this moment - world.js:8280's
  // line for this host.
  ...
  var modes = createWorldModes({
    ...
    buildingDirectory: () => ({
      ...
      // two inputs the world host uses (world.js:9202).
   ...
   *  second ctx here is the shape FS1 was written about. */
  {
     ...
     *  encounter mount above, and world.js:3586-3602 is the shape.
     ...
     *  world.js:3516-3517 does it. ROAD-G TAIL: a WATCHMAN transforms
  ...
  function frame(now) {
    ...
    // through the player for ever. world.js:10820-11020 is the shape.
    arrows.update(dt, {
    ...
    if (walkMode && !tpMode) {
      ...
      for (const ev of weaponRig.frame(dt, { paralyzed })) {
        ...
        if (!cityGuards.resolvePlayerHit(weaponRig.playerWeapon, eye, fwd, player.pos, makeInView(proj, view, multiply), guardHitSound)) {
          ...
          // BEFORE civilians - world.js:11127's order, and the order

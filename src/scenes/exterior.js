...
export async function bootExterior(canvas, renderer, params, status) {
  ...
  const magic = createPlayerMagic({
    ...
    // dungeonContext.js:1986-1987); `questBridge` is assigned below this
  ...
  var modes = createWorldModes({
    ...
    buildingDirectory: () => ({
      ...
      // two inputs the world host uses (world.js:6854).
  ...
  function frame(now) {
    ...
    // through the player for ever. world.js:8153-8272 is the shape.
    arrows.update(dt, {
    ...
    if (walkMode && !tpMode) {
      ...
      for (const ev of weaponRig.frame(dt, { paralyzed })) {
        ...
        if (!cityGuards.resolvePlayerHit(weaponRig.playerWeapon, eye, fwd, player.pos, makeInView(proj, view, multiply), guardHitSound)) {
          ...
          // BEFORE civilians - world.js:8341's order, and the order

...
export async function bootExterior(canvas, renderer, params, status) {
  ...
  const cityGuards = createCityGuards({
    ...
    // own note describes. Written in world.js:2778's shape, so one pin
   ...
   *  it (world.js:2776-2858) and is its own slice; this pool does not
   *  wait on it. */
  ...
  const outdoorRestDeps = createRestDeps(playerEntity, {
    ...
    advanceMinutes: (n, sharedEnd) => { playerTicker.advance(n); runEncounterTick(walkMode ? player.pos : cam.pos, sharedEnd, true); },   // CAMP1-REST: every tick this drives IS a rest   // ROAD-G TAIL: the catch-up loop rides the rest's minutes, as world.js:3921 has it   // RESTX2: sharedEnd is the session's local sim-minutes online, so the roll still gets a fresh `now` while the real clock stands
   ...
   *  (effects.js:881/:895) - world.js:3185 hoisted its copy for exactly
  ...
  const magic = createPlayerMagic({
    ...
    // two engine-owning hosts wire the identical pair (world.js:3090-3091,
  ...
  const questWorld = {
    ...
    // other. world.js:7184 is the same line.
     ...
     *  %rn/%rt's producer - world.js:7083-7096. */
    ...
    // QG1: CastSpellDo's two world reads, world.js:6999-7002's pair.
     ...
     *  mode-aware (worldModes:1258), so this is world.js:6972's line
     ...
     *  Bridge-only, no host state - world.js:6980's call verbatim. */
     ...
     *  Everything else is world.js:7084's arm term for term: the cast
  ...
  questBridge = createQuestBridge({
    ...
    // closure S40 gave this host, and world.js:7737's line.
    ...
    // encounter half is world.js:7817's line and the watch half is this
  ...
  // same order as world.js:8038/:8043, for the same reason: the
  ...
  // here, so the sink is handed down at this moment - world.js:8022's
  // line for this host.
  ...
  var modes = createWorldModes({
    ...
    buildingDirectory: () => ({
      ...
      // two inputs the world host uses (world.js:8919).
   ...
   *  second ctx here is the shape FS1 was written about. */
  {
     ...
     *  encounter mount above, and world.js:3367-3383 is the shape.
     ...
     *  world.js:3297-3298 does it. ROAD-G TAIL: a WATCHMAN transforms
  ...
  function frame(now) {
    ...
    // through the player for ever. world.js:10489-10663 is the shape.
    arrows.update(dt, {
    ...
    if (walkMode && !tpMode) {
      ...
      for (const ev of weaponRig.frame(dt, { paralyzed })) {
        ...
        if (!cityGuards.resolvePlayerHit(weaponRig.playerWeapon, eye, fwd, player.pos, makeInView(proj, view, multiply), guardHitSound)) {
          ...
          // BEFORE civilians - world.js:10759's order, and the order

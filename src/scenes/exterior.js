...
export async function bootExterior(canvas, renderer, params, status) {
  ...
  function runEncounterTick(playerFeet, simMinutesEnd = null) {
    ...
    for (let l = 0; l < span; l++) {
      ...
      if (getPref('wildernessCamps') !== false) {
  ...
  const outdoorRestDeps = createRestDeps(playerEntity, {
    ...
    advanceMinutes: (n, sharedEnd) => { playerTicker.advance(n); runEncounterTick(walkMode ? player.pos : cam.pos, sharedEnd); },   // ROAD-G TAIL: the catch-up loop rides the rest's minutes, as world.js:3499 has it   // RESTX2: sharedEnd is the session's local sim-minutes online, so the roll still gets a fresh `now` while the real clock stands

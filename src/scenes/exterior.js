...
export async function bootExterior(canvas, renderer, params, status) {
  ...
  //     continue;`, world.js:1190) and stands them in batches of their
  ...
  function onExhaustedExterior() {
    ...
    try {
      const out = exhaustionOutcome({
        ...
        enemiesNearby: areEnemiesNearby([...(cityGuards?.guards ?? []), ...(exteriorFoes?.foes ?? [])]),   // ROAD-G G2: BOTH street pools, world.js:1842's line
  ...
  // pool the mode machine holds, which is world.js:2407's line.
  ...
  const cityGuards = createCityGuards({
    ...
    // own note describes. Written in world.js:2576's shape, so one pin
   ...
   *  the other two exterior hosts mount - world.js:2459 over the street
   ...
   *  it (world.js:2574-2656) and is its own slice; this pool does not
   *  wait on it. */
  ...
  const outdoorRestDeps = createRestDeps(playerEntity, {
    ...
    advanceMinutes: (n, sharedEnd) => { playerTicker.advance(n); runEncounterTick(walkMode ? player.pos : cam.pos, sharedEnd, true); },   // CAMP1-REST: every tick this drives IS a rest   // ROAD-G TAIL: the catch-up loop rides the rest's minutes, as world.js:3692 has it   // RESTX2: sharedEnd is the session's local sim-minutes online, so the roll still gets a fresh `now` while the real clock stands
   ...
   *  (effects.js:881/:895) - world.js:2967 hoisted its copy for exactly
  ...
  const magic = createPlayerMagic({
    ...
    // two engine-owning hosts wire the identical pair (world.js:2880-2881,
  ...
  const questWorld = {
    ...
    // other. world.js:6800 is the same line.
     ...
     *  %rn/%rt's producer - world.js:6699-6712. */
    ...
    // QG1: CastSpellDo's two world reads, world.js:6615-6618's pair.
     ...
     *  mode-aware (worldModes:1258), so this is world.js:6588's line
     ...
     *  Bridge-only, no host state - world.js:6596's call verbatim. */
     ...
     *  Everything else is world.js:6700's arm term for term: the cast
  ...
  questBridge = createQuestBridge({
    ...
    // closure S40 gave this host, and world.js:7353's line.
    ...
    // encounter half is world.js:7433's line and the watch half is this
  ...
  // same order as world.js:7654/:7659, for the same reason: the
  ...
  // here, so the sink is handed down at this moment - world.js:7638's
  // line for this host.
  ...
  var modes = createWorldModes({
    ...
    buildingDirectory: () => ({
      ...
      // two inputs the world host uses (world.js:8477).
   ...
   *  second ctx here is the shape FS1 was written about. */
  {
     ...
     *  encounter mount above, and world.js:3149-3165 is the shape.
     ...
     *  world.js:3088-3089 does it. ROAD-G TAIL: a WATCHMAN transforms
  ...
  function frame(now) {
    ...
    // through the player for ever. world.js:10014-10135 is the shape.
    arrows.update(dt, {
    ...
    if (walkMode && !tpMode) {
      ...
      for (const ev of weaponRig.frame(dt, { paralyzed })) {
        ...
        if (!cityGuards.resolvePlayerHit(weaponRig.playerWeapon, eye, fwd, player.pos, makeInView(proj, view, multiply), guardHitSound)) {
          ...
          // BEFORE civilians - world.js:10208's order, and the order

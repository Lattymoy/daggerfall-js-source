...
import { actionOf, held, moveHeld, anyMove, swallowBrowserKey, mouseCode, isSwingButton, keyboardLook, routeAction, installContextMenuGuard } from '../ui/input.js';
...
export async function bootExterior(canvas, renderer, params, status) {
  ...
  const outdoorRestDeps = createRestDeps(playerEntity, {
    ...
    advanceMinutes: (n) => { playerTicker.advance(n); runEncounterTick(walkMode ? player.pos : cam.pos); },   // ROAD-G TAIL: the catch-up loop rides the rest's minutes, as world.js:3177 has it
   ...
   *  (effects.js:881/:895) - world.js:2602 hoisted its copy for exactly
  ...
  const questWorld = {
    ...
    // other. world.js:5846 is the same line.
     ...
     *  %rn/%rt's producer - world.js:5745-5758. */
    ...
    // QG1: CastSpellDo's two world reads, world.js:5661-5664's pair.
     ...
     *  mode-aware (worldModes:1258), so this is world.js:5634's line
     ...
     *  Bridge-only, no host state - world.js:5642's call verbatim. */
     ...
     *  Everything else is world.js:5746's arm term for term: the cast
  ...
  questBridge = createQuestBridge({
    ...
    // closure S40 gave this host, and world.js:6399's line.
    ...
    // encounter half is world.js:6479's line and the watch half is this
  ...
  // same order as world.js:6700/:6705, for the same reason: the
  ...
  // here, so the sink is handed down at this moment - world.js:6684's
  // line for this host.
  ...
  var modes = createWorldModes({
    ...
    buildingDirectory: () => ({
      ...
      // two inputs the world host uses (world.js:7201).
   ...
   *  second ctx here is the shape FS1 was written about. */
  {
     ...
     *  encounter mount above, and world.js:2784-2800 is the shape.
     ...
     *  world.js:2723-2724 does it. ROAD-G TAIL: a WATCHMAN transforms
  ...
  function frame(now) {
    ...
    // through the player for ever. world.js:8557-8677 is the shape.
    arrows.update(dt, {
    ...
    if (walkMode && !tpMode) {
      ...
      for (const ev of weaponRig.frame(dt, { paralyzed })) {
        ...
        if (!cityGuards.resolvePlayerHit(weaponRig.playerWeapon, eye, fwd, player.pos, makeInView(proj, view, multiply), guardHitSound)) {
          ...
          // BEFORE civilians - world.js:8746's order, and the order

...
import { spellRecordOfIndex } from '../systems/loot.js';   // QG1: CastSpellDo's classic-record read (the G4 registry) - world.js:129's import
...
import { armUnloadGuard } from '../systems/unloadGuard.js';   // MAC-L3: one door in front of every way out of a running game   // I2: the rebindable registry; AUDIT 39r: the mouse half of the held set
...
export async function bootExterior(canvas, renderer, params, status) {
  ...
  //     continue;`, world.js:1078) and stands them in batches of their
  ...
  function onExhaustedExterior() {
    ...
    try {
      const out = exhaustionOutcome({
        ...
        enemiesNearby: areEnemiesNearby([...(cityGuards?.guards ?? []), ...(exteriorFoes?.foes ?? [])]),   // ROAD-G G2: BOTH street pools, world.js:1691's line
  ...
  // pool the mode machine holds, which is world.js:2166's line.
  ...
  const cityGuards = createCityGuards({
    ...
    // own note describes. Written in world.js:2265's shape, so one pin
   ...
   *  the other two exterior hosts mount - world.js:2206 over the street
   ...
   *  it (world.js:2263-2345) and is its own slice; this pool does not
   *  wait on it. */
  ...
  const outdoorRestDeps = createRestDeps(playerEntity, {
    ...
    advanceMinutes: (n) => { playerTicker.advance(n); runEncounterTick(walkMode ? player.pos : cam.pos); },   // ROAD-G TAIL: the catch-up loop rides the rest's minutes, as world.js:3152 has it
   ...
   *  (effects.js:881/:895) - world.js:2577 hoisted its copy for exactly
  ...
  const magic = createPlayerMagic({
    ...
    // two engine-owning hosts wire the identical pair (world.js:2511-2512,
    // dungeonContext.js:2002-2003); `questBridge` is assigned below this
  ...
  const makeJournalWindow = (mode) => {
    ...
    // gate could never run. dungeonContext.js:1290-1292 is the shape:
  ...
  const questWorld = {
    ...
    // other. world.js:5808 is the same line.
     ...
     *  %rn/%rt's producer - world.js:5707-5720. */
    ...
    // QG1: CastSpellDo's two world reads, world.js:5623-5626's pair.
     ...
     *  mode-aware (worldModes:1258), so this is world.js:5596's line
     ...
     *  Bridge-only, no host state - world.js:5604's call verbatim. */
     ...
     *  Everything else is world.js:5708's arm term for term: the cast
  ...
  questBridge = createQuestBridge({
    ...
    // closure S40 gave this host, and world.js:6361's line.
    ...
    // encounter half is world.js:6441's line and the watch half is this
  ...
  // same order as world.js:6662/:6667, for the same reason: the
  ...
  // here, so the sink is handed down at this moment - world.js:6646's
  // line for this host.
  ...
  var modes = createWorldModes({
    ...
    buildingDirectory: () => ({
      ...
      // two inputs the world host uses (world.js:7109).
   ...
   *  second ctx here is the shape FS1 was written about. */
  {
     ...
     *  encounter mount above, and world.js:2759-2775 is the shape.
     ...
     *  world.js:2698-2699 does it. ROAD-G TAIL: a WATCHMAN transforms
  ...
  function frame(now) {
    ...
    // through the player for ever. world.js:8450-8570 is the shape.
    arrows.update(dt, {
    ...
    if (walkMode && !tpMode) {
      ...
      for (const ev of weaponRig.frame(dt, { paralyzed })) {
        ...
        if (!cityGuards.resolvePlayerHit(weaponRig.playerWeapon, eye, fwd, player.pos, makeInView(proj, view, multiply), guardHitSound)) {
          ...
          // BEFORE civilians - world.js:8639's order, and the order

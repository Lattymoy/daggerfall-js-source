...
export async function bootExterior(canvas, renderer, params, status) {
  ...
  const magic = createPlayerMagic({
    ...
    // two engine-owning hosts wire the identical pair (world.js:3282-3283,
    // dungeonContext.js:2213-2214); `questBridge` is assigned below this
  ...
  const makeJournalWindow = (mode) => {
    ...
    // gate could never run. dungeonContext.js:1404-1409 is the shape:
  ...
  const questWorld = {
    ...
    // other. world.js:7524 is the same line.
     ...
     *  %rn/%rt's producer - world.js:7423-7436. */
    ...
    // QG1: CastSpellDo's two world reads, world.js:7336-7339's pair.
     ...
     *  mode-aware (worldModes:1258), so this is world.js:7309's line
     ...
     *  Bridge-only, no host state - world.js:7317's call verbatim. */
     ...
     *  Everything else is world.js:7424's arm term for term: the cast
  ...
  questBridge = createQuestBridge({
    ...
    // closure S40 gave this host, and world.js:8087's line.
    ...
    // encounter half is world.js:8167's line and the watch half is this
  ...
  // same order as world.js:8388/:8393, for the same reason: the
  ...
  // here, so the sink is handed down at this moment - world.js:8372's
  // line for this host.
  ...
  var modes = createWorldModes({
    ...
    buildingDirectory: () => ({
      ...
      // two inputs the world host uses (world.js:9341).
  ...
  function frame(now) {
    ...
    // through the player for ever. world.js:10980-11180 is the shape.
    arrows.update(dt, {
    ...
    if (walkMode && !tpMode) {
      ...
      for (const ev of weaponRig.frame(dt, { paralyzed })) {
        ...
        if (!cityGuards.resolvePlayerHit(weaponRig.playerWeapon, eye, fwd, player.pos, makeInView(proj, view, multiply), guardHitSound)) {
          ...
          // BEFORE civilians - world.js:11287's order, and the order

...
export async function bootExterior(canvas, renderer, params, status) {
  ...
  //     continue;`, world.js:1095) and stands them in batches of their
  ...
  function onExhaustedExterior() {
    ...
    try {
      const out = exhaustionOutcome({
        ...
        enemiesNearby: areEnemiesNearby([...(cityGuards?.guards ?? []), ...(exteriorFoes?.foes ?? [])]),   // ROAD-G G2: BOTH street pools, world.js:1708's line
  ...
  // pool the mode machine holds, which is world.js:2207's line.
  ...
  const cityGuards = createCityGuards({
    ...
    // own note describes. Written in world.js:2306's shape, so one pin
   ...
   *  the other two exterior hosts mount - world.js:2247 over the street
   ...
   *  it (world.js:2304-2386) and is its own slice; this pool does not
   *  wait on it. */
  ...
  const outdoorRestDeps = createRestDeps(playerEntity, {
    ...
    advanceMinutes: (n) => { playerTicker.advance(n); runEncounterTick(walkMode ? player.pos : cam.pos); },   // ROAD-G TAIL: the catch-up loop rides the rest's minutes, as world.js:3251 has it
   ...
   *  (effects.js:881/:895) - world.js:2639 hoisted its copy for exactly
  ...
  const magic = createPlayerMagic({
    ...
    // two engine-owning hosts wire the identical pair (world.js:2552-2553,
  ...
  const questWorld = {
    ...
    // other. world.js:5972 is the same line.
     ...
     *  %rn/%rt's producer - world.js:5871-5884. */
    ...
    // QG1: CastSpellDo's two world reads, world.js:5787-5790's pair.
     ...
     *  mode-aware (worldModes:1258), so this is world.js:5760's line
     ...
     *  Bridge-only, no host state - world.js:5768's call verbatim. */
     ...
     *  Everything else is world.js:5872's arm term for term: the cast
  ...
  questBridge = createQuestBridge({
    ...
    // closure S40 gave this host, and world.js:6525's line.
    ...
    // encounter half is world.js:6605's line and the watch half is this
  ...
  // same order as world.js:6826/:6831, for the same reason: the
  ...
  // here, so the sink is handed down at this moment - world.js:6810's
  // line for this host.
  ...
  var modes = createWorldModes({
    ...
    buildingDirectory: () => ({
      ...
      // two inputs the world host uses (world.js:7605).
   ...
   *  second ctx here is the shape FS1 was written about. */
  {
     ...
     *  encounter mount above, and world.js:2821-2837 is the shape.
     ...
     *  world.js:2760-2761 does it. ROAD-G TAIL: a WATCHMAN transforms
  ...
  function frame(now) {
    ...
    // through the player for ever. world.js:8986-9106 is the shape.
    arrows.update(dt, {
    ...
    if (walkMode && !tpMode) {
      ...
      for (const ev of weaponRig.frame(dt, { paralyzed })) {
        ...
        if (!cityGuards.resolvePlayerHit(weaponRig.playerWeapon, eye, fwd, player.pos, makeInView(proj, view, multiply), guardHitSound)) {
          ...
          // BEFORE civilians - world.js:9175's order, and the order

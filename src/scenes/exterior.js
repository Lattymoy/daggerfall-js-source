...
export async function bootExterior(canvas, renderer, params, status) {
  ...
  // pool the mode machine holds, which is world.js:2669's line.
  ...
  const cityGuards = createCityGuards({
    ...
    // own note describes. Written in world.js:2994's shape, so one pin
   ...
   *  the other two exterior hosts mount - world.js:2730 over the street
   ...
   *  it (world.js:2992-3196) and is its own slice; this pool does not
   *  wait on it. */
   ...
   *  (effects.js:887/:901) - world.js:3527 hoisted its copy for exactly
  ...
  const magic = createPlayerMagic({
    ...
    // two engine-owning hosts wire the identical pair (world.js:3475-3476,
    // dungeonContext.js:2229-2230); `questBridge` is assigned below this
  ...
  const makeJournalWindow = (mode) => {
    ...
    // gate could never run. dungeonContext.js:1444-1449 is the shape:
  ...
  const questWorld = {
    ...
    // other. world.js:7883 is the same line.
     ...
     *  %rn/%rt's producer - world.js:7782-7795. */
    ...
    // QG1: CastSpellDo's two world reads, world.js:7698-7701's pair.
     ...
     *  mode-aware (worldModes:1258), so this is world.js:7671's line
     ...
     *  Bridge-only, no host state - world.js:7679's call verbatim. */
     ...
     *  Everything else is world.js:7783's arm term for term: the cast
  ...
  questBridge = createQuestBridge({
    ...
    // closure S40 gave this host, and world.js:8442's line.
    ...
    // encounter half is world.js:8533's line and the watch half is this
  ...
  // same order as world.js:8748/:8753, for the same reason: the
  ...
  // here, so the sink is handed down at this moment - world.js:8732's
  // line for this host.
  ...
  var modes = createWorldModes({
    ...
    buildingDirectory: () => ({
      ...
      // two inputs the world host uses (world.js:10910).
   ...
   *  second ctx here is the shape FS1 was written about. */
  {
     ...
     *  encounter mount above, and world.js:3717-3733 is the shape.
     ...
     *  world.js:3647-3648 does it. ROAD-G TAIL: a WATCHMAN transforms
  ...
  function frame(now) {
    ...
    // through the player for ever. world.js:12619-12848 is the shape.
    arrows.update(dt, {
    ...
    if (walkMode && !tpMode) {
      ...
      for (const ev of weaponRig.frame(dt, { paralyzed })) {
        ...
        if (!cityGuards.resolvePlayerHit(weaponRig.playerWeapon, eye, fwd, player.pos, makeInView(proj, view, multiply), guardHitSound)) {
          ...
          // BEFORE civilians - world.js:12958's order, and the order

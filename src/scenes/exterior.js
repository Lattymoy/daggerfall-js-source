...
import { spellRecordOfIndex } from '../systems/loot.js';   // QG1: CastSpellDo's classic-record read (the G4 registry) - world.js:177's import
...
export async function bootExterior(canvas, renderer, params, status) {
  ...
  const cityGuards = createCityGuards({
    ...
    // own note describes. Written in world.js:2959's shape, so one pin
   ...
   *  it (world.js:2957-3039) and is its own slice; this pool does not
   *  wait on it. */
  ...
  const outdoorRestDeps = createRestDeps(playerEntity, {
    ...
    advanceMinutes: (n, sharedEnd) => { playerTicker.advance(n); runEncounterTick(walkMode ? player.pos : cam.pos, sharedEnd, true); },   // CAMP1-REST: every tick this drives IS a rest   // ROAD-G TAIL: the catch-up loop rides the rest's minutes, as world.js:4106 has it   // RESTX2: sharedEnd is the session's local sim-minutes online, so the roll still gets a fresh `now` while the real clock stands
   ...
   *  (effects.js:881/:895) - world.js:3366 hoisted its copy for exactly
  ...
  const magic = createPlayerMagic({
    ...
    // two engine-owning hosts wire the identical pair (world.js:3271-3272,
  ...
  const questWorld = {
    ...
    // other. world.js:7392 is the same line.
     ...
     *  %rn/%rt's producer - world.js:7291-7304. */
    ...
    // QG1: CastSpellDo's two world reads, world.js:7207-7210's pair.
     ...
     *  mode-aware (worldModes:1258), so this is world.js:7180's line
     ...
     *  Bridge-only, no host state - world.js:7188's call verbatim. */
     ...
     *  Everything else is world.js:7292's arm term for term: the cast
  ...
  questBridge = createQuestBridge({
    ...
    // closure S40 gave this host, and world.js:7946's line.
    ...
    // encounter half is world.js:8026's line and the watch half is this
  ...
  // same order as world.js:8247/:8252, for the same reason: the
  ...
  // here, so the sink is handed down at this moment - world.js:8231's
  // line for this host.
  ...
  var modes = createWorldModes({
    ...
    buildingDirectory: () => ({
      ...
      // two inputs the world host uses (world.js:9153).
   ...
   *  second ctx here is the shape FS1 was written about. */
  {
     ...
     *  encounter mount above, and world.js:3548-3564 is the shape.
     ...
     *  world.js:3478-3479 does it. ROAD-G TAIL: a WATCHMAN transforms

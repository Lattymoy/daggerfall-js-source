...
import { spellRecordOfIndex } from '../systems/loot.js';   // QG1: CastSpellDo's classic-record read (the G4 registry) - world.js:130's import
...
export async function bootExterior(canvas, renderer, params, status) {
  ...
  //     host's static NPCs take it (interiorContext.js:397-415).
  ...
  //     continue;`, world.js:1089) and stands them in batches of their
  ...
  function onExhaustedExterior() {
    ...
    try {
      const out = exhaustionOutcome({
        ...
        enemiesNearby: areEnemiesNearby([...(cityGuards?.guards ?? []), ...(exteriorFoes?.foes ?? [])]),   // ROAD-G G2: BOTH street pools, world.js:1702's line
  ...
  // pool the mode machine holds, which is world.js:2197's line.
  ...
  const cityGuards = createCityGuards({
    ...
    // own note describes. Written in world.js:2296's shape, so one pin
   ...
   *  the other two exterior hosts mount - world.js:2237 over the street
   ...
   *  it (world.js:2294-2376) and is its own slice; this pool does not
   *  wait on it. */
  ...
  const outdoorRestDeps = createRestDeps(playerEntity, {
    ...
    advanceMinutes: (n) => { playerTicker.advance(n); runEncounterTick(walkMode ? player.pos : cam.pos); },   // ROAD-G TAIL: the catch-up loop rides the rest's minutes, as world.js:3204 has it
   ...
   *  (effects.js:881/:895) - world.js:2629 hoisted its copy for exactly
  ...
  const magic = createPlayerMagic({
    ...
    // two engine-owning hosts wire the identical pair (world.js:2542-2543,
    // dungeonContext.js:2003-2004); `questBridge` is assigned below this
  ...
  const makeJournalWindow = (mode) => {
    ...
    // gate could never run. dungeonContext.js:1291-1293 is the shape:
  ...
  const questWorld = {
    ...
    // other. world.js:5909 is the same line.
     ...
     *  %rn/%rt's producer - world.js:5808-5821. */
    ...
    // QG1: CastSpellDo's two world reads, world.js:5724-5727's pair.
     ...
     *  mode-aware (worldModes:1258), so this is world.js:5697's line
     ...
     *  Bridge-only, no host state - world.js:5705's call verbatim. */
     ...
     *  Everything else is world.js:5809's arm term for term: the cast
  ...
  questBridge = createQuestBridge({
    ...
    // closure S40 gave this host, and world.js:6462's line.
    ...
    // encounter half is world.js:6542's line and the watch half is this
  ...
  // same order as world.js:6763/:6768, for the same reason: the
  ...
  // here, so the sink is handed down at this moment - world.js:6747's
  // line for this host.
  ...
  var modes = createWorldModes({
    ...
    buildingDirectory: () => ({
      ...
      // two inputs the world host uses (world.js:7496).
   ...
   *  second ctx here is the shape FS1 was written about. */
  {
     ...
     *  encounter mount above, and world.js:2811-2827 is the shape.
     ...
     *  world.js:2750-2751 does it. ROAD-G TAIL: a WATCHMAN transforms
  ...
  function frame(now) {
    ...
    // through the player for ever. world.js:8852-8972 is the shape.
    arrows.update(dt, {
    ...
    if (walkMode && !tpMode) {
      ...
      for (const ev of weaponRig.frame(dt, { paralyzed })) {
        ...
        if (!cityGuards.resolvePlayerHit(weaponRig.playerWeapon, eye, fwd, player.pos, makeInView(proj, view, multiply), guardHitSound)) {
          ...
          // BEFORE civilians - world.js:9041's order, and the order

...
import { spellRecordOfIndex } from '../systems/loot.js';   // QG1: CastSpellDo's classic-record read (the G4 registry) - world.js:191's import
...
export async function bootExterior(canvas, renderer, params, status) {
  ...
  //     continue;`, world.js:1369) and stands them in batches of their
  ...
  function onExhaustedExterior() {
    ...
    try {
      const out = exhaustionOutcome({
        ...
        enemiesNearby: areEnemiesNearby([...(cityGuards?.guards ?? []), ...(exteriorFoes?.foes ?? [])]),   // ROAD-G G2: BOTH street pools, world.js:2082's line
  ...
  // pool the mode machine holds, which is world.js:2650's line.
  ...
  const cityGuards = createCityGuards({
    ...
    // own note describes. Written in world.js:2880's shape, so one pin
   ...
   *  the other two exterior hosts mount - world.js:2711 over the street
   ...
   *  it (world.js:2878-3070) and is its own slice; this pool does not
   *  wait on it. */
  ...
  const outdoorRestDeps = createRestDeps(playerEntity, {
    ...
    advanceMinutes: (n, sharedEnd) => { playerTicker.advance(n); runEncounterTick(walkMode ? player.pos : cam.pos, sharedEnd, true); },   // CAMP1-REST: every tick this drives IS a rest   // ROAD-G TAIL: the catch-up loop rides the rest's minutes, as world.js:4212 has it   // RESTX2: sharedEnd is the session's local sim-minutes online, so the roll still gets a fresh `now` while the real clock stands
   ...
   *  (effects.js:881/:895) - world.js:3401 hoisted its copy for exactly
  ...
  const magic = createPlayerMagic({
    ...
    // machine's CastSpellDo / CastEffectDo latches (machine.js:837/:843;
    ...
    // two engine-owning hosts wire the identical pair (world.js:3349-3350,
    // dungeonContext.js:2190-2191); `questBridge` is assigned below this
  ...
  const makeJournalWindow = (mode) => {
    ...
    // gate could never run. dungeonContext.js:1428-1433 is the shape:
  ...
  const questWorld = {
    ...
    // other. world.js:7644 is the same line.
     ...
     *  %rn/%rt's producer - world.js:7543-7556. */
    ...
    // QG1: CastSpellDo's two world reads, world.js:7459-7462's pair.
     ...
     *  mode-aware (worldModes:1258), so this is world.js:7432's line
     ...
     *  Bridge-only, no host state - world.js:7440's call verbatim. */
     ...
     *  Everything else is world.js:7544's arm term for term: the cast
  ...
  questBridge = createQuestBridge({
    ...
    // closure S40 gave this host, and world.js:8198's line.
    ...
    // encounter half is world.js:8289's line and the watch half is this
  ...
  // same order as world.js:8504/:8509, for the same reason: the
  ...
  // here, so the sink is handed down at this moment - world.js:8488's
  // line for this host.
  ...
  var modes = createWorldModes({
    ...
    buildingDirectory: () => ({
      ...
      // two inputs the world host uses (world.js:9818).
   ...
   *  second ctx here is the shape FS1 was written about. */
  {
     ...
     *  encounter mount above, and world.js:3583-3599 is the shape.
     ...
     *  world.js:3513-3514 does it. ROAD-G TAIL: a WATCHMAN transforms
  ...
  function frame(now) {
    ...
    // through the player for ever. world.js:11487-11687 is the shape.
    arrows.update(dt, {
    ...
    if (walkMode && !tpMode) {
      ...
      for (const ev of weaponRig.frame(dt, { paralyzed })) {
        ...
        if (!cityGuards.resolvePlayerHit(weaponRig.playerWeapon, eye, fwd, player.pos, makeInView(proj, view, multiply), guardHitSound)) {
          ...
          // BEFORE civilians - world.js:11794's order, and the order

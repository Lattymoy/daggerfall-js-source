...
import { spellRecordOfIndex } from '../systems/loot.js';   // QG1: CastSpellDo's classic-record read (the G4 registry) - world.js:119's import
...
export async function bootExterior(canvas, renderer, params, status) {
  ...
  //     host's static NPCs take it (interiorContext.js:386-404).
  ...
  //     continue;`, world.js:1017) and stands them in batches of their
  ...
  function onExhaustedExterior() {
    ...
    try {
      const out = exhaustionOutcome({
        ...
        enemiesNearby: areEnemiesNearby([...(cityGuards?.guards ?? []), ...(exteriorFoes?.foes ?? [])]),   // ROAD-G G2: BOTH street pools, world.js:1612's line
  ...
  // pool the mode machine holds, which is world.js:2067's line.
  ...
  const cityGuards = createCityGuards({
    ...
    // own note describes. Written in world.js:2155's shape, so one pin
   ...
   *  the other two exterior hosts mount - world.js:2107 over the street
   ...
   *  it (world.js:2153-2237) and is its own slice; this pool does not
   *  wait on it. */
  ...
  const outdoorRestDeps = createRestDeps(playerEntity, {
    ...
    advanceMinutes: (n) => { playerTicker.advance(n); runEncounterTick(walkMode ? player.pos : cam.pos); },   // ROAD-G TAIL: the catch-up loop rides the rest's minutes, as world.js:3038 has it
   ...
   *  (effects.js:881/:895) - world.js:2295 hoisted its copy for exactly
  ...
  const magic = createPlayerMagic({
    ...
    // two engine-owning hosts wire the identical pair (world.js:2403-2404,
  ...
  const questWorld = {
    ...
    // other. world.js:5502 is the same line.
     ...
     *  %rn/%rt's producer - world.js:5504-5517. */
    ...
    // QG1: CastSpellDo's two world reads, world.js:5420-5423's pair.
     ...
     *  mode-aware (worldModes:1258), so this is world.js:5393's line
     ...
     *  Bridge-only, no host state - world.js:5401's call verbatim. */
     ...
     *  Everything else is world.js:5505's arm term for term: the cast
  ...
  questBridge = createQuestBridge({
    ...
    // closure S40 gave this host, and world.js:6158's line.
    ...
    // encounter half is world.js:6080's line and the watch half is this
  ...
  // here, so the sink is handed down at this moment - world.js:6442's
  // line for this host.
  ...
  var modes = createWorldModes({
    ...
    buildingDirectory: () => ({
      ...
      // two inputs the world host uses (world.js:6556).
   ...
   *  second ctx here is the shape FS1 was written about. */
  {
     ...
     *  encounter mount above, and world.js:2650-2666 is the shape.
     ...
     *  world.js:2592-2593 does it. ROAD-G TAIL: a WATCHMAN transforms
  ...
  function frame(now) {
    ...
    // through the player for ever. world.js:7828-7947 is the shape.
    arrows.update(dt, {
    ...
    if (walkMode && !tpMode) {
      ...
      for (const ev of weaponRig.frame(dt, { paralyzed })) {
        ...
        if (!cityGuards.resolvePlayerHit(weaponRig.playerWeapon, eye, fwd, player.pos, makeInView(proj, view, multiply), guardHitSound)) {
          ...
          // BEFORE civilians - world.js:8016's order, and the order

...
import { spellRecordOfIndex } from '../systems/loot.js';   // QG1: CastSpellDo's classic-record read (the G4 registry) - world.js:205's import
...
export async function bootExterior(canvas, renderer, params, status) {
  ...
  //     continue;`, world.js:1806) and stands them in batches of their
  ...
  function onExhaustedExterior() {
    ...
    try {
      const out = exhaustionOutcome({
        ...
        enemiesNearby: areEnemiesNearby([...(cityGuards?.guards ?? []), ...(exteriorFoes?.foes ?? [])]),   // ROAD-G G2: BOTH street pools, world.js:2683's line
  ...
  // pool the mode machine holds, which is world.js:3280's line.
  ...
  const cityGuards = createCityGuards({
    ...
    // own note describes. Written in world.js:3615's shape, so one pin
   ...
   *  the other two exterior hosts mount - world.js:3341 over the street
   ...
   *  it (world.js:3613-3817) and is its own slice; this pool does not
   *  wait on it. */
  ...
  const outdoorRestDeps = createRestDeps(playerEntity, {
    ...
    advanceMinutes: (n, sharedEnd) => { playerTicker.advance(n); runEncounterTick(walkMode ? player.pos : cam.pos, sharedEnd, true); },   // CAMP1-REST: every tick this drives IS a rest   // ROAD-G TAIL: the catch-up loop rides the rest's minutes, as world.js:5010 has it   // RESTX2: sharedEnd is the session's local sim-minutes online, so the roll still gets a fresh `now` while the real clock stands
   ...
   *  (effects.js:887/:901) - world.js:4168 hoisted its copy for exactly
  ...
  const magic = createPlayerMagic({
    ...
    // machine's CastSpellDo / CastEffectDo latches (machine.js:847/:853;
    ...
    // two engine-owning hosts wire the identical pair (world.js:4116-4117,
  ...
  const questWorld = {
    ...
    // other. world.js:8645 is the same line.
     ...
     *  %rn/%rt's producer - world.js:8544-8557. */
    ...
    // QG1: CastSpellDo's two world reads, world.js:8460-8463's pair.
     ...
     *  mode-aware (worldModes:1258), so this is world.js:8433's line
     ...
     *  Bridge-only, no host state - world.js:8441's call verbatim. */
     ...
     *  Everything else is world.js:8545's arm term for term: the cast
  ...
  questBridge = createQuestBridge({
    ...
    // closure S40 gave this host, and world.js:9204's line.
    ...
    // encounter half is world.js:9295's line and the watch half is this
  ...
  // same order as world.js:9510/:9515, for the same reason: the
  ...
  // here, so the sink is handed down at this moment - world.js:9494's
  // line for this host.
  ...
  var modes = createWorldModes({
    ...
    buildingDirectory: () => ({
      ...
      // two inputs the world host uses (world.js:12075).
   ...
   *  second ctx here is the shape FS1 was written about. */
  {
     ...
     *  encounter mount above, and world.js:4360-4376 is the shape.
     ...
     *  world.js:4290-4291 does it. ROAD-G TAIL: a WATCHMAN transforms
  ...
  function frame(now) {
    ...
    // through the player for ever. world.js:13869-14108 is the shape.
    arrows.update(dt, {
    ...
    if (walkMode && !tpMode) {
      ...
      for (const ev of weaponRig.frame(dt, { paralyzed })) {
        ...
        if (!cityGuards.resolvePlayerHit(weaponRig.playerWeapon, eye, fwd, player.pos, makeInView(proj, view, multiply), guardHitSound, { swing })) {
          ...
          // BEFORE civilians - world.js:14218's order, and the order

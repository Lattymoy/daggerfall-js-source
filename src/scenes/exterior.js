...
export async function bootExterior(canvas, renderer, params, status) {
  ...
  function onExhaustedExterior() {
    ...
    try {
      const out = exhaustionOutcome({
        ...
        enemiesNearby: areEnemiesNearby([...(cityGuards?.guards ?? []), ...(exteriorFoes?.foes ?? [])]),   // ROAD-G G2: BOTH street pools, world.js:1681's line
  ...
  // pool the mode machine holds, which is world.js:2137's line.
  ...
  const cityGuards = createCityGuards({
    ...
    // own note describes. Written in world.js:2225's shape, so one pin
   ...
   *  the other two exterior hosts mount - world.js:2177 over the street
   ...
   *  it (world.js:2223-2305) and is its own slice; this pool does not
   *  wait on it. */
  ...
  const outdoorRestDeps = createRestDeps(playerEntity, {
    ...
    advanceMinutes: (n) => { playerTicker.advance(n); runEncounterTick(walkMode ? player.pos : cam.pos); },   // ROAD-G TAIL: the catch-up loop rides the rest's minutes, as world.js:3106 has it
   ...
   *  (effects.js:881/:895) - world.js:2536 hoisted its copy for exactly
  ...
  const magic = createPlayerMagic({
    ...
    // two engine-owning hosts wire the identical pair (world.js:2471-2472,
  ...
  const questWorld = {
    ...
    // other. world.js:5723 is the same line.
     ...
     *  %rn/%rt's producer - world.js:5622-5635. */
    ...
    // QG1: CastSpellDo's two world reads, world.js:5538-5541's pair.
     ...
     *  mode-aware (worldModes:1258), so this is world.js:5511's line
     ...
     *  Bridge-only, no host state - world.js:5519's call verbatim. */
     ...
     *  Everything else is world.js:5623's arm term for term: the cast
  ...
  questBridge = createQuestBridge({
    ...
    // closure S40 gave this host, and world.js:6276's line.
    ...
    // encounter half is world.js:6356's line and the watch half is this
  ...
  // same order as world.js:6577/:6582, for the same reason: the
  ...
  // here, so the sink is handed down at this moment - world.js:6561's
  // line for this host.
  ...
  var modes = createWorldModes({
    ...
    buildingDirectory: () => ({
      ...
      // two inputs the world host uses (world.js:7000).
   ...
   *  second ctx here is the shape FS1 was written about. */
  {
     ...
     *  encounter mount above, and world.js:2718-2734 is the shape.
     ...
     *  world.js:2657-2658 does it. ROAD-G TAIL: a WATCHMAN transforms
  ...
  function frame(now) {
    ...
    // through the player for ever. world.js:8285-8405 is the shape.
    arrows.update(dt, {
    ...
    if (walkMode && !tpMode) {
      ...
      for (const ev of weaponRig.frame(dt, { paralyzed })) {
        ...
        if (!cityGuards.resolvePlayerHit(weaponRig.playerWeapon, eye, fwd, player.pos, makeInView(proj, view, multiply), guardHitSound)) {
          ...
          // BEFORE civilians - world.js:8474's order, and the order

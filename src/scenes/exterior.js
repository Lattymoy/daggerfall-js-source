...
import { spellRecordOfIndex } from '../systems/loot.js';   // QG1: CastSpellDo's classic-record read (the G4 registry) - world.js:170's import
...
export async function bootExterior(canvas, renderer, params, status) {
  ...
  //     continue;`, world.js:1269) and stands them in batches of their
  ...
  function onExhaustedExterior() {
    ...
    try {
      const out = exhaustionOutcome({
        ...
        enemiesNearby: areEnemiesNearby([...(cityGuards?.guards ?? []), ...(exteriorFoes?.foes ?? [])]),   // ROAD-G G2: BOTH street pools, world.js:1980's line
  ...
  // pool the mode machine holds, which is world.js:2547's line.
  ...
  const cityGuards = createCityGuards({
    ...
    // own note describes. Written in world.js:2761's shape, so one pin
   ...
   *  the other two exterior hosts mount - world.js:2608 over the street
   ...
   *  it (world.js:2759-2841) and is its own slice; this pool does not
   *  wait on it. */
   ...
   *  (effects.js:881/:895) - world.js:3168 hoisted its copy for exactly
  ...
  const magic = createPlayerMagic({
    ...
    // two engine-owning hosts wire the identical pair (world.js:3073-3074,
    // dungeonContext.js:2093-2094); `questBridge` is assigned below this
  ...
  const makeJournalWindow = (mode) => {
    ...
    // gate could never run. dungeonContext.js:1316-1321 is the shape:
  ...
  const questWorld = {
    ...
    // other. world.js:7167 is the same line.
     ...
     *  %rn/%rt's producer - world.js:7066-7079. */
    ...
    // QG1: CastSpellDo's two world reads, world.js:6982-6985's pair.
     ...
     *  mode-aware (worldModes:1258), so this is world.js:6955's line
     ...
     *  Bridge-only, no host state - world.js:6963's call verbatim. */
     ...
     *  Everything else is world.js:7067's arm term for term: the cast
  ...
  questBridge = createQuestBridge({
    ...
    // closure S40 gave this host, and world.js:7720's line.
    ...
    // encounter half is world.js:7800's line and the watch half is this
  ...
  // same order as world.js:8021/:8026, for the same reason: the
  ...
  // here, so the sink is handed down at this moment - world.js:8005's
  // line for this host.
  ...
  var modes = createWorldModes({
    ...
    buildingDirectory: () => ({
      ...
      // two inputs the world host uses (world.js:8902).
   ...
   *  second ctx here is the shape FS1 was written about. */
  {
     ...
     *  encounter mount above, and world.js:3350-3366 is the shape.
     ...
     *  world.js:3280-3281 does it. ROAD-G TAIL: a WATCHMAN transforms
  ...
  function frame(now) {
    ...
    // through the player for ever. world.js:10472-10646 is the shape.
    arrows.update(dt, {
    ...
    bloodMarks.draw();   // BLOOD1a: the marks go down BEFORE the billboards, so a body standing in its own blood is over it and not under it. ABOVE setFlatWind for the reason its own neighbour gives: WIND3 pins the wind and the draw as ADJACENT.
    ...
    if (walkMode && !tpMode) {
      ...
      for (const ev of weaponRig.frame(dt, { paralyzed })) {
        ...
        if (!cityGuards.resolvePlayerHit(weaponRig.playerWeapon, eye, fwd, player.pos, makeInView(proj, view, multiply), guardHitSound)) {
          ...
          // BEFORE civilians - world.js:10742's order, and the order

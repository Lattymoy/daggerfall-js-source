...
export async function bootExterior(canvas, renderer, params, status) {
  ...
  const magic = createPlayerMagic({
    ...
    // two engine-owning hosts wire the identical pair (world.js:3283-3284,
  ...
  addEventListener('keydown', (e) => {
    ...
    if (!townTalk.overlayActive && (modes?.mode ?? 'exterior') === 'exterior') {
      ...
      // own `POLLED_ACTIONS.has(act)` decline (ui/input.js:669) never
  ...
  const questWorld = {
    ...
    // other. world.js:7525 is the same line.
     ...
     *  %rn/%rt's producer - world.js:7424-7437. */
    ...
    // QG1: CastSpellDo's two world reads, world.js:7337-7340's pair.
     ...
     *  mode-aware (worldModes:1258), so this is world.js:7310's line
     ...
     *  Bridge-only, no host state - world.js:7318's call verbatim. */
     ...
     *  Everything else is world.js:7425's arm term for term: the cast
  ...
  questBridge = createQuestBridge({
    ...
    // closure S40 gave this host, and world.js:8088's line.
    ...
    // encounter half is world.js:8168's line and the watch half is this
  ...
  // same order as world.js:8389/:8394, for the same reason: the
  ...
  // here, so the sink is handed down at this moment - world.js:8373's
  // line for this host.
  ...
  var modes = createWorldModes({
    ...
    buildingDirectory: () => ({
      ...
      // two inputs the world host uses (world.js:9371).
  ...
  function frame(now) {
    ...
    // through the player for ever. world.js:11010-11210 is the shape.
    arrows.update(dt, {
    ...
    if (walkMode && !tpMode) {
      ...
      for (const ev of weaponRig.frame(dt, { paralyzed })) {
        ...
        if (!cityGuards.resolvePlayerHit(weaponRig.playerWeapon, eye, fwd, player.pos, makeInView(proj, view, multiply), guardHitSound)) {
          ...
          // BEFORE civilians - world.js:11317's order, and the order

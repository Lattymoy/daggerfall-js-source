...
import { actionOf, held, moveHeld, anyMove, swallowBrowserKey, mouseCode, isSwingButton, keyboardLook } from '../ui/input.js';   // I2: the rebindable registry; AUDIT 39r: the mouse half of the held set
...
export async function bootExterior(canvas, renderer, params, status) {
  ...
  function onExhaustedExterior() {
    ...
    try {
      const out = exhaustionOutcome({
        ...
        enemiesNearby: areEnemiesNearby([...(cityGuards?.guards ?? []), ...(exteriorFoes?.foes ?? [])]),   // ROAD-G G2: BOTH street pools, world.js:1678's line
  ...
  // pool the mode machine holds, which is world.js:2145's line.
  ...
  const cityGuards = createCityGuards({
    ...
    // own note describes. Written in world.js:2244's shape, so one pin
   ...
   *  the other two exterior hosts mount - world.js:2185 over the street
   ...
   *  it (world.js:2242-2324) and is its own slice; this pool does not
   *  wait on it. */
  ...
  const outdoorRestDeps = createRestDeps(playerEntity, {
    ...
    advanceMinutes: (n) => { playerTicker.advance(n); runEncounterTick(walkMode ? player.pos : cam.pos); },   // ROAD-G TAIL: the catch-up loop rides the rest's minutes, as world.js:3131 has it
   ...
   *  (effects.js:881/:895) - world.js:2556 hoisted its copy for exactly
  ...
  const magic = createPlayerMagic({
    ...
    // two engine-owning hosts wire the identical pair (world.js:2490-2491,
  ...
  const questWorld = {
    ...
    // other. world.js:5744 is the same line.
     ...
     *  %rn/%rt's producer - world.js:5643-5656. */
    ...
    // QG1: CastSpellDo's two world reads, world.js:5559-5562's pair.
     ...
     *  mode-aware (worldModes:1258), so this is world.js:5532's line
     ...
     *  Bridge-only, no host state - world.js:5540's call verbatim. */
     ...
     *  Everything else is world.js:5644's arm term for term: the cast
  ...
  questBridge = createQuestBridge({
    ...
    // closure S40 gave this host, and world.js:6297's line.
    ...
    // encounter half is world.js:6377's line and the watch half is this
  ...
  // same order as world.js:6598/:6603, for the same reason: the
  ...
  // here, so the sink is handed down at this moment - world.js:6582's
  // line for this host.
  ...
  var modes = createWorldModes({
    ...
    buildingDirectory: () => ({
      ...
      // two inputs the world host uses (world.js:7034).
   ...
   *  second ctx here is the shape FS1 was written about. */
  {
     ...
     *  encounter mount above, and world.js:2738-2754 is the shape.
     ...
     *  world.js:2677-2678 does it. ROAD-G TAIL: a WATCHMAN transforms
  ...
  function frame(now) {
    ...
    // through the player for ever. world.js:8375-8495 is the shape.
    arrows.update(dt, {
    ...
    if (walkMode && !tpMode) {
      ...
      for (const ev of weaponRig.frame(dt, { paralyzed })) {
        ...
        if (!cityGuards.resolvePlayerHit(weaponRig.playerWeapon, eye, fwd, player.pos, makeInView(proj, view, multiply), guardHitSound)) {
          ...
          // BEFORE civilians - world.js:8564's order, and the order

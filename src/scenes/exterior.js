...
export async function bootExterior(canvas, renderer, params, status) {
  ...
  const outdoorRestDeps = createRestDeps(playerEntity, {
    ...
    advanceMinutes: (n, sharedEnd) => { playerTicker.advance(n); runEncounterTick(walkMode ? player.pos : cam.pos, sharedEnd, true); },   // CAMP1-REST: every tick this drives IS a rest   // ROAD-G TAIL: the catch-up loop rides the rest's minutes, as world.js:4997 has it   // RESTX2: sharedEnd is the session's local sim-minutes online, so the roll still gets a fresh `now` while the real clock stands
  ...
  const questWorld = {
    ...
    // other. world.js:8622 is the same line.
     ...
     *  %rn/%rt's producer - world.js:8521-8534. */
    ...
    // QG1: CastSpellDo's two world reads, world.js:8437-8440's pair.
     ...
     *  mode-aware (worldModes:1258), so this is world.js:8410's line
     ...
     *  Bridge-only, no host state - world.js:8418's call verbatim. */
     ...
     *  Everything else is world.js:8522's arm term for term: the cast
  ...
  questBridge = createQuestBridge({
    ...
    // closure S40 gave this host, and world.js:9181's line.
    ...
    // encounter half is world.js:9272's line and the watch half is this
  ...
  // same order as world.js:9487/:9492, for the same reason: the
  ...
  // here, so the sink is handed down at this moment - world.js:9471's
  // line for this host.
  ...
  var modes = createWorldModes({
    ...
    buildingDirectory: () => ({
      ...
      // two inputs the world host uses (world.js:11967).
   ...
   *  second ctx here is the shape FS1 was written about. */
  {
     ...
     *  world.js:4278-4279 does it. ROAD-G TAIL: a WATCHMAN transforms
  ...
  function frame(now) {
    ...
    // through the player for ever. world.js:13748-13987 is the shape.
    arrows.update(dt, {

...
export async function bootExterior(canvas, renderer, params, status) {
  ...
  // pool the mode machine holds, which is world.js:2121's line.
  ...
  const cityGuards = createCityGuards({
    ...
    // own note describes. Written in world.js:2209's shape, so one pin
   ...
   *  the other two exterior hosts mount - world.js:2161 over the street
   ...
   *  it (world.js:2207-2289) and is its own slice; this pool does not
   *  wait on it. */
   ...
   *  (effects.js:881/:895) - world.js:2520 hoisted its copy for exactly
  ...
  const magic = createPlayerMagic({
    ...
    // dungeonContext.js:2000-2001); `questBridge` is assigned below this
  ...
  const makeJournalWindow = (mode) => {
    ...
    // gate could never run. dungeonContext.js:1300-1302 is the shape:
  ...
  const questWorld = {
    ...
    // other. world.js:5707 is the same line.
  ...
  questBridge = createQuestBridge({
    ...
    // encounter half is world.js:6340's line and the watch half is this
  ...
  // same order as world.js:6561/:6566, for the same reason: the
  ...
  var modes = createWorldModes({
    ...
    buildingDirectory: () => ({
      ...
      // two inputs the world host uses (world.js:6979).
  ...
  function frame(now) {
    ...
    // through the player for ever. world.js:8255-8374 is the shape.
    arrows.update(dt, {
    ...
    if (walkMode && !tpMode) {
      ...
      for (const ev of weaponRig.frame(dt, { paralyzed })) {
        ...
        if (!cityGuards.resolvePlayerHit(weaponRig.playerWeapon, eye, fwd, player.pos, makeInView(proj, view, multiply), guardHitSound)) {
          ...
          // BEFORE civilians - world.js:8443's order, and the order

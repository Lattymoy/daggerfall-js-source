...
export async function bootExterior(canvas, renderer, params, status) {
  ...
  // pool the mode machine holds, which is world.js:2146's line.
  ...
  const cityGuards = createCityGuards({
    ...
    // own note describes. Written in world.js:2245's shape, so one pin
   ...
   *  the other two exterior hosts mount - world.js:2186 over the street
   ...
   *  it (world.js:2243-2325) and is its own slice; this pool does not
   *  wait on it. */
  ...
  const outdoorRestDeps = createRestDeps(playerEntity, {
    ...
    advanceMinutes: (n) => { playerTicker.advance(n); runEncounterTick(walkMode ? player.pos : cam.pos); },   // ROAD-G TAIL: the catch-up loop rides the rest's minutes, as world.js:3127 has it
   ...
   *  (effects.js:881/:895) - world.js:2557 hoisted its copy for exactly
  ...
  const magic = createPlayerMagic({
    ...
    // two engine-owning hosts wire the identical pair (world.js:2491-2492,
    // dungeonContext.js:1985-1986); `questBridge` is assigned below this
  ...
  const questWorld = {
    ...
    // other. world.js:5769 is the same line.
     ...
     *  %rn/%rt's producer - world.js:5668-5681. */
    ...
    // QG1: CastSpellDo's two world reads, world.js:5584-5587's pair.
     ...
     *  mode-aware (worldModes:1258), so this is world.js:5557's line
     ...
     *  Bridge-only, no host state - world.js:5565's call verbatim. */
     ...
     *  Everything else is world.js:5669's arm term for term: the cast
  ...
  questBridge = createQuestBridge({
    ...
    // closure S40 gave this host, and world.js:6322's line.
    ...
    // encounter half is world.js:6402's line and the watch half is this
  ...
  // same order as world.js:6623/:6628, for the same reason: the
  ...
  // here, so the sink is handed down at this moment - world.js:6607's
  // line for this host.
  ...
  var modes = createWorldModes({
    ...
    buildingDirectory: () => ({
      ...
      // two inputs the world host uses (world.js:7046).
   ...
   *  second ctx here is the shape FS1 was written about. */
  {
     ...
     *  encounter mount above, and world.js:2739-2755 is the shape.
     ...
     *  world.js:2678-2679 does it. ROAD-G TAIL: a WATCHMAN transforms

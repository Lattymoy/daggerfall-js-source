...
export async function bootExterior(canvas, renderer, params, status) {
  ...
  //     continue;`, world.js:1808) and stands them in batches of their
  ...
  // pool the mode machine holds, which is world.js:3278's line.
  ...
  const cityGuards = createCityGuards({
    ...
    // own note describes. Written in world.js:3613's shape, so one pin
   ...
   *  the other two exterior hosts mount - world.js:3339 over the street
   ...
   *  it (world.js:3611-3815) and is its own slice; this pool does not
   *  wait on it. */
  ...
  const outdoorRestDeps = createRestDeps(playerEntity, {
    ...
    advanceMinutes: (n, sharedEnd) => { playerTicker.advance(n); runEncounterTick(walkMode ? player.pos : cam.pos, sharedEnd, true); },   // CAMP1-REST: every tick this drives IS a rest   // ROAD-G TAIL: the catch-up loop rides the rest's minutes, as world.js:5026 has it   // RESTX2: sharedEnd is the session's local sim-minutes online, so the roll still gets a fresh `now` while the real clock stands
  ...
  questBridge = createQuestBridge({
    ...
    // closure S40 gave this host, and world.js:9228's line.
    ...
    // encounter half is world.js:9324's line and the watch half is this
  ...
  // same order as world.js:9538/:9543, for the same reason: the
  ...
  // here, so the sink is handed down at this moment - world.js:9522's
  // line for this host.
  ...
  var modes = createWorldModes({
    ...
    buildingDirectory: () => ({
      ...
      // two inputs the world host uses (world.js:12511).
  ...
  function frame(now) {
    ...
    // through the player for ever. world.js:14308-14548 is the shape.
    arrows.update(dt, {
    ...
    if (walkMode && !tpMode) {
      ...
      for (const ev of weaponRig.frame(dt, { paralyzed })) {
        ...
        if (!cityGuards.resolvePlayerHit(weaponRig.playerWeapon, eye, fwd, player.pos, makeInView(proj, view, multiply), guardHitSound, { swing })) {
          ...
          // BEFORE civilians - world.js:14658's order, and the order

...
import { spellRecordOfIndex } from '../systems/loot.js';   // QG1: CastSpellDo's classic-record read (the G4 registry) - world.js:118's import
...
export async function bootExterior(canvas, renderer, params, status) {
  ...
  //     continue;`, world.js:1009) and stands them in batches of their
  ...
  function onExhaustedExterior() {
    ...
    try {
      const out = exhaustionOutcome({
        ...
        enemiesNearby: areEnemiesNearby([...(cityGuards?.guards ?? []), ...(exteriorFoes?.foes ?? [])]),   // ROAD-G G2: BOTH street pools, world.js:1600's line
  ...
  // pool the mode machine holds, which is world.js:2055's line.
  ...
  const cityGuards = createCityGuards({
    ...
    // own note describes. Written in world.js:2143's shape, so one pin
   ...
   *  the other two exterior hosts mount - world.js:2095 over the street
   ...
   *  it (world.js:2141-2225) and is its own slice; this pool does not
   *  wait on it. */
  ...
  const outdoorRestDeps = createRestDeps(playerEntity, {
    ...
    advanceMinutes: (n) => { playerTicker.advance(n); runEncounterTick(walkMode ? player.pos : cam.pos); },   // ROAD-G TAIL: the catch-up loop rides the rest's minutes, as world.js:3026 has it
   ...
   *  (effects.js:881/:895) - world.js:2283 hoisted its copy for exactly
  ...
  const magic = createPlayerMagic({
    ...
    // two engine-owning hosts wire the identical pair (world.js:2391-2392,
  ...
  const questWorld = {
    ...
    // other. world.js:5490 is the same line.
     ...
     *  %rn/%rt's producer - world.js:5492-5505. */
    ...
    // QG1: CastSpellDo's two world reads, world.js:5408-5411's pair.
     ...
     *  mode-aware (worldModes:1258), so this is world.js:5381's line
     ...
     *  Bridge-only, no host state - world.js:5389's call verbatim. */
     ...
     *  Everything else is world.js:5493's arm term for term: the cast
  ...
  questBridge = createQuestBridge({
    ...
    // closure S40 gave this host, and world.js:6146's line.
    ...
    // encounter half is world.js:6068's line and the watch half is this
  ...
  // here, so the sink is handed down at this moment - world.js:6430's
  // line for this host.
  ...
  var modes = createWorldModes({
    ...
    buildingDirectory: () => ({
      ...
      // two inputs the world host uses (world.js:6544).
   ...
   *  second ctx here is the shape FS1 was written about. */
  {
     ...
     *  encounter mount above, and world.js:2638-2654 is the shape.
     ...
     *  world.js:2580-2581 does it. ROAD-G TAIL: a WATCHMAN transforms

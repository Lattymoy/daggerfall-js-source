...
export async function bootExterior(canvas, renderer, params, status) {
   ...
   *  it (world.js:3622-3826) and is its own slice; this pool does not
   *  wait on it. */
  ...
  const outdoorRestDeps = createRestDeps(playerEntity, {
    ...
    advanceMinutes: (n, sharedEnd) => { playerTicker.advance(n); runEncounterTick(walkMode ? player.pos : cam.pos, sharedEnd, true); },   // CAMP1-REST: every tick this drives IS a rest   // ROAD-G TAIL: the catch-up loop rides the rest's minutes, as world.js:5037 has it   // RESTX2: sharedEnd is the session's local sim-minutes online, so the roll still gets a fresh `now` while the real clock stands
   ...
   *  (effects.js:885/:899) - world.js:4186 hoisted its copy for exactly
  ...
  const magic = createPlayerMagic({
    ...
    // two engine-owning hosts wire the identical pair (world.js:4134-4135,
  ...
  const questWorld = {
    ...
    // other. world.js:8681 is the same line.
     ...
     *  %rn/%rt's producer - world.js:8580-8593. */
    ...
    // QG1: CastSpellDo's two world reads, world.js:8496-8499's pair.
     ...
     *  mode-aware (worldModes:1258), so this is world.js:8469's line
     ...
     *  Bridge-only, no host state - world.js:8477's call verbatim. */
     ...
     *  Everything else is world.js:8581's arm term for term: the cast
  ...
  questBridge = createQuestBridge({
    ...
    // closure S40 gave this host, and world.js:9241's line.
    ...
    // encounter half is world.js:9337's line and the watch half is this
  ...
  // same order as world.js:9551/:9556, for the same reason: the
  ...
  // here, so the sink is handed down at this moment - world.js:9535's
  // line for this host.
  ...
  var modes = createWorldModes({
    ...
    buildingDirectory: () => ({
      ...
      // two inputs the world host uses (world.js:12536).
   ...
   *  encounter mount above, and world.js:4366-4382 is the shape.
   ...
   *  second ctx here is the shape FS1 was written about. */
  {
     ...
     *  world.js:4313-4314 does it. ROAD-G TAIL: a WATCHMAN transforms
  ...
  function frame(now) {
    ...
    // through the player for ever. world.js:14333-14573 is the shape.
    arrows.update(dt, {
    ...
    if (walkMode && !tpMode) {
      ...
      for (const ev of weaponRig.frame(dt, { paralyzed })) {
        ...
        if (!cityGuards.resolvePlayerHit(weaponRig.playerWeapon, eye, fwd, player.pos, makeInView(proj, view, multiply), guardHitSound, { swing })) {
          ...
          // BEFORE civilians - world.js:14683's order, and the order

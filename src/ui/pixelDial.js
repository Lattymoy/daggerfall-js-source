...
import { eventMeans } from './input.js';   // KB1: the key that opened it is the registry's QuickDial
...
export function mountPixelDial(hostEl, { entries = [], onClose = () => {} } = {}) {
  ...
  keyHandler = (e) => {
    ...
    if (eventMeans(e, 'QuickDial')) { e.preventDefault(); e.stopPropagation(); if (!e.repeat) close(); return; }   // PX15: the key that opened it closes it - KB1: QuickDial's, wherever it is bound (it was a literal Tab)

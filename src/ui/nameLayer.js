...
export function createNameLayer({ doc = document, now = () => Date.now() } = {}) {
  ...
  const makeTag = () => {
    ...
    // RENOWN1: the level FIRST in the row - left of the name, as Mac put it
    const lv = doc.createElement('span');
    lv.className = 'dfname-renown';
  ...
  return {
     ...
     *  of it - the damage numbers' own layer has the same problem and enhancedHud.js:806 solves it the same way,

...
//
// ── the one integration prerequisite ──────────────────────────────
// neutralBody.js's cloth loop hardcodes its material:
//     displace(z.groups, z.yLo, z.yHi, z.th ?? 0.008, cxFor, 1, 'cloth')
// so today an entire outfit resolves to ONE colour. The armour loop
// directly above it already passes `z.mat`. These designs give every
// zone its own `mat`, which needs that one argument to become
// `z.mat || 'cloth'`. Nothing else here depends on an engine change.
...
const skirt   = (mat, yLo = 0.62, yHi = 1.14, th = 0.030) => ({ groups: [B, LL, LR], yLo, yHi, th, mat, leg: true });
...
const hood    = (mat, yLo = 1.58, yHi = 1.95, th = 0.022) => ({ groups: ['head', B], yLo, yHi, th, mat });
...
export const VILLAGER_DESIGNS = Object.freeze([
  // ── Redguard, male ──────────────────────────────────────────────
  ...
  { archive: 383, race: 'Redguard', gender: 'male', name: 'Spice seller', build: 'average',
    ...
    zones: [torso('robe', 1.04), sleeves('robe', 1.30), sash('sash'), skirt('robe', 0.70, 1.10), feet('sandal', 0, 0.09)],
  ...
  { archive: 384, race: 'Redguard', gender: 'male', name: 'Caravan master', build: 'tall',
    ...
    zones: [collar('coat'), torso('coat', 1.10), sleeves('coat', 1.22), sash('sash'), skirt('coat', 0.66, 1.12), feet('boot', 0, 0.20)],
  ...
  { archive: 395, race: 'Redguard', gender: 'female', name: 'Well keeper', build: 'average',
    ...
    zones: [torso('shift', 1.20), skirt('shift', 0.58, 1.14), sash('sash'), feet('sandal', 0, 0.09)],
  ...
  { archive: 396, race: 'Redguard', gender: 'female', name: 'Dyer', build: 'slight',
    ...
    zones: [torso('shift', 1.16), sleeves('shift', 1.34), skirt('overskirt', 0.60, 1.12), sash('sash'), feet('sandal', 0, 0.09)],
  ...
  { archive: 397, race: 'Redguard', gender: 'female', name: 'Herbalist', build: 'average',
    hair: { style: 'medium', ramp: 'black' },
    zones: [hood('veil'), torso('shift', 1.12), sleeves('shift', 1.26), skirt('shift', 0.62, 1.12), feet('sandal', 0, 0.09)],
  ...
  { archive: 398, race: 'Redguard', gender: 'female', name: 'Silk factor', build: 'tall',
    hair: { style: 'long', ramp: 'black' },
    zones: [hood('veil'), collar('gown'), torso('gown', 1.10), sleeves('gown', 1.20), skirt('gown', 0.54, 1.12), sash('sash'), feet('slipper', 0, 0.08)],
  ...
  { archive: 390, race: 'Nord', gender: 'male', name: 'Timber factor', build: 'tall',
    ...
    zones: [collar('coat'), torso('coat', 1.08), sleeves('coat', 1.22), skirt('coat', 0.74, 1.10), legs('trews', 0.30), feet('boot', 0, 0.28)],
  ...
  { archive: 392, race: 'Nord', gender: 'female', name: 'Brewer', build: 'stout',
    ...
    zones: [torso('wool', 1.18), sleeves('wool', 1.32), skirt('wool', 0.56, 1.12), pelvis('apron', 0.88, 1.16, 0.020), feet('boot', 0, 0.20)],
  ...
  { archive: 393, race: 'Nord', gender: 'female', name: 'Netmender', build: 'average',
    ...
    zones: [hood('hood'), torso('wool', 1.14), sleeves('wool', 1.28), skirt('wool', 0.60, 1.12), feet('boot', 0, 0.22)],
    mats: { hood: RAMPS.teal, wool: RAMPS.teal, boot: RAMPS.leather } },
  { archive: 451, race: 'Nord', gender: 'female', name: 'Shepherd', build: 'average',
    hair: { style: 'medium', ramp: 'blonde' },
    zones: [hood('shawl'), torso('wool', 1.12), sleeves('wool', 1.26), skirt('wool', 0.58, 1.12), feet('boot', 0, 0.26)],
    mats: { shawl: RAMPS.moss, wool: RAMPS.sage, boot: RAMPS.leather } },
  { archive: 452, race: 'Nord', gender: 'female', name: 'Hall matron', build: 'tall',
    hair: { style: 'long', ramp: 'blonde' },
    zones: [hood('shawl'), collar('gown'), torso('gown', 1.06), sleeves('gown', 1.18), skirt('gown', 0.50, 1.12), sash('sash'), feet('boot', 0, 0.18)],
  ...
  { archive: 391, race: 'Breton', gender: 'male', name: 'Clerk', build: 'slight',
    ...
    zones: [collar('gown'), torso('gown', 1.06), sleeves('gown', 1.16), skirt('gown', 0.68, 1.10), feet('shoe', 0, 0.12)],
    mats: { gown: RAMPS.indigo, shoe: RAMPS.leather } },
  { archive: 394, race: 'Breton', gender: 'male', name: 'Guildmaster', build: 'broad',
    hair: { style: 'medium', ramp: 'brown' },
    zones: [hood('chaperon'), collar('coat'), torso('coat', 1.04), sleeves('coat', 1.14), skirt('coat', 0.62, 1.10), sash('chain'), feet('shoe', 0, 0.14)],
  ...
  { archive: 453, race: 'Breton', gender: 'female', name: 'Baker', build: 'stout',
    ...
    zones: [torso('kirtle', 1.16), sleeves('kirtle', 1.30), skirt('kirtle', 0.58, 1.12), pelvis('apron', 0.86, 1.16, 0.020), feet('shoe', 0, 0.12)],
  ...
  { archive: 454, race: 'Breton', gender: 'female', name: 'Seamstress', build: 'slight',
    hair: { style: 'medium', ramp: 'blonde' },
    zones: [torso('bodice', 1.18, 1.56, 0.014), sleeves('shift', 1.28), skirt('kirtle', 0.56, 1.12), feet('shoe', 0, 0.12)],
  ...
  { archive: 455, race: 'Breton', gender: 'female', name: 'Almswoman', build: 'average',
    ...
    zones: [hood('coif'), torso('kirtle', 1.12), sleeves('kirtle', 1.26), skirt('kirtle', 0.58, 1.12), feet('shoe', 0, 0.12)],
  ...
  { archive: 456, race: 'Breton', gender: 'female', name: 'Merchant wife', build: 'tall',
    hair: { style: 'long', ramp: 'blonde' },
    zones: [collar('gown'), torso('gown', 1.04), sleeves('shift', 1.14), skirt('gown', 0.48, 1.12), sash('sash'), feet('slipper', 0, 0.08)],

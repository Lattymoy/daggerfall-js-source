-- ADV1 (2026-09-24) - THE ADVENTURING LEVEL.
--
...
-- "No" Adventuring XP (src/net/advLevel.js holds all three).
--
...
-- both ends share (src/net/advLevel.js advLevelForXp), so the total and
...
CREATE TABLE IF NOT EXISTS adv_tracks (
...
CREATE INDEX IF NOT EXISTS idx_adv_recent ON adv_tracks (player, updated_at);

-- THE HOUR'S BOUND IS THE ACCOUNT'S, across all its characters, so a
-- second character is not a second allowance. `adv_hour` is the clock
-- hour the window counts (epoch seconds / 3600), `adv_hour_xp` what the
-- account has earned in it, and `adv_last_credit` what the last report
...
ALTER TABLE players ADD COLUMN adv_hour INTEGER NOT NULL DEFAULT 0;
ALTER TABLE players ADD COLUMN adv_hour_xp INTEGER NOT NULL DEFAULT 0;
ALTER TABLE players ADD COLUMN adv_last_credit INTEGER NOT NULL DEFAULT 0;

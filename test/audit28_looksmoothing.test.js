...
test('AUDIT 28 F-C1/F-C2: settle drops the owed look (SetFacing -> Init), and every host answers Update\'s three cases - paused waits, a held swing drops, else pays', () => {
  ...
  // ROAD-tail: the PAUSED arm is no longer written out at each host -
  // it is the one primitive, ui/windowStack.js's `paused()`, reached
  // through the host's single pause reader (`gamePaused` in the two
  // outdoor hosts and in the standalone interior; `ctx.uiOverlayActive`
  // is dungeonContext's, which is its stack's `paused()` too). The
  // interior host's arm was the literal `false` - a gate written open -

...
test('SOC1 wire: the hub is the world channel, the bounds are what Mac asked for, and the version moved (mutants: the hub a room of its own that nobody is in; a five-seat party; the version left at world77)', () => {
  ...
  assert.equal(RELAY_VERSION, 'world102', 'HCC-PARK + RIDE (the park frame and the pose\'s riding fields - world102); SPELLFX1 (the pose\'s cast element and arrows loosed - world98); PARTY-REST DROP (the pose\'s rest kind, the vote and cancel stamps, the 32-bit building key - world96); AUDIT DROPS (the trade bytes per sender, the hub\'s quest cooldown, the quest budget\'s order - world92); QUEST1 + TRADE1 + PEER-FS1 (three drops, one deploy: the quest frame, the trade frame, the pose\'s fk - world91); SOC1 changed the relay: bumped; AUDIT SOC again; RESPAWN1 again (the foe door\'s team pair); AUDIT WATCH1 again (wire.js gained CELL_WATCH_PUPPETS_MAX); the main merge again (a comment line in wire.js moved - the bytes are the law)');
...
test('SOC1 hub: the source - the account is handled after the channel\'s welcome and join, the leave stamps the account, the sweep spares acct: and asecret:, and the presence rooms are untouched (mutants: the picture before the welcome; the leave silent; a cell reading the account)', () => {
  ...
  assert.match(w, /export const RELAY_VERSION = 'world102';/, 'HCC-PARK + RIDE moved it last (world102); AUDIT SOC moved it, RESPAWN1 moved it again, AUDIT WATCH1 again, the main merge again, RELAY-H1 again, ACC1d again, the three drops again (world91), AUDIT DROPS again (world92), the party-rest drop again (world96), ALLY-CAST and its audit (world97), SPELLFX1 pose fields (world98)');
});

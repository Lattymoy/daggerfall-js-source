...
test('HARD3: the renderer\'s contract is types only, and the shapes it names are the ones the renderer mints', () => {
  ...
  assert.equal(minted.length, 35, `the factory mints ${minted.length} fields and the walk should see every one: ${minted}`);   // EL5: `bounds`; BLOOD1b: `_quads` and `_dyn`; PERF-EXT10: the 23 a batch used to gain after birth, minted undefined (one hidden class); PERF-EXT1: `_place`

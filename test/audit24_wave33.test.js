...
test('audit24 wave33: no host freezes the animation, and every host consumes its latch', () => {
  ...
  for (const [name, src] of [['dungeonContext.js', d], ['exteriorFoes.js', xf], ['cityGuards.js', cg]]) {
    ...
    assert.equal(drops, name === 'dungeonContext.js' ? 4 : name === 'exteriorFoes.js' ? 2 : 0, `${name}: the puppet's drops, marked, and nowhere else (WORLD6b: the cell's puppet drops both, one marked line) (WORLD3: a puppet's own frame drops them unless the blow is at ME or a shaft flies at anyone - the arm consumes those)`);

...
test('EE7: the draw goes through every door the renderer has, and reports', () => {
  ...
  assert.match(w, /renderer\.tileNormalFor\(p\.groundArchive\) \/\* EE6 \*\/\);\n[\s\S]{0,900}if \(p\.grass\) \{[\s\S]{0,700}renderer\.drawGrass\(p\.grass, pixelMatrix,/, 'drawn right after its terrain (EE16/EE17: the fine ground and the wind sit between)');

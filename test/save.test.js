...
test('save: round-trip restores everything; extras carried; deep copies', () => {
  ...
  assert.deepEqual(extras, { position: [1, 2, 3], pose: null, classicMinutes: 77.5, readiedSpellIndex: 97, world, locationKey: 'dungeon:42', quest: null, talk: null, interior: null, dungeon: null, travelMap: null, escortingFaces: null, quickslots: null, spawns: null, smallerDungeonsState: 0 });   // TTL1: `spawns` is the spawned-dungeon ledger, and `quickslots` is QS1's block, which this name list had never carried (so every save dropped it)   // S12: the world rides the envelope; Q4-v/TK-i: the quest + talk slots (null when the host passed none); IS1: the interior slot (null outside a building)

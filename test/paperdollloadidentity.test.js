...
// Load Game AND Online alike (main.js:175), so every session after the
...
test('THE ART FOLLOWS THE ENTITY: a character restored by restorePlayer draws HER OWN body, face and backdrop, not the boot stand-in\'s (mutant: drop the drift check in refreshPaperDoll and the doll is Breton, male, face 0 for the rest of the session)', async () => {
  ...
  // stand-in again, and the host warms the doll with it (world.js:3009,
  // exterior.js:1255 - the boot warm names no identity at all).
  ...
  // ...and then `?load` restores the character (main.js:175 - Continue,
  ...
  // The inventory opens (nativeInventory.js:447 - refreshPaperDoll on

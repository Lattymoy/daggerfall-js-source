...
test('EV8: the wiring - enhanced-gated, weather-gated, inside the sky\'s foreign span, one origin', () => {
  ...
  assert.equal((world.match(/renderer\.markForeignPass\(\);/g) || []).length, 10,   // DUEL1: the duel ring's wall; WB2: the gate's fire and beacon; WB4a: the court's telegraph; WB6a: the Deadlands' sea and sky

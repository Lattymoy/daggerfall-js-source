...
test('EV8: the wiring - enhanced-gated, weather-gated, inside the sky\'s foreign span, one origin', () => {
  ...
  assert.equal((world.match(/renderer\.markForeignPass\(\);/g) || []).length, 5,
    'the ring shares the sky\'s seam - the EV6 count stands (GR1 added the grass, the third; WIND3 the wisps, the fourth; WEATHER2d the sand, the fifth)');
});

...
// enchantCtx at the strike site (formulas.js:505 defaults it null) -
...
test('AUDIT 58: the WABBAJACK re-stands a foe in the pool that owns it', () => {
  ...
  // (exteriorFoes.js:417-422) and both pools share the host's one
  ...
  assert.match(body, /exteriorFoes\.spawnFoe\(mobileType, feet, \{ replacing: true \}\)\.then\(stamp\)\.catch\(\(\) => \{\}\);/,
    'the re-stand is the encounter pool either way, and unconditional');
...
test('AUDIT 58 (f2/hosts): the EXTERIOR host mounts the same body over its own pools', () => {
  ...
  // (world.js:4341's shape) - the removal, never the re-stand
  ...
  assert.match(sf, /\? d\.spawnLooseFoe\(mt, pos, \{ yawRad: so\.yawRad, allied: so\.allied \}\)\n\s*: exteriorFoes\.spawnFoe\(mt, pos, \{ yaw: so\.yawRad, allied: so\.allied, loose: true \}\)/);
});
...
test('EC1/ROAD-G G1: the two SPAWN arms stand a foe in the world the player IS in, through that world\'s own pool', () => {
  ...
  assert.match(body, /: exteriorFoes\.spawnFoe\(mt, pos, \{ yaw: o\.yawRad, allied: o\.allied, loose: true \}\)/,

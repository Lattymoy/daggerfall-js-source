...
test('SPELLFX1 wire (world98): the pose carries the cast\'s element (`ce`, 0..4) and the arrows loosed (`ar`); a pose from before them reads Magic and none (mutants: the fields dropped by the projection; an element past the table)', () => {
  assert.equal(RELAY_VERSION, 'world106'); assert.equal(POSE_CAST_ELEMENTS, 5);   // HT-WAIST-NET's pose hl moved it on (world106), AUDIT 68's relay law before it (world105), TITLE-N before it (world104); world98 carried these fields; HCC-PARK + RIDE moved the version on (world99) with the park frame and the pose's mount, DISC7 (world100) with its half-speed bit, DISC12 (world101) with its hand and beast bits, and the community arc's frames and AUDIT ATTACH's meters (world102)

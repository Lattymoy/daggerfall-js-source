...
test('VC3: the shaders - the composite\'s ray is the dome\'s line for line, every uniform declared is fetched, the blend is sky * T + cloud', () => {
  ...
  assert.match(COMPOSITE_FS, /outColor = vec4\(cloud \* \(1\.0 \+ uFlash \* 2\.0 \+ bolt \* 3\.0\), c\.a\);/, 'the composite lights every texel for the frame (WEATHER3d: and a distant strike its own cloud beside it)');
...
test('VC3: the seam - the clouds ride the dome only, behind the one switch, on the same row, dt and drift; the hosts hand the viewport and the flash', () => {
  ...
  assert.match(shared, /const cb = dreadW > 0 \? \{ word: skyWord, row: weatherRowNow \} : cloudBaseOf\(extra, weatherName, weatherRowNow\);[^\n]*\n\s*const cloudSky = cb\.row === weatherRowNow \? enhancedSky\.state : skyState\(\{ minuteOfDay, weather: cb\.word, classicMinutes: extra\?\.classicMinutes \?\? 0, seconds, drift: driftXZ, row: cb\.row \}\);\s*\n\s*clouds\.setState\(cloudSky, cb\.row, cb\.word, easeDt, driftXZ, \(extra\?\.flash \?\? 0\) \+ dreadGlow, extra\?\.pos \?\? null, extra\?\.cells \?\? null\);/, 'the eased row, the front-stretched dt, the one drift integral, the host\'s flash and position (WEATHER2c: and the field\'s cells; WEATHER3c: the dome\'s own state off the map\'s lane, clear air\'s on it)');
  ...
  assert.match(lab, /if \(cloudsDoor !== 'off'\) sky\.cloudsExternal = true;/, 'DS2: the lab too, under either sky');

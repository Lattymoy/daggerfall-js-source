...
test('JAN1 (fist): THE POSE IS THE PLAYER\'S, NOT THE RIG\'S - the port has four PlayerWeapons against DFU\'s one WeaponManager; a rig frozen at a left hand with an empty left slot hands the screen back a null weapon (the fist) while the torch, read off the entity, is right; the pair crosses through HARD2c\'s one home, and EVERY door of the interior host carries it', () => {
  ...
  assert.match(body("function exitDungeonNow()", "mwViewTransition('Exterior')"), /const pose = weaponPoseOf\(dungeonCtx\.weaponRig\?\.\(\)\?\.playerWeapon \?\? null\);\s*\n\s*dungeonCtx\.destroy\(\);[\s\S]*?host\.applyWeaponPose\?\.\(pose\);/, 'a dungeon\'s exit reads the pair BEFORE the teardown and hands it over after the mode');
  assert.match(body("forceExitToExterior({ cacheScene = true } = {}) {", "AUDIT 63r F30"), /const pose = mode === 'dungeon' \? weaponPoseOf\(dungeonCtx\?\.weaponRig\?\.\(\)\?\.playerWeapon \?\? null\) : mode === 'interior' \? weaponPoseOf\(interiorWeapon\.playerWeapon\) : null;[\s\S]*?if \(pose\) host\.applyWeaponPose\?\.\(pose\);/, 'the forced exit (a load, a teleport) the same, from whichever rig is live');
  assert.match(wm, /mode = 'interior';\n\s*host\.unlockOn\?\.\(\);[^\n]*\n\s*setWeaponPose\(interiorWeapon\.playerWeapon, host\.weaponPose\?\.\(\) \?\? null\);/, 'a building\'s entry seeds the interior rig from the exterior\'s');
  assert.match(wm, /mode = 'dungeon';\n\s*host\.unlockOn\?\.\(\);[^\n]*\n\s*setWeaponPose\(dungeonCtx\?\.weaponRig\?\.\(\)\?\.playerWeapon \?\? null, host\.weaponPose\?\.\(\) \?\? null\);/, 'a dungeon\'s entry seeds the dungeon rig');

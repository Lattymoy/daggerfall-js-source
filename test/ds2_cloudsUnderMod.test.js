...
test('DS2: the seam - built on the lane, the mod’s sheets stand down, the clouds and the deck under the mod', () => {
  ...
  assert.match(shared, /const cb = cloudBaseOf\(extra, weatherName, weatherRowNow\);[^\n]*\n\s*clouds\.setState\(cloudsStateUnderMod\(st, dynamicMoons, \{ minuteOfDay, weather: cb\.word, classicMinutes: nowMinutes, seconds, drift: driftXZ, row: cb\.row \}\),\s*\n\s*cb\.row, cb\.word, easeDt, driftXZ, extra\?\.flash \?\? 0, extra\?\.pos \?\? null, extra\?\.cells \?\? null\);/, 'the same row, dt, drift, flash and position the dome path hands them (WEATHER3c: the row and word the clouds stand on - the eased worn row off the map\'s lane, clear air on it)');

...
test('audit24 wave43: all four spawn sites roll, AFTER the equipment', () => {
  ...
  const gen = at('entity.items = generateItems('); const eq = at('const eq = equipEnemy(entity, mobileType, player.level, rolls, { player });'); const ex = at('addEnemyLootExtras(entity.items, basics, rolls);');

...
test('AUDIT WORLD6b-iii(a) by source: the sinks say whose blow it is (B2, the three doors); the leap spends no token (B7); every release counts, the rider too (A9); the wall arm\'s caster (A1); the dungeon\'s probe is MY capsule and its stand-in the shared one (C2/A10); Testing.md and the arc no longer state the retired law (C5/C7); the record', () => {
  ...
  assert.match(x, /hurt: \(n, o\) => damageFoe\(f, n, null, null, \{ fromPlayer: false, kind: 'spell', whole: !!o\?\.whole \}\),/, 'B2: the pool\'s sink (AUDIT PSCALE1 DOORS-1: a kill\'s flag rides it)');
  assert.match(rd('src/scenes/world.js'), /const foeSinks = \(g, fromPlayer = true\) => \(\{[^\n]*\n\s*hurt: \(n, o\) => \{ const fp = o\?\.fromPlayer \?\? fromPlayer; if \(n > 0\) \(g\._encounter \? exteriorFoes\.damageFoe\(g, n, player\.pos, null, \{ fromPlayer: fp, kind: 'spell', whole: !!o\?\.whole \}\) : cityGuards\.hurtGuard\(g, n, player\.pos, null, \{ fromPlayer: fp \}\)\); \},/, 'B2: the host\'s sink reads the engine\'s provenance (AUDIT PSCALE1 DOORS-1: and a kill\'s flag)');

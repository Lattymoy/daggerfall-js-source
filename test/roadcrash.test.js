...
// shape, with the host's onClose wired as world.js:7792 wires it).
...
function rig() {
  ...
  const said = [], scales = [];
  let to = null;
  // world.js:7792 - CAMP / close is InterruptTravel; the port's panel is
  ...
  to = createTravelOptions({
    ...
    // world.js:7886's own answer for both.
...
function standInTheRing(r) {
  ...
  r.to.onMapPixelChanged({ x: 500, y: 250 });   // world.js:3967 - the rects for the pixel the player stands in

...
// and stops doing so the moment it stops being drawn. With the lantern not
...
import { createEotbBody, bodyState, loadLanternArt, LANTERN_ART_KEY, HIP_LANTERN_SPRITE } from '../src/player/eotbBody.js';
...
test('HT-WAIST (EOTB): it SWINGS with the sprite\'s walk - a walk begun tilts the quad in the view plane and shortens it as it swings toward or away from the eye; it settles upright at rest (mutant: the swing not stepped, or not drawn)', async () => {
  const { b, r } = await liveBody();
  ticks(b, 12, state({ hipLantern: true }));
  await drawn(b);
  ...
  // turning to walk to the right of the screen: it swings in the view plane now - the quad tilts
  const sideways = state({ hipLantern: true }, { forward: 0, strafe: 1, standing: false, speed: 5 });
  ticks(b, 40, sideways);
  b.draw(null, view);
...
test('HT-WAIST (EOTB): it lights you from where it hangs, and stops the moment it is not drawn - put out, first person, the saddle, the beast, the body toggled off, another lane (mutant: the light point never written, or left behind)', async () => {
  ...
  try {
    ...
    // first person: the billboard is the first-person one - no lantern
...
test('HT-WAIST (EOTB): the picture is the Lantern template\'s own world texture - read off the template, never a literal; no data, no throw (mutant: the art door throwing, or a hard-wired record)', async () => {
  ...
  const src = rd('src/player/eotbBody.js');

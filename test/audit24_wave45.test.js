...
test('audit24 wave45: the two streaming hosts route through it and RETURN on both non-look answers', () => {
  for (const f of ['src/scenes/world.js', 'src/scenes/exterior.js']) {
    ...
    const look = body.indexOf('lookFilter.add(');   // AUDIT 28 W7: the look goes to the filter now
...
test('audit24 wave45: the standalone dungeon host keeps its own shape, and it is the correct one', () => {
  // dungeon.js:214-220 - attack, then return, with no mode in the test
  ...
  const ret = body.indexOf('return;');
  assert.ok(body.indexOf('lookFilter.add(') > ret, 'and the look is downstream');   // AUDIT 28 W7
});

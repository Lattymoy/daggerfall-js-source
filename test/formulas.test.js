...
import {
  ...
  calculateSuccessfulHit, handToHandAttackDamage, weaponAttackDamage,
...
test('formulas: to-hit chain (dodging/4 bug preserved, clamp 3..97)', () => {
  ...
  // chance = 40 + 0 + 1 + (-10) [crit roll 0.99 fails] = 31; hit roll 0.30 -> 30 < 31 hits
  assert.equal(calculateSuccessfulHit(A, T, 40, 0, seq(0.99, 0.30)), true);
  assert.equal(calculateSuccessfulHit(A, T, 40, 0, seq(0.99, 0.31)), false);
  // clamp floor 3: even absurd negatives hit on a 0.02 roll
  const T2 = { ...T, armor: -200 };
  assert.equal(calculateSuccessfulHit(A, T2, 40, 0, seq(0.99, 0.02)), true);
  assert.equal(calculateSuccessfulHit(A, T2, 40, 0, seq(0.99, 0.03)), false);
});

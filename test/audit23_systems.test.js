...
test('AUDIT 23 characters-7 + items-1: the spawn bands and the pile gender ride the constructions', () => {
  ...
  assert.ok(dc.includes('const items = generateLootItems(lootKey, { level: playerEntity.level, gender: playerEntity.gender }, undefined, elite ? { itemChanceScale: ELITE_LOOT_DROP_MULT } : {});') && dc.includes('const items = rollPileItems();'), 'piles roll the player gender (WORLD8: through the one pile roll)');

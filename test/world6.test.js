...
test('WORLD6a: the wire - a building is a world room at both ends (the room roomKeyFor mints, the map id unsigned, the building key up to MakeBuildingKey\'s sentinel), a slugged location or a town is not, and the relay says which one it is', () => {
  ...
  assert.equal(RELAY_VERSION, 'world6');
});
...
test('WORLD6a: the pure half - the vocabulary is the cache\'s own keys, one spelling each; an opened container is said with what is left and the day it was stocked, a never-opened one is not, and one past the cap stays this player\'s own, said once', () => {
  ...
  assert.deepEqual(interiorLootRecords(ctx, ['shelf:0', 'shelf:1', 'container:0', 'loot:0']), [{ k: 'shelf:0', r: [{ name: 'Dagger', templateIndex: 1 }, { name: 'Bread', templateIndex: 2 }], d: 12 }], 'the opened shelf alone');
...
test('WORLD6a: the pure half - the room\'s word lands IN PLACE on an opened container, WHOLE on one this client never opened (a second reader adopts the first\'s roll), with the day it was stocked, and never under this player\'s open window', () => {
  ...
  assert.equal(n, 3);
  assert.equal(ctx.shelves[0].items, live, 'the same array'); assert.deepEqual(live, [{ name: 'Bread', templateIndex: 2 }]); assert.equal(ctx.shelves[0].stockedDate, 20);
  assert.deepEqual(ctx.shelves[1].items, [{ name: 'Robe', templateIndex: 3 }, { name: 'Boots', templateIndex: 4 }], 'never opened here: the room\'s list, whole'); assert.equal(ctx.shelves[1].stockedDate, 20);
...
test('WORLD6a: the pure half - the memory is the opened containers the room knows of and every action record\'s SHARED half; it lands once, projected, never its own or another building\'s; a refused act\'s keys are re-read current', () => {
  ...
  assert.deepEqual(joiner.shelves[0].items, [{ name: 'Dagger', templateIndex: 1 }, { name: 'Bread', templateIndex: 2 }]); assert.equal(joiner.shelves[0].stockedDate, 12);
...
test('WORLD6a: the hosts by source - the interior mode keys and stamps its room at the mount (an owned house or ship keeps none), wires the doors\' seam, claims a container on the open, says a restock and the close through the frame\'s settle, fires the leave before both teardowns, and the mode machine dispatches the PLACE; the world host walks the one path; the pane says it', () => {
  ...
  assert.match(m, /const owned = \(b\?\.buildingType === BUILDING_TYPES\.Ship && ownsShip\(playerEntity\)\) \|\| isHouseOwned\(playerEntity\.houses \?\? \[\], b\?\.regionIndex \?\? 0, b\?\.buildingKey\);\s*const key = owned \? null : interiorLocationKey\(questSceneCtx\?\.\(\)\?\.mapId \?\? 0, b\?\.buildingKey \?\? 0\);\s*_intShared = \{ key, stamp: Math\.random\(\)\.toString\(36\)\.slice\(2\), seen: new Set\(\), tooBig: new Set\(\), applied: false, openKey: null, openWin: null \};\s*if \(key\) ctx\.actions\.onChanged = \(recs\) => host\.onActions\?\.\(\{ k: key, a: recs \}\);/, 'the mount');
  assert.match(m, /interiorPublishLoot\(key\);   \/\/ WORLD6a: the new day's stock is the room's/, 'a cupboard\'s restock');
  assert.match(m, /if \(win\) \{ interiorOverlay = win; if \(!owned\) interiorLootOpened\(key, win\); \}/, 'a stranger\'s cupboard claimed on the open, an owner\'s never');
  assert.match(m, /interiorPublishLoot\(`shelf:\$\{i\}`\);   \/\/ WORLD6a: the new day's stock is the room's, whoever browsed first/, 'a shelf\'s restock');
  assert.match(m, /if \(win\) \{ interiorOverlay = win; interiorLootOpened\(`shelf:\$\{i\}`, win\); \}/, 'the closed shop\'s shelf, on the open');
  assert.match(m, /interiorOverlay = openTradeWindow\(shelf, b, 'Buy'\);\s*interiorLootOpened\(`shelf:\$\{i\}`, interiorOverlay\);/, 'the trade window\'s shelf, on the open');
  assert.match(m, /if \(shelf\) interiorLootOpened\('shelf:0', win\);/, 'the merchant\'s sell screen');
  assert.match(m, /if \(mode === 'interior'\) interiorLootSettle\(\);/, 'the close is the frame\'s settle');
  assert.match(m, /function interiorLootSettle\(\) \{\s*const s = _intShared;\s*if \(!s\?\.openWin\) return;\s*if \(interiorOverlay === s\.openWin && !s\.openWin\.done\) return;\s*const k = s\.openKey;\s*s\.openKey = null; s\.openWin = null;\s*interiorPublishLoot\(k\);\s*\}/);
  ...
  assert.match(m, /placeSharedWorld\(\) \{\s*if \(mode === 'dungeon'\) return dungeonCtx \? dungeonCtx\.sharedWorld\(\) : null;\s*return mode === 'interior' && interiorCtx && _intShared\?\.key \? composeInteriorShared\(interiorCtx, _intShared\) : null;/);
  assert.match(m, /restorePlaceSharedWorld\(shared\) \{[\s\S]*?if \(ok\) _intShared\.applied = true;/, 'the memory lands once per context');
  assert.match(m, /applyPlaceActions\(id, data\) \{[\s\S]*?data\.k !== _intShared\.key\) return false;/, 'another building\'s act is not this one\'s');
  assert.match(m, /placeActionRecords\(keys\) \{[\s\S]*?interiorActionRecords\(interiorCtx, keys, \{ locationKey: _intShared\.key, tooBig: _intShared\.tooBig \}\)/);

...
test('AUDIT 58: all three foe damage doors consult the Shield pool, and the zeroing door bypasses it', () => {
  ...
  for (const file of ['scenes/dungeonContext.js', 'scenes/exteriorFoes.js']) {
    assert.match(src(file), /zeroFoeHealth: \(f\) => \{ if \(!f\.dead(?: && !f\.puppet)?\) damageFoe\(f, f\.entity\.health, null, null, \{ fromPlayer: false, bypassShield: true \}\); \}/,   // AUDIT RENOWN1 GAME-6: and nobody's blow   // AUDIT WORLD6b B9: the encounter pool's door refuses a peer's foe

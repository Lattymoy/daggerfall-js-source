...
test('audit24 wave24: the duplicate-declaration count does not grow', () => {
  ...
  const dupes = [...decl.entries()].filter(([, v]) => v.size > 1);
  // 27 -> 29 (Mac 2026-08-31): TABS and filterByTab, the enhanced
  // pack's armour split. Both have their reason in HOMONYMS above; the
  // ratchet is a raw total, so a deliberate homonym still has to be
  // paid for here rather than slipping in.
...
test('audit24 wave24: GetDisplayName names the NPC from the seed, and the click uses it', () => {
  ...
  assert.match(wm, /\{ data: npcData, isChildNPC: isChildNPCData\(npcData\), displayName \},/);   // F020

...
test('ECON1 by source: the shared clock installs and removes the world\'s price source; the player\'s walk and the world\'s share one step and one flag home; every consumer reads through regionPriceAdjustment (no host reads regionPrices directly); the powers keep their own law; the wire is untouched', () => {
  ...
  assert.ok(rd('src/net/wire.js').includes("export const RELAY_VERSION = 'world98'"), 'no wire change of this slice\'s: the economy is computed, not streamed (world84 was RELAY-H1\'s, world85 is ACC1d\'s token field, world91 the three drops\', world92 AUDIT DROPS\' relay meters, world96 the party-rest drop\'s pose fields)');
});

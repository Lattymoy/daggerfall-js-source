...
test('FT15: every note is one or two sentences, and the panel stays under its budget', () => {
  const MAX_ROW = 450;
  const MAX_TOTAL = 8450;   // BA1: two more mod rows (IF1, BA1) at under 160 chars each; EL1: the lighting row (~380 chars); QS: the quickslot diamond's row, under 200; ORL1: the leveling mod's row, 243; every row still under MAX_ROW
  let total = 0;
  for (const f of FEATURES) {

...
import '../src/systems/featureLanes.js';   // FT18: the wind, the quick slots and the blood
...
test('FT0: three kinds in label order, three stores, and the registry is empty and sound', () => {
  ...
  assert.deepEqual(FEATURES.map((f) => f.id), ['smaller-dungeons', 'land-view-distance', 'enhanced-environments', 'enhanced-ai', 'enhanced-water', 'enhanced-lighting', 'grass', 'cloud-quality', 'ground-sharpness', 'render-scale', 'enhanced-combat-visuals', 'quick-loot', 'loot-rarity', 'wind', 'enhanced-sounds', 'first-person-lighting', 'enhanced-map', 'weather-events', 'mod-windmills-kamer', 'mod-seasons-iliac-bay', 'mod-roads-hazelnut', 'mod-travel-options', 'mod-world-of-daggerfall', 'mod-aquatic-sprites', 'mod-detailed-ships', 'mod-warm-ashes-ships', 'mod-iliac-puddle-no-more', 'mod-meanermonsters', 'mod-pcaao', 'mod-unleveledloot', 'mod-weapon-widget', 'mod-shield-widget', 'mod-diverse-weapons', 'mod-roleplay-realism-items', 'mod-roleplay-realism', 'mod-handheld-torches', 'mod-ambient-text', 'mod-eye-of-the-beholder', 'mod-immersive-footsteps', 'mod-world-tooltips', 'mod-better-ambience', 'mod-horse-cart-and-cargo', 'mod-weapon-sheathing', 'mod-oblivion-remaster-leveling', 'enemy-infighting', 'varied-dungeon-monsters', 'torches-from-items', 'combat-voices', 'near-death-warning', 'bows-left-hand', 'choose-guild-jobs', 'dungeon-wall-style', 'quick-slots', 'wilderness-camps', 'town-watch', 'blood', 'mod-climates-calories'], 'FT0 shipped the home empty; FT1, FT2, FT4-FT11 moved rows in (one slice at a time); LR1 added the first row built in house; WIND3 three more; WEATHER2b the weather field; WM3 gave Windmills the switch it had never had; AT0 Ambient Text; EOTB0 Eye Of The Beholder; IF1 Immersive Footsteps; BA1 Better Ambience; EL1 Enhanced Lighting; WS1 Weapon Sheathing; TO1 Travel Options; MAC-I first-person lighting; MAP-TOGGLE the enhanced map; ORL1 Oblivion Remaster Like Leveling; SURV2 the survival arc; SW1 Shield Widget; GRAIN2 ground sharpness; GRASS-PX the grass style; WORLD-HOVER World Tooltips; QUICK-LOOT the plaque that takes; DW1 Diverse Weapons; RRI1 Roleplay & Realism: Items; RR1 Roleplay & Realism; HB1 the quickbar-or-hotbar switch; DISC19-F the town watch; FT18 condensed ten rows into four - grass (density and style), wind (sway and wisps), quick slots (the diamond and the bar) and blood (marks, overkill, lens and gore)');
...
test('FT15: every note is one or two sentences, and the panel stays under its budget', () => {
  const MAX_ROW = 450;
  const MAX_TOTAL = 14738;   // DW-D: the Iliac Puddle No More row, 356 chars - the ceiling rises by a new row's own size and no more; WA1: the Warm Ashes - Ships row, 321 chars - the ceiling rises by a new row's own size and no more; DS1: the Detailed Ships row, 289 chars - the ceiling rises by a new row's own size and no more; AS1: the Aquatic Sprites row, 236 chars - the ceiling rises by a new row's own size and no more; SIGIL1: the loot-rarity row's sigil sentence, +72 chars - the ceiling rises by the growth and no more; PERF-SCALE: the render-scale row, 228 chars - the ceiling rises by a new row's own size and no more; FT18: four tiles' worth of notes were eight - grass, wind, quick slots and blood each say theirs once - so the ceiling FALLS to the new total, a drift guard held tight; DISC19-F: the town-watch row, 264 chars; HB1: the quickbar-or-hotbar row, 188 chars - the ceiling rises by a new row's own size and no more; WOD1: the World of Daggerfall row, 313 chars; HCC: the Horse Cart and Cargo row, 244 chars - the ceiling rises by a new row's own size and no more; SURV-TIERS: the Climates & Calories row names its three tiers and what each costs, +77 chars on a row whose one control became three - the ceiling rises by the growth and no more (AUDIT SURV-TIERS: +30 more, Casual's stamina is LENT and repaid, and the row is two sentences now, not three; the second pass: +9, the needs BORROW - "lends" had the loan backwards - and "half the bar" says whose half it is); MAP-TOGGLE: the enhanced-map row, 199 chars, pays its own way; QUICK-LOOT: the quick-loot row, 210 chars - the port's own, so it pays its own way like every row before it; WORLD-HOVER: the World Tooltips row, 391 chars - the ceiling rises by a new row's own size and no more; GRAIN AUDIT 1: the ground-sharpness row says which end costs, 250 chars; GRASS AUDIT 1: the grass-style row says what it is inert without, 283 chars; BLOOD2g: the gore row, 215 chars; BLOOD2e: the lens row, 182 chars; BLOOD1b: the overkill row, 206 chars; BLOOD1: the blood row, 333 chars; GRAIN2: the ground-sharpness row, 158 chars - the ceiling rises by a new row's own size and no more; SW1: the shield widget's row, 258 chars; TO1: the travel row (~350 chars); SURV2: the survival arc's row, 123 chars; CAMP1: wilderness camps & packs, 179 chars; ORL1: the leveling mod's row, 243; WS1: the sheathing row, under 230 chars; BA1: two more mod rows (IF1, BA1) at under 160 chars each; EL1: the lighting row (~380 chars); QS: the quickslot diamond's row, under 200; MAC-I: first-person lighting, 235 - the ceiling is a DRIFT guard, so a new row raises it by its own size and no more; every row still under MAX_ROW; DW1/RRI1/RR1: Diverse Weapons 324, Roleplay & Realism: Items 222, Roleplay & Realism 440 (merged 2026-09-23)
  let total = 0;
  for (const f of FEATURES) {

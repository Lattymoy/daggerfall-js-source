...
test('FT15: every note is one or two sentences, and the panel stays under its budget', () => {
  const MAX_ROW = 450;
  const MAX_TOTAL = 8680;   // WS1: the sheathing row, under 230 chars; BA1: two more mod rows (IF1, BA1) at under 160 chars each; EL1: the lighting row (~380 chars); QS: the quickslot diamond's row, under 200; MAC-I: first-person lighting, 235 - the ceiling is a DRIFT guard, so a new row raises it by its own size and no more; every row still under MAX_ROW
  let total = 0;
  for (const f of FEATURES) {

...
test('U42: every mutation lands on the player\'s OWN array - the save envelope sees it', () => {
  // PlayerEntity.GetSpells() is the book itself and save.js:172 maps
...
test('U42 rename: a COPY takes the new name, marked custom so the save carries it', () => {
  ...
  // `custom` flag is what save.js:172 reads to store the whole
...
test('U42 rename: the renamed COPY survives the save envelope', () => {
  // The `custom` flag is not decoration - save.js:172 stores the whole
...
test('U42 clicks: a list row selects, and a second click inside the double-click window readies it', () => {
  ...
  // - townTalk.js:1156, worldModes.js:7590, dungeonContext.js:5907 -

...
test('TL3: the fast-travel arrival takes DFU\'s reposition - the start marker or the edge, never the pixel\'s centre - so TL2\'s roof guard is live on it', () => {
  ...
  // (AUDIT SURV-TIERS, the third pass: 9000 -> 10000 - the camps' passage through natives across the re-anchored
  // frame sits above these needles, and the facing's had been nine characters inside the old bound)
  const core = w.slice(w.indexOf('async function _teleportToPixel('), w.indexOf('async function _teleportToPixel(') + 10000);

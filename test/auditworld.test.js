...
test('AUDIT WORLD A: the relay - a large frame is the host\'s memory or nothing, refused with no hello, ignored UNPARSED for anyone else and metered like a pose (A1); the prefix admits the size and the type keeps the cap (A2); a world room is a map id\'s, and its memory is forgotten WORLD_TTL_MS after the last drain unless someone is in (A3); a reconnect keeps the host\'s seat (A4) and the floor is the room\'s stamp (A5); the farewell is admitted once inside the floor, the host\'s alone (B5); the put and the tail\'s delete are one write (A6)', async () => {
  ...
  assert.match(room, /if \(typeof message === 'string' && \(message\.length > MAX_FRAME_BYTES \|\| message\.startsWith\(WORLD_PREFIX\) \|\| message\.startsWith\(FOES_PREFIX\)\)\) \{\s*const foesLike = message\.startsWith\(FOES_PREFIX\);\s*if \(!a\.id\) \{ this\._refuse\(ws, foesLike \? 'foes before hello' : 'world before hello'\); return; \}[\s\S]*?if \(!\(foesLike \? streamsFoes\(a\.key\) : isWorldRoom\(a\.key\)\) && message\.length > MAX_FRAME_BYTES\) \{ this\._refuse\(ws, 'frame too large'\); return; \}\s*a = foesLike \? this\._meterFoes\(ws, a, Date\.now\(\)\) : this\._meter\(ws, a, Date\.now\(\)\);\s*if \(!a\) return;\s*if \(!\(foesLike && isCellRoom\(a\.key\)\) && \(!isWorldRoom\(a\.key\) \|\| a\.id !== this\._hostOf\(\)\)\) \{[^\n]*\n[\s\S]*?const junk = \(a\.junk \?\? 0\) \+ 1;[\s\S]*?if \(junk > DROP_STRIKES_MAX\) this\._refuse\(ws, 'too many frames'\);\s*return;\s*\}\s*doored = foesLike \? 'foes' : 'world';\s*\}\s*const m = parseClient\(/, 'A1: the door before the parse (WORLD2: the foes frame through it on its own bucket; AUDIT WORLD2 A3/A4/A7: the prefix remembered, a large frame refused outside a world room, a non-host\'s stream struck out, the refusal named right; WORLD6b: a cell\'s foes frame is anyone\'s)');

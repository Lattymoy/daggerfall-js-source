...
test('AUDIT WORLD A: the relay - a large frame is the host\'s memory or nothing, refused with no hello, ignored UNPARSED for anyone else and metered like a pose (A1); the prefix admits the size and the type keeps the cap (A2); a world room is a map id\'s, and its memory is forgotten WORLD_TTL_MS after the last drain unless someone is in (A3); a reconnect keeps the host\'s seat (A4) and the floor is the room\'s stamp (A5); the farewell is admitted once inside the floor, the host\'s alone (B5); the put and the tail\'s delete are one write (A6)', async () => {
  ...
  assert.match(room, /if \(typeof message === 'string' && \(message\.length > MAX_FRAME_BYTES \|\| message\.startsWith\(WORLD_PREFIX\) \|\| message\.startsWith\(FOES_PREFIX\)\)\) \{\s*const foesLike = message\.startsWith\(FOES_PREFIX\);\s*if \(!a\.id\) \{ this\._refuse\(ws, foesLike \? 'foes before hello' : 'world before hello'\); return; \}[\s\S]*?if \(!\(foesLike \? streamsFoes\(a\.key\) : isWorldRoom\(a\.key\)\) && message\.length > MAX_FRAME_BYTES\) \{ this\._refuse\(ws, 'frame too large'\); return; \}\s*a = foesLike \? this\._meterFoes\(ws, a, Date\.now\(\)\) : this\._meter\(ws, a, Date\.now\(\)\);\s*if \(!a\) return;\s*(?:\/\/[^\n]*\n\s*)*if \(foesLike && isCellRoom\(a\.key\)\) \{ const ingress = byteGate\(this\._roomFoesIn, Date\.now\(\), message\.length, FOES_ROOM_BYTES_PER_S\); this\._roomFoesIn = ingress\.bucket; if \(!ingress\.pass\) return; \}\s*if \(!\(foesLike && isCellRoom\(a\.key\)\) && \(!isWorldRoom\(a\.key\) \|\| a\.id !== this\._hostOf\(\)\)\) \{[^\n]*\n[\s\S]*?const junk = \(a\.junk \?\? 0\) \+ 1;[\s\S]*?if \(junk > DROP_STRIKES_MAX\) this\._refuse\(ws, 'too many frames'\);\s*return;\s*\}\s*doored = foesLike \? 'foes' : 'world';\s*\}\s*const m = parseClient\(/, 'A1: the door before the parse (WORLD2: the foes frame through it on its own bucket; AUDIT WORLD2 A3/A4/A7: the prefix remembered, a large frame refused outside a world room, a non-host\'s stream struck out, the refusal named right; WORLD6b: a cell\'s foes frame is anyone\'s; AUDIT WORLD6b A3: under the room\'s ingress budget)');
...
test('AUDIT WORLD: the hosts by source - the dungeon host stamps its memory and applies one once (B1/B7), carries and reads the species (B4), keeps the drops the save\'s alone (B3), says its death and its rig (B6/C1) and its one "Game loaded." (B10); the mode machine names the live rig and the death screen (C1/B6); the world host publishes into a world room alone as a farewell when forced and says a refusal once (B5/B8/B9) and gates the dead on the mode\'s slot (B6); the arm casts sheathed (C2); the body re-latches on the linger (C7); the record\'s false sentences are gone (D2/D6/D7/D8)', () => {
  ...
  assert.match(w, /instanceof DeathScreen \|\| modes\?\.deathUp\?\.\(\)\) \{\s*\n\s*if \(_deathWasOnline == null\) _deathWasOnline = _onlineWorldSession\(\);[^\n]*\n\s*if \(online\.room\) \{ worldPublish\(now, true\); online\.leave\(\); exteriorFoes\.clearPuppets\(\); _foesRoom = null; \}/, 'B6: the dungeon\'s death farewell (AUDIT WORLD6b C8: and the cell\'s puppets go with the room; D-ONLINE1: the online snapshot is taken BEFORE the leave clears the room)');

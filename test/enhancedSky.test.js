...
test('ES1 seam: enhanced skin only, one renderer field, the classic pass untouched', () => {
  ...
  for (const host of ['src/scenes/world.js', 'src/scenes/exterior.js']) {
    assert.match(read(host), /\{ weather, violence: weatherOverride \?\? currentWeatherRaw\(\), classicMinutes: playerTicker\.classicMinutes, sun: wxNow\.sun, flash: flash - 1, pos: [^}]+ \}\);\s*\/\/ WEATHER2a[^\n]*ES1/, `${host} feeds the sky`);
  }
...
test('ES1c clouds and dither: two decks lit by the sun, and a triangular dither on the gradient', () => {
  ...
  // hash with interleaved gradient noise; FT3 removed the retro pass, so
  // this is the dome's ONE dither now - what is pinned is that there is
  // one, and where.)
...
// ── ES1e / ES1f / ES1g: THE RETRO PASS - REMOVED (FT3, 2026-09-14, Mac:
// "Remove our version of pixelated sky"). The pins went with the pass.

...
test('silhouette: front IoU vs the classic body mask', { skip: skipReal }, () => {
  ...
  // Floor = fit 0.8371 minus margin; ratchets UP only.
  assert.ok(iou >= 0.82, `IoU ${iou.toFixed(4)} below the floor`);
  ...
  assert.ok(excess <= 0.13, `excess ${(excess * 100).toFixed(1)}% of sprite area above the 13% ceiling`);
});

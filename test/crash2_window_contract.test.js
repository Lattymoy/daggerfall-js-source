...
//       townTalk.js:1179 paints every COVERED window too
...
test('CRASH2: the required arms are DERIVED from the hosts, not typed here', () => {
  ...
  for (const arm of ['hover', 'pointer', 'keyup', 'click']) {
    assert.ok(optional.has(arm), `\`${arm}\` is guarded at every call site, so it is optional by design (townTalk.js:488 says so)`);
  }
});

...
const HOSTS = [
  ...
  },
  {
    ...
    keyUp: /interiorOverlay\?\.keyup\?\.\(e\.code, e\);/,      // STATUS-LIVE: the arm's gate is interiorPaused(); the slot read inside it is optional
...
//
// THIS PORT OPENS ON THE PRESS in all four hosts (world.js:7525/:7527,
// exterior.js:3092/:3100, ui/input.js:579/:585) and then routes that
// same key's release into the window it just mounted (world.js:7561 ->
...
/** world.js:7497's keydown arm, verbatim in shape: the host consumes
...
test('E-FIX LIVE (townTalk): the R that OPENS the rest window does not close it on its own release', () => {
  ...
  try {
    ...
    // world.js:7548 delivers THAT SAME KEY'S release into the slot.

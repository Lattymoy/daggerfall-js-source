...
test('AUDIT PARTY-REST wire (world95, then world96 under ALLY-CAST, world97 under its audit): `readyAt` rides the party pose under voteAt\'s own bounds - rounded, floored at zero, absent as null; the version moved with it', () => {
  ...
  assert.equal(RELAY_VERSION, 'world106', 'AUDIT 68 (the relay\'s own law, world105) last; TITLE-N (the dm frame, the badge vocabulary) before it; ALLY-CAST moved it again, AUDIT ALLY-CAST once more, SPELLFX1\'s pose fields a third time, HCC-PARK + RIDE (the park frame, the pose\'s riding fields) a fourth, DISC7\'s hs and DISC12\'s lh/wb after, and the community arc\'s frames and AUDIT ATTACH\'s meters after them (world102)');

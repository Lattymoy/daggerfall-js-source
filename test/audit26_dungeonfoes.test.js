...
test('F051: the ?dungeon fly-cam is gated on the OVERLAY, not on the imported input helper', () => {
  // `held` at dungeon.js:44 is the input helper - a function, always
...
test('F212: the world host collects both pools with the pixel, which is also what the teleport tears down', () => {
  ...
  // PIN MOVED AGAIN (AUDIT SURV-TIERS, the third pass), 3700 -> 4300: the
  // camps now go through natives across the re-anchored frame, their
  // snapshot and teardown with their own note above the needles.
  const core = WORLD.slice(t, t + 4300);

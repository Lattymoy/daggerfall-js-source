...
/** A rest window wired exactly as exterior.js:774 and world.js:1843
...
const DOORS = {
  ...
  // same re-entrancy hazard the press drain was.
  'backing out of the selection page': (w) => { w.state = 'selection'; w.keyup('back'); },
...
test('CRASH 2026-08-29: the real talk host survives a window that closes it from inside dispose', () => {
  ...
  const win = fakeWindow({
    ...
    dispose() {
      ...
      if (tt.overlay?.isRestWindow) tt.closeOverlay?.();   // exterior.js:774, world.js:1843
      depth--;
    },
  });
  tt.showOverlay(win);

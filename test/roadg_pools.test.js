...
//      `foes` (exteriorFoes.js:447-452) and both pools share the host's
...
test('ROAD-G G1(a): all three arrow hosts ROUTE the hostility seam by pool', () => {
  ...
  // (exteriorFoes.js:2040), so every host can reach it.
...
test('ROAD-G G1(b): both hosts route the transform by POOL MEMBERSHIP', () => {
  ...
  // (exteriorFoes.js:447-452) never looks a record up in `foes` and

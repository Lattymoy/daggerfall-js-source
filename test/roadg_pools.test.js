...
//      `foes` (exteriorFoes.js:406-411) and both pools share the host's
...
test('ROAD-G G1(a): a ZERO-DAMAGE player ARROW reaches the watch\'s door too', () => {
  ...
  // (arrowFlight.js:305-311), and `onAttackFromPlayer`, which it calls
  // unconditionally at :314 because that is where WeaponManager.cs:630
...
test('ROAD-G G1(a): all three arrow hosts ROUTE the hostility seam by pool', () => {
  ...
  // (exteriorFoes.js:1884), so every host can reach it.
...
test('ROAD-G G1(b): both hosts route the transform by POOL MEMBERSHIP', () => {
  ...
  // (exteriorFoes.js:406-411) never looks a record up in `foes` and

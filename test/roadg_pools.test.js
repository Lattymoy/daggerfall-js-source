...
//      `foes` (exteriorFoes.js:380-385) and both pools share the host's
...
test('ROAD-G G1(a): the door is gated on the PLAYER being the source (F035\'s law, kept)', () => {
  ...
  // (the cross-pool `hurtFromFoe` minted at cityGuards.js:297) or
...
test('ROAD-G G1(a): a ZERO-DAMAGE player ARROW reaches the watch\'s door too', () => {
  ...
  // (arrowFlight.js:297-303), and `onAttackFromPlayer`, which it calls
  // unconditionally at :306 because that is where WeaponManager.cs:630
...
test('ROAD-G G1(a): all three arrow hosts ROUTE the hostility seam by pool', () => {
  ...
  // (exteriorFoes.js:1773), so every host can reach it.
...
test('ROAD-G G1(b): both hosts route the transform by POOL MEMBERSHIP', () => {
  ...
  // (exteriorFoes.js:380-385) never looks a record up in `foes` and
  // both pools share the host's one renderer, so the old arm tore a
  // watchman down exactly as `removeGuard` does - batch freed,
  // `dead = true`, no corpse, skipped by cityGuards.js:817 and spliced
  // at :1006 in that same pass. The router is an OWNERSHIP fix, not a

...
test('ROAD-G G1(a): all three arrow hosts ROUTE the hostility seam by pool', () => {
  ...
  // (exteriorFoes.js:1731), so every host can reach it.
...
test('ROAD-G G1(b): both hosts route the transform by POOL MEMBERSHIP', () => {
  ...
  // `dead = true`, no corpse, skipped by cityGuards.js:787 and spliced
  // at :974 in that same pass. The router is an OWNERSHIP fix, not a

...
test('MAIL1 service: four routes behind a session, none open; acct6; a GUEST can neither read nor write (mail-needs-account, 403) and cannot be written to; the wrong method is refused (mutants: the wall dropped; a route opened to strangers)', async () => {
  ...
  assert.equal(ACCOUNT_VERSION, 'acct12');   // GUILD1c moved it on (acct12); acct6 was MAIL1's letters; TITLE-N's Dungeon Master and Patreon tiers moved it on (acct7); DUEL1's duelling record (acct8), FOUNDER2's cutoff again (acct9); RENOWN1's Renown, HOME1's homes, DECOR1's decor and GUILD1's guilds (acct10); RENOWN4's total in the mint's answer (acct11)

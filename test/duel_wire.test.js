...
test('DUEL1 the parser and the version: `duel` is its own arm (after the hello, under its own door); world107 is the first relay that routes it (mutants: the frame parsed before the hello; an old relay trusted)', () => {
  ...
  assert.equal(RELAY_VERSION, 'world111');   // AUDIT WB's relay half moved it on (world111); WB3's gate frame moved it on (world110); HT-WAIST-NET's hl moved it on (world108); DUEL1 was world105 on its branch; main's world105 (AUDIT 68) and world106 (DISC23-B) landed first

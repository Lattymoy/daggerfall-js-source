...
const RATIO = 40, WX0 = 100000, WZ0 = 200000;
const PARTS = { wheelLeftPivot: [-0.9, 0.5, -0.6], wheelRightPivot: [0.9, 0.5, -0.6], wheelRadius: 0.5, bounds: { min: [-1, 0, -1.6], max: [1, 1.3, 1] } };

/** A flat world at y = 0 around a player who owns what `items` says, the mod's settings at their defaults. */
function makeWorld({ cart = true, horse = true, settings = {}, ground = 0 } = {}) {
  const w = {
  ...
  const phys = {
    now: () => w.now,
    raycastAll: (o, d, max) => (d[1] < 0 && o[1] >= ground && o[1] - ground <= max ? [{ point: [o[0], ground, o[2]], distance: o[1] - ground, normal: [0, 1, 0] }] : []),

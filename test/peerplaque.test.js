...
test('PEER-PLAQUE1 hosts by source: the street races the F key\'s own pick and names it in the PORT\'s own (ungated) namers, with the badge marks; the building and the dungeon race and name it through the outer host\'s two doors', () => {
  ...
  assert.match(namer, /const lead = advLevelText\(online\?\.levelOf\?\.\(id\) \?\? null\);/, 'ADV1: their Adventuring Level leads the name, as it does over their head');
  assert.match(namer, /return \{ title: `\$\{lead \? `\$\{lead\} ` : ''\}\$\{marks \? `\$\{name\} \$\{marks\}` : name\}`, subs: \[cast, peerRelationText\(acts\)\]\.filter\(Boolean\), actions: acts \? socialPlaqueRows\(id, acts\) : \[\], actionsUnlit: true \};/);   // ALLY-CAST: the cast line above the relation; ACT-MENU: the card's rows, unlit

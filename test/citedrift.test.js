...
const SOURCE_CITES = [
  ...
  ['src/systems/advancement.js', /exterior\.js:(\d+)\/:1210/, EX, /^ {4}onLevelUp: \(\) => \{$/],
  ...
  ['test/overlayreentry.test.js', /exterior\.js:(\d+), world\.js:1843/,
    EX, /if \(townTalk\.overlay\?\.isRestWindow\) townTalk\.closeOverlay\?\.\(\);/],
  ['test/probehygiene.test.js', /keydown ladder, exterior\.js:(\d+)-1942/,
  ...
  ['test/roade_up_seam.test.js', /exterior\.js:(\d+)\/:2020/,
    EX, /if \(act === 'Rest'\) \{ e\.preventDefault\(\); hudCtx\.toggleRest\(\); return; \}/],
  ['test/roade_up_seam.test.js', /exterior\.js:2012\/:(\d+)/,
    EX, /if \(act === 'Escape' && pauseDoorReady\(\)\) \{ hudCtx\.togglePause\(\); return; \}/],
  ...
  ['bible/09-Testing/Testing.md', /keydown ladder \(exterior\.js:(\d+)-1942\)/,
    EX, /addEventListener\('keydown', \(e\) => \{/],

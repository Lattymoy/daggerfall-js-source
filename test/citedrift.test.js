...
//     (Port-Ledger.md:739)". Section A carried no widget row at all -
...
const SOURCE_CITES = [
  ...
  ['src/combat/weaponRig.js', /\(exterior\.js:(\d+), world\.js:2487\)/,
  ...
  ['src/systems/advancement.js', /exterior\.js:(\d+)\/:1529/, EX, /^ {4}onLevelUp: \(\) => \{$/],
  ['src/systems/advancement.js', /exterior\.js:937\/:(\d+)/, EX, /^ {4}onLevelUp: \(\) => \{$/],   // AUDIT WORLD6b-iii(a): the aim law's import moved the first; the whole cite re-aimed by hand (its world and worldModes halves were stale)
  ['src/systems/chargenSession.js', /exterior\.js:(\d+)\/:1136-1138/,
    EX, /from '\.\.\/systems\/chargenSession\.js'/],
  ['src/systems/equip.js', /world\.js:1944, exterior\.js:(\d+)\)/,
    EX, /if \(playerEntity\.chargenDone\) seedStartingEquipment\(playerEntity\);/],
  ['src/systems/loot.js', /world\.js:1958 and exterior\.js:(\d+)/,
  ...
  ['src/systems/startingGear.js', /world\.js:1944 and exterior\.js:(\d+) seed it/,
    EX, /if \(playerEntity\.chargenDone\) seedStartingEquipment\(playerEntity\);/],
  ...
  ['test/daychange.test.js', /exterior\.js:(\d+), world\.js:823/, EX, /playerTicker\.advance\(60\);/],
  ['test/overlayreentry.test.js', /exterior\.js:(\d+) and world\.js:2266/,
    EX, /if \(townTalk\.overlay\?\.isRestWindow\) townTalk\.closeOverlay\?\.\(\);/],
  ['test/overlayreentry.test.js', /exterior\.js:(\d+), world\.js:2266/,
    EX, /if \(townTalk\.overlay\?\.isRestWindow\) townTalk\.closeOverlay\?\.\(\);/],
  ['test/probehygiene.test.js', /keydown ladder, exterior\.js:(\d+)-2285/,
    EX, /addEventListener\('keydown', \(e\) => \{/],
  ['test/probehygiene.test.js', /exterior\.js:(\d+)-1165 and world\.js's copy/,
    EX, /if \(!playerEntity\.chargenDone && params\.has\('class'\)\) \{/],
  ['test/roade_up_seam.test.js', /exterior\.js:(\d+)\/:2378/,
    EX, /if \(act === 'Rest'\) \{ e\.preventDefault\(\); hudCtx\.toggleRest\(\); return; \}/],
  ['test/roade_up_seam.test.js', /exterior\.js:2370\/:(\d+)/,
    EX, /if \(act === 'Escape' && pauseDoorReady\(\)\) \{ hudCtx\.togglePause\(\); return; \}/],
  ['bible/01-Overview/Audit-58.md', /`src\/scenes\/exterior\.js:(\d+)` now/, EX, /setDefaultEnchantCtx/],
  ['bible/06-Systems/Systems-Arc.md', /`exterior\.js:(\d+)`, `world\.js:818`/, EX, /playerTicker\.advance\(60\);/],
  ['bible/09-Testing/Testing.md', /keydown ladder \(exterior\.js:(\d+)-2285\)/,
    EX, /addEventListener\('keydown', \(e\) => \{/],
  ...
  // range that cannot exist (`exterior.js:1223-1190`).
  ['bible/01-Overview/Port-Ledger.md', /`exterior\.js:(\d+)-1236` build `createDetectFeed`/,
    EX, /const detectFeed = createDetectFeed\(playerEntity, \{/],
  ['bible/01-Overview/Port-Ledger.md', /`exterior\.js:1226-(\d+)` build `createDetectFeed`/,
    EX, /^ {2}\}\);$/],
  ['bible/01-Overview/Port-Ledger.md', /`world\.js:1087`, `exterior\.js:(\d+)`/,
    EX, /const droppedLoot = createDroppedLoot\(/],
  ['bible/01-Overview/Port-Ledger.md', /createTownTalk passes no engine, `exterior\.js:(\d+)-999`/,
  ...
  ['bible/01-Overview/Port-Ledger.md', /`exterior\.js:(\d+)-3615` return on modal frames/,
    EX, /if \(modes\.frame\(dt, now\)\) \{/],
  ['bible/01-Overview/Port-Ledger.md', /`exterior\.js:3589-(\d+)` return on modal frames/,
    EX, /^ {4}\}$/],
...
// multi-number citation: `cityGuards.js:743-693`, `world.js:5587-5561`,

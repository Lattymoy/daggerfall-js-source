...
//     (Port-Ledger.md:738)". Section A carried no widget row at all -
...
const SOURCE_CITES = [
  ...
  ['src/systems/advancement.js', /exterior\.js:(\d+)\/:1533/, EX, /^ {4}onLevelUp: \(\) => \{$/],
  ['src/systems/advancement.js', /exterior\.js:932\/:(\d+)/, EX, /^ {4}onLevelUp: \(\) => \{$/],   // AUDIT WORLD6b-iii(a): the aim law's import moved the first; the whole cite re-aimed by hand (its world and worldModes halves were stale)
  ['src/systems/chargenSession.js', /exterior\.js:(\d+)\/:1167-1169/,
    EX, /from '\.\.\/systems\/chargenSession\.js'/],
  ['src/systems/equip.js', /world\.js:1939, exterior\.js:(\d+)\)/,
    EX, /if \(playerEntity\.chargenDone\) seedStartingEquipment\(playerEntity\);/],
  ['src/systems/loot.js', /world\.js:1953 and exterior\.js:(\d+)/,
  ...
  ['src/systems/startingGear.js', /world\.js:1939 and exterior\.js:(\d+) seed it/,
    EX, /if \(playerEntity\.chargenDone\) seedStartingEquipment\(playerEntity\);/],
  ...
  ['test/daychange.test.js', /exterior\.js:(\d+), world\.js:819/, EX, /playerTicker\.advance\(60\);/],
  ['test/overlayreentry.test.js', /exterior\.js:(\d+) and world\.js:2272/,
    EX, /if \(townTalk\.overlay\?\.isRestWindow\) townTalk\.closeOverlay\?\.\(\);/],
  ['test/overlayreentry.test.js', /exterior\.js:(\d+), world\.js:2272/,
    EX, /if \(townTalk\.overlay\?\.isRestWindow\) townTalk\.closeOverlay\?\.\(\);/],
  ['test/probehygiene.test.js', /keydown ladder, exterior\.js:(\d+)-2292/,
    EX, /addEventListener\('keydown', \(e\) => \{/],
  ['test/probehygiene.test.js', /exterior\.js:(\d+)-1160 and world\.js's copy/,
    EX, /if \(!playerEntity\.chargenDone && params\.has\('class'\)\) \{/],
  ['test/roade_up_seam.test.js', /exterior\.js:(\d+)\/:2385/,
    EX, /if \(act === 'Rest'\) \{ e\.preventDefault\(\); hudCtx\.toggleRest\(\); return; \}/],
  ['test/roade_up_seam.test.js', /exterior\.js:2377\/:(\d+)/,
    EX, /if \(act === 'Escape' && pauseDoorReady\(\)\) \{ hudCtx\.togglePause\(\); return; \}/],
  ['bible/01-Overview/Audit-58.md', /`src\/scenes\/exterior\.js:(\d+)` now/, EX, /setDefaultEnchantCtx/],
  ['bible/06-Systems/Systems-Arc.md', /`exterior\.js:(\d+)`, `world\.js:814`/, EX, /playerTicker\.advance\(60\);/],
  ['bible/09-Testing/Testing.md', /keydown ladder \(exterior\.js:(\d+)-2292\)/,
    EX, /addEventListener\('keydown', \(e\) => \{/],
  ['bible/10-UI/UI-Arc.md', /exterior\.js:(\d+)\. It is the only window/, EX, /createSpellbookWindow\(\{/],
  ['bible/10-UI/Settings-Screen-Spec.md', /`exterior\.js:(\d+)`, `dungeon\.js:835`/, EX, /^ {6}fieldOfView\(\),$/],
  ...
  // range that cannot exist (`exterior.js:1227-1185`).
  ['bible/01-Overview/Port-Ledger.md', /`exterior\.js:(\d+)-1240` build `createDetectFeed`/,
    EX, /const detectFeed = createDetectFeed\(playerEntity, \{/],
  ['bible/01-Overview/Port-Ledger.md', /`exterior\.js:1230-(\d+)` build `createDetectFeed`/,
    EX, /^ {2}\}\);$/],
  ['bible/01-Overview/Port-Ledger.md', /`world\.js:1087`, `exterior\.js:(\d+)`/,
    EX, /const droppedLoot = createDroppedLoot\(/],
  ['bible/01-Overview/Port-Ledger.md', /createTownTalk passes no engine, `exterior\.js:(\d+)-994`/,
  ...
  ['bible/01-Overview/Port-Ledger.md', /`exterior\.js:3596-(\d+)` return on modal frames/,
    EX, /^ {4}\}$/],
...
// multi-number citation: `cityGuards.js:743-693`, `world.js:5600-5574`,
// `worldModes.js:1162 against :1056`. Forty of them came out as ranges

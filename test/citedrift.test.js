...
//     (Port-Ledger.md:741)". Section A carried no widget row at all -
...
test('CD1: Ledger A row TB1 exists, in section A, and names the windows that cite it', () => {
  ...
  // doctrine.test.js:218 skips the file because it shouts no DEPARTURE
...
const SOURCE_CITES = [
  ...
  ['src/combat/weaponRig.js', /\(exterior\.js:(\d+), world\.js:2494\)/,
  ...
  ['src/systems/advancement.js', /exterior\.js:(\d+)\/:1534/, EX, /^ {4}onLevelUp: \(\) => \{$/],
  ['src/systems/advancement.js', /exterior\.js:942\/:(\d+)/, EX, /^ {4}onLevelUp: \(\) => \{$/],   // AUDIT WORLD6b-iii(a): the aim law's import moved the first; the whole cite re-aimed by hand (its world and worldModes halves were stale)
  ['src/systems/chargenSession.js', /exterior\.js:(\d+)\/:1141-1143/,
    EX, /from '\.\.\/systems\/chargenSession\.js'/],
  ['src/systems/equip.js', /world\.js:1951, exterior\.js:(\d+)\)/,
    EX, /if \(playerEntity\.chargenDone\) seedStartingEquipment\(playerEntity\);/],
  ['src/systems/loot.js', /world\.js:1965 and exterior\.js:(\d+)/,
  ...
  ['src/systems/startingGear.js', /world\.js:1951 and exterior\.js:(\d+) seed it/,
    EX, /if \(playerEntity\.chargenDone\) seedStartingEquipment\(playerEntity\);/],
  ...
  ['test/daychange.test.js', /exterior\.js:(\d+), world\.js:830/, EX, /playerTicker\.advance\(60\);/],
  ['test/overlayreentry.test.js', /exterior\.js:(\d+) and world\.js:2273/,
    EX, /if \(townTalk\.overlay\?\.isRestWindow\) townTalk\.closeOverlay\?\.\(\);/],
  ['test/overlayreentry.test.js', /exterior\.js:(\d+), world\.js:2273/,
    EX, /if \(townTalk\.overlay\?\.isRestWindow\) townTalk\.closeOverlay\?\.\(\);/],
  ['test/probehygiene.test.js', /keydown ladder, exterior\.js:(\d+)-2290/,
    EX, /addEventListener\('keydown', \(e\) => \{/],
  ['test/probehygiene.test.js', /exterior\.js:(\d+)-1170 and world\.js's copy/,
    EX, /if \(!playerEntity\.chargenDone && params\.has\('class'\)\) \{/],
  ['test/roade_up_seam.test.js', /exterior\.js:(\d+)\/:2383/,
    EX, /if \(act === 'Rest'\) \{ e\.preventDefault\(\); hudCtx\.toggleRest\(\); return; \}/],
  ['test/roade_up_seam.test.js', /exterior\.js:2375\/:(\d+)/,
    EX, /if \(act === 'Escape' && pauseDoorReady\(\)\) \{ hudCtx\.togglePause\(\); return; \}/],
  ['bible/01-Overview/Audit-58.md', /`src\/scenes\/exterior\.js:(\d+)` now/, EX, /setDefaultEnchantCtx/],
  ['bible/06-Systems/Systems-Arc.md', /`exterior\.js:(\d+)`, `world\.js:825`/, EX, /playerTicker\.advance\(60\);/],
  ['bible/09-Testing/Testing.md', /keydown ladder \(exterior\.js:(\d+)-2290\)/,
    EX, /addEventListener\('keydown', \(e\) => \{/],
  ['bible/10-UI/UI-Arc.md', /exterior\.js:(\d+)\. It is the only window/, EX, /createSpellbookWindow\(\{/],
  ['bible/10-UI/Settings-Screen-Spec.md', /`exterior\.js:(\d+)`, `dungeon\.js:834`/, EX, /^ {6}fieldOfView\(\),$/],
  ...
  ['bible/01-Overview/Port-Ledger.md', /`exterior\.js:(\d+)`, `dungeonContext\.js:1304`/,
    EX, /drinkPotion: \(key\) => magic\.drinkPotion\(key\)/],
  ['bible/01-Overview/Port-Ledger.md', /`world\.js:3809`, `exterior\.js:(\d+)`/,
    EX, /renderer\.setWindowEmission\(windowEmissionRGB\(/],
  ['bible/01-Overview/Port-Ledger.md', /`world\.js:638`, `exterior\.js:(\d+)` pass `getNameBankOfRegion`/,
  ...
  // range that cannot exist (`exterior.js:1228-1195`).
  ['bible/01-Overview/Port-Ledger.md', /`exterior\.js:(\d+)-1241` build `createDetectFeed`/,
    EX, /const detectFeed = createDetectFeed\(playerEntity, \{/],
  ['bible/01-Overview/Port-Ledger.md', /`exterior\.js:1231-(\d+)` build `createDetectFeed`/,
    EX, /^ {2}\}\);$/],
  ['bible/01-Overview/Port-Ledger.md', /`world\.js:1087`, `exterior\.js:(\d+)`/,
    EX, /const droppedLoot = createDroppedLoot\(/],
  ['bible/01-Overview/Port-Ledger.md', /createTownTalk passes no engine, `exterior\.js:(\d+)-1004`/,
  ...
  ['bible/01-Overview/Port-Ledger.md', /`exterior\.js:(\d+)-3620` return on modal frames/,
    EX, /if \(modes\.frame\(dt, now\)\) \{/],
  ['bible/01-Overview/Port-Ledger.md', /`exterior\.js:3594-(\d+)` return on modal frames/,
    EX, /^ {4}\}$/],
...
const NO_LINE_LEFT = [
  [/`world\.js:4100-4103` and its `exterior\.js` twin, both DELETED by FX1/, 'no loot'],
...
// multi-number citation: `cityGuards.js:743-693`, `world.js:5594-5568`,
// `worldModes.js:1150 against :1056`. Forty of them came out as ranges

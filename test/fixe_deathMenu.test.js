...
test('FIX-E: F11 reaches the world host’s quickload from UNDER the death screen, above the rung that eats every key', () => {
  ...
  const arm = w.indexOf("if (townTalk.overlayActive && !isTextEntryTarget(e.target) && (modes?.mode ?? 'exterior') === 'exterior' && actionForCode(bindings(), e.code) === 'QuickLoad' && !retroToggleKey(e, keys)) {\n      e.preventDefault();");
  ...
  assert.match(read('src/ui/input.js'), /if \(actionOf\(e, keys\) === 'QuickLoad' && !retroToggleKey\(e, keys\)\) \{ ctx\.quickLoad\?\.\(setPlayerPos\); return true; \}/);
});

...
test('ROADS 10: road corners are blurred, the rest of the terrain is untouched, and the switch is honoured', async () => {
  ...
  // sample(x, y) = s[x * hDim + y] (terrainSampler.js:155, DFU's
  ...
  // loop is the mirrored east-west strip the port's old base blurred.
...
test('ROADS 25: a pixel says whether a network was present, and the host rebuilds the ones painted without one', async () => {
  ...
  // SPAWN-ROADS: the sweep's first act takes back crossed provisional ruins, one line more before the rebuild
  assert.match(world, /function sweepRoadless\(\) \{[\s\S]{0,520}for \(const k of again\) destroyPixel\(k\.px, k\.py, \{ collectLoose: false \}\);\s*\n\s*queue\.push\(\.\.\.again\.sort\(/);

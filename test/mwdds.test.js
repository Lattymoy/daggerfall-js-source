...
test('mwdds: the committed fixture.dds decodes to its four quadrants', () => {
  // The asset mesh.nif references and the viewer renders: 8x8 DXT1,
  // solid red/green/blue/white 4x4 quadrants, generated by
  // test/fixtures/mw/generate.py with hand-packed blocks.
  ...
  const m = img.mips[0];
  assert.deepEqual(px(m, 0, 0), [255, 0, 0, 255]);
  assert.deepEqual(px(m, 3, 3), [255, 0, 0, 255]);
  assert.deepEqual(px(m, 4, 0), [0, 255, 0, 255]);
  assert.deepEqual(px(m, 0, 4), [0, 0, 255, 255]);
  assert.deepEqual(px(m, 7, 7), [255, 255, 255, 255]);
});

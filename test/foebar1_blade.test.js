...
test('FOEBAR1: the two pictures are one crop, the pref defaults to the plain bar, and the HUD, the sheet and the menu carry the face', () => {
  const empty = pngSize('public/hud/foe-blade-empty.png'), full = pngSize('public/hud/foe-blade-full.png');
  ...
  assert.match(css, /background: url\(\.\/hud\/foe-blade-empty\.png\) center \/ 100% 100% no-repeat; \}\s*\n\.hud-bladefull \{ background-image: url\(\.\/hud\/foe-blade-full\.png\); \}/, 'page-relative, as ./skin/ is');

...
test('FD1: both skins open on the enhanced door; classic collapses its game doors into BEGIN, which resolves into the classic sequence with the data gated first (mutant: Begin on the enhanced rail, or the classic rail keeping New Game)', () => {
  ...
  assert.match(menu, /const SECTIONS_CLASSIC = \['Begin', 'Online', 'Settings', 'Features', 'Overhauls', 'About'\];/);   // OVH1: the three looks   // FT16: Controls is a Settings category
  assert.match(menu, /const SECTIONS_BOOT = \['Continue', 'New Game', 'Load Game', 'Online', 'Test Room', 'Settings', 'Features', 'Overhauls', 'About'\];/, 'the enhanced rail keeps its three doors and loses the Enhanced entry (SO1) and the Controls one (FT16)');

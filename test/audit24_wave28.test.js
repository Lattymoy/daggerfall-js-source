...
test('audit24 wave28: the to-hit armour term has NO scalar fallback', () => {
  ...
  assert.match(f, /chance \+= \(target\.armorValues\?\.\[struckBodyPart\] \?\? 0\) \+ entityArmorMod\(target, struckBodyPart\);/);   // E1 widened the term with the enchantment channel, RF1 folded the port's onto it (one read); the NO-SCALAR law stands

...
test('MWBODY1: the host - the gate is the enhanced skin (read once), the arms switch and Morrowind data; the bodies sync before the dolls, nearest first, and hand the dolls their height; the draw hook calls the bodies and the mode machine gets it; the bodies draw right after the player\'s own in the exterior pass and in both modal passes; the run bit rides the pose; the feet follow the origin; the dead stand no body', () => {
  ...
  assert.match(w, /const drawable = online\.drawable\(\);\s*\n\s*peerCastVisuals\(drawable\);[^\n]*\n\s*peerBodies\.sync\(drawable, onlineToScene, dt, player\.pos, \{ priority: \(id\) => !!social\?\.isPartyPeer\(id\) \}\);[^\n]*\n\s*remotePlayers\.sync\(drawable, onlineToScene, \{ bodyHeight: \(id\) => peerBodies\.heightOf\(id\), dt, eye: player\.pos \}\);/, 'the bodies sync before the dolls (SPELLFX1: a peer\'s new cast is read off the same drawable list first)');
  ...
  assert.match(w, /'pagehide', \(\) => \{\s*try \{ worldPublish\(performance\.now\(\), true\); \}\s*catch \(e\) \{[^\n]*\}\s*online\?\.leave\(\);\s*for \(const link of chatLinks\?\.values\(\) \?\? \[\]\) link\.leave\(\);\s*peerBodies\?\.destroy\(\);\s*remotePlayers\?\.destroy\(\);\s*\}/, 'the page\'s hide releases the rigs (and, CHAT1, leaves every channel in the same goodbye - the panel stays for a restore, AUDIT CHAT B4)');

...
test('MWBODY1: the host - the gate is the enhanced skin (read once) and Morrowind data (MWA4: the files are the arms\' switch); the bodies sync before the dolls, nearest first, and hand the dolls their height; the draw hook calls the bodies and the mode machine gets it; the bodies draw right after the player\'s own in the exterior pass and in both modal passes; the run bit rides the pose; the feet follow the origin; the dead stand no body', () => {
  const w = rd('src/scenes/world.js');
  assert.match(w, /const enhanced = isEnhanced\(\);[^\n]*\n\s*peerBodies = new PeerBodies\(\{ renderer, enabled: \(\) => enhanced && morrowindDataCount\(\) > 0, generation: morrowindDataGeneration \}\);/, 'the enhanced skin, the data, and its generation');
  assert.match(w, /const drawable = online\.drawable\(\);\s*\n(?:\s*peerCandlesFrame\(drawable, dt\);[^\n]*\n)?\s*peerCastVisuals\(drawable\);[^\n]*\n(?:\s*\/\/[^\n]*\n)*\s*peerRiders\.sync\(drawable,[^\n]*\n\s*const afoot = drawable\.filter\(\(d\) => !peerRiders\.isRiding\(d\.id\) && !d\.shown\?\.wb\);[^\n]*\n\s*peerBodies\.sync\(afoot, onlineToScene, dt, player\.pos, \{ priority: \(id\) => !!social\?\.isPartyPeer\(id\) \}\);[^\n]*\n(?:(?!\s*peerBodies|\s*remotePlayers\.sync)[^\n]*\n){0,40}?\s*remotePlayers\.sync\(drawable, onlineToScene, \{ bodyHeight: \(id\) => peerRiders\.heightOf\(id\) \|\| peerBodies\.heightOf\(id\) \|\| peerWalkers\.heightOf\(id\), dt, eye: player\.pos, poseAgeMs: \(p\) => online\.poseAgeMs\(p\) \}\);/, 'the bodies sync before the dolls - PCORPSE1/3\'s fallen bodies are settled between the two (PEERLIGHT2: the others\' Light-spell candles, then SPELLFX1: a peer\'s new cast is read off the same drawable list first; RIDE: a rider is drawn next and the bodies stand the peers afoot, the doll\'s height the rider\'s, else the body\'s)');
  assert.match(w, /const drawPeerBodies = \(proj, view, eye\) => \{ if \(peerBodies\) peerBodies\.draw\(canvas, \{ proj, view, eye, flashOf: peerFlashOf \}\); peerWalkers\?\.drawLanterns\(\); \};/, 'the hook draws the bodies (HITFLASH1: each with its hurt flash - hitflash1.test.js; HT-WAIST-BACK: and, beside them, the walkers\' lanterns - htwaistback.test.js)');
  assert.match(w, /drawPeerBodies: \(\{ proj, view, eye \}\) => drawPeerBodies\(proj, view, eye\),/, 'the mode machine gets the hook');
  assert.match(w, /mwViewDrawBody\(canvas, \{ proj, view, eye: mwv\.eye, feet: player\.bodyFeetAt\(\), yaw: cam\.yaw \}\);[^\n]*\n\s*drawPeerBodies\(proj, view, mwv\.eye\);/, 'after the player\'s own body, the same pass (DISC18: at the body\'s own feet)');
  assert.match(w, /onlineFrame\(now, dt\)/, 'the frame\'s dt steps the bodies');
  assert.match(w, /const mv = !moved \? 0 : riding \? \(chooseTable\(\{ riding: true, stopped: false, moveSpeed: tableMoveSpeed\(player\.moveForward, player\.moveStrafe, player\.moveSpeed\) \}\) === 'GallopHorse' \? 2 : 1\) : \(player\.isRunning \? 2 : 1\);/, 'the run bit (AUDIT RIDE: in the saddle, the gallop the rider\'s own sprite shows)'); assert.match(w, /online\.sendPose\(\{ \.\.\.pose, \.\.\.arm, \.\.\.poseFx\(\) \}\)/);
  assert.match(w, /online\.setLook\(composeLook\(playerEntity\)\); online\.join\(key, \{ \.\.\.pose, \.\.\.arm \}\);/, 'the next room\'s hello carries the gear worn now (PROFILE2: through setLook - and a room already open is told too)');
  assert.match(w, /if \(peerBodies\) peerBodies\.offsetAll\(r\.offset\);/, 'the recenter shifts the bodies (D5)');
  assert.match(w, /instanceof DeathScreen \|\| modes\?\.deathUp\?\.\(\)\) \{\s*\n\s*if \(_deathWasOnline == null\)[^\n]*\n(?:(?!\s*if \(online\.room\) \{ worldPublish)[^\n]*\n)*?\s*if \(online\.room\) \{ worldPublish\(now, true\); online\.leave\(\); exteriorFoes\.clearPuppets\(\); _foesRoom = null; \}\s*\n(?:\s*(?:\/\/|if \(!_respawning\) \{|if \(!_rezSeen\)|const rez = |if \(rez\)|\})[^\n]*\n)*\s*peerBodies\.destroy\(\); remotePlayers\.sync\(\[\], onlineToScene\); peerRiders\?\.destroy\(\); peerWalkers\?\.destroy\(\); peerCandlesFrame\(\[\], dt\); return;[^\n]*\n\s*\}/, 'the dead stand no body, no doll, no rider and no walker (DISC23-B), and no candle of anyone\'s hangs over them (PEERLIGHT2) - AUDIT RIDE (D-ONLINE1 split the line: the online snapshot is taken first)');
  assert.match(w, /'pagehide', \(\) => \{\s*try \{ worldPublish\(performance\.now\(\), true\); \}\s*catch \(e\) \{[^\n]*\}\s*online\?\.leave\(\);\s*for \(const link of chatLinks\?\.values\(\) \?\? \[\]\) link\.leave\(\);\s*peerBodies\?\.destroy\(\);\s*remotePlayers\?\.destroy\(\);\s*peerRiders\?\.destroy\(\);[^\n]*\s*peerWalkers\?\.destroy\(\);[^\n]*\s*\}/, 'the page\'s hide releases the rigs (RIDE: and the riders\' sprites; DISC23-B: and the walkers\') (and, CHAT1, leaves every channel in the same goodbye - the panel stays for a restore, AUDIT CHAT B4)');
  const m = rd('src/scenes/worldModes.js');
  assert.equal((m.match(/mwViewDrawBody\(canvas, \{ proj, view, eye: mwv\.eye, feet: player\.bodyFeetAt\(\), yaw: cam\.yaw \}\);   \/\/ MW-D24[^\n]*\n\s*host\.drawPeerBodies\?\.\(\{ proj, view, eye: mwv\.eye \}\);/g) || []).length, 2, 'the dungeon\'s and the interior\'s passes');

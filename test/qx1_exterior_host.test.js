...
const QW_PARAMS = [
  ...
  // own `getClassicSpellEffects` (world.js:8873's seam).
  'spellRecordOfIndex',
];
...
test('QX1 review: every faction read is the PERSISTENT store, and the Person chain\'s faction-type family is mounted', () => {
  ...
  // expressions are world.js:8927/8929's verbatim, and talk.js's
...
test('ROAD-G G2 review: the cast engine raises the two ready-spell doors into THIS host\'s machine', () => {
  ...
  // shop entered from it - was permanently deaf. world.js:4441-4442 and
  // dungeonContext.js:2282-2283 wire the identical pair.
...
test('ROAD-G G2 review: questWorld answers CastSpellDo\'s two classic-spell reads', () => {
  ...
  // dead on this route even with the doors above wired. world.js:8873's
...
test('ROAD-G G2 review: the encounter pool\'s frame seams - the tick, the draw, the one senses context, the enemy-arrow target', () => {
  ...
  // world.js:15196-15268's arrow shape: an enemy shaft hunts a WALKING
...
test('QX1: the journal doors U43 left hanging finally have something to open', () => {
  ...
  assert.match(SRC, /if \(act === 'LogBook'\) \{ e\.preventDefault\(\); hudCtx\.toggleLogbook\(\); return true; \}/);
  assert.match(SRC, /if \(act === 'NoteBook'\) \{ e\.preventDefault\(\); hudCtx\.toggleNotebook\(\); return true; \}/);

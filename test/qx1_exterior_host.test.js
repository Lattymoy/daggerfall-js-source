...
const QW_PARAMS = [
  ...
  // own `getClassicSpellEffects` (world.js:5651's seam).
  'spellRecordOfIndex',
];
...
test('QX1 review: every faction read is the PERSISTENT store, and the Person chain\'s faction-type family is mounted', () => {
  ...
  // expressions are world.js:5702/5704's verbatim, and talk.js's
...
test('ROAD-G G2 review: the cast engine raises the two ready-spell doors into THIS host\'s machine', () => {
  ...
  // (actions.js:2697 - C# subscribes them in its constructor). This
  ...
  // shop entered from it - was permanently deaf. world.js:2569-2570 and
  // dungeonContext.js:1983-1984 wire the identical pair.
...
test('ROAD-G G2 review: questWorld answers CastSpellDo\'s two classic-spell reads', () => {
  ...
  // (actions.js:2751/:2758 - no effects, so C#'s template completes and
  // the task can never fire), which would have left `cast X spell do`
  // dead on this route even with the doors above wired. world.js:5651's
...
test('ROAD-G G2 review: the encounter pool\'s frame seams - the tick, the draw, the one senses context, the enemy-arrow target', () => {
  ...
  // world.js:8484-8538's arrow shape: an enemy shaft hunts a WALKING

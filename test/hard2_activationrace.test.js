...
test('HARD2: both exterior hosts ask the ONE race and hand-roll none of it, and no other host grew a second copy', () => {
  ...
  for (const host of HOSTS) {
    ...
    // WB2: and the world host one more, the Oblivion Gate (online alone - the ?exterior host stands none)
    const gateRow = host === 'src/scenes/world.js' ? String.raw`\s*gate: _gatePick,[^\n]*\n` : '';
    assert.match(src, new RegExp(String.raw`const _race = raceActivation\(\{\s*\n\s*corpse: _corpsePick,\s*\n\s*pile: _pilePick,\s*\n\s*torch: _torchPick,\s*\n\s*wagon: _wagonPick,\s*\n\s*horseCart: _hccPick,[^\n]*\n\s*camp: _campPick,[^\n]*\n\s*water: _springPick,[^\n]*\n` + gateRow + String.raw`\s*doorDistance: modes\.exteriorActivationDistance\(cam\.pos, useFwd\),\s*\n\s*personDistances: _livePersons\.map\(\(p\) => rayPersonDistance\(cam\.pos, useFwd, p\.pos\)\),\s*\n\s*\}\);`), `${host}: and feeds it every family the ray can strike`);

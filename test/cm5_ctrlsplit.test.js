...
test('CM5: held Control forces the quantity popup even when the whole stack fits', () => {
  ...
  assert.ok(w.inputBox, 'a whole-stack move is intercepted while Control is held');
  assert.equal(w.inputBox.label, HOW_MANY_ITEMS(5));
  assert.equal(w.inputBox.maxCharacters, SPLIT_INPUT_MAX);
  assert.equal(w.inputBox.numeric, true);
  assert.equal(w.inputBox.value, '0', 'DFU seeds a Control-forced field with "0"');

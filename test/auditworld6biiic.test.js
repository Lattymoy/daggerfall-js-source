...
test('AUDIT WORLD6b-iii(c) C3: the Room - the hit arm counts BYTES (HIT_ROOM_BYTES_PER_S a second, the room\'s) - over the budget a frame is dropped and nobody struck; inside it a grant lands; the relay says world69', async () => {
  assert.equal(HIT_ROOM_BYTES_PER_S, 256 * 1024); assert.equal(RELAY_VERSION, 'world69');

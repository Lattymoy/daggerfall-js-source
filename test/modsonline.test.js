...
test('MODS-ONLINE-4: every mod switch is the player\'s online, except the thirteen the room owns', () => {
  ...
  assert.equal(forced, 13, 'the lane forces thirteen mod switches in the whole shelf');   // RRI1/RR1 (merged 2026-09-23): five of Roleplay & Realism: Items' (the items that change hands) and two of Roleplay & Realism's (the location, the host's foes)   // WOD1: World of Daggerfall's Enabled, the second floor
...
test('MODS-ONLINE-2: the lock, the pane and the door all say the same true thing', () => {
  ...
  assert.match(menu, /if \(ground !== undefined\) lockOnline\(b, null, \{ note: ONLINE_GROUND_VENDORS\.includes\(vendor\) \? ONLINE_GROUND_NOTE : ONLINE_SHARED_NOTE, value: ground \}\);/);

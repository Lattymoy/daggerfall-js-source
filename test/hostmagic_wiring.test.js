...
test('M5: spell damage reaches guards through the ONE damage door', () => {
  ...
  for (const f of HOSTS) {
    assert.ok(/hurt: \(n(?:, o)?\) => \{ (?:const fp = o\?\.fromPlayer \?\? fromPlayer; )?if \(n > 0\) (cityGuards\.hurtGuard\(g, n, player\.pos\)|\(g\._encounter \? exteriorFoes\.damageFoe\(g, n, player\.pos, null, \{ fromPlayer(?:: fp)?, kind: 'spell'(?:, whole: !!o\?\.whole)? \}\) : cityGuards\.hurtGuard\(g, n, player\.pos, null, \{ fromPlayer(?:: fp)? \}\)\))/.test(src(f)),   // AUDIT WORLD6b-iii(a) B2: the world host's carries the engine's provenance

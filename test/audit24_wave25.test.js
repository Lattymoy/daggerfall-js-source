...
test('audit24 wave25: the factionListener comments name PlayerActivate, and claim nothing pending', () => {
  ...
  assert.match(m, /src\/scenes\/worldModes\.js:478/);   // AUDIT WORLD6b-iii(a) C3 by one more (the aim law's import)   // FIX-F/E moved worldModes by eight lines; AUDIT 65 by one more; PERF5 by one more (the identity constant under the imports); UL1 by one more (the Unleveled Loot import); WORLD6a by one more (the interiorShared import)

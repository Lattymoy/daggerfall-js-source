...
test('FT9: every vendored mod with a switch has exactly one row over it, and every mod row is a real mod\'s - both ways', () => {
  ...
  assert.equal(rows.length, 6, 'six direct rows (Dynamic Skies rides the outdoors row through `also`; WW1 added Weapon Widget)');
  assert.equal(filterFeatures(FEATURES, 'mod').length, 7);
});
...
test('FT9: a mod row is the mod\'s own - the creator in the title, the modsettings description as the note, one source', () => {
  ...
  assert.deepEqual(rows.map((f) => f.id), ['mod-seasons-iliac-bay', 'mod-roads-hazelnut', 'mod-meanermonsters', 'mod-pcaao', 'mod-unleveledloot', 'mod-weapon-widget'], 'the Mods pane\'s order');
});

...
test('U60: walkTravelPath is exactly the calculator\'s pixel sequence', () => {
...
test('U60: the height law is byte*8 floored at the ocean, through one documented relief', () => {
...
test('U60: both port water tests, OR-ed - and the swamp-green ocean trap is closed', () => {
...
test('U60: the grid puts a vertex on every pixel CENTER with north at +z', () => {
...
test('U60: marker buckets ARE getPixelColorIndex - one law, both skins', () => {
...
test('U60: the discovery law gates every marker, and position is the pixel center', () => {
  const noFilters = { dungeons: false, temples: false, homes: false, towns: false };
...
test('U60: the dot tables carry exactly the fourteen classic slots', () => {
...
test('U60: the route line anchors on the start pixel the time law never charges', () => {
...
test('U60: the classic skin still gets the canvas map - or its honest null without art', () => {
  skin('classic');
  withDocument(() => {
...
test('U60: the fork asks the SKIN, not only the document', () => {
  assert.match(read('src/ui/travelMapDoor.js'),
    /if \(isEnhanced\(\) && typeof document !== 'undefined'\) \{/,
    'both clauses, in that order');
});

test('U60: a host with no document keeps the classic arm, on either skin', () => {
  skin('enhanced');
...
test('U60: the door needs classic art only where the classic map draws it', () => {
  skin('classic');
...
test('U60: the enhanced skin gets the overworld, holding the classic contract', () => {
  skin('enhanced');
  withDocument((doc) => {
...
test('U60: the world host builds through the door, once, and gates on it', () => {
...
test('U60: the other three hosts still refuse the map, by name', () => {
...
test('U60: the door is a STATIC fork and says why', () => {
...
test('U60: the filters are the LIVE store object, edited in place', () => {
  withDocument(() => {
    const win = mkWin();
...
test('U60: the save envelope is travelMapState\'s own, live panel winning', () => {
  withDocument(() => {
    const win = mkWin();
...
test('U60: the trip on the panel is the law\'s own answer, live per toggle', () => {
  withDocument(() => {
...
test('U60: disease speaks BEFORE gold, the gate is two-sided, and the commit is shaped', () => {
  withDocument(() => {
    let sick = 1;
...
test('U60: Begin mints the classic pick/opts/computed shapes and flies; teleport skips the flight', () => {
  withDocument(() => {
    const traveled = [];
    const ported = [];
...
test('U60: gotoPlace is a one-shot consumed on the first tick', () => {
  withDocument(() => {
    const win = mkWin();
...
test('U60: onClose is owed on every close, once', () => {
  withDocument(() => {
    let closed = 0;
...
test('U60: the window computes no travel law of its own', () => {
...
test('U60: the model buckets through the classic window, not a copied table', () => {
...
test('U60: the no-op host arms say why they are empty', () => {
...
test('U60: the overworld pass restores what it touches', () => {
...
test('U60: the veil phases never open a beginFrame - the host\'s live frame is the world below the clouds', () => {
  const src = read('src/ui/overworldMap.js');

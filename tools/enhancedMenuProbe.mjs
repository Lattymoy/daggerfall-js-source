...
async function run(label, opts) {
  ...
  const pane = await page.evaluate(() => {
    ...
    const sky = find('Enhanced environments');
    return {
  ...
  check(`${label}: enhanced environments read ON by default`, pane.sky === 'On', String(pane.sky));
  if (label === 'phone') check("phone: the environments switch is a thumb's target", pane.skyTarget >= 38, `${pane.skyTarget}px`);
  ...
  const skyOff = await page.evaluate(async () => {
    ...
    m._resetForTests();
    return m.getPref('enhancedEnvironments');
  });
  check(`${label}: the environments switch PERSISTS`, skyOff === false, String(skyOff));
  await page.locator('#enhanced-menu .row', { hasText: 'Enhanced environments' }).locator('.ctl .act').click();

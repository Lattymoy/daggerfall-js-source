...
const R = {
  ...
  uploadTexture(archive, record, c32) {
    ...
    gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA, c32.width, c32.height, 0, gl.RGBA, gl.UNSIGNED_BYTE, c32.colors);   // TR6: the pipeline's shape
...
window.__setup = async (archive, standIns, layout) => {
  ...
  for (const [rec, s] of Object.entries(standIns)) {
    ...
    const color32 = { width: img.width, height: img.height, colors: rgba.data };   // TR6: getColor32's shape - the probe's stand-in had worn `.data`, so it never caught the game's mismatch
    ...
    const tb = trees.build('probe', archive, Number(rec), m, centers, bbH, bm, { color32, upload: (k, raster) => R.uploadTexture(archive, k, raster) });   // TR6: one stand-in pixel
    if (tb) built++;
  }
  return { built, batches: trees.batches.size };
};
...
window.__frame = (t, eye, at, windV) => {
  ...
  R._camPos.set(eye);
  // TR6: the host hands each batch its pixel's translation; the probe's one pixel stands at the origin
  const batches = [...trees.batches.values()];
  for (const b of batches) { b.origin = [0, 0, 0]; b.gustOrigin = [0, 0]; }
  trees.draw(R, proj, view, t, { windV }, [1, 1, 1], batches);
  return trees.count;
};
window.__ready = true;
</script></body></html>`;

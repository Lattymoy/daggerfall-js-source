...
def _subjectMask(rgb, m):
    """Lit pixels of whatever hue this subject actually IS - skin, fur or scale.
    A "warm skin" mask assumes pink or brown: Argonian scale is RGB (38,58,37),
    green-dominant, so R-B>34 fired ZERO times and both the rotation detector
    and the recentre had nothing to measure."""
    rgb=np.asarray(rgb,dtype=float)
    lum=0.30*rgb[:,:,0]+0.59*rgb[:,:,1]+0.11*rgb[:,:,2]
    if not m.any(): return m
    ref=np.percentile(lum[m],70)
    mx=rgb.max(axis=2); mn=rgb.min(axis=2)
    chroma=(mx-mn)/np.maximum(mx,1.0)
    med=np.array([np.median(rgb[:,:,k][m]) for k in range(3)])
    dom=int(np.argmax(med))
    return m & (lum>ref*0.55) & (rgb[:,:,dom] >= mx-2) & (chroma>0.06)
...
import os as _os
_sp=_json.load(open('spans_kh.json'))[str(FACE-1)]
...
V={}
for Y in YAWS:
    im=Image.open(f'heads_kh/f{FACE}_{SRC[Y]:03d}.png').convert('RGBA'); A=np.array(im)
    m=A[:,:,3]>0; H,W=m.shape
...
_w=_subjectMask(_b.astype(float), np.ones(_b.shape[:2],bool))
...
Image.fromarray(cell).save(f'heads_kh/cell_{FACE}.png')
print(f'baked head cell {HW}x{HH} -> heads_kh/cell_{FACE}.png')

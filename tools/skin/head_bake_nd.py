...
import os as _os
_sp=_json.load(open('spans_nd.json'))[str(FACE-1)]
...
V={}
for Y in YAWS:
    im=Image.open(f'heads_nd/f{FACE}_{SRC[Y]:03d}.png').convert('RGBA'); A=np.array(im)
    m=A[:,:,3]>0; H,W=m.shape
    ...
    for r in range(top, sh+1):
        ...
        _d=np.diff(xs); _brk=np.where(_d>GAP)[0]
        _segs=np.split(xs,_brk+1)
        _big=max(len(t) for t in _segs)
        _keep=[t for t in _segs if len(t) >= _big*0.25]
        runs[r]=(int(_keep[0][0]), int(_keep[-1][-1]))
    ...
    def smooth(a, deg=4):
        n=len(a)
        if n < deg+3: return np.asarray(a,dtype=float)
        x=np.arange(n,dtype=float)
        try: return np.polyval(np.polyfit(x,np.asarray(a,dtype=float),deg), x)
        except Exception: return np.asarray(a,dtype=float)
...
def sample(px,py,pz,arc):
    ...
    for Yd in YAWS:
        ...
        # map across the SMOOTHED run, not the raw silhouette edges. Switching to
        # arc-linear dropped the smoothed profile by accident, putting the ragged
        # outline straight back into the map: Redguard edges jitter 3.03px per
        # row against Breton's 1.46 (max 133 vs 83), and every jump displaces
        # that whole row - which is the banding, and why garment busts were worse
        # than hair busts.
        e0, e1 = sc - sr, sc + sr
        col=int(round(e0+rel*(e1-e0)))
...
Image.fromarray(cell).save(f'heads_nd/cell_{FACE}.png')
print(f'baked head cell {HW}x{HH} -> heads_nd/cell_{FACE}.png')

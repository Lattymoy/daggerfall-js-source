#!/bin/bash
# The round-two branch (a65/r2) into HEAD (= main): conflicts, citeMerge by provenance, the
...
OURS=$(git rev-parse HEAD); THEIRS=$(git rev-parse a65/r2)
echo "ours $OURS theirs $THEIRS"; git log --oneline $OURS..a65/r2 | cat | tail -3
git merge --no-commit a65/r2 2>&1 | grep -c CONFLICT || true
python3 - <<'PY'
...
for f in files:
    ...
    def keep(m):
        ...
        if all(o in tl for o in ol):
            no+=1; return theirs   # theirs (r2) is ours-plus-additions: take it
        bad.append((f,ours[:300],theirs[:300])); return m.group(0)
    s2=re.sub(r'<<<<<<< HEAD\n(.*?)=======\n(.*?)>>>>>>> a65/r2\n', keep, s, flags=re.S)
    open(f,'w').write(s2)
    if '<<<<<<<' not in s2: subprocess.run(['git','add',f],check=True)
print(f'resolved {len(files)} files: {nt} numeric hunks to r2, {no} text hunks to r2 (superset)')
if bad:
    for f,o,t in bad: print('UNRESOLVED',f); print('  ours:',o); print('  theirs:',t)
    sys.exit(3)
PY
node tools/citeMerge.mjs $OURS a65/r2 --apply > $S/citeMerge-report-$THEIRS.txt 2>&1 || true
...
tm=re.search(r'dungeon\\\.js:(\d+) and worldModes\\\.js',T)
if tm and int(tm.group(1))!=dn: open('test/citedrift.test.js','w').write(T.replace(f'/dungeon\\.js:{tm.group(1)} and worldModes',f'/dungeon\\.js:{dn} and worldModes')); print('CD4 literal',tm.group(1),'->',dn)
...
echo "=== full suite"; [ "$1" = --no-full ] && exit 0; npm test > $S/fullsuite-$THEIRS.txt 2>&1 && echo "FULL SUITE GREEN" || { echo "FULL SUITE RED"; grep "^not ok" $S/fullsuite-$THEIRS.txt | head -5; }
